...
dnl #################################################################
dnl
dnl PMIX_CHECK_INLINE_GCC([action-if-found], [action-if-not-found])
dnl
...
dnl
dnl DEFINE PMIX_C_GCC_INLINE_ASSEMBLY to 0 or 1 depending on GCC
dnl                support
dnl
dnl #################################################################
AC_DEFUN([PMIX_CHECK_INLINE_C_GCC],[
    AC_CACHE_CHECK([if $CC supports GCC inline assembly],
       [pmix_cv_asm_gcc_inline_assembly],
       [PMIX_VAR_SCOPE_PUSH([asm_result pmix_gcc_inline_assign PMIX_C_GCC_INLINE_ASSEMBLY])
        ...
        AS_IF([test  "$pmix_gcc_inline_assign" != ""],
            [AC_RUN_IFELSE([AC_LANG_PROGRAM([AC_INCLUDES_DEFAULT],[[
int ret = 1;
int negone = -1;
__asm__ __volatile__ ($pmix_gcc_inline_assign);
return ret;
                ]])],
...
return ret;
            ...
            [asm_result="yes"], [asm_result="no"])])
    pmix_cv_asm_gcc_inline_assembly="$asm_result"
    PMIX_VAR_SCOPE_POP])

    AS_IF([test "$pmix_cv_asm_gcc_inline_assembly" = "yes"],
...
dnl   - PMIX_C_GCC_INLINE_ASSEMBLY - 1 if C compiler supports
dnl         GCC-style inline assembly
dnl   - PMIX_USE_C11_ATOMICS - 1 if atomics implementation should
dnl         use C11-style atomics
dnl   - PMIX_USE_GCC_BUILTIN_ATOMICS - 1 if atomics implementation
...
AC_DEFUN([PMIX_CONFIG_ASM],[
    ...
    PMIX_VAR_SCOPE_PUSH([atomics_found want_c11_atomics want_gcc_builtin_atomics])
    ...
    PMIX_CHECK_INLINE_C_GCC([gcc_inline=1], [gcc_inline=0])
    ...
    AC_ARG_ENABLE([c11-atomics],
        [AS_HELP_STRING([--enable-c11-atomics],
             [Enable use of C11 atomics if available. Note: GCC builtin atomics are currently preferred over C11 atomics. (default: use if available)])])

    AC_ARG_ENABLE([builtin-atomics],
        [AS_HELP_STRING([--enable-builtin-atomics],
             [Enable use of GCC built-in atomics. Currently preferred over C11 atomics. (default: use if available)])])
    ...
    AS_IF([test "$atomics_found" = "no" -a "$enable_builtin_atomics" != "no"],
          [PMIX_CHECK_GCC_ATOMIC_BUILTINS
           AS_IF([test $pmix_cv_have___atomic = "yes"],
    ...
    AS_IF([test "$atomics_found" = "no" -a "$enable_c11_atomics" != "no" -a "$pmix_cv_c11_supported" = "yes" -a "$pmix_cv_c_compiler_vendor" != "intel"],
