                            ...
                            pmix_event_notification_cbfunc_fn_t cbfunc, void *cbdata)
{
    ...
    if ((info[0].value.data.proc != NULL)
        ...
        for (i = 0; i < ninfo; i++) {
            fprintf(stderr,
                    "%s Client %s:%d NOTIFIED with status %d and error proc %s:%d key %s \n", name,
                    myproc.nspace, myproc.rank, status, info[i].value.data.proc->nspace,
                    info[i].value.data.proc->rank, info[i].key);
        }
        completed = true;
        if (NULL != cbfunc) {
