...
# Minimum versions
my $prte_automake_version = "1.13.4";
my $prte_autoconf_version = "2.69";
my $prte_libtool_version = "2.4.2";

# Search paths
my $prte_autoconf_search = "autoconf";
my $prte_automake_search = "automake";
my $prte_libtoolize_search = "libtoolize;glibtoolize";
...
sub ignored {
    ...
    # If this directory does not have .prte_ignore, or if it has a
    # .prte_unignore that has my username in it, then add it to the
    # list of components.
    my $ignored = 0;

    if (-f "$dir/.prte_ignore") {
        $ignored = 1;
    }
    if (-f "$dir/.prte_unignore") {
        open(UNIGNORE, "$dir/.prte_unignore") ||
            my_die "Can't open $dir/.prte_unignore file";
        my $unignore;
        $unignore .= $_
            while (<UNIGNORE>);
        close(UNIGNORE);
    ...
    if (exists($exclude_list->{$framework}) &&
        ...
        if (-d $dir) {
            ...
            foreach my $d (sort(readdir(DIR))) {
                ...
                # Check ignore status
                if (ignored("$dir/$d")) {
                    verbose "    => Ignored (found .prte_ignore file)\n";
                } else {
...
PRTE_EXPORT extern char *prte_framework_names[];
    ...
    open(FRAMEWORKS_OUT, ">src/include/prte_frameworks.c");
    printf FRAMEWORKS_OUT "%s", "/*
 * This file is autogenerated by autogen.pl. Do not edit this file by hand.
 */
...
m4_define([mca_prte_framework_list], [$frameworks_comma])
...
";
    ...
    foreach my $f (@mykeys) {
        ...
        $m4 .= "dnl Components in the prte / $f framework
m4_define([mca_prte_${f}_m4_config_component_list], [$m4_config_component_list])
m4_define([mca_prte_${f}_no_config_component_list], [$no_config_component_list])
...
tools:
    ...
    GNU Autoconf: $prte_autoconf_version
    GNU Automake: $prte_automake_version
    GNU Libtool: $prte_libtool_version
=================================================================\n";
    my_exit(1);
}
...
sub patch_autotools_output {
    ...
    my $replace_string = 'darwin*)
      # PRRTE patched for Darwin / MacOS Big Sur.  See
...
&find_and_check("automake", $prte_automake_search, $prte_automake_version);
