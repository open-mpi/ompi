...
"""Compare one to_self operation across MPI implementations."""
...
from datatype_bench_common import (  # noqa: E402
    TO_SELF_HEADERS, cpu_description, sha256, to_self_compile_inputs,
)
...
# to_self --validate emits one such comment per datatype: pack/unpack against
# the by-hand reference, plus packed-signature checks used by --send=packed
# and --recv=packed.  Each field is PASS/FAIL/SKIP.
VALIDATION_RE = re.compile(
    r"^# VALIDATION\s+([a-z0-9_]+)\s+pack=(PASS|FAIL|SKIP)\s+unpack=(PASS|FAIL|SKIP)"
    r"\s+packed_send=(PASS|FAIL|SKIP)\s+packed_recv=(PASS|FAIL|SKIP)$"
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Compare one to_self operation across a base MPI and other implementations.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "example:\n"
            "  run_to_self_comparison.py --base current_dir \\\n"
    ...
    parser.add_argument(
        ...
        help="to_self.c used when an installation prefix is provided; companion "
             "sources (datatype_corpus.c) are compiled from the same directory",
    )
    ...
    parser.add_argument(
        ...
        help="command placed after the launcher and before each executable, for example "
             "'taskset -c 3'. Communication runs apply it to every rank, so a single-core "
             "pin serializes the transfer.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="+/- percent band treated as noise when counting faster/slower points; "
             "forwarded to the plotter's shaded band",
    )
    parser.add_argument(
        "--plotter", type=Path, default=directory / "plot_to_self_comparison.py"
    )
...
def compile_benchmark(
    ...
    sources, source_includes = to_self_compile_inputs(args.source)
    hashed = list(sources) + [sources[0].parent / name for name in TO_SELF_HEADERS]
    ...
    build_inputs = {
        ...
        "source_sha256": ":".join(sha256(path) for path in hashed),
        "mpicc": str(mpicc),
    ...
    if not args.force and output.is_file() and metadata_path.is_file():
        ...
        identity_matches = all(
            previous.get(name) == value for name, value in build_inputs.items()
        )
    ...
    command = [
        str(mpicc), "-std=gnu11", *shlex.split(args.cflags), *source_includes,
        *[str(path) for path in sources], "-lm", "-o", str(output),
    ]
    result = subprocess.run(command, text=True, capture_output=True)
    metadata = build_inputs | {
        "command": command,
...
def resolve_benchmark(
    ...
    if path.is_file():
        ...
        if not to_self_supports_cli(path):
            raise RuntimeError(
                f"{label}: {path} predates the options this comparison requires "
                "(--data/--trials/--raw-timers); provide that MPI's mpicc instead"
            )
...
def implementation_command(
    ...
    """Build a command that wraps the executable, not the launcher.

    The prefix belongs in front of the tester so `taskset`/`numactl` apply to the
    rank being measured.  Putting it in front of mpirun pins the daemon and every
    rank to the same inherited mask, which silently serializes a two-rank transfer.
    """
    if implementation.launcher is not None:
        return [
            str(implementation.launcher), "-n", str(processes), *launcher_args,
            *command_prefix, str(implementation.executable),
        ]
    if 1 != processes:
...
def _worse_validation(*verdicts: str) -> str:
    """Return the most severe of PASS/FAIL/SKIP/UNKNOWN (FAIL dominates)."""
    severity = {"FAIL": 3, "UNKNOWN": 2, "SKIP": 1, "PASS": 0}
    return max(verdicts, key=lambda verdict: severity.get(verdict, 0))


def parse_validation_status(text: str, operation: str) -> str:
    """Reduce to_self's per-datatype '# VALIDATION' line to the measured checks.

    Returns PASS/FAIL/SKIP for the checks the operation exercises, or UNKNOWN
    when the run carried no validation line (an executable too old for --validate,
    or a run that did not request it).  Pack and unpack use their own by-hand
    verdict plus both packed-signature checks (to_self aborts on a failure in
    either).  Communication moves ddt -> packed -> ddt and needs every check.
    FAIL dominates, then SKIP.
    """
    pack = unpack = packed_send = packed_recv = None
    for line in text.splitlines():
        match = VALIDATION_RE.match(line)
        if match:
            pack, unpack = match.group(2), match.group(3)
            packed_send, packed_recv = match.group(4), match.group(5)
            break
    if pack is None:
        return "UNKNOWN"
    packed = (packed_send, packed_recv)
    if operation.startswith("unpack"):
        return _worse_validation(unpack, *packed)
    if operation.startswith("pack"):
        return _worse_validation(pack, *packed)
    return _worse_validation(pack, unpack, *packed)
...
def run_measurement(
    ...
    if output.exists() and not args.force:
        ...
        try:
            ...
            # An unparseable cache is not a successful result; drop it so the
            # next run re-measures this file instead of failing forever.
            output.unlink()
            raise
    ...
    # Keep stderr for debugging, but write the reusable .out only after the
    # output parses: a later "--force"-less run treats an existing .out as a
    # valid cached result, so a failed or unparseable run must not leave one
    # behind.
...
def write_tables(
    ...
    trial_fields = [
        "datatype_index", "datatype", "implementation_key", "implementation_label", "operation",
        "repetition", "size_bytes", "trial", "seconds", "retained", "raw_file", "valid",
    ]
    ...
    summary_fields = [
        ...
        "ci_low_pct", "ci_high_pct", "mad_pct", "repetitions", "valid",
    ]
    validity_severity = {"FAIL": 3, "UNKNOWN": 2, "SKIP": 1, "PASS": 0}

    def worst_validity(impl_key: str, index: int, datatype: str, size: int) -> str:
        """Worst correctness verdict across all repetitions of one implementation at one point."""
        verdicts = [
            str(grouped[(index, datatype, size, impl_key, rep)]["valid"])
            for rep in range(1, args.repetitions + 1)
        ]
        return max(verdicts, key=lambda v: validity_severity.get(v, 0))
            ...
            for size in sizes:
                ...
                for statistic, field in (("filtered", "seconds"), ("unfiltered", "unfiltered_seconds")):
                    ...
                    for subject, baseline in pairs:
                        ...
                        subject_valid = worst_validity(subject.key, index, datatype, size)
                        baseline_valid = worst_validity(baseline.key, index, datatype, size)
                        row_valid = (
                            subject_valid
                            if validity_severity.get(subject_valid, 0)
                            >= validity_severity.get(baseline_valid, 0)
                            else baseline_valid
                        )
                        summary_row = {
                    ...
                    writer.writerow(
                        {
                            ...
                            "faster_points": sum(value > args.noise for value in values),
                            "slower_points": sum(value < -args.noise for value in values),
                            "noise_points": sum(-args.noise <= value <= args.noise for value in values),
                        }
                    )
    return summary_path
...
def main() -> int:
    args = parse_args()
    if 1 < args.processes and args.command_prefix.strip():
        print(
            "warning: --command-prefix wraps every rank's executable, not mpirun; "
            "a single-core pin such as 'taskset -c 3' schedules sender and receiver "
            "onto the same core",
            file=sys.stderr,
        )
    ...
    if not args.no_plot:
        command = [
            ...
            "--output", str(args.output / "graphs"), "--noise", str(args.noise),
        ]
