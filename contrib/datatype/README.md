...
Build `to_self` from the same sources as the in-tree tester (`to_self.c` and
`datatype_corpus.c`; see `to_self_SOURCES` in `ompi/test/datatype/Makefile.am`). `--source` names
`to_self.c`; companion translation units and `datatype_corpus.h` are taken from that directory.
This is useful when the benchmark source comes from the current checkout but the executable must
link against an installed release. The output is validated with `--help`, and `<output>.build.json`
records the complete compiler command, working directory, standard output, and standard error.
...
wrappers cause the runner to compile `ompi/test/datatype/to_self.c` together with
`datatype_corpus.c` from the same directory; communication operations also use
...
```sh
python3 contrib/datatype/run_to_self_comparison.py \
  --base-label current \
...
used, the runner also invokes `plot_to_self_comparison.py`.
...
## `plot_to_self_comparison.py`

Regenerate graphs from one `summary.tsv` produced by `run_to_self_comparison.py`. It creates a
...
```sh
python3 contrib/datatype/plot_to_self_comparison.py \
...
`run_to_self_comparison.py`. Where the runner compares implementations launched side by side in
...
`datatype_summary.tsv`), so `plot_to_self_comparison.py` renders it: a positive `speedup_pct`
...
The two runs must have measured the same work: `compare_pack_runs.py` refuses to run when either
directory has no `run_config.json`, or when the recorded controls (operation, cycles, trials,
warmups, minimum work bytes, datatype selection, endpoints, and process count) differ, and warns
when `manifest.txt` is missing from either directory or when the recorded machine identity differs.
Binary identity is intentionally *not* required, since a regression check expects two different
executables. Pass `--allow-mismatch` to downgrade a missing `run_config.json` or a control mismatch
to a warning and `--no-plot` to skip rendering. A regression check usually cares about a single
...
Run the complete comparison suite by invoking `run_to_self_comparison.py` for five scenarios:
...
Drive `ompi/test/datatype/pack_description_sweep` over exact descriptor shapes. The sweep
controls operation, backend, convertor fragment size, predefined-element size, DATA count, block
length, loop-body item count, loop iterations, datatype count, and gaps. Pass `--commit-description`
to measure the optimizer's committed description instead of the synthetic (hand-built) descriptor,
`--backend` to compare the public `MPI_Pack`/`MPI_Unpack` path with the current, reference,
accelerator, or general interpreters (comma-delimited; swept as a matrix dimension), and
`--fragment-bytes` to exercise the fragmented convertor. Unlike `to_self`, this tester can therefore
install implementation-specific optimized descriptions and directly compare equivalent copy and loop
geometries.
...
and the loop-unroll item and DATA-byte limits. It leaves a candidate unset when the tested
alternatives do not beat the configured noise threshold consistently. Pack and unpack
vectorized-block cutoffs stay in the mover evidence tables: the current/reference experiment
swaps whole interpreters and cannot force a typed mover after the selector has chosen the
reference path.
