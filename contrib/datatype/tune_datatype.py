...
def parse_to_self(stdout: str, threshold: int) -> list[dict[str, object]]:
    ...
    for line in stdout.splitlines():
        ...
        if stripped.startswith("# Unpack ("):
            operation = "unpack"
            continue
        if stripped.startswith("# Isend") or stripped.startswith("# Irecv"):
            # Communication tests never reset the pack/unpack tag.  Without this,
            # their timing rows would be stored as the previous local operation
            # and overwrite the matching (datatype, operation, length) point.
            operation = "communication"
            continue
        match = timing.match(line)
        if match:
            # Continue for every timing row, not only pack/unpack.  By-hand and
            # communication sections leave `operation` as "byhand" or never
            # reset it; those rows would otherwise fall through and replace
            # `datatype` with the timing text itself.  Unpack of the same
            # entry then keys analyze_corpus() on that garbage and is dropped.
...
def analyze_loops(rows: list[dict[str, object]], noise_pct: float) -> dict[str, object]:
    ...
    def score_sizes(operation: str, loop_items: int) -> list[dict[str, object]]:
        ...
        # smaller size.  Score each data size at a fixed unroll factor and take
        # the largest ascending-safe cutoff rather than the largest winning size.
        selected_rows = [row for row in details
                         if row["operation"] == operation and int(row["loop_items"]) == loop_items]
    ...
    # Per-operation loop-factor scores, kept so the shared item cap can be
    # derived from evidence common to both ops.  Per-byte scores for the shared
    # byte cap are filled in after that shared factor is known: the cap applies
    # together with opal_datatype_optimize_loop_unroll_max_items, so scoring
    # pack at pack's own factor and unpack at unpack's would mix two different
    # unroll depths, neither of which is the factor that will actually be used.
    ...
    for operation in ("pack", "unpack"):
        ...
        recommendations[operation] = {
            ...
            "largest_beneficial_data_bytes": largest_safe_cutoff(
                score_sizes(operation, selected), "data_bytes", 0, noise_pct
            ),
            "scores": scores,
        }
        acceptable_items[operation] = {int(row["loop_items"]): row for row in scores}
    ...
    shared_items = int(recommendations["shared_candidate_max_items"])
    byte_scores: dict[str, dict[int, dict[str, object]]] = {}
    for operation in ("pack", "unpack"):
        byte_scores[operation] = {
            int(row["data_bytes"]): row for row in score_sizes(operation, shared_items)
        }
...
def _combine_scores(per_operation: dict[str, dict[int, dict[str, object]]],
    ...
    Only values measured in *both* operations survive; the merged geomean is the
    more pessimistic of the two and the regression/sample counts are pooled, so
    largest_safe_cutoff() cannot pick a value that is fine for one operation but
...
def main() -> int:
    ...
    # Resolve each binary lazily: only when the requested stages actually need it,
    # so --check=corpus does not fail on a tree without pack_description_sweep and
    # vice versa.  Record the resolved path and sha256 in the manifest only for
    # binaries that were actually used.
    to_self: Path | None = None
    sweep: Path | None = None
    if "corpus" in args.check:
        to_self = resolve_executable(
            args.to_self, build_dir / "ompi" / "test" / "datatype" / "to_self",
            "to_self", "--to-self"
        )
        manifest["to_self"] = str(to_self)
        manifest["to_self_sha256"] = sha256(to_self)
    if any(stage in args.check for stage in ("loops", "movers")):
        sweep = resolve_executable(
            args.description_sweep,
            build_dir / "ompi" / "test" / "datatype" / "pack_description_sweep",
            "pack_description_sweep", "--description-sweep"
        )
        manifest["pack_description_sweep"] = str(sweep)
        manifest["pack_description_sweep_sha256"] = sha256(sweep)
    config = {
    ...
    # The current/reference A/B swaps whole interpreters. It can validate an
    # existing selector but cannot force a typed mover after that selector has
    # chosen the reference path, so pack/unpack vectorized-block cutoffs stay
    # in the mover evidence tables rather than becoming candidate constants.
