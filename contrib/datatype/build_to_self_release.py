...
"""Build to_self (and its companion sources) with an MPI compiler wrapper."""
...
from datatype_bench_common import to_self_compile_inputs  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="benchmark to_self.c; companion sources (datatype_corpus.c) are "
             "compiled from the same directory",
    )
...
def main() -> int:
    ...
    if not mpicc.is_file():
        raise RuntimeError(f"mpicc does not exist: {mpicc}")
    ...
    command = [
        str(mpicc), "-std=gnu11", *shlex.split(args.cflags), *source_includes, *include_flags,
        *configured_flags, *[str(path) for path in sources], "-lm", "-o", str(output),
    ]
