...
The to_self runner (run_to_self_comparison.py) already compares
...
datatype_summary.tsv), so plot_to_self_comparison.py plots it unchanged:
...
from run_to_self_comparison import BOOTSTRAP_SAMPLES  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--allow-mismatch", action="store_true",
                        help="downgrade a missing run_config.json, or differing controls, "
    ...
    parser.add_argument("--plotter", type=Path, default=directory / "plot_to_self_comparison.py",
...
def read_manifest(run_dir: Path) -> dict[str, str] | None:
    """Parse the key=value manifest.txt written by the runner.

    Return None when the file is absent so the caller can report a missing
    manifest instead of treating every environment key as unset.
    """
...
def read_run_config(run_dir: Path) -> dict[str, object] | None:
    """Parse run_config.json; return None when it is absent.

    CONTROL_DEFAULTS apply only to keys omitted from an existing file (a run
    written before a newer control existed).  An absent file is not equivalent
    to those defaults: the caller must treat it as a comparability problem.
    """
...
def guard_comparable(
    ...
    if baseline_config is None:
        problems.append(f"{baseline_dir} has no run_config.json")
    if candidate_config is None:
        problems.append(f"{candidate_dir} has no run_config.json")
    if baseline_config is not None and candidate_config is not None:
    ...
    if baseline_manifest is None:
        warnings.append(f"{baseline_dir} has no manifest.txt")
    if candidate_manifest is None:
        warnings.append(f"{candidate_dir} has no manifest.txt")
    if baseline_manifest is not None and candidate_manifest is not None:
        for key in ENVIRONMENT_KEYS:
            if baseline_manifest.get(key) != candidate_manifest.get(key):
                warnings.append(
                    f"environment '{key}' differs: "
                    f"baseline={baseline_manifest.get(key)!r} "
...
def load_trials(
    ...
    with path.open(newline="") as stream:
        ...
        # The valid column is optional: absent in trials.tsv written before it was
        # added; treat missing as PASS so old runs still compare.
        has_valid = "valid" in reader.fieldnames
        failed_points: set[tuple[str, str, int, str, int]] = set()
        failed_reps: int = 0
        for line_no, row in enumerate(reader):
            ...
            valid = row["valid"] if has_valid else "PASS"
            labels[impl_key] = row["implementation_label"]
            operations.add(operation)
            datatypes.setdefault(name, (index, name))
            if "FAIL" == valid:
                failed_points.add((impl_key, operation, index, name, size))
                failed_reps += 1
                continue
    ...
    if failed_points:
        print(
            f"note: {run_dir.name}: skipped {failed_reps} repetition(s) with a FAIL "
            f"correctness verdict across {len(failed_points)} point(s); "
            "PASS repetitions for those points are still included",
            file=sys.stderr,
        )
    if not samples:
