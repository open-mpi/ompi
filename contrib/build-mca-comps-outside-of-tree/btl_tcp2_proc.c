...
mca_btl_tcp2_interface_t* local_interfaces[MAX_KERNEL_INTERFACES];
mca_btl_tcp2_interface_t* peer_interfaces[MAX_KERNEL_INTERFACES];
...
enum mca_btl_tcp2_connection_quality **weights;
struct mca_btl_tcp2_addr_t ***best_addr;
...
void mca_btl_tcp2_proc_construct(mca_btl_tcp2_proc_t* proc)
{
    proc->proc_ompi = 0;
...
void mca_btl_tcp2_proc_destruct(mca_btl_tcp2_proc_t* proc)
{
...
mca_btl_tcp2_proc_t* mca_btl_tcp2_proc_create(ompi_proc_t* ompi_proc)
{
    ...
    if(0 != (size % sizeof(mca_btl_tcp2_addr_t))) {
        BTL_ERROR(("mca_base_modex_recv: invalid size %d: btl-size: %d\n",
          size, sizeof(mca_btl_tcp2_addr_t)));
        return NULL;
    }
