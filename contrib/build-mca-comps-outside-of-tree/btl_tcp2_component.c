...
int mca_btl_tcp2_component_open(void)
{
    ...
    mca_btl_tcp2_component.tcp_if_include =
        mca_btl_tcp2_param_register_string("if_include", NULL, "");
    mca_btl_tcp2_component.tcp_if_exclude =
        mca_btl_tcp2_param_register_string("if_exclude", NULL, "lo");
...
static int mca_btl_tcp2_component_create_instances(void)
{
    ...
    argv = include = opal_argv_split(mca_btl_tcp2_component.tcp_if_include,',');
    while(argv && *argv) {
        char* if_name = *argv;
    ...
    if(mca_btl_tcp2_component.tcp_num_btls) {
        ret = OMPI_SUCCESS;
        goto cleanup;
    }
...
static int mca_btl_tcp2_component_create_listen(uint16_t af_family)
{
    ...
    {  /* Don't reuse ports */
        ...
        if (setsockopt (sd, SOL_SOCKET, SO_REUSEADDR, (const char *)&flg, sizeof (flg)) < 0) {
            BTL_ERROR((0, "mca_btl_tcp2_create_listen: unable to unset the "
