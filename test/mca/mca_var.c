...
static mca_base_var_include_list_t include_list_var;
...
static void check_include_list_contents(const char *check_str, const char *value, const mca_base_var_include_list_t *include_list)
{
...
static int test_mca_base_var_parse_include_list(void)
{
    test_init("mca_base_var_parse_include_list");

    fprintf (stderr, "Testing mca_base_var_parse_include_list....\n");

    fprintf (stderr, "  Test mca_base_var_include_list_t construction: ");

    OBJ_CONSTRUCT(&include_list_var, mca_base_var_include_list_t);
    if (NULL != include_list_var.items || include_list_var.is_exclude == true) {
        OBJ_DESTRUCT(&include_list_var);
    ...
    int ret = mca_base_var_parse_include_list(exclude_list, &include_list_var);
    ...
    ret = mca_base_var_parse_include_list(include_list, &include_list_var);
    TEST_EXPECT(ret == OPAL_SUCCESS, "");
    check_include_list_contents("include", include_list, &include_list_var);

    fprintf (stderr, "  Test mca_base_var_include_list_t destruction: ");
    OBJ_DESTRUCT(&include_list_var);
    if (NULL != include_list_var.items || include_list_var.is_exclude == true) {
...
static int test_include_list_variable(void)
{
    test_init("include/exclude list MCA variables...");
    ...
    ret = mca_base_var_register(kProjectName, kFrameworkName, kComponentName, kTestVarName,
                                "String variable for test", MCA_BASE_VAR_TYPE_INCLUDE_LIST,
                                /*enumerator=*/NULL, /*bind=*/0, MCA_BASE_VAR_FLAG_SETTABLE,
...
int main(int argc, char *argv[])
{
    ...
    test_mca_base_var_parse_include_list();
    test_include_list_variable();
