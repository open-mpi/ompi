...
int main(int argc, char *argv[])
{
    ...
    ofi_make_filename(filename, sizeof(filename), "set-view");
    ...
    ofi_check_rc(&res, rc, "MPI_Type_contiguous");
    if (MPI_SUCCESS == rc) {
        rc = MPI_Type_commit(&filetype);
        ofi_check_rc(&res, rc, "MPI_Type_commit");
    }
    if (MPI_SUCCESS != rc) {
        MPI_File_close(&fh);
        MPI_Finalize();
    ...
    if (MPI_SUCCESS == rc) {
        ...
        ofi_check_rc(&res, rc, "MPI_File_set_view (with hints)");
        MPI_Info_free(&info);
    }

    rc = MPI_File_get_info(fh, &info_used);
    ofi_check_rc(&res, rc, "MPI_File_get_info (after set_view)");
    if (MPI_SUCCESS == rc) {
        ofi_expect_value(&res, info_used, "cb_nodes", "1");
        ofi_expect_value(&res, info_used, "collective_buffering", "false");
    ...
    if (MPI_SUCCESS == rc) {
        ...
        rc = MPI_File_set_view(fh, 0, MPI_BYTE, filetype, "native", info);
        ofi_check_rc(&res, rc, "MPI_File_set_view (unknown only)");
        MPI_Info_free(&info);
    }

    rc = MPI_File_get_info(fh, &info_used);
    ofi_check_rc(&res, rc, "MPI_File_get_info (after unknown-only set_view)");
    if (MPI_SUCCESS == rc) {
        ofi_expect_value(&res, info_used, "cb_nodes", "1");
        ofi_expect_value(&res, info_used, "collective_buffering", "false");
        ofi_expect_no_key(&res, info_used, "unknown_ompio_hint");
