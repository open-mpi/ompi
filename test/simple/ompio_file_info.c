...
static void expect_key(MPI_Info info, const char *key, int rank)
{
    ...
    if (!info_has_key(info, key, value, MPI_MAX_INFO_VAL + 1)) {
...
/*
 * When the user supplies both the canonical spelling and its historical alias
 * for a layout hint, OMPIO must keep only the canonical spelling public,
 * independent of the order the keys were added.  The alias is given a distinct
 * value so the assertion proves the canonical value (not the alias) survived.
 */
static void check_lustre_both_spellings(const char *filename, int rank)
{
    MPI_File fh;
    MPI_Info info;
    MPI_Info info_used;
    ...
    rc = MPI_Info_set(info, "stripe_size", "2097152");
    check_rc(rc, "MPI_Info_set", rank);
    rc = MPI_Info_set(info, "striping_unit", "1048576");
    check_rc(rc, "MPI_Info_set", rank);
    rc = MPI_Info_set(info, "stripe_width", "4");
    check_rc(rc, "MPI_Info_set", rank);
    rc = MPI_Info_set(info, "striping_factor", "2");
    check_rc(rc, "MPI_Info_set", rank);
    ...
    check_rc(rc, "MPI_File_get_info", rank);
    if (MPI_SUCCESS == rc) {
        expect_value(info_used, "striping_unit", "1048576", rank);
        expect_value(info_used, "striping_factor", "2", rank);
        expect_no_key(info_used, "stripe_size", rank);
        expect_no_key(info_used, "stripe_width", rank);
        MPI_Info_free(&info_used);
    }
...
static int check_lustre_info(int rank)
{
    ...
    make_filename_in_dir(filename, sizeof(filename), dir, "lustre-both", rank);
    check_lustre_both_spellings(filename, rank);
