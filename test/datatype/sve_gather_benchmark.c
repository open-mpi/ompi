...
int main( int argc, char* argv[] )
{
    ...
    /*
     * repetitions: the more the better for sve, this depends the main optimized in gather_load. Full block gather
     * blocklen:
    ...
    if(*sve_yes=='s') {
        if( run_tests & DO_CONSTANT_GAP ) {
            ...
            ddt = create_optimized_indexed_constant_gap_ddt( repetitions, 1, 1 );
            MPI_DDT_DUMP( ddt );
