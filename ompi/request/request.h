 ...
 */
typedef int (*ompi_request_start_fn_t)(size_t count, struct ompi_request_t **requests);
...
#define OMPI_REQUEST_INIT(request, persistent)                                        \
    do {                                                                              \
        (request)->req_complete = (persistent) ? REQUEST_COMPLETED : REQUEST_PENDING; \
...
#define OMPI_REQUEST_FINI(request)                                                               \
    do {                                                                                         \
        ...
        if (MPI_UNDEFINED != (request)->req_f_to_c_index) {                                      \
            opal_pointer_array_set_item(&ompi_request_f_to_c_table, (request)->req_f_to_c_index, \
                                        NULL);                                                   \
 ...
 */
typedef int (*ompi_request_test_fn_t)(ompi_request_t **rptr, int *completed,
 ...
 * request handle at index set to NULL.
 */
typedef int (*ompi_request_test_any_fn_t)(size_t count, ompi_request_t **requests, int *index,
 ...
 */
typedef int (*ompi_request_test_all_fn_t)(size_t count, ompi_request_t **requests, int *completed,
 ...
 *
 */
typedef int (*ompi_request_test_some_fn_t)(size_t count, ompi_request_t **requests, int *outcount,
 ...
 *
 */
typedef int (*ompi_request_wait_any_fn_t)(size_t count, ompi_request_t **requests, int *index,
                                          ompi_status_public_t *status);
/**
 ...
 *
 */
typedef int (*ompi_request_wait_all_fn_t)(size_t count, ompi_request_t **requests,
                                          ompi_status_public_t *statuses);
/**
 ...
 *
 */
typedef int (*ompi_request_wait_some_fn_t)(size_t count, ompi_request_t **requests, int *outcount,
                                           int *indices, ompi_status_public_t *statuses);
...
#    define ompi_request_is_failed(req) \
        OPAL_UNLIKELY(ompi_ftmpi_enabled ? ompi_request_is_failed_fn(req) : false)
    ...
    redo:
        ...
        if (OPAL_UNLIKELY(OMPI_SUCCESS != sync.status)) {
            OPAL_OUTPUT_VERBOSE((50, ompi_ftmpi_output_handle,
                                 "Status %d reported for sync %p rearming req %p", sync.status,
                                 (void *) &sync, (void *) req));
            _tmp_ptr = &sync;
            if (OPAL_ATOMIC_COMPARE_EXCHANGE_STRONG_PTR(&req->req_complete, &_tmp_ptr,
                                                        REQUEST_PENDING)) {
                opal_output_verbose(10, ompi_ftmpi_output_handle,
                                    "Status %d reported for sync %p rearmed req %p", sync.status,
                                    (void *) &sync, (void *) req);
    ...
    if (0 == rc) {
        if (OPAL_LIKELY(with_signal)) {
            ...
            if (!OPAL_ATOMIC_COMPARE_EXCHANGE_STRONG_PTR(&request->req_complete, &_tmp_ptr,
                                                         REQUEST_COMPLETED)) {
                ompi_wait_sync_t *tmp_sync = (ompi_wait_sync_t *)
                    OPAL_ATOMIC_SWAP_PTR(&request->req_complete, REQUEST_COMPLETED);
                /* In the case where another thread concurrently changed the request to
                 * REQUEST_PENDING */
...
static inline int ompi_request_set_callback(ompi_request_t *request, ompi_request_complete_fn_t cb,
                                            void *cb_data)
{
