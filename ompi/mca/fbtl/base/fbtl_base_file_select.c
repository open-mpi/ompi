...
int mca_fbtl_base_file_select(struct ompio_file_t *file, mca_base_component_t *preferred)
{
    int priority;
    int best_priority;
    ...
    if (NULL != preferred) {
        ...
        opal_output_verbose(10, ompi_fbtl_base_framework.framework_output,
                            "fbtl:base:file_select: Checking preferred component: %s", str);
        ...
        if (NULL != module && NULL != module->fbtl_module_init) {
    ...
    OPAL_LIST_FOREACH (cli, &ompi_fbtl_base_framework.framework_components,
