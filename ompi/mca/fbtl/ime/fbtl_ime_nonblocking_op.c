...
static ssize_t mca_fbtl_ime_nonblocking_op(ompio_file_t *fh, ompi_request_t *request, int io_op);
...
static ssize_t mca_fbtl_ime_nonblocking_op(ompio_file_t *fh, ompi_request_t *request, int io_op)
{
    ...
    /* We might allocate too much memory here because we don't know
       ...
       not just one. */
    data->allocated_data = (void *) malloc(
        fh->f_num_of_io_entries
        * (sizeof(struct iovec) + sizeof(struct ime_aiocb) + sizeof(ssize_t)));
    ...
    data->aio_reqs = (struct ime_aiocb *) (data->aio_iovecs + fh->f_num_of_io_entries);
    data->aio_req_status = (ssize_t *) (data->aio_reqs + fh->f_num_of_io_entries);
    ...
    for (i = 0; i < fh->f_num_of_io_entries; i++) {
        ...
        if (data->aio_reqs[req_index].iovcnt == 0) {
            ...
            data->aio_reqs[req_index].file_offset = (off_t) fh->f_io_array[i].offset;
            data->aio_reqs[req_index].fd = fh->fd;
            data->aio_reqs[req_index].complete_cb = &mca_fbtl_ime_complete_cb;
            data->aio_reqs[req_index].user_context = (intptr_t) &data->aio_req_status[req_index];
    ...
    for (i = 0; i < data->aio_last_active_req; i++) {
        ...
        case FBTL_IME_READ:
            ...
            if (ret < 0) {
                opal_output(1,
                            "mca_fbtl_ime_nonblocking_op: error in "
        ...
        default:
            opal_output(1,
                        "mca_fbtl_ime_nonblocking_op: an unsupported "
