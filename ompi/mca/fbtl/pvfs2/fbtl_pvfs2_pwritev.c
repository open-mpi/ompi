    ...
    for (i = 0; i < fh->f_num_of_io_entries; i++) {
        ...
        */
        if (merge) {
            ...
            k = 0;
            while (merge >= 0) {
                memcpy(merge_buf + k, fh->f_io_array[i - merge].memory_address,
            ...
            ret = PVFS_sys_write(pvfs2_fs->object_ref, file_req, merge_offset, merge_buf, mem_req,
            ...
            ret = PVFS_sys_write(pvfs2_fs->object_ref, file_req,
                                 (OMPI_MPI_OFFSET_TYPE) fh->f_io_array[i].offset,
                                 fh->f_io_array[i].memory_address, mem_req,
                                 &(pvfs2_fs->credentials), &resp_io);
            if (ret != 0) {
                perror("PVFS_sys_write() error");
                return OMPI_ERROR;
