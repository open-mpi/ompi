...
struct mca_fbtl_base_module_1_0_0_t *mca_fbtl_posix_component_file_query(ompio_file_t *fh,
                                                                         int *priority)
{
                ...
                */
                ...
                if (data->aio_reqs[i].aio_nbytes != (size_t) ret2) {
                    ...
                    if (data->aio_req_type == FBTL_POSIX_WRITE) {
                        ret_code = mca_fbtl_posix_lock(&data->aio_lock, data->aio_fh, F_WRLCK,
                                                       start_offset, total_length,
                                                       OMPIO_LOCK_ENTIRE_REGION,
                                                       &data->aio_lock_counter);
                        if (0 < ret_code) {
                            opal_output(
                                1, "mca_fbtl_posix_progress: error in mca_fbtl_posix_lock() %d",
                                ret_code);
                        ...
                        ret_code = mca_fbtl_posix_lock(&data->aio_lock, data->aio_fh, F_RDLCK,
                                                       start_offset, total_length,
                                                       OMPIO_LOCK_ENTIRE_REGION,
                                                       &data->aio_lock_counter);
                        if (0 < ret_code) {
...
#    endif
    if ((lcount == data->aio_req_chunks) && (0 != data->aio_open_reqs)) {
        ...
        end_offset = data->aio_reqs[data->aio_last_active_req - 1].aio_offset
                     + data->aio_reqs[data->aio_last_active_req - 1].aio_nbytes;
        total_length = (end_offset - start_offset);

        if (FBTL_POSIX_READ == data->aio_req_type) {
            ret_code = mca_fbtl_posix_lock(&data->aio_lock, data->aio_fh, F_RDLCK, start_offset,
                                           total_length, OMPIO_LOCK_ENTIRE_REGION,
                                           &data->aio_lock_counter);
        } else if (FBTL_POSIX_WRITE == data->aio_req_type) {
            ret_code = mca_fbtl_posix_lock(&data->aio_lock, data->aio_fh, F_WRLCK, start_offset,
                                           total_length, OMPIO_LOCK_ENTIRE_REGION,
                                           &data->aio_lock_counter);
        }
        if (0 < ret_code) {
