...
static ssize_t mca_fbtl_posix_pwritev_datasieving(ompio_file_t *fh, struct flock *lock,
                                                  int *lock_counter);
static ssize_t mca_fbtl_posix_pwritev_generic(ompio_file_t *fh, struct flock *lock,
                                              int *lock_counter);
static ssize_t mca_fbtl_posix_pwritev_single(ompio_file_t *fh, struct flock *lock,
                                             int *lock_counter);
...
ssize_t mca_fbtl_posix_pwritev(ompio_file_t *fh)
{
    ...
    if (fh->f_num_of_io_entries > 1) {
        ...
        if (false == mca_fbtl_posix_write_datasieving || 0 == avg_gap_size
            || avg_block_size > mca_fbtl_posix_max_block_size
            || avg_gap_size > mca_fbtl_posix_max_gap_size || ompi_mpi_thread_multiple
...
ssize_t mca_fbtl_posix_pwritev_datasieving(ompio_file_t *fh, struct flock *lock, int *lock_counter)
{
    ...
    while (!done) {
        ...
        if (0 < ret) {
            opal_output(
                1, "mca_fbtl_posix_pwritev_datasieving: error in mca_fbtl_posix_lock() ret=%d: %s",
                ret, strerror(errno));
            /* Just in case some part of the lock worked */
            mca_fbtl_posix_unlock(lock, fh, lock_counter);
            free(temp_buf);
        ...
        while (total_bytes < len) {
            ...
            if (ret_code == -1) {
                opal_output(1, "mca_fbtl_posix_pwritev_datasieving: error in pread:%s",
                            strerror(errno));
                mca_fbtl_posix_unlock(lock, fh, lock_counter);
                free(temp_buf);
                return OMPI_ERROR;
        ...
        total_bytes = 0;
        while (total_bytes < len) {
            ret_code = pwrite(fh->fd, temp_buf + total_bytes, len - total_bytes,
                              start + total_bytes);
            if (ret_code == -1) {
                opal_output(1, "mca_fbtl_posix_pwritev_datasieving: error in pwrite:%s",
                            strerror(errno));
                mca_fbtl_posix_unlock(lock, fh, lock_counter);
                free(temp_buf);
                return OMPI_ERROR;
    ...
    for (i = 0; i < fh->f_num_of_io_entries; i++) {
        ...
        */
        ...
        ret = mca_fbtl_posix_lock(lock, fh, F_WRLCK, iov_offset, total_length, OMPIO_LOCK_SELECTIVE,
                                  lock_counter);
        if (0 < ret) {
            opal_output(1, "mca_fbtl_posix_pwritev: error in mca_fbtl_posix_lock() error ret=%d %s",
                        ret, strerror(errno));
            free(iov);
            /* just in case some part of the lock worked */
            mca_fbtl_posix_unlock(lock, fh, lock_counter);
