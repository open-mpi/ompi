...
static ssize_t mca_fbtl_posix_preadv_datasieving(ompio_file_t *fh, struct flock *lock,
                                                 int *lock_counter);
static ssize_t mca_fbtl_posix_preadv_generic(ompio_file_t *fh, struct flock *lock,
                                             int *lock_counter);
static ssize_t mca_fbtl_posix_preadv_single(ompio_file_t *fh, struct flock *lock,
                                            int *lock_counter);
...
ssize_t mca_fbtl_posix_preadv(ompio_file_t *fh)
{
    ...
    if (fh->f_num_of_io_entries > 1) {
        ...
        if (false == mca_fbtl_posix_read_datasieving || 0 == avg_gap_size
...
ssize_t mca_fbtl_posix_preadv_datasieving(ompio_file_t *fh, struct flock *lock, int *lock_counter)
{
    ...
    while (!done) {
        ...
        if (0 < ret) {
            opal_output(
                1, "mca_fbtl_posix_preadv_datasieving: error in mca_fbtl_posix_lock() ret=%d: %s",
                ret, strerror(errno));
            /* Just in case some part of the lock worked */
            mca_fbtl_posix_unlock(lock, fh, lock_counter);
            free(temp_buf);
        ...
        while (total_bytes < len) {
            ret_code = pread(fh->fd, temp_buf + total_bytes, len - total_bytes,
                             start + total_bytes);
            if (ret_code == -1) {
                opal_output(1, "mca_fbtl_posix_preadv_datasieving: error in (p)read(v):%s",
                            strerror(errno));
                mca_fbtl_posix_unlock(lock, fh, lock_counter);
                free(temp_buf);
                return OMPI_ERROR;
...
ssize_t mca_fbtl_posix_preadv_generic(ompio_file_t *fh, struct flock *lock, int *lock_counter)
{
    ...
    for (i = 0; i < fh->f_num_of_io_entries; i++) {
        ...
        if (0 < ret) {
            opal_output(1,
                        "mca_fbtl_posix_preadv_generic: error in mca_fbtl_posix_lock() ret=%d: %s",
                        ret, strerror(errno));
            free(iov);
            /* Just in case some part of the lock worked */
            mca_fbtl_posix_unlock(lock, fh, lock_counter);
