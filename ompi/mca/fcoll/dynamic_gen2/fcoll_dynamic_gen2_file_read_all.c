...
static int read_heap_sort(mca_io_ompio_local_io_array *io_array, int num_entries, int *sorted);

int mca_fcoll_dynamic_gen2_file_read_all(ompio_file_t *fh, void *buf, int count,
                                         ...
                                         ompi_status_public_t *status)
{
    ...
    ret = mca_common_ompio_set_aggregator_props((struct ompio_file_t *) fh,
                                                dynamic_gen2_num_io_procs, max_data);
    if (OMPI_SUCCESS != ret) {
        goto exit;
    }
    ...
    if (my_aggregator == fh->f_rank) {
        for (i = 0; i < total_fview_count; i++) {
            printf("%d: OFFSET: %p   LENGTH: %d\n", fh->f_rank,
                   global_iov_array[sorted[i]].iov_base, global_iov_array[sorted[i]].iov_len);
        }
    }
#endif
    ...
    for (index = 0; index < cycles; index++) {
            ...
            */
            ...
            if (bytes_remaining) {
                ...
                if (bytes_to_read_in_cycle
                    ...
                    if (fh->f_procs_in_group[n] == fh->f_rank) {
                        bytes_received += global_iov_array[sorted[current_index]].iov_len;
                    }
                    bytes_to_read_in_cycle -= global_iov_array[sorted[current_index]].iov_len;
                    current_index++;
                    continue;
                }
            }
        ...
        if (my_aggregator == fh->f_rank) {
            ...
            read_heap_sort(file_offsets_for_agg, entries_per_aggregator, sorted_file_offsets);
            ...
            *********************************************************/
            ...
            for (i = 0; i < entries_per_aggregator; i++) {
                temp_index = file_offsets_for_agg[sorted_file_offsets[i]].process_id;
                ...
                if (temp_disp_index[temp_index] < disp_index[temp_index]) {
                    ...
                    printf("temp_disp_index[%d]: %d is greater than disp_index[%d]: %d\n",
                           temp_index, temp_disp_index[temp_index], temp_index,
                           disp_index[temp_index]);
                }
            }
            if (NULL != temp_disp_index) {
            ...
            for (i = 0; i < fh->f_procs_per_group; i++) {
                ...
                if (0 < disp_index[i]) {
                    ompi_datatype_create_hindexed(disp_index[i], blocklen_per_process[i],
                                                  displs_per_process[i], MPI_BYTE, &sendtype[i]);
                    ompi_datatype_commit(&sendtype[i]);
                    opal_datatype_type_size(&sendtype[i]->super, &datatype_size);

                    if (datatype_size) {
                        ret = MCA_PML_CALL(
                            isend(global_buf, 1, sendtype[i], fh->f_procs_in_group[i],
                                  FCOLL_DYNAMIC_GEN2_SHUFFLE_TAG, MCA_PML_BASE_SEND_STANDARD,
                                  fh->f_comm, &send_req[i]));
                        if (OMPI_SUCCESS != ret) {
                            goto exit;
                        }
                    }
        ...
        if (bytes_received) {
            ...
            while (remaining) {
                ...
                if (0 == block_index) {
                    recv_mem_address = (ptrdiff_t)(decoded_iov[iov_index].iov_base)
                                       + current_position;
                } else {
                ...
                if (remaining >= (decoded_iov[iov_index].iov_len - current_position)) {
                    blocklength_proc[block_index] = decoded_iov[iov_index].iov_len
                                                    - current_position;
            ...
            start_rcomm_time = MPI_Wtime();
#endif
            ret = MCA_PML_CALL(irecv((char *) recv_mem_address, 1, newType, my_aggregator,
                                     FCOLL_DYNAMIC_GEN2_SHUFFLE_TAG, fh->f_comm, &recv_req));
        ...
        if (my_aggregator == fh->f_rank) {
            ret = ompi_request_wait_all(fh->f_procs_per_group, send_req, MPI_STATUS_IGNORE);
            if (OMPI_SUCCESS != ret) {
                goto exit;
            }
        }
    ...
    if (!mca_common_ompio_full_print_queue(fh->f_coll_read_time)) {
        mca_common_ompio_register_print_entry(fh->f_coll_read_time, nentry);
    }
#endif
