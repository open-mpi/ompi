...
int mca_fcoll_dynamic_gen2_break_file_view(
    struct iovec *decoded_iov, int iov_count, struct iovec *local_iov_array, int local_count,
    struct iovec ***broken_decoded_iovs, int **broken_iov_counts, struct iovec ***broken_iov_arrays,
    int **broken_counts, MPI_Aint **broken_total_lengths, int stripe_count, int stripe_size);

int mca_fcoll_dynamic_gen2_get_configuration(ompio_file_t *fh, int *dynamic_gen2_num_io_procs,
                                             int **ret_aggregators);

static int local_heap_sort(mca_io_ompio_local_io_array *io_array, int num_entries, int *sorted);

int mca_fcoll_dynamic_gen2_split_iov_array(ompio_file_t *fh,
                                           mca_common_ompio_io_array_t *work_array, int num_entries,
                                           int *last_array_pos, int *last_pos_in_field);

int mca_fcoll_dynamic_gen2_file_write_all(ompio_file_t *fh, const void *buf, int count,
                                          ...
                                          ompi_status_public_t *status)
{
    ...
    ret = mca_common_ompio_decode_datatype((struct ompio_file_t *) fh, datatype, count, buf,
                                           &max_data, fh->f_mem_convertor, &decoded_iov,
                                           &iov_count);
    if (OMPI_SUCCESS != ret) {
        goto exit;
    }
    ...
    */
    if (fh->f_stripe_count > 1) {
        ...
        dynamic_gen2_num_io_procs = fh->f_get_mca_parameter_value("num_aggregators",
                                                                  strlen("num_aggregators"));
    ...
    for (i = 0; i < dynamic_gen2_num_io_procs; i++) {
        ...
        // to allocate.
        aggr_data[i] = (mca_io_ompio_aggregator_data *)
            calloc(1, sizeof(mca_io_ompio_aggregator_data));
     ...
     ********************************************************************/
    ret = fh->f_generate_current_file_view((struct ompio_file_t *) fh, max_data, &local_iov_array,
                                           &local_count);
    if (ret != OMPI_SUCCESS) {
        goto exit;
    }
    ...
    ret = mca_fcoll_dynamic_gen2_break_file_view(decoded_iov, iov_count, local_iov_array,
                                                 local_count, &broken_decoded_iovs,
                                                 &broken_iov_counts, &broken_iov_arrays,
                                                 &broken_counts, &broken_total_lengths,
    ...
    if (1 == mca_fcoll_dynamic_gen2_num_groups) {
        ...
        total_bytes_per_process = (MPI_Aint *) malloc(dynamic_gen2_num_io_procs
        ...
        ret = ompi_fcoll_base_coll_allgather_array(broken_total_lengths, dynamic_gen2_num_io_procs,
                                                   MPI_LONG, total_bytes_per_process,
                                                   dynamic_gen2_num_io_procs, MPI_LONG, 0,
                                                   fh->f_procs_in_group, fh->f_procs_per_group,
                                                   fh->f_comm);
        if (OMPI_SUCCESS != ret) {
            goto exit;
        }
        for (i = 0; i < dynamic_gen2_num_io_procs; i++) {
            ...
            for (j = 0; j < fh->f_procs_per_group; j++) {
                broken_total_lengths[i] += total_bytes_per_process[j * dynamic_gen2_num_io_procs
                                                                   + i];
            }
        }
        if (NULL != total_bytes_per_process) {
...
#endif
    if (1 == mca_fcoll_dynamic_gen2_num_groups) {
        ret = fh->f_comm->c_coll->coll_allgather(broken_counts, dynamic_gen2_num_io_procs, MPI_INT,
                                                 result_counts, dynamic_gen2_num_io_procs, MPI_INT,
                                                 fh->f_comm,
                                                 fh->f_comm->c_coll->coll_allgather_module);
    } else {
        ret = ompi_fcoll_base_coll_allgather_array(broken_counts, dynamic_gen2_num_io_procs,
                                                   MPI_INT, result_counts,
                                                   dynamic_gen2_num_io_procs, MPI_INT, 0,
                                                   fh->f_procs_in_group, fh->f_procs_per_group,
                                                   fh->f_comm);
    }
    if (OMPI_SUCCESS != ret) {
    ...
    for (i = 0; i < dynamic_gen2_num_io_procs; i++) {
        ...
        if (aggregators[i] == fh->f_rank) {
            for (j = 0; j < fh->f_procs_per_group; i++) {
                printf("%d: PROCESS: %d  ELEMENTS: %d  DISPLS: %d\n", fh->f_rank, j,
                       aggr_data[i]->fview_count[j], displs[j]);
            }
        }
#endif
...
#endif
        if (1 == mca_fcoll_dynamic_gen2_num_groups) {
            ret = fh->f_comm->c_coll->coll_allgatherv(broken_iov_arrays[i], broken_counts[i],
                                                      fh->f_iov_type,
                                                      aggr_data[i]->global_iov_array,
                                                      aggr_data[i]->fview_count, displs,
                                                      fh->f_iov_type, fh->f_comm,
                                                      fh->f_comm->c_coll->coll_allgatherv_module);
        } else {
            ret = ompi_fcoll_base_coll_allgatherv_array(broken_iov_arrays[i], broken_counts[i],
                                                        fh->f_iov_type,
                                                        aggr_data[i]->global_iov_array,
                                                        aggr_data[i]->fview_count, displs,
                                                        fh->f_iov_type, aggregators[i],
                                                        fh->f_procs_in_group, fh->f_procs_per_group,
                                                        fh->f_comm);
        }
        if (OMPI_SUCCESS != ret) {
        ...
        if (aggregators[i] == fh->f_rank) {
            ...
            aggr_data[i]->blocklen_per_process = (int **) calloc(fh->f_procs_per_group,
                                                                 sizeof(int *));
            ...
            aggr_data[i]->displs_per_process = (MPI_Aint **) calloc(fh->f_procs_per_group,
                                                                    sizeof(MPI_Aint *));
    ...
    reqs1 = (ompi_request_t **) malloc((fh->f_procs_per_group + 1) * dynamic_gen2_num_io_procs
                                       * sizeof(ompi_request_t *));
    reqs2 = (ompi_request_t **) malloc((fh->f_procs_per_group + 1) * dynamic_gen2_num_io_procs
                                       * sizeof(ompi_request_t *));
    ...
    if (!mca_common_ompio_full_print_queue(fh->f_coll_write_time)) {
        mca_common_ompio_register_print_entry(fh->f_coll_write_time, nentry);
    }
#endif
...
static int write_init(ompio_file_t *fh, int aggregator, mca_io_ompio_aggregator_data *aggr_data)
{
    ...
    if (aggregator == fh->f_rank && aggr_data->prev_num_io_entries) {
        ...
        while (aggr_data->prev_bytes_to_write > 0) {
            ssize_t tret;
            aggr_data->prev_bytes_to_write
                -= mca_fcoll_dynamic_gen2_split_iov_array(fh, aggr_data->prev_io_array,
                        ...
                        mca_io_ompio_aggregator_data *data, ompi_request_t **reqs)
{
        ...
        */
        ...
        if (data->bytes_remaining) {
            ...
            if (data->bytes_remaining <= data->bytes_to_write_in_cycle) {
                ...
                if (aggregator == rank) {
                    ...
                    data->displs_per_process[data->n][data->disp_index[data->n]]
                        = (ptrdiff_t) data->global_iov_array[data->sorted[data->current_index]]
                              .iov_base
                    ...
                    if (data->disp_index[data->n] == data->max_disp_index[data->n]) {
                        ...
                        data->blocklen_per_process[data->n] = (int *)
                            realloc((void *) data->blocklen_per_process[data->n],
                                    (data->max_disp_index[data->n]) * sizeof(int));
                        data->displs_per_process[data->n] = (MPI_Aint *)
                            realloc((void *) data->displs_per_process[data->n],
            ...
            if (data->bytes_to_write_in_cycle
                ...
                data->bytes_remaining = data->global_iov_array[data->sorted[data->current_index]]
                                            .iov_len
                                        - data->bytes_to_write_in_cycle;
                data->bytes_to_write_in_cycle = 0;
                break;
...
#if DEBUG_ON
        printf("%d: cycle: %d, bytes_sent: %d\n ", rank, index, bytes_sent);
        printf("%d : Entries per aggregator : %d\n", rank, entries_per_aggregator);
#endif

        if (entries_per_aggregator > 0) {
            file_offsets_for_agg = (mca_io_ompio_local_io_array *) malloc(
                entries_per_aggregator * sizeof(mca_io_ompio_local_io_array));
                ...
                for (j = 0; j < data->disp_index[i]; j++) {
                    if (data->blocklen_per_process[i][j] > 0) {
                        ...
                        printf("%d sends blocklen[%d]: %d, disp[%d]: %ld to %d\n",
                               data->procs_in_group[i], j, data->blocklen_per_process[i][j], j,
            ...
            local_heap_sort(file_offsets_for_agg, entries_per_aggregator, sorted_file_offsets);
            ...
            for (i = 0; i < entries_per_aggregator; i++) {
                ...
                global_count += file_offsets_for_agg[sorted_file_offsets[i]].length;
            }
            ...
            for (i = 0; i < data->procs_per_group; i++) {
                ...
                if (0 < data->disp_index[i]) {
                    ...
                    if (datatype_size) {
                        ret = MCA_PML_CALL(
                            irecv(data->global_buf, 1, data->recvtype[i], data->procs_in_group[i],
                                  FCOLL_DYNAMIC_GEN2_SHUFFLE_TAG + index, data->comm, &reqs[i]));
                        if (OMPI_SUCCESS != ret) {
                            goto exit;
                        }
                    }
    ...
    if (bytes_sent) {
        ...
        while (remaining) {
            ...
            if (0 == block_index) {
                ...
                if (0 == block_index % INIT_LEN) {
                    ...
                    blocklength_proc = (int *) realloc(blocklength_proc,
                                                       blocklength_size * sizeof(int));
                    displs_proc = (ptrdiff_t *) realloc(displs_proc,
                                                        blocklength_size * sizeof(ptrdiff_t));
                }
        ...
        if (0 <= block_index) {
            ompi_datatype_create_hindexed(block_index + 1, blocklength_proc, displs_proc, MPI_BYTE,
                                          &newType);
            ompi_datatype_commit(&newType);

            ret = MCA_PML_CALL(isend((char *) send_mem_address, 1, newType, aggregator,
                                     FCOLL_DYNAMIC_GEN2_SHUFFLE_TAG + index,
                                     MCA_PML_BASE_SEND_STANDARD, data->comm,
    ...
    if (aggregator == rank && entries_per_aggregator > 0) {
        ...
        data->io_array[0].offset = (IOVBASE_TYPE *) (intptr_t)
                                       file_offsets_for_agg[sorted_file_offsets[0]]
                                           .offset;
        data->io_array[0].length = file_offsets_for_agg[sorted_file_offsets[0]].length;
        data->io_array[0].memory_address = data->global_buf
                                           + memory_displacements[sorted_file_offsets[0]];
        data->num_io_entries++;
        ...
        for (i = 1; i < entries_per_aggregator; i++) {
            ...
            if (file_offsets_for_agg[sorted_file_offsets[i - 1]].offset
                ...
                data->io_array[data->num_io_entries].offset
                    = (IOVBASE_TYPE *) (intptr_t) file_offsets_for_agg[sorted_file_offsets[i]]
                          .offset;
        ...
        for (i = 0; i < num_of_io_entries; i++) {
            printf(" ADDRESS: %p  OFFSET: %ld   LENGTH: %ld\n", io_array[i].memory_address,
                   (ptrdiff_t) io_array[i].offset, io_array[i].length);
        }
...
int mca_fcoll_dynamic_gen2_break_file_view(
    struct iovec *mem_iov, int mem_count, struct iovec *file_iov, int file_count,
    struct iovec ***ret_broken_mem_iovs, int **ret_broken_mem_counts,
    struct iovec ***ret_broken_file_iovs, int **ret_broken_file_counts,
    MPI_Aint **ret_broken_total_lengths, int stripe_count, int stripe_size)
{
    ...
    if (NULL == broken_mem_counts || NULL == broken_file_counts || NULL == broken_total_lengths) {
        ret = OMPI_ERR_OUT_OF_RESOURCE;
        goto exit;
    }
...
int mca_fcoll_dynamic_gen2_split_iov_array(ompio_file_t *fh, mca_common_ompio_io_array_t *io_array,
                                           int num_entries, int *ret_array_pos, int *ret_pos)
{

    int array_pos = *ret_array_pos;
    int pos = *ret_pos;
    size_t bytes_written = 0;
    off_t baseaddr = ((off_t) io_array[array_pos].offset + pos)
                     - (((off_t) io_array[array_pos].offset + pos) % (off_t) fh->f_stripe_size);
    off_t endaddr = baseaddr + fh->f_stripe_size;

    if (0 == array_pos && 0 == pos) {
        fh->f_io_array = (mca_common_ompio_io_array_t *) malloc(
            num_entries * sizeof(mca_common_ompio_io_array_t));
...
static int local_heap_sort(mca_io_ompio_local_io_array *io_array, int num_entries, int *sorted)
{
    ...
    for (i = num_entries / 2 - 1; i >= 0; i--) {
        ...
        while (!done) {
            ...
            if ((right <= heap_size)
                && (io_array[temp_arr[right]].offset > io_array[temp_arr[largest]].offset)) {
                largest = right;
            }
            if (largest != j) {
