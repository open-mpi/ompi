                                    ...
                                    struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    ...
    for (index = 0; index < cycles; index++) {
            ...
            *********************************************************/
            ...
            for (i = 0; i < fh->f_procs_per_group; i++) {
                ...
                if (0 < disp_index[i]) {
                    ...
                    ret = MCA_PML_CALL(isend(global_buf, 1, sendtype[i], fh->f_procs_in_group[i],
                                             123, MCA_PML_BASE_SEND_STANDARD, fh->f_comm,
                                             &send_req[i]));
                    if (OMPI_SUCCESS != ret) {
                        goto exit;
                    }
        ...
        start_rcomm_time = MPI_Wtime();
#endif
        ret = MCA_PML_CALL(irecv(receive_buf, bytes_received, MPI_BYTE, my_aggregator, 123,
                                 fh->f_comm, &recv_req));
        if (OMPI_SUCCESS != ret) {
            goto exit;
        }
        ...
        if (!recvbuf_is_contiguous) {
            ...
            while (remaining) {
                ...
                if (remaining >= (decoded_iov[iov_index].iov_len - current_position)) {
                    memcpy((IOVBASE_TYPE *) mem_address, receive_buf + temp_position,
                    ...
                    memcpy((IOVBASE_TYPE *) mem_address, receive_buf + temp_position, remaining);
