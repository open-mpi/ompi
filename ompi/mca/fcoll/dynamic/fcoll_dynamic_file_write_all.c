...
int mca_fcoll_dynamic_file_write_all(ompio_file_t *fh, const void *buf, int count,
                                     struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    ...
    if ((ftype_extent == (ptrdiff_t) ftype_size)
        && opal_datatype_is_contiguous_memory_layout(&datatype->super, 1) && 0 == lb) {
    ...
    dynamic_num_io_procs = fh->f_get_mca_parameter_value("num_aggregators",
                                                         strlen("num_aggregators"));
    if (OMPI_ERR_MAX == dynamic_num_io_procs) {
        ret = OMPI_ERROR;
        goto exit;
    }
    ret = mca_common_ompio_set_aggregator_props((struct ompio_file_t *) fh, dynamic_num_io_procs,
                                                max_data);
...
#endif
    ret = ompi_fcoll_base_coll_allgather_array(&max_data, 1, MPI_LONG, total_bytes_per_process, 1,
    ...
    for (i = 0; i < local_count; i++) {
        ...
        printf("%d: OFFSET: %d   LENGTH: %ld\n", fh->f_rank, local_iov_array[i].iov_base,
    ...
    start_comm_time = MPI_Wtime();
#endif
    ret = ompi_fcoll_base_coll_allgather_array(&local_count, 1, MPI_INT, fview_count, 1, MPI_INT, 0,
    ...
    if (0 != total_fview_count) {
        global_iov_array = (struct iovec *) malloc(total_fview_count * sizeof(struct iovec));
    ...
    start_comm_time = MPI_Wtime();
#endif
    ret = ompi_fcoll_base_coll_allgatherv_array(local_iov_array, local_count, fh->f_iov_type,
                                                global_iov_array, fview_count, displs,
                                                fh->f_iov_type, 0, fh->f_procs_in_group,
                                                fh->f_procs_per_group, fh->f_comm);
    if (OMPI_SUCCESS != ret) {
        goto exit;
    }
    ...
    ******************************************************************************************/
    ...
    if (my_aggregator == fh->f_rank) {
        ...
        for (tv = 0; tv < total_fview_count; tv++) {
            printf("%d: OFFSET: %lld   LENGTH: %ld\n", fh->f_rank,
                   global_iov_array[sorted[tv]].iov_base, global_iov_array[sorted[tv]].iov_len);
        }
    }
#endif
    /*************************************************************
    ...
    for (index = 0; index < cycles; index++) {
            ...
            if (bytes_remaining) {
                ...
                if (bytes_remaining <= bytes_to_write_in_cycle) {
                    ...
                    if (my_aggregator == fh->f_rank) {
                        ...
                        blocklen_per_process[n] = (int *) realloc((void *) blocklen_per_process[n],
                                                                  (disp_index[n] + 1)
                                                                      * sizeof(int));
                        displs_per_process[n] = (MPI_Aint *) realloc((void *) displs_per_process[n],
                                                                     (disp_index[n] + 1)
                                                                         * sizeof(MPI_Aint));
                ...
                if (bytes_to_write_in_cycle
                    ...
                    bytes_to_write_in_cycle -= global_iov_array[sorted[current_index]].iov_len;
                    current_index++;
                    continue;
                }
            }
        ...
        if (my_aggregator == fh->f_rank) {
            ...
            if (entries_per_aggregator > 0) {
                ...
                for (i = 0; i < fh->f_procs_per_group; i++) {
                    for (j = 0; j < disp_index[i]; j++) {
                        if (blocklen_per_process[i][j] > 0) {
                            file_offsets_for_agg[temp_index].length = blocklen_per_process[i][j];
                            file_offsets_for_agg[temp_index].process_id = i;
                            file_offsets_for_agg[temp_index].offset = displs_per_process[i][j];
                            temp_index++;
                            ...
                            printf("%d sends blocklen[%d]: %d, disp[%d]: %ld to %d\n",
                                   fh->f_procs_in_group[i], j, blocklen_per_process[i][j], j,
                                   displs_per_process[i][j], fh->f_rank);
#endif
                        }
                    }
                }
            ...
            for (i = 0; i < fh->f_procs_per_group; i++) {
                ...
                if (0 < disp_index[i]) {
                    ompi_datatype_create_hindexed(disp_index[i], blocklen_per_process[i],
                                                  displs_per_process[i], MPI_BYTE, &recvtype[i]);
                    ompi_datatype_commit(&recvtype[i]);
                    opal_datatype_type_size(&recvtype[i]->super, &datatype_size);

                    if (datatype_size) {
                        ret = MCA_PML_CALL(irecv(global_buf, 1, recvtype[i],
                                                 fh->f_procs_in_group[i], 123, fh->f_comm,
                                                 &recv_req[i]));
                        if (OMPI_SUCCESS != ret) {
                            goto exit;
                        }
        ...
        if (sendbuf_is_contiguous) {
            ...
            while (remaining) {
                mem_address = (ptrdiff_t)(decoded_iov[iov_index].iov_base) + current_position;
                ...
                if (remaining >= (decoded_iov[iov_index].iov_len - current_position)) {
                    ...
                    remaining = remaining - (decoded_iov[iov_index].iov_len - current_position);
                    temp_position = temp_position
                                    + (decoded_iov[iov_index].iov_len - current_position);
                    iov_index = iov_index + 1;
                    current_position = 0;
                } else {
                    memcpy(send_buf + temp_position, (IOVBASE_TYPE *) mem_address, remaining);
        ...
        if (bytes_sent) {
            ret = MCA_PML_CALL(isend(send_buf, bytes_sent, MPI_BYTE, my_aggregator, 123,
                                     MCA_PML_BASE_SEND_STANDARD, fh->f_comm, &send_req));
        ...
        if (my_aggregator == fh->f_rank) {
            ret = ompi_request_wait_all(fh->f_procs_per_group, recv_req, MPI_STATUS_IGNORE);
        ...
        if (my_aggregator == fh->f_rank) {
            ...
            for (i = 0; i < fh->f_num_of_io_entries; i++) {
                printf(" ADDRESS: %p  OFFSET: %ld   LENGTH: %ld\n",
                       fh->f_io_array[i].memory_address, (ptrdiff_t) fh->f_io_array[i].offset,
