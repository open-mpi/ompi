...
int ompi_fcoll_base_sort_iovec(struct iovec *iov, int num_entries, int *sorted)
{
    int i = 0;
    int j = 0;
    int left = 0;
    ...
    for (i = num_entries / 2 - 1; i >= 0; i--) {
        ...
        while (!done) {
            left = j * 2 + 1;
            right = j * 2 + 2;
            if ((left <= heap_size) && (iov[temp_arr[left]].iov_base > iov[temp_arr[j]].iov_base)) {
                largest = left;
            } else {
                largest = j;
            }
            if ((right <= heap_size)
                && (iov[temp_arr[right]].iov_base > iov[temp_arr[largest]].iov_base)) {
                largest = right;
            }
            if (largest != j) {
