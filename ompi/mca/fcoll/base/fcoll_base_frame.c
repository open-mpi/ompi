...
MCA_BASE_FRAMEWORK_DECLARE(ompi, fcoll, NULL, NULL, NULL, NULL, mca_fcoll_base_static_components,
                           0);
...
mca_fcoll_base_component_t *mca_fcoll_base_component_lookup(const char *name)
{
    ...
    OPAL_LIST_FOREACH (cli, &ompi_fcoll_base_framework.framework_components,
                       mca_base_component_list_item_t) {
        mca_fcoll_base_component_t *component = (mca_fcoll_base_component_t *) cli->cli_component;
        if (strcmp(component->fcollm_version.mca_component_name, name) == 0) {
            return component;
        }
    }
    return NULL;
