                                   ...
                                   struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    ...
    ret = mca_common_ompio_set_aggregator_props((struct ompio_file_t *) fh, vulcan_num_io_procs,
                                                max_data);
    if (OMPI_SUCCESS != ret) {
        goto exit;
    }
    ...
    for (index = 0; index < cycles; index++) {
            ...
            *********************************************************/
            ...
            for (i = 0; i < fh->f_procs_per_group; i++) {
                ...
                if (0 < disp_index[i]) {
                    ...
                    if (datatype_size) {
                        ret = MCA_PML_CALL(isend(global_buf, 1, sendtype[i],
                                                 fh->f_procs_in_group[i], FCOLL_VULCAN_SHUFFLE_TAG,
                                                 MCA_PML_BASE_SEND_STANDARD, fh->f_comm,
                                                 &send_req[i]));
                        if (OMPI_SUCCESS != ret) {
                            goto exit;
                        }
        ...
        if (bytes_received) {
            ...
            ret = MCA_PML_CALL(irecv((char *) recv_mem_address, 1, newType, my_aggregator,
                                     FCOLL_VULCAN_SHUFFLE_TAG, fh->f_comm, &recv_req));
