...
int mca_fcoll_vulcan_break_file_view(struct iovec *decoded_iov, int iov_count,
                                     ...
                                     MPI_Aint **broken_total_lengths, int stripe_count,
                                     size_t stripe_size);
...
static int mca_fcoll_vulcan_minmax(ompio_file_t *fh, struct iovec *iov, int iov_count,
                                   int num_aggregators, long *new_stripe_size);

int mca_fcoll_vulcan_file_write_all(ompio_file_t *fh, const void *buf, int count,
                                    struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
     ...
     **************************************************************************/
    vulcan_num_io_procs = fh->f_get_mca_parameter_value("num_aggregators",
    ...
    if ((1 == mca_fcoll_vulcan_async_io) && (NULL == fh->f_fbtl->fbtl_ipwritev)) {
        opal_output(1,
                    "vulcan_write_all: fbtl Does NOT support ipwritev() (asynchrounous write) \n");
    ...
    ret = mca_fcoll_vulcan_get_configuration(fh, vulcan_num_io_procs, mca_fcoll_vulcan_num_groups,
                                             max_data);
    if (OMPI_SUCCESS != ret) {
        goto exit;
    }
    ...
    ret = mca_fcoll_vulcan_break_file_view(decoded_iov, iov_count, local_iov_array, local_count,
                                           &broken_decoded_iovs, &broken_iov_counts,
                                           &broken_iov_arrays, &broken_counts,
                                           &broken_total_lengths, fh->f_num_aggrs, domain_size);
    ...
    if (1 == mca_fcoll_vulcan_num_groups) {
        ret = fh->f_comm->c_coll->coll_allreduce(MPI_IN_PLACE, broken_total_lengths,
                                                 fh->f_num_aggrs, MPI_LONG, MPI_SUM, fh->f_comm,
        ...
        total_bytes_per_process = (MPI_Aint *) malloc(fh->f_num_aggrs * fh->f_procs_per_group
                                                      * sizeof(MPI_Aint));
        ...
        ret = ompi_fcoll_base_coll_allgather_array(broken_total_lengths, fh->f_num_aggrs, MPI_LONG,
                                                   total_bytes_per_process, fh->f_num_aggrs,
                                                   MPI_LONG, 0, fh->f_procs_in_group,
                                                   fh->f_procs_per_group, fh->f_comm);
        if (OMPI_SUCCESS != ret) {
            goto exit;
        }
    ...
    if (1 == mca_fcoll_vulcan_num_groups) {
        ...
        ret = ompi_fcoll_base_coll_allgather_array(broken_counts, fh->f_num_aggrs, MPI_INT,
                                                   result_counts, fh->f_num_aggrs, MPI_INT, 0,
    ...
    for (i = 0; i < fh->f_num_aggrs; i++) {
        ...
        if (1 == mca_fcoll_vulcan_num_groups) {
            ...
            ret = ompi_fcoll_base_coll_allgatherv_array(broken_iov_arrays[i], broken_counts[i],
                                                        ...
                                                        aggr_data[i]->global_iov_array,
                                                        aggr_data[i]->fview_count, displs,
                                                        fh->f_iov_type, fh->f_aggr_list[i],
                                                        fh->f_procs_in_group, fh->f_procs_per_group,
                                                        fh->f_comm);
        }
        if (OMPI_SUCCESS != ret) {
    ...
    reqs = (ompi_request_t **) malloc((fh->f_procs_per_group + 1) * fh->f_num_aggrs
    ...
    // In fact it should be: if ((1 == mca_fcoll_vulcan_async_io) && (NULL !=
    // fh->f_fbtl->fbtl_ipwritev)) But we've already tested that.
    if ((1 == mca_fcoll_vulcan_async_io)
        || ((0 == mca_fcoll_vulcan_async_io) && (NULL != fh->f_fbtl->fbtl_ipwritev)
            && (2 < cycles))) {
        write_synch_type = 1;
    }
    ...
    ret = ompi_request_wait_all((fh->f_procs_per_group + 1) * fh->f_num_aggrs, reqs,
                                MPI_STATUS_IGNORE);
                    ...
                    if (datatype_size) {
                        ret = MCA_PML_CALL(
                            irecv(data->global_buf, 1, data->recvtype[i], data->procs_in_group[i],
                                  FCOLL_VULCAN_SHUFFLE_TAG + index, data->comm, &reqs[i]));
                        if (OMPI_SUCCESS != ret) {
                            goto exit;
                        }
                    }
    ...
    if (bytes_sent) {
        ...
        if (0 <= block_index) {
            ...
            ret = MCA_PML_CALL(isend((char *) send_mem_address, 1, newType, aggregator,
                                     FCOLL_VULCAN_SHUFFLE_TAG + index, MCA_PML_BASE_SEND_STANDARD,
                                     data->comm, &reqs[data->procs_per_group]));
                                   ...
                                   int num_aggregators, long *new_stripe_size)
{
    ...
    fh->f_comm->c_coll->coll_allreduce(&min, &globalmin, 1, MPI_LONG, MPI_MIN, fh->f_comm,
                                       fh->f_comm->c_coll->coll_allreduce_module);

    fh->f_comm->c_coll->coll_allreduce(&max, &globalmax, 1, MPI_LONG, MPI_MAX, fh->f_comm,
                                       fh->f_comm->c_coll->coll_allreduce_module);

    //    if ( fh->f_rank < 10 ) printf("[%d]: min=%ld max=%ld globalmin=%ld, globalmax=%ld
...
int mca_fcoll_vulcan_break_file_view(struct iovec *mem_iov, int mem_count, struct iovec *file_iov,
                                     int file_count, struct iovec ***ret_broken_mem_iovs,
                                     int **ret_broken_mem_counts,
                                     struct iovec ***ret_broken_file_iovs,
                                     int **ret_broken_file_counts,
                                     MPI_Aint **ret_broken_total_lengths, int stripe_count,
                                     size_t stripe_size)
{
...
int mca_fcoll_vulcan_split_iov_array(ompio_file_t *fh, mca_common_ompio_io_array_t *io_array,
