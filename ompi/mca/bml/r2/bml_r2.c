...
static int mca_bml_r2_add_btls(void)
{
    ...
    mca_bml_r2.btl_modules = (mca_btl_base_module_t **) malloc(sizeof(mca_btl_base_module_t *)
                                                               * num_btls);
    mca_bml_r2.btl_progress = (mca_btl_base_component_progress_fn_t *) malloc(
        sizeof(mca_btl_base_component_progress_fn_t) * num_btls);

    if (NULL == mca_bml_r2.btl_modules || NULL == mca_bml_r2.btl_progress) {
    ...
    OPAL_LIST_FOREACH (selected_btl, btls, mca_btl_base_selected_module_t) {
        ...
        for (i = 0; NULL != btl_names_argv && NULL != btl_names_argv[i]; ++i) {
            if (0
                == strcmp(btl_names_argv[i], btl->btl_component->btl_version.mca_component_name)) {
                break;
            }
        }
    ...
    qsort(mca_bml_r2.btl_modules, mca_bml_r2.num_btl_modules,
          sizeof(struct mca_btl_base_module_t *), btl_exclusivity_compare);
...
{
    mca_bml_base_btl_t *b1 = (mca_bml_base_btl_t *) v1, *b2 = (mca_bml_base_btl_t *) v2;

    return b2->btl->btl_bandwidth - b1->btl->btl_bandwidth;
}

static void mca_bml_r2_calculate_bandwidth_latency(mca_bml_base_btl_array_t *btl_array,
                                                   double *total_bandwidth, uint32_t *latency)
{
...
static void mca_bml_r2_register_progress(mca_btl_base_module_t *btl, bool hp)
{
    if (NULL != btl->btl_component->btl_progress) {
        ...
        if (found == false || hp) {
            if (found == false) {
                mca_bml_r2.btl_progress[mca_bml_r2.num_btl_progress++] = btl->btl_component
                                                                             ->btl_progress;
            }
...
static int mca_bml_r2_endpoint_add_btl(struct ompi_proc_t *proc,
                                       mca_bml_base_endpoint_t *bml_endpoint,
                                       ...
                                       struct mca_btl_base_endpoint_t *btl_endpoint)
{
    ...
    if ((btl_flags & MCA_BTL_FLAGS_PUT) && (NULL == btl->btl_put)) {
        opal_output(0,
                    "%s: The PUT flag is specified for"
    ...
    if ((btl_flags & MCA_BTL_FLAGS_GET) && (NULL == btl->btl_get)) {
        opal_output(0,
                    "%s: The GET flag is specified for"
        ...
        if (!bml_btl || bml_btl->btl->btl_exclusivity <= btl->btl_exclusivity) {
            ...
            if (NULL == bml_btl || (bml_btl->btl->btl_exclusivity <= btl->btl_exclusivity)) {
                ...
                opal_output(0,
                            ...
                            bml_btl->btl->btl_exclusivity, btl->btl_exclusivity);
                free(errhost);
            }
    ...
    if (((btl_in_use && (btl_flags & MCA_BTL_FLAGS_RDMA))
         || (btl_flags & (MCA_BTL_FLAGS_RDMA | MCA_BTL_FLAGS_ATOMIC_FOPS))
...
static void mca_bml_r2_compute_endpoint_metrics(mca_bml_base_endpoint_t *bml_endpoint)
{
    ...
    qsort(bml_endpoint->btl_send.bml_btls, n_send, sizeof(mca_bml_base_btl_t),
    ...
    for (size_t n_index = 0; n_index < n_send; ++n_index) {
        mca_bml_base_btl_t *bml_btl = mca_bml_base_btl_array_get_index(&bml_endpoint->btl_send,
                                                                       n_index);
        mca_btl_base_module_t *btl = bml_btl->btl;
         ...
         */
        if (btl->btl_latency == latency) {
            mca_bml_base_btl_t *bml_btl_new = mca_bml_base_btl_array_insert(
    ...
    /* sort BTLs in descending order according to bandwidth value */
    qsort(bml_endpoint->btl_rdma.bml_btls, n_rdma, sizeof(mca_bml_base_btl_t),
          btl_bandwidth_compare);
    ...
    for (size_t n_index = 0; n_index < n_rdma; ++n_index) {
        mca_bml_base_btl_t *bml_btl = mca_bml_base_btl_array_get_index(&bml_endpoint->btl_rdma,
                                                                       n_index);
    ...
    if (!btl_in_use) {
        ...
        if (mca_bml_r2.show_unreach_errors) {
            ...
            opal_show_help("help-mca-bml-r2.txt", "unreachable proc", true,
                           OMPI_NAME_PRINT(&(ompi_proc_local_proc->super.proc_name)), localhost,
                           OMPI_NAME_PRINT(&(proc->super.proc_name)), errhost, btl_names);
            free(errhost);
            free(localhost);
        }
...
static int mca_bml_r2_add_procs(size_t nprocs, struct ompi_proc_t **procs,
                                struct opal_bitmap_t *reachable)
{
    ...
    btl_endpoints = (struct mca_btl_base_endpoint_t **) malloc(
        nprocs * sizeof(struct mca_btl_base_endpoint_t *));
    ...
    for (size_t p_index = 0; p_index < mca_bml_r2.num_btl_modules; ++p_index) {
        ...
        rc = btl->btl_add_procs(btl, n_new_procs, (opal_proc_t **) new_procs, btl_endpoints,
                                reachable);
        if (OMPI_SUCCESS != rc) {
    ...
    for (size_t p = 0; p < n_new_procs; ++p) {
        mca_bml_base_endpoint_t *bml_endpoint = (mca_bml_base_endpoint_t *) new_procs[p]
...
static int mca_bml_r2_del_procs(size_t nprocs, struct ompi_proc_t **procs)
{
    for (size_t p = 0; p < nprocs; ++p) {
        ompi_proc_t *proc = procs[p];
        mca_bml_base_endpoint_t *bml_endpoint = (mca_bml_base_endpoint_t *) proc
                                                    ->proc_endpoints[OMPI_PROC_ENDPOINT_TAG_BML];
        ...
        for (size_t r_index = 0; r_index < r_size; ++r_index) {
            mca_bml_base_btl_t *rdma_btl = mca_bml_base_btl_array_get_index(&bml_endpoint->btl_rdma,
                                                                            r_index);
            mca_btl_base_module_t *btl = rdma_btl->btl;
            bool needs_del = true;

            for (size_t f_index = 0; f_index < f_size; ++f_index) {
                mca_bml_base_btl_t *bml_btl = mca_bml_base_btl_array_get_index(&bml_endpoint
                                                                                    ->btl_send,
                                                                               f_index);
            ...
            if (needs_del) {
                int rc = btl->btl_del_procs(btl, 1, (opal_proc_t **) &proc,
                                            &rdma_btl->btl_endpoint);
                if (OPAL_SUCCESS != rc) {
                    return rc;
                }
            }
...
{
    mca_bml_base_endpoint_t *ep = (mca_bml_base_endpoint_t *)
    ...
    if (mca_bml_base_btl_array_remove(&ep->btl_send, btl)) {
        ...
        for (b = 0; b < mca_bml_base_btl_array_get_size(&ep->btl_send); b++) {
            ...
            if (ep_btl->btl_bandwidth > 0) {
                ...
                bml_btl->btl_weight = (float) (1.0
                                               / mca_bml_base_btl_array_get_size(&ep->btl_send));
            }
        }
    }
    ...
    if (mca_bml_base_btl_array_remove(&ep->btl_rdma, btl)) {
        ...
        for (b = 0; b < mca_bml_base_btl_array_get_size(&ep->btl_rdma); b++) {
            ...
            if (ep_btl->btl_bandwidth > 0) {
                ...
                bml_btl->btl_weight = (float) (1.0
                                               / mca_bml_base_btl_array_get_size(&ep->btl_rdma));
            }
        }
    }
 ...
 */
static int mca_bml_r2_register(mca_btl_base_tag_t tag, mca_btl_base_module_recv_cb_fn_t cbfunc,
                               void *data)
{
...
static int mca_bml_r2_register_error(mca_btl_base_module_error_cb_fn_t cbfunc)
{
    ...
    for (i = 0; i < mca_bml_r2.num_btl_modules; i++) {
        ...
        if (ver >= ((1 << 16) | (0 << 8) | 1) && NULL != btl->btl_register_error) {
