...
OBJ_CLASS_INSTANCE(mca_bml_base_btl_array_t, opal_object_t, mca_bml_base_btl_array_construct,
...
static void mca_bml_base_completion(struct mca_btl_base_module_t *btl,
                      ...
                      mca_btl_base_tag_t tag)
{
    ...
    if (mca_bml_base_error_count <= 0 && mca_bml_base_error_rate_ceiling > 0) {
        ...
        if (mca_bml_base_error_count % 2) {
            ...
            /* corrupt data */
            mca_bml_base_context_t *ctx = (mca_bml_base_context_t *) malloc(
                sizeof(mca_bml_base_context_t));
            if (NULL != ctx) {
                opal_output(0, "%s:%d: corrupting data\n", __FILE__, __LINE__);
                ctx->index = (size_t)(
                    (des->des_segments[0].seg_len * opal_rand(&mca_bml_base_rand_buff) * 1.0)
                    / (UINT32_MAX + 1.0));
