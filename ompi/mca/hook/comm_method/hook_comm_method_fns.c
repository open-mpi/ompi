...
static void mystrncpy(char *to, const char *from, int n)
{
    strncpy(to, from, n - 1);
    to[n - 1] = 0;
}
...
static char *lookup_pml_name(void)
{
...
static char *lookup_mtl_name(void)
{
...
static char *lookup_btl_name_for_send(ompi_communicator_t *comm, int rank)
{
    ...
    if (endpoint && endpoint->btl_send.bml_btls && endpoint->btl_send.bml_btls[0].btl) {
        return (char *) endpoint->btl_send.bml_btls[0]
...
static void init_string_to_conversion_struct(comm_method_string_conversion_t *data)
{
    data->n = 0;
...
static int lookup_string_in_conversion_struct(comm_method_string_conversion_t *data, char *string)
{
    int i;
    for (i = 0; i < data->n; ++i) {
...
static void add_string_to_conversion_struct(comm_method_string_conversion_t *data, char *string)
{
    int i;
    if (0 == strcmp(string, "n/a")) {
        return;
    }
...
static int string_to_comm_method(char *str)
{
...
static char *comm_method_to_string(int id)
{
...
typedef void (*VoidFuncPtr)(
    void); // a function pointer to a function that takes no arguments and returns void.
static char *comm_method_string(MPI_Comm comm, int rank, int *comm_mode);
static int comm_method(MPI_Comm comm, int rank);
...
static void ompi_report_comm_methods(int called_from_location) // 1 = from init, 2 = from finalize
{
    int numhosts, i, j, k;
    ...
    if (ompi_mpi_comm_parent != MPI_COMM_NULL) {
        return;
    }
    ...
    ret = ompi_comm_split_type(MPI_COMM_WORLD, MPI_COMM_TYPE_SHARED, 0, NULL, &local_comm);
    ...
    ret = ompi_comm_split(MPI_COMM_WORLD, (0 == mylocalrank) ? 0 : MPI_UNDEFINED, hpmp_myrank,
                          &leader_comm, false);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
...

    {
        ...
        ompi_group_translate_ranks(local_group, nlocalranks, ranklist_in, world_group, ranklist);
        ...
        sprintf(hoststring, "Host %d [%s] ranks ", myleaderrank, opal_process_info.nodename);

        abbreviate_list_into_string(&hoststring[strlen(hoststring)], len - strlen(hoststring),
                                    ranklist, nlocalranks);
        free(ranklist);
    }
    ...
    if (called_from_location == 1) {
        for (i = 0; i <= nleaderranks / 2; ++i) {
            ...
            sbuf = rbuf = 0;
            MCA_PML_CALL(isend(&sbuf, 1, MPI_INT, speer, 99, MCA_PML_BASE_SEND_STANDARD,
                               leader_comm, &sreq));
            MCA_PML_CALL(irecv(&rbuf, 1, MPI_INT, rpeer, 99, leader_comm, &rreq));
    ...
    leader_comm->c_coll->coll_allreduce(MPI_IN_PLACE, (void *) &comm_method_string_conversion, 1,
                                        mydt, myop, leader_comm,
    ...
    // the sizes first and allocate the receiving string.
    {
        ...
        leader_comm->c_coll->coll_gather(&len, 1, MPI_INT, lens, 1, MPI_INT, 0, leader_comm,
                                         leader_comm->c_coll->coll_gather_module);
        if (myleaderrank == 0) {
            int tlen = 0;
            char *p;
            for (i = 0; i < nleaderranks; ++i) {
            ...
            leader_comm->c_coll->coll_gatherv(hoststring, strlen(hoststring) + 1, MPI_CHAR,
                                              &allhoststrings[0][0], lens, disps, MPI_CHAR, 0,
                                              leader_comm,
                                              leader_comm->c_coll->coll_gatherv_module);
        } else {
            // matching above call from rank 0, just &allhoststrings[0][0]
            // isn't legal here, and those args aren't used at non-root anyway
            leader_comm->c_coll->coll_gatherv(hoststring, strlen(hoststring) + 1, MPI_CHAR, NULL,
                                              NULL, NULL, MPI_CHAR, 0, leader_comm,
                                              leader_comm->c_coll->coll_gatherv_module);
        }
        if (myleaderrank == 0) {
            free(lens);
        ...
        leader_comm->c_coll->coll_gather(method, nleaderranks, MPI_INT, method, nleaderranks,
                                         MPI_INT, 0, leader_comm,
                                         leader_comm->c_coll->coll_gather_module);
    }
    ompi_comm_free(&local_comm);
    ompi_comm_free(&leader_comm);
    ...
    if (myleaderrank == 0) {
        ...
        if (nleaderranks <= max2Dprottable) {
            ...
            while (!done) {
                ...
                for (i = 0; i < NUM_COMM_METHODS; ++i) {
                    if (!char_code[i] && method_count[i]) {
                        ...
                        if ((next_idx == -1) || (method_count[i] < method_count[next_idx])) {
                            next_idx = i;
                        }
                    }
                }
            ...
            for (i = 0; i < NUM_COMM_METHODS; ++i) {
                for (k = 0; k < NUM_COMM_METHODS; ++k) {
                    if (char_code[k] == 'A' + i) {
                        printf("key: %c == %s\n", char_code[k], comm_method_to_string(k));
                    }
                }
            }
            printf("\n");
        ...
        // - print ranks with non-majority settings
        {
            ...
            char *btl_etc = "btl";
            if (comm_mode == MODE_IS_MTL) {
                btl_etc = "mtl";
            }
            if (comm_mode == MODE_IS_PML) {
                btl_etc = "pml";
            }
            ...
            for (i = 0; i < nleaderranks; ++i) {
                ...
                if (majority_method_onhost == -1 && this_method != COMM_METHOD_SELF) {
                    majority_method_onhost = this_method;
                }
                if (majority_method_onhost != -1 && this_method != majority_method_onhost
            ...
            for (i = 0; i < NUM_COMM_METHODS; ++i) {
                if (method_count[i] > 0 && majority_method_onhost == COMM_METHOD_SELF) {
            ...
            if (method_count[COMM_METHOD_SELF] > 0) {
                any_self = 1;
            }
            ...
            if (!uniformity_onhost || !uniformity_offhost) {
                ...
                for (i = 0; i < nleaderranks; ++i) {
                    ...
                    for (k = 0; k < nleaderranks; ++k) {
                        ...
                        if (k == i && this_method != majority_method_onhost
                            && this_method != COMM_METHOD_SELF && this_method != 0) {
                            is_nonconformist = 1;
                        }
                        if (k != i && this_method != majority_method_offhost && this_method != 0) {
                            is_nonconformist = 1;
                        }
                    }
                    if (is_nonconformist) {
                        ...
                        for (k = 0; k < NUM_COMM_METHODS; ++k) {
                            if (method_count[k] > 0) {
                                ...
                                //                                  strcat(str, " /");
                                //                              }
                                sprintf(&str[strlen(str)], " [%dx %s]", method_count[k],
