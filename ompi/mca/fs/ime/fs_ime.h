...
int mca_fs_ime_file_open(struct ompi_communicator_t *comm, const char *filename, int amode,
                         struct opal_info_t *info, ompio_file_t *fh);

int mca_fs_ime_file_close(ompio_file_t *fh);

int mca_fs_ime_file_delete(char *filename, struct opal_info_t *info);

int mca_fs_ime_file_set_size(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE size);
