...
#include "ompi/mca/fs/ceph/fs_ceph.h"
 ...
 * *******************************************************************
 */
static mca_fs_base_module_1_0_0_t ceph =  {
    mca_fs_ceph_module_init, /* initialise after being selected */
    mca_fs_ceph_module_finalize, /* close a module on a communicator */
    mca_fs_ceph_file_open,
...
int mca_fs_ceph_component_init_query(bool enable_progress_threads,
...
mca_fs_ceph_component_file_query (ompio_file_t *fh, int *priority)
{
    char *tmp;
    ...
    *priority = mca_fs_ceph_priority;
	...
	if (!strncmp(fh->f_filename, "ceph:", 7) ||
	    !strncmp(fh->f_filename, "CEPH:", 7)) {
            fh->f_fstype = CEPH;
        }
    }
...
int mca_fs_ceph_component_file_unquery (ompio_file_t *file)
{
...
int mca_fs_ceph_module_init (ompio_file_t *file)
{
...
int mca_fs_ceph_module_finalize (ompio_file_t *file)
{
    return OMPI_SUCCESS;
}
