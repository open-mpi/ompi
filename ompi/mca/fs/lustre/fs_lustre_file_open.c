...
/*
 * Map a canonical Lustre layout hint to its historical OMPIO alias.  When the
 * canonical spelling is accepted, the alias is dropped so MPI_File_get_info
 * reports a single spelling regardless of the order the two keys were applied.
 */
static const char *mca_fs_lustre_layout_alias(const char *key)
{
                                                ...
                                                const char *value)
{
    ...
    /*
     * The canonical spelling was accepted.  If the user also supplied the
     * historical alias, drop it now so get_info never reports both spellings,
     * independent of the order in which the two keys were applied.
     */
    alias = mca_fs_lustre_layout_alias(key);
    if (NULL != alias && NULL != object->s_info) {
        opal_info_delete(object->s_info, alias);
    }
