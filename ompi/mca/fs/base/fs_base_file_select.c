...
int mca_fs_base_file_select(struct ompio_file_t *file, mca_base_component_t *preferred)
{
    int priority;
    int best_priority;
    ...
    if (NULL != preferred) {
        ...
        if (NULL != module && NULL != module->fs_module_init) {
    ...
    OPAL_LIST_FOREACH (cli, &ompi_fs_base_framework.framework_components,
