...
int mca_fs_gpfs_file_set_info(ompio_file_t *fh, struct ompi_info_t *info)
{
    ...
    */
    ...
    if (flag) {
        opal_output(ompi_fs_base_framework.framework_output, "GPFS Access Range is set: %s: %s\n",
        ...
        gpfs_hint_AccessRange.gpfsAccessRange.structLen = sizeof(
            gpfs_hint_AccessRange.gpfsAccessRange);
        ...
        if (rc != 0) {
            rc = errno;
            opal_output(
                ompi_fs_base_framework.framework_output,
    ...
    ompi_info_get(info_selected, gpfsHintsKey, &info_str, &flag);
    if (flag) {
        opal_output(ompi_fs_base_framework.framework_output, "GPFS Free Range is set: %s: %s\n",
                    gpfsHintsKey, info_str->string);
        ...
        gpfs_hint_FreeRange.gpfsFreeRange.structLen = sizeof(gpfs_hint_FreeRange.gpfsFreeRange);
    ...
    ompi_info_get_bool(info_selected, gpfsHintsKey, &info_bool, &flag);
    if (flag && info_bool) {
        opal_output(ompi_fs_base_framework.framework_output, "GPFS Clear File Cache is set: %s\n",
                    gpfsHintsKey);
        ...
        gpfs_hint_ClearFileCache.gpfsClearFileCache.structLen = sizeof(
            gpfs_hint_ClearFileCache.gpfsClearFileCache);
    ...
    ompi_info_get_bool(info_selected, gpfsHintsKey, &info_bool, &flag);
    if (flag && info_bool) {
        opal_output(ompi_fs_base_framework.framework_output, "GPFS Cancel Hints is set: %s\n",
                    gpfsHintsKey);
        ...
        gpfs_hint_CancelHints.gpfsCancelHints.structLen = sizeof(
            gpfs_hint_CancelHints.gpfsCancelHints);
    ...
    if (flag) {
        ...
        gpfs_hint_SetReplication.gpfsSetReplication.structLen = sizeof(
            gpfs_hint_SetReplication.gpfsSetReplication);
    ...
    ompi_info_get(info_selected, gpfsHintsKey, &info_str, &flag);
    if (flag) {
        opal_output(ompi_fs_base_framework.framework_output, "GPFS Byte Range is set: %s: %s\n",
                    gpfsHintsKey, info_str->string);
    ...
    ompi_info_get(info_selected, gpfsHintsKey, &info_str, &flag);
    if (flag) {
        opal_output(ompi_fs_base_framework.framework_output, "GPFS Restripe Data is set: %s: %s\n",
                    gpfsHintsKey, info_str->string);
        ...
        gpfs_hint_RestripeData.gpfsRestripeData.structLen = sizeof(
...
// CN: configure option to enable/disable SIOX support?
    ...
    if (siox_gpfs_uiid == SIOX_INVALID_ID) {
        siox_gpfs_uiid = siox_system_information_lookup_interface_id("MPI", "Generic");
        ...
        siox_gpfs_component_activity = siox_component_register_activity(siox_gpfs_uiid,
                                                                        "MPI_File_open");
    }
    // DEBUG: fprintf(stderr, "Beginning the SIOX_activity in mca_fs_gpfs_siox_io_selection()\n");

    fh->f_siox_component = siox_gpfs_component;
    fh->f_siox_activity = siox_activity_begin(siox_gpfs_component, siox_gpfs_component_activity);
    ...
    siox_attribute_array = (siox_attribute **) malloc(sizeof(siox_attribute *) * number_of_info);
    ...
    siox_attribute_array[i] = siox_ontology_register_attribute("MPI", "descriptor/filename",
                                                               SIOX_STORAGE_STRING);
    siox_activity_set_attribute(fh->f_siox_activity, siox_attribute_array[i], fh->f_filename);
    i++;
    ...
    if (flag) {
        ...
        siox_attribute_array[i] = siox_ontology_register_attribute("GPFS", "sioxAccessRange",
                                                                   SIOX_STORAGE_STRING);
        siox_activity_set_attribute(fh->f_siox_activity, siox_attribute_array[i], &value);
        if (siox_suggest_optimal_value_for_str(fh->f_siox_component, siox_attribute_array[i],
                                               fh->f_siox_activity, optimal_value_str, valueLen)) {
    ...
    if (flag) {
        ...
        siox_attribute_array[i] = siox_ontology_register_attribute("GPFS", "sioxFreeRange",
                                                                   SIOX_STORAGE_STRING);
        siox_activity_set_attribute(fh->f_siox_activity, siox_attribute_array[i], &value);
        if (siox_suggest_optimal_value_for_str(fh->f_siox_component, siox_attribute_array[i],
                                               fh->f_siox_activity, optimal_value_str, valueLen)) {
    ...
    if (flag) {
        ...
        siox_attribute_array[i] = siox_ontology_register_attribute("GPFS", "sioxClearFileCache",
                                                                   SIOX_STORAGE_STRING);
        siox_activity_set_attribute(fh->f_siox_activity, siox_attribute_array[i], &value);
        if (siox_suggest_optimal_value_for_str(fh->f_siox_component, siox_attribute_array[i],
                                               fh->f_siox_activity, optimal_value_str, valueLen)) {
    ...
    if (flag) {
        ...
        siox_attribute_array[i] = siox_ontology_register_attribute("GPFS", "sioxCancelHints",
                                                                   SIOX_STORAGE_STRING);
        siox_activity_set_attribute(fh->f_siox_activity, siox_attribute_array[i], &value);
        if (siox_suggest_optimal_value_for_str(fh->f_siox_component, siox_attribute_array[i],
                                               fh->f_siox_activity, optimal_value_str, valueLen)) {
    ...
    if (flag) {
        ...
        siox_attribute_array[i] = siox_ontology_register_attribute("GPFS", "sioxDataShipStart",
                                                                   SIOX_STORAGE_STRING);
        siox_activity_set_attribute(fh->f_siox_activity, siox_attribute_array[i], &value);
        if (siox_suggest_optimal_value_for_str(fh->f_siox_component, siox_attribute_array[i],
                                               fh->f_siox_activity, optimal_value_str, valueLen)) {
    ...
    if (flag) {
        ...
        if (siox_suggest_optimal_value_for_str(fh->f_siox_component, siox_attribute_array[i],
                                               fh->f_siox_activity, optimal_value_str, valueLen)) {
            // DEBUG: fprintf(stderr, "Getting optimal value of sioxSetReplication hints from SIOX:
            // %s \n", optimal_value_str);
            ompi_info_set(info_selected, sioxHintsKey, optimal_value_str);
        }
        i++;
    ...
    if (flag) {
        ...
        siox_attribute_array[i] = siox_ontology_register_attribute("GPFS", "sioxByteRange",
                                                                   SIOX_STORAGE_STRING);
        siox_activity_set_attribute(fh->f_siox_activity, siox_attribute_array[i], &value);
        if (siox_suggest_optimal_value_for_str(fh->f_siox_component, siox_attribute_array[i],
                                               fh->f_siox_activity, optimal_value_str, valueLen)) {
    ...
    if (flag) {
        ...
        siox_attribute_array[i] = siox_ontology_register_attribute("GPFS", "sioxRestripeData",
                                                                   SIOX_STORAGE_STRING);
        siox_activity_set_attribute(fh->f_siox_activity, siox_attribute_array[i], &value);
        if (siox_suggest_optimal_value_for_str(fh->f_siox_component, siox_attribute_array[i],
                                               fh->f_siox_activity, optimal_value_str, valueLen)) {
    ...
    // DEBUG: fprintf(stderr, "Stopping and ending the SIOX activity in
    // mca_fs_gpfs_siox_io_selection()\n");
