...
int mca_sharedfp_sm_request_position(ompio_file_t *fh, int bytes_requested,
                                     OMPI_MPI_OFFSET_TYPE *offset)
{
    ...
    *offset = 0;
    if (mca_sharedfp_sm_verbose) {
        opal_output(ompi_sharedfp_base_framework.framework_output, "Aquiring lock, rank=%d...",
                    fh->f_rank);
    }
    ...
    if (mca_sharedfp_sm_verbose) {
        opal_output(ompi_sharedfp_base_framework.framework_output, "Read last_offset=%lld!\n",
                    old_offset);
    }
    ...
    if (mca_sharedfp_sm_verbose) {
        opal_output(ompi_sharedfp_base_framework.framework_output, "Releasing sm lock...rank=%d",
                    fh->f_rank);
    }
