...
AC_DEFUN([MCA_ompi_sharedfp_sm_CONFIG],[
    ...
    sharedfp_sm_open_happy=no
    sharedfp_sm_init_happy=no
    AC_CHECK_HEADER([semaphore.h],
                    [AC_CHECK_FUNCS([sem_open],[sharedfp_sm_open_happy=yes],[])])


    if test "$sharedfp_sm_open_happy" = "yes" ; then
	sharedfp_sm_happy="yes"
    else
	AC_CHECK_HEADER([semaphore.h],
            [AC_CHECK_FUNCS([sem_init],[sharedfp_sm_init_happy=yes],[])])
	
	if test "$sharedfp_sm_init_happy" = "yes" ; then
	    sharedfp_sm_happy="yes"
	fi
    fi
