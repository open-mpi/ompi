...
int mca_sharedfp_sm_iread(ompio_file_t *fh, void *buf, int count, ompi_datatype_t *datatype,
                          MPI_Request *request)
{
...
int mca_sharedfp_sm_read_ordered_begin(ompio_file_t *fh, void *buf, int count,
                                       struct ompi_datatype_t *datatype)
{
    ...
    */
    ...
    ret = fh->f_comm->c_coll->coll_scatter(buff, sendcnt, OMPI_OFFSET_DATATYPE, &offsetBuff,
                                           recvcnt, OMPI_OFFSET_DATATYPE, 0, fh->f_comm,
...
int mca_sharedfp_sm_read_ordered_end(ompio_file_t *fh, void *buf, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
