                                             ...
                                             ompio_file_t *ompio_fh)
{
    ...
    if (mca_sharedfp_individual_verbose) {
        opal_output(ompi_sharedfp_base_framework.framework_output, "Nodes of each process = %d\n",
                    nodesoneachprocess);
    }
    ...
    ret = ompio_fh->f_comm->c_coll->coll_allgather(&nodesoneachprocess, 1, MPI_INT, countbuff, 1,
                                                   MPI_INT, ompio_fh->f_comm,
    ...
    ret = mca_sharedfp_individual_create_buff(&timestampbuff, &offsetbuff, totalnodes,
                                              ompio_fh->f_size);
    if (OMPI_SUCCESS != ret) {
        goto exit;
    }

    ret = ompio_fh->f_comm->c_coll
              ->coll_allgatherv(ind_ts, countbuff[ompio_fh->f_rank], MPI_DOUBLE, timestampbuff,
                                countbuff, displ, MPI_DOUBLE, ompio_fh->f_comm,
    ...
    ret = ompio_fh->f_comm->c_coll
              ->coll_allgatherv(ind_recordlength, countbuff[ompio_fh->f_rank], OMPI_OFFSET_DATATYPE,
                                offsetbuff, countbuff, displ, OMPI_OFFSET_DATATYPE,
                                ompio_fh->f_comm, ompio_fh->f_comm->c_coll->coll_allgatherv_module);
    if (OMPI_SUCCESS != ret) {
        goto exit;
    }
    ...
    for (i = 0; i < nodesoneachprocess; i++) {
        ...
        ret = mca_common_ompio_file_read_at(headnode->datafilehandle, local_off[i], buff,
                                            ind_recordlength[i], MPI_BYTE, &status);
        if (OMPI_SUCCESS != ret) {
            goto exit;
        }

        idx = mca_sharedfp_individual_getoffset(ind_ts[i], timestampbuff, ranks, ompio_fh->f_rank,
                                                totalnodes);

        if (mca_sharedfp_individual_verbose) {
            opal_output(ompi_sharedfp_base_framework.framework_output,
                        "sharedfp_individual_collaborate_data: Process %d writing %ld bytes to "
                        "main file at position"
                        "%lld (%d)\n",
                        ompio_fh->f_rank, ind_recordlength[i], offsetbuff[idx], idx);
        }

        /*Write into main data file*/
        ret = mca_common_ompio_file_write_at(ompio_fh, offsetbuff[idx], buff, ind_recordlength[i],
                                             MPI_BYTE, &status);
        if (OMPI_SUCCESS != ret) {
            goto exit;
        }
                                                          ...
                                                          struct mca_sharedfp_base_data_t *sh)
{
    ...
    if (headnode->numofrecordsonfile > 0) {
        ...
        for (i = 0; i < headnode->numofrecordsonfile; i++) {
            ...
            ret = mca_common_ompio_file_read_at(headnode->metadatafilehandle, metaoffset, &rec, 32,
                                                MPI_BYTE, &status);
            if (OMPI_SUCCESS != ret) {
                goto exit;
            }
    ...
    while (currnode) {
        ...
        *(*offbuff + ctr) = currnode->localposition;
        ...
        if (mca_sharedfp_individual_verbose) {
            opal_output(ompi_sharedfp_base_framework.framework_output,
                        "sharedfp_individual_get_timestamps_and_reclengths: node deleted from the "
                        "metadatalinked list\n");
        }
        free(currnode);
        currnode = headnode->next;
...
int mca_sharedfp_individual_sort_timestamps(double **ts, MPI_Offset **off, int **ranks,
                                            int totalnodes)
{
...
int mca_sharedfp_individual_getoffset(double timestamp, double *ts, int *ranks, int myrank,
                                      int totalnodes)
{
    int i = 0;
    int notfound = 1;
