...
int mca_sharedfp_individual_iwrite(ompio_file_t *fh, const void *buf, int count,
                                   ompi_datatype_t *datatype, MPI_Request *request)
{
    ...
    ret = mca_common_ompio_file_iwrite_at(headnode->datafilehandle, headnode->datafile_offset, buf,
...
int mca_sharedfp_individual_write_ordered_begin(ompio_file_t *fh, const void *buf, int count,
                                                struct ompi_datatype_t *datatype)
{
    ...
    if (NULL == headnode) {
        opal_output(0,
                    "sharedfp_individual_write_ordered_begin: headnode is NULL but file is open\n");
        return OMPI_ERROR;
    }
    ...
    ret = fh->f_comm->c_coll->coll_gather(&totalbytes, 1, OMPI_OFFSET_DATATYPE, offbuff, 1,
    ...
    ret = fh->f_comm->c_coll->coll_scatter(offbuff, 1, OMPI_OFFSET_DATATYPE, &offset, 1,
                                           OMPI_OFFSET_DATATYPE, 0, fh->f_comm,
    ...
    ret = fh->f_comm->c_coll->coll_bcast(&global_offset, 1, OMPI_OFFSET_DATATYPE, 0, fh->f_comm,
                                         fh->f_comm->c_coll->coll_bcast_module);
    if (OMPI_SUCCESS != ret) {
        opal_output(
            0, "sharedfp_individual_write_ordered_begin: Error while bcasting global offset \n");
        goto exit;
    }
    ...
    if (OMPI_SUCCESS != ret) {
        opal_output(0,
                    "sharedfp_individual_write_ordered_begin: Error while writing the datafile \n");
    }
...
int mca_sharedfp_individual_write_ordered_end(ompio_file_t *fh, const void *buf,
