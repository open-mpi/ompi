...
static int init_query(const mca_base_component_t *m, mca_base_component_list_item_t *entry,
                      bool enable_progress_threads, bool enable_mpi_threads);
static int init_query_2_0_0(const mca_base_component_t *component,
                            mca_base_component_list_item_t *entry, bool enable_progress_threads,
                            bool enable_mpi_threads);

int mca_sharedfp_base_find_available(bool enable_progress_threads, bool enable_mpi_threads)
{
    ...
    while (item != opal_list_get_end(&ompi_sharedfp_base_framework.framework_components)) {
        ...
        if (OMPI_SUCCESS
            != init_query(cli->cli_component, cli, enable_progress_threads, enable_mpi_threads)) {
                      ...
                      bool enable_progress_threads, bool enable_mpi_threads)
{
    ...
    if (2 == m->mca_type_major_version && 0 == m->mca_type_minor_version
        ...
        ret = init_query_2_0_0(m, entry, enable_progress_threads, enable_mpi_threads);
    } else {
...
static int init_query_2_0_0(const mca_base_component_t *component,
                            ...
                            bool enable_mpi_threads)
{
    mca_sharedfp_base_component_2_0_0_t *sharedfp = (mca_sharedfp_base_component_2_0_0_t *)
        component;

    return sharedfp->sharedfpm_init_query(enable_progress_threads, enable_mpi_threads);
}
