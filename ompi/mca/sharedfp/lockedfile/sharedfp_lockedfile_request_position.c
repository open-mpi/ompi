...
int mca_sharedfp_lockedfile_request_position(struct mca_sharedfp_base_data_t *sh,
                                             int bytes_requested, OMPI_MPI_OFFSET_TYPE *offset)
{
    ...
    *offset = 0;
    ...
    if (fcntl(fd, F_SETLKW, &fl) == -1) {
        opal_output(
            0,
            "sharedfp_lockedfile_request_position: errorr acquiring lock: fcntl(%d,F_SETLKW,&fl)\n",
            fd);
        opal_output(0, "sharedfp_lockedfile_request_position: error(%i): %s", errno,
                    strerror(errno));
        return OMPI_ERROR;
    } else {
    ...
    if (mca_sharedfp_lockedfile_verbose) {
        opal_output(ompi_sharedfp_base_framework.framework_output,
                    "sharedfp_lockedfile_request_position: old_offset=%lld, bytes_requested=%d, "
                    "new offset=%lld!\n",
