...
int mca_sharedfp_lockedfile_seek(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE off, int whence)
{
    ...
    if (0 == fh->f_rank) {
        if (MPI_SEEK_SET == whence) {
            ...
            if (offset < 0) {
                opal_output(0,
                            "sharedfp_lockedfile_seek - MPI_SEEK_SET, offset must be > 0,"
                            " got offset=%lld.\n",
                            offset);
                return OMPI_ERROR;
            }
            ...
            if (offset < 0) {
                opal_output(0,
                            "sharedfp_lockedfile_seek - MPI_SEEK_CUR, offset must be > 0, got "
                            "offset=%lld.\n",
                            offset);
                return OMPI_ERROR;
            }
