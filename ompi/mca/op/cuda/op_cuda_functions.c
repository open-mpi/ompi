...
#define FORT_LOC_INT_FUNC(name, type_name, type)                                                 \
    ...
    void ompi_op_cuda_2buff_##name##_##type_name(const void *in, void *inout, int *count,        \
                                                   ...
                                                   struct ompi_op_base_module_1_0_0_t *module) { \
                                                                                                 \
        _Static_assert(sizeof(type) >= sizeof(int8_t) && sizeof(type) <= sizeof(int64_t),        \
                       "Unsupported Fortran integer type");                                      \
        switch(sizeof(type)) {  \
            case sizeof(int8_t):  \
                ompi_op_cuda_2buff_##name##_2int8(in, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int16_t): \
                ompi_op_cuda_2buff_##name##_2int16(in, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int32_t): \
                ompi_op_cuda_2buff_##name##_2int32(in, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int64_t): \
                ompi_op_cuda_2buff_##name##_2int64(in, inout, count, dtype, device, stream, module); \
                break; \
        } \
    }
...
#define FORT_LOC_FLOAT_FUNC(name, type_name, type)                                                  \
    ...
    void ompi_op_cuda_2buff_##name##_##type_name(const void *in, void *inout, int *count,           \
        ...
        switch(sizeof(type)) {  \
            case sizeof(float):  \
                ompi_op_cuda_2buff_##name##_2float(in, inout, count, dtype, device, stream, module);  \
                break;  \
            case sizeof(double): \
                ompi_op_cuda_2buff_##name##_2double(in, inout, count, dtype, device, stream, module); \
                break; \
        } \
    }
...
#define FORT_LOC_INT_FUNC_3BUF(name, type_name, type)                                                       \
    static                                                                                                  \
    void ompi_op_cuda_3buff_##name##_##type_name(const void *in1, const void *in2, void *inout, int *count, \
                                                   struct ompi_datatype_t **dtype,                          \
                                                   int device,                                              \
                                                   opal_accelerator_stream_t *stream,                       \
                                                   struct ompi_op_base_module_1_0_0_t *module) {            \
        ...
        switch(sizeof(type)) {  \
            case sizeof(int8_t):  \
                ompi_op_cuda_3buff_##name##_2int8(in1, in2, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int16_t): \
                ompi_op_cuda_3buff_##name##_2int16(in1, in2, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int32_t): \
                ompi_op_cuda_3buff_##name##_2int32(in1, in2, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int64_t): \
                ompi_op_cuda_3buff_##name##_2int64(in1, in2, inout, count, dtype, device, stream, module); \
                break; \
        } \
    }
...
#define FORT_LOC_FLOAT_FUNC_3BUF(name, type_name, type)                                                         \
    ...
    void ompi_op_cuda_3buff_##name##_##type_name(const void *in1, const void  *in2, void *inout, int *count,    \
                                                   ...
                                                   struct ompi_op_base_module_1_0_0_t *module) {                \
        _Static_assert(sizeof(type) >= sizeof(float) && sizeof(type) <= sizeof(double),                         \
                       "IUnsuported Fortran float size");                                                       \
        switch(sizeof(type)) {  \
            case sizeof(float):  \
                ompi_op_cuda_3buff_##name##_2float(in1, in2, inout, count, dtype, device, stream, module);  \
                break;  \
            case sizeof(double): \
                ompi_op_cuda_3buff_##name##_2double(in1, in2, inout, count, dtype, device, stream, module); \
                break; \
        } \
    }
