...
/**
 * Disable warning about empty macro var-args.
 * We use varargs to suppress expansion of typenames
 * (e.g., int32_t -> int) which could lead to collisions
 * for similar base types. */
        ...
        *device = *target_device;
        ...
        if (0 == target_rc || 0 == source1_rc || *target_device != *source1_device) {
            ...
            if (0 == target_rc) {
                ...
                opal_accelerator.mem_copy_async(*target_device, *source1_device,
                                                *target, orig_target, nbytes, stream,
                                                MCA_ACCELERATOR_TRANSFER_UNSPEC);
            ...
            if (0 == source1_rc || *device != *source1_device) {
                ...
                opal_accelerator.mem_copy_async(*target_device, *source1_device,
                                                *source1, orig_source1, nbytes, stream,
                                                MCA_ACCELERATOR_TRANSFER_UNSPEC);
            }
        ...
        if (NULL != source2_device && *target_device != *source2_device) {
            ...
            opal_accelerator.mem_copy_async(*target_device, *source2_device,
                                            *source2, orig_source2, nbytes, stream,
                                            MCA_ACCELERATOR_TRANSFER_UNSPEC);
        }
    }
...
#define FUNC(name, type_name, type)                                                                             \
    ...
    void ompi_op_rocm_2buff_##name##_##type_name(const void *in, void *inout, int *count,                       \
                                                   ...
                                                   int device,                                                  \
                                                   opal_accelerator_stream_t *stream,                           \
                                                   struct ompi_op_base_module_1_0_0_t *module) __opal_attribute_unused__; \
    static                                                                                                      \
    void ompi_op_rocm_2buff_##name##_##type_name(const void *in, void *inout, int *count,                       \
                                                   struct ompi_datatype_t **dtype,                              \
                                                   int device,                                                  \
...
/* concatenate type_name and type to avoid expansion (e.g., int32_t -> int)  */
#define OP_FUNC(name, type_name, type, ...) FUNC(name, __VA_ARGS__##type_name, __VA_ARGS__##type)

/* reuse the macro above */
 ...
 */
#define LOC_FUNC(name, type_name) FUNC(name, type_name, ompi_op_predefined_##type_name##_t)
...
#define FORT_INT_FUNC(name, type_name, type)                                                     \
    ...
    void ompi_op_rocm_2buff_##name##_##type_name(const void *in, void *inout, int *count,        \
                                                   ...
                                                   struct ompi_op_base_module_1_0_0_t *module) { \
                                                                                                 \
        _Static_assert(sizeof(type) >= sizeof(int8_t) && sizeof(type) <= sizeof(int64_t), "Unsupported integer size (<1B, >8B)");       \
        switch(sizeof(type)) {  \
            case sizeof(int8_t):  \
...
#define FORT_FLOAT_FUNC(name, type_name, type)                                                      \
    ...
    void ompi_op_rocm_2buff_##name##_##type_name(const void *in, void *inout, int *count,           \
                                                   ...
                                                   opal_accelerator_stream_t *stream,               \
                                                   struct ompi_op_base_module_1_0_0_t *module) {    \
        _Static_assert(sizeof(type) >= sizeof(float) && sizeof(type) <= sizeof(long double), "Unsupported float size (<4B, >8B)"); \
        switch(sizeof(type)) {  \
            case sizeof(float):  \
...
#define FORT_LOC_INT_FUNC(name, type_name, type)                                                 \
    static                                                                                       \
    void ompi_op_rocm_2buff_##name##_##type_name(const void *in, void *inout, int *count,        \
                                                   struct ompi_datatype_t **dtype,               \
                                                   int device,                                   \
        ...
        _Static_assert(sizeof(type) >= sizeof(int8_t) && sizeof(type) <= sizeof(int64_t),        \
                       "Unsupported Fortran integer size");                                      \
        switch(sizeof(type)) {  \
            case sizeof(int8_t):  \
                ompi_op_rocm_2buff_##name##_2int8(in, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int16_t): \
                ompi_op_rocm_2buff_##name##_2int16(in, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int32_t): \
                ompi_op_rocm_2buff_##name##_2int32(in, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int64_t): \
                ompi_op_rocm_2buff_##name##_2int64(in, inout, count, dtype, device, stream, module); \
                break; \
        } \
    }

#define FORT_LOC_FLOAT_FUNC(name, type_name, type)                                                  \
    static                                                                                          \
    void ompi_op_rocm_2buff_##name##_##type_name(const void *in, void *inout, int *count,           \
                                                   struct ompi_datatype_t **dtype,                  \
                                                   int device,                                      \
                                                   opal_accelerator_stream_t *stream,               \
                                                   struct ompi_op_base_module_1_0_0_t *module) {    \
        _Static_assert(sizeof(type) >= sizeof(float) && sizeof(type) <= sizeof(double),             \
                       "Unsupported Fortran float size");                                           \
        switch(sizeof(type)) {  \
            case sizeof(float):  \
                ompi_op_rocm_2buff_##name##_2float(in, inout, count, dtype, device, stream, module);  \
                break;  \
            case sizeof(double): \
                ompi_op_rocm_2buff_##name##_2double(in, inout, count, dtype, device, stream, module); \
                break; \
        } \
    }
...
/* Fortran integer */
#if OMPI_HAVE_FORTRAN_INTEGER
FORT_LOC_INT_FUNC(maxloc, 2integer, ompi_fortran_integer_t)
#endif
/* Fortran float */
#if OMPI_HAVE_FORTRAN_REAL
FORT_LOC_FLOAT_FUNC(maxloc, 2real, ompi_fortran_real_t)
#endif
#if OMPI_HAVE_FORTRAN_DOUBLE_PRECISION
FORT_LOC_FLOAT_FUNC(maxloc, 2double_precision, ompi_fortran_double_precision_t)
#endif
...
/* Fortran integer */
#if OMPI_HAVE_FORTRAN_INTEGER
FORT_LOC_INT_FUNC(minloc, 2integer, ompi_fortran_integer_t)
#endif
/* Fortran float */
#if OMPI_HAVE_FORTRAN_REAL
FORT_LOC_FLOAT_FUNC(minloc, 2real, ompi_fortran_real_t)
#endif
#if OMPI_HAVE_FORTRAN_DOUBLE_PRECISION
FORT_LOC_FLOAT_FUNC(minloc, 2double_precision, ompi_fortran_double_precision_t)
#endif
...
#define FUNC_3BUF(name, type_name, type)                                                                        \
    ...
    void ompi_op_rocm_3buff_##name##_##type_name(const void *in1, const void *in2, void *out, int *count,       \
        ...
        device_op_pre(in1, (void**)&source1, &source1_device,                                                   \
                      ...
                      n, *dtype,                                                                                \
                      &threads_per_block, &max_blocks, &device, stream);                                        \
        hipStream_t *hipstream = (hipStream_t*)stream->stream;                                                  \
        ompi_op_rocm_3buff_##name##_##type_name##_submit(source1, source2, target, n, threads_per_block, max_blocks, *hipstream);\
        device_op_post(source1, source1_device, source2, source2_device, out, target, target_device, n, *dtype, device, stream);\
    }


#define OP_FUNC_3BUF(name, type_name, type, ...) FUNC_3BUF(name, __VA_ARGS__##type_name, __VA_ARGS__##type)
 ...
 * This macro is for minloc and maxloc
 */
#define LOC_FUNC_3BUF(name, type_name) FUNC_3BUF(name, type_name, ompi_op_predefined_##type_name##_t)
...
#define FORT_LOC_INT_FUNC_3BUF(name, type_name, type)                                          \
    static                                                                                       \
    void ompi_op_rocm_3buff_##name##_##type_name(const void *in1, const void *in2, void *inout, int *count, \
                                                   struct ompi_datatype_t **dtype,               \
                                                   int device,                                   \
                                                   opal_accelerator_stream_t *stream,            \
                                                   struct ompi_op_base_module_1_0_0_t *module) { \
        ...
        switch(sizeof(type)) {  \
            case sizeof(int8_t):  \
                ompi_op_rocm_3buff_##name##_2int8(in1, in2, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int16_t): \
                ompi_op_rocm_3buff_##name##_2int16(in1, in2, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int32_t): \
                ompi_op_rocm_3buff_##name##_2int32(in1, in2, inout, count, dtype, device, stream, module); \
                break; \
            case sizeof(int64_t): \
                ompi_op_rocm_3buff_##name##_2int64(in1, in2, inout, count, dtype, device, stream, module); \
                break; \
        } \
    }

#define FORT_LOC_FLOAT_FUNC_3BUF(name, type_name, type)                                             \
    static                                                                                          \
    void ompi_op_rocm_3buff_##name##_##type_name(const void *in1, const void *in2, void *inout, int *count, \
                                                   struct ompi_datatype_t **dtype,                  \
                                                   int device,                                      \
        ...
        switch(sizeof(type)) {  \
            case sizeof(float):  \
                ompi_op_rocm_3buff_##name##_2float(in1, in2, inout, count, dtype, device, stream, module);  \
                break;  \
            case sizeof(double): \
                ompi_op_rocm_3buff_##name##_2double(in1, in2, inout, count, dtype, device, stream, module); \
                break; \
        } \
    }
...
/* Fortran integer */
#if OMPI_HAVE_FORTRAN_INTEGER
FORT_LOC_INT_FUNC_3BUF(maxloc, 2integer, ompi_fortran_integer_t)
#endif
/* Fortran float */
#if OMPI_HAVE_FORTRAN_REAL
FORT_LOC_FLOAT_FUNC_3BUF(maxloc, 2real, ompi_fortran_real_t)
#endif
#if OMPI_HAVE_FORTRAN_DOUBLE_PRECISION
FORT_LOC_FLOAT_FUNC_3BUF(maxloc, 2double_precision, ompi_fortran_double_precision_t)
#endif
...
/* Fortran integer */
#if OMPI_HAVE_FORTRAN_INTEGER
FORT_LOC_INT_FUNC_3BUF(minloc, 2integer, ompi_fortran_integer_t)
#endif
/* Fortran float */
#if OMPI_HAVE_FORTRAN_REAL
FORT_LOC_FLOAT_FUNC_3BUF(minloc, 2real, ompi_fortran_real_t)
#endif
#if OMPI_HAVE_FORTRAN_DOUBLE_PRECISION
FORT_LOC_FLOAT_FUNC_3BUF(minloc, 2double_precision, ompi_fortran_double_precision_t)
#endif
