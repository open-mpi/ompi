...
#include <immintrin.h>
/**
 * The following logic is necessary to cope with distro maintainer's desire to change the
 * compilation flags after the configure step, leading to inconsistencies between what OMPI has
 * detected and what code can be generated during make. If we detect that the current code
 * generation architecture has been changed from our own setting and cannot generate the code we
 * need (AVX512, AVX2) we fall back to a lesser support (AVX512 -> AVX2, AVX2 -> AVX, AVX -> error
 * out).
 */
...
#define OMPI_OP_AVX_HAS_FLAGS(_flag) (((_flag) &mca_op_avx_component.flags) == (_flag))
...
#define OP_AVX_FUNC(name, type_sign, type_size, type, op)                      \
    static void OP_CONCAT(ompi_op_avx_2buff_##name##_##type,                   \
                          PREPEND)(const void *_in, void *_out, int *count,    \
    ...
    {                                                                          \
        ...
        while (left_over > 0) {                                                \
            ...
            switch (how_much) {                                                \
            case 8:                                                            \
                out[7] = current_func(out[7], in[7]);                          \
            case 7:                                                            \
                out[6] = current_func(out[6], in[6]);                          \
            case 6:                                                            \
                out[5] = current_func(out[5], in[5]);                          \
            case 5:                                                            \
                out[4] = current_func(out[4], in[4]);                          \
            case 4:                                                            \
                out[3] = current_func(out[3], in[3]);                          \
            case 3:                                                            \
                out[2] = current_func(out[2], in[2]);                          \
            case 2:                                                            \
                out[1] = current_func(out[1], in[1]);                          \
            case 1:                                                            \
                out[0] = current_func(out[0], in[0]);                          \
            }                                                                  \
            left_over -= how_much;                                             \
            out += how_much;                                                   \
            in += how_much;                                                    \
...
#define OP_AVX_FLOAT_FUNC(op)                                                  \
    static void OP_CONCAT(ompi_op_avx_2buff_##op##_float,                      \
                          PREPEND)(const void *_in, void *_out, int *count,    \
                                   struct ompi_datatype_t **dtype,             \
                                   struct ompi_op_base_module_1_0_0_t *module) \
    {                                                                          \
...
#define OP_AVX_DOUBLE_FUNC(op)                                                 \
    static void OP_CONCAT(ompi_op_avx_2buff_##op##_double,                     \
                          PREPEND)(const void *_in, void *_out, int *count,    \
                                   struct ompi_datatype_t **dtype,             \
                                   struct ompi_op_base_module_1_0_0_t *module) \
    {                                                                          \
...
#if defined(GENERATE_SSE3_CODE) && defined(OMPI_MCA_OP_HAVE_SSE41)    \
    && (1 == OMPI_MCA_OP_HAVE_SSE41) && defined(OMPI_MCA_OP_HAVE_AVX) \
    && (1 == OMPI_MCA_OP_HAVE_AVX)
#    if __SSE3__ && __SSE2__
...
#define OP_AVX_FUNC_3(name, type_sign, type_size, type, op)                              \
    static void OP_CONCAT(ompi_op_avx_3buff_##name##_##type,                             \
                          PREPEND)(const void *restrict _in1, const void *restrict _in2, \
    ...
    {                                                                                    \
        ...
        while (left_over > 0) {                                                          \
            ...
            switch (how_much) {                                                          \
            case 8:                                                                      \
                out[7] = current_func(in1[7], in2[7]);                                   \
            case 7:                                                                      \
                out[6] = current_func(in1[6], in2[6]);                                   \
            case 6:                                                                      \
                out[5] = current_func(in1[5], in2[5]);                                   \
            case 5:                                                                      \
                out[4] = current_func(in1[4], in2[4]);                                   \
            case 4:                                                                      \
                out[3] = current_func(in1[3], in2[3]);                                   \
            case 3:                                                                      \
                out[2] = current_func(in1[2], in2[2]);                                   \
            case 2:                                                                      \
                out[1] = current_func(in1[1], in2[1]);                                   \
            case 1:                                                                      \
                out[0] = current_func(in1[0], in2[0]);                                   \
            }                                                                            \
            left_over -= how_much;                                                       \
            out += how_much;                                                             \
            in1 += how_much;                                                             \
...
#define OP_AVX_BIT_FUNC_3(name, type_size, type, op)                                           \
    static void OP_CONCAT(ompi_op_avx_3buff_##op##_##type,                                     \
                          PREPEND)(const void *_in1, const void *_in2, void *_out, int *count, \
                                   struct ompi_datatype_t **dtype,                             \
                                   struct ompi_op_base_module_1_0_0_t *module)                 \
    {                                                                                          \
        int types_per_step, left_over = *count;                                                \
...
#define OP_AVX_FLOAT_FUNC_3(op)                                                                \
    static void OP_CONCAT(ompi_op_avx_3buff_##op##_float,                                      \
                          PREPEND)(const void *_in1, const void *_in2, void *_out, int *count, \
                                   struct ompi_datatype_t **dtype,                             \
                                   struct ompi_op_base_module_1_0_0_t *module)                 \
    {                                                                                          \
...
#define OP_AVX_DOUBLE_FUNC_3(op)                                                               \
    static void OP_CONCAT(ompi_op_avx_3buff_##op##_double,                                     \
                          PREPEND)(const void *_in1, const void *_in2, void *_out, int *count, \
                                   struct ompi_datatype_t **dtype,                             \
                                   struct ompi_op_base_module_1_0_0_t *module)                 \
    {                                                                                          \
...
#    define C_INTEGER_OPTIONAL(name, ftype)                                                   \
        C_INTEGER_8_16_32(name, ftype),                                                       \
            [OMPI_OP_BASE_TYPE_INT64_T] = OP_CONCAT(ompi_op_avx_##ftype##_##name##_int64_t,   \
                                                    PREPEND),                                 \
            [OMPI_OP_BASE_TYPE_UINT64_T] = OP_CONCAT(ompi_op_avx_##ftype##_##name##_uint64_t, \
                                                     PREPEND)
#else
#    define C_INTEGER_OPTIONAL(name, ftype) C_INTEGER_8_16_32(name, ftype)
#endif
...
#define FLOATING_POINT(name, ftype)                                                         \
    [OMPI_OP_BASE_TYPE_SHORT_FLOAT] = NULL, [OMPI_OP_BASE_TYPE_FLOAT] = FLOAT(name, ftype), \
