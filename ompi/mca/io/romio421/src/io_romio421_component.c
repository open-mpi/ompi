...
#define ROMIO_VERSION_STRING "from MPICH v4.2.1"
 ...
 */
opal_mutex_t mca_io_romio421_mutex = {{0}};


/*
 * Public string showing this component's version number
 */
const char *mca_io_romio421_component_version_string =
"OMPI/MPI ROMIO io MCA component version " OMPI_VERSION ", " ROMIO_VERSION_STRING;


mca_io_base_component_3_0_0_t mca_io_romio421_component = {
    /* First, the mca_base_component_t struct containing meta information
       about the component itself */

    .io_version = {
        MCA_IO_BASE_VERSION_3_0_0,
        .mca_component_name = "romio421",
...
static char *ompi_io_romio421_version = NULL;
static char *ompi_io_romio421_user_configure_params = NULL;
static char *ompi_io_romio421_complete_configure_params = NULL;

static int register_component(void)
{
    /* Use a low priority, but allow other components to be lower */
    priority_param = 10;
    (void) mca_base_component_var_register(&mca_io_romio421_component.io_version,
    ...
    ompi_io_romio421_version = ROMIO_VERSION_STRING;
    (void) mca_base_component_var_register(&mca_io_romio421_component.io_version,
                                           "version", "Version of ROMIO", MCA_BASE_VAR_TYPE_STRING,
                                           NULL, 0, MCA_BASE_VAR_FLAG_DEFAULT_ONLY,
                                           OPAL_INFO_LVL_9,
                                           MCA_BASE_VAR_SCOPE_READONLY, &ompi_io_romio421_version);
    ompi_io_romio421_user_configure_params = MCA_io_romio421_USER_CONFIGURE_FLAGS;
    (void) mca_base_component_var_register(&mca_io_romio421_component.io_version,
                                           ...
                                           OPAL_INFO_LVL_9,
                                           MCA_BASE_VAR_SCOPE_READONLY, &ompi_io_romio421_user_configure_params);
    ompi_io_romio421_complete_configure_params = MCA_io_romio421_COMPLETE_CONFIGURE_FLAGS;
    (void) mca_base_component_var_register(&mca_io_romio421_component.io_version,
                                           ...
                                           MCA_BASE_VAR_FLAG_DEFAULT_ONLY,
                                           OPAL_INFO_LVL_9,
                                           MCA_BASE_VAR_SCOPE_READONLY, &ompi_io_romio421_complete_configure_params);
...
{
    /* Create the mutex */
    OBJ_CONSTRUCT(&mca_io_romio421_mutex, opal_mutex_t);
...
static int close_component(void)
{
    OBJ_DESTRUCT(&mca_io_romio421_mutex);
           ...
           int *priority)
{
    ...
    *priority = priority_param;
    ...
    data = malloc(sizeof(mca_io_romio421_data_t));
    if (NULL == data) {
        return NULL;
    }
    data->romio_fh = NULL;
...
// isn't ideal but it only happens a few places.
    ...
    OPAL_THREAD_LOCK (&mca_io_romio421_mutex);
    ret = ROMIO_PREFIX(MPI_File_delete)(filename, &ompi_info);
    OPAL_THREAD_UNLOCK (&mca_io_romio421_mutex);
