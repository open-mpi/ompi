                            ...
                            mca_base_component_t *preferred)
{
    ...
    if (!strcmp (selected.ai_component.v2_0_0.io_version.mca_component_name,
        ...
        if (OMPI_SUCCESS != mca_base_framework_open(&ompi_fcoll_base_framework, MCA_BASE_OPEN_DEFAULT)) {
            opal_mutex_unlock(&ompi_mpi_ompio_bootstrap_mutex);
            return err;
        }
        if (OMPI_SUCCESS != mca_base_framework_open(&ompi_fbtl_base_framework, MCA_BASE_OPEN_DEFAULT)) {
            opal_mutex_unlock(&ompi_mpi_ompio_bootstrap_mutex);
            return err;
        }
        if (OMPI_SUCCESS != mca_base_framework_open(&ompi_sharedfp_base_framework, MCA_BASE_OPEN_DEFAULT)) {
            opal_mutex_unlock(&ompi_mpi_ompio_bootstrap_mutex);
            return err;
        }
        opal_mutex_unlock(&ompi_mpi_ompio_bootstrap_mutex);
        ...
        if (OMPI_SUCCESS !=
            mca_fcoll_base_find_available(OPAL_ENABLE_PROGRESS_THREADS, 1)) {
            return err;
        }
        if (OMPI_SUCCESS !=
            mca_fbtl_base_find_available(OPAL_ENABLE_PROGRESS_THREADS, 1)) {
            return err;
        }
        if (OMPI_SUCCESS !=
            mca_sharedfp_base_find_available(OPAL_ENABLE_PROGRESS_THREADS, 1)) {
            return err;
        }
