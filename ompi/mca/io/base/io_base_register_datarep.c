...
int mca_io_base_register_datarep(const char *datarep, MPI_Datarep_conversion_function *read_fn,
                                 ...
                                 MPI_Datarep_extent_function *extent_fn, void *state)
{
    ...
    OPAL_LIST_FOREACH (cli, &ompi_io_base_framework.framework_components,
                       mca_base_component_list_item_t) {
        component = cli->cli_component;

        /* Only know how to handle v2.0.0 components for now */
        if (component->mca_type_major_version == 2 && component->mca_type_minor_version == 0
            && component->mca_type_release_version == 0) {
            v200 = (mca_io_base_component_2_0_0_t *) component;

            /* return first non-good error-code */
            tmp = v200->io_register_datarep(datarep, read_fn, write_fn, extent_fn, state);
