...
int mca_io_ompio_file_read(ompi_file_t *fp, void *buf, int count, struct ompi_datatype_t *datatype,
...
int mca_io_ompio_file_read_at(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, void *buf, int count,
...
int mca_io_ompio_file_iread(ompi_file_t *fh, void *buf, int count, struct ompi_datatype_t *datatype,
...
int mca_io_ompio_file_iread_at(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, void *buf, int count,
...
/******************************************************/
int mca_io_ompio_file_read_all(ompi_file_t *fh, void *buf, int count,
                               struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
    ...
    OPAL_THREAD_LOCK(&fh->f_lock);
    ret = mca_common_ompio_file_read_all(&data->ompio_fh, buf, count, datatype, status);
    OPAL_THREAD_UNLOCK(&fh->f_lock);
                                ...
                                struct ompi_datatype_t *datatype, ompi_request_t **request)
{
    ...
    OPAL_THREAD_LOCK(&fh->f_lock);
    ret = mca_common_ompio_file_iread_all(&data->ompio_fh, buf, count, datatype, request);
    OPAL_THREAD_UNLOCK(&fh->f_lock);

    return ret;
}

int mca_io_ompio_file_read_at_all(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, void *buf,
                                   ...
                                   ompi_request_t **request)
{
    ...
    OPAL_THREAD_LOCK(&fh->f_lock);
    ret = mca_common_ompio_file_iread_at_all(&data->ompio_fh, offset, buf, count, datatype,
                                             request);
    OPAL_THREAD_UNLOCK(&fh->f_lock);
    return ret;
}
...
/******************************************************/
int mca_io_ompio_file_read_shared(ompi_file_t *fp, void *buf, int count,
                                  struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
    ...
    if (NULL == shared_fp_base_module) {
        opal_output(
            0,
            "No shared file pointer component found for the given communicator. Can not execute\n");
        return OMPI_ERROR;
    }
    OPAL_THREAD_LOCK(&fp->f_lock);
...
int mca_io_ompio_file_iread_shared(ompi_file_t *fh, void *buf, int count,
                                   struct ompi_datatype_t *datatype, ompi_request_t **request)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
...
int mca_io_ompio_file_read_ordered(ompi_file_t *fh, void *buf, int count,
                                   struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
...
int mca_io_ompio_file_read_ordered_begin(ompi_file_t *fh, void *buf, int count,
...
int mca_io_ompio_file_read_ordered_end(ompi_file_t *fh, void *buf, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
    ompio_file_t *ompio_fh;
...
/**********************************************************************/
int mca_io_ompio_file_read_all_begin(ompi_file_t *fh, void *buf, int count,
                                     struct ompi_datatype_t *datatype)
{
    int ret = OMPI_SUCCESS;
    ompio_file_t *fp;
...
int mca_io_ompio_file_read_all_end(ompi_file_t *fh, void *buf, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    ompio_file_t *fp;
    mca_common_ompio_data_t *data;
...
int mca_io_ompio_file_read_at_all_begin(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, void *buf,
                                        int count, struct ompi_datatype_t *datatype)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
    ...
    OPAL_THREAD_LOCK(&fh->f_lock);
    ret = mca_common_ompio_file_iread_at_all(fp, offset, buf, count, datatype,
                                             &fp->f_split_coll_req);
    OPAL_THREAD_UNLOCK(&fh->f_lock);
    fp->f_split_coll_in_use = true;
    return ret;
...
int mca_io_ompio_file_read_at_all_end(ompi_file_t *fh, void *buf, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
    ompio_file_t *fp = NULL;
