...
int mca_io_ompio_file_write(ompi_file_t *fp, const void *buf, int count,
...
int mca_io_ompio_file_write_at(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, const void *buf,
...
int mca_io_ompio_file_iwrite(ompi_file_t *fp, const void *buf, int count,
...
int mca_io_ompio_file_iwrite_at(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, const void *buf,
                                int count, struct ompi_datatype_t *datatype,
...
int mca_io_ompio_file_write_all(ompi_file_t *fh, const void *buf, int count,
                                struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
    ...
    OPAL_THREAD_LOCK(&fh->f_lock);
    ret = mca_common_ompio_file_write_all(&data->ompio_fh, buf, count, datatype, status);
    OPAL_THREAD_UNLOCK(&fh->f_lock);
...
int mca_io_ompio_file_write_at_all(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, const void *buf,
                                   int count, struct ompi_datatype_t *datatype,
                                   ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
...
int mca_io_ompio_file_iwrite_all(ompi_file_t *fh, const void *buf, int count,
                                 struct ompi_datatype_t *datatype, ompi_request_t **request)
{
    int ret = OMPI_SUCCESS;
    ...
    OPAL_THREAD_LOCK(&fh->f_lock);
    ret = mca_common_ompio_file_iwrite_all(&data->ompio_fh, buf, count, datatype, request);
    OPAL_THREAD_UNLOCK(&fh->f_lock);

    return ret;
}

int mca_io_ompio_file_iwrite_at_all(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, const void *buf,
                                    int count, struct ompi_datatype_t *datatype,
                                    ompi_request_t **request)
{
    int ret = OMPI_SUCCESS;
    ...
    data = (mca_common_ompio_data_t *) fh->f_io_selected_data;
    OPAL_THREAD_LOCK(&fh->f_lock);
    ret = mca_common_ompio_file_iwrite_at_all(&data->ompio_fh, offset, buf, count, datatype,
                                              request);
    OPAL_THREAD_UNLOCK(&fh->f_lock);
    return ret;
}
...
int mca_io_ompio_file_write_shared(ompi_file_t *fp, const void *buf, int count,
                                   struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
    ...
    if (NULL == shared_fp_base_module) {
        opal_output(
            0, "No shared file pointer component found for this communicator. Can not execute\n");
        return OMPI_ERROR;
    }
    OPAL_THREAD_LOCK(&fp->f_lock);
...
int mca_io_ompio_file_iwrite_shared(ompi_file_t *fp, const void *buf, int count,
                                    struct ompi_datatype_t *datatype, ompi_request_t **request)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
...
int mca_io_ompio_file_write_ordered(ompi_file_t *fp, const void *buf, int count,
                                    struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
...
int mca_io_ompio_file_write_ordered_begin(ompi_file_t *fp, const void *buf, int count,
...
int mca_io_ompio_file_write_ordered_end(ompi_file_t *fp, const void *buf,
                                        ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
...
/**********************************************************************/
int mca_io_ompio_file_write_all_begin(ompi_file_t *fh, const void *buf, int count,
                                      struct ompi_datatype_t *datatype)
{
    int ret = OMPI_SUCCESS;
    ompio_file_t *fp;
    ...
    if (true == fp->f_split_coll_in_use) {
        printf("Only one split collective I/O operation allowed per file handle at any given point "
               "in time!\n");
...
int mca_io_ompio_file_write_all_end(ompi_file_t *fh, const void *buf, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    ompio_file_t *fp;
    mca_common_ompio_data_t *data;
...
int mca_io_ompio_file_write_at_all_begin(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset,
                                         ...
                                         struct ompi_datatype_t *datatype)
{
    ...
    OPAL_THREAD_LOCK(&fh->f_lock);
    ret = mca_common_ompio_file_iwrite_at_all(fp, offset, buf, count, datatype,
                                              &fp->f_split_coll_req);
    OPAL_THREAD_UNLOCK(&fh->f_lock);
    fp->f_split_coll_in_use = true;
...
int mca_io_ompio_file_write_at_all_end(ompi_file_t *fh, const void *buf,
                                       ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
