...
int mca_io_ompio_file_open(ompi_communicator_t *comm, const char *filename, int amode,
...
int mca_io_ompio_file_preallocate(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE diskspace)
{
    ...
    ret = data->ompio_fh.f_comm->c_coll
              ->coll_bcast(&tmp, 1, OMPI_OFFSET_DATATYPE, OMPIO_ROOT, data->ompio_fh.f_comm,
    ...
    ret = data->ompio_fh.f_fs->fs_file_get_size(&data->ompio_fh, &current_size);
    if (OMPI_SUCCESS != ret) {
        OPAL_THREAD_UNLOCK(&fh->f_lock);
        return OMPI_ERROR;
    }
    ...
    */
    if (OMPIO_ROOT == data->ompio_fh.f_rank) {
        ...
        cycles = (size + OMPIO_PREALLOC_MAX_BUF_SIZE - 1) / OMPIO_PREALLOC_MAX_BUF_SIZE;
...
int mca_io_ompio_file_set_size(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE size)
{
    int ret = OMPI_SUCCESS;
    ...
    ret = data->ompio_fh.f_comm->c_coll
              ->coll_barrier(data->ompio_fh.f_comm,
...
int mca_io_ompio_file_get_size(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE *size)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
...
int mca_io_ompio_file_get_amode(ompi_file_t *fh, int *amode)
{
    mca_common_ompio_data_t *data;
...
int mca_io_ompio_file_get_type_extent(ompi_file_t *fh, struct ompi_datatype_t *datatype,
                                      MPI_Aint *extent)
{
...
int mca_io_ompio_file_set_atomicity(ompi_file_t *fh, int flag)
{
    ...
    tmp = flag;
    data->ompio_fh.f_comm->c_coll->coll_bcast(&tmp, 1, MPI_INT, OMPIO_ROOT, data->ompio_fh.f_comm,
                                              data->ompio_fh.f_comm->c_coll->coll_bcast_module);
...
int mca_io_ompio_file_get_atomicity(ompi_file_t *fh, int *flag)
{
    mca_common_ompio_data_t *data;
...
static void mca_io_ompio_file_get_eof_offset(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE in_offset,
    ...
    */
    ...
    if (fh->f_view_size > 0) {
        ...
        while (offset <= in_offset && index_in_file_view < fh->f_iov_count) {
            prev_offset = offset;
            offset = start_offset
                     + (OMPI_MPI_OFFSET_TYPE)(intptr_t) fh->f_decoded_iov[index_in_file_view++]
                           .iov_base;
        }
...
int mca_io_ompio_file_seek(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE off, int whence)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
    ...
    case MPI_SEEK_SET:
        ...
        break;
    case MPI_SEEK_CUR:
        ret = mca_common_ompio_file_get_position(&data->ompio_fh, &temp_offset);
        ...
        break;
    case MPI_SEEK_END:
        ret = data->ompio_fh.f_fs->fs_file_get_size(&data->ompio_fh, &temp_offset2);
        mca_io_ompio_file_get_eof_offset(&data->ompio_fh, temp_offset2, &temp_offset);
        offset += temp_offset;
...
int mca_io_ompio_file_get_position(ompi_file_t *fd, OMPI_MPI_OFFSET_TYPE *offset)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data = NULL;
...
int mca_io_ompio_file_get_byte_offset(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset,
...
int mca_io_ompio_file_seek_shared(ompi_file_t *fp, OMPI_MPI_OFFSET_TYPE offset, int whence)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
    ompio_file_t *fh;
...
int mca_io_ompio_file_get_position_shared(ompi_file_t *fp, OMPI_MPI_OFFSET_TYPE *offset)
{
    int ret = OMPI_SUCCESS;
    mca_common_ompio_data_t *data;
    ompio_file_t *fh;
