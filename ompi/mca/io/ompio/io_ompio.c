...
int ompi_io_ompio_generate_current_file_view(struct ompio_file_t *fh, size_t max_data,
                                             struct iovec **f_iov, int *iov_count)
{
    ...
    iov = (struct iovec *) calloc(OMPIO_IOVEC_INITIAL_SIZE, sizeof(struct iovec));
    if (NULL == iov) {
    ...
    while (bytes_to_write) {
        ...
        if (fh->f_decoded_iov[j].iov_len - (fh->f_total_bytes - sum_previous_counts) <= 0) {
        ...
        if ((fh->f_decoded_iov[j].iov_len - (fh->f_total_bytes - sum_previous_counts))
            >= bytes_to_write) {
    ...
    *f_iov = iov;
    ...
    if (mca_io_ompio_record_offset_info) {
        ...
        fh->f_comm->c_coll->coll_gather(&k, 1, MPI_INT, recvcounts, 1, MPI_INT, OMPIO_ROOT,
                                        fh->f_comm, fh->f_comm->c_coll->coll_gather_module);

        per_process = (mca_io_ompio_offlen_array_t *) malloc(k
                                                             * sizeof(mca_io_ompio_offlen_array_t));
        if (NULL == per_process) {
        ...
        for (i = 0; i < k; i++) {
            per_process[i].offset = (OMPI_MPI_OFFSET_TYPE)(intptr_t) iov[i].iov_base;
            per_process[i].length = (MPI_Aint) iov[i].iov_len;
        ...
        ompi_datatype_create_struct(3, blocklen, d, types, &io_array_type);
        ...
        if (OMPIO_ROOT == fh->f_rank) {
            ...
            all_process = (mca_io_ompio_offlen_array_t *) malloc(
                tot_entries * sizeof(mca_io_ompio_offlen_array_t));
            if (NULL == all_process) {
                opal_output(1, "Error while allocating per process!\n");
                free(per_process);
                free(recvcounts);
            ...
            sorted = (int *) malloc(tot_entries * sizeof(int));
            if (NULL == sorted) {
                opal_output(1, "Error while allocating per process!\n");
                free(all_process);
                free(per_process);
            ...
            adj_matrix = (int **) malloc(fh->f_size * sizeof(int *));
            if (NULL == adj_matrix) {
                opal_output(1, "Error while allocating per process!\n");
                free(sorted);
                free(all_process);
            ...
            for (i = 0; i < fh->f_size; i++) {
                adj_matrix[i] = (int *) malloc(fh->f_size * sizeof(int));
        ...
        fh->f_comm->c_coll->coll_gatherv(per_process, k, io_array_type, all_process, recvcounts,
                                         displs, io_array_type, OMPIO_ROOT, fh->f_comm,
        ...
        if (OMPIO_ROOT == fh->f_rank) {
            ...
            ompi_io_ompio_sort_offlen(all_process, tot_entries, sorted);
            ...
            row_index = (int *) malloc((fh->f_size + 1) * sizeof(int));
            if (NULL == row_index) {
...
int ompi_io_ompio_sort_offlen(mca_io_ompio_offlen_array_t *io_array, int num_entries, int *sorted)
{
