 ...
 * caller.
 */
int mca_part_base_select(bool enable_progress_threads, bool enable_mpi_threads)
{
    ...
    OPAL_LIST_FOREACH (cli, &ompi_part_base_framework.framework_components,
        ...
        for (i = 0; i < opal_pointer_array_get_size(&mca_part_base_part); i++) {
            ...
            if (0
                == strncmp(component->partm_version.mca_component_name, tmp_val,
                           strlen(component->partm_version.mca_component_name))) {
                found_part = true;
                break;
            }
        }
        ...
        module = component->partm_init(&priority, enable_progress_threads, enable_mpi_threads);
        if (NULL == module) {
            opal_output_verbose(10, ompi_part_base_framework.framework_output,
    ...
    if (NULL == best_component) {
        opal_show_help("help-mca-base.txt", "find-available:none found", true, "part",
                       opal_process_info.nodename, "part");
        for (i = 0; i < opal_pointer_array_get_size(&mca_part_base_part); i++) {
            char *tmp_val = NULL;
            tmp_val = (char *) opal_pointer_array_get_item(&mca_part_base_part, i);
            if (NULL == tmp_val) {
