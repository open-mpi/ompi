...
struct mca_part_persist_request_t {
    ...
    volatile int32_t
        req_part_complete; /**< flag indicating if the pt-2-pt layer is done with this request */
    ...
    struct ompi_mca_persist_setup_t
        setup_info[2]; /**< Setup info to send durring initialization. */

    struct mca_part_persist_list_t
        *progress_elem; /**< pointer to progress list element for removal durring free. */
};
