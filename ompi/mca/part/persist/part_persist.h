...
struct ompi_part_persist_t {
    ...
    ompi_communicator_t *part_comm; /* This approach requires a separate tag space, so we need a
                                       dedicated communicator. */
    ompi_request_t *part_comm_req;
    int32_t part_comm_ready;
    ompi_communicator_t
        *part_comm_setup; /* We create a second communicator to send set-up messages (rational:
                             these messages go in the opposite direction of normal messages, need to
                             use MPI_ANY_SOURCE to support different communicators, and thus need to
                             have a unique tag. Because tags are controled by the sender in this
                             model, we cannot assume that the tag will be unused in part_comm. */
    ompi_request_t *part_comm_sreq;
    int32_t part_comm_sready;
    int32_t init_comms;
    int32_t init_world;
    int32_t my_world_rank; /* Because the back end communicators use a world rank, we need to
                              communicate ours to set up the requests. */
...
/**
 * This is a helper function that frees a request. This requires ompi_part_persist.lock be held
 * before calling.
 */
...
{
    opal_free_list_init(&mca_part_base_precv_requests, sizeof(mca_part_persist_precv_request_t),
                        opal_cache_line_size, OBJ_CLASS(mca_part_persist_precv_request_t), 0,
                        opal_cache_line_size, ompi_part_persist.free_list_num,
                        ompi_part_persist.free_list_max, ompi_part_persist.free_list_inc, NULL, 0,
                        NULL, NULL, NULL);
    opal_free_list_init(&mca_part_base_psend_requests, sizeof(mca_part_persist_psend_request_t),
                        opal_cache_line_size, OBJ_CLASS(mca_part_persist_psend_request_t), 0,
                        opal_cache_line_size, ompi_part_persist.free_list_num,
                        ompi_part_persist.free_list_max, ompi_part_persist.free_list_inc, NULL, 0,
                        NULL, NULL, NULL);
 ...
 */
__opal_attribute_always_inline__ static inline int mca_part_persist_progress(void)
{
    ...
    if (0 == ompi_part_persist.init_world) {
        ...
        err = ompi_comm_idup(&ompi_mpi_comm_world.comm, &ompi_part_persist.part_comm,
                             &ompi_part_persist.part_comm_req);
        if (err != OMPI_SUCCESS) {
            exit(-1);
        }
        ompi_part_persist.part_comm_ready = 0;
        err = ompi_comm_idup(&ompi_mpi_comm_world.comm, &ompi_part_persist.part_comm_setup,
                             &ompi_part_persist.part_comm_sreq);
        if (err != OMPI_SUCCESS) {
            exit(-1);
        }
    ...
    if (0 == ompi_part_persist.init_comms) {
        if (0 == ompi_part_persist.part_comm_ready) {
            ompi_request_test(&ompi_part_persist.part_comm_req, &ompi_part_persist.part_comm_ready,
                              MPI_STATUS_IGNORE);
        }
        if (0 == ompi_part_persist.part_comm_sready) {
            ompi_request_test(&ompi_part_persist.part_comm_sreq,
                              &ompi_part_persist.part_comm_sready, MPI_STATUS_IGNORE);
        }
    ...
    OPAL_LIST_FOREACH (current, ompi_part_persist.progress_list, mca_part_persist_list_t) {
        ...
        if (false == req->initialized) {
            ...
            if (true == req->flag_post_setup_recv) {
                err = MCA_PML_CALL(irecv(&(req->setup_info[1]),
                                         sizeof(struct ompi_mca_persist_setup_t), MPI_BYTE,
                                         OMPI_ANY_SOURCE, req->my_recv_tag,
                                         ompi_part_persist.part_comm_setup, &req->setup_req[1]));
            ...
            if (done) {
                ...
                if (MCA_PART_PERSIST_REQUEST_PSEND == req->req_type) {
                    ...
                    req->persist_reqs = (ompi_request_t **) malloc(sizeof(ompi_request_t *)
                                                                   * (req->real_parts));
                    for (i = 0; i < req->real_parts; i++) {
                        void *buf = ((void *) (((char *) req->req_addr) + (bytes * i)));
                        err = MCA_PML_CALL(
                            isend_init(buf, req->real_count, req->req_datatype, req->world_peer,
                                       req->my_send_tag + i, MCA_PML_BASE_SEND_STANDARD,
                                       ompi_part_persist.part_comm, &(req->persist_reqs[i])));
                    }
                } else {
                    /* parse message */
                    req->world_peer = req->setup_info[1].world_rank;
                    ...
                    for (i = 0; i < req->real_parts; i++) {
                        void *buf = ((void *) (((char *) req->req_addr) + (bytes * i)));
                        err = MCA_PML_CALL(irecv_init(buf, req->real_count, req->req_datatype,
                                                      req->world_peer, req->my_send_tag + i,
                                                      ompi_part_persist.part_comm,
                                                      &(req->persist_reqs[i])));
                    }
                    err = req->persist_reqs[0]->req_start(req->real_parts,
                                                          (&(req->persist_reqs[0])));

                    /* Send back a message */
                    req->setup_info[0].world_rank = ompi_part_persist.my_world_rank;
                    err = MCA_PML_CALL(
                        isend(&(req->setup_info[0]), sizeof(struct ompi_mca_persist_setup_t),
                              MPI_BYTE, req->world_peer, req->my_recv_tag,
                              MCA_PML_BASE_SEND_STANDARD, ompi_part_persist.part_comm_setup,
                              &req->setup_req[0]));
                    if (OMPI_SUCCESS != err)
                        return OMPI_ERROR;
                }
            ...
            if (false == req->req_part_complete && REQUEST_COMPLETED != req->req_ompi.req_complete
                && OMPI_REQUEST_ACTIVE == req->req_ompi.req_state) {
                for (i = 0; i < req->real_parts; i++) {

                    /* Check to see if partition is queued for being started. Only applicable to
                     * sends. */
                    if (-2 == req->flags[i]) {
                    ...
                    if (0 == req->flags[i]) {
                        ompi_request_test(&(req->persist_reqs[i]), &(req->flags[i]),
                                          MPI_STATUS_IGNORE);
                        if (0 != req->flags[i])
                            req->done_count++;
                    }
            ...
            if (true == req->req_free_called && true == req->req_part_complete
                && REQUEST_COMPLETED == req->req_ompi.req_complete
...
__opal_attribute_always_inline__ static inline int
mca_part_persist_precv_init(void *buf, size_t parts, size_t count, ompi_datatype_t *datatype,
                            ...
                            struct ompi_request_t **request)
{
    ...
    MCA_PART_PERSIST_PRECV_REQUEST_INIT(recvreq, ompi_proc, comm, tag, src, datatype, buf, parts,
                                        count, flags);
    ...
    err = MCA_PML_CALL(irecv(&req->setup_info[1], sizeof(struct ompi_mca_persist_setup_t), MPI_BYTE,
                             src, tag, comm, &req->setup_req[1]));
    if (OMPI_SUCCESS != err)
        return OMPI_ERROR;
...
__opal_attribute_always_inline__ static inline int
mca_part_persist_psend_init(const void *buf, size_t parts, size_t count, ompi_datatype_t *datatype,
                            int dst, int tag, ompi_communicator_t *comm, ompi_request_t **request)
{
    int err = OMPI_SUCCESS;
    size_t dt_size_;
    int dt_size;
    ...
    if (OPAL_UNLIKELY(NULL == sendreq))
        return OMPI_ERR_OUT_OF_RESOURCE;
    MCA_PART_PERSIST_PSEND_REQUEST_INIT(sendreq, ompi_proc, comm, tag, dst, datatype, buf, parts,
                                        count, flags);
    ...
    ompi_part_persist.next_send_tag += parts;
    ...
    err = MCA_PML_CALL(isend(&(req->setup_info[0]), sizeof(struct ompi_mca_persist_setup_t),
                             MPI_BYTE, dst, tag, MCA_PML_BASE_SEND_STANDARD, comm,
                             &req->setup_req[0]));
    if (OMPI_SUCCESS != err)
        return OMPI_ERROR;
    ...
    if (1 == ompi_part_persist.init_comms) {
        err = MCA_PML_CALL(irecv(&(req->setup_info[1]), sizeof(struct ompi_mca_persist_setup_t),
                                 MPI_BYTE, MPI_ANY_SOURCE, req->my_recv_tag,
                                 ompi_part_persist.part_comm_setup, &req->setup_req[1]));
        if (OMPI_SUCCESS != err)
            return OMPI_ERROR;
        req->flag_post_setup_recv = false;
...
__opal_attribute_always_inline__ static inline int mca_part_persist_start(size_t count,
...
__opal_attribute_always_inline__ static inline int
mca_part_persist_pready(size_t min_part, size_t max_part, ompi_request_t *request)
{
    int err = OMPI_SUCCESS;
    size_t i;
    ...
    if (true == req->initialized) {
        err = req->persist_reqs[min_part]->req_start(max_part - min_part + 1,
                                                     (&(req->persist_reqs[min_part])));
...
__opal_attribute_always_inline__ static inline int
mca_part_persist_parrived(size_t min_part, size_t max_part, int *flag, ompi_request_t *request)
{
    int err = OMPI_SUCCESS;
    size_t i;
    int _flag = false;
 ...
 */
__opal_attribute_always_inline__ static inline int mca_part_persist_free(ompi_request_t **request)
{
    mca_part_persist_request_t *req = *(mca_part_persist_request_t **) request;

    if (true == req->req_free_called)
        return OMPI_ERROR;
