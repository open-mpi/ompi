...
#define MCA_PART_PERSIST_PRECV_REQUEST_ALLOC(precvreq)                      \
    do {                                                                    \
        precvreq = (mca_part_persist_precv_request_t *) opal_free_list_get( \
            &mca_part_base_precv_requests);                                 \
 ...
 */
#define MCA_PART_PERSIST_PRECV_REQUEST_INIT(request, ompi_proc, comm, tag, src, datatype, addr,   \
                                            parts, count, flags)                                  \
    do {                                                                                          \
        OBJ_RETAIN(comm);                                                                         \
...
#define MCA_PART_PERSIST_PRECV_REQUEST_RETURN(recvreq)                                             \
    {                                                                                              \
        ...
        opal_free_list_return(&mca_part_base_precv_requests, (opal_free_list_item_t *) (recvreq)); \
    }
