...
#define MCA_PART_PERSIST_PSEND_REQUEST_ALLOC(sendreq, comm, dst, ompi_proc) \
    do {                                                                    \
        sendreq = (mca_part_persist_psend_request_t *) opal_free_list_wait( \
            &mca_part_base_psend_requests);                                 \
        sendreq->req_base.req_type = MCA_PART_PERSIST_REQUEST_PSEND;        \
    } while (0)

#define MCA_PART_PERSIST_PSEND_REQUEST_INIT(req_send, ompi_proc, comm, tag, dst, datatype, buf,    \
                                            parts, count, flags)                                   \
    do {                                                                                           \
...
#define MCA_PART_PERSIST_PSEND_REQUEST_RETURN(sendreq)                                           \
    {                                                                                            \
        ...
        opal_free_list_return(&mca_part_base_psend_requests, (opal_free_list_item_t *) sendreq); \
    }
