 ...
 * @retval MPI_SUCCESS
 */
int mca_topo_base_cart_get(ompi_communicator_t *comm, int maxdims, int *dims, int *periods,
                           int *coords)
{
    int m = (maxdims <= comm->c_topo->mtc.cart->ndims) ? maxdims : comm->c_topo->mtc.cart->ndims;
