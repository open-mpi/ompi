                              ...
                              mca_topo_base_module_t **selected_module, uint32_t type)
{
    ...
    opal_output_verbose(10, ompi_topo_base_framework.framework_output,
                        "topo:base:comm_select: new communicator: %s (cid %d)", comm->c_name,
                        comm->c_contextid);
    ...
    OPAL_LIST_FOREACH (cli, &ompi_topo_base_framework.framework_components,
    ...
    if (ompi_ftmpi_enabled) {
        ...
        opal_show_help("help-ft-mpi.txt", "module:untested:failundef", true,
                       ...
                       best_component->topoc_version.mca_component_name, ft_whitelist);
    }
...
static void fill_null_pointers(int type, mca_topo_base_module_t *module)
{
    if (OMPI_COMM_CART == type) {
        ...
        if (NULL == module->topo.dist_graph.dist_graph_create_adjacent) {
            module->topo.dist_graph.dist_graph_create_adjacent
        ...
        if (NULL == module->topo.dist_graph.dist_graph_neighbors_count) {
            module->topo.dist_graph.dist_graph_neighbors_count
