...
int mca_topo_base_dist_graph_distribute(mca_topo_base_module_t *module, ompi_communicator_t *comm,
                                        ...
                                        mca_topo_base_comm_dist_graph_2_2_0_t **ptopo)
{
     ...
     */
    cnt = (mca_topo_base_dist_graph_elem_t *) calloc(3 * csize,
                                                     sizeof(mca_topo_base_dist_graph_elem_t));
    if (NULL == cnt) {
    ...
    rin = (int *) calloc(2
                             * (pos[csize - 1].in + cnt[csize - 1].in + pos[csize - 1].out
                                + cnt[csize - 1].out),
                         sizeof(int));
    if (NULL == rin) {
        err = OMPI_ERR_OUT_OF_RESOURCE;
    ...
    for (pending_reqs = i = 0; i < csize; ++i) {
        ...
        if (0 != (count = cnt[i].in)) {
            ...
            err = MCA_PML_CALL(isend(&rin[position], count, (ompi_datatype_t *) &ompi_mpi_int, i,
                                     MCA_TOPO_BASE_TAG_DIST_EDGE_IN, MCA_PML_BASE_SEND_STANDARD,
        ...
        if (0 != (count = cnt[i].out)) {
            ...
            err = MCA_PML_CALL(isend(&rout[position], count, (ompi_datatype_t *) &ompi_mpi_int, i,
                                     MCA_TOPO_BASE_TAG_DIST_EDGE_OUT, MCA_PML_BASE_SEND_STANDARD,
                                     comm, &reqs[pending_reqs]));
            pending_reqs++;
        }
    }
    ...
    for (left_over = count, current_pos = i = 0; left_over > 0; i++) {
        ...
        MCA_PML_CALL(recv(&temp[count - left_over], left_over,
                          (ompi_datatype_t *) &ompi_mpi_int, /* keep receiving in the same buffer */
                          MPI_ANY_SOURCE, MCA_TOPO_BASE_TAG_DIST_EDGE_IN, comm, &status));
    ...
    for (left_over = count, current_pos = i = 0; left_over > 0; i++) {
        ...
        MCA_PML_CALL(recv(&temp[count - left_over], left_over,
                          (ompi_datatype_t *) &ompi_mpi_int, /* keep receiving in the same buffer */
                          MPI_ANY_SOURCE, MCA_TOPO_BASE_TAG_DIST_EDGE_OUT, comm, &status));
        how_much = status._ucount / int_size;
...
int mca_topo_base_dist_graph_create(mca_topo_base_module_t *module, ompi_communicator_t *comm_old,
                                    int n, const int nodes[], const int degrees[],
                                    const int targets[], const int weights[], opal_info_t *info,
                                    int reorder, ompi_communicator_t **newcomm)
{
    ...
    err = mca_topo_base_dist_graph_distribute(module, *newcomm, n, nodes, degrees, targets, weights,
...
static void
mca_topo_base_comm_dist_graph_2_2_0_construct(mca_topo_base_comm_dist_graph_2_2_0_t *dist_graph)
{
    dist_graph->in = NULL;
    dist_graph->inw = NULL;
    dist_graph->out = NULL;
...
static void
mca_topo_base_comm_dist_graph_2_2_0_destruct(mca_topo_base_comm_dist_graph_2_2_0_t *dist_graph)
{
