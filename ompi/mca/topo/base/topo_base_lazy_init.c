...
int mca_topo_base_lazy_init(void)
{
    ...
    if (!mca_base_framework_is_open(&ompi_topo_base_framework)) {
        /**
         * Register and open all available components, giving them a chance to access the MCA
         * parameters.
         */
        ...
        if (OMPI_SUCCESS != (err = mca_topo_base_find_available(OPAL_ENABLE_PROGRESS_THREADS, 1))) {
            return err;
        }
    }
