 ...
 */
static int check_oversubscribing(int rank, int num_nodes, int num_objs_in_node,
                                 int num_procs_in_node, int *nodes_roots, int *local_procs,
                                 ompi_communicator_t *comm_old)
{
    ...
    if (OMPI_SUCCESS
        != (err = comm_old->c_coll->coll_allreduce(&local_oversub, &oversubscribed, 1, MPI_INT,
                                                   MPI_SUM, comm_old,
                                                   comm_old->c_coll->coll_allreduce_module)))
        return err;
...
#ifdef __DEBUG__
static void dump_int_array(int level, int output_id, char *prolog, char *line_prolog, int *array,
                           size_t length)
{
    size_t i;
    if (-1 == output_id)
...
static void dump_double_array(int level, int output_id, char *prolog, char *line_prolog,
                              double *array, size_t length)
{
    size_t i;
                                         ...
                                         ompi_communicator_t **newcomm)
{
    ...
    if (OMPI_SUCCESS
        != (err = mca_topo_base_dist_graph_distribute(topo_module, comm_old, n, nodes, degrees,
                                                      targets, weights,
    ...
    OPAL_OUTPUT_VERBOSE(
        (10, ompi_topo_base_framework.framework_output, "Process rank is : %i\n", rank));
    /**
    ...
    for (i = 0; i < size; i++) {
        ...
        if ((i == rank) || (OPAL_PROC_ON_LOCAL_NODE(proc->super.proc_flags)))
...
#ifdef __DEBUG__
    if (0 == rank) {
        dump_int_array(10, ompi_topo_base_framework.framework_output, "lindex_to_grank : ", "",
                       lindex_to_grank, num_procs_in_node);
        dump_int_array(10, ompi_topo_base_framework.framework_output, "Vpids : ", "", colors, size);
    }
#endif
    ...
    if (0 == rank) {
        ...
        for (i = idx = 0; i < size; i++)
            if (vpids[i] != -1)
                nodes_roots[idx++] = i;
        OPAL_OUTPUT_VERBOSE(
            (10, ompi_topo_base_framework.framework_output, "num nodes is %i\n", num_nodes));
#ifdef __DEBUG__
        dump_int_array(10, ompi_topo_base_framework.framework_output, "Root nodes are :\n", "root ",
                       nodes_roots, num_nodes);
#endif
    }
    free(vpids);
    ...
    if (hwloc_bitmap_isincluded(root_obj->cpuset,
                                set)) { /* processes are not bound on the machine */
        if (0 == rank)
    ...
    if ((0 == num_objs_in_node)
        || (0 == num_pus_in_node)) { /* deal with bozo cases: COVERITY 1418505 */
        free(colors);
        free(lindex_to_grank);
        goto fallback; /* return with success */
    }
    /* Check for oversubscribing */
    oversubscribing_objs = check_oversubscribing(rank, num_nodes, num_objs_in_node,
                                                 num_procs_in_node, nodes_roots, lindex_to_grank,
                                                 comm_old);
    ...
    if (oversubscribing_objs) {
        if (hwloc_bitmap_isincluded(root_obj->cpuset,
            ...
            oversubscribed_pus = check_oversubscribing(rank, num_nodes, num_pus_in_node,
                                                       num_procs_in_node, nodes_roots,
                                                       lindex_to_grank, comm_old);
            ...
            oversubscribed_pus = check_oversubscribing(rank, num_nodes, num_objs_in_node,
                                                       num_procs_in_node, nodes_roots,
                                                       lindex_to_grank, comm_old);
        }
    }
    ...
    if (!oversubscribing_objs && !oversubscribed_pus) {
        if (hwloc_bitmap_isincluded(root_obj->cpuset,
            ...
            OPAL_OUTPUT_VERBOSE((10, ompi_topo_base_framework.framework_output,
                                 ...
                                 rank, obj_rank, num_objs_in_node, num_pus_in_node));
        }
    } else {
        OPAL_OUTPUT_VERBOSE((10, ompi_topo_base_framework.framework_output,
    ...
    if (rank == lindex_to_grank[0]) { /* local leader cleans the hierarchy */
        ...
        OPAL_OUTPUT_VERBOSE(
            (10, ompi_topo_base_framework.framework_output,
        ...
        for (i = 1; i < num_procs_in_node; i++) {
            if (OMPI_SUCCESS
                != (err = MCA_PML_CALL(irecv(&localrank_to_objnum[i], 1, MPI_INT,
        ...
        if (OMPI_SUCCESS
            != (err = ompi_request_wait_all(num_procs_in_node - 1, reqs, MPI_STATUSES_IGNORE))) {
            free(reqs);
            reqs = NULL;
            goto release_and_return;
        }
        ...
        if (OMPI_SUCCESS
            != (err = MCA_PML_CALL(send(&obj_rank, 1, MPI_INT, lindex_to_grank[0], -111,
                                        MCA_PML_BASE_SEND_STANDARD, comm_old)))) {
            free(reqs);
            reqs = NULL;
            goto release_and_return;
        }
    ...
    if (0 == mca_topo_treematch_component.reorder_mode) {
        ...
        err = comm_old->c_coll->coll_gather((0 == rank ? MPI_IN_PLACE : local_pattern), size,
                                            MPI_DOUBLE, local_pattern, size,
                                            MPI_DOUBLE, /* ignored on non-root */
        ...
        if (rank == lindex_to_grank[0]) {
            ...
            if (0 == rank) {
                if (num_nodes > 1) {
                    ...
                    for (i = 1; i < num_nodes; i++)
                        if (OMPI_SUCCESS
                            != (err = MCA_PML_CALL(irecv(objs_per_node + i, 1, MPI_INT,
                                                         nodes_roots[i], -112, comm_old,
                                                         &reqs[i - 1])))) {
                    ...
                    for (i = 1; i < num_nodes; i++) {
                        if (OMPI_SUCCESS
                            != (err = MCA_PML_CALL(irecv(obj_mapping + displ, objs_per_node[i],
                                                         MPI_INT, nodes_roots[i], -113, comm_old,
                                                         &reqs[i - 1])))) {
                            free(obj_to_rank_in_comm);
                            free(objs_per_node);
                            free(obj_mapping);
...
#ifdef __DEBUG__
                dump_int_array(10, ompi_topo_base_framework.framework_output, "Obj mapping : ", "",
                               obj_mapping, num_objs_total);
#endif
            } else {
                if (num_nodes > 1) {
                    if (OMPI_SUCCESS
                        != (err = MCA_PML_CALL(send(&num_objs_in_node, 1, MPI_INT, 0, -112,
                                                    MCA_PML_BASE_SEND_STANDARD, comm_old)))) {
                        free(obj_to_rank_in_comm);
                        goto release_and_return;
                    }
                    if (OMPI_SUCCESS
                        != (err = MCA_PML_CALL(send(obj_to_rank_in_comm, num_objs_in_node, MPI_INT,
                                                    0, -113, MCA_PML_BASE_SEND_STANDARD,
                                                    comm_old)))) {
                        free(obj_to_rank_in_comm);
                        goto release_and_return;
                    }
            ...
            if (num_nodes > 1) {
                if (rank != 0) {
                    if (OMPI_SUCCESS
                        != (err = MCA_PML_CALL(send(hierarchies, (TM_MAX_LEVELS + 1), MPI_INT, 0,
                    ...
                    for (i = 1; i < num_nodes; i++)
                        if (OMPI_SUCCESS
                            != (err = MCA_PML_CALL(irecv(hierarchies + i * (TM_MAX_LEVELS + 1),
                                                         (TM_MAX_LEVELS + 1), MPI_INT,
                                                         nodes_roots[i], -114, comm_old,
                                                         &reqs[i - 1])))) {
                            free(obj_mapping);
                            free(hierarchies);
                            goto release_and_return;
            ...
            if (0 == rank) {
...
#ifdef __DEBUG__
                dump_int_array(10, ompi_topo_base_framework.framework_output, "hierarchies : ", "",
                               hierarchies, num_nodes * (TM_MAX_LEVELS + 1));
#endif
                ...
                for (i = 0; i < num_nodes; i++) {
                    ...
                    for (j = 1; j <= suppl; j++)
                        *(base_ptr + tm_topology->nb_levels) *= *(base_ptr + tm_topology->nb_levels
                                                                  + j);
                }
                if (num_nodes > 1) {
                    ...
                    for (i = 1; i < tm_topology->nb_levels;
                         i++) { /* compute the minimum for each level */
                        min = hierarchies[i];
                ...
                for (i = 1; i < tm_topology->nb_levels; i++)
                    tm_topology->nb_nodes[i] = tm_topology->nb_nodes[i - 1]
                                               * tm_topology->arity[i - 1];
                ...
                for (i = 0; i < num_objs_total; i++) {
                    tm_topology->node_id[i] = obj_mapping[i]; /* use process ranks instead of core
                                                                 numbers */
                    if (obj_mapping[i] != -1) /* so that k[i] is the new rank of process i */
                        tm_topology->node_rank[obj_mapping[i]] = i; /* after computation by
                                                                       TreeMatch */
                }
                ...
                for (idx = 0, i = 0; i < tm_topology->nb_proc_units; i++)
                    if (obj_mapping[i] != -1)
                        tm_topology->constraints[idx++]
                            = obj_mapping[i]; /* use process ranks instead of core numbers */
#ifdef __DEBUG__
                ...
                */
                ...
                dump_int_array(10, ompi_topo_base_framework.framework_output, "Matching : ", "",
                               sol->sigma, sol->sigma_length);
#endif
                free(obj_mapping);
                free(comm_pattern);
        ...
        if (OMPI_SUCCESS
            != (err = comm_old->c_coll->coll_scatter(k, 1, MPI_INT, &newrank, 1, MPI_INT, 0,
                                                     comm_old,
        ...
        if (OMPI_SUCCESS
            != (err = ompi_comm_split(comm_old, colors[rank], rank, &localcomm, false))) {
            goto release_and_return;
        }

        lrank_to_grank = (int *) calloc(num_procs_in_node, sizeof(int));
        if (OMPI_SUCCESS
            != (err = localcomm->c_coll
                          ->coll_allgather(&rank, 1, MPI_INT, lrank_to_grank, 1, MPI_INT, localcomm,
        ...
        if (rank == lindex_to_grank[0]) {
            ...
            local_pattern = (double *) calloc(num_procs_in_node * num_procs_in_node,
                                              sizeof(double));
        } else {
        ...
        if (OMPI_SUCCESS
            != (err = localcomm->c_coll->coll_gather((rank == lindex_to_grank[0] ? MPI_IN_PLACE
                                                                                 : local_pattern),
                                                     num_procs_in_node, MPI_DOUBLE, local_pattern,
                                                     num_procs_in_node, MPI_DOUBLE, 0, localcomm,
        ...
        if (rank == lindex_to_grank[0]) {
            ...
            for (i = 0; i < tm_topology->nb_levels; i++) {
                OPAL_OUTPUT_VERBOSE((10, ompi_topo_base_framework.framework_output,
                                     ...
                                     tm_topology->nb_nodes[i], tm_topology->arity[i]));
            }
            dump_int_array(10, ompi_topo_base_framework.framework_output, "", "Obj id ",
                           tm_topology->node_id, tm_topology->nb_nodes[tm_topology->nb_levels - 1]);
            tm_display_topology(tm_topology);
#endif
            // tm_optimize_topology(&tm_topology);
        ...
        /* scatter the ranks */
        if (OMPI_SUCCESS
            != (err = localcomm->c_coll->coll_scatter(k, 1, MPI_INT, &newrank, 1, MPI_INT, 0,
                                                      localcomm,
...
release_and_return:
    ...
    if (NULL != local_pattern)
        free(local_pattern);
    free(colors);
    if (NULL != lindex_to_grank)
        free(lindex_to_grank);
    if (NULL != nodes_roots)
        free(nodes_roots); /* only on root */
    if (NULL != localrank_to_objnum)
