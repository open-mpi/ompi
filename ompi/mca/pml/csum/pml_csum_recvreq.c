        ...
        bool nordma)
{
    ...
    OPAL_OUTPUT_VERBOSE((1, mca_pml_base_output,
                         "%s Sending \'ACK\' with header csum:0x%04x\n", OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ack->hdr_common.hdr_csum));
                                             ...
                                             size_t num_segments )
{
    ...
    if (bytes_received > 0) {
        ...
        OPAL_OUTPUT_VERBOSE((1, mca_pml_base_output,
                             ...
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), csum, hdr->hdr_frag.hdr_csum, (unsigned long)bytes_received));
        if(csum != hdr->hdr_frag.hdr_csum) {
            opal_output(0, "%s:%s:%d: Invalid \'frag data\' - received csum:0x%x  != computed csum:0x%x\n",
                        OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __FILE__, __LINE__, hdr->hdr_frag.hdr_csum, csum);
                                             ...
                                             size_t num_segments )
{
    ...
    if( 0 < bytes_received ) {
        ...
        OPAL_OUTPUT_VERBOSE((1, mca_pml_base_output,
                             ...
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), csum, hdr->hdr_match.hdr_csum, (unsigned long)bytes_received));
        if (csum != hdr->hdr_match.hdr_csum) {
            opal_output(0, "%s:%s:%d: Invalid \'rndv data\' - received csum:0x%x  != computed csum:0x%x\n",
                        OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __FILE__, __LINE__, hdr->hdr_match.hdr_csum, csum);
            dump_csum_error_data(segments, num_segments);
            ompi_rte_abort(-1,NULL);
        }
    }
