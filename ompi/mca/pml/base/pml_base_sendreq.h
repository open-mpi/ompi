...
#define MCA_PML_BASE_SEND_REQUEST_INIT(request, addr, count, datatype, peer, tag, comm, mode, \
                                       persistent, convertor_flags)                           \
    {                                                                                         \
        ...
        if (count > 0) {                                                                      \
            ...
            opal_convertor_copy_and_prepare_for_send(                                         \
                ...
                &((request)->req_base.req_datatype->super), (request)->req_base.req_count,    \
                (request)->req_base.req_addr, convertor_flags,                                \
...
#define MCA_PML_BASE_SEND_REQUEST_RESET(request)                                        \
    if ((request)->req_bytes_packed > 0) {                                              \
        size_t cnt = 0;                                                                 \
        opal_convertor_set_position(&(sendreq)->req_send.req_base.req_convertor, &cnt); \
    }
