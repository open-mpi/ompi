...
static mca_allocator_base_module_t
    *mca_pml_bsend_allocator;                 /* sub-allocator to manage users buffer */
...
int mca_pml_base_bsend_init(bool thread_safe)
{
    ...
    if (NULL
        == (mca_pml_bsend_allocator_component = mca_allocator_component_lookup(
...
int mca_pml_base_bsend_attach(void *addr, int size)
{
    ...
    mca_pml_bsend_allocator = mca_pml_bsend_allocator_component
                                  ->allocator_init(thread_safe, mca_pml_bsend_alloc_segment, NULL,
                                                   NULL);
    if (NULL == mca_pml_bsend_allocator) {
        OPAL_THREAD_UNLOCK(&mca_pml_bsend_mutex);
        return OMPI_ERR_BUFFER;
...
int mca_pml_base_bsend_request_start(ompi_request_t *request)
{
    ...
    if (sendreq->req_bytes_packed > 0) {
        ...
        sendreq->req_addr = mca_pml_bsend_allocator->alc_alloc(mca_pml_bsend_allocator,
                                                               sendreq->req_bytes_packed, 0);
        ...
        if ((rc = opal_convertor_pack(&sendreq->req_base.req_convertor, &iov, &iov_count,
                                      &max_data))
            < 0) {
            return OMPI_ERROR;
        }
        ...
        opal_convertor_prepare_for_send(&sendreq->req_base.req_convertor,
                                        &(ompi_mpi_packed.dt.super), max_data, sendreq->req_addr);
...
void *mca_pml_base_bsend_request_alloc_buf(size_t length)
{
    ...
    /* allocate a buffer to hold packed message */
    buf = mca_pml_bsend_allocator->alc_alloc(mca_pml_bsend_allocator, length, 0);
    if (NULL == buf) {
        /* release resources when request is freed */
        OPAL_THREAD_UNLOCK(&mca_pml_bsend_mutex);
        /* progress communications, with the hope that more resources
