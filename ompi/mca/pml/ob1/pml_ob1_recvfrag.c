...
OBJ_CLASS_INSTANCE(mca_pml_ob1_buffer_t, opal_free_list_item_t, NULL, NULL);

OBJ_CLASS_INSTANCE(mca_pml_ob1_recv_frag_t, opal_list_item_t, NULL, NULL);
...
static void append_frag_to_list(opal_list_t *queue, mca_btl_base_module_t *btl,
                                ...
                                const mca_btl_base_segment_t *segments, size_t num_segments,
...
static void append_frag_to_umq(custom_match_umq *queue, mca_btl_base_module_t *btl,
                               const mca_pml_ob1_match_hdr_t *hdr,
                               const mca_btl_base_segment_t *segments, size_t num_segments,
                               mca_pml_ob1_recv_frag_t *frag)
{
 ...
 */
void append_frag_to_ordered_list(mca_pml_ob1_recv_frag_t **queue, mca_pml_ob1_recv_frag_t *frag,
                                 uint16_t seq)
{
    ...
    if (hdr->hdr_seq < prior->hdr.hdr_match.hdr_seq) {
        ...
        } while ((hdr->hdr_seq < prior->hdr.hdr_match.hdr_seq) && (d1 > d2) && (prior != *queue));
    } else {
        uint16_t prior_seq = prior->hdr.hdr_match.hdr_seq,
                 next_seq = ((mca_pml_ob1_recv_frag_t *) (prior->super.super.opal_list_next))
    ...
    if (NULL == parent->range) {
        ...
        mca_pml_ob1_recv_frag_t *largest = (mca_pml_ob1_recv_frag_t *)
                                               parent->range->super.super.opal_list_prev;
...
merge_ranges:
    ...
    if (NULL != parent->range) {
        upper = ((mca_pml_ob1_recv_frag_t *) parent->range->super.super.opal_list_prev)
                    ->hdr.hdr_match.hdr_seq;
    }
 ...
 */
static mca_pml_ob1_recv_frag_t *remove_head_from_ordered_list(mca_pml_ob1_recv_frag_t **queue)
{
 ...
 */
static int
mca_pml_ob1_recv_frag_match_proc(mca_btl_base_module_t *btl, ompi_communicator_t *comm_ptr,
                                 mca_pml_ob1_comm_proc_t *proc, const mca_pml_ob1_match_hdr_t *hdr,
                                 const mca_btl_base_segment_t *segments, size_t num_segments,
                                 int type, mca_pml_ob1_recv_frag_t *frag);
...
static inline int pml_ob1_frag_is_revoked(ompi_communicator_t *ompi_comm,
                                          mca_pml_ob1_recv_frag_t *frag)
{
    ...
    return ((ompi_comm_is_revoked(ompi_comm) && !ompi_request_tag_is_ft(hdr->hdr_match.hdr_tag))
            || (ompi_comm_coll_revoked(ompi_comm)
                && ompi_request_tag_is_collective(hdr->hdr_match.hdr_tag)));
}
...
#    endif /* OPAL_ENABLE_DEBUG */
    ...
    for (i = 0; i < comm->num_procs; i++) {
        ...
        for (it = opal_list_get_first(frags_list); it != opal_list_get_end(frags_list);
        ...
        while (NULL != (it = opal_list_remove_first(&keep_list))) {
            append_frag_to_ordered_list(&proc->frags_cant_match, (mca_pml_ob1_recv_frag_t *) it,
...
#    if OPAL_ENABLE_DEBUG
    if (opal_list_get_size(&nack_list)) {
        OPAL_OUTPUT_VERBOSE((15, ompi_ftmpi_output_handle,
                             "ob1_revoke_comm: purging unexpected and cantmatch frags for in comm "
                             "%d (%s): nacking %zu frags",
                             ompi_comm->c_contextid,
                             coll_only ? "collective frags only" : "all revoked",
...
#    endif
    ...
    while (NULL != (it = opal_list_remove_first(&nack_list))) {
        ...
        if (MCA_PML_OB1_HDR_TYPE_MATCH != hdr->hdr_common.hdr_type) {
            ...
            OPAL_OUTPUT_VERBOSE((2, ompi_ftmpi_output_handle, "ob1_revoke_comm: sending NACK to %d",
                                 hdr->hdr_rndv.hdr_match.hdr_src));
            ...
            mca_pml_ob1_recv_request_ack_send(NULL, proc->ompi_proc, hdr->hdr_rndv.hdr_src_req.lval,
                                              NULL, 0, 0, false);
        } else {
            /* if it's a TYPE_MATCH, the sender is not expecting anything
             * from us. So we are done. */
            OPAL_OUTPUT_VERBOSE((15, ompi_ftmpi_output_handle,
                                 "ob1_revoke_comm: dropping silently frag from %d",
                                 hdr->hdr_rndv.hdr_match.hdr_src));
        }
                                          ...
                                          const mca_btl_base_receive_descriptor_t *descriptor)
{
    ...
    if (OPAL_UNLIKELY(NULL == comm_ptr)) {
         ...
         */
        append_frag_to_list(&mca_pml_ob1.non_existing_communicator_pending, btl, hdr, segments,
                            num_segments, NULL);
        return;
    }
     ...
     */
    PERUSE_TRACE_MSG_EVENT(PERUSE_COMM_MSG_ARRIVED, comm_ptr, hdr->hdr_src, hdr->hdr_tag,
                           PERUSE_RECV);
    ...
    if (OPAL_UNLIKELY((ompi_comm_is_revoked(comm_ptr) && !ompi_request_tag_is_ft(hdr->hdr_tag))
                      || (ompi_comm_coll_revoked(comm_ptr)
                          && ompi_request_tag_is_collective(hdr->hdr_tag)))) {
     ...
     */
    PERUSE_TRACE_MSG_EVENT(PERUSE_COMM_SEARCH_POSTED_Q_BEGIN, comm_ptr, hdr->hdr_src, hdr->hdr_tag,
                           PERUSE_RECV);
     ...
     */
    PERUSE_TRACE_MSG_EVENT(PERUSE_COMM_SEARCH_POSTED_Q_END, comm_ptr, hdr->hdr_src, hdr->hdr_tag,
                           PERUSE_RECV);
    ...
    if (OPAL_LIKELY(match)) {
        ...
        if (match->req_bytes_expected > 0) {
            ...
            while (iov_count < num_segments) {
                ...
                iov[iov_count].iov_base = (IOVBASE_TYPE *) ((unsigned char *) segments[iov_count]
                                                                .seg_addr.pval);
                iov_count++;
            }
            opal_convertor_unpack(&match->req_recv.req_base.req_convertor, iov, &iov_count,
                                  &bytes_received);
            match->req_bytes_received = bytes_received;
            SPC_USER_OR_MPI(match->req_recv.req_base.req_ompi.req_status.MPI_TAG,
                                         ...
                                         const mca_btl_base_receive_descriptor_t *descriptor)
{
    ...
    mca_pml_ob1_recv_frag_match(btl, &hdr->hdr_match, segments, descriptor->des_segment_count,
                                         ...
                                         const mca_btl_base_receive_descriptor_t *descriptor)
{
    ...
    if (recvreq->req_recv.req_base.req_convertor.flags & CONVERTOR_CUDA_ASYNC) {
        ...
        mca_pml_ob1_recv_request_frag_copy_start(recvreq, btl, segments,
                                                 descriptor->des_segment_count, NULL);
...
static inline mca_pml_ob1_recv_request_t *get_next_posted_recv(opal_list_t *queue,
                                                   ...
                                                   mca_pml_ob1_comm_proc_t *proc)
{
    ...
    wild_recv_seq = wild_recv ? wild_recv->req_recv.req_base.req_sequence : PML_MAX_SEQ;
    specific_recv_seq = specific_recv ? specific_recv->req_recv.req_base.req_sequence : PML_MAX_SEQ;
                                             ...
                                             mca_pml_ob1_recv_frag_t *frag)
{
    ...
    do {
        ...
        if (OPAL_LIKELY(NULL != match)) {
            ...
            PERUSE_TRACE_COMM_EVENT(PERUSE_COMM_MSG_MATCH_POSTED_REQ, &(match->req_recv.req_base),
                                    PERUSE_RECV);
            SPC_TIMER_STOP(OMPI_SPC_MATCH_TIME, &timer);
            return match;
        }
...
#if MCA_PML_OB1_CUSTOM_MATCH
        append_frag_to_umq(comm->umq, btl, hdr, segments, num_segments, frag);
#else
        append_frag_to_list(&proc->unexpected_frags, btl, hdr, segments, num_segments, frag);
#endif
        ...
        PERUSE_TRACE_MSG_EVENT(PERUSE_COMM_MSG_INSERT_IN_UNEX_Q, comm_ptr, hdr->hdr_src,
                                       ...
                                       int type)
{
    ...
    if (OPAL_UNLIKELY((ompi_comm_is_revoked(comm_ptr) && !ompi_request_tag_is_ft(hdr->hdr_tag)))
        ...
        if (MCA_PML_OB1_HDR_TYPE_MATCH != hdr->hdr_common.hdr_type) {
            ...
            mca_pml_ob1_recv_request_ack_send(NULL, proc->ompi_proc, hdr_rndv->hdr_src_req.lval,
                                              NULL, 0, 0, false);
            OPAL_OUTPUT_VERBOSE((2, ompi_ftmpi_output_handle,
                                 "Recvfrag: comm %d is revoked or collectives force errors, "
                                 "sending a NACK to the RDV/RGET match from %d\n",
    ...
    /* mca_pml_ob1_recv_frag_match_proc() will release the lock. */
    return mca_pml_ob1_recv_frag_match_proc(btl, comm_ptr, proc, hdr, segments, num_segments, type,
                                            NULL);
}
