...
#define MCA_PML_OB1_RECV_FRAG_ALLOC(frag)                                                \
    do {                                                                                 \
        frag = (mca_pml_ob1_recv_frag_t *) opal_free_list_wait(&mca_pml_ob1.recv_frags); \
    } while (0)
...
#define MCA_PML_OB1_RECV_FRAG_INIT(frag, hdr, segs, cnt, btl)                                  \
    do {                                                                                       \
        ...
        if (_size <= mca_pml_ob1.unexpected_limit) {                                           \
            ...
            buffers[0].len = _size;                                                            \
            buffers[0].addr = (char *) mca_pml_ob1.allocator->alc_alloc(mca_pml_ob1.allocator, \
                                                                        buffers[0].len, 0);    \
...
#define MCA_PML_OB1_RECV_FRAG_RETURN(frag)                                                 \
    do {                                                                                   \
        if (frag->segments[0].seg_len > mca_pml_ob1.unexpected_limit) {                    \
            /* return buffers */                                                           \
            mca_pml_ob1.allocator->alc_free(mca_pml_ob1.allocator, frag->buffers[0].addr); \
        }                                                                                  \
        frag->num_segments = 0;                                                            \
                                                                                           \
        /* return recv_frag */                                                             \
        opal_free_list_return(&mca_pml_ob1.recv_frags, (opal_free_list_item_t *) frag);    \
    } while (0)
...
extern void
mca_pml_ob1_recv_frag_callback_rndv(mca_btl_base_module_t *btl,
                                    const mca_btl_base_receive_descriptor_t *descriptor);
/**
...
extern void
mca_pml_ob1_recv_frag_callback_rget(mca_btl_base_module_t *btl,
                                    const mca_btl_base_receive_descriptor_t *descriptor);
...
extern void
mca_pml_ob1_recv_frag_callback_frag(mca_btl_base_module_t *btl,
                                    const mca_btl_base_receive_descriptor_t *descriptor);
 ...
 */
extern mca_pml_ob1_recv_frag_t *check_cantmatch_for_match(mca_pml_ob1_comm_proc_t *proc);
