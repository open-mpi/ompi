...
OBJ_CLASS_INSTANCE(mca_pml_ob1_send_range_t, opal_free_list_item_t, NULL, NULL);
...
void mca_pml_ob1_send_request_process_pending(mca_bml_base_btl_t *bml_btl)
{
    ...
    for (i = 0; i < s; i++) {
        ...
        case MCA_PML_OB1_SEND_PENDING_START:
            send_dst = mca_bml_base_btl_array_find(&sendreq->req_endpoint->btl_eager, bml_btl->btl);
            if (NULL == send_dst) {
                ...
                if (OMPI_ERR_OUT_OF_RESOURCE == rc) {
                    ...
                    add_request_to_send_pending(sendreq, MCA_PML_OB1_SEND_PENDING_START, false);
                    return;
                }
            }
            break;
...
static int mca_pml_ob1_send_request_free(struct ompi_request_t **request)
{
    ...
    if (false == sendreq->req_send.req_base.req_free_called) {
        ...
        PERUSE_TRACE_COMM_EVENT(PERUSE_COMM_REQ_NOTIFY, &(sendreq->req_send.req_base), PERUSE_SEND);
...
OBJ_CLASS_INSTANCE(mca_pml_ob1_send_request_t, mca_pml_base_send_request_t,
                   mca_pml_ob1_send_request_construct, mca_pml_ob1_send_request_destruct);

/**
 * Completion of a short message - nothing left to schedule.
 */

static inline void mca_pml_ob1_match_completion_free_request(mca_bml_base_btl_t *bml_btl,
                                                             mca_pml_ob1_send_request_t *sendreq)
{
    if (sendreq->req_send.req_bytes_packed > 0) {
        PERUSE_TRACE_COMM_EVENT(PERUSE_COMM_REQ_XFER_BEGIN, &(sendreq->req_send.req_base),
                                PERUSE_SEND);
    }
...
static void mca_pml_ob1_match_completion_free(struct mca_btl_base_module_t *btl,
                                              ...
                                              struct mca_btl_base_descriptor_t *des, int status)
{
    ...
    if (OPAL_UNLIKELY(OMPI_SUCCESS != status)) {
        opal_output_verbose(mca_pml_ob1_output, 1, "pml:ob1: %s: operation failed with code %d",
                            __func__, status);
...
static inline void mca_pml_ob1_rndv_completion_request(mca_bml_base_btl_t *bml_btl,
                                                       mca_pml_ob1_send_request_t *sendreq,
                                                       size_t req_bytes_delivered)
{
    ...
    SPC_USER_OR_MPI(sendreq->req_send.req_base.req_ompi.req_status.MPI_TAG,
                    (ompi_spc_value_t) req_bytes_delivered, OMPI_SPC_BYTES_SENT_USER,
 ...
 */
static void mca_pml_ob1_rndv_completion(mca_btl_base_module_t *btl,
                                        struct mca_btl_base_endpoint_t *ep,
                                        struct mca_btl_base_descriptor_t *des, int status)
{
    mca_pml_ob1_send_request_t *sendreq = (mca_pml_ob1_send_request_t *) des->des_cbdata;
...
static void mca_pml_ob1_rget_completion(mca_pml_ob1_rdma_frag_t *frag, int64_t rdma_length)
{
    ...
    if (OPAL_LIKELY(0 < rdma_length)) {
        frag_remaining = OPAL_THREAD_SUB_FETCH_SIZE_T(&frag->rdma_bytes_remaining,
                                                      (size_t) rdma_length);
        SPC_USER_OR_MPI(sendreq->req_send.req_base.req_ompi.req_status.MPI_TAG,
                        (ompi_spc_value_t) rdma_length, OMPI_SPC_BYTES_SENT_USER,
                        OMPI_SPC_BYTES_SENT_MPI);
...
static void mca_pml_ob1_send_ctl_completion(mca_btl_base_module_t *btl,
                                            struct mca_btl_base_endpoint_t *ep,
                                            struct mca_btl_base_descriptor_t *des, int status)
{
    mca_bml_base_btl_t *bml_btl = (mca_bml_base_btl_t *) des->des_context;
...
static void mca_pml_ob1_frag_completion(mca_btl_base_module_t *btl,
                                        struct mca_btl_base_endpoint_t *ep,
                                        struct mca_btl_base_descriptor_t *des, int status)
{
    mca_pml_ob1_send_request_t *sendreq = (mca_pml_ob1_send_request_t *) des->des_cbdata;
 ...
 */
static void mca_pml_ob1_copy_frag_completion(mca_btl_base_module_t *btl,
                                             struct mca_btl_base_endpoint_t *ep,
                                             struct mca_btl_base_descriptor_t *des, int status)
{
    int rc;
...
int mca_pml_ob1_send_request_start_buffered(mca_pml_ob1_send_request_t *sendreq,
                                            mca_bml_base_btl_t *bml_btl, size_t size)
{
    ...
    /* allocate descriptor */
    mca_bml_base_alloc(bml_btl, &des, MCA_BTL_NO_ORDER, sizeof(mca_pml_ob1_rendezvous_hdr_t) + size,
    ...
    iov_count = 1;
    max_data = size;
    if ((rc = opal_convertor_pack(&sendreq->req_send.req_base.req_convertor, &iov, &iov_count,
                                  &max_data))
        < 0) {
                                        ...
                                        mca_bml_base_btl_t *bml_btl, size_t size)
{
    ...
    if (NULL != bml_btl->btl->btl_sendi) {
        ...
        rc = mca_bml_base_sendi(bml_btl, &sendreq->req_send.req_base.req_convertor, &match,
                                OMPI_PML_OB1_MATCH_HDR_LEN, size, MCA_BTL_NO_ORDER,
                                MCA_BTL_DES_FLAGS_PRIORITY | MCA_BTL_DES_FLAGS_BTL_OWNERSHIP,
                                MCA_PML_OB1_HDR_TYPE_MATCH, &des);
        if (OPAL_LIKELY(OMPI_SUCCESS == rc)) {
            /* signal request completion */
            SPC_USER_OR_MPI(sendreq->req_send.req_base.req_ompi.req_status.MPI_TAG,
                            (ompi_spc_value_t) size, OMPI_SPC_BYTES_SENT_USER,
                            OMPI_SPC_BYTES_SENT_MPI);
            send_request_pml_complete(sendreq);
            return OMPI_SUCCESS;
        }
        ...
        /* allocate descriptor */
        mca_bml_base_alloc(bml_btl, &des, MCA_BTL_NO_ORDER, OMPI_PML_OB1_MATCH_HDR_LEN + size,
                           MCA_BTL_DES_FLAGS_PRIORITY | MCA_BTL_DES_FLAGS_BTL_OWNERSHIP);
    }
    if (OPAL_UNLIKELY(NULL == des)) {
        return OMPI_ERR_OUT_OF_RESOURCE;
    ...
    if (size > 0) {
        ...
        MEMCHECKER(memchecker_call(&opal_memchecker_base_mem_defined,
                                   ...
                                   sendreq->req_send.req_base.req_count,
                                   sendreq->req_send.req_base.req_datatype););
        (void) opal_convertor_pack(&sendreq->req_send.req_base.req_convertor, &iov, &iov_count,
                                   &max_data);
        /*
                                           ...
                                           mca_bml_base_btl_t *bml_btl, size_t size)
{
    ...
    /* prepare descriptor */
    mca_bml_base_prepare_src(bml_btl, &sendreq->req_send.req_base.req_convertor, MCA_BTL_NO_ORDER,
                             OMPI_PML_OB1_MATCH_HDR_LEN, &size,
                             MCA_BTL_DES_FLAGS_PRIORITY | MCA_BTL_DES_FLAGS_BTL_OWNERSHIP, &des);
    if (OPAL_UNLIKELY(NULL == des)) {
        return OMPI_ERR_OUT_OF_RESOURCE;
    }
    segment = des->des_segments;
                                        ...
                                        mca_bml_base_btl_t *bml_btl, size_t size)
{
    ...
    if (!(bml_btl->btl_flags & (MCA_BTL_FLAGS_GET | MCA_BTL_FLAGS_CUDA_GET))) {
        sendreq->rdma_frag = NULL;
        /* This BTL does not support get. Use rendezvous to start the RDMA operation using put
         * instead. */
        return mca_pml_ob1_send_request_start_rndv(sendreq, bml_btl, 0,
    ...
    /* do not store the local handle in the fragment. it will be released by
    ...
    mca_pml_ob1_rget_hdr_prepare(hdr, MCA_PML_OB1_HDR_FLAGS_CONTIG | MCA_PML_OB1_HDR_FLAGS_PIN,
                                 ...
                                 sendreq->req_send.req_base.req_tag,
                                 (uint16_t) sendreq->req_send.req_base.req_sequence,
                                 sendreq->req_send.req_bytes_packed, sendreq, frag, data_ptr,
                                 local_handle, reg_size);
...
int mca_pml_ob1_send_request_start_rndv(mca_pml_ob1_send_request_t *sendreq,
                                        mca_bml_base_btl_t *bml_btl, size_t size, int flags)
{
    mca_btl_base_descriptor_t *des;
    mca_btl_base_segment_t *segment;
    mca_pml_ob1_hdr_t *hdr;
    ...
    /* prepare descriptor */
    if (size == 0) {
        mca_bml_base_alloc(bml_btl, &des, MCA_BTL_NO_ORDER, sizeof(mca_pml_ob1_rendezvous_hdr_t),
                           MCA_BTL_DES_FLAGS_PRIORITY | MCA_BTL_DES_FLAGS_BTL_OWNERSHIP);
    } else {
        MEMCHECKER(memchecker_call(&opal_memchecker_base_mem_defined,
                                   sendreq->req_send.req_base.req_addr,
    ...
    /* build hdr */
    hdr = (mca_pml_ob1_hdr_t *) segment->seg_addr.pval;
    mca_pml_ob1_rendezvous_hdr_prepare(&hdr->hdr_rndv, MCA_PML_OB1_HDR_TYPE_RNDV,
                                       flags | MCA_PML_OB1_HDR_FLAGS_SIGNAL,
                                       sendreq->req_send.req_base.req_comm->c_contextid,
                                       sendreq->req_send.req_base.req_comm->c_my_rank,
                                       sendreq->req_send.req_base.req_tag,
                                       (uint16_t) sendreq->req_send.req_base.req_sequence,
...
void mca_pml_ob1_send_request_copy_in_out(mca_pml_ob1_send_request_t *sendreq, uint64_t send_offset,
                                          uint64_t send_length)
{
    ...
    for (n = 0; n < num_btls && n < mca_pml_ob1.max_send_per_range; n++) {
        sr->range_btls[n].bml_btl = mca_bml_base_btl_array_get_next(&bml_endpoint->btl_send);
        weight_total += sr->range_btls[n].bml_btl->btl_weight;
    }

    sr->range_btl_cnt = n;
    mca_pml_ob1_calc_weighted_length(sr->range_btls, n, send_length, weight_total);
...
static inline mca_pml_ob1_send_range_t *get_send_range_nolock(mca_pml_ob1_send_request_t *sendreq)
{
...
static inline mca_pml_ob1_send_range_t *get_send_range(mca_pml_ob1_send_request_t *sendreq)
{
...
static inline mca_pml_ob1_send_range_t *get_next_send_range(mca_pml_ob1_send_request_t *sendreq,
                                                            mca_pml_ob1_send_range_t *range)
{
    OPAL_THREAD_LOCK(&sendreq->req_send_range_lock);
...
int mca_pml_ob1_send_request_schedule_once(mca_pml_ob1_send_request_t *sendreq)
{
    ...
    while (range
           && (false == sendreq->req_throttle_sends
               || sendreq->req_pipeline_depth < mca_pml_ob1.send_pipeline_depth)) {
        ...
        if (OPAL_UNLIKELY(num_fail == range->range_btl_cnt)) {
            ...
            add_request_to_send_pending(sendreq, MCA_PML_OB1_SEND_PENDING_SCHEDULE, true);
    ...
    cannot_pack:
        ...
        if (bml_btl->btl->btl_max_send_size != 0) {
            ...
            if ((sendreq->req_send.req_base.req_convertor.flags & CONVERTOR_CUDA)
                && (bml_btl->btl->btl_cuda_max_send_size != 0)) {
                max_send_size = bml_btl->btl->btl_cuda_max_send_size
                                - sizeof(mca_pml_ob1_frag_hdr_t);
            } else {
                max_send_size = bml_btl->btl->btl_max_send_size - sizeof(mca_pml_ob1_frag_hdr_t);
            }
#else  /* OPAL_CUDA_SUPPORT */
            size_t max_send_size = bml_btl->btl->btl_max_send_size - sizeof(mca_pml_ob1_frag_hdr_t);
#endif /* OPAL_CUDA_SUPPORT */
        ...
        opal_convertor_set_position(&sendreq->req_send.req_base.req_convertor, &offset);
        ...
        MEMCHECKER(memchecker_call(&opal_memchecker_base_mem_defined,
                                   ...
                                   sendreq->req_send.req_base.req_datatype););
        mca_bml_base_prepare_src(bml_btl, &sendreq->req_send.req_base.req_convertor,
                                 MCA_BTL_NO_ORDER, sizeof(mca_pml_ob1_frag_hdr_t), &size,
        ...
        ob1_hdr_hton(hdr, MCA_PML_OB1_HDR_TYPE_FRAG, sendreq->req_send.req_base.req_proc);

#if OMPI_WANT_PERUSE
        PERUSE_TRACE_COMM_OMPI_EVENT(PERUSE_COMM_REQ_XFER_CONTINUE, &(sendreq->req_send.req_base),
                                     size, PERUSE_SEND);
                                       ...
                                       void *context, void *cbdata, int status)
{
    ...
    if (OPAL_UNLIKELY(OMPI_SUCCESS == status)) {
        /* TODO -- read ordering */
        mca_pml_ob1_send_fin(sendreq->req_send.req_base.req_proc, bml_btl,
                             frag->rdma_hdr.hdr_rdma.hdr_frag, frag->rdma_length, 0, 0);

        /* check for request completion */
        OPAL_THREAD_ADD_FETCH_SIZE_T(&sendreq->req_bytes_delivered, frag->rdma_length);
        SPC_USER_OR_MPI(sendreq->req_send.req_base.req_ompi.req_status.MPI_TAG,
                        (ompi_spc_value_t) frag->rdma_length, OMPI_SPC_BYTES_SENT_USER,
                        OMPI_SPC_BYTES_SENT_MPI);
...
int mca_pml_ob1_send_request_put_frag(mca_pml_ob1_rdma_frag_t *frag)
{
    ...
    PERUSE_TRACE_COMM_OMPI_EVENT(PERUSE_COMM_REQ_XFER_CONTINUE,
                                 &(((mca_pml_ob1_send_request_t *) frag->rdma_req)
                                       ->req_send.req_base),
                                 frag->rdma_length, PERUSE_SEND);

    rc = mca_bml_base_put(bml_btl, frag->local_address, frag->remote_address, local_handle,
                          (mca_btl_base_registration_handle_t *) frag->remote_handle,
                          frag->rdma_length, 0, MCA_BTL_NO_ORDER, mca_pml_ob1_put_completion, frag);
...
void mca_pml_ob1_send_request_put(mca_pml_ob1_send_request_t *sendreq, mca_btl_base_module_t *btl,
                                  const mca_pml_ob1_rdma_hdr_t *hdr)
{
    ...
    if (NULL == sendreq->rdma_frag) {
        ...
        if (OPAL_UNLIKELY(NULL == frag)) {
            /* send fragment by copy in/out */
            mca_pml_ob1_send_request_copy_in_out(sendreq, hdr->hdr_rdma_offset, hdr->hdr_dst_size);
            /* if a pointer to a receive request is not set it means that
             * ACK was not yet received. Don't schedule sends before ACK */
            if (NULL != sendreq->req_recv.pval) {
                mca_pml_ob1_send_request_schedule(sendreq);
