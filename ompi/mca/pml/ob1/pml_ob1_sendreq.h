...
/**
 * Make this request live, exactly once. It becomes OMPI_REQUEST_ACTIVE,
 * so a waiter may hang its sync off req_complete; a second call would
 * store REQUEST_PENDING over that field and leave the waiter to sleep
 * through the completion. Hence the split from
 * mca_pml_ob1_send_request_start_endpoint(): a request parked on an
 * unreachable peer is live but has not reached a btl.
 */
static inline void
...
/**
 * Hand an already-live request to the btls. Re-arms nothing, so it serves
 * both a fresh request and a parked one whose peer became reachable.
 */
static inline int
...
mca_pml_ob1_send_request_start( mca_pml_ob1_send_request_t* sendreq )
{
    ...
    /* MPI_Send_init prepared this convertor without touching the bml, so
     * it may predate learning the peer's architecture -- which getting
     * the endpoint above has now done. */
