...
static inline void add_request_to_send_pending(mca_pml_ob1_send_request_t *sendreq,
...
#define MCA_PML_OB1_SEND_REQUEST_ALLOC(comm, dst, sendreq)                \
    {                                                                     \
        ompi_proc_t *proc = ompi_comm_peer_lookup(comm, dst);             \
                                                                          \
        if (OPAL_LIKELY(NULL != proc)) {                                  \
            sendreq = (mca_pml_ob1_send_request_t *) opal_free_list_wait( \
                &mca_pml_base_send_requests);                             \
            sendreq->req_send.req_base.req_proc = proc;                   \
        }                                                                 \
    }

#define MCA_PML_OB1_SEND_REQUEST_INIT(sendreq, buf, count, datatype, dst, tag, comm, sendmode,     \
                                      persistent)                                                  \
    {                                                                                              \
        MCA_PML_BASE_SEND_REQUEST_INIT(&(sendreq)->req_send, buf, count, datatype, dst, tag, comm, \
                                       sendmode, persistent, 0); /* convertor_flags */             \
...
#define MCA_PML_OB1_SEND_REQUEST_MPI_COMPLETE(sendreq, with_signal)                      \
    do {                                                                                 \
        ...
        PERUSE_TRACE_COMM_EVENT(PERUSE_COMM_REQ_COMPLETE, &(sendreq->req_send.req_base), \
                                PERUSE_SEND);                                            \
                                                                                         \
 ...
 *
 */
static inline void send_request_pml_complete(mca_pml_ob1_send_request_t *sendreq)
{
    if (false == sendreq->req_send.req_base.req_pml_complete) {
        if (sendreq->req_send.req_bytes_packed > 0) {
            PERUSE_TRACE_COMM_EVENT(PERUSE_COMM_REQ_XFER_END, &(sendreq->req_send.req_base),
                                    PERUSE_SEND);
        }
            ...
            if (!REQUEST_COMPLETE(&((sendreq->req_send).req_base.req_ompi))) {
                ...
                if (MPI_SUCCESS != sendreq->req_send.req_base.req_ompi.req_status.MPI_ERROR) {
                    ...
                    ompi_mpi_errors_are_fatal_comm_handler(NULL, &err,
                                                           "Send error after request freed");
                }
            }
        } else {
...
static inline bool send_request_pml_complete_check(mca_pml_ob1_send_request_t *sendreq)
{
    opal_atomic_rmb();
 ...
 */
int mca_pml_ob1_send_request_schedule_once(mca_pml_ob1_send_request_t *);

static inline int mca_pml_ob1_send_request_schedule_exclusive(mca_pml_ob1_send_request_t *sendreq)
{
    int rc;
    do {
...
static inline void mca_pml_ob1_send_request_schedule(mca_pml_ob1_send_request_t *sendreq)
{
    /*
...
static inline int mca_pml_ob1_send_request_start_btl(mca_pml_ob1_send_request_t *sendreq,
                                                     mca_bml_base_btl_t *bml_btl)
{
    ...
    if (OPAL_LIKELY(size <= eager_limit)) {
        ...
        } else if (opal_convertor_need_buffers(&sendreq->req_send.req_base.req_convertor)
                   == false) {
            unsigned char *base;
            opal_convertor_get_current_pointer(&sendreq->req_send.req_base.req_convertor,
                                               (void **) &base);

            if (0
                != (sendreq->req_rdma_cnt = (uint32_t)
                        mca_pml_ob1_rdma_btls(sendreq->req_endpoint, base,
                                              sendreq->req_send.req_bytes_packed,
                                              sendreq->req_rdma))) {
...
static inline int mca_pml_ob1_send_request_start_seq(mca_pml_ob1_send_request_t *sendreq,
...
static inline int mca_pml_ob1_send_request_start(mca_pml_ob1_send_request_t *sendreq)
{
    mca_bml_base_endpoint_t *endpoint = mca_bml_base_get_endpoint(
        sendreq->req_send.req_base.req_proc);
    ompi_communicator_t *comm = sendreq->req_send.req_base.req_comm;
    mca_pml_ob1_comm_proc_t *ob1_proc = mca_pml_ob1_peer_lookup(comm, sendreq->req_send.req_base
                                                                          .req_peer);
    int32_t seqn;
