...
OBJ_CLASS_INSTANCE(mca_pml_ob1_pckt_pending_t, opal_free_list_item_t, NULL, NULL);
...
mca_pml_ob1_component_init(int *priority, bool enable_progress_threads, bool enable_mpi_threads);
...
static inline int mca_pml_ob1_param_register_int(const char *param_name, int default_value,
                                                 int *storage)
{
    *storage = default_value;
    (void) mca_base_component_var_register(&mca_pml_ob1_component.pmlm_version, param_name, NULL,
                                           MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_9,
                                           MCA_BASE_VAR_SCOPE_READONLY, storage);
    return *storage;
}

static inline unsigned int mca_pml_ob1_param_register_uint(const char *param_name,
...
static int mca_pml_ob1_comm_size_notify(mca_base_pvar_t *pvar, mca_base_pvar_event_t event,
...
static int mca_pml_ob1_get_unex_msgq_size(const struct mca_base_pvar_t *pvar, void *value,
                                          void *obj_handle)
{
    ...
    for (i = 0; i < comm_size; ++i) {
        ...
        if (pml_proc) {
            ...
            values[i] = custom_match_umq_size(
                pml_comm
                    ->umq); // TODO: given the structure of custom match this does not make sense,
...
static int mca_pml_ob1_get_posted_recvq_size(const struct mca_base_pvar_t *pvar, void *value,
                                             void *obj_handle)
{
    ompi_communicator_t *comm = (ompi_communicator_t *) obj_handle;
    mca_pml_ob1_comm_t *pml_comm = comm->c_pml_comm;
    ...
    for (i = 0; i < comm_size; ++i) {
        ...
        if (pml_proc) {
#if MCA_PML_OB1_CUSTOM_MATCH
            values[i] = custom_match_prq_size(
                pml_comm
                    ->prq); // TODO: given the structure of custom match this does not make sense,
                            //       as we only have one set of queues.
#else
...
static int mca_pml_ob1_component_register(void)
{
    ...
    /*     mca_pml_ob1_param_register_sizet("rdma_retries_limit", 5,
    ...
    (void) mca_base_component_pvar_register(&mca_pml_ob1_component.pmlm_version,
                                            "unexpected_msgq_length",
                                            "Number of unexpected messages "
                                            "received by each peer in a communicator",
                                            ...
                                            mca_pml_ob1_get_unex_msgq_size, NULL,
                                            mca_pml_ob1_comm_size_notify, NULL);

    (void) mca_base_component_pvar_register(&mca_pml_ob1_component.pmlm_version,
                                            "posted_recvq_length",
                                            "Number of unmatched receives "
                                            ...
                                            MCA_BASE_PVAR_FLAG_READONLY
                                                | MCA_BASE_PVAR_FLAG_CONTINUOUS,
                                            mca_pml_ob1_get_posted_recvq_size, NULL,
                                            mca_pml_ob1_comm_size_notify, NULL);
...
mca_pml_ob1_component_init(int *priority, bool enable_progress_threads, bool enable_mpi_threads)
{
    ...
    opal_output_verbose(10, mca_pml_ob1_output, "in ob1, my priority is %d\n",
    ...
    *priority = mca_pml_ob1.priority;
    ...
    mca_pml_ob1.allocator = allocator_component->allocator_init(true, mca_pml_ob1_seg_alloc,
