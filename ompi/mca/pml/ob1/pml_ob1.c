...
extern void mca_pml_ob1_cuda_add_ipc_support(struct mca_btl_base_module_t *btl, int32_t flags,
                                             ompi_proc_t *errproc, char *btlinfo);
#endif /* OPAL_CUDA_SUPPORT */

void mca_pml_ob1_error_handler(struct mca_btl_base_module_t *btl, int32_t flags,
...
int mca_pml_ob1_enable(bool enable)
{
    ...
    opal_free_list_init(&mca_pml_ob1.rdma_frags, sizeof(mca_pml_ob1_rdma_frag_t),
                        opal_cache_line_size, OBJ_CLASS(mca_pml_ob1_rdma_frag_t), 0,
                        opal_cache_line_size, mca_pml_ob1.free_list_num, mca_pml_ob1.free_list_max,
                        mca_pml_ob1.free_list_inc, NULL, 0, NULL, NULL, NULL);
    ...
    opal_free_list_init(&mca_pml_ob1.recv_frags,
                        ...
                        opal_cache_line_size, OBJ_CLASS(mca_pml_ob1_recv_frag_t), 0,
                        opal_cache_line_size, mca_pml_ob1.free_list_num, mca_pml_ob1.free_list_max,
                        mca_pml_ob1.free_list_inc, NULL, 0, NULL, NULL, NULL);

    OBJ_CONSTRUCT(&mca_pml_ob1.pending_pckts, opal_free_list_t);
    opal_free_list_init(&mca_pml_ob1.pending_pckts, sizeof(mca_pml_ob1_pckt_pending_t),
                        opal_cache_line_size, OBJ_CLASS(mca_pml_ob1_pckt_pending_t), 0,
                        opal_cache_line_size, mca_pml_ob1.free_list_num, mca_pml_ob1.free_list_max,
                        mca_pml_ob1.free_list_inc, NULL, 0, NULL, NULL, NULL);
    ...
    opal_free_list_init(&mca_pml_ob1.send_ranges,
                        ...
                        opal_cache_line_size, OBJ_CLASS(mca_pml_ob1_send_range_t), 0,
                        opal_cache_line_size, mca_pml_ob1.free_list_num, mca_pml_ob1.free_list_max,
                        mca_pml_ob1.free_list_inc, NULL, 0, NULL, NULL, NULL);
    ...
    opal_free_list_init(&mca_pml_base_send_requests,
                        ...
                        opal_cache_line_size, OBJ_CLASS(mca_pml_ob1_send_request_t), 0,
                        opal_cache_line_size, mca_pml_ob1.free_list_num, mca_pml_ob1.free_list_max,
                        mca_pml_ob1.free_list_inc, NULL, 0, NULL, NULL, NULL);
    ...
    opal_free_list_init(&mca_pml_base_recv_requests,
                        sizeof(mca_pml_ob1_recv_request_t)
                            + sizeof(mca_pml_ob1_com_btl_t[mca_pml_ob1.max_rdma_per_request]),
                        opal_cache_line_size, OBJ_CLASS(mca_pml_ob1_recv_request_t), 0,
                        opal_cache_line_size, mca_pml_ob1.free_list_num, mca_pml_ob1.free_list_max,
                        mca_pml_ob1.free_list_inc, NULL, 0, NULL, NULL, NULL);
...
int mca_pml_ob1_add_comm(ompi_communicator_t *comm)
{
    ...
    OPAL_LIST_FOREACH_SAFE (frag, next_frag, &mca_pml_ob1.non_existing_communicator_pending,
         ...
         */
        PERUSE_TRACE_MSG_EVENT(PERUSE_COMM_MSG_ARRIVED, comm, hdr->hdr_src, hdr->hdr_tag,
                               PERUSE_RECV);
...
#endif
            PERUSE_TRACE_MSG_EVENT(PERUSE_COMM_MSG_INSERT_IN_UNEX_Q, comm, hdr->hdr_src,
                                   hdr->hdr_tag, PERUSE_RECV);
            continue;
        }
...
int mca_pml_ob1_add_procs(ompi_proc_t **procs, size_t nprocs)
{
    ...
    if (OMPI_SUCCESS != (rc = mca_pml_base_pml_check_selected("ob1", procs, nprocs))) {
        return rc;
    }
    ...
    OPAL_LIST_FOREACH (sm, &mca_btl_base_modules_initialized, mca_btl_base_selected_module_t) {
        if ((MCA_BTL_FLAGS_SEND & sm->btl_module->btl_flags)
            && sm->btl_module->btl_eager_limit < sizeof(mca_pml_ob1_hdr_t)) {
            opal_show_help("help-mpi-pml-ob1.txt", "eager_limit_too_small", true,
        ...
        if (0 != sm->btl_module->btl_cuda_eager_limit) {
            if (sm->btl_module->btl_cuda_eager_limit < sizeof(mca_pml_ob1_hdr_t)) {
                opal_show_help("help-mpi-pml-ob1.txt", "cuda_eager_limit_too_small", true,
                               sm->btl_component->btl_version.mca_component_name,
                               ompi_process_info.nodename,
                               sm->btl_component->btl_version.mca_component_name,
        ...
        if (0 == sm->btl_module->btl_cuda_rdma_limit) {
            ...
            if (sm->btl_module->btl_cuda_rdma_limit < sm->btl_module->btl_cuda_eager_limit) {
                opal_show_help("help-mpi-pml-ob1.txt", "cuda_rdma_limit_too_small", true,
                               sm->btl_component->btl_version.mca_component_name,
                               ompi_process_info.nodename,
                               sm->btl_component->btl_version.mca_component_name,
    ...
    rc = mca_bml.bml_register(MCA_PML_OB1_HDR_TYPE_MATCH, mca_pml_ob1_recv_frag_callback_match,
                              NULL);
    if (OMPI_SUCCESS != rc)
        goto cleanup_and_return;

    rc = mca_bml.bml_register(MCA_PML_OB1_HDR_TYPE_RNDV, mca_pml_ob1_recv_frag_callback_rndv, NULL);
    if (OMPI_SUCCESS != rc)
        goto cleanup_and_return;

    rc = mca_bml.bml_register(MCA_PML_OB1_HDR_TYPE_RGET, mca_pml_ob1_recv_frag_callback_rget, NULL);
    if (OMPI_SUCCESS != rc)
        goto cleanup_and_return;

    rc = mca_bml.bml_register(MCA_PML_OB1_HDR_TYPE_ACK, mca_pml_ob1_recv_frag_callback_ack, NULL);
    if (OMPI_SUCCESS != rc)
        goto cleanup_and_return;

    rc = mca_bml.bml_register(MCA_PML_OB1_HDR_TYPE_FRAG, mca_pml_ob1_recv_frag_callback_frag, NULL);
    if (OMPI_SUCCESS != rc)
        goto cleanup_and_return;

    rc = mca_bml.bml_register(MCA_PML_OB1_HDR_TYPE_PUT, mca_pml_ob1_recv_frag_callback_put, NULL);
    if (OMPI_SUCCESS != rc)
        goto cleanup_and_return;

    rc = mca_bml.bml_register(MCA_PML_OB1_HDR_TYPE_FIN, mca_pml_ob1_recv_frag_callback_fin, NULL);
    if (OMPI_SUCCESS != rc)
        goto cleanup_and_return;
...
static void mca_pml_ob1_dump_hdr(mca_pml_ob1_hdr_t *hdr)
{
    ...
    case MCA_PML_OB1_HDR_TYPE_MATCH:
        type = "MATCH";
        snprintf(header, 128, "ctx %5d src %d tag %d seq %d", hdr->hdr_match.hdr_ctx,
                 hdr->hdr_match.hdr_src, hdr->hdr_match.hdr_tag, hdr->hdr_match.hdr_seq);
        break;
    ...
    case MCA_PML_OB1_HDR_TYPE_RGET:
        type = "RGET";
        snprintf(header, 128,
                 "ctx %5d src %d tag %d seq %d msg_length %" PRIu64 "frag %" PRIu64
                 " src_ptr %" PRIu64,
                 hdr->hdr_rndv.hdr_match.hdr_ctx, hdr->hdr_rndv.hdr_match.hdr_src,
                 hdr->hdr_rndv.hdr_match.hdr_tag, hdr->hdr_rndv.hdr_match.hdr_seq,
    ...
    case MCA_PML_OB1_HDR_TYPE_FRAG:
        ...
        snprintf(header, 128, "offset %" PRIu64 " src_req %p dst_req %p",
                 hdr->hdr_frag.hdr_frag_offset, hdr->hdr_frag.hdr_src_req.pval,
                 hdr->hdr_frag.hdr_dst_req.pval);
        break;
    case MCA_PML_OB1_HDR_TYPE_PUT:
        type = "PUT";
        snprintf(header, 128,
                 "dst_req %p src_frag %p recv_req %p offset %" PRIu64 " dst_ptr %" PRIu64
                 " dst_size %" PRIu64,
    ...
    opal_output(0, "hdr %s [%s] %s", type,
                (hdr->hdr_common.hdr_flags & MCA_PML_OB1_HDR_FLAGS_NBO ? "nbo" : "   "), header);
}
...
static void mca_pml_ob1_dump_frag_list(opal_list_t *queue, bool is_req)
{
    ...
    for (item = opal_list_get_first(queue); item != opal_list_get_end(queue);
         item = opal_list_get_next(item)) {

        if (is_req) {
            mca_pml_base_request_t *req = &(
                ((mca_pml_ob1_recv_request_t *) item)->req_recv.req_base);
...
int mca_pml_ob1_dump(struct ompi_communicator_t *comm, int verbose)
{
    ...
    for (i = 0; i < (int) pml_comm->num_procs; i++) {
        ...
        opal_output(0, "[Rank %d] expected_seq %d ompi_proc %p send_seq %d\n", i,
                    proc->expected_sequence, (void *) proc->ompi_proc, proc->send_sequence);
 ...
 * later.
 */
int mca_pml_ob1_send_fin(ompi_proc_t *proc, mca_bml_base_btl_t *bml_btl, opal_ptr_t hdr_frag,
                         uint64_t rdma_size, uint8_t order, int status)
{
    ...
    /* fill in header */
    mca_pml_ob1_fin_hdr_prepare((mca_pml_ob1_fin_hdr_t *) fin->des_segments->seg_addr.pval, 0,
                                hdr_frag.lval, status ? status : (int64_t) rdma_size);

    ob1_hdr_hton((mca_pml_ob1_hdr_t *) fin->des_segments->seg_addr.pval, MCA_PML_OB1_HDR_TYPE_FIN,
                 proc);
...
void mca_pml_ob1_process_pending_packets(mca_bml_base_btl_t *bml_btl)
{
    ...
    for (i = 0; i < s; i++) {
        ...
        pckt = (mca_pml_ob1_pckt_pending_t *) opal_list_remove_first(&mca_pml_ob1.pckt_pending);
        OPAL_THREAD_UNLOCK(&mca_pml_ob1.lock);
        if (NULL == pckt)
            break;
        if (pckt->bml_btl != NULL && pckt->bml_btl->btl == bml_btl->btl) {
            send_dst = pckt->bml_btl;
        } else {
            mca_bml_base_endpoint_t *endpoint = (mca_bml_base_endpoint_t *) pckt->proc
                                                    ->proc_endpoints[OMPI_PROC_ENDPOINT_TAG_BML];
            send_dst = mca_bml_base_btl_array_find(&endpoint->btl_eager, bml_btl->btl);
        }
        if (NULL == send_dst) {
            OPAL_THREAD_LOCK(&mca_pml_ob1.lock);
            opal_list_append(&mca_pml_ob1.pckt_pending, (opal_list_item_t *) pckt);
            OPAL_THREAD_UNLOCK(&mca_pml_ob1.lock);
            continue;
        }
        ...
        case MCA_PML_OB1_HDR_TYPE_ACK:
            rc = mca_pml_ob1_recv_request_ack_send_btl(pckt->proc, send_dst,
                                                       ...
                                                       pckt->hdr.hdr_common.hdr_flags
                                                           & MCA_PML_OB1_HDR_FLAGS_NORDMA);
            ...
            break;
        case MCA_PML_OB1_HDR_TYPE_FIN:
            rc = mca_pml_ob1_send_fin(pckt->proc, send_dst, pckt->hdr.hdr_fin.hdr_frag,
                                      pckt->hdr.hdr_fin.hdr_size, pckt->order, pckt->status);
            if (OPAL_UNLIKELY(OMPI_ERR_OUT_OF_RESOURCE == rc)) {
...
void mca_pml_ob1_process_pending_rdma(void)
{
    ...
    for (i = 0; i < s; i++) {
        OPAL_THREAD_LOCK(&mca_pml_ob1.lock);
        frag = (mca_pml_ob1_rdma_frag_t *) opal_list_remove_first(&mca_pml_ob1.rdma_pending);
        OPAL_THREAD_UNLOCK(&mca_pml_ob1.lock);
        if (NULL == frag)
            break;
