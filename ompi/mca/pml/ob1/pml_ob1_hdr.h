...
struct mca_pml_ob1_match_hdr_t {
    ...
    uint8_t hdr_padding[2]; /**< explicitly pad to 16 bytes.  Compilers seem to already prefer to do
                               this, but make it explicit just in case */
#endif
};
#if OPAL_ENABLE_HETEROGENEOUS_SUPPORT || OPAL_ENABLE_DEBUG
...
static inline void mca_pml_ob1_match_hdr_prepare(mca_pml_ob1_match_hdr_t *hdr, uint8_t hdr_type,
                                                 uint8_t hdr_flags, uint16_t hdr_ctx,
                                                 int32_t hdr_src, int32_t hdr_tag, uint16_t hdr_seq)
{
...
static inline void mca_pml_ob1_rendezvous_hdr_prepare(mca_pml_ob1_rendezvous_hdr_t *hdr,
                                                      uint8_t hdr_type, uint8_t hdr_flags,
                                                      uint16_t hdr_ctx, int32_t hdr_src,
                                                      int32_t hdr_tag, uint16_t hdr_seq,
                                                      uint64_t hdr_msg_length, void *hdr_src_req)
{
    mca_pml_ob1_match_hdr_prepare(&hdr->hdr_match, hdr_type, hdr_flags, hdr_ctx, hdr_src, hdr_tag,
                                  hdr_seq);
...
static inline void mca_pml_ob1_rget_hdr_prepare(mca_pml_ob1_rget_hdr_t *hdr, uint8_t hdr_flags,
                                                uint16_t hdr_ctx, int32_t hdr_src, int32_t hdr_tag,
                                                uint16_t hdr_seq, uint64_t hdr_msg_length,
                                                void *hdr_src_req, void *hdr_frag,
                                                void *hdr_src_ptr, void *local_handle,
                                                size_t local_handle_size)
{
    mca_pml_ob1_rendezvous_hdr_prepare(&hdr->hdr_rndv, MCA_PML_OB1_HDR_TYPE_RGET, hdr_flags,
                                       hdr_ctx, hdr_src, hdr_tag, hdr_seq, hdr_msg_length,
                                       hdr_src_req);
#if OPAL_ENABLE_DEBUG
    hdr->hdr_padding[0] = 0;
    hdr->hdr_padding[1] = 0;
...
struct mca_pml_ob1_rdma_hdr_t {
    mca_pml_ob1_common_hdr_t hdr_common; /**< common attributes */
#if OPAL_ENABLE_HETEROGENEOUS_SUPPORT || OPAL_ENABLE_DEBUG
    uint8_t hdr_padding[6]; /** two to pad out the hdr to a 4 byte alignment.  hdr_req will then be
                               8 byte aligned after 4 for hdr_seg_cnt */
#endif
...
static inline void mca_pml_ob1_rdma_hdr_prepare(mca_pml_ob1_rdma_hdr_t *hdr, uint8_t hdr_flags,
                                                uint64_t hdr_req, void *hdr_frag,
                                                void *hdr_recv_req, uint64_t hdr_rdma_offset,
                                                void *hdr_dst_ptr, uint64_t hdr_dst_size,
                                                void *local_handle, size_t local_handle_size)
{
...
static inline __opal_attribute_always_inline__ void ob1_hdr_ntoh(mca_pml_ob1_hdr_t *hdr,
                                                                 const uint8_t hdr_type)
{
...
#else
#    define ob1_hdr_ntoh(h, t) \
        do {                   \
        } while (0)
#endif

#if OPAL_ENABLE_HETEROGENEOUS_SUPPORT
#    define ob1_hdr_hton(h, t, p) ob1_hdr_hton_intr((mca_pml_ob1_hdr_t *) h, t, p)
static inline __opal_attribute_always_inline__ void
ob1_hdr_hton_intr(mca_pml_ob1_hdr_t *hdr, const uint8_t hdr_type, const ompi_proc_t *proc)
{
...
static inline __opal_attribute_always_inline__ void ob1_hdr_copy(mca_pml_ob1_hdr_t *src,
                                                                 mca_pml_ob1_hdr_t *dst)
{
