...
#define MCA_PML_OB1_RECV_REQUEST_ALLOC(recvreq)                                                   \
    do {                                                                                          \
        recvreq = (mca_pml_ob1_recv_request_t *) opal_free_list_get(&mca_pml_base_recv_requests); \
    } while (0)
...
#define MCA_PML_OB1_RECV_REQUEST_INIT(request, addr, count, datatype, src, tag, comm, persistent) \
    do {                                                                                          \
        MCA_PML_BASE_RECV_REQUEST_INIT(&(request)->req_recv, addr, count, datatype, src, tag,     \
                                       comm, persistent);                                         \
    } while (0)
...
#define MCA_PML_OB1_RECV_REQUEST_MPI_COMPLETE(recvreq)                                   \
    do {                                                                                 \
        PERUSE_TRACE_COMM_EVENT(PERUSE_COMM_REQ_COMPLETE, &(recvreq->req_recv.req_base), \
                                PERUSE_RECV);                                            \
...
#define MCA_PML_OB1_RECV_REQUEST_RETURN(recvreq)                                                 \
    {                                                                                            \
        ...
        opal_free_list_return(&mca_pml_base_recv_requests, (opal_free_list_item_t *) (recvreq)); \
    }
 ...
 *  @param recvreq (IN)  Receive request.
 */
static inline void recv_request_pml_complete(mca_pml_ob1_recv_request_t *recvreq)
{
    size_t i;
    ...
    if (false == recvreq->req_recv.req_base.req_pml_complete) {
        ...
        if (recvreq->req_recv.req_bytes_packed > 0) {
            PERUSE_TRACE_COMM_EVENT(PERUSE_COMM_REQ_XFER_END, &recvreq->req_recv.req_base,
                                    PERUSE_RECV);
        }
        ...
        if (true == recvreq->req_recv.req_base.req_free_called) {
            if (MPI_SUCCESS != recvreq->req_recv.req_base.req_ompi.req_status.MPI_ERROR) {
                ...
                ompi_mpi_errors_are_fatal_comm_handler(NULL, &err,
                                                       "Recv error after request freed");
            }
            MCA_PML_OB1_RECV_REQUEST_RETURN(recvreq);
        } else {
            /* initialize request status */
            recvreq->req_recv.req_base.req_pml_complete = true;
            recvreq->req_recv.req_base.req_ompi.req_status._ucount = recvreq->req_bytes_received;
            if (recvreq->req_recv.req_bytes_packed > recvreq->req_bytes_expected) {
                recvreq->req_recv.req_base.req_ompi.req_status._ucount = recvreq->req_recv
                                                                             .req_bytes_packed;
                recvreq->req_recv.req_base.req_ompi.req_status.MPI_ERROR = MPI_ERR_TRUNCATE;
            }
...
static inline bool recv_request_pml_complete_check(mca_pml_ob1_recv_request_t *recvreq)
{
    opal_atomic_rmb();
...
static inline void prepare_recv_req_converter(mca_pml_ob1_recv_request_t *req)
{
    if (req->req_recv.req_base.req_datatype->super.size | req->req_recv.req_base.req_count) {
        opal_convertor_copy_and_prepare_for_recv(req->req_recv.req_base.req_proc->super
                                                     .proc_convertor,
                                                 &(req->req_recv.req_base.req_datatype->super),
                                                 req->req_recv.req_base.req_count,
                                                 req->req_recv.req_base.req_addr, 0,
                                    ...
                                    const mca_pml_ob1_match_hdr_t *hdr)
{
    ...
    if (req->req_recv.req_bytes_packed > 0) {
        ...
        PERUSE_TRACE_COMM_EVENT(PERUSE_COMM_REQ_XFER_BEGIN, &req->req_recv.req_base, PERUSE_RECV);
    }
}
...
#define MCA_PML_OB1_RECV_REQUEST_UNPACK(request, segments, num_segments, seg_offset, data_offset, \
        ...
        if (request->req_recv.req_bytes_packed > 0) {                                             \
            ...
            for (n = 0; n < num_segments; n++, segment++) {                                       \
                if (offset >= segment->seg_len) {                                                 \
                    ...
                    iov[iov_count].iov_base = (IOVBASE_TYPE *) ((unsigned char *)                 \
            ...
            PERUSE_TRACE_COMM_OMPI_EVENT(PERUSE_COMM_REQ_XFER_CONTINUE,                           \
                                         &(request->req_recv.req_base), max_data, PERUSE_RECV);   \
            opal_convertor_set_position(&(request->req_recv.req_base.req_convertor),              \
                                        &data_offset);                                            \
            opal_convertor_unpack(&(request)->req_recv.req_base.req_convertor, iov, &iov_count,   \
                                  &max_data);                                                     \
...
void mca_pml_ob1_recv_request_progress_match(mca_pml_ob1_recv_request_t *req,
...
void mca_pml_ob1_recv_request_progress_frag(mca_pml_ob1_recv_request_t *req,
                                            struct mca_btl_base_module_t *btl,
                                            const mca_btl_base_segment_t *segments,
                                            size_t num_segments);

#if OPAL_CUDA_SUPPORT
void mca_pml_ob1_recv_request_frag_copy_start(mca_pml_ob1_recv_request_t *req,
                                              struct mca_btl_base_module_t *btl,
                                              const mca_btl_base_segment_t *segments,
...
void mca_pml_ob1_recv_request_progress_rget(mca_pml_ob1_recv_request_t *req,
                                            struct mca_btl_base_module_t *btl,
                                            const mca_btl_base_segment_t *segments,
                                            size_t num_segments);
...
void mca_pml_ob1_recv_request_matched_probe(mca_pml_ob1_recv_request_t *req,
                                            struct mca_btl_base_module_t *btl,
                                            const mca_btl_base_segment_t *segments,
                                            size_t num_segments);
...
int mca_pml_ob1_recv_request_schedule_once(mca_pml_ob1_recv_request_t *req,
                                           mca_bml_base_btl_t *start_bml_btl);

static inline int mca_pml_ob1_recv_request_schedule_exclusive(mca_pml_ob1_recv_request_t *req,
                                                              mca_bml_base_btl_t *start_bml_btl)
{
    int rc;
...
static inline void mca_pml_ob1_recv_request_schedule(mca_pml_ob1_recv_request_t *req,
                                                     mca_bml_base_btl_t *start_bml_btl)
{
...
#define MCA_PML_OB1_ADD_ACK_TO_PENDING(P, S, D, O, Sz)                           \
    do {                                                                         \
        ...
        opal_list_append(&mca_pml_ob1.pckt_pending, (opal_list_item_t *) _pckt); \
...
static inline int mca_pml_ob1_recv_request_ack_send(mca_btl_base_module_t *btl, ompi_proc_t *proc,
                                                    uint64_t hdr_src_req, void *hdr_dst_req,
                                                    uint64_t hdr_send_offset, uint64_t size,
                                                    bool nordma)
{
    size_t i;
    ...
    for (i = 0; i < mca_bml_base_btl_array_get_size(&endpoint->btl_eager); i++) {
        ...
        if ((NULL == btl) || (btl == bml_btl->btl)) {
            if (mca_pml_ob1_recv_request_ack_send_btl(proc, bml_btl, hdr_src_req, hdr_dst_req,
    ...
    MCA_PML_OB1_ADD_ACK_TO_PENDING(proc, hdr_src_req, hdr_dst_req, hdr_send_offset, size);
