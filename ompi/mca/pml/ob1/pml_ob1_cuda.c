...
size_t mca_pml_ob1_rdma_cuda_btls(mca_bml_base_endpoint_t *bml_endpoint, unsigned char *base,
                                  size_t size, mca_pml_ob1_com_btl_t *rdma_btls);

int mca_pml_ob1_cuda_need_buffers(void *rreq, mca_btl_base_module_t *btl);
                                        ...
                                        mca_bml_base_btl_t *bml_btl, size_t size)
{
    ...
    if ((sendreq->req_send.req_base.req_convertor.flags & CONVERTOR_CUDA)
        && (sendreq->req_send.req_bytes_packed
            > (bml_btl->btl->btl_cuda_rdma_limit - sizeof(mca_pml_ob1_hdr_t)))) {
    ...
    if (opal_convertor_need_buffers(&sendreq->req_send.req_base.req_convertor) == false) {
        ...
        if (0
            != (sendreq->req_rdma_cnt = (uint32_t)
                    mca_pml_ob1_rdma_cuda_btls(sendreq->req_endpoint, base,
                                               sendreq->req_send.req_bytes_packed,
                                               sendreq->req_rdma))) {
                                  ...
                                  size_t size, mca_pml_ob1_com_btl_t *rdma_btls)
{
    ...
    for (n = 0; n < num_btls && num_btls_used < mca_pml_ob1.max_rdma_per_request; n++) {
        mca_bml_base_btl_t *bml_btl = mca_bml_base_btl_array_get_index(&bml_endpoint->btl_send, n);
        ...
        if (bml_btl->btl_flags & MCA_BTL_FLAGS_CUDA_GET) {
            ...
            if (NULL != bml_btl->btl->btl_register_mem) {
                /* register the memory */
                handle = bml_btl->btl->btl_register_mem(bml_btl->btl, bml_btl->btl_endpoint, base,
                                                        size,
...
int mca_pml_ob1_cuda_need_buffers(void *rreq, mca_btl_base_module_t *btl)
{
    ...
    mca_bml_base_endpoint_t *bml_endpoint = mca_bml_base_get_endpoint(
        ...
        if (ep->btl_send.bml_btls[i].btl == btl) {
            if (4 < opal_output_get_verbosity(btl_verbose_stream)) {
                ...
                opal_output(0,
                            ...
                            ((ompi_process_name_t *) &errproc->super.proc_name)->vpid, errhost);
                free(errhost);
            }
