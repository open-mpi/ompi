...
static inline void *custom_match_prq_find_verify(custom_match_prq *list, int tag, int peer)
{
    ...
    while (elem) {
        for (i = elem->start; i <= elem->end; i++) {
            if (elem->value[i]) {
                result = ((elem->tags[i] & elem->tmask[i]) == (tag & elem->tmask[i]))
                         && ((elem->srcs[i] & elem->smask[i]) == (peer & elem->smask[i]));
                if (result) {
                    return elem->value[i];
                }
            }
                                           ...
                                           int source)
{
    ...
    if ((!list->tail) || list->tail->end == PRQ_SIZE - 1) {
        ...
        for (i = 0; i < PRQ_SIZE; i++) {
            elem->value[i] = 0;
            elem->tags[i] = ~0; // TODO: we only have to do this type of initialization for freshly
                                // malloc'd entries.
            elem->tmask[i] = ~0;
            elem->srcs[i] = ~0;
            elem->smask[i] = ~0;
