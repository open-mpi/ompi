...
static inline void *custom_match_prq_find_verify(custom_match_prq *list, int tag, int peer)
{
    ...
    while (elem) {
        result = _mm512_cmpeq_epi8_mask(_mm512_and_epi32(elem->keys, elem->mask),
    ...
    while (elem) {
        ...
        for (int iter = elem->start; iter <= elem->end; iter++) {
            printf("Search = %x, Element Key = %x, Element mask = %x\n", ((int8_t *) &search)[iter],
                   ((int8_t *) &elem->keys)[iter], ((int8_t *) &elem->mask)[iter]);
        }
#endif
        result = _mm512_cmpeq_epi8_mask(_mm512_and_epi32(elem->keys, elem->mask),
                                        _mm512_and_epi32(search, elem->mask));
    ...
    for (elem = list->head; elem; elem = elem->next) {
        ...
        for (j = 0; j < 64; j++) {
            printf("%d:%d The key is %d, the mask is %d, the value is %ld\n", i, j,
                   ((int8_t *) (&(elem->keys)))[j], ((int8_t *) (&(elem->mask)))[j],
                   elem->value[j]);
        }
        i++;
    }
                                           ...
                                           void *payload)
{
    ...
    if ((!list->tail) || list->tail->end == 63) {
        ...
        elem->keys = _mm512_set1_epi8(~0); // TODO: we may only have to do this type of
