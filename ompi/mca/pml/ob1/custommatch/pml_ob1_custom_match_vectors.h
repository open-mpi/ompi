...
static inline void *custom_match_prq_find_verify(custom_match_prq *list, int tag, int peer)
{
    ...
    while (elem) {
        result = _mm512_cmpeq_epi32_mask(_mm512_and_epi32(elem->tags, elem->tmask),
                                         _mm512_and_epi32(tsearch, elem->tmask))
                 & _mm512_cmpeq_epi32_mask(_mm512_and_epi32(elem->srcs, elem->smask),
                                           _mm512_and_epi32(ssearch, elem->smask));
        if (result) {
            for (i = elem->start; i <= elem->end; i++) {
                if ((0x1 << i & result) && elem->value[i]) {
#if CUSTOM_MATCH_DEBUG_VERBOSE
                    mca_pml_base_request_t *req = (mca_pml_base_request_t *) elem->value[i];
                    printf("Found list: %p tag: %x peer: %x\n", (void *) list, req->req_tag,
                           req->req_peer);
#endif
                    return elem->value[i];
                }
...
static inline void *custom_match_prq_find_dequeue_verify(custom_match_prq *list, int tag, int peer)
{
    ...
    while (elem) {
        ...
        for (int iter = elem->start; iter <= elem->end; iter++) {
            // printf("Search = %x, Element Key = %x, Element mask = %x", ((int32_t*)
            // &search)[iter], ((int32_t*) &elem->keys)[iter], ((int32_t*) &elem->mask)[iter]);
        }
#endif
        result = _mm512_cmpeq_epi32_mask(_mm512_and_epi32(elem->tags, elem->tmask),
                                         _mm512_and_epi32(tsearch, elem->tmask))
                                           ...
                                           int source)
{
    ...
    printf("custom_match_prq_append list: %p mask_src: %x mask_tag: %x tag: %x peer: %x\n",
           (void *) list, mask_src, mask_tag, req->req_tag, req->req_peer);
#endif
    int i;
    ...
    for (elem = list->head; elem; elem = elem->next) {
        ...
        for (j = 0; j < 16; j++) {
            printf("%d:%d The key is %d, the mask is %d, the value is %lu\n", i, j,
                   ((int *) (&(elem->tags)))[j], ((int *) (&(elem->tmask)))[j],
                   (uintptr_t) elem->value[j]);
        }
        i++;
    }
}
