...
static inline void *custom_match_prq_find_verify(custom_match_prq *list, int tag, int peer)
{
    ...
    while (elem) {
        result = _mm512_cmpeq_epi32_mask(_mm512_and_epi32(elem->keys, elem->mask),
    ...
    for (elem = list->head; elem; elem = elem->next) {
        ...
        for (j = 0; j < 16; j++) {
            printf("%d:%d The key is %d, the mask is %d, the value is %lu\n", i, j,
                   ((int *) (&(elem->keys)))[j], ((int *) (&(elem->mask)))[j],
                                           ...
                                           void *payload)
{
    ...
    if ((!list->tail) || list->tail->end == 15) {
        ...
        elem->keys = _mm512_set1_epi32(~0); // TODO: we only have to do this type of initialization
    ...
    for (elem = list->head; elem; elem = elem->next) {
        ...
        for (j = 0; j < 16; j++) {
            if (elem->value[j]) {
                ...
                printf("%lx %x %x\n", (uintptr_t) elem->value[j], req->hdr.hdr_match.hdr_tag,
