        ...
        for (i = elem->start; i <= elem->end; i++) {
            if (elem->value[i] == req) {
                ...
                if (i == elem->start || i == elem->end) {
                    while ((elem->start <= elem->end) && (!(elem->value[elem->start])))
                        elem->start++;
                    while ((elem->start <= elem->end) && (!(elem->value[elem->end])))
                        elem->end--;
...
static inline void *custom_match_prq_find_verify(custom_match_prq *list, int tag, int peer)
{
    ...
    while (elem) {
        result = _mm512_cmpeq_epi16_mask(_mm512_and_epi32(elem->keys, elem->mask),
                                         _mm512_and_epi32(search, elem->mask));
        if (result) {
            for (i = elem->start; i <= elem->end; i++) {
                if ((0x1 << i & result) && elem->value[i]) {
                    mca_pml_base_request_t *req = (mca_pml_base_request_t *) elem->value[i];
                    if ((req->req_peer == peer || req->req_peer == OMPI_ANY_SOURCE)
                        && (req->req_tag == tag || req->req_tag == OMPI_ANY_TAG)) {
#if CUSTOM_MATCH_DEBUG_VERBOSE
...
static inline void *custom_match_prq_find_dequeue_verify(custom_match_prq *list, int tag, int peer)
{
    ...
    while (elem) {
        ...
        for (int iter = elem->start; iter <= elem->end; iter++) {
            printf("Search = %x, Element Key = %x, Element mask = %x", ((int32_t *) &search)[iter],
                   ((int32_t *) &elem->keys)[iter], ((int32_t *) &elem->mask)[iter]);
        }
#endif
        result = _mm512_cmpeq_epi16_mask(_mm512_and_epi32(elem->keys, elem->mask),
                                         _mm512_and_epi32(search, elem->mask));
        if (result) {
            for (i = elem->start; i <= elem->end; i++) {
                mca_pml_base_request_t *req = (mca_pml_base_request_t *) elem->value[i];
                if ((0x1 << i & result) && req
                    && ((req->req_peer == peer || req->req_peer == OMPI_ANY_SOURCE)
                        && (req->req_tag == tag || req->req_tag == OMPI_ANY_TAG))) {
                    void *payload = elem->value[i];
                    ((short *) (&(elem->keys)))[i] = ~0;
                    ((short *) (&(elem->mask)))[i] = ~0;
    ...
    for (elem = list->head; elem; elem = elem->next) {
        ...
        for (j = 0; j < 32; j++) {
            printf("%d:%d The key is %d, the mask is %d, the value is %ld\n", i, j,
                   ((short *) (&(elem->keys)))[j], ((short *) (&(elem->mask)))[j], elem->value[j]);
        }
        i++;
    }
}
    ...
    for (elem = list->head; elem; elem = elem->next) {
        ...
        for (j = 0; j < 32; j++) {
            if (elem->value[j]) {
                ...
                opal_output(0,
                            "req %p peer %s tag %s addr %p count %lu datatype %s [%p] [%s %s] "
                            "req_seq %" PRIu64,
        ...
        if (result) {
            for (i = elem->start; i <= elem->end; i++) {
                if ((0x1 << i & result) && elem->value[i]) {
                    ...
                    if ((req->hdr.hdr_match.hdr_src == peer || peer == OMPI_ANY_SOURCE)
                        && (req->hdr.hdr_match.hdr_tag == tag || tag == OMPI_ANY_TAG)) {
#if CUSTOM_MATCH_DEBUG_VERBOSE
                        printf("Found list: %x tag: %x peer: %x\n", list,
                                           ...
                                           void *payload)
{
    ...
    printf("custom_match_umq_append list: %x key: %x payload: %x tag: %d src: %d\n", list, key,
           payload, req->hdr.hdr_match.hdr_tag, req->hdr.hdr_match.hdr_src);
#endif
    int i;
    ...
    if ((!list->tail) || list->tail->end == 31) {
        ...
        elem->keys = _mm512_set1_epi16(~0); // TODO: we may only have to do this type of
                                            // initialization for freshly malloc'd entries.
        elem->next = 0;
        elem->start = 0;
        elem->end = -1; // we don't have an element yet
        for (i = 0; i < 32; i++)
    ...
    for (elem = list->head; elem; elem = elem->next) {
        ...
        for (j = 0; j < 32; j++) {
            if (elem->value[j]) {
                ...
                printf("%x %x %x\n", elem->value[j], req->hdr.hdr_match.hdr_tag,
                ...
                // opal_output(0, "peer %s tag %s",// addr %p count %lu datatype %s [%p] [%s %s]
                // req_seq %" PRIu64,
                //         /*(void*) req,*/ cpeer, ctag,
