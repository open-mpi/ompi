...
#    define MCA_PML_CM_THIN_SEND_REQUEST_ALLOC(sendreq, comm, dst, ompi_proc)              \
        do {                                                                               \
            ...
            if (OPAL_UNLIKELY(NULL == ompi_proc)) {                                        \
                ...
                sendreq = (mca_pml_cm_thin_send_request_t *) opal_free_list_wait(          \
...
#if (OPAL_ENABLE_HETEROGENEOUS_SUPPORT)
#    define MCA_PML_CM_HVY_SEND_REQUEST_ALLOC(sendreq, comm, dst, ompi_proc)               \
        {                                                                                  \
            ompi_proc = ompi_comm_peer_lookup(comm, dst);                                  \
            if (OPAL_UNLIKELY(NULL == ompi_proc)) {                                        \
                sendreq = NULL;                                                            \
            } else {                                                                       \
                sendreq = (mca_pml_cm_hvy_send_request_t *) opal_free_list_wait(           \
                    &mca_pml_base_send_requests);                                          \
...
#if (OPAL_ENABLE_HETEROGENEOUS_SUPPORT)
#    define MCA_PML_CM_HVY_SEND_REQUEST_INIT_COMMON(req_send, ompi_proc, comm, tag, datatype, \
                                                    sendmode, buf, count, flags)              \
        {                                                                                     \
            OBJ_RETAIN(comm);                                                                 \
            ...
            (req_send)->req_base.req_ompi.req_status.MPI_SOURCE = comm->c_my_rank;            \
...
#if (OPAL_ENABLE_HETEROGENEOUS_SUPPORT)
#    define MCA_PML_CM_SEND_REQUEST_INIT_COMMON(req_send, ompi_proc, comm, tag, datatype,   \
                                                sendmode, buf, count, flags)                \
        {                                                                                   \
            OBJ_RETAIN(comm);                                                               \
            OMPI_DATATYPE_RETAIN(datatype);                                                 \
...
#    define MCA_PML_CM_SEND_REQUEST_INIT_COMMON(req_send, ompi_proc, comm, tag, datatype,       \
        ...
        {                                                                                       \
            ...
            if (opal_datatype_is_contiguous_memory_layout(&datatype->super, count)) {           \
                (req_send)->req_base.req_convertor.remoteArch = ompi_mpi_local_convertor        \
                                                                    ->remoteArch;               \
                (req_send)->req_base.req_convertor.flags = ompi_mpi_local_convertor->flags;     \
                (req_send)->req_base.req_convertor.master = ompi_mpi_local_convertor->master;   \
                (req_send)->req_base.req_convertor.local_size = count * datatype->super.size;   \
                (req_send)->req_base.req_convertor.pBaseBuf = (unsigned char *) buf             \
                                                              + datatype->super.true_lb;        \
                ...
                if (opal_built_with_cuda_support) {                                             \
                    ...
                    opal_convertor_prepare_for_send(&req_send->req_base.req_convertor,          \
...
#define MCA_PML_CM_HVY_SEND_REQUEST_INIT(sendreq, ompi_proc, comm, tag, dst, datatype, sendmode, \
                                         persistent, blocking, buf, count, flags)                \
    do {                                                                                         \
        ...
        MCA_PML_CM_HVY_SEND_REQUEST_INIT_COMMON((&sendreq->req_send), ompi_proc, comm, tag,      \
                                                datatype, sendmode, buf, count, flags)           \
...
#define MCA_PML_CM_THIN_SEND_REQUEST_INIT(sendreq, ompi_proc, comm, tag, dst, datatype, sendmode, \
                                          buf, count, flags)                                      \
    do {                                                                                          \
        OMPI_REQUEST_INIT(&(sendreq->req_send.req_base.req_ompi), false);                         \
        MCA_PML_CM_SEND_REQUEST_INIT_COMMON((&sendreq->req_send), ompi_proc, comm, tag, datatype, \
                                            sendmode, buf, count, flags);                         \
...
#define MCA_PML_CM_SEND_REQUEST_START_SETUP(req_send)                  \
    do {                                                               \
        ...
        (req_send)->req_base.req_ompi.req_state = OMPI_REQUEST_ACTIVE; \
        (req_send)->req_base.req_ompi.req_status._cancelled = 0;       \
    } while (0)

#define MCA_PML_CM_THIN_SEND_REQUEST_START(sendreq, comm, tag, dst, sendmode, blocking, ret)     \
    do {                                                                                         \
        MCA_PML_CM_SEND_REQUEST_START_SETUP(&(sendreq)->req_send);                               \
        ret = OMPI_MTL_CALL(isend(ompi_mtl, comm, dst, tag,                                      \
                                  &sendreq->req_send.req_base.req_convertor, sendmode, blocking, \
                                  &sendreq->req_mtl));                                           \
    } while (0)
...
#define MCA_PML_CM_HVY_SEND_REQUEST_BSEND_ALLOC(sendreq, ret)                                    \
    do {                                                                                         \
        ...
        if (sendreq->req_count > 0) {                                                            \
            sendreq->req_buff = mca_pml_base_bsend_request_alloc_buf(sendreq->req_count);        \
            if (NULL == sendreq->req_buff) {                                                     \
                ...
                opal_convertor_prepare_for_send(&sendreq->req_send.req_base.req_convertor,       \
                                                &(ompi_mpi_packed.dt.super), max_data,           \
                                                sendreq->req_buff);                              \
            }                                                                                    \
        }                                                                                        \
    } while (0);
...
#define MCA_PML_CM_HVY_SEND_REQUEST_START(sendreq, ret)                                            \
    do {                                                                                           \
        ...
        if (OMPI_SUCCESS == ret) {                                                                 \
            ret = OMPI_MTL_CALL(                                                                   \
                isend(ompi_mtl, sendreq->req_send.req_base.req_comm, sendreq->req_peer,            \
                      sendreq->req_tag, &sendreq->req_send.req_base.req_convertor,                 \
                      sendreq->req_send.req_send_mode, sendreq->req_blocking, &sendreq->req_mtl)); \
            if (OMPI_SUCCESS == ret                                                                \
...
#define MCA_PML_CM_HVY_SEND_REQUEST_RETURN(sendreq)                                            \
    {                                                                                          \
        ...
        opal_free_list_return(&mca_pml_base_send_requests, (opal_free_list_item_t *) sendreq); \
    }
...
extern void mca_pml_cm_send_request_completion(struct mca_mtl_request_t *mtl_request);
