...
mca_pml_cm_add_procs(struct ompi_proc_t** procs, size_t nprocs)
{
     ...
     * modex data is read, so an unknown one compares equal to ours. */
    ...
    /* The mtl endpoint has to be in place before the flag that announces
     * it: readers test the flag with a relaxed load, and nothing in what
     * they read next depends on it. */
    opal_atomic_wmb();
...
mca_pml_cm_del_procs(struct ompi_proc_t** procs, size_t nprocs)
{
    ...
    /* Mirror of add_procs: the flag goes before what it announces, so
     * whoever still sees it set finds an endpoint that is whole. */
    opal_atomic_wmb();
...
int mca_pml_cm_ensure_proc(ompi_proc_t *proc)
{
     ...
     * runtime, which can call back into this PML. Two threads reaching
     * the same unwired peer both wire it, which the MTL has to make
     * harmless. NOT_READY goes back out; the caller stages or waits. */
...
/* Registered exactly while requests are parked: a staged request has
 * nothing outstanding that would come back for it. */
...
/* A staged request was prepared with the local convertor. No fixup: this
 * PML refuses a peer whose architecture differs (mca_pml_cm_add_procs()),
 * so a peer that wires keeps the local convertor and any other fails. */
/* True once the request is off our hands, started or failed. */
static bool mca_pml_cm_start_staged(mca_pml_cm_request_t *req)
{
    ...
    if (OMPI_ERR_NOT_READY == rc) {
        /* The blob may still be on its way; come back on a later tick. */
    ...
    if (MCA_PML_CM_REQUEST_SEND_HEAVY == req->req_pml_type) {
        ...
        if (OMPI_SUCCESS != rc) {
            /* The user holds this request; its status is the only
             * channel left. */
...
/* Slow paths, taken the first time an operation names an unwired peer.
 * A thin request does not remember its peer, tag or mode, so these build
 * a heavy one even where the fast path would not. */
...
static int mca_pml_cm_staged_progress_cb(void)
{
    ...
    if (opal_list_is_empty(&ompi_pml_cm.modex_pending)) {
        /* opal_progress() allows a callback to remove itself; a request
         * staged after this point registers it again. */
        if (mca_pml_cm_staged_progress) {
    ...
    /* Everything comes off; start_staged() puts back what still cannot
     * reach its peer. Asking is what fetches a peer's data. */
