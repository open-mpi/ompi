...
int mca_pml_cm_enable(bool enable)
{
    ...
    opal_free_list_init(&mca_pml_base_send_requests,
                        ...
                        opal_cache_line_size, OBJ_CLASS(mca_pml_cm_hvy_send_request_t), 0,
                        opal_cache_line_size, ompi_pml_cm.free_list_num, ompi_pml_cm.free_list_max,
                        ompi_pml_cm.free_list_inc, NULL, 0, NULL, NULL, NULL);

    opal_free_list_init(&mca_pml_base_recv_requests,
                        sizeof(mca_pml_cm_hvy_recv_request_t) + ompi_mtl->mtl_request_size,
                        opal_cache_line_size, OBJ_CLASS(mca_pml_cm_hvy_recv_request_t), 0,
                        opal_cache_line_size, ompi_pml_cm.free_list_num, ompi_pml_cm.free_list_max,
                        ompi_pml_cm.free_list_inc, NULL, 0, NULL, NULL, NULL);
...
int mca_pml_cm_add_comm(ompi_communicator_t *comm)
{
...
int mca_pml_cm_del_comm(ompi_communicator_t *comm)
{
    /* notify the MTL about the deleted communicator */
    return OMPI_MTL_CALL(del_comm(ompi_mtl, comm));
}

int mca_pml_cm_add_procs(struct ompi_proc_t **procs, size_t nprocs)
{
    int ret;
    ...
    if (OMPI_SUCCESS != (ret = mca_pml_base_pml_check_selected("cm", procs, nprocs))) {
        return ret;
    }
...
int mca_pml_cm_del_procs(struct ompi_proc_t **procs, size_t nprocs)
{
    int ret;
...
int mca_pml_cm_dump(struct ompi_communicator_t *comm, int verbose)
{
