...
int mca_pml_cm_start(size_t count, ompi_request_t **requests)
{
    int rc;
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        case MCA_PML_CM_REQUEST_SEND_HEAVY: {
            ...
            if (!sendreq->req_send.req_base.req_pml_complete) {
                ...
                rc = mca_pml_cm_isend_init(sendreq->req_addr, sendreq->req_count,
                                           sendreq->req_send.req_base.req_datatype,
                                           sendreq->req_peer, sendreq->req_tag,
                                           sendreq->req_send.req_send_mode,
                                           sendreq->req_send.req_base.req_comm, &request);
