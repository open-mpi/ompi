...
struct mca_pml_cm_thin_recv_request_t {
    ...
    mca_mtl_request_t
        req_mtl; /**< the mtl specific memory. This field should be the last in the struct */
};
...
#define MCA_PML_CM_THIN_RECV_REQUEST_ALLOC(recvreq)                                \
    do {                                                                           \
        recvreq = (mca_pml_cm_thin_recv_request_t *) opal_free_list_get(           \
...
#define MCA_PML_CM_HVY_RECV_REQUEST_ALLOC(recvreq)                                 \
    do {                                                                           \
        recvreq = (mca_pml_cm_hvy_recv_request_t *) opal_free_list_get(            \
            &mca_pml_base_recv_requests);                                          \
...
#    define MCA_PML_CM_THIN_RECV_REQUEST_INIT(request, ompi_proc, comm, src, datatype, addr, \
                                              count, flags)                                  \
        do {                                                                                 \
            ...
            opal_convertor_copy_and_prepare_for_recv(ompi_proc->super.proc_convertor,        \
                                                     &(datatype->super), count, addr, flags, \
...
#if OPAL_ENABLE_HETEROGENEOUS_SUPPORT
#    define MCA_PML_CM_HVY_RECV_REQUEST_INIT(request, ompi_proc, comm, tag, src, datatype, addr, \
                                             count, flags, persistent)                           \
        do {                                                                                     \
...
#define MCA_PML_CM_THIN_RECV_REQUEST_START(request, comm, tag, src, ret)                           \
    do {                                                                                           \
        ...
        ret = OMPI_MTL_CALL(                                                                       \
            irecv(ompi_mtl, comm, src, tag, &recvreq->req_base.req_convertor, &recvreq->req_mtl)); \
    } while (0)
...
#define MCA_PML_CM_THIN_RECV_REQUEST_MATCHED_START(request, message, ret)                    \
    do {                                                                                     \
        ...
        request->req_base.req_ompi.req_status._cancelled = 0;                                \
        ret = OMPI_MTL_CALL(                                                                 \
            imrecv(ompi_mtl, &recvreq->req_base.req_convertor, message, &recvreq->req_mtl)); \
    } while (0)
...
#define MCA_PML_CM_HVY_RECV_REQUEST_START(request, ret)                                    \
    do {                                                                                   \
        ...
        request->req_base.req_ompi.req_status.MPI_ERROR = OMPI_SUCCESS;                    \
        request->req_base.req_ompi.req_status._cancelled = 0;                              \
        ret = OMPI_MTL_CALL(irecv(ompi_mtl, request->req_base.req_comm, request->req_peer, \
                                  request->req_tag, &recvreq->req_base.req_convertor,      \
                                  &recvreq->req_mtl));                                     \
    } while (0)
