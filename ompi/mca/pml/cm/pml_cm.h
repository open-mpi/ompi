...
/**
 * Wire a peer if needed. OMPI_ERR_NOT_READY means its connection info has
 * not arrived: stage the operation, or use mca_pml_cm_wait_proc() instead.
 */
...
/** Park a request until its peer is wired, then replay it. */
...
/**
 * Wire the peer of a persistent request. OMPI_ERR_NOT_READY means it was
 * parked instead and must not be started.
 */
OMPI_DECLSPEC extern int mca_pml_cm_start_peer(mca_pml_cm_request_t *req, int peer,
                                               bool must_wait);

/** Can this peer go straight to the MTL? MPI_ANY_SOURCE names no peer,
 *  so it always can. */
...
/** Slow paths for an operation whose peer is not wired yet; they park
 *  the request if its connection info has not arrived. */
...
/** Wire a peer for an operation that may block, so the call below it
 *  can assume the peer is usable. */
...
{
    if (OPAL_UNLIKELY(!mca_pml_cm_peer_wired(comm, src))) {
        /* Nothing can have arrived from an unwired peer, and "no match
         * yet" is a legal answer; wiring makes the next probe decisive. */
