 ...
 */
extern void (*send_completion_callbacks[])(struct mca_mtl_request_t *mtl_request);
...
mca_pml_cm_irecv_init(void *addr, size_t count, ompi_datatype_t *datatype, int src, int tag,
                      struct ompi_communicator_t *comm, struct ompi_request_t **request)
{
    ...
    MCA_PML_CM_HVY_RECV_REQUEST_INIT(recvreq, ompi_proc, comm, tag, src, datatype, addr, count,
                                     flags, true);
...
__opal_attribute_always_inline__ static inline int
mca_pml_cm_irecv(void *addr, size_t count, ompi_datatype_t *datatype, int src, int tag,
                 struct ompi_communicator_t *comm, struct ompi_request_t **request)
{
    int ret;
    uint32_t flags = 0;
    ...
    MCA_PML_CM_THIN_RECV_REQUEST_INIT(recvreq, ompi_proc, comm, src, datatype, addr, count, flags);
...
__opal_attribute_always_inline__ static inline int
mca_pml_cm_recv(void *addr, size_t count, ompi_datatype_t *datatype, int src, int tag,
                struct ompi_communicator_t *comm, ompi_status_public_t *status)
{
    ...
    opal_convertor_copy_and_prepare_for_recv(ompi_proc->super.proc_convertor, &(datatype->super),
                                             count, addr, flags, &convertor);
#else
    MCA_PML_CM_SWITCH_CUDA_CONVERTOR_OFF(flags, datatype, count);

    opal_convertor_copy_and_prepare_for_recv(ompi_mpi_local_convertor, &(datatype->super), count,
                                             addr, flags, &convertor);
#endif

    ret = OMPI_MTL_CALL(irecv(ompi_mtl, comm, src, tag, &convertor, req_mtl));
...
__opal_attribute_always_inline__ static inline int
mca_pml_cm_isend_init(const void *buf, size_t count, ompi_datatype_t *datatype, int dst, int tag,
                      ...
                      ompi_request_t **request)
{
    ...
    MCA_PML_CM_HVY_SEND_REQUEST_INIT(sendreq, ompi_proc, comm, tag, dst, datatype, sendmode, true,
                                     false, buf, count, flags);
...
__opal_attribute_always_inline__ static inline int
mca_pml_cm_isend(const void *buf, size_t count, ompi_datatype_t *datatype, int dst, int tag,
                 mca_pml_base_send_mode_t sendmode, ompi_communicator_t *comm,
                 ompi_request_t **request)
{
    int ret;
    ...
    if (sendmode == MCA_PML_BASE_SEND_BUFFERED) {
        ...
        MCA_PML_CM_HVY_SEND_REQUEST_INIT(sendreq, ompi_proc, comm, tag, dst, datatype, sendmode,
                                         false, false, buf, count, flags);
        ...
        MCA_PML_CM_THIN_SEND_REQUEST_INIT(sendreq, ompi_proc, comm, tag, dst, datatype, sendmode,
                                          buf, count, flags);

        MCA_PML_CM_THIN_SEND_REQUEST_START(sendreq, comm, tag, dst, sendmode, false, ret);
...
__opal_attribute_always_inline__ static inline int
mca_pml_cm_send(const void *buf, size_t count, ompi_datatype_t *datatype, int dst, int tag,
                mca_pml_base_send_mode_t sendmode, ompi_communicator_t *comm)
{
    int ret = OMPI_ERROR;
    uint32_t flags = 0;
...
#    endif
...
#endif
        {
            ...
            opal_convertor_copy_and_prepare_for_send(ompi_proc->super.proc_convertor,
                                                     &datatype->super, count, buf, flags,
                                                     &convertor);
        }

        ret = OMPI_MTL_CALL(send(ompi_mtl, comm, dst, tag, &convertor, sendmode));
        OBJ_DESTRUCT(&convertor);
    }
...
__opal_attribute_always_inline__ static inline int
mca_pml_cm_iprobe(int src, int tag, struct ompi_communicator_t *comm, int *matched,
                  ompi_status_public_t *status)
{
    return OMPI_MTL_CALL(iprobe(ompi_mtl, comm, src, tag, matched, status));
}

__opal_attribute_always_inline__ static inline int
mca_pml_cm_probe(int src, int tag, struct ompi_communicator_t *comm, ompi_status_public_t *status)
{
    int ret, matched = 0;

    while (true) {
        ret = OMPI_MTL_CALL(iprobe(ompi_mtl, comm, src, tag, &matched, status));
        if (OMPI_SUCCESS != ret)
            break;
        if (matched)
            break;
...
__opal_attribute_always_inline__ static inline int
mca_pml_cm_improbe(int src, int tag, struct ompi_communicator_t *comm, int *matched,
                   struct ompi_message_t **message, ompi_status_public_t *status)
{
    return OMPI_MTL_CALL(improbe(ompi_mtl, comm, src, tag, matched, message, status));
}

__opal_attribute_always_inline__ static inline int
mca_pml_cm_mprobe(int src, int tag, struct ompi_communicator_t *comm,
                  struct ompi_message_t **message, ompi_status_public_t *status)
{
    int ret, matched = 0;

    while (true) {
        ret = OMPI_MTL_CALL(improbe(ompi_mtl, comm, src, tag, &matched, message, status));
        if (OMPI_SUCCESS != ret)
            break;
        if (matched)
            break;
...
__opal_attribute_always_inline__ static inline int
mca_pml_cm_imrecv(void *buf, size_t count, ompi_datatype_t *datatype,
                  struct ompi_message_t **message, struct ompi_request_t **request)
{
    ...
    MCA_PML_CM_THIN_RECV_REQUEST_INIT(recvreq, ompi_proc, comm, (*message)->peer, datatype, buf,
                                      count, flags);
...
__opal_attribute_always_inline__ static inline int mca_pml_cm_mrecv(void *buf, size_t count,
...
OMPI_DECLSPEC extern int mca_pml_cm_dump(struct ompi_communicator_t *comm, int verbose);
