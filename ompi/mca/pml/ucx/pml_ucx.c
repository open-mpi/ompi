...
#define PML_UCX_TRACE_SEND(_msg, _buf, _count, _datatype, _dst, _tag, _mode, _comm, ...)      \
    PML_UCX_VERBOSE(8, _msg " buf %p count %zu type '%s' dst %d tag %d mode %s comm %d '%s'", \
                    __VA_ARGS__, (_buf), (_count), (_datatype)->name, (_dst), (_tag),         \
                    mca_pml_ucx_send_mode_name(_mode), (_comm)->c_contextid, (_comm)->c_name);

#define PML_UCX_TRACE_RECV(_msg, _buf, _count, _datatype, _src, _tag, _comm, ...)                  \
    PML_UCX_VERBOSE(8, _msg " buf %p count %zu type '%s' src %d tag %d comm %d '%s'", __VA_ARGS__, \
                    (_buf), (_count), (_datatype)->name, (_src), (_tag), (_comm)->c_contextid,     \
                    (_comm)->c_name);

#define PML_UCX_TRACE_PROBE(_msg, _src, _tag, _comm)                                           \
    PML_UCX_VERBOSE(8, _msg " src %d tag %d comm %d '%s'", _src, (_tag), (_comm)->c_contextid, \
                    (_comm)->c_name);

#define PML_UCX_TRACE_MRECV(_msg, _buf, _count, _datatype, _message)                         \
    PML_UCX_VERBOSE(8, _msg " buf %p count %zu type '%s' msg *%p=%p (%p)", (_buf), (_count), \
                    (_datatype)->name, (void *) (_message), (void *) *(_message),            \
                    (*(_message))->req_ptr);
...
static int mca_pml_ucx_send_worker_address_type(int addr_flags, int modex_scope)
{
    ...
    OPAL_MODEX_SEND(rc, modex_scope, &mca_pml_ucx_component.pmlm_version, (void *) attrs.address,
    ...
    PML_UCX_VERBOSE(2, "Pack %s worker address, size %ld",
                    (modex_scope == PMIX_LOCAL) ? "local" : "remote", attrs.address_length);
...
static int mca_pml_ucx_send_worker_address(void)
{
    ...
    OPAL_MODEX_SEND(rc, PMIX_GLOBAL, &mca_pml_ucx_component.pmlm_version, (void *) address,
                    addrlen);
    ...
    status = mca_pml_ucx_send_worker_address_type(UCP_WORKER_ADDRESS_FLAG_NET_ONLY, PMIX_REMOTE);
    if (UCS_OK != status) {
        goto err;
    }
...
static int mca_pml_ucx_recv_worker_address(ompi_proc_t *proc, ucp_address_t **address_p,
                                           size_t *addrlen_p)
{
    int ret;
    ...
    *address_p = NULL;
    ...
    if (ret < 0) {
        PML_UCX_ERROR("Failed to receive UCX worker address: %s (%d)", opal_strerror(ret), ret);
    }

    PML_UCX_VERBOSE(2, "Got proc %d address, size %ld", proc->super.proc_name.vpid, *addrlen_p);
    return ret;
}
...
int mca_pml_ucx_open(void)
{
    ...
    PML_UCX_VERBOSE(1, "mca_pml_ucx_open: UCX version %u.%u.%u", major_version, minor_version,
                    release_number);
    ...
    params.field_mask = UCP_PARAM_FIELD_FEATURES | UCP_PARAM_FIELD_REQUEST_SIZE
                        ...
                        | UCP_PARAM_FIELD_TAG_SENDER_MASK | UCP_PARAM_FIELD_MT_WORKERS_SHARED
...
int mca_pml_ucx_init(int enable_mpi_threads)
{
    ...
    status = ucp_worker_create(ompi_pml_ucx.ucp_context, &params, &ompi_pml_ucx.ucp_worker);
    if (UCS_OK != status) {
    ...
    PML_UCX_VERBOSE(2, "created ucp context %p, worker %p", (void *) ompi_pml_ucx.ucp_context,
...
static ucp_ep_h mca_pml_ucx_add_proc_common(ompi_proc_t *proc)
{
    ...
    free(address);
    if (UCS_OK != status) {
        PML_UCX_ERROR("ucp_ep_create(proc=%d) failed: %s", proc->super.proc_name.vpid,
...
__opal_attribute_always_inline__ static inline ucp_ep_h
mca_pml_ucx_get_ep(ompi_communicator_t *comm, int rank)
{
...
int mca_pml_ucx_enable(bool enable)
{
    ...
    ret = ompi_attr_create_keyval(TYPE_ATTR, copy_fn, del_fn, &ompi_pml_ucx.datatype_attr_keyval,
                                  NULL, 0, NULL);
    if (ret != OMPI_SUCCESS) {
    ...
    PML_UCX_FREELIST_INIT(&ompi_pml_ucx.persistent_reqs, mca_pml_ucx_persistent_request_t, 128, -1,
                          128);
    PML_UCX_FREELIST_INIT(&ompi_pml_ucx.convs, mca_pml_ucx_convertor_t, 128, -1, 128);
    return OMPI_SUCCESS;
}
                           ...
                           struct ompi_communicator_t *comm, struct ompi_request_t **request)
{
    ...
    PML_UCX_TRACE_RECV("irecv_init request *%p=%p", buf, count, datatype, src, tag, comm,
                       (void *) request, (void *) req);
...
int mca_pml_ucx_irecv(void *buf, size_t count, ompi_datatype_t *datatype, int src, int tag,
                      struct ompi_communicator_t *comm, struct ompi_request_t **request)
{
    ...
    PML_UCX_TRACE_RECV("irecv request *%p", buf, count, datatype, src, tag, comm, (void *) request);

    PML_UCX_MAKE_RECV_TAG(ucp_tag, ucp_tag_mask, tag, src, comm);
#if HAVE_DECL_UCP_TAG_RECV_NBX
    req = (ompi_request_t *) ucp_tag_recv_nbx(ompi_pml_ucx.ucp_worker, buf,
                                              mca_pml_ucx_get_data_size(op_data, count), ucp_tag,
                                              ucp_tag_mask, param);
#else
    req = (ompi_request_t *) ucp_tag_recv_nb(ompi_pml_ucx.ucp_worker, buf, count,
                                             mca_pml_ucx_get_datatype(datatype), ucp_tag,
                                             ucp_tag_mask, mca_pml_ucx_recv_completion);
#endif
...
int mca_pml_ucx_recv(void *buf, size_t count, ompi_datatype_t *datatype, int src, int tag,
                     struct ompi_communicator_t *comm, ompi_status_public_t *mpi_status)
{
    ...
    PML_UCX_MAKE_RECV_TAG(ucp_tag, ucp_tag_mask, tag, src, comm);
#if HAVE_DECL_UCP_TAG_RECV_NBX
    ucp_tag_recv_nbx(ompi_pml_ucx.ucp_worker, buf, mca_pml_ucx_get_data_size(op_data, count),
                     ucp_tag, ucp_tag_mask, &param);
#else
    ucp_tag_recv_nbr(ompi_pml_ucx.ucp_worker, buf, count, mca_pml_ucx_get_datatype(datatype),
    ...
    MCA_COMMON_UCX_PROGRESS_LOOP(ompi_pml_ucx.ucp_worker)
    {
        ...
        if (status != UCS_INPROGRESS) {
            ...
            SPC_USER_OR_MPI(tag, dt_size * count, OMPI_SPC_BYTES_RECEIVED_USER,
...
__opal_attribute_always_inline__ static inline const char *
mca_pml_ucx_send_mode_name(mca_pml_base_send_mode_t mode)
{
    switch (mode) {
...
int mca_pml_ucx_isend_init(const void *buf, size_t count, ompi_datatype_t *datatype, int dst,
                           ...
                           struct ompi_request_t **request)
{
    ...
    PML_UCX_TRACE_SEND("isend_init request *%p=%p", buf, count, datatype, dst, tag, mode, comm,
                       (void *) request, (void *) req)
                                          ...
                                          ompi_datatype_t *datatype, uint64_t pml_tag)
{
    ...
    opal_convertor_copy_and_prepare_for_send(ompi_proc_local_proc->super.proc_convertor,
                                             &datatype->super, count, buf, 0, &opal_conv);
    ...
    req = (ompi_request_t *) ucp_tag_send_nb(ep, packed_data, packed_length, ucp_dt_make_contig(1),
                                             pml_tag, mca_pml_ucx_bsend_completion);
    if (NULL == req) {
...
__opal_attribute_always_inline__ static inline ucs_status_ptr_t
mca_pml_ucx_common_send(ucp_ep_h ep, const void *buf, size_t count, ompi_datatype_t *datatype,
                        ucp_datatype_t ucx_datatype, ucp_tag_t tag, mca_pml_base_send_mode_t mode,
...
__opal_attribute_always_inline__ static inline ucs_status_ptr_t
mca_pml_ucx_common_send_nbx(ucp_ep_h ep, const void *buf, size_t count, ompi_datatype_t *datatype,
                            ...
                            ucp_request_param_t *param)
{
    ...
    if (OPAL_UNLIKELY(MCA_PML_BASE_SEND_BUFFERED == mode)) {
        ...
        return ucp_tag_send_sync_nb(ep, buf, count, mca_pml_ucx_get_datatype(datatype), tag,
                                    (ucp_send_callback_t) param->cb.send);
    } else {
        return ucp_tag_send_nbx(ep, buf, mca_pml_ucx_get_data_size(op_data, count), tag, param);
    }
}
#endif

int mca_pml_ucx_isend(const void *buf, size_t count, ompi_datatype_t *datatype, int dst, int tag,
                      ...
                      struct ompi_request_t **request)
{
    ...
    PML_UCX_TRACE_SEND("i%ssend request *%p", buf, count, datatype, dst, tag, mode, comm,
                       mode == MCA_PML_BASE_SEND_BUFFERED ? "b" : "", (void *) request)
...
#if HAVE_DECL_UCP_TAG_SEND_NBX
    req = (ompi_request_t *)
        mca_pml_ucx_common_send_nbx(ep, buf, count, datatype, PML_UCX_MAKE_SEND_TAG(tag, comm),
                                    mode, &mca_pml_ucx_get_op_data(datatype)->op_param.send);
#else
    ...
    size_t dt_size;
    ompi_datatype_type_size(datatype, &dt_size);
    SPC_USER_OR_MPI(tag, dt_size * count, OMPI_SPC_BYTES_SENT_USER, OMPI_SPC_BYTES_SENT_MPI);
#endif
...
mca_pml_ucx_send_nb(ucp_ep_h ep, const void *buf, size_t count, ompi_datatype_t *datatype,
                    ucp_datatype_t ucx_datatype, ucp_tag_t tag, mca_pml_base_send_mode_t mode,
                    ucp_send_callback_t cb)
{
    ompi_request_t *req;
    ...
    req = (ompi_request_t *) mca_pml_ucx_common_send(ep, buf, count, datatype,
                                                     mca_pml_ucx_get_datatype(datatype), tag, mode,
                                                     cb);
...
static inline __opal_attribute_always_inline__ int
mca_pml_ucx_send_nbr(ucp_ep_h ep, const void *buf, size_t count, ompi_datatype_t *datatype,
                     ucp_tag_t tag)
{
    /* coverity[bad_alloc_arithmetic] */
...
#    if HAVE_DECL_UCP_TAG_SEND_NBX
    pml_ucx_datatype_t *op_data = mca_pml_ucx_get_op_data(datatype);
    ucp_request_param_t param = {.op_attr_mask = UCP_OP_ATTR_FIELD_REQUEST
                                                 | (op_data->op_param.send.op_attr_mask
                                                    & UCP_OP_ATTR_FIELD_DATATYPE)
                                                 | UCP_OP_ATTR_FLAG_FAST_CMPL,
                                 .datatype = op_data->op_param.send.datatype,
                                 .request = req};

    req = ucp_tag_send_nbx(ep, buf, mca_pml_ucx_get_data_size(op_data, count), tag, &param);
...
#    else
    ucs_status_t status;
    status = ucp_tag_send_nbr(ep, buf, count, mca_pml_ucx_get_datatype(datatype), tag, req);
...
int mca_pml_ucx_send(const void *buf, size_t count, ompi_datatype_t *datatype, int dst, int tag,
                     mca_pml_base_send_mode_t mode, struct ompi_communicator_t *comm)
{
    ucp_ep_h ep;
    ...
    if (OPAL_LIKELY((MCA_PML_BASE_SEND_BUFFERED != mode)
        ...
        return mca_pml_ucx_send_nbr(ep, buf, count, datatype, PML_UCX_MAKE_SEND_TAG(tag, comm));
    }
#endif

    return mca_pml_ucx_send_nb(ep, buf, count, datatype, mca_pml_ucx_get_datatype(datatype),
                               PML_UCX_MAKE_SEND_TAG(tag, comm), mode, mca_pml_ucx_send_completion);
}

int mca_pml_ucx_iprobe(int src, int tag, struct ompi_communicator_t *comm, int *matched,
                       ompi_status_public_t *mpi_status)
{
    ...
    PML_UCX_MAKE_RECV_TAG(ucp_tag, ucp_tag_mask, tag, src, comm);
    ucp_msg = ucp_tag_probe_nb(ompi_pml_ucx.ucp_worker, ucp_tag, ucp_tag_mask, 0, &info);
    if (ucp_msg != NULL) {
        *matched = 1;
        mca_pml_ucx_set_recv_status_safe(mpi_status, UCS_OK, &info);
    } else {
        (++progress_count % opal_common_ucx.progress_iterations)
            ? (void) ucp_worker_progress(ompi_pml_ucx.ucp_worker)
            : opal_progress();
        *matched = 0;
    }
    return OMPI_SUCCESS;
...
int mca_pml_ucx_improbe(int src, int tag, struct ompi_communicator_t *comm, int *matched,
                        struct ompi_message_t **message, ompi_status_public_t *mpi_status)
{
    ...
    PML_UCX_MAKE_RECV_TAG(ucp_tag, ucp_tag_mask, tag, src, comm);
    ucp_msg = ucp_tag_probe_nb(ompi_pml_ucx.ucp_worker, ucp_tag, ucp_tag_mask, 1, &info);
    if (ucp_msg != NULL) {
                       ...
                       struct ompi_message_t **message, struct ompi_request_t **request)
{
    ...
    req = (ompi_request_t *) ucp_tag_msg_recv_nb(ompi_pml_ucx.ucp_worker, buf, count,
                                                 mca_pml_ucx_get_datatype(datatype),
                                                 (*message)->req_ptr, mca_pml_ucx_recv_completion);
    if (UCS_PTR_IS_ERR(req)) {
...
int mca_pml_ucx_start(size_t count, ompi_request_t **requests)
{
    ...
    for (i = 0; i < count; ++i) {
        ...
        if (preq->flags & MCA_PML_UCX_REQUEST_FLAG_SEND) {
            tmp_req = (ompi_request_t *) mca_pml_ucx_common_send(preq->send.ep, preq->buffer,
                                                                 preq->count,
                                                                 preq->datatype.ompi_datatype,
                                                                 preq->datatype.datatype, preq->tag,
                                                                 preq->send.mode,
                                                                 mca_pml_ucx_psend_completion);
        } else {
            PML_UCX_VERBOSE(8, "start recv request %p", (void *) preq);
            tmp_req = (ompi_request_t *) ucp_tag_recv_nb(ompi_pml_ucx.ucp_worker, preq->buffer,
                                                         preq->count, preq->datatype.datatype,
                                                         preq->tag, preq->recv.tag_mask,
