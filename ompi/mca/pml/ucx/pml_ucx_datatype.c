...
static void *pml_ucx_generic_datatype_start_pack(void *context, const void *buffer, size_t count)
{
...
static void *pml_ucx_generic_datatype_start_unpack(void *context, void *buffer, size_t count)
{
    ompi_datatype_t *datatype = context;
    mca_pml_ucx_convertor_t *convertor;
...
static size_t pml_ucx_generic_datatype_pack(void *state, size_t offset, void *dest,
                                            size_t max_length)
{
    mca_pml_ucx_convertor_t *convertor = state;
    uint32_t iov_count;
...
static ucs_status_t pml_ucx_generic_datatype_unpack(void *state, size_t offset, const void *src,
                                                    size_t length)
{
    mca_pml_ucx_convertor_t *convertor = state;
    ...
    if (offset != convertor->offset) {
        ...
        opal_convertor_copy_and_prepare_for_recv(ompi_proc_local_proc->super.proc_convertor,
                                                 ...
                                                 convertor->opal_conv.pBaseBuf, 0, &conv);
...
int mca_pml_ucx_datatype_attr_del_fn(ompi_datatype_t *datatype, int keyval, void *attr_val,
                                     void *extra)
{
...
mca_pml_ucx_datatype_is_contig(ompi_datatype_t *datatype)
{
    ptrdiff_t lb;
    ...
    return (datatype->super.flags & OPAL_DATATYPE_FLAG_CONTIGUOUS)
           && (datatype->super.flags & OPAL_DATATYPE_FLAG_NO_GAPS) && (lb == 0);
}

#ifdef HAVE_UCP_REQUEST_PARAM_T
__opal_attribute_always_inline__ static inline pml_ucx_datatype_t *
mca_pml_ucx_init_nbx_datatype(ompi_datatype_t *datatype, ucp_datatype_t ucp_datatype, size_t size)
{
...
ucp_datatype_t mca_pml_ucx_init_datatype(ompi_datatype_t *datatype)
{
    ...
    if (mca_pml_ucx_datatype_is_contig(datatype)) {
        ...
        status = ucp_dt_create_generic(&pml_ucx_generic_datatype_ops, datatype, &ucp_datatype);
        if (status != UCS_OK) {
            int err = MPI_ERR_INTERN;
    ...
    if (ompi_datatype_is_predefined(datatype)) {
        ...
        ret = ompi_attr_set_c(TYPE_ATTR, datatype, &datatype->d_keyhash,
                              ompi_pml_ucx.datatype_attr_keyval, (void *) ucp_datatype, false);
        if (ret != OMPI_SUCCESS) {
            int err = MPI_ERR_INTERN;
            PML_UCX_ERROR("Failed to add UCX datatype attribute for %s: %d", datatype->name, ret);
            /* TODO: this error should return to the caller and invoke an error
             * handler from the MPI API call.
             * For now, it is fatal. */
            ompi_mpi_errors_are_fatal_comm_handler(NULL, &err,
                                                   "Failed to allocate datatype structure");
        }
    }

#ifdef HAVE_UCP_REQUEST_PARAM_T
    UCS_STATIC_ASSERT(sizeof(datatype->pml_data) >= sizeof(pml_ucx_datatype_t *));
    datatype->pml_data = (uint64_t) mca_pml_ucx_init_nbx_datatype(datatype, ucp_datatype, size);
#else
...
OBJ_CLASS_INSTANCE(mca_pml_ucx_convertor_t, opal_free_list_item_t, mca_pml_ucx_convertor_construct,
