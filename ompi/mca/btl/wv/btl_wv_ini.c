 ...
 *   of these
 */
typedef struct wv_device_values_t {
...
static void device_values_constructor(wv_device_values_t *s);
static void device_values_destructor(wv_device_values_t *s);

OBJ_CLASS_INSTANCE(wv_device_values_t,
                   opal_list_item_t,
...
static void reset_values(ompi_btl_wv_ini_values_t *v);
 ...
 */
int ompi_btl_wv_ini_query(uint32_t vendor_id, uint32_t vendor_part_id,
                              ompi_btl_wv_ini_values_t *values)
{
    int ret;
    wv_device_values_t *h;
 ...
 */
int ompi_btl_wv_ini_finalize(void)
{
    opal_list_item_t *item;
...
static int parse_file(char *filename)
{
    ...
    /* Do the parsing */
    btl_wv_ini_parse_done = false;
    btl_wv_ini_yynewlines = 1;
    btl_wv_ini_init_buffer(btl_wv_ini_yyin);
    while (!btl_wv_ini_parse_done) {
        val = btl_wv_ini_yylex();
        switch (val) {
        case BTL_WV_INI_PARSE_DONE:
            /* This will also set btl_wv_ini_parse_done to true, so just
               break here */
            break;

        case BTL_WV_INI_PARSE_NEWLINE:
            /* blank line!  ignore it */
            break;

        case BTL_WV_INI_PARSE_SECTION:
            ...
            section.name = strdup(btl_wv_ini_yytext);
            break;

        case BTL_WV_INI_PARSE_SINGLE_WORD:
...
static int parse_line(parsed_section_values_t *sv)
{
    ...
    /* Save the name name */
    if (key_buffer_len < strlen(btl_wv_ini_yytext) + 1) {
        char *tmp;
        key_buffer_len = strlen(btl_wv_ini_yytext) + 1;
    ...
    strncpy(key_buffer, btl_wv_ini_yytext, key_buffer_len);

    /* The first thing we have to see is an "=" */
    val = btl_wv_ini_yylex();
    if (btl_wv_ini_parse_done || BTL_WV_INI_PARSE_EQUAL != val) {
    ...
    val = btl_wv_ini_yylex();
    if (BTL_WV_INI_PARSE_SINGLE_WORD == val ||
        BTL_WV_INI_PARSE_VALUE == val) {
        value = strdup(btl_wv_ini_yytext);

        /* Now we need to see the newline */
        val = btl_wv_ini_yylex();
        if (BTL_WV_INI_PARSE_NEWLINE != val &&
            BTL_WV_INI_PARSE_DONE != val) {
    ...
    else if (BTL_WV_INI_PARSE_DONE != val &&
             BTL_WV_INI_PARSE_NEWLINE != val) {
        show_help("ini file:expected newline");
        return OMPI_ERROR;
    }
    ...
    if (0 == strcasecmp(key_buffer, "vendor_id")) {
        if (OMPI_SUCCESS != (ret = ompi_btl_wv_ini_intify_list(value, &sv->vendor_ids,
                                               &sv->vendor_ids_len))) {
            return ret;
        }
    }
        ...
        if (OMPI_SUCCESS != (ret = ompi_btl_wv_ini_intify_list(value, &sv->vendor_part_ids,
        ...
        /* Single value */
        sv->values.mtu = (uint32_t) ompi_btl_wv_ini_intify(value);
        sv->values.mtu_set = true;
    }

    else if (0 == strcasecmp(key_buffer, "use_eager_rdma")) {
        /* Single value */
        sv->values.use_eager_rdma = (uint32_t) ompi_btl_wv_ini_intify(value);
        ...
        /* Single value */
        sv->values.max_inline_data = (int32_t) ompi_btl_wv_ini_intify(value);
        ...
        sv->values.rdmacm_reject_causes_connect_error = 
            (bool) ompi_btl_wv_ini_intify(value);
...
/*
 * Construct an wv_device_values_t and set all of its values to known states
 */
static void device_values_constructor(wv_device_values_t *s)
{
    s->section_name = NULL;
...
/*
 * Destruct an wv_device_values_t and free any memory that it has
 */
static void device_values_destructor(wv_device_values_t *s)
{
 ...
 */
int ompi_btl_wv_ini_intify(char *str)
{
    while (isspace(*str)) {
        ++str;
    }
 ...
 */
int ompi_btl_wv_ini_intify_list(char *value, uint32_t **values, int *len)
{
    char *comma;
    char *str = value;
    ...
    *len = 0;
    ...
    if (NULL == comma) {
        ...
        *values[0] = (uint32_t) ompi_btl_wv_ini_intify(str);
        *len = 1;
    } else {
        int newsize = 1;
        ...
        *values = (uint32_t *) malloc(sizeof(uint32_t) * newsize);
        ...
        do {
            *comma = '\0';
            (*values)[*len] = (uint32_t) ompi_btl_wv_ini_intify(str);
            ++(*len);
            str = comma + 1;
            comma = strchr(str, ',');
        } while (NULL != comma);
