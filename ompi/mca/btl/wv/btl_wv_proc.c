...
static void mca_btl_wv_proc_construct(mca_btl_wv_proc_t* proc);
static void mca_btl_wv_proc_destruct(mca_btl_wv_proc_t* proc);

OBJ_CLASS_INSTANCE(mca_btl_wv_proc_t,
        opal_list_item_t, mca_btl_wv_proc_construct,
        mca_btl_wv_proc_destruct);

void mca_btl_wv_proc_construct(mca_btl_wv_proc_t* ib_proc)
{
    ib_proc->proc_ompi = 0;
    ...
    opal_list_append(&mca_btl_wv_component.ib_procs, &ib_proc->super);
...
void mca_btl_wv_proc_destruct(mca_btl_wv_proc_t* ib_proc)
{
    /* remove from list of all proc instances */
    OPAL_THREAD_LOCK(&mca_btl_wv_component.ib_lock);
    opal_list_remove_item(&mca_btl_wv_component.ib_procs, &ib_proc->super);
    OPAL_THREAD_UNLOCK(&mca_btl_wv_component.ib_lock);
 ...
 */
static mca_btl_wv_proc_t* mca_btl_wv_proc_lookup_ompi(ompi_proc_t* ompi_proc)
{
...
mca_btl_wv_proc_t* mca_btl_wv_proc_create(ompi_proc_t* ompi_proc)
{
    ...
    mca_btl_wv_modex_message_t dummy;

    /* Check if we have already created a IB proc
     * structure for this ompi process */
    module_proc = mca_btl_wv_proc_lookup_ompi(ompi_proc);
    if (NULL != module_proc) {
        /* Gotcha! */
        return module_proc;
    }
    ...
    module_proc = OBJ_NEW(mca_btl_wv_proc_t);
    ...
    rc = ompi_modex_recv(&mca_btl_wv_component.super.btl_version,
                         ompi_proc,
                         &message,
                         &msg_size);
    if (OMPI_SUCCESS != rc) {
    ...
    /* Message was packed in btl_wv_component.c; the format is
    ...
    if (module_proc->proc_port_count > 0) {
        module_proc->proc_ports = (mca_btl_wv_proc_modex_t *)
            malloc(sizeof(mca_btl_wv_proc_modex_t) * 
    ...
    for (i = 0; i < module_proc->proc_port_count; i++) {
        ...
        MCA_BTL_WV_MODEX_MSG_NTOH(module_proc->proc_ports[i].pm_port_info);
#endif
        offset += size;
        ...
        module_proc->proc_ports[i].pm_cpc_data = (ompi_btl_wv_connect_base_module_data_t *)
            calloc(module_proc->proc_ports[i].pm_cpc_data_count,
                   sizeof(ompi_btl_wv_connect_base_module_data_t));
        ...
        for (j = 0; j < module_proc->proc_ports[i].pm_cpc_data_count; ++j) {
            uint8_t u8;
            ompi_btl_wv_connect_base_module_data_t *cpcd;
            ...
            cpcd->cbm_component = 
                ompi_btl_wv_connect_base_get_cpc_byindex(u8);
...
{
    size_t i;
    mca_btl_wv_proc_t* ib_proc = NULL;

    /* Remove endpoint from the wv BTL version of the proc as
       well */
    ib_proc = mca_btl_wv_proc_lookup_ompi(proc);
    if (NULL != ib_proc) {
 ...
 * it an address.
 */
int mca_btl_wv_proc_insert(mca_btl_wv_proc_t* module_proc,
