...
int ompi_osc_base_get_primitive_type_info(ompi_datatype_t *datatype,
                                          ompi_datatype_t **prim_datatype, uint32_t *prim_count)
{
...
int ompi_osc_base_process_op(void *outbuf, void *inbuf, size_t inbuflen,
                             struct ompi_datatype_t *datatype, int count, ompi_op_t *op)
{
    ...
    if (MPI_MINLOC == op || MPI_MAXLOC == op) {
        if (MPI_SHORT_INT == datatype || MPI_DOUBLE_INT == datatype || MPI_LONG_INT == datatype
            ...
            opal_show_help("help-mpi-api.txt", "MPI function not supported", true,
                           (MPI_MINLOC == op) ? "MPI_MINLOC" : "MPI_MAXLOC", reason);
            free(reason);
            /* TODO: this error should return to the caller and invoke an error
             * handler from the MPI API call.
             * For now, it is fatal. */
            ompi_mpi_errors_are_fatal_win_handler(NULL, &err,
                                                  "OSC unsupported MINLOC/MAXLOC datatype");
        }
    }
...
int ompi_osc_base_sndrcv_op(const void *origin, int32_t origin_count,
                            struct ompi_datatype_t *origin_dt, void *target, int32_t target_count,
                            struct ompi_datatype_t *target_dt, ompi_op_t *op)
{
    ...
    do {
        ...
        while (origin_iov_index != origin_iov_count) {
            if (target_iov_index == target_iov_count) {
                ...
                target_iov_index = 0;
                (void) opal_convertor_raw(&target_convertor, target_iovec, &target_iov_count,
                                          &target_size);
            }
            ...
            ompi_op_reduce(op, origin_iovec[origin_iov_index].iov_base,
                           target_iovec[target_iov_index].iov_base, primitive_count,
                           target_primitive);
            ...
            target_iovec[target_iov_index].iov_base
                = (void *) ((intptr_t) target_iovec[target_iov_index].iov_base + acc_len);
            origin_iovec[origin_iov_index].iov_base
                = (void *) ((intptr_t) origin_iovec[origin_iov_index].iov_base + acc_len);
