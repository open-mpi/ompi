...
static bool check_config_value_equal(char *key, opal_info_t *info, char *value)
{
    int ret, flag, param;
...
static int progress_callback(void)
{
    ...
    while (true) {
        ...
        if (PTL_OK == ret) {
            ...
            opal_output_verbose(1, ompi_osc_base_framework.framework_output,
                                "%s:%d: PtlEQGet reported dropped event", __FILE__, __LINE__);
            goto process;
    ...
    process:
        if (ev.ni_fail_type != PTL_OK) {
            opal_output_verbose(1, ompi_osc_base_framework.framework_output,
                                "%s:%d: event failure: %d %d", __FILE__, __LINE__, ev.type,
                                ev.ni_fail_type);
            return 0;
        }
...
static int component_init(bool enable_progress_threads, bool enable_mpi_threads)
{
    ...
    ret = PtlNIInit(PTL_IFACE_DEFAULT, PTL_NI_PHYSICAL | PTL_NI_MATCHING, PTL_PID_ANY, NULL,
                    &actual, &mca_osc_portals4_component.matching_ni_h);
    if (PTL_OK != ret) {
        opal_output_verbose(1, ompi_osc_base_framework.framework_output,
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_osc_base_framework.framework_output, "max_size = %lu",
                         mca_osc_portals4_component.ptl_max_msg_size));
    ...
    mca_osc_portals4_component.matching_atomic_ordered_size = MAX(actual.max_waw_ordered_size,
                                                                  actual.max_war_ordered_size);

    ret = PtlEQAlloc(mca_osc_portals4_component.matching_ni_h, 4096,
    ...
    ret = PtlPTAlloc(mca_osc_portals4_component.matching_ni_h, 0,
                     mca_osc_portals4_component.matching_eq_h, REQ_OSC_TABLE_ID,
    ...
    if (mca_osc_portals4_component.matching_pt_idx != REQ_OSC_TABLE_ID) {
        opal_output_verbose(1, ompi_osc_base_framework.framework_output,
                            ...
                            __LINE__, mca_osc_portals4_component.matching_pt_idx);
        return ret;
    }
    ...
    ret = opal_free_list_init(&mca_osc_portals4_component.requests,
                              sizeof(ompi_osc_portals4_request_t), opal_cache_line_size,
                              OBJ_CLASS(ompi_osc_portals4_request_t), 0, 0, 8, 0, 8, NULL, 0, NULL,
                              NULL, NULL);
    if (OMPI_SUCCESS != ret) {
        opal_output_verbose(1, ompi_osc_base_framework.framework_output,
                            ...
                            int *model)
{
    ...
    module = (ompi_osc_portals4_module_t *) calloc(1, sizeof(ompi_osc_portals4_module_t));
    if (NULL == module)
        return OMPI_ERR_TEMP_OUT_OF_RESOURCE;

    /* fill in the function pointer part */
    memcpy(module, &ompi_osc_portals4_module_template, sizeof(ompi_osc_base_module_t));
    ...
    tmp = disp_unit;
    ret = module->comm->c_coll->coll_bcast(&tmp, 1, MPI_INT, 0, module->comm,
    ...
    ret = PtlMEAppend(module->ni_h, module->pt_idx, &me, PTL_PRIORITY_LIST, &module->ct_link,
                      &module->data_me_h);
    if (PTL_OK != ret) {
        opal_output_verbose(1, ompi_osc_base_framework.framework_output,
    ...
    *model = MPI_WIN_SEPARATE;
    ...
    while (module->ct_link != 2) {
        opal_condition_wait(&mca_osc_portals4_component.cond, &mca_osc_portals4_component.lock);
    }
...
error:
    ...
    if (0 != module->data_me_h)
        PtlMEUnlink(module->data_me_h);
    if (0 != module->req_md_h)
