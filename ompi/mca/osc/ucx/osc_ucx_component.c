...
static int component_register(void)
{
    ...
    mca_osc_ucx_component.priority = UCX_VERSION(major, minor, release_number)
                                             >= UCX_VERSION(1, 5, 0)
                                         ? 60
                                         : 0;
    ...
    (void) mca_base_component_var_register(&mca_osc_ucx_component.super.osc_version, "priority",
    ...
    (void) mca_base_component_var_register(&mca_osc_ucx_component.super.osc_version, "no_locks",
                                           description_str, MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0,
                                           OPAL_INFO_LVL_5, MCA_BASE_VAR_SCOPE_GROUP,
                                           &mca_osc_ucx_component.no_locks);
    free(description_str);
    ...
    (void) mca_base_component_var_register(&mca_osc_ucx_component.super.osc_version,
                                           ...
                                           &mca_osc_ucx_component.acc_single_intrinsic);
    free(description_str);
                             ...
                             int **disps_ptr, void *metadata)
{
    ...
    ret = comm->c_coll->coll_allgather(&my_info_len, 1, MPI_INT, lens, 1, MPI_INT, comm,
    ...
    ret = comm->c_coll->coll_allgatherv(my_info, my_info_len, MPI_BYTE, (void *) (*recv_info_ptr),
                                        lens, (*disps_ptr), MPI_BYTE, comm,
...
static const char *ompi_osc_ucx_set_no_lock_info(opal_infosubscriber_t *obj, const char *key,
                                                 const char *value)
{
                            ...
                            int *model)
{
    ...
    if (mca_osc_ucx_component.env_initialized == false) {
        ...
        ret = opal_free_list_init(&mca_osc_ucx_component.requests, sizeof(ompi_osc_ucx_request_t),
                                  opal_cache_line_size, OBJ_CLASS(ompi_osc_ucx_request_t), 0, 0, 8,
                                  0, 8, NULL, 0, NULL, NULL, NULL);
        if (OMPI_SUCCESS != ret) {
            OSC_UCX_VERBOSE(1, "opal_free_list_init failed: %d", ret);
            goto select_unlock;
        }

        ret = opal_common_ucx_wpool_init(mca_osc_ucx_component.wpool, ompi_proc_world_size(),
...
select_unlock:
    ...
    *model = MPI_WIN_UNIFIED;
    ...
    ret = module->comm->c_coll->coll_allreduce(MPI_IN_PLACE, values, 2, MPI_LONG, MPI_MIN,
                                               module->comm,
    ...
    if (values[0] == -values[1]) { /* everyone has the same disp_unit, we do not need O(p) space */
        ...
        ret = module->comm->c_coll->coll_allgather(&disp_unit, 1, MPI_INT, module->disp_units, 1,
                                                   MPI_INT, module->comm,
    ...
    ret = opal_common_ucx_wpctx_create(mca_osc_ucx_component.wpool, comm_size, &exchange_len_info,
    ...
    if (flavor == MPI_WIN_FLAVOR_ALLOCATE || flavor == MPI_WIN_FLAVOR_CREATE) {
        ...
        ret = opal_common_ucx_wpmem_create(module->ctx, base, size, mem_type, &exchange_len_info,
                                           (void *) module->comm, &my_mem_addr, &my_mem_addr_size,
                                           &module->mem);
        if (ret != OMPI_SUCCESS) {
            goto error;
        }
    ...
    ret = opal_common_ucx_wpmem_create(module->ctx, &state_base, sizeof(ompi_osc_ucx_state_t),
    ...
    ret = comm->c_coll->coll_allgather((void *) my_info, 2 * sizeof(uint64_t), MPI_BYTE, recv_buf,
                                       2 * sizeof(uint64_t), MPI_BYTE, comm,
                                       comm->c_coll->coll_allgather_module);
    if (ret != OMPI_SUCCESS) {
        goto error;
    }
    ...
    for (i = 0; i < comm_size; i++) {
        ...
        memcpy(&(module->state_addrs[i]), recv_buf + i * 2 * sizeof(uint64_t) + sizeof(uint64_t),
               sizeof(uint64_t));
    }
    free(recv_buf);
                                           ...
                                           int max_index, uint64_t base, size_t len, int *insert)
{
    ...
    if (dynamic_wins[mid_index].base > base) {
        return ompi_osc_find_attached_region_position(dynamic_wins, min_index, mid_index - 1, base,
                                                      len, insert);
    } else if (base + len < dynamic_wins[mid_index].base + dynamic_wins[mid_index].size) {
        return mid_index;
    } else {
        return ompi_osc_find_attached_region_position(dynamic_wins, mid_index + 1, max_index, base,
                                                      len, insert);
    }
}
...
int ompi_osc_ucx_win_attach(struct ompi_win_t *win, void *base, size_t len)
{
    ...
    if (module->state.dynamic_win_count > 0) {
        contain_index = ompi_osc_find_attached_region_position(
            (ompi_osc_dynamic_win_info_t *) module->state.dynamic_wins, 0,
            (int) module->state.dynamic_win_count, (uint64_t) base, len, &insert_index);
        if (contain_index >= 0) {
        ...
        memmove((void *) &module->local_dynamic_win_info[insert_index + 1],
                ...
                (OMPI_OSC_UCX_ATTACH_MAX - (insert_index + 1))
                    * sizeof(ompi_osc_local_dynamic_win_info_t));
        memmove((void *) &module->state.dynamic_wins[insert_index + 1],
                (void *) &module->state.dynamic_wins[insert_index],
                (OMPI_OSC_UCX_ATTACH_MAX - (insert_index + 1))
                    * sizeof(ompi_osc_dynamic_win_info_t));
    } else {
        insert_index = 0;
    }

    ret = opal_common_ucx_wpmem_create(module->ctx, &base, len, OPAL_COMMON_UCX_MEM_MAP,
                                       &exchange_len_info, (void *) module->comm,
                                       &(module->local_dynamic_win_info[insert_index].my_mem_addr),
                                       &(module->local_dynamic_win_info[insert_index]
                                             .my_mem_addr_size),
...
int ompi_osc_ucx_win_detach(struct ompi_win_t *win, const void *base)
{
    ...
    contain = ompi_osc_find_attached_region_position((ompi_osc_dynamic_win_info_t *)
                                                         module->state.dynamic_wins,
    ...
    if (module->local_dynamic_win_info[contain].refcnt == 0) {
        ...
        memmove((void *) &(module->local_dynamic_win_info[contain]),
                ...
                (OMPI_OSC_UCX_ATTACH_MAX - (contain + 1))
                    * sizeof(ompi_osc_local_dynamic_win_info_t));
