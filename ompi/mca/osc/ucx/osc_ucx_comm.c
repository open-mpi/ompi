...
static inline int check_sync_state(ompi_osc_ucx_module_t *module, int target, bool is_req_ops)
{
    if (is_req_ops == false) {
...
static inline int ddt_put_get(ompi_osc_ucx_module_t *module, const void *origin_addr,
                              int origin_count, struct ompi_datatype_t *origin_dt,
                              bool is_origin_contig, ptrdiff_t origin_lb, int target,
                              uint64_t remote_addr, int target_count,
                              struct ompi_datatype_t *target_dt, bool is_target_contig,
                              ptrdiff_t target_lb, bool is_get)
{
    ...
    if (!is_origin_contig) {
        ret = create_iov_list(origin_addr, origin_count, origin_dt, &origin_ucx_iov,
                              &origin_ucx_iov_count);
        if (ret != OMPI_SUCCESS) {
            goto cleanup;
        }
    }
    ...
    if (!is_target_contig) {
        ret = create_iov_list(NULL, target_count, target_dt, &target_ucx_iov,
                              &target_ucx_iov_count);
        if (ret != OMPI_SUCCESS) {
            goto cleanup;
        }
    }
    ...
    if (!is_origin_contig && !is_target_contig) {
        ...
        while (origin_ucx_iov_idx < origin_ucx_iov_count) {
            ...
            status = opal_common_ucx_wpmem_putget(module->mem, op, target,
                                                  ...
                                                  remote_addr
                                                      + (uint64_t)(
                                                          target_ucx_iov[target_ucx_iov_idx].addr));
            ...
            origin_ucx_iov[origin_ucx_iov_idx].addr
                = (void *) ((intptr_t) origin_ucx_iov[origin_ucx_iov_idx].addr + curr_len);
            target_ucx_iov[target_ucx_iov_idx].addr
                = (void *) ((intptr_t) target_ucx_iov[target_ucx_iov_idx].addr + curr_len);
...
static inline int start_atomicity(ompi_osc_ucx_module_t *module, int target, bool *lock_acquired)
{
...
static inline int end_atomicity(ompi_osc_ucx_module_t *module, int target, bool lock_acquired,
                                void *free_ptr)
{
    uint64_t remote_addr = (module->state_addrs)[target] + OSC_UCX_STATE_ACC_LOCK_OFFSET;
    int ret = OMPI_SUCCESS;
    ...
    if (lock_acquired) {
        ...
        ret = opal_common_ucx_wpmem_fetch(module->state_mem, UCP_ATOMIC_FETCH_OP_SWAP,
                                          TARGET_LOCK_UNLOCKED, target, &result_value,
                                          sizeof(result_value), remote_addr);
...
static inline int get_dynamic_win_info(uint64_t remote_addr, ompi_osc_ucx_module_t *module,
                                       int target)
{
    uint64_t remote_state_addr = (module->state_addrs)[target]
                                 + OSC_UCX_STATE_DYNAMIC_WIN_CNT_OFFSET;
    ...
    ret = opal_common_ucx_wpmem_putget(module->state_mem, OPAL_COMMON_UCX_GET, target,
                                       (void *) ((intptr_t) temp_buf), len, remote_state_addr);
    if (OPAL_SUCCESS != ret) {
    ...
    contain = ompi_osc_find_attached_region_position(temp_dynamic_wins, 0, win_count, remote_addr,
                                                     1, &insert);
    assert(contain >= 0 && (uint64_t) contain < win_count);

    if (module->local_dynamic_win_info[contain].mem->mem_addrs == NULL) {
        module->local_dynamic_win_info[contain].mem->mem_addrs
            = calloc(ompi_comm_size(module->comm), OMPI_OSC_UCX_MEM_ADDR_MAX_LEN);
        module->local_dynamic_win_info[contain].mem->mem_displs = calloc(ompi_comm_size(
                                                                             module->comm),
                                                                         sizeof(int));
    }

    memcpy(module->local_dynamic_win_info[contain].mem->mem_addrs
               + target * OMPI_OSC_UCX_MEM_ADDR_MAX_LEN,
           temp_dynamic_wins[contain].mem_addr, OMPI_OSC_UCX_MEM_ADDR_MAX_LEN);
    module->local_dynamic_win_info[contain].mem->mem_displs[target]
        = target * OMPI_OSC_UCX_MEM_ADDR_MAX_LEN;
...
static inline bool use_atomic_op(ompi_osc_ucx_module_t *module, struct ompi_op_t *op,
                                 uint64_t remote_addr, struct ompi_datatype_t *origin_dt,
                                 struct ompi_datatype_t *target_dt, int origin_count,
                                 int target_count)
{

    if (module->acc_single_intrinsic && ompi_datatype_is_predefined(origin_dt) && origin_count == 1
        && (op == &ompi_mpi_op_replace.op || op == &ompi_mpi_op_sum.op
...
static int do_atomic_op_intrinsic(ompi_osc_ucx_module_t *module, struct ompi_op_t *op, int target,
                                  const void *origin_addr, int count, struct ompi_datatype_t *dt,
                                  ptrdiff_t target_disp, void *result_addr,
                                  ompi_osc_ucx_request_t *ucx_req)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        ret = opal_common_ucx_wpmem_fetch_nb(module->mem, opcode, value, target, output_addr,
                                             origin_dt_bytes, remote_addr, user_req_cb,
                                             user_req_ptr);
...
static int accumulate_req(const void *origin_addr, int origin_count,
                          struct ompi_datatype_t *origin_dt, int target, ptrdiff_t target_disp,
                          int target_count, struct ompi_datatype_t *target_dt, struct ompi_op_t *op,
                          struct ompi_win_t *win, ompi_osc_ucx_request_t *ucx_req)
{
    ...
    if (use_atomic_op(module, op, target_disp, origin_dt, target_dt, origin_count, target_count)) {
        return do_atomic_op_intrinsic(module, op, target, origin_addr, origin_count, origin_dt,
    ...
    if (op == &ompi_mpi_op_replace.op) {
        ...
        ret = ompi_osc_ucx_get(temp_addr, (int) temp_count, temp_dt, target, target_disp,
                               target_count, target_dt, win);
        if (ret != OMPI_SUCCESS) {
            free(temp_addr);
            return ret;
        ...
        if (ompi_datatype_is_predefined(origin_dt) || is_origin_contig) {
            ...
            if ((op != &ompi_mpi_op_maxloc.op && op != &ompi_mpi_op_minloc.op)
                ...
                while (origin_ucx_iov_idx < origin_ucx_iov_count) {
                    ...
                    ompi_op_reduce(op, origin_ucx_iov[origin_ucx_iov_idx].addr, curr_temp_addr,
                                   curr_count, temp_dt);
                ...
                for (i = 0; i < (int) temp_count; i++) {
                    ompi_op_reduce(op, curr_origin_addr,
                                   (void *) ((char *) temp_addr + i * temp_extent), 1, temp_dt);
                    curr_origin_addr = (void *) ((char *) curr_origin_addr + temp_extent);
                    origin_ucx_iov_idx++;
                    if (curr_origin_addr
                        >= (void *) ((char *) origin_ucx_iov[origin_ucx_iov_idx].addr
                                     + origin_ucx_iov[origin_ucx_iov_idx].len)) {
                        origin_ucx_iov_idx++;
...
{
    return accumulate_req(origin_addr, origin_count, origin_dt, target, target_disp, target_count,
                          target_dt, op, win, NULL);
}

static int do_atomic_compare_and_swap(const void *origin_addr, const void *compare_addr,
                                      void *result_addr, struct ompi_datatype_t *dt, int target,
                                      uint64_t remote_addr, ompi_osc_ucx_module_t *module)
{
    int ret;
    bool lock_acquired = false;
    size_t dt_bytes;
    ...
    ret = opal_common_ucx_wpmem_cmpswp_nb(module->mem, compare_val, value, target, result_addr,
                                          dt_bytes, remote_addr, NULL, NULL);
                                  ...
                                  ptrdiff_t target_disp, struct ompi_win_t *win)
{
    ...
    if (ompi_osc_base_is_atomic_size_supported(remote_addr, dt_bytes)) {
        ...
        return do_atomic_compare_and_swap(origin_addr, compare_addr, result_addr, dt, target,
                                          remote_addr, module);
    }
    ...
    ret = opal_common_ucx_wpmem_putget(module->mem, OPAL_COMMON_UCX_GET, target, &result_addr,
                                       dt_bytes, remote_addr);
    if (OPAL_SUCCESS != ret) {
        OSC_UCX_VERBOSE(1, "opal_common_ucx_mem_putget failed: %d", ret);
        return OMPI_ERROR;
    }
                              ...
                              struct ompi_op_t *op, struct ompi_win_t *win)
{
    ...
    if (ompi_osc_base_is_atomic_size_supported(remote_addr, dt_bytes)
        ...
        ret = opal_common_ucx_wpmem_fetch_nb(module->mem, opcode, value, target,
                                             (void *) result_addr, dt_bytes, remote_addr, NULL,
                                             NULL);
        ...
        return ompi_osc_ucx_get_accumulate(origin_addr, 1, dt, result_addr, 1, dt, target,
                                           target_disp, 1, dt, op, win);
    }
}

static int get_accumulate_req(const void *origin_addr, int origin_count,
                              struct ompi_datatype_t *origin_dt, void *result_addr,
                              int result_count, struct ompi_datatype_t *result_dt, int target,
                              ...
                              struct ompi_win_t *win, ompi_osc_ucx_request_t *ucx_req)
{
    ...
    ret = ompi_osc_ucx_get(result_addr, result_count, result_dt, target, target_disp, target_count,
                           target_dt, win);
    if (ret != OMPI_SUCCESS) {
        return ret;
    }
    ...
    if (op != &ompi_mpi_op_no_op.op) {
        if (op == &ompi_mpi_op_replace.op) {
            ...
            ompi_datatype_t *temp_dt;
            ptrdiff_t temp_lb, temp_extent;
            bool is_origin_contig = ompi_datatype_is_contiguous_memory_layout(origin_dt,
                                                                              origin_count);
                                ...
                                struct ompi_win_t *win)
{
    ...
    return get_accumulate_req(origin_addr, origin_count, origin_dt, result_addr, result_count,
                              result_dt, target, target_disp, target_count, target_dt, op, win,
                              NULL);
}
                      ...
                      struct ompi_request_t **request)
{
    ...
    ret = ompi_osc_ucx_put(origin_addr, origin_count, origin_dt, target, target_disp, target_count,
                           target_dt, win);
    if (ret != OMPI_SUCCESS) {
        return ret;
    }
    ...
    ret = opal_common_ucx_wpmem_fetch_nb(module->mem, UCP_ATOMIC_FETCH_OP_FADD, 0, target,
                                         &(module->req_result), sizeof(uint64_t),
                                         remote_addr & (~0x7), req_completion, ucx_req);
    if (ret != OMPI_SUCCESS) {
                      ...
                      struct ompi_request_t **request)
{
    ...
    ret = ompi_osc_ucx_get(origin_addr, origin_count, origin_dt, target, target_disp, target_count,
                           target_dt, win);
    if (ret != OMPI_SUCCESS) {
        return ret;
    }
                             ...
                             struct ompi_request_t **request)
{
    ...
    ret = accumulate_req(origin_addr, origin_count, origin_dt, target, target_disp, target_count,
                         target_dt, op, win, ucx_req);
    if (ret != OMPI_SUCCESS) {
        OMPI_OSC_UCX_REQUEST_RETURN(ucx_req);
        return ret;
    }
                                 ...
                                 struct ompi_win_t *win, struct ompi_request_t **request)
{
    ...
    ret = get_accumulate_req(origin_addr, origin_count, origin_datatype, result_addr, result_count,
                             result_datatype, target, target_disp, target_count, target_datatype,
                             op, win, ucx_req);
    if (ret != OMPI_SUCCESS) {
        OMPI_OSC_UCX_REQUEST_RETURN(ucx_req);
        return ret;
