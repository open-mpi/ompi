...
int ompi_osc_ubcl_get(void *origin_addr, size_t origin_count,
                      struct ompi_datatype_t *origin_dt, int target,
                      ptrdiff_t target_disp, size_t target_count,
                      struct ompi_datatype_t *target_dt, struct ompi_win_t *win)
{
    return ompi_osc_ubcl_rget(origin_addr, origin_count, origin_dt,
                              target, target_disp, target_count, target_dt,
                              win, NULL);
}

int ompi_osc_ubcl_rget(void *origin_addr, size_t origin_count,
                       struct ompi_datatype_t *origin_dt, int target,
                       ptrdiff_t target_disp, size_t target_count,
                       ...
                       struct ompi_request_t **ompi_req)
{
    ...
    OPAL_OUTPUT_VERBOSE(
        (50, mca_osc_ubcl_component.output, "UBCL_OSC_GET to window %lu\n", module->wid));
    ...
    target_span = opal_datatype_span((const opal_datatype_t *) target_dt, target_count, &gap);
    if (0 == target_span) {
        if (NULL != ompi_req) {
    ...
    opal_convertor_copy_and_prepare_for_recv(proc->super.proc_convertor, &origin_dt->super,
    ...
    if (ompi_datatype_is_contiguous_memory_layout(target_dt, target_count)) {
        ...
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            ...
            opal_free_list_return(&mca_osc_ubcl_component.req_free_list,
                                  (opal_free_list_item_t *) osc_req);
            return ret;
        }
    }

    err = ubcl_get(sbuf_md, target_iov, target_iov_count, endpoint->rank, module->wid,
