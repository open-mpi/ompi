 ...
 * @file
 *
 * OSC/UBCL datatype and convertor related functions
 *
 */
...
size_t osc_ubcl_datatype_pack(void *pack_buf, const void *usr_handle, size_t pack_size,
                              size_t offset)
{
    ...
    if (-1 == ret) {
        mca_osc_ubcl_error(ret, "opal_convertor_unpack failed\n");
    }

    return pack_size;
}

size_t osc_ubcl_datatype_unpack(void *usr_handle, const void *pack_buf, size_t pack_size,
                                size_t offset)
{
    opal_convertor_t *convertor = (opal_convertor_t *) usr_handle;
