...
#define OSC_MONITORING_GENERATE_TEMPLATE_ACCUMULATE(template)                                      \
    ...
    {                                                                                              \
         ...
         */                                                                                        \
        if (OPAL_SUCCESS                                                                           \
            == mca_common_monitoring_get_world_rank(target_rank, win->w_group, &world_rank)) {     \
            size_t type_size;                                                                      \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template).osc_compare_and_swap(origin_addr,     \
                                                                                  compare_addr,    \
                                                                                  result_addr, dt, \
                                                                                  target_rank,     \
                                                                                  target_disp,     \
    ...
    {                                                                                              \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template)                                       \
            .osc_get_accumulate(origin_addr, origin_count, origin_datatype, result_addr,           \
    ...
    {                                                                                              \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template)                                       \
            .osc_rget_accumulate(origin_addr, origin_count, origin_datatype, result_addr,          \
                                 result_count, result_datatype, target_rank, target_disp,          \
    ...
    {                                                                                              \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template)                                       \
            .osc_raccumulate(origin_addr, origin_count, origin_datatype, target_rank, target_disp, \
                             target_count, target_datatype, op, win, request);                     \
    }                                                                                              \
                                                                                                   \
    ...
    {                                                                                              \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template)                                       \
            .osc_accumulate(origin_addr, origin_count, origin_datatype, target_rank, target_disp,  \
                            target_count, target_datatype, op, win);                               \
    }                                                                                              \
                                                                                                   \
    static int ompi_osc_monitoring_##template##_fetch_and_op(const void *origin_addr,              \
                                                             void *result_addr,                    \
                                                             ompi_datatype_t *dt, int target_rank, \
                                                             ptrdiff_t target_disp, ompi_op_t *op, \
                                                             ompi_win_t *win)                      \
    {                                                                                              \
        int world_rank;                                                                            \
        /**                                                                                        \
        ...
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template).osc_fetch_and_op(origin_addr,         \
                                                                              result_addr, dt,     \
                                                                              target_rank,         \
                                                                              target_disp, op,     \
                                                                              win);                \
