...
#define OSC_MONITORING_GENERATE_TEMPLATE_ACTIVE_TARGET(template)                                \
                                                                                                \
    static int ompi_osc_monitoring_##template##_post(ompi_group_t *group, int mpi_assert,       \
                                                     ompi_win_t *win)                           \
    {                                                                                           \
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template).osc_post(group, mpi_assert, win);  \
    }                                                                                           \
                                                                                                \
    static int ompi_osc_monitoring_##template##_start(ompi_group_t *group, int mpi_assert,      \
                                                      ompi_win_t *win)                          \
    {                                                                                           \
