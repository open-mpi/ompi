...
#define OSC_MONITORING_GENERATE_TEMPLATE_PASSIVE_TARGET(template)                                \
    ...
    {                                                                                            \
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template).osc_flush_local_all(win);           \
                                                                                                 ...
                                                                                                 \
    static int ompi_osc_monitoring_##template##_lock(int lock_type, int target, int mpi_assert,  \
                                                     ompi_win_t *win)                            \
    {                                                                                            \
        return OMPI_OSC_MONITORING_MODULE_VARIABLE(template).osc_lock(lock_type, target,         \
                                                                      mpi_assert, win);          \
    }                                                                                            \
                                                                                                 \
