 ...
 */
static int ompi_osc_rdma_peer_btl_endpoint(struct ompi_osc_rdma_module_t *module, int peer_id,
                                           uint8_t *btl_index_out,
                                           struct mca_btl_base_endpoint_t **endpoint)
{
    ...
    for (int module_btl_index = 0; module_btl_index < module->btls_in_use; ++module_btl_index) {
        for (int btl_index = 0; btl_index < num_btls; ++btl_index) {
            if (bml_endpoint->btl_rdma.bml_btls[btl_index].btl
                == module->selected_btls[module_btl_index]) {
    ...
    for (int module_btl_index = 0; module_btl_index < module->btls_in_use; ++module_btl_index) {
        for (int btl_index = 0; btl_index < num_btls; ++btl_index) {
            if (bml_endpoint->btl_eager.bml_btls[btl_index].btl
                == module->selected_btls[module_btl_index]) {
                *btl_index_out = module_btl_index;
 ...
 *
 * This function reads the registration handle and state pointer from the peer that holds that data.
 * If necessary it will then ready information about the peer from its state data structure. This
 * information includes the displacement unit, base pointer, window size, and registation handle (if
 * applicable).
 */
static int ompi_osc_rdma_peer_setup(ompi_osc_rdma_module_t *module, ompi_osc_rdma_peer_t *peer)
{
    ...
    /* each node is responsible for holding a part of the rank -> node/local rank mapping array.
     * this code calculates the node and offset the mapping can be found. once the mapping has been
     * read the state part of the peer structure can be initialized. */
    node_id = peer->rank / RANK_ARRAY_COUNT(module);
    array_peer_data = (ompi_osc_rdma_region_t *) ((intptr_t) module->node_comm_info
                                                  + node_id * module->region_size);
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_DEBUG,
                     "reading region data for %d from rank: %d, index: %d, pointer: 0x%" PRIx64
                     ", size: %lu",
                     peer->rank, node_rank, array_index, array_pointer, sizeof(rank_data));

    ret = ompi_osc_get_data_blocking(module, array_btl_index, array_endpoint, array_pointer,
                                     (mca_btl_base_registration_handle_t *)
                                         array_peer_data->btl_handle_data,
                                     &rank_data, sizeof(rank_data));
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }

    /* initialize the state part of the peer object. NTH: for now the state data is for every node
     * is stored on every node. this gives a good balance of code complexity and memory usage at
     * this time. we take advantage of this by re-using the endpoint and pointer stored in the
     * node_comm_info array. */
    node_peer_data = (ompi_osc_rdma_region_t *) ((intptr_t) module->node_comm_info
                                                 + rank_data.node_id * module->region_size);
    ...
    ret = ompi_osc_rdma_peer_btl_endpoint(module,
                                          NODE_ID_TO_RANK(module, node_peer_data,
                                                          rank_data.node_id),
    ...
    ret = ompi_osc_get_data_blocking(module, peer->state_btl_index, peer->state_endpoint,
                                     peer->state + peer_data_offset, peer->state_handle, peer_data,
                                     peer_data_size);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    if (!module->same_disp_unit) {
        ...
        peer_data += offsetof(ompi_osc_rdma_state_t, regions)
                     - offsetof(ompi_osc_rdma_state_t, disp_unit);
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_DEBUG,
                     "peer %d: remote base region: 0x%" PRIx64 ", size: %" PRId64
                     ", flags: 0x%x, disp_unit: %d",
                     peer->rank, base_region->base, base_region->len, peer->flags, disp_unit);
    ...
    if (base_region->len) {
        if (registration_handle_size) {
            ...
            memcpy(ex_peer->super.base_handle, base_region->btl_handle_data,
 ...
 */
static struct ompi_osc_rdma_peer_t *
ompi_osc_rdma_peer_lookup_internal(struct ompi_osc_rdma_module_t *module, int peer_id)
{
    ompi_osc_rdma_peer_t *peer;
    int ret;
...
OBJ_CLASS_INSTANCE(ompi_osc_rdma_peer_t, opal_list_item_t, ompi_osc_rdma_peer_construct,
...
OBJ_CLASS_INSTANCE(ompi_osc_rdma_peer_basic_t, ompi_osc_rdma_peer_t,
                   ompi_osc_rdma_peer_basic_construct, ompi_osc_rdma_peer_basic_destruct);

OBJ_CLASS_INSTANCE(ompi_osc_rdma_peer_extended_t, ompi_osc_rdma_peer_basic_t, NULL, NULL);
...
OBJ_CLASS_INSTANCE(ompi_osc_rdma_peer_dynamic_t, ompi_osc_rdma_peer_t,
                   ompi_osc_rdma_peer_dynamic_construct, ompi_osc_rdma_peer_dynamic_destruct);
