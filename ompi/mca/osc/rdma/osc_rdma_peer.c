                                            ...
                                            struct mca_btl_base_endpoint_t **endpoint)
{
    ...
    if (NULL == bml_endpoint) {
        /* NOT_READY is not a failure: ompi_osc_rdma_peer_lookup() waits
         * that out. */
    ...
    if (module->use_accelerated_btl) {
        ...
        /* Creation committed to this btl only after waiting for every
         * peer's connection info, so reaching this means a genuine
         * asymmetry: peers offer each other a btl they do not offer us.
         * The window is unusable -- they sized their state for it. */
...
static int ompi_osc_rdma_peer_setup (ompi_osc_rdma_module_t *module, ompi_osc_rdma_peer_t *peer)
{
    ...
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        /* Keep the reason: the caller retries NOT_READY, UNREACH is final. */
        return ret;
    }
                                                        ...
                                                        int *status)
{
    ...
    if (OPAL_UNLIKELY(NULL == peer && OMPI_ERR_NOT_READY == rc)) {
        /* Backstop: window creation already waited for every peer of the
         * communicator. The peer lock must be dropped first -- the wait
         * runs progress, which can re-enter this path. */
