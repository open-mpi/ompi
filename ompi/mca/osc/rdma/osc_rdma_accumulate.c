...
static inline void ompi_osc_rdma_peer_accumulate_cleanup(ompi_osc_rdma_module_t *module,
                                                         ompi_osc_rdma_peer_t *peer,
                                                         bool lock_acquired)
{
    if (lock_acquired) {
        (void) ompi_osc_rdma_lock_release_exclusive(module, peer,
                                                    offsetof(ompi_osc_rdma_state_t,
                                                             accumulate_lock));
    }
...
static int ompi_osc_rdma_gacc_local(const void *source_buffer, int source_count,
                                    ompi_datatype_t *source_datatype, void *result_buffer,
                                    ...
                                    ompi_op_t *op, ompi_osc_rdma_module_t *module,
                                    ompi_osc_rdma_request_t *request, bool lock_acquired)
{
    ...
    do {
        ...
        if (NULL != result_datatype) {
            ...
            ret = ompi_datatype_sndrcv((void *) (intptr_t) target_address, target_count,
                                       target_datatype, result_buffer, result_count,
                                       result_datatype);
        ...
        if (&ompi_mpi_op_no_op.op != op) {
            if (&ompi_mpi_op_replace.op != op) {
                ret = ompi_osc_base_sndrcv_op(source_buffer, source_count, source_datatype,
                                              (void *) (intptr_t) target_address, target_count,
                                              target_datatype, op);
            } else {
                ret = ompi_datatype_sndrcv(source_buffer, source_count, source_datatype,
                                           (void *) (intptr_t) target_address, target_count,
                                           target_datatype);
            }
        }
    ...
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_ERROR, "local accumulate failed with ompi error code %d",
                         ret);
        return ret;
    }
...
static inline int ompi_osc_rdma_cas_local(const void *source_addr, const void *compare_addr,
                                          void *result_addr, ompi_datatype_t *datatype,
                                          ompi_osc_rdma_peer_t *peer, uint64_t target_address,
                                          mca_btl_base_registration_handle_t *target_handle,
...
static int ompi_osc_rdma_op_mapping[OMPI_OP_NUM_OF_TYPES + 1] = {
    [OMPI_OP_MAX] = MCA_BTL_ATOMIC_MAX,   [OMPI_OP_MIN] = MCA_BTL_ATOMIC_MIN,
    [OMPI_OP_SUM] = MCA_BTL_ATOMIC_ADD,   [OMPI_OP_BAND] = MCA_BTL_ATOMIC_AND,
    [OMPI_OP_BOR] = MCA_BTL_ATOMIC_OR,    [OMPI_OP_BXOR] = MCA_BTL_ATOMIC_XOR,
    [OMPI_OP_LAND] = MCA_BTL_ATOMIC_LAND, [OMPI_OP_LOR] = MCA_BTL_ATOMIC_LOR,
    [OMPI_OP_LXOR] = MCA_BTL_ATOMIC_LXOR, [OMPI_OP_REPLACE] = MCA_BTL_ATOMIC_SWAP,
};

/* set the appropriate flags for this atomic */
static inline int ompi_osc_rdma_set_btl_flags(ompi_osc_rdma_module_t *module, ompi_datatype_t *dt,
                                              ptrdiff_t extent)
{
...
static int ompi_osc_rdma_fetch_and_op_atomic(ompi_osc_rdma_sync_t *sync, const void *origin_addr,
                                             void *result_addr, ompi_datatype_t *dt,
                                             ptrdiff_t extent, ompi_osc_rdma_peer_t *peer,
                                             ...
                                             ompi_op_t *op, ompi_osc_rdma_request_t *req)
{
    ...
    if ((8 != extent && !((MCA_BTL_ATOMIC_SUPPORTS_32BIT & atomic_flags) && 4 == extent))
        || (!(OMPI_DATATYPE_FLAG_DATA_INT & dt->super.flags)
            && !(MCA_BTL_ATOMIC_SUPPORTS_FLOAT & atomic_flags))
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "initiating fetch-and-op using %d-bit btl atomics. origin: 0x%" PRIx64,
                     (4 == extent) ? 32 : 64, *((int64_t *) origin_addr));

    origin = (8 == extent) ? ((int64_t *) origin_addr)[0] : ((int32_t *) origin_addr)[0];

    return ompi_osc_rdma_btl_fop(module, peer->data_btl_index, peer->data_endpoint, target_address,
                                 target_handle, btl_op, origin, flags, result_addr, true, NULL,
                                 NULL, NULL);
}

static int ompi_osc_rdma_fetch_and_op_cas(ompi_osc_rdma_sync_t *sync, const void *origin_addr,
                                          void *result_addr, ompi_datatype_t *dt, ptrdiff_t extent,
                                          ompi_osc_rdma_peer_t *peer, uint64_t target_address,
                                          mca_btl_base_registration_handle_t *target_handle,
                                          ompi_op_t *op, ompi_osc_rdma_request_t *req)
{
    ...
    /* align the address. the user should not call with an unaligned address so don't need to range
     * check here */
    ...
    ret = ompi_osc_get_data_blocking(module, peer->data_btl_index, peer->data_endpoint, address,
                                     target_handle, &old_value, 8);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    do {
        ...
        if (&ompi_mpi_op_replace.op == op) {
            memcpy((void *) ((intptr_t) &new_value + offset),
                   (void *) ((intptr_t) origin_addr + dt->super.true_lb), extent);
        } else if (&ompi_mpi_op_no_op.op != op) {
            ompi_op_reduce(op, (void *) ((intptr_t) origin_addr + dt->super.true_lb),
                           (void *) ((intptr_t) &new_value + offset), 1, dt);
        }

        ret = ompi_osc_rdma_btl_cswap(module, peer->data_btl_index, peer->data_endpoint, address,
                                      target_handle, old_value, new_value, 0,
                                      (int64_t *) &new_value);
...
static int ompi_osc_rdma_acc_single_atomic(ompi_osc_rdma_sync_t *sync, const void *origin_addr,
                                           ompi_datatype_t *dt, ptrdiff_t extent,
                                           ompi_osc_rdma_peer_t *peer, uint64_t target_address,
                                           mca_btl_base_registration_handle_t *target_handle,
                                           ompi_op_t *op, ompi_osc_rdma_request_t *req)
{
    ...
    if (!(selected_btl->btl_flags & MCA_BTL_FLAGS_ATOMIC_OPS)) {
        ...
        return ompi_osc_rdma_fetch_and_op_atomic(sync, origin_addr, NULL, dt, extent, peer,
                                                 target_address, target_handle, op, req);
    }
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "initiating accumulate using 64-bit btl atomics. origin: 0x%" PRIx64,
                     *((int64_t *) origin_addr));

    /* if we locked the peer its best to wait for completion before returning */
    return ompi_osc_rdma_btl_op(module, peer->data_btl_index, peer->data_endpoint, target_address,
                                target_handle, btl_op, origin, flags, true, NULL, NULL, NULL);
}

static inline int ompi_osc_rdma_gacc_amo(ompi_osc_rdma_module_t *module, ompi_osc_rdma_sync_t *sync,
                                         const void *source, void *result, int result_count,
                                         ompi_datatype_t *result_datatype,
                                         opal_convertor_t *result_convertor,
                                         ompi_osc_rdma_peer_t *peer, uint64_t target_address,
                                         mca_btl_base_registration_handle_t *target_handle,
                                         ...
                                         ompi_osc_rdma_request_t *request)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "using network atomics for accumulate operation with count %d", count);
    ...
    for (int i = 0; i < count;) {
        if (use_amo) {
            if (NULL == result) {
                ret = ompi_osc_rdma_acc_single_atomic(sync, source, datatype, dt_size, peer,
                                                      target_address, target_handle, op, request);
            } else {
                ret = ompi_osc_rdma_fetch_and_op_atomic(sync, source, result, datatype, dt_size,
                                                        peer, target_address, target_handle, op,
                                                        request);
            }
        } else {
            ret = ompi_osc_rdma_fetch_and_op_cas(sync, source, result, datatype, dt_size, peer,
                                                 target_address, target_handle, op, request);
        }
...
static inline int ompi_osc_rdma_gacc_contig(ompi_osc_rdma_sync_t *sync, const void *source,
                                            int source_count, ompi_datatype_t *source_datatype,
                                            ...
                                            mca_btl_base_registration_handle_t *target_handle,
                                            int target_count, ompi_datatype_t *target_datatype,
                                            ompi_op_t *op, ompi_osc_rdma_request_t *request)
{
    ompi_osc_rdma_module_t *module = sync->module;
    ...
    OSC_RDMA_VERBOSE(
        ...
        "initiating accumulate on contiguous region of %lu bytes to remote address %" PRIx64
        ", sync %p",
        len, target_address, (void *) sync);

    /* if the datatype is small enough (and the count is 1) then try to directly use the hardware to
     * execute the atomic operation. this should be safe in all cases as either 1) the user has
     * assured us they will
     * never use atomics with count > 1, 2) we have the accumulate lock, or 3) we have an exclusive
     * lock */
    if ((target_dtype_size <= 8)
        && (((unsigned long) target_count) <= module->network_amo_max_count)
        && ompi_osc_base_is_atomic_size_supported(target_address, target_dtype_size)) {
        ret = ompi_osc_rdma_gacc_amo(module, sync, source, result, result_count, result_datatype,
                                     result_convertor, peer, target_address, target_handle,
        ...
        OSC_RDMA_VERBOSE(
            MCA_BASE_VERBOSE_TRACE,
            "network atomics not available. falling back to get-op-put implementation...");
    }

    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "using get-op-put to execute accumulate with count %d",
                     target_count);

    if (&ompi_mpi_op_replace.op != op || OMPI_OSC_RDMA_TYPE_GET_ACC == request->type) {
        ptr = malloc(len);
        if (OPAL_UNLIKELY(NULL == ptr)) {
            OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_WARN,
                             "could not allocate a temporary buffer for accumulate");
            return OMPI_ERR_OUT_OF_RESOURCE;
        }
        ...
        if (&ompi_mpi_op_replace.op == op) {
            return ompi_osc_rdma_put_contig(sync, peer, target_address, target_handle,
                                            (void *) source, len, request);
        }

        if (&ompi_mpi_op_no_op.op != op) {
            /* NTH: need to cast away const for the source buffer. the buffer will not be modified
             * by this call */
            ompi_op_reduce(op, (void *) source, ptr, source_count, source_datatype);

            return ompi_osc_rdma_put_contig(sync, peer, target_address, target_handle, ptr, len,
                                            request);
        }
    ...
    return ompi_osc_rdma_put_contig(sync, peer, target_address, target_handle, (void *) source, len,
                                    request);
}

static void ompi_osc_rdma_gacc_master_cleanup(ompi_osc_rdma_request_t *request)
{
    ompi_osc_rdma_peer_accumulate_cleanup(request->module, request->peer,
                                          !ompi_osc_rdma_peer_is_exclusive(request->peer));
}

static inline int ompi_osc_rdma_gacc_master(ompi_osc_rdma_sync_t *sync, const void *source_addr,
                                            int source_count, ompi_datatype_t *source_datatype,
                                            ...
                                            ompi_op_t *op, ompi_osc_rdma_request_t *request)
{
    ...
    if (&ompi_mpi_op_no_op.op == op) {
        /* NTH: just zero these out to catch any coding errors (they should be ignored in the no-op
         * case) */
        source_count = 0;
    ...
    if (OPAL_LIKELY((!source_count || ompi_datatype_is_predefined(source_datatype))
        ...
        ret = ompi_osc_rdma_gacc_contig(sync, source_addr, source_count, source_datatype,
                                        result_addr, result_count, result_datatype, NULL, peer,
                                        target_address, target_handle, target_count,
    ...
    ret = ompi_osc_base_get_primitive_type_info(target_datatype, &target_primitive,
    ...
    if (source_datatype) {
        ret = ompi_osc_base_get_primitive_type_info(source_datatype, &source_primitive,
    ...
    if (source_datatype) {
        ...
        ret = opal_convertor_copy_and_prepare_for_send(ompi_mpi_local_convertor,
                                                       &source_datatype->super, source_count,
                                                       source_addr, 0, &source_convertor);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            return ret;
        }
        source_iov_count = 0;
    ...
    if (result_datatype) {
        ...
        ret = opal_convertor_copy_and_prepare_for_recv(ompi_mpi_local_convertor,
                                                       ...
                                                       result_addr, 0, &result_convertor);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            return ret;
        }
    }
    ...
    ret = opal_convertor_copy_and_prepare_for_send(ompi_mpi_local_convertor,
                                                   ...
                                                   (void *) (intptr_t) target_address, 0,
                                                   &target_convertor);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    do {
        ...
        while (target_iov_index != target_iov_count) {
            if (source_iov_count == source_iov_index) {
                ...
                source_iov_index = 0;
                (void) opal_convertor_raw(&source_convertor, source_iovec, &source_iov_count,
                                          &source_size);
            }
            ...
            if (source_count) {
                acc_len = min(min(target_iovec[target_iov_index].iov_len,
                                  source_iovec[source_iov_index].iov_len),
                              acc_limit);
            } else {
            ...
            if (0 != acc_len) {
                ...
                if (!subreq) {
                    ...
                    subreq->type = result_datatype ? OMPI_OSC_RDMA_TYPE_GET_ACC
                                                   : OMPI_OSC_RDMA_TYPE_ACC;
                    (void) OPAL_THREAD_ADD_FETCH32(&request->outstanding_requests, 1);
                }

                ret = ompi_osc_rdma_gacc_contig(sync, source_iovec[source_iov_index].iov_base,
                                                acc_len / target_primitive->super.size,
                                                target_primitive, NULL, 0, NULL,
                                                result_datatype ? &result_convertor : NULL, peer,
                                                (uint64_t)(intptr_t) target_iovec[target_iov_index]
                                                    .iov_base,
                                                target_handle,
                                                acc_len / target_primitive->super.size,
                                                target_primitive, op, subreq);
                if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            ...
            if (source_datatype) {
                ...
                source_iovec[source_iov_index].iov_base
                    = (void *) ((intptr_t) source_iovec[source_iov_index].iov_base + acc_len);
            }
        }
    } while (!done);
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "finished scheduling rdma on non-contiguous datatype(s)");
...
static inline int ompi_osc_rdma_cas_atomic(ompi_osc_rdma_sync_t *sync, const void *source_addr,
                                           ...
                                           bool lock_acquired)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "initiating compare-and-swap using %d-bit btl atomics. compare: 0x%" PRIx64
                     ", origin: 0x%" PRIx64,
                     (int) size * 8, *((int64_t *) compare_addr), *((int64_t *) source_addr));

    ret = ompi_osc_rdma_btl_cswap(module, peer->data_btl_index, peer->data_endpoint, target_address,
                                  target_handle, compare, source, flags, result_addr);
    if (OPAL_LIKELY(OMPI_SUCCESS == ret)) {
        ompi_osc_rdma_peer_accumulate_cleanup(module, peer, lock_acquired);
    }
 ...
 */
static void ompi_osc_rdma_cas_put_complete(struct mca_btl_base_module_t *btl,
                                           struct mca_btl_base_endpoint_t *endpoint,
                                           void *local_address,
                                           mca_btl_base_registration_handle_t *local_handle,
 ...
 */
static inline int cas_rdma(ompi_osc_rdma_sync_t *sync, const void *source_addr,
                           const void *compare_addr, void *result_addr, ompi_datatype_t *datatype,
                           ompi_osc_rdma_peer_t *peer, uint64_t target_address,
                           mca_btl_base_registration_handle_t *target_handle, bool lock_acquired)
{
    ...
    OSC_RDMA_VERBOSE(
        MCA_BASE_VERBOSE_TRACE,
        "initiating compare-and-swap using RMDA on %lu bytes to remote address %" PRIx64
        ", sync %p",
        len, target_address, (void *) sync);

    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "RDMA compare-and-swap initiating blocking btl get...");
    ret = ompi_osc_get_data_blocking(module, peer->data_btl_index, peer->data_endpoint,
                                     target_address, target_handle, result_addr, len);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "RDMA compare-and-swap initiating blocking btl put...");

    do {
        ret = btl->btl_put(btl, peer->data_endpoint, ptr, target_address, local_handle,
                           target_handle, len, 0, MCA_BTL_NO_ORDER, ompi_osc_rdma_cas_put_complete,
                           (void *) &complete, NULL);
        if (OPAL_SUCCESS == ret
            || (OPAL_ERR_OUT_OF_RESOURCE != ret && OPAL_ERR_TEMP_OUT_OF_RESOURCE != ret)) {
            break;
        }
...
int ompi_osc_rdma_compare_and_swap(const void *origin_addr, const void *compare_addr,
                                   void *result_addr, ompi_datatype_t *dt, int target_rank,
                                   ptrdiff_t target_disp, ompi_win_t *win)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "cswap: 0x%lx, 0x%lx, 0x%lx, %s, %d, %d, %s",
                     ...
                     (unsigned long) result_addr, dt->name, target_rank, (int) target_disp,
                     win->w_name);
    ...
    ret = osc_rdma_get_remote_segment(module, peer, target_disp, true_lb + true_extent,
    ...
    if (!ompi_osc_rdma_peer_is_exclusive(peer)
        && !(module->acc_single_intrinsic || win->w_acc_ops <= OMPI_WIN_ACCUMULATE_OPS_SAME_OP)) {
        (void) ompi_osc_rdma_lock_acquire_exclusive(module, peer,
                                                    offsetof(ompi_osc_rdma_state_t,
                                                             accumulate_lock));
        lock_acquired = true;
    }
    ...
    if (!use_shared_mem) {
        ...
        if (ompi_osc_base_is_atomic_size_supported(target_address, dt->super.size)) {
            ret = ompi_osc_rdma_cas_atomic(sync, origin_addr, compare_addr, result_addr, dt, peer,
                                           target_address, target_handle, lock_acquired);
            if (OMPI_SUCCESS == ret) {
                return OMPI_SUCCESS;
            }
        }
    ...
    if (ompi_osc_rdma_peer_local_base(peer)) {
        ret = ompi_osc_rdma_cas_local(origin_addr, compare_addr, result_addr, dt, peer,
                                      target_address, target_handle, module, lock_acquired);
    } else {
    ...
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        /* operation failed. the application will most likely abort but we still want to leave the
         * window in working state if possible. on successful completion the above calls with clear
         * the lock and accumulate state */
        ompi_osc_rdma_peer_accumulate_cleanup(module, peer, lock_acquired);
    }
...
static inline int ompi_osc_rdma_rget_accumulate_internal(
    ompi_win_t *win, const void *origin_addr, int origin_count, ompi_datatype_t *origin_datatype,
    void *result_addr, int result_count, ompi_datatype_t *result_datatype, int target_rank,
    MPI_Aint target_disp, int target_count, ompi_datatype_t *target_datatype, ompi_op_t *op,
    ompi_request_t **request_out)
{
    ...
    ret = osc_rdma_get_remote_segment(module, peer, target_disp, target_span + target_lb,
                                      &target_address, &target_handle);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    if (ompi_osc_rdma_peer_local_base(peer)) {
        ...
        ret = ompi_osc_rdma_gacc_local(origin_addr, origin_count, origin_datatype, result_addr,
                                       result_count, result_datatype, peer, target_address,
                                       target_handle, target_count, target_datatype, op, module,
                                       rdma_request, lock_acquired);
    } else {
        /* do not need to pass the lock acquired flag to this function. the value of the flag can be
         * obtained just by calling ompi_osc_rdma_peer_is_exclusive() in this case. */
        ret = ompi_osc_rdma_gacc_master(sync, origin_addr, origin_count, origin_datatype,
                                        result_addr, result_count, result_datatype, peer,
                                        target_address, target_handle, target_count,
...
int ompi_osc_rdma_get_accumulate(const void *origin_addr, int origin_count,
                                 ompi_datatype_t *origin_datatype, void *result_addr,
                                 int result_count, ompi_datatype_t *result_datatype,
...
int ompi_osc_rdma_raccumulate(const void *origin_addr, int origin_count,
                              ompi_datatype_t *origin_datatype, int target_rank,
                              ...
                              ompi_request_t **request)
{
    ...
    return ompi_osc_rdma_rget_accumulate_internal(win, origin_addr, origin_count, origin_datatype,
                                                  NULL, 0, NULL, target_rank, target_disp,
                                                  target_count, target_datatype, op, request);
}
                             ...
                             ompi_datatype_t *target_datatype, ompi_op_t *op, ompi_win_t *win)
{
    ...
    return ompi_osc_rdma_rget_accumulate_internal(win, origin_addr, origin_count, origin_datatype,
                                                  NULL, 0, NULL, target_rank, target_disp,
                                                  target_count, target_datatype, op, NULL);
}

int ompi_osc_rdma_fetch_and_op(const void *origin_addr, void *result_addr, ompi_datatype_t *dt,
                               int target_rank, ptrdiff_t target_disp, ompi_op_t *op,
                               ompi_win_t *win)
{
