 ...
 * returns -1.
 */
static inline ompi_osc_rdma_region_t *
ompi_osc_rdma_find_region_containing(ompi_osc_rdma_region_t *regions, int min_index, int max_index,
                                     intptr_t base, intptr_t bound, size_t region_size,
                                     int *region_index)
{
    int mid_index = (max_index + min_index) >> 1;
    ompi_osc_rdma_region_t *region = (ompi_osc_rdma_region_t *) ((intptr_t) regions
                                                                 + mid_index * region_size);
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_DEBUG,
                     "checking memory region %p-%p against %p-%p (index %d) (min_index = %d, "
                     "max_index = %d)",
                     (void *) base, (void *) bound, (void *) region->base,
                     (void *) (region->base + region->len), mid_index, min_index, max_index);

    if (region->base > base) {
        return ompi_osc_rdma_find_region_containing(regions, min_index, mid_index - 1, base, bound,
    ...
    return ompi_osc_rdma_find_region_containing(regions, mid_index + 1, max_index, base, bound,
                                                region_size, region_index);
}

/* binary search for insertion point */
static ompi_osc_rdma_region_t *find_insertion_point(ompi_osc_rdma_region_t *regions, int min_index,
                                                    int max_index, intptr_t base,
                                                    size_t region_size, int *region_index)
{
    ...
    if (region->base > base || (region->base == base && (size_t) region->len > region_size)) {
        return find_insertion_point(regions, min_index, mid_index - 1, base, region_size,
                                    region_index);
    }
...
static bool ompi_osc_rdma_find_conflicting_attachment(ompi_osc_rdma_handle_t *handle, intptr_t base,
                                                      intptr_t bound)
{
    ...
    OPAL_LIST_FOREACH (attachment, &handle->attachments, ompi_osc_rdma_attachment_t) {
        ...
        if ((base >= attachment->base && base < region_bound)
            ...
            OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                             "existing region {%p, %p} overlaps region {%p, %p}",
                             (void *) attachment->base, (void *) region_bound, (void *) base,
                             (void *) bound);
            return true;
        }
    }
...
int ompi_osc_rdma_attach(struct ompi_win_t *win, void *base, size_t len)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "attach: %s, %p, %lu", win->w_name, base,
                     (unsigned long) len);

    OPAL_THREAD_LOCK(&module->lock);
    ompi_osc_rdma_lock_acquire_exclusive(module, my_peer,
                                         offsetof(ompi_osc_rdma_state_t, regions_lock));
    ...
    if (region_count == mca_osc_rdma_component.max_attach) {
        ...
        OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                         "attach: could not attach. max attachment count reached.");
        return OMPI_ERR_RMA_ATTACH;
    }
    ...
    region = ompi_osc_rdma_find_region_containing((ompi_osc_rdma_region_t *) module->state->regions,
                                                  0, region_count - 1, aligned_base, aligned_bound,
                                                  module->region_size, &region_index);
    if (NULL != region) {
        /* validates that the region does not overlap with an existing region even if they are on
         * the same page */
        ret = ompi_osc_rdma_add_attachment(module->dynamic_handles[region_index], (intptr_t) base,
                                           len);
        OPAL_THREAD_UNLOCK(&module->lock);
        ompi_osc_rdma_lock_release_exclusive(module, my_peer,
                                             offsetof(ompi_osc_rdma_state_t, regions_lock));
    ...
    if (region_count) {
        region = find_insertion_point((ompi_osc_rdma_region_t *) module->state->regions, 0,
                                      region_count - 1, (intptr_t) base, module->region_size,
                                      &region_index);

        if (region_index < region_count) {
            memmove((void *) ((intptr_t) region + module->region_size), region,
                    (region_count - region_index) * module->region_size);
            memmove(module->dynamic_handles + region_index + 1,
                    module->dynamic_handles + region_index,
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_DEBUG,
                     "attaching dynamic memory region {%p, %p} aligned {%p, %p}, at index %d", base,
                     (void *) ((intptr_t) base + len), (void *) aligned_base,
                     (void *) aligned_bound, region_index);
    ...
    if (module->use_memory_registration) {
        ...
        ret = ompi_osc_rdma_register(module, MCA_BTL_ENDPOINT_ANY, (void *) region->base,
                                     region->len, MCA_BTL_REG_FLAG_ACCESS_ANY, &handle);
        ...
        memcpy(region->btl_handle_data, handle,
    ...
    for (int i = 0; i < region_count + 1; ++i) {
        region = (ompi_osc_rdma_region_t *) ((intptr_t) module->state->regions
                                             + i * module->region_size);
...
int ompi_osc_rdma_detach(struct ompi_win_t *win, const void *base)
{
    ompi_osc_rdma_module_t *module = GET_MODULE(win);
    const int my_rank = ompi_comm_rank(module->comm);
    ompi_osc_rdma_peer_dynamic_t *my_peer = (ompi_osc_rdma_peer_dynamic_t *)
    ...
    ompi_osc_rdma_lock_acquire_exclusive(module, &my_peer->super,
                                         offsetof(ompi_osc_rdma_state_t, regions_lock));
    ...
    for (region_index = 0; region_index < region_count; ++region_index) {
        ...
        region = (ompi_osc_rdma_region_t *) ((intptr_t) module->state->regions
                                             + region_index * module->region_size);
        OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_INFO,
                         "checking attachments at index %d {.base=%p, len=%lu} for attachment %p"
                         ", region handle=%p",
                         region_index, (void *) region->base, (unsigned long) region->len, base,
                         (void *) rdma_region_handle);

        if ((uintptr_t) region->base > (uintptr_t) base
            || (uintptr_t)(region->base + region->len) < (uintptr_t) base) {
            continue;
        }
    ...
    if (region_index == region_count) {
        OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_INFO, "could not find dynamic memory attachment for %p",
                         base);
        OPAL_THREAD_UNLOCK(&module->lock);
        ompi_osc_rdma_lock_release_exclusive(module, &my_peer->super,
                                             offsetof(ompi_osc_rdma_state_t, regions_lock));
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_DEBUG,
                     "detaching dynamic memory region {%p, %p} from index %d", base,
                     (void *) ((intptr_t) base + region->len), region_index);
 ...
 *
 * This function does the work of keeping the local view of a remote peer in sync with what is
 * attached to the remote window. It is called on every address translation since there is no way
 * (currently) to detect that the attached regions have changed. To reduce the amount of data read
 * we first read the region count (which contains an id). If that hasn't changed the region data is
 * not updated. If the list of attached regions has changed then all valid regions are read from the
 * peer while holding their region lock.
 */
static int ompi_osc_rdma_refresh_dynamic_region(ompi_osc_rdma_module_t *module,
                                                ompi_osc_rdma_peer_dynamic_t *peer)
{
    osc_rdma_counter_t region_count, region_id;
    uint64_t source_address;
    int ret;

    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "refreshing dynamic memory regions for target %d",
                     peer->super.rank);
    ...
    do {
        ...
        source_address = (uint64_t)(intptr_t) peer->super.state
                         + offsetof(ompi_osc_rdma_state_t, region_count);
        ret = ompi_osc_get_data_blocking(module, peer->super.state_btl_index,
                                         peer->super.state_endpoint, source_address,
                                         peer->super.state_handle, &remote_value,
                                         sizeof(remote_value));
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            return ret;
        }
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_DEBUG,
                     "target region: id 0x%lx, count 0x%lx (cached: 0x%x, 0x%x)",
                     (unsigned long) region_id, (unsigned long) region_count, peer->region_id,
                     peer->region_count);
    ...
    if (peer->region_id != region_id) {
        ...
        OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_DEBUG,
                         "dynamic memory cache is out of data. reloading from peer");
        ...
        /* lock the region */
        ompi_osc_rdma_lock_acquire_shared(module, &peer->super, 1,
                                          offsetof(ompi_osc_rdma_state_t, regions_lock),
                                          OMPI_OSC_RDMA_LOCK_EXCLUSIVE);

        source_address = (uint64_t)(intptr_t) peer->super.state
                         + offsetof(ompi_osc_rdma_state_t, regions);
        ret = ompi_osc_get_data_blocking(module, peer->super.state_btl_index,
                                         peer->super.state_endpoint, source_address,
                                         peer->super.state_handle, peer->regions, region_len);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            OPAL_THREAD_UNLOCK(&module->lock);
            return ret;
        }
        ...
        ompi_osc_rdma_lock_release_shared(module, &peer->super, -1,
                                          offsetof(ompi_osc_rdma_state_t, regions_lock));
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "finished refreshing dynamic memory regions for target %d", peer->super.rank);
                                      ...
                                      uint64_t base, size_t len, ompi_osc_rdma_region_t **region)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "locating dynamic memory region matching: {%" PRIx64 ", %" PRIx64 "}"
                     " (len %lu)",
                     base, base + len, (unsigned long) len);

    OPAL_THREAD_LOCK(&module->lock);
    // Make sure region isn't being touched.
    ompi_osc_rdma_lock_acquire_exclusive(module, peer,
                                         offsetof(ompi_osc_rdma_state_t, regions_lock));
    if (!ompi_osc_rdma_peer_local_state(peer)) {
        ret = ompi_osc_rdma_refresh_dynamic_region(module, dy_peer);
        if (OMPI_SUCCESS != ret) {
            ompi_osc_rdma_lock_release_exclusive(module, peer,
                                                 offsetof(ompi_osc_rdma_state_t, regions_lock));
            return ret;
        }
