...
static int ompi_osc_rdma_component_query(struct ompi_win_t *win, void **base, size_t size,
...
static int ompi_osc_rdma_query_alternate_btls(ompi_communicator_t *comm,
                                              ompi_osc_rdma_module_t *module);
static int ompi_osc_rdma_query_mtls(void);

static const char *ompi_osc_rdma_set_no_lock_info(opal_infosubscriber_t *obj, const char *key,
                                                  const char *value);
...
static int ompi_osc_rdma_component_register(void)
{
    ...
    (void) mca_base_component_var_register(&mca_osc_rdma_component.super.osc_version, "no_locks",
                                           description_str, MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0,
                                           OPAL_INFO_LVL_5, MCA_BASE_VAR_SCOPE_GROUP,
                                           &mca_osc_rdma_component.no_locks);
    ...
    (void) mca_base_component_var_register(&mca_osc_rdma_component.super.osc_version,
                                           "acc_single_intrinsic", description_str,
                                           MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, OPAL_INFO_LVL_5,
                                           MCA_BASE_VAR_SCOPE_GROUP,
                                           &mca_osc_rdma_component.acc_single_intrinsic);
    free(description_str);
    ...
    opal_asprintf(&description_str,
                  ...
                  "used instread (default: %s)",
                  mca_osc_rdma_component.acc_use_amo ? "true" : "false");
    ...
    opal_asprintf(&description_str, "Size of temporary buffers (default: %d)",
    ...
    (void) mca_base_component_var_register(&mca_osc_rdma_component.super.osc_version, "buffer_size",
                                           description_str, MCA_BASE_VAR_TYPE_UNSIGNED_INT, NULL, 0,
                                           0, OPAL_INFO_LVL_3, MCA_BASE_VAR_SCOPE_LOCAL,
                                           &mca_osc_rdma_component.buffer_size);
    free(description_str);
    ...
    opal_asprintf(&description_str,
                  "Priority of the osc/rdma component when using non-RDMA btls (default: %d)",
                  mca_osc_rdma_component.alternate_priority);
    (void) mca_base_component_var_register(&mca_osc_rdma_component.super.osc_version,
                                           "alternate_priority", description_str,
                                           MCA_BASE_VAR_TYPE_UNSIGNED_INT, NULL, 0, 0,
                                           OPAL_INFO_LVL_3, MCA_BASE_VAR_SCOPE_GROUP,
                                           &mca_osc_rdma_component.alternate_priority);
    free(description_str);

    (void) mca_base_var_enum_create("osc_rdma_locking_mode", ompi_osc_rdma_locking_modes,
                                    &new_enum);

    mca_osc_rdma_component.locking_mode = OMPI_OSC_RDMA_LOCKING_TWO_LEVEL;
    (void) mca_base_component_var_register(
        &mca_osc_rdma_component.super.osc_version, "locking_mode",
        "Locking mode to use for passive-target synchronization (default: two_level)",
        MCA_BASE_VAR_TYPE_INT, new_enum, 0, 0, OPAL_INFO_LVL_3, MCA_BASE_VAR_SCOPE_GROUP,
        &mca_osc_rdma_component.locking_mode);
    ...
    (void) mca_base_component_var_register(&mca_osc_rdma_component.super.osc_version, "btls",
                                           description_str, MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0,
                                           OPAL_INFO_LVL_3, MCA_BASE_VAR_SCOPE_GROUP,
                                           &ompi_osc_rdma_btl_names);
    free(description_str);
    ...
    opal_asprintf(
        &description_str,
        "Comma-delimited list of alternate BTL component names to allow without verifying "
        "connectivity (default: %s)",
        ompi_osc_rdma_btl_alternate_names);
    (void) mca_base_component_var_register(&mca_osc_rdma_component.super.osc_version,
                                           "alternate_btls", description_str,
    ...
    opal_asprintf(&description_str,
                  ...
                  "osc component (default: %s)",
                  ompi_osc_rdma_mtl_names);
    ...
    (void) mca_base_component_var_register(
        &mca_osc_rdma_component.super.osc_version, "backing_directory",
        ...
        &mca_osc_rdma_component.backing_directory);

    mca_osc_rdma_component.network_amo_max_count = 32;
    (void) mca_base_component_var_register(
        &mca_osc_rdma_component.super.osc_version, "network_max_amo",
        ...
        MCA_BASE_VAR_TYPE_UNSIGNED_LONG, NULL, 0, 0, OPAL_INFO_LVL_3, MCA_BASE_VAR_SCOPE_LOCAL,
        &mca_osc_rdma_component.network_amo_max_count);

    /* register performance variables */

    (void) mca_base_component_pvar_register(
        &mca_osc_rdma_component.super.osc_version, "put_retry_count",
        "Number of times put transaction were retried due to resource limitations", OPAL_INFO_LVL_4,
        MCA_BASE_PVAR_CLASS_COUNTER, MCA_BASE_VAR_TYPE_UNSIGNED_LONG, NULL,
        MCA_BASE_VAR_BIND_MPI_WIN, MCA_BASE_PVAR_FLAG_CONTINUOUS, ompi_osc_rdma_pvar_read, NULL,
        NULL, (void *) (intptr_t) offsetof(ompi_osc_rdma_module_t, put_retry_count));

    (void) mca_base_component_pvar_register(
        &mca_osc_rdma_component.super.osc_version, "get_retry_count",
        "Number of times get transaction were retried due to resource limitations", OPAL_INFO_LVL_4,
        MCA_BASE_PVAR_CLASS_COUNTER, MCA_BASE_VAR_TYPE_UNSIGNED_LONG, NULL,
        MCA_BASE_VAR_BIND_MPI_WIN, MCA_BASE_PVAR_FLAG_CONTINUOUS, ompi_osc_rdma_pvar_read, NULL,
        NULL, (void *) (intptr_t) offsetof(ompi_osc_rdma_module_t, get_retry_count));
...
static int ompi_osc_rdma_component_init(bool enable_progress_threads, bool enable_mpi_threads)
{
    ...
    ret = opal_free_list_init(&mca_osc_rdma_component.frags, sizeof(ompi_osc_rdma_frag_t), 8,
                              OBJ_CLASS(ompi_osc_rdma_frag_t), mca_osc_rdma_component.buffer_size,
                              8, 4, -1, 4, NULL, 0, NULL, NULL, NULL);
    if (OPAL_SUCCESS != ret) {
        opal_output_verbose(1, ompi_osc_base_framework.framework_output,
                            "%s:%d: opal_free_list_init_new failed: %d", __FILE__, __LINE__, ret);
        return ret;
    }

    OBJ_CONSTRUCT(&mca_osc_rdma_component.requests, opal_free_list_t);
    ret = opal_free_list_init(&mca_osc_rdma_component.requests, sizeof(ompi_osc_rdma_request_t), 8,
                              OBJ_CLASS(ompi_osc_rdma_request_t), 0, 0, 0, -1, 32, NULL, 0, NULL,
                              NULL, NULL);
    if (OPAL_SUCCESS != ret) {
        opal_output_verbose(1, ompi_osc_base_framework.framework_output,
...
int ompi_osc_rdma_component_finalize(void)
{
    ...
    if (0 != (num_modules = opal_hash_table_get_size(&mca_osc_rdma_component.modules))) {
        opal_output(ompi_osc_base_framework.framework_output,
                    "WARNING: There were %d Windows created but "
                    "not freed.",
                    (int) num_modules);
    }
...
static int ompi_osc_rdma_initialize_region(ompi_osc_rdma_module_t *module, void **base, size_t size)
{
    ...
    if (module->use_memory_registration && size) {
        if (MPI_WIN_FLAVOR_ALLOCATE != module->flavor || NULL == module->state_handle) {
            ret = ompi_osc_rdma_register(module, MCA_BTL_ENDPOINT_ANY, *base, size,
                                         MCA_BTL_REG_FLAG_ACCESS_ANY, &module->base_handle);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
                return OMPI_ERR_OUT_OF_RESOURCE;
            }

            memcpy(region->btl_handle_data, module->base_handle,
                   module->selected_btls[0]->btl_registration_handle_size);
        } else {
            memcpy(region->btl_handle_data, module->state_handle,
                   module->selected_btls[0]->btl_registration_handle_size);
        }
    }
...
static int allocate_state_single(ompi_osc_rdma_module_t *module, void **base, size_t size)
{
    ...
    /* allocate anything that will be accessed remotely in the same region. this cuts down on the
     * number of registration handles needed to access this data. */
    total_size = local_rank_array_size + module->region_size + module->state_size
                 + leader_peer_data_size;
    ...
    /* the local data is ordered as follows: rank array (leader, offset mapping), state, leader peer
     * data, and base (if using MPI_Win_allocate). In this case the leader peer data array does not
     * need to be stored in the same
    ...
    module->state = (ompi_osc_rdma_state_t *) ((intptr_t) module->rank_array
                                               + module->state_offset);
...
static int synchronize_errorcode(int errorcode, ompi_communicator_t *comm)
{
    ...
    ret = comm->c_coll->coll_allreduce(MPI_IN_PLACE, &err, 1, MPI_INT, MPI_MIN, comm,
...
static int allocate_state_shared(ompi_osc_rdma_module_t *module, void **base, size_t size)
{
    ...
    /* CPU atomics can be used if every process is on the same node or the NIC allows mixing CPU and
     * NIC atomics */
    ...
    if (!module->single_node) {
        for (int i = 0; i < module->btls_in_use; ++i) {
            module->use_cpu_atomics = module->use_cpu_atomics
                                      && !!(module->selected_btls[i]->btl_flags
    ...
    do {
        ...
        ret = shared_comm->c_coll->coll_allgather(MPI_IN_PLACE, sizeof(*temp), MPI_BYTE, temp,
                                                  sizeof(*temp), MPI_BYTE, shared_comm,
                                                  shared_comm->c_coll->coll_allgather_module);
        if (OMPI_SUCCESS != ret) {
            break;
        }
        ...
        if (0 == local_rank) {
            ...
            ret = opal_asprintf(&data_file, "%s" OPAL_PATH_SEP "osc_rdma.%s.%x.%d",
                                mca_osc_rdma_component.backing_directory,
                                ompi_process_info.nodename, OMPI_PROC_MY_NAME->jobid,
        ...
        state_region = (ompi_osc_rdma_region_t *) ((uintptr_t) module->segment_base
                                                   + local_rank_array_size);
        module->state = (ompi_osc_rdma_state_t *) ((uintptr_t) module->segment_base + state_base
                                                   + module->state_size * local_rank);

        /* all local ranks share the array containing the peer data of leader ranks */
        module->node_comm_info = (unsigned char *) ((uintptr_t) module->segment_base + state_base
                                                    + module->state_size * local_size);
        ...
        if (0 == local_rank) {
            ...
            /* just go ahead and register the whole segment */
            ret = ompi_osc_rdma_register(module, MCA_BTL_ENDPOINT_ANY, module->segment_base,
                                         total_size, MCA_BTL_REG_FLAG_ACCESS_ANY,
                                         &module->state_handle);
        ...
        for (int i = 0; i < local_size; ++i) {
            ...
            ompi_osc_rdma_state_t *peer_state = (ompi_osc_rdma_state_t
                                                     *) ((uintptr_t) module->segment_base
                                                         + state_base + module->state_size * i);
            ...
            if (module->use_cpu_atomics) {
                ...
                /* use my endpoint handle to modify the peer's state */
                if (module->use_memory_registration) {
                    peer->state_handle = (mca_btl_base_registration_handle_t *)
                                             state_region->btl_handle_data;
                }
                peer->state = (osc_rdma_counter_t)((uintptr_t) state_region->base + state_base
                                                   + module->state_size * i);
                if (i > 0) {
            ...
            if (module->use_cpu_atomics
                && (MPI_WIN_FLAVOR_ALLOCATE == module->flavor || peer_rank == my_rank)) {
                /* base is local and cpu atomics are available */
                if (MPI_WIN_FLAVOR_ALLOCATE == module->flavor) {
                ...
                if (module->use_memory_registration) {
                    ex_peer->super.base_handle = (mca_btl_base_registration_handle_t *)
                                                     peer_region->btl_handle_data;
                }
            }
        }
    } while (0);
...
static int ompi_osc_rdma_query_mtls(void)
{
    ...
    if (mtls_to_use && ompi_mtl_base_selected_component) {
        for (int i = 0; mtls_to_use[i]; ++i) {
            if (0
                == strcmp(mtls_to_use[i],
                          ompi_mtl_base_selected_component->mtl_version.mca_component_name)) {
                                              ...
                                              ompi_osc_rdma_module_t *module)
{
    ...
    if (NULL == btls_to_use) {
        OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_INFO, "no alternate BTLs requested: %s",
                         ompi_osc_rdma_btl_alternate_names);
    ...
    for (int i = 0; btls_to_use[i]; ++i) {
        ...
        OPAL_LIST_FOREACH (item, &mca_btl_base_modules_initialized,
...
static int ompi_osc_rdma_query_btls(ompi_communicator_t *comm, ompi_osc_rdma_module_t *module)
{
        ...
        OPAL_LIST_FOREACH (item, &mca_btl_base_modules_initialized,
            ...
            for (int i = 0; btls_to_use[i]; ++i) {
                ...
                if ((item->btl_module->btl_flags & (MCA_BTL_FLAGS_RDMA)) == MCA_BTL_FLAGS_RDMA
                    && (item->btl_module->btl_flags
                        & (MCA_BTL_FLAGS_ATOMIC_FOPS | MCA_BTL_FLAGS_ATOMIC_OPS))) {
                    if (!selected_btl
                        || item->btl_module->btl_latency < selected_btl->btl_latency) {
    ...
    for (int rank = 0; rank < comm_size; ++rank) {
        ...
        for (int i_btl = 0; i_btl < num_btls; ++i_btl) {
            ...
            if ((endpoint->btl_rdma.bml_btls[i_btl].btl->btl_flags
                 & (MCA_BTL_FLAGS_RDMA | MCA_BTL_FLAGS_ATOMIC_FOPS))
                    == (MCA_BTL_FLAGS_RDMA | MCA_BTL_FLAGS_ATOMIC_FOPS)
                && (endpoint->btl_rdma.bml_btls[i_btl].btl->btl_atomic_flags
    ...
    do {
        ...
        if (0 == ompi_comm_rank(module->shared_comm)) {
            ...
            if (ompi_comm_size(module->local_leaders) > 1) {
                ret = module->local_leaders->c_coll
                          ->coll_allgather(MPI_IN_PLACE, module->region_size, MPI_BYTE,
                                           module->node_comm_info, module->region_size, MPI_BYTE,
...
static int ompi_osc_rdma_create_groups(ompi_osc_rdma_module_t *module)
{
    ...
    ret = ompi_comm_split(module->comm, (0 == local_rank) ? 0 : MPI_UNDEFINED, comm_rank,
                          &module->local_leaders, false);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_ERROR,
 ...
 *
 * This function checks if all ranks have the same displacement unit or size and sets the
 * appropriate flags on the module.
 */
static int ompi_osc_rdma_check_parameters(ompi_osc_rdma_module_t *module, int disp_unit,
                                          size_t size)
{
    long values[4];
    int ret;
                                          ...
                                          struct opal_info_t *info, int flavor, int *model)
{
    ...
    module->selected_btls = calloc(module->selected_btls_size,
    ...
    if (MPI_WIN_FLAVOR_DYNAMIC == flavor) {
        ...
        module->dynamic_handles = (ompi_osc_rdma_handle_t **)
            calloc(mca_osc_rdma_component.max_attach, sizeof(module->dynamic_handles[0]));
    ...
    ret = opal_hash_table_set_value_uint32(&mca_osc_rdma_component.modules,
                                           ompi_comm_get_cid(module->comm), module);
