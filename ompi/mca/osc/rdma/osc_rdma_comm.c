...
/* helper functions */
static inline void ompi_osc_rdma_cleanup_rdma(ompi_osc_rdma_sync_t *sync, bool dec_always,
...
static int ompi_osc_rdma_get_contig(ompi_osc_rdma_sync_t *sync, ompi_osc_rdma_peer_t *peer,
                                    uint64_t source_address,
                                    mca_btl_base_registration_handle_t *source_handle,
                                    void *target_buffer, size_t size,
                                    ompi_osc_rdma_request_t *request);

static void ompi_osc_get_data_complete(struct mca_btl_base_module_t *btl,
                               ...
                               size_t len)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "reading data from endpoint %p. source: 0x%" PRIx64 " (aligned: 0x%" PRIx64
                     "), len: %lu (aligned: %lu)",
                     (void *) endpoint, source_address, aligned_addr, (unsigned long) len,
    ...
    if (btl->btl_register_mem && len >= btl->btl_get_local_registration_threshold) {
        ...
        OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "allocated temporary buffer %p in fragment %p",
                         (void *) ptr, (void *) frag);
    }
    ...
    do {
        ret = btl->btl_get(btl, endpoint, ptr, aligned_addr, local_handle, source_handle,
                           aligned_len, 0, MCA_BTL_NO_ORDER, ompi_osc_get_data_complete,
                           (void *) &read_complete, NULL);
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "finished reading state data from endpoint %p",
                     (void *) endpoint);
 ...
 * @param[in] remote_handle   btl registration handle for remote region (must be valid for the
 * entire region)
 ...
 *
 * This function does the work of scheduling a contiguous transfer between the local and remote
 * regions.
 */
typedef int (*ompi_osc_rdma_fn_t)(ompi_osc_rdma_sync_t *sync, ompi_osc_rdma_peer_t *peer,
                                  uint64_t remote_address,
                                  mca_btl_base_registration_handle_t *remote_handle,
                                  void *local_address, size_t size,
                                  ompi_osc_rdma_request_t *request);
 ...
 * @param[in] alloc_reqs       true if rdma_fn requires a valid request object (any allocated
 * objects will be marked internal)
 *
 * This function does the work of breaking a non-contiguous rdma transfer into contiguous
 * components. It will continue to submit rdma transfers until the entire region is transferred or a
 * fatal error occurs.
 */
static int ompi_osc_rdma_master_noncontig(ompi_osc_rdma_sync_t *sync, void *local_address,
                                          int local_count, ompi_datatype_t *local_datatype,
                                          ompi_osc_rdma_peer_t *peer, uint64_t remote_address,
                                          mca_btl_base_registration_handle_t *remote_handle,
                                          int remote_count, ompi_datatype_t *remote_datatype,
                                          ompi_osc_rdma_request_t *request,
                                          const size_t max_rdma_len,
                                          const ompi_osc_rdma_fn_t rdma_fn, const bool alloc_reqs)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "scheduling rdma on non-contiguous datatype(s) or large region");
    ...
    ret = opal_convertor_copy_and_prepare_for_send(ompi_mpi_local_convertor,
                                                   &remote_datatype->super, remote_count,
                                                   (void *) (intptr_t) remote_address, 0,
                                                   &remote_convertor);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    ret = opal_convertor_copy_and_prepare_for_send(ompi_mpi_local_convertor, &local_datatype->super,
                                                   local_count, local_address, 0, &local_convertor);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    do {
        ...
        while (remote_iov_index != remote_iov_count) {
            if (local_iov_index == local_iov_count) {
                ...
                local_iov_index = 0;
                (void) opal_convertor_raw(&local_convertor, local_iovec, &local_iov_count,
                                          &local_size);
            }
            ...
            rdma_len = min(min(local_iovec[local_iov_index].iov_len,
                               remote_iovec[remote_iov_index].iov_len),
                           max_rdma_len);
            ...
            ret = rdma_fn(sync, peer, (uint64_t)(intptr_t) remote_iovec[remote_iov_index].iov_base,
                          remote_handle, local_iovec[local_iov_index].iov_base, rdma_len, subreq);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
                if (OPAL_UNLIKELY(OMPI_ERR_OUT_OF_RESOURCE != ret)) {
                    if (request) {
            ...
            local_iovec[local_iov_index].iov_base
                = (void *) ((intptr_t) local_iovec[local_iov_index].iov_base + rdma_len);
            remote_iovec[remote_iov_index].iov_base
                = (void *) ((intptr_t) remote_iovec[remote_iov_index].iov_base + rdma_len);
...
static inline int ompi_osc_rdma_master(ompi_osc_rdma_sync_t *sync, void *local_address,
                                       int local_count, ompi_datatype_t *local_datatype,
                                       ompi_osc_rdma_peer_t *peer, uint64_t remote_address,
                                       mca_btl_base_registration_handle_t *remote_handle,
                                       int remote_count, ompi_datatype_t *remote_datatype,
                                       ...
                                       const ompi_osc_rdma_fn_t rdma_fn, const bool alloc_reqs)
{
    ...
    if (OPAL_LIKELY(ompi_datatype_is_contiguous_memory_layout(local_datatype, local_count)
        ...
        OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                         "performing rdma on contiguous region. local: %p, "
                         "remote: 0x%lx, length: %lu",
                         local_address, (unsigned long) remote_address, rdma_len);

        do {
            ret = rdma_fn(sync, peer, remote_address, remote_handle, local_address, rdma_len,
                          request);
    ...
    return ompi_osc_rdma_master_noncontig(sync, local_address, local_count, local_datatype, peer,
                                          remote_address, remote_handle, remote_count,
                                          remote_datatype, request, max_rdma_len, rdma_fn,
                                          alloc_reqs);
}

static int ompi_osc_rdma_copy_local(const void *source, int source_count,
                                    ompi_datatype_t *source_datatype, void *target,
                                    ...
                                    ompi_osc_rdma_request_t *request)
{
    ...
    opal_atomic_mb();
    ret = ompi_datatype_sndrcv(source, source_count, source_datatype, target, target_count,
                               target_datatype);
...
static void ompi_osc_rdma_put_complete(struct mca_btl_base_module_t *btl,
                                       struct mca_btl_base_endpoint_t *endpoint,
                                       void *local_address,
                                       mca_btl_base_registration_handle_t *local_handle,
                                       void *context, void *data, int status)
{
    ...
    if ((intptr_t) context & 0x1) {
        ompi_osc_rdma_request_t *request = request = (ompi_osc_rdma_request_t *) ((intptr_t) context
                                                                                  & ~1);
        sync = request->sync;
    ...
    OSC_RDMA_VERBOSE(status ? MCA_BASE_VERBOSE_ERROR : MCA_BASE_VERBOSE_TRACE,
                     "btl put complete on sync %p. local "
                     "address %p. opal status %d",
                     (void *) sync, local_address, status);
...
static void ompi_osc_rdma_put_complete_flush(struct mca_btl_base_module_t *btl,
                                             struct mca_btl_base_endpoint_t *endpoint,
                                             void *local_address,
                                             mca_btl_base_registration_handle_t *local_handle,
                                             void *context, void *data, int status)
{
    ...
    OSC_RDMA_VERBOSE(status ? MCA_BASE_VERBOSE_ERROR : MCA_BASE_VERBOSE_TRACE,
                     "btl put complete on module %p. local "
                     "address %p. opal status %d",
                     (void *) module, local_address, status);
...
static int ompi_osc_rdma_put_real(ompi_osc_rdma_sync_t *sync, ompi_osc_rdma_peer_t *peer,
                                  ...
                                  mca_btl_base_rdma_completion_fn_t cb, void *context, void *cbdata)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "initiating btl put of %lu bytes to remote address %" PRIx64 ", sync "
                     "object %p...",
                     (unsigned long) size, target_address, (void *) sync);
    ...
    do {
        ret = btl->btl_put(btl, peer->data_endpoint, ptr, target_address, local_handle,
                           target_handle, size, 0, MCA_BTL_NO_ORDER, cb, context, cbdata);
        if (OPAL_UNLIKELY(OMPI_SUCCESS == ret)) {
            return OMPI_SUCCESS;
        }
                             ...
                             size_t size, ompi_osc_rdma_request_t *request)
{
    ...
    if (btl->btl_register_mem && size > btl->btl_put_local_registration_threshold) {
        ...
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            ret = ompi_osc_rdma_register(module, peer->data_endpoint, source_buffer, size, 0,
                                         &local_handle);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
                return ret;
            }
    ...
    if (ompi_osc_rdma_use_btl_flush(module)) {
        /* NTH: when using the btl_flush function there is no guarantee that the callback will
         * happen before the flush is complete. because of this there is a chance that the sync
         * object will be released before there is a callback. to handle this case we call different
         * callback that doesn't use the sync object. its possible the btl sematics will change in
         * the future and the callback will happen *before* flush is considered complete. if that is
         * the case this workaround can be removed */
    ...
    ret = ompi_osc_rdma_put_real(sync, peer, target_address, target_handle, ptr, local_handle, size,
                                 cbfunc, cbcontext, frag);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
...
static void ompi_osc_rdma_get_complete(struct mca_btl_base_module_t *btl,
                                       struct mca_btl_base_endpoint_t *endpoint,
                                       void *local_address,
                                       mca_btl_base_registration_handle_t *local_handle,
                                       void *context, void *data, int status)
{
    ...
    OSC_RDMA_VERBOSE(status ? MCA_BASE_VERBOSE_ERROR : MCA_BASE_VERBOSE_TRACE,
                     "btl get complete on sync %p. local "
                     "address %p. origin %p. opal status %d",
                     (void *) sync, local_address, origin_addr, status);
...
static int ompi_osc_rdma_get_partial(ompi_osc_rdma_sync_t *sync, ompi_osc_rdma_peer_t *peer,
                                     uint64_t source_address,
                                     mca_btl_base_registration_handle_t *source_handle,
                                     void *target_buffer, size_t size,
                                     ompi_osc_rdma_request_t *request)
{
    ...
    ret = ompi_osc_rdma_get_contig(sync, peer, source_address, source_handle, target_buffer, size,
                                   subreq);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        OMPI_OSC_RDMA_REQUEST_RETURN(subreq);
        ompi_osc_rdma_request_deref(request);
                                    ...
                                    ompi_osc_rdma_request_t *request)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                     "initiating get of %lu bytes from remote ptr %" PRIx64 " to local ptr %p",
    ...
    if ((btl->btl_register_mem && size > btl->btl_get_local_registration_threshold)
        ...
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            if (OMPI_ERR_VALUE_OUT_OF_BOUNDS == ret) {
                ...
                if ((source_address & btl_alignment_mask)
                    && (source_address & btl_alignment_mask)
                           == ((intptr_t) target_buffer & btl_alignment_mask)) {
                    /* remote region has the same alignment but the base is not aligned. perform a
                     * small buffered get of the beginning of the remote region */
                    aligned_source_base = OPAL_ALIGN(source_address, btl->btl_get_alignment,
                                                     osc_rdma_base_t);
                    subsize = (size_t)(aligned_source_base - source_address);

                    ret = ompi_osc_rdma_get_partial(sync, peer, source_address, source_handle,
                                                    target_buffer, subsize, request);
                    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
                        return ret;
                    }
                ...
                if (!(((uint64_t) target_buffer | source_address) & btl_alignment_mask)
                    ...
                    size = aligned_len;
                    ret = ompi_osc_rdma_get_partial(sync, peer, source_address + aligned_len,
                                                    source_handle,
                                                    (void *) ((intptr_t) target_buffer
                                                              + aligned_len),
                                                    subsize, request);
                    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
                        return ret;
            ...
            if (NULL != ptr) {
                (void) ompi_osc_rdma_register(module, peer->data_endpoint, ptr, aligned_len,
                                              MCA_BTL_REG_FLAG_LOCAL_WRITE, &local_handle);
            }
            ...
            OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE,
                             "using internal buffer %p in fragment %p for get of size %lu bytes, "
                             "source address 0x%lx",
                             (void *) ptr, (void *) frag, (unsigned long) aligned_len,
                             (unsigned long) aligned_source_base);
            local_handle = frag->handle;
        }
    }
    ...
    if (request->buffer) {
        /* always increment the outstanding RDMA counter as the btl_flush function does not
         * guarantee callback completion, just operation completion. */
        counter_needs_inc = true;
        ompi_osc_rdma_sync_rdma_inc_always(sync);
    } else {
        /* if this operation is being buffered with a frag then ompi_osc_rdma_sync_rdma_complete()
         * can use the number of pending operations on the rdma_frag as an indicator as to whether
         * the operation is complete. this can only be done since there is only on rdma frag per
         * module. if that changes this logic will need to be changed
         * as well. this path also covers the case where the get operation is not buffered. */
        ompi_osc_rdma_sync_rdma_inc(sync);
    }

    do {
        ret = btl->btl_get(btl, peer->data_endpoint, ptr, aligned_source_base, local_handle,
                           source_handle, aligned_len, 0, MCA_BTL_NO_ORDER,
...
static inline int ompi_osc_rdma_put_w_req(ompi_osc_rdma_sync_t *sync, const void *origin_addr,
                                          ...
                                          ompi_osc_rdma_request_t *request)
{
    ...
    ret = osc_rdma_get_remote_segment(module, peer, target_disp, offset + len, &target_address,
                                      &target_handle);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
        return ret;
    }
    ...
    if (ompi_osc_rdma_peer_local_base(peer)) {
        return ompi_osc_rdma_copy_local(origin_addr, origin_count, origin_datatype,
    ...
    return ompi_osc_rdma_master(sync, (void *) origin_addr, origin_count, origin_datatype, peer,
                                target_address, target_handle, target_count, target_datatype,
                                request, btl->btl_put_limit, ompi_osc_rdma_put_contig, false);
}

static inline int ompi_osc_rdma_get_w_req(ompi_osc_rdma_sync_t *sync, void *origin_addr,
                                          int origin_count, ompi_datatype_t *origin_datatype,
                                          ompi_osc_rdma_peer_t *peer, ptrdiff_t source_disp,
                                          ...
                                          ompi_osc_rdma_request_t *request)
{
    ...
    if (ompi_osc_rdma_peer_local_base(peer)) {
        return ompi_osc_rdma_copy_local((void *) (intptr_t) source_address, source_count,
                                        source_datatype, origin_addr, origin_count, origin_datatype,
                                        request);
    }

    return ompi_osc_rdma_master(sync, origin_addr, origin_count, origin_datatype, peer,
                                source_address, source_handle, source_count, source_datatype,
                                request, btl->btl_get_limit, ompi_osc_rdma_get_contig, true);
}
                      ...
                      ompi_datatype_t *target_datatype, ompi_win_t *win)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "put: 0x%lx, %d, %s, %d, %d, %d, %s, %s",
    ...
    return ompi_osc_rdma_put_w_req(sync, origin_addr, origin_count, origin_datatype, peer,
                                   target_disp, target_count, target_datatype, NULL);
}

int ompi_osc_rdma_rput(const void *origin_addr, int origin_count, ompi_datatype_t *origin_datatype,
                       int target_rank, ptrdiff_t target_disp, int target_count,
                       ompi_datatype_t *target_datatype, ompi_win_t *win, ompi_request_t **request)
{
    ompi_osc_rdma_module_t *module = GET_MODULE(win);
    ompi_osc_rdma_peer_t *peer;
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "rput: 0x%lx, %d, %s, %d, %d, %d, %s, %s",
                     (unsigned long) origin_addr, origin_count, origin_datatype->name, target_rank,
                     (int) target_disp, target_count, target_datatype->name, win->w_name);
    ...
    ret = ompi_osc_rdma_put_w_req(sync, origin_addr, origin_count, origin_datatype, peer,
                                  target_disp, target_count, target_datatype, rdma_request);
    if (OMPI_SUCCESS != ret) {
                      ...
                      ompi_datatype_t *source_datatype, ompi_win_t *win)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "get: 0x%lx, %d, %s, %d, %d, %d, %s, %s",
                     (unsigned long) origin_addr, origin_count, origin_datatype->name, source_rank,
                     (int) source_disp, source_count, source_datatype->name, win->w_name);
...
int ompi_osc_rdma_rget(void *origin_addr, int origin_count, ompi_datatype_t *origin_datatype,
                       int source_rank, ptrdiff_t source_disp, int source_count,
                       ompi_datatype_t *source_datatype, ompi_win_t *win, ompi_request_t **request)
{
    ompi_osc_rdma_module_t *module = GET_MODULE(win);
    ompi_osc_rdma_peer_t *peer;
    ompi_osc_rdma_request_t *rdma_request;
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "rget: 0x%lx, %d, %s, %d, %d, %d, %s, %s",
                     (unsigned long) origin_addr, origin_count, origin_datatype->name, source_rank,
                     (int) source_disp, source_count, source_datatype->name, win->w_name);
