...
int ompi_osc_rdma_flush_all(struct ompi_win_t *win)
{
    ...
    /* flush all locks */
    ret = opal_hash_table_get_first_key_uint32(&module->outstanding_locks, &key, (void **) &lock,
                                               &node);
...
/* locking via atomics */
static inline int ompi_osc_rdma_lock_atomic_internal(ompi_osc_rdma_module_t *module,
                                                     ompi_osc_rdma_peer_t *peer,
                                                     ompi_osc_rdma_sync_t *lock)
{
    ...
    if (MPI_LOCK_EXCLUSIVE == lock->sync.lock.type) {
        do {
            ...
            ret = ompi_osc_rdma_lock_try_acquire_exclusive(module, peer,
                                                           offsetof(ompi_osc_rdma_state_t,
                                                                    local_lock));
            if (ret) {
                /* release the global lock */
                if (OMPI_OSC_RDMA_LOCKING_TWO_LEVEL == locking_mode) {
                    ompi_osc_rdma_lock_release_shared(module, module->leader, -1,
        ...
        do {
            ...
            ret = ompi_osc_rdma_lock_acquire_shared(module, peer, 1,
                                                    offsetof(ompi_osc_rdma_state_t, local_lock),
                                                       ...
                                                       ompi_osc_rdma_sync_t *lock)
{
    ...
    if (MPI_LOCK_EXCLUSIVE == lock->sync.lock.type) {
        ...
        ompi_osc_rdma_lock_release_shared(module, peer, -1,
                                          offsetof(ompi_osc_rdma_state_t, local_lock));
    ...
    OPAL_THREAD_SCOPED_LOCK(
        &peer->lock, do {
            if (!ompi_osc_rdma_peer_is_demand_locked(peer)) {
                ...
                OPAL_THREAD_SCOPED_LOCK(&lock->lock,
                                        opal_list_append(&lock->demand_locked_peers, &peer->super));
                peer->flags |= OMPI_OSC_RDMA_PEER_DEMAND_LOCKED;
            }
        } while (0););
...
int ompi_osc_rdma_lock_atomic(int lock_type, int target, int mpi_assert, ompi_win_t *win)
{
    ...
    OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_TRACE, "lock: %d, %d, %d, %s", lock_type, target, mpi_assert,
                     win->w_name);
    ...
    if (module->all_sync.epoch_active
        && (OMPI_OSC_RDMA_SYNC_TYPE_LOCK != module->all_sync.type
            || MPI_LOCK_EXCLUSIVE == lock_type)) {
...
int ompi_osc_rdma_unlock_atomic(int target, ompi_win_t *win)
{
    ...
    if (OPAL_UNLIKELY(NULL == lock)) {
        OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_INFO, "target %d is not locked in window %s", target,
                         win->w_name);
        OPAL_THREAD_UNLOCK(&module->lock);
        return OMPI_ERR_RMA_SYNC;
    }
...
int ompi_osc_rdma_lock_all_atomic(int mpi_assert, struct ompi_win_t *win)
{
    ...
    if (module->all_sync.epoch_active) {
        OSC_RDMA_VERBOSE(MCA_BASE_VERBOSE_INFO,
                         "attempted lock_all when active target epoch is %s "
                         "and lock all epoch is %s",
                         (OMPI_OSC_RDMA_SYNC_TYPE_LOCK != module->all_sync.type
                          && module->all_sync.epoch_active)
                             ? "active"
                             : "inactive",
                         (OMPI_OSC_RDMA_SYNC_TYPE_LOCK == module->all_sync.type) ? "active"
                                                                                 : "inactive");
        OPAL_THREAD_UNLOCK(&module->lock);
        return OMPI_ERR_RMA_SYNC;
    }
