 ...
 */
int mca_coll_cuda_init_query(bool enable_progress_threads, bool enable_mpi_threads)
{
    /* Nothing to do */
 ...
 */
mca_coll_base_module_t *mca_coll_cuda_comm_query(struct ompi_communicator_t *comm, int *priority)
{
 ...
 */
int mca_coll_cuda_module_enable(mca_coll_base_module_t *module, struct ompi_communicator_t *comm)
{
    bool good = true;
    char *msg = NULL;
...
#define CHECK_AND_RETAIN(src, dst, name)                                          \
    ...
    opal_show_help("help-mpi-coll-cuda.txt", "missing collective", true, ompi_process_info.nodename,
