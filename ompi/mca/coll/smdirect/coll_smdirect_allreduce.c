                                ...
                                mca_coll_base_module_t *module)
{
    ...
    if (MPI_IN_PLACE == sbuf) {
        ...
        if (0 == rank) {
            ret = mca_coll_smdirect_reduce_intra(sbuf, rbuf, count, dtype, op, 0,
                                           comm, module);
        } else {
            ret = mca_coll_smdirect_reduce_intra(rbuf, NULL, count, dtype, op, 0,
                                           comm, module);
        }
    } else {
        ret = mca_coll_smdirect_reduce_intra(sbuf, rbuf, count, dtype, op, 0,
    ...
    return (ret == OMPI_SUCCESS) ?
        mca_coll_smdirect_bcast_intra(rbuf, count, dtype, 0, comm, module) : ret;
}
