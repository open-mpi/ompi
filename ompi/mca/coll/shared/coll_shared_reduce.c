                                 ...
                                 mca_coll_base_module_t * module)
{
    ...
    if (count * extent <= 32 * 1024) {
        ompi_coll_base_reduce_intra_binomial(sbuf, rbuf, count, dtype, op,
                                             root, comm, module, 2048,
                                             128);
    } else {
        mca_coll_shared_reduce_ring_intra(sbuf, rbuf, count, dtype, op,
                                          root, comm, module);
    }
                                      ...
                                      mca_coll_base_module_t * module)
{
    ...
    /* At last, root copies data from the shared data buffer */
    if (rank == root) {
        char *c;
        c = rbuf;
        for (i = 0; i < size; i++) {
