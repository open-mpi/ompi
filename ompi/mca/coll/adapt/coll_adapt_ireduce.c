...
static int ompi_coll_adapt_ireduce_generic(IREDUCE_ARGS, ompi_coll_tree_t *tree, size_t seg_size);
...
int ompi_coll_adapt_ireduce_register(void)
{
    ...
    mca_base_component_var_register(c, "reduce_algorithm",
                                    "Algorithm of reduce, 1: binomial, 2: in_order_binomial, 3: "
                                    "binary, 4: pipeline, 5: chain, 6: linear",
    ...
    mca_base_component_var_register(c, "reduce_max_send_requests",
                                    "Maximum number of send requests", MCA_BASE_VAR_TYPE_INT, NULL,
                                    0, 0, OPAL_INFO_LVL_5, MCA_BASE_VAR_SCOPE_READONLY,
    ...
    mca_base_component_var_register(
        c, "inbuf_free_list_inc", "Number of segments to allocate when growing the inbuf free list",
 ...
 */
static ompi_coll_adapt_item_t *get_next_ready_item(ompi_coll_adapt_constant_reduce_context_t *con,
                                                   int num_children)
{
...
static int add_to_recv_list(ompi_coll_adapt_constant_reduce_context_t *con, int id)
{
    ...
    OPAL_LIST_FOREACH (item, &con->recv_list, ompi_coll_adapt_item_t) {
        if (item->id == id) {
            ...
            OPAL_THREAD_UNLOCK(&con->mutex_recv_list);
            OPAL_OUTPUT_VERBOSE(
                (30, mca_coll_adapt_component.adapt_output, "add_to_recv_list_return 1\n"));
            return 1;
        }
    }
    ...
    if (context->con->accumbuf != NULL) {
        if (context->con->rank != context->con->root) {
            for (int i = 0; i < context->con->num_segs; i++) {
                OPAL_OUTPUT_VERBOSE(
                    (30, mca_coll_adapt_component.adapt_output, "[%d]: Return accumbuf %d %p\n",
                     ompi_comm_rank(context->con->comm), i,
                     (void *) to_inbuf(context->con->accumbuf[i], context->con->distance)));
                opal_free_list_return_st(&context->con->inbuf_list,
                                         (opal_free_list_item_t *)
                                             to_inbuf(context->con->accumbuf[i],
...
{
    ompi_coll_adapt_reduce_context_t *context = (ompi_coll_adapt_reduce_context_t *)
    ...
    /* Send a new segment */
    ompi_coll_adapt_item_t *item = get_next_ready_item(context->con,
                                                       context->con->tree->tree_nextsize);

    if (item != NULL) {
        /* Get new context item from free list */
        ompi_coll_adapt_reduce_context_t *send_context = (ompi_coll_adapt_reduce_context_t *)
        ...
        if (context->con->tree->tree_nextsize > 0) {
            ...
            send_context->buff = context->buff
                                 + (item->id - context->seg_index)
                                       * context->con->segment_increment;
        }
    ...
    int32_t num_sent = opal_atomic_add_fetch_32((opal_atomic_int32_t *) &(
                                                    context->con->num_sent_segs),
                                                1);
    OPAL_OUTPUT_VERBOSE((30, mca_coll_adapt_component.adapt_output,
    ...
    if (num_sent == context->con->num_segs
        && context->con->num_recv_segs
               == context->con->num_segs * context->con->tree->tree_nextsize) {
...
static int recv_cb(ompi_request_t *req)
{
    ompi_coll_adapt_reduce_context_t *context = (ompi_coll_adapt_reduce_context_t *)
                                                    req->req_complete_cb_data;
    int32_t new_id = opal_atomic_add_fetch_32((opal_atomic_int32_t *) &(
                                                  context->con->next_recv_segs[context->child_id]),
                                              1);
    int err;
    ...
    if (new_id < context->con->num_segs) {
        ...
        /* Get new context item from free list */
        ompi_coll_adapt_reduce_context_t *recv_context = (ompi_coll_adapt_reduce_context_t *)
            opal_free_list_wait(mca_coll_adapt_component.adapt_ireduce_context_free_list);
        ...
        OPAL_OUTPUT_VERBOSE(
            (30, mca_coll_adapt_component.adapt_output,
             ...
             context->con->rank, recv_context->seg_index, recv_context->peer, (void *) inbuf,
    ...
    if (NULL == context->con->accumbuf[context->seg_index]) {
        if (NULL == context->inbuf) {
            ...
            context->con->accumbuf[context->seg_index] = context->inbuf->buff
                                                         - context->con->lower_bound;
        }
        /* Op sbuf and accmbuf to accumbuf */
        ompi_op_reduce(context->con->op,
                       context->con->sbuf
                           + (ptrdiff_t) context->seg_index
                                 * (ptrdiff_t) context->con->segment_increment,
        ...
        if (NULL == context->inbuf) {
            ...
            opal_free_list_return(&context->con->inbuf_list,
                                  (opal_free_list_item_t *)
                                      to_inbuf(context->con->accumbuf[context->seg_index],
                                               context->con->distance));
    ...
    if (context->con->rank != context->con->tree->tree_root
        ...
        if (NULL != item) {
            ...
            OPAL_OUTPUT_VERBOSE((30, mca_coll_adapt_component.adapt_output,
                                 ...
                                 send_context->con->rank, send_context->seg_index,
                                 send_context->peer,
                                 send_context->con->ireduce_tag - send_context->seg_index));

            ompi_request_t *send_req;
            err = MCA_PML_CALL(
                isend(send_context->buff, send_count, send_context->con->datatype,
                      send_context->peer, send_context->con->ireduce_tag - send_context->seg_index,
                      MCA_PML_BASE_SEND_STANDARD, send_context->con->comm, &send_req));
            if (MPI_SUCCESS != err) {
                return err;
            }
    ...
    int32_t num_recv_segs = opal_atomic_add_fetch_32((opal_atomic_int32_t *) &(
                                                         context->con->num_recv_segs),
                                                     1);
    OPAL_OUTPUT_VERBOSE(
        (30, mca_coll_adapt_component.adapt_output,
         "[%d]: In recv_cb, tree = %p, root = %d, num_recv = %d, num_segs = %d, num_child = %d\n",
         context->con->rank, (void *) context->con->tree, context->con->tree->tree_root,
    ...
    if (num_recv_segs == context->con->num_segs * context->con->tree->tree_nextsize
        && (context->con->tree->tree_root == context->con->rank
            || context->con->num_sent_segs == context->con->num_segs)) {
        ireduce_request_fini(context);
    } else {
                            ...
                            ompi_request_t **request, mca_coll_base_module_t *module)
{
    ...
    if (!ompi_op_is_commute(op)) {
        ...
        OPAL_OUTPUT_VERBOSE((30, mca_coll_adapt_component.adapt_output,
                             "ADAPT cannot handle reduce with this (non-commutative) operation. It "
                             "needs to fall back on another component\n"));
    ...
    OPAL_OUTPUT_VERBOSE(
        (10, mca_coll_adapt_component.adapt_output,
         "ireduce root %d, algorithm %d, coll_adapt_ireduce_segment_size %zu, "
         "coll_adapt_ireduce_max_send_requests %d, coll_adapt_ireduce_max_recv_requests %d\n",
    ...
    return ompi_coll_adapt_ireduce_generic(
        sbuf, rbuf, count, dtype, op, root, comm, request, module,
                                    ...
                                    size_t seg_size)
{
    ...
    if (NULL == mca_coll_adapt_component.adapt_ireduce_context_free_list) {
        ...
        opal_free_list_init(fl, sizeof(ompi_coll_adapt_reduce_context_t), opal_cache_line_size,
                            OBJ_CLASS(ompi_coll_adapt_reduce_context_t), 0, opal_cache_line_size,
        ...
        if (!OPAL_ATOMIC_COMPARE_EXCHANGE_STRONG_PTR(
                (opal_atomic_intptr_t *) &mca_coll_adapt_component.adapt_ireduce_context_free_list,
                &(intptr_t){0}, fl)) {
            OBJ_RELEASE(fl);
        }
    }
    ...
    *request = (ompi_request_t *) temp_request;
    ...
    ompi_coll_adapt_constant_reduce_context_t *con = OBJ_NEW(
        ompi_coll_adapt_constant_reduce_context_t);
    con->count = count;
    ...
    if (tree->tree_nextsize > 0) {
        ...
        opal_free_list_init(&con->inbuf_list, sizeof(ompi_coll_adapt_inbuf_t) + real_seg_size,
                            opal_cache_line_size, OBJ_CLASS(ompi_coll_adapt_inbuf_t), 0,
                            opal_cache_line_size, num_allocate_elems,
                            mca_coll_adapt_component.adapt_inbuf_free_list_max,
                            mca_coll_adapt_component.adapt_inbuf_free_list_inc, NULL, 0, NULL, NULL,
                            NULL);
        /* Set up next_recv_segs */
        con->next_recv_segs = (int32_t *) malloc(sizeof(opal_atomic_int32_t) * tree->tree_nextsize);
        ompi_coll_adapt_inbuf_t *temp_inbuf = (ompi_coll_adapt_inbuf_t *) opal_free_list_wait_st(
            &con->inbuf_list);
        con->distance = (char *) temp_inbuf->buff - lower_bound
                        - (char *) temp_inbuf; // address of inbuf->buff to address of inbuf
        OPAL_OUTPUT_VERBOSE(
            (30, mca_coll_adapt_component.adapt_output,
             "[%d]: distance %d, inbuf %p, inbuf->buff %p, inbuf->buff-lb %p, to_inbuf %p, "
             "inbuf_list %p\n",
        ...
        for (int32_t seg_index = 0; seg_index < min; seg_index++) {
            ...
            for (int32_t i = 0; i < tree->tree_nextsize; i++) {
                ...
                if (i == 0 && sbuf != MPI_IN_PLACE && root == rank) {
                    temp_recv_buf = (char *) rbuf
                                    + (ptrdiff_t) seg_index * (ptrdiff_t) segment_increment;
                } else {
                ...
                OPAL_OUTPUT_VERBOSE((30, mca_coll_adapt_component.adapt_output,
                                     "[%d]: In ireduce, create irecv for seg %d, peer %d, "
                                     "recv_count %d, inbuf %p tag %d\n",
                                     context->con->rank, context->seg_index, context->peer,
                                     recv_count, (void *) inbuf, con->ireduce_tag - seg_index));

                /* Create a recv request */
                ompi_request_t *recv_req;
                err = MCA_PML_CALL(irecv(temp_recv_buf, recv_count, dtype, tree->tree_next[i],
        ...
        for (int32_t seg_index = 0; seg_index < min; seg_index++) {
            ...
            OPAL_OUTPUT_VERBOSE(
                (30, mca_coll_adapt_component.adapt_output,
                 ...
                 context->con->rank, context->seg_index, context->peer, send_count,
                 con->ireduce_tag - context->seg_index));

            /* Create send request */
            ompi_request_t *send_req;
            err = MCA_PML_CALL(isend(context->buff, send_count, dtype, tree->tree_prev,
                                     con->ireduce_tag - context->seg_index, sendmode, comm,
                                     &send_req));
            if (MPI_SUCCESS != err) {
                return err;
            }
