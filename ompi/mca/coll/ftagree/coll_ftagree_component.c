...
const mca_coll_base_component_2_4_0_t mca_coll_ftagree_component = {
    ...
    {MCA_COLL_BASE_VERSION_2_4_0,
     ...
     "ftagree", OMPI_MAJOR_VERSION, OMPI_MINOR_VERSION, OMPI_RELEASE_VERSION,

     /* Component open and close functions */
     NULL, ftagree_close, NULL, ftagree_register},
...
static int ftagree_register(void)
{
    ...
    (void) mca_base_component_var_register(&mca_coll_ftagree_component.collm_version, "priority",
                                           "Priority of the ftagree coll component",
                                           MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_6,
                                           MCA_BASE_VAR_SCOPE_READONLY, &mca_coll_ftagree_priority);

    if (ompi_ftmpi_enabled)
        value = 1;
    else
        value = 0; /* NOFT: do not initialize ERA */
    (void) mca_base_component_var_register(
        &mca_coll_ftagree_component.collm_version, "agreement",
        "Agreement algorithm 0: Allreduce (NOT FAULT TOLERANT); 1: Early Returning Concensus "
        "(era); 2: Early Terminating Concensus (eta)",
        MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_6, MCA_BASE_VAR_SCOPE_READONLY, &value);
    switch (value) {
    case 0:
    ...
    (void)
        mca_base_component_var_register(&mca_coll_ftagree_component.collm_version, "era_topology",
                                        "ERA topology 1: binary tree; 2: star tree; 3: string tree",
                                        MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_6,
                                        MCA_BASE_VAR_SCOPE_READONLY,
                                        &mca_coll_ftagree_cur_era_topology);

    /* TODO: add an adaptative rebuilding strategy */
    mca_coll_ftagree_era_rebuild = 0; /* by default do not rebuild, master-worker application
                                         patterns can benefit greatly from rebuilding... */
    (void) mca_base_component_var_register(&mca_coll_ftagree_component.collm_version, "era_rebuild",
                                           "ERA rebuild/rebalance the tree in a first post-failure "
                                           "agreement 0: no rebalancing; 1: rebalance all the time",
                                           MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_6,
                                           MCA_BASE_VAR_SCOPE_READONLY,
    ...
    (void) mca_base_component_var_register(
        &mca_coll_ftagree_component.collm_version, "era_debug_fault_proba",
        "Inject faults with set probability at points of interest in the Agreement algorithm; For "
        "debugging only. (0.: no faults, 1.: always inject; 0.05 is typical)",
...
static void mca_coll_ftagree_module_construct(mca_coll_ftagree_module_t *module)
{
...
static void mca_coll_ftagree_module_destruct(mca_coll_ftagree_module_t *module)
{
...
OBJ_CLASS_INSTANCE(mca_coll_ftagree_module_t, mca_coll_base_module_t,
                   mca_coll_ftagree_module_construct, mca_coll_ftagree_module_destruct);
