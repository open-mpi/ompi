...
int mca_coll_ftagree_eta_intra(void *contrib, int dt_count, ompi_datatype_t *dt, ompi_op_t *op,
                               ompi_group_t **group, bool update_grp, ompi_communicator_t *comm,
                               mca_coll_base_module_t *module)
{
    ftagree_eta_agreement_msg_t *out, *in;
    size_t dt_size, msg_size;
    int *proc_status; /**< char would be enough, but we use the same area to build the group of dead
    ...
    reqs = (ompi_request_t **)
        calloc(2 * np, sizeof(ompi_request_t *)); /** < Need to calloc or set to MPI_REQUEST_NULL to
    ...
    OPAL_OUTPUT_VERBOSE((5, ompi_ftmpi_output_handle,
                         "%s ftagree:agreement (ETA) Starting Agreement with message size of %lu "
                         "bytes (%lu bytes for the agreement value)\n",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), msg_size, dt_count * dt_size));

    { /* ignore acked failures (add them later to the result) */
        ompi_group_t *ackedgrp = NULL;
        int npa;
        int *aranks, *cranks;
        ackedgrp = *group;
...
#define NEED_TO_RECV(_i)                               \
    (me != _i && (!(proc_status[_i] & STATUS_CRASHED)) \
     && (!(proc_status[_i] & STATUS_TOLD_ME_HE_KNOWS)))
#define NEED_TO_SEND(_i)                               \
    (me != _i && (!(proc_status[_i] & STATUS_CRASHED)) \
     && (!(proc_status[_i] & STATUS_KNOWS_I_KNOW)))
        ...
        for (i = 0; i < np; i++) {
            ...
            if ((double) rand() / (double) RAND_MAX < mca_coll_ftagree_rank_fault_proba) {
                OPAL_OUTPUT_VERBOSE((0, ompi_ftmpi_output_handle,
                                     "%s ftagree:agreement (ETA) INJECT: Killing myself just "
                                     "before posting message reception to/from %d\n",
                                     OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), i));
                raise(SIGKILL);
            }
#endif
            ...
            if (NEED_TO_RECV(i)) {
                ...
                MCA_PML_CALL(irecv(((char *) in) + (i * msg_size), msg_size, MPI_BYTE, i,
                                   FTAGREE_ETA_TAG_AGREEMENT, comm, &reqs[nr++]));
                proc_status[i] &= ~STATUS_RECV_COMPLETE;
                OPAL_OUTPUT_VERBOSE(
                    (100, ompi_ftmpi_output_handle,
            ...
            if (NEED_TO_SEND(i)) {
                ...
                MCA_PML_CALL(isend(out, msg_size, MPI_BYTE, i, FTAGREE_ETA_TAG_AGREEMENT,
                                   MCA_PML_BASE_SEND_STANDARD, comm, &reqs[nr++]));
            ...
            for (i = 0; i < np; i++) {
                if (!(proc_status[i] & STATUS_RECV_COMPLETE)) {
                    if ((rc == MPI_SUCCESS) || (MPI_SUCCESS == statuses[ri].MPI_ERROR)) {
                        ...
                        out->pf |= in[i].pf;
                        proc_status[i] |= ((in[i].knows * STATUS_TOLD_ME_HE_KNOWS)
                                           | STATUS_RECV_COMPLETE);
                        nbrecv++;

                        OPAL_OUTPUT_VERBOSE((100, ompi_ftmpi_output_handle,
                                             "%s ftagree:agreement (ETA) Request %d(%p) for recv "
                                             "of rank %d is completed.\n",
                                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ri,
                                             (void *) reqs[ri], i));
                    } else {
                        if ((MPI_ERR_PROC_FAILED == statuses[ri].MPI_ERROR)) {
                            ...
                            OPAL_OUTPUT_VERBOSE(
                                (10, ompi_ftmpi_output_handle,
                                 "%s ftagree:agreement (ETA) recv with rank %d failed on request "
                                 "at index %d(%p). Mark it as dead!",
                            ...
                            OPAL_OUTPUT_VERBOSE(
                                (100, ompi_ftmpi_output_handle,
                                 "%s ftagree:agreement (ETA) Request %d(%p) for recv of rank %d "
                ...
                if (!(proc_status[i] & STATUS_SEND_COMPLETE)) {
                    if ((rc == MPI_SUCCESS) || (MPI_SUCCESS == statuses[ri].MPI_ERROR)) {
                        assert(MPI_REQUEST_NULL == reqs[ri]);
                        proc_status[i] |= ((out->knows * STATUS_KNOWS_I_KNOW)
                                           | STATUS_SEND_COMPLETE);

                        OPAL_OUTPUT_VERBOSE((100, ompi_ftmpi_output_handle,
                                             "%s ftagree:agreement (ETA) Request %d(%p) for send "
                                             "of rank %d is completed.\n",
                                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ri,
                                             (void *) reqs[ri], i));
                    } else {
                        if ((MPI_ERR_PROC_FAILED == statuses[ri].MPI_ERROR)) {
                            ...
                            OPAL_OUTPUT_VERBOSE((10, ompi_ftmpi_output_handle,
                                                 "%s ftagree:agreement (ETA) send with rank %d "
                                                 "failed on Request %d(%p). Mark it as dead!",
                                                 OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), i, ri,
                                                 (void *) reqs[ri]));
                            out->pf = 1;
                            ...
                            OPAL_OUTPUT_VERBOSE(
                                (100, ompi_ftmpi_output_handle,
                                 "%s ftagree:agreement (ETA) Request %d(%p) for send of rank %d "
                                 "remains pending. Renaming it as Request %d\n",
                                 OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ri, (void *) reqs[ri], i, nr));
                            reqs[nr] = reqs[ri];
                            if (ri != nr)
...
#undef NEED_TO_RECV
        ...
        OPAL_OUTPUT_VERBOSE((50, ompi_ftmpi_output_handle,
                             ...
                             "%d, nbrecv = %d. out.knows = %d\n",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), round, nbcrashed, nbknow, nbrecv,
                             out->knows));
...
clean_and_exit:
    ...
    OPAL_OUTPUT_VERBOSE((5, ompi_ftmpi_output_handle,
                         "%s ftagree:agreement (ETA) return %d with 4 first bytes of result 0x%08x "
                         "and dead group with %d processes",
