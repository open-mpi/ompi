 ...
 */
int mca_coll_ftagree_init_query(bool enable_progress_threads, bool enable_mpi_threads)
{
 ...
 */
mca_coll_base_module_t *mca_coll_ftagree_comm_query(struct ompi_communicator_t *comm, int *priority)
{
    int size;
    ...
    *priority = mca_coll_ftagree_priority;
    ...
    ftagree_module->mccb_reqs = (ompi_request_t **) malloc(sizeof(ompi_request_t *)
                                                           * ftagree_module->mccb_num_reqs);

    ftagree_module->mccb_num_statuses = size * 2; /* x2 for alltoall */
    ftagree_module->mccb_statuses = (ompi_status_public_t *) malloc(
        sizeof(ompi_status_public_t) * ftagree_module->mccb_num_statuses);
 ...
 */
int mca_coll_ftagree_module_enable(mca_coll_base_module_t *module, struct ompi_communicator_t *comm)
{
    /* All done */
    return OMPI_SUCCESS;
}
