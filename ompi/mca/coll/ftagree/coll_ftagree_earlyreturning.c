                     ...
                     int         *ack_failed)
{
    ...
    assert(NULL != peer);
    /* Runs from a btl callback as well as from the agreement itself, so
     * it cannot block for a peer's connection info; one merely not
     * connected yet is left to the result-request path, which retries. */
