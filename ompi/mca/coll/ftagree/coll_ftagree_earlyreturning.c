...
typedef enum { MSG_UP = 1, MSG_DOWN, MSG_RESULT_REQUEST } era_msg_type_t;
...
typedef struct {
    union {
        ...
        struct {
            ...
            uint16_t contextid; /**< Although context ids are 32 bit long, only the lower 16 bits
                                   are used */
            uint32_t epoch;
        } fields;
        uint64_t uint64;
...
typedef struct {
    ...
    uint16_t max_aid; /**< at era_agreement_value_set_gcrange, forall this->header.min_aid <= id <=
                       * this->header.max_aid, exists p in era_passed_agreements, such that
                       *       (this->agreement_id.contextid == key(p).contextid &&
                       *        this->agreement_id.epoch     == key(p).epoch &&
                       *        key(p).agreementid           == id)
                       *   at era_decide, min_passed_aid is the max over all ranks, max_passed_aid
                       * is min over all ranks. */
...
#define ERA_VALUE_BYTES_COUNT(_h)                                                  \
    (((ompi_datatype_t *) opal_pointer_array_get_item(&ompi_datatype_f_to_c_table, \
                                                      (_h)->datatype))             \
         ->super.size                                                              \
     * (_h)->dt_count)
...
#define ERA_MSG_SIZE(_h, _n)                                                                \
    (sizeof(era_msg_header_t) + ERA_VALUE_BYTES_COUNT(_h) + (_h)->nb_new_dead * sizeof(int) \
     + (_n) * sizeof(int))
...
static void era_agreement_comm_specific_constructor(era_comm_agreement_specific_t *comm_specific)
{
    ...
    comm_specific->ags_status
        = AGS_TREE_DIRTY | AGS_AFR_DIRTY; /**< the AGS does not exist, so it needs to be created */
}
...
OBJ_CLASS_INSTANCE(era_comm_agreement_specific_t, opal_object_t,
                   era_agreement_comm_specific_constructor, era_agreement_comm_specific_destructor);
...
typedef struct {
    ...
    opal_list_t gathered_info; /**< The list of direct descendents that provided information (even
                                  partially) */
    opal_list_t
        waiting_res_from; /**< A list of ranks and status, from which we requested to "restart" the
    ...
    opal_list_t early_requesters; /**< Remember anybody who requests to receive the decision as
                                   *   the FD may be too slow to provide a symmetric view of the
                                   * world, and passing over the passed agreements after the
                                   * decision is taken is too slow */
...
static void era_agreement_info_constructor(era_agreement_info_t *agreement_info)
{
    ...
    OPAL_OUTPUT_VERBOSE((20, ompi_ftmpi_output_handle,
                         ...
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), (void *) agreement_info));
}
...
OBJ_CLASS_INSTANCE(era_agreement_info_t, opal_object_t, era_agreement_info_constructor,
...
OBJ_CLASS_INSTANCE(era_iagree_request_t, ompi_request_t, NULL, NULL);
...
static era_agreement_info_t *era_lookup_agreement_info(era_identifier_t agreement_id)
{
    ...
    if (opal_hash_table_get_value_uint64(&era_ongoing_agreements, agreement_id.ERAID_KEY, &value)
        == OPAL_SUCCESS) {
                                                       ...
                                                       era_value_header_t *header)
{
...
#if OPAL_ENABLE_DEBUG
    void *value;
    assert(opal_hash_table_get_value_uint64(&era_ongoing_agreements, agreement_id.ERAID_KEY, &value)
           != OPAL_SUCCESS);
#endif /* OPAL_ENABLE_DEBUG */
    ...
    /* ci->current_value->bytes will be filled up in combine_agreement_value, but let's allocate it
     * here */
...
#if OPAL_ENABLE_DEBUG
static void era_debug_print_group(int lvl, ompi_group_t *group, ompi_communicator_t *comm,
                                  char *info)
{
    int *gra = NULL;
    int *cra = NULL;
    ...
    snprintf(str + p, s - p, ")");
    OPAL_OUTPUT_VERBOSE((lvl, ompi_ftmpi_output_handle, "%s ftagree:agreement (ERA) %s: %s\n",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), info, str));
    free(str);
    if (NULL != cra)
        free(cra);
    if (NULL != gra)
...
#    define era_debug_print_group(g, c, i, h) \
        do {                                  \
        } while (0)
#endif /* OPAL_ENABLE_DEBUG */
...
static void era_update_return_value(era_agreement_info_t *ci, int nb_acked, int *acked)
{
    ...
    if (ci->current_value->header.ret == MPI_ERR_PROC_FAILED) {
        OPAL_OUTPUT_VERBOSE((3, ompi_ftmpi_output_handle,
                             "%s ftagree:agreement (ERA) compute local return value for Agreement "
                             "ID = (%d.%d).%d: already decided FAILED at line %s:%d\n",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
                             ci->agreement_id.ERAID_FIELDS.contextid,
                             ci->agreement_id.ERAID_FIELDS.epoch,
                             ci->agreement_id.ERAID_FIELDS.agreementid, __FILE__, __LINE__));
        return;
    }
    ...
    if (nb_acked >= 0) {
        ...
        if (ci->acked == NULL) {
            ...
            if (nb_acked != ci->nb_acked) {
                OPAL_OUTPUT_VERBOSE(
                    (3, ompi_ftmpi_output_handle,
                     "%s ftagree:agreement (ERA) compute local return value for Agreement ID = "
                     "(%d.%d).%d: decide FAILED because the acked arrays are of different size\n",
                     OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
                     ci->agreement_id.ERAID_FIELDS.epoch,
            ...
            for (r = 0; r < nb_acked; r++) {
                ...
                if (acked[r] != ci->acked[r]) {
                    OPAL_OUTPUT_VERBOSE((3, ompi_ftmpi_output_handle,
                                         "%s ftagree:agreement (ERA) compute local return value "
                                         "for Agreement ID = (%d.%d).%d: decide FAILED because the "
                                         "acked arrays do not hold the same elements\n",
                                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
                                         ci->agreement_id.ERAID_FIELDS.contextid,
                                         ci->agreement_id.ERAID_FIELDS.epoch,
                                         ci->agreement_id.ERAID_FIELDS.agreementid));
    ...
    if (ompi_group_size(afp) > ci->nb_acked) {
        ...
        ompi_group_intersection(tmp_sub_group, ci->comm->c_local_group, &ack_after_agreement_group);
        ...
        if (ompi_group_size(ack_after_agreement_group) != ci->nb_acked) {
            OPAL_OUTPUT_VERBOSE(
                (3, ompi_ftmpi_output_handle,
                 "%s ftagree:agreement (ERA) compute local return value for Agreement ID = "
                 "(%d.%d).%d: decide FAILED because new failures happened: |acked| = %d, |afp on "
                 "c_local_group| = %d\n",
                 OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
                 ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid,
                 ci->nb_acked, ompi_group_size(ack_after_agreement_group)));
            ci->current_value->header.ret = MPI_ERR_PROC_FAILED;
            OBJ_RELEASE(afp);
            goto cleanup;
        } else {
        ...
        OPAL_OUTPUT_VERBOSE(
            (3, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) compute local return value for Agreement ID = (%d.%d).%d: "
             "decide SUCCESS (2)\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
             ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid));
    }
    ...
    return; /** < We don't clean the list of acked processes if the result is SUCCESS as we need to
               communicate it */
...
static void era_merge_new_dead_list(era_agreement_info_t *ci, int nb_src, int *src)
{
    ...
    if (nb_merge > nb_dst) {
        ci->current_value->new_dead_array = (int *) realloc(ci->current_value->new_dead_array,
                                                            nb_merge * sizeof(int));
...
static void era_agreement_value_set_gcrange(era_identifier_t eid, era_value_t *era_value)
{
    ...
    for (rc = opal_hash_table_get_first_key_uint64(&era_passed_agreements, &key64, &value, &node);
         OPAL_SUCCESS == rc;
         rc = opal_hash_table_get_next_key_uint64(&era_passed_agreements, &key64, &value, node,
                                                  &node)) {
    ...
    OPAL_OUTPUT_VERBOSE((17, ompi_ftmpi_output_handle,
                         "%s ftagree:agreement (ERA) GC: agreement (%d.%d).%d: agreements %u to %u "
                         "have been locally acknowledged\n",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), eid.ERAID_FIELDS.contextid,
                         eid.ERAID_FIELDS.epoch, eid.ERAID_FIELDS.agreementid,
                         era_value->header.min_aid, era_value->header.max_aid));
}
...
static void era_update_new_dead_list(era_agreement_info_t *ci)
{
    ...
    OPAL_OUTPUT_VERBOSE(
        (30, ompi_ftmpi_output_handle,
         "%s ftagree:agreement (ERA) agreement (%d.%d).%d -- adding %d procs to the list of newly "
         "dead processes",
         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
         ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid, r));
...
static void era_ci_get_clean_ags_copy(era_agreement_info_t *ci)
{
    ...
    if (AGS(ci->comm)->ags_status & (AGS_TREE_DIRTY | AGS_AFR_DIRTY)) {
        ...
        if (AGS(ci->comm)->parent.obj_reference_count > 1) {
            ...
            old->ags_status
                = 0; /**< The old is clean w.r.t. the tree, and it does not need the AFR */
            OBJ_RELEASE(old);
        }
    }
    ...
    OPAL_OUTPUT_VERBOSE(
        (30, ompi_ftmpi_output_handle,
         "%s ftagree:agreement (ERA) Agreement (%d.%d).%d: the tree structure is %s, the AFR is "
         "%s\n",
         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
         ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid,
                                        ...
                                        ompi_group_t *acked_group)
{
    ...
    if (grp_size > 0) {
        if (grp_size != AGS(comm)->acked_ra_size) {
            ...
            ompi_group_translate_ranks(acked_group, grp_size, src_ra, comm->c_local_group,
                                       AGS(comm)->acked_ra);
            free(src_ra);
...
static void send_msg(ompi_communicator_t *comm, int dst, opal_process_name_t *proc_name,
                     era_identifier_t agreement_id, era_msg_type_t type, era_value_t *value,
                     int nb_up_msg, int *ack_failed);
...
static void era_combine_agreement_values(era_agreement_info_t *ni, era_value_t *value)
{
    ...
    if (ni->status & OP_NOT_DEFINED) {
        ...
        op = opal_pointer_array_get_item(ompi_op_f_to_c_table, ni->current_value->header.operand);
        assert(NULL != op);
        dt = opal_pointer_array_get_item(&ompi_datatype_f_to_c_table,
                                         ni->current_value->header.datatype);
        assert(NULL != dt);
        assert(dt->d_f_to_c_index == ni->current_value->header.datatype);
...
static int era_tree_check_node(era_tree_t *tree, int tree_size, int r, int display)
{
    ...
    if (display) {
        fprintf(stderr, "TC %s -- %d/%d (%d) -- Parent: %d\n", OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
                r, tree_size, tree[r].rank_in_comm, tree[r].parent);
    }

    if ((p = tree[r].parent) != r) {

        for (c = tree[p].first_child; c != r && c != tree_size; c = tree[c].next_sibling) {
            /** Nothing */
        }
        if (c != r) {
            tree_errors++;
            fprintf(stderr,
                    "TC %s -- %d/%d(%d): my parent (%d) should have me in one of its children\n",
                    OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), r, tree_size, tree[r].rank_in_comm,
                    tree[r].parent);
        }
    }

    if (tree[r].rank_in_comm < tree[tree[r].parent].rank_in_comm) {
        tree_errors++;
        fprintf(stderr,
                "TC %s -- %d/%d(%d): broken hiearchy as my parent is %d in the communicator\n",
                OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), r, tree_size, tree[r].rank_in_comm,
                tree[tree[r].parent].rank_in_comm);
    }

    for (c = tree[r].first_child; c != tree_size; c = tree[c].next_sibling) {
        nb++;
        if (display) {
            fprintf(stderr, "TC %s -- %d/%d(%d) -- Child %d: %d\n",
                    OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), r, tree_size, tree[r].rank_in_comm, nb, c);
        }
        if (tree[c].parent != r) {
            tree_errors++;
            fprintf(stderr,
                    "TC %s -- %d/%d(%d): Each of my children should have me as their parent. %d "
                    "has %d as parent.\n",
                    OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), r, tree_size, tree[r].rank_in_comm, c,
                    tree[c].parent);
        }
        if (nb > tree_size) {
            tree_errors++;
            fprintf(stderr,
                    "TC %s -- %d/%d(%d): There is a cycle somewhere: I counted %d children, which "
                    "is more than the tree size.\n",
...
{
    snprintf(era_tree_buffer + strlen(era_tree_buffer),
             ERA_TREE_BUFFER_SIZE - strlen(era_tree_buffer), "(%d", tree[node].rank_in_comm);
    if (tree[node].first_child != tree_size) {
        snprintf(era_tree_buffer + strlen(era_tree_buffer),
                 ERA_TREE_BUFFER_SIZE - strlen(era_tree_buffer), ", ");
...
static void era_build_tree_structure(era_agreement_info_t *ci)
{
    ...
    for (m = 0; m < AGS(ci->comm)->tree_size; m++) { /**< O(comm_size) comp. overhead */
        ...
        while (next_dead_idx < AGS(ci->comm)->afr_size
               && (m + offset) == AGS(ci->comm)->agreed_failed_ranks[next_dead_idx]) {
            next_dead_idx++;
            offset++;
        }
    ...
    if (ompi_comm_rank(ci->comm) == 0) {
        OPAL_OUTPUT_VERBOSE(
            (4, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) Agreement (%d.%d).%d: re-built the tree structure with "
             "size %d: %s\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
             ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid,
             AGS(ci->comm)->tree_size, era_debug_tree(ci->ags->tree, ci->ags->tree_size)));
    }
...
static int era_tree_rank_from_comm_rank(era_agreement_info_t *ci, int r_in_comm)
{
    ...
    while (ci->ags->tree[r_in_tree].rank_in_comm != r_in_comm) {
        ...
        assert(ci->ags->tree[r_in_tree].rank_in_comm == -1
               || ci->ags->tree[r_in_tree].rank_in_comm > r_in_comm);
#endif /* OPAL_ENABLE_DEBUG */
        assert(r_in_tree > 0);
        r_in_tree--;
    }
...
static void era_tree_remove_node(era_agreement_info_t *ci, int r_in_tree)
{
    ...
    if (p != r_in_tree) {
        ...
        if (ci->ags->tree[p].first_child == r_in_tree) {
            ...
            for (t = ci->ags->tree[p].first_child; ci->ags->tree[t].next_sibling != r_in_tree;
                 t = ci->ags->tree[t].next_sibling) {
                assert(
                    t != ci->ags->tree_size); /** r_in_tree should still be chained to its parent */
            }
            if (s == ci->ags->tree_size) {
        ...
        last_sib = -1;
        prev_root = -1;
        for (p = ci->ags->tree[r_in_tree].first_child; p != ci->ags->tree_size;
...
#if OPAL_ENABLE_DEBUG
    {
        ...
        OPAL_OUTPUT_VERBOSE(
            (30, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) Agreement (%d.%d).%d: removed node %d (at position %d) "
             "from the tree structure: %s\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
             ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid, prank,
             r_in_tree, era_debug_tree(ci->ags->tree, ci->ags->tree_size)));
    }
#endif /* OPAL_ENABLE_DEBUG */
}
...
static int era_next_child(era_agreement_info_t *ci, int prev_child_in_comm)
{
    ...
    prev_child_in_tree = prev_child_in_comm == -1
                             ? -1
                             : era_tree_rank_from_comm_rank(ci, prev_child_in_comm);
...
static void era_collect_passed_agreements(era_identifier_t agreement_id, uint16_t min_aid,
                                          uint16_t max_aid)
{
    void *value;
    int r;
    ...
    OPAL_OUTPUT_VERBOSE((17, ompi_ftmpi_output_handle,
                         "%s ftagree:agreement (ERA) Agreement (%d.%d).%d: GC: Agreements %u to %u "
                         "have been acknowledged by all and can be collected\n",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), agreement_id.ERAID_FIELDS.contextid,
                         agreement_id.ERAID_FIELDS.epoch, agreement_id.ERAID_FIELDS.agreementid,
                         min_aid, max_aid));
    if (min_aid <= max_aid) {
        era_identifier_t pid;
        ...
        for (r = min_aid; r <= max_aid; r++) {
            ...
            if (opal_hash_table_get_value_uint64(&era_passed_agreements, pid.ERAID_KEY, &value)
                == OMPI_SUCCESS) {
                era_value_t *av = (era_value_t *) value;
                opal_hash_table_remove_value_uint64(&era_passed_agreements, pid.ERAID_KEY);
                OBJ_RELEASE(av);
                OPAL_OUTPUT_VERBOSE(
                    (17, ompi_ftmpi_output_handle,
                     "%s ftagree:agreement (ERA) Agreement (%d.%d).%d: GC: collect agreement "
                     "(%d.%d).%d\n",
                     OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), agreement_id.ERAID_FIELDS.contextid,
                     agreement_id.ERAID_FIELDS.epoch, agreement_id.ERAID_FIELDS.agreementid,
                     pid.ERAID_FIELDS.contextid, pid.ERAID_FIELDS.epoch,
...
static void era_decide(era_value_t *decided_value, era_agreement_info_t *ci)
{
...
#if OPAL_ENABLE_DEBUG
    r = era_parent(ci);
    if (opal_hash_table_get_value_uint64(&era_passed_agreements, ci->agreement_id.ERAID_KEY, &value)
        == OMPI_SUCCESS) {
        /**
    ...
    OPAL_OUTPUT_VERBOSE(
        ((ci->comm->c_my_rank == r) ? 2 : 10, ompi_ftmpi_output_handle,
         ...
         decided_value->header.ret, decided_value->header.nb_new_dead,
         ci->agreement_id.ERAID_FIELDS.contextid, ci->agreement_id.ERAID_FIELDS.epoch,
         ci->agreement_id.ERAID_FIELDS.agreementid));

    opal_hash_table_set_value_uint64(&era_passed_agreements, ci->agreement_id.ERAID_KEY,
                                     decided_value);
    ...
    if (decided_value->header.nb_new_dead != 0) {
        OPAL_OUTPUT_VERBOSE(
            (30, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) decide %08x.%d.%d on agreement (%d.%d).%d: adding up to "
             "%d processes to the list of agreed deaths\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
             (0 != ERA_VALUE_BYTES_COUNT(&decided_value->header)) ? *(int *) decided_value->bytes
                                                                  : 0,
             decided_value->header.ret, decided_value->header.nb_new_dead,
             ci->agreement_id.ERAID_FIELDS.contextid, ci->agreement_id.ERAID_FIELDS.epoch,
             ci->agreement_id.ERAID_FIELDS.agreementid, decided_value->header.nb_new_dead));
        ...
        AGS(comm)->agreed_failed_ranks = (int *) realloc(AGS(comm)->agreed_failed_ranks,
                                                         dead_size * sizeof(int));
    ...
    OPAL_OUTPUT_VERBOSE(
        (10, ompi_ftmpi_output_handle,
         "%s ftagree:agreement (ERA) decide %08x.%d.%d.. on agreement (%d.%d).%d: group of agreed "
         "deaths is of size %d\n",
         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
         (0 != ERA_VALUE_BYTES_COUNT(&decided_value->header)) ? *(int *) decided_value->bytes : 0,
         decided_value->header.ret, decided_value->header.nb_new_dead,
         ci->agreement_id.ERAID_FIELDS.contextid, ci->agreement_id.ERAID_FIELDS.epoch,
         ci->agreement_id.ERAID_FIELDS.agreementid, AGS(comm)->afr_size));
    ...
    era_collect_passed_agreements(ci->agreement_id, decided_value->header.min_aid,
    ...
    if (ci->req) {
        OPAL_OUTPUT_VERBOSE(
            (50, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) calling complete for Agreement ID = (%d.%d).%d, current "
             "status = %s\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
             ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid,
...
static void era_check_status(era_agreement_info_t *ci)
{
    ...
    OPAL_OUTPUT_VERBOSE(
        (10, ompi_ftmpi_output_handle,
         "%s ftagree:agreement (ERA) check_status for Agreement ID = (%d.%d).%d, current status = "
         "%s\n",
         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
         ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid,
         era_status_to_string(ci->status)));

    assert(ci->status != BROADCASTING && ci->status != COMPLETED);

    if (ci->status == NOT_CONTRIBUTED) {
        /* Well, I haven't contributed to this agreement yet, and you'll not make a decision without
         * me */
        return;
    }
    ...
    if (ci->status == GATHERING) {
        ...
        (void) era_parent(ci); /* Maybe I'm becoming the root, need to recheck my children. TODO:
                                  only rebuild the necessary part */
        r = -1;
        while ((r = era_next_child(ci, r)) < ompi_comm_size(ci->comm)) {
            OPAL_OUTPUT_VERBOSE((30, ompi_ftmpi_output_handle,
                                 "%s ftagree:agreement (ERA) check_status for Agreement ID = "
                                 "(%d.%d).%d, child %d is supposed to contribute\n",
                                 OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
                                 ci->agreement_id.ERAID_FIELDS.contextid,
                                 ci->agreement_id.ERAID_FIELDS.epoch,
                                 ci->agreement_id.ERAID_FIELDS.agreementid, r));
            for (rl = (era_rank_item_t *) opal_list_get_first(&ci->gathered_info);
                ...
                if (rl->rank == r) {
                    OPAL_OUTPUT_VERBOSE((20, ompi_ftmpi_output_handle,
                                         "%s ftagree:agreement (ERA) check_status for Agreement ID "
                                         "= (%d.%d).%d, child %d has sent its message\n",
                                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
                                         ci->agreement_id.ERAID_FIELDS.contextid,
                                         ci->agreement_id.ERAID_FIELDS.epoch,
                                         ci->agreement_id.ERAID_FIELDS.agreementid, r));
            ...
            if (rl == (era_rank_item_t *) opal_list_get_end(&ci->gathered_info)) {
                OPAL_OUTPUT_VERBOSE((20, ompi_ftmpi_output_handle,
                                     "%s ftagree:agreement (ERA) check_status for Agreement ID = "
                                     "(%d.%d).%d, some children have not contributed\n",
                                     OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
                                     ci->agreement_id.ERAID_FIELDS.contextid,
                                     ci->agreement_id.ERAID_FIELDS.epoch,
                                     ci->agreement_id.ERAID_FIELDS.agreementid));
        ...
        if (ci->comm->c_my_rank == (r = era_parent(ci))) {
            OPAL_OUTPUT_VERBOSE(
                (20, ompi_ftmpi_output_handle,
                 "%s ftagree:agreement (ERA) check_status for Agreement ID = (%d.%d).%d, all "
                 "children of root have contributed\n",
                 OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
                 ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid));
            ...
            OPAL_OUTPUT_VERBOSE(
                (20, ompi_ftmpi_output_handle,
                 "%s ftagree:agreement (ERA) check_status for Agreement ID = (%d.%d).%d, all "
                 "children of non-root have contributed\n",
                 OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
                 ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid));

            /* Let's forward up and wait for the DOWN messages */
            ci->waiting_down_from = r;
            send_msg(ci->comm, r, NULL, ci->agreement_id, MSG_UP, ci->current_value, ci->nb_acked,
                     ci->acked);
...
static void restart_agreement_from_me(era_agreement_info_t *ci)
{
    ...
    while ((r = era_next_child(ci, r)) != ompi_comm_size(ci->comm)) {
        ...
        send_msg(ci->comm, r, NULL, ci->agreement_id, MSG_RESULT_REQUEST, ci->current_value, 0,
                 NULL);
    }
...
static void era_mark_process_failed(era_agreement_info_t *ci, int rank)
{
    ...
    if (ci->status > NOT_CONTRIBUTED) {
        ...
        OPAL_OUTPUT_VERBOSE(
            (30, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) Handling failure of process %d: Agreement (%d.%d).%d will "
             "have to return ERR_PROC_FAILED.\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), rank, ci->agreement_id.ERAID_FIELDS.contextid,
             ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid));
        ci->current_value->header.ret = MPI_ERR_PROC_FAILED;
    }
    if (ci->status == BROADCASTING) {
        OPAL_OUTPUT_VERBOSE(
            (20, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) Handling failure of process %d: Agreement (%d.%d).%d is "
             "in the BROADCASTING state.\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), rank, ci->agreement_id.ERAID_FIELDS.contextid,
             ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid));
        ...
        if (rank == ci->waiting_down_from) {
            ...
            if (r == ci->comm->c_my_rank) {
                OPAL_OUTPUT_VERBOSE((20, ompi_ftmpi_output_handle,
                                     "%s ftagree:agreement (ERA) Handling failure of process %d: "
                                     "Restarting Agreement (%d.%d).%d as I am the new root.\n",
                                     OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), rank,
                                     ci->agreement_id.ERAID_FIELDS.contextid,
                                     ci->agreement_id.ERAID_FIELDS.epoch,
                                     ci->agreement_id.ERAID_FIELDS.agreementid));
                ...
                OPAL_OUTPUT_VERBOSE(
                    (20, ompi_ftmpi_output_handle,
                     "%s ftagree:agreement (ERA) Handling failure of process %d: My parent changed "
                     "to %d for Agreement (%d.%d).%d, sending the UP message to it\n",
                     OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), rank, r,
                     ci->agreement_id.ERAID_FIELDS.contextid, ci->agreement_id.ERAID_FIELDS.epoch,
                     ci->agreement_id.ERAID_FIELDS.agreementid));
                ci->waiting_down_from = r;
        ...
        OPAL_OUTPUT_VERBOSE(
            (20, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) Retaining agreement info for (%d.%d).%d while resolving "
             "failure during agreement.\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci->agreement_id.ERAID_FIELDS.contextid,
             ci->agreement_id.ERAID_FIELDS.epoch, ci->agreement_id.ERAID_FIELDS.agreementid));
        OBJ_RETAIN(ci);
        ...
        for (rl = (era_rank_item_t *) opal_list_get_first(&ci->waiting_res_from);
            ...
            if (rl->rank == rank) {
                OPAL_OUTPUT_VERBOSE(
                    (20, ompi_ftmpi_output_handle,
                     "%s ftagree:agreement (ERA) Handling failure of process %d: I was waiting for "
                     "the contribution of that process for Agreement (%d.%d).%d.\n",
                     OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), rank,
                     ci->agreement_id.ERAID_FIELDS.contextid, ci->agreement_id.ERAID_FIELDS.epoch,
                     ci->agreement_id.ERAID_FIELDS.agreementid));
                ...
                while ((r = era_next_child(ci, r)) != ompi_comm_size(ci->comm)) {
                    ...
                    if (rl == (era_rank_item_t *) opal_list_get_end(&ci->waiting_res_from)) {
                        OPAL_OUTPUT_VERBOSE(
                            (20, ompi_ftmpi_output_handle,
                             "%s ftagree:agreement (ERA) Handling failure of process %d: "
                             "Requesting contribution of process %d for Agreement (%d.%d).%d.\n",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), rank, r,
                             ci->agreement_id.ERAID_FIELDS.contextid,
                             ci->agreement_id.ERAID_FIELDS.epoch,
                             ci->agreement_id.ERAID_FIELDS.agreementid));
                        ...
                        send_msg(ci->comm, r, NULL, ci->agreement_id, MSG_RESULT_REQUEST,
                                 ci->current_value, 0, NULL);
                    }
                }
                break;
            }
...
static void fragment_sent_cb(struct mca_btl_base_module_t *module,
                             ...
                             struct mca_btl_base_descriptor_t *descriptor, int status)
{
    (void) module;
    (void) endpoint;
    (void) descriptor;
...
static void send_msg(ompi_communicator_t *comm, int dst, opal_process_name_t *proc_name,
                     era_identifier_t agreement_id, era_msg_type_t type, era_value_t *value,
                     int nb_ack_failed, int *ack_failed)
{
    ...
    if ((double) rand() / (double) RAND_MAX < mca_coll_ftagree_rank_fault_proba) {
        OPAL_OUTPUT_VERBOSE(
            (0, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) INJECT: Killing myself just before sending message "
             "[(%d.%d).%d, %s, %08x.%d.%d..] to %d/%s\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), agreement_id.ERAID_FIELDS.contextid,
             agreement_id.ERAID_FIELDS.epoch, agreement_id.ERAID_FIELDS.agreementid,
             era_msg_type_to_string(type),
             (0 != ERA_VALUE_BYTES_COUNT(&value->header)) ? *(int *) value->bytes : 0,
             value->header.ret, value->header.nb_new_dead, dst,
    ...
    if (MSG_UP == type) {
        OPAL_OUTPUT_VERBOSE(
            (5, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) send message [(%d.%d).%d, %s, %08x.%d.%d/%d] to %d/%s\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), agreement_id.ERAID_FIELDS.contextid,
             agreement_id.ERAID_FIELDS.epoch, agreement_id.ERAID_FIELDS.agreementid,
             era_msg_type_to_string(type),
             (0 != ERA_VALUE_BYTES_COUNT(&value->header)) ? *(int *) value->bytes : 0,
             value->header.ret, value->header.nb_new_dead, nb_ack_failed, dst,
             NULL != proc_name ? OMPI_NAME_PRINT(proc_name) : "(null)"));
    } else {
        OPAL_OUTPUT_VERBOSE(
            (5, ompi_ftmpi_output_handle,
    ...
    OPAL_OUTPUT_VERBOSE(
        (30, ompi_ftmpi_output_handle,
         "%s ftagree:agreement (ERA) send message [(%d.%d).%d, %s, %08x.%d.%d/%d..] to %d/%s: send "
         "%d bytes through iov (nb = %lu, lens = %lu,%lu,%lu,%lu)\n",
         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), agreement_id.ERAID_FIELDS.contextid,
         agreement_id.ERAID_FIELDS.epoch, agreement_id.ERAID_FIELDS.agreementid,
         era_msg_type_to_string(type),
         (0 != ERA_VALUE_BYTES_COUNT(&value->header)) ? *(int *) value->bytes : 0,
         value->header.ret, value->header.nb_new_dead, msg_header.nb_ack, dst,
         NULL != proc_name ? OMPI_NAME_PRINT(proc_name) : "(null)", to_send, niov, iov[0].iov_len,
...
#if OPAL_ENABLE_DEBUG
    {
        ...
        do {
            ...
            w += snprintf(strbytes + strlen(strbytes), 256 - strlen(strbytes), "%02x",
                          ((uint8_t *) iov[i].iov_base)[b]);
            b++;
        } while (w < 256);
        ...
        OPAL_OUTPUT_VERBOSE(
            (30, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) send message [(%d.%d).%d, %s, %08x.%d.%d/%d..] to %d/%s: "
             "%d bytes including header = %s\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), agreement_id.ERAID_FIELDS.contextid,
             agreement_id.ERAID_FIELDS.epoch, agreement_id.ERAID_FIELDS.agreementid,
             era_msg_type_to_string(type),
             (0 != ERA_VALUE_BYTES_COUNT(&value->header)) ? *(int *) value->bytes : 0,
             value->header.ret, value->header.nb_new_dead, msg_header.nb_ack, dst,
             NULL != proc_name ? OMPI_NAME_PRINT(proc_name) : "(null)", to_send, strbytes));
    }
#endif /* OPAL_ENABLE_DEBUG */
    ...
    while (sent < to_send) {
        ...
        des = btl->btl_alloc(btl, btl_endpoint, MCA_BTL_NO_ORDER,
                             sizeof(era_frag_t) + to_send - sent,
...
static void result_request(era_msg_header_t *msg_header)
{
    ...
    OPAL_OUTPUT_VERBOSE((5, ompi_ftmpi_output_handle,
                         "%s ftagree:agreement (ERA) Received RESULT_REQUEST Message: Agreement ID "
                         "= (%d.%d).%d, sender: %d/%s\n",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
                         ...
                         msg_header->src_comm_rank, OMPI_NAME_PRINT(&msg_header->src_proc_name)));

    if (opal_hash_table_get_value_uint64(&era_passed_agreements, msg_header->agreement_id.ERAID_KEY,
                                         &value)
        == OPAL_SUCCESS) {
        old_agreement_value = (era_value_t *) value;
        send_msg(NULL, msg_header->src_comm_rank, &msg_header->src_proc_name,
                 msg_header->agreement_id, MSG_DOWN, old_agreement_value, 0, NULL);
        return;
    }
    ...
    if (NULL != ci && ci->status == BROADCASTING) {
        ...
        r = era_parent(ci);
        if (r == ci->comm->c_my_rank) {
            /** OK, weird case: a guy sent me that request, but died before I answered, and I
             * receive it now... */
        ...
        if (msg_header->agreement_value_header.dt_count == 0) {
            ...
            send_msg(NULL, msg_header->src_comm_rank, &msg_header->src_proc_name,
                     msg_header->agreement_id, MSG_DOWN, &success_value, 0, NULL);
...
static void msg_up(era_msg_header_t *msg_header, uint8_t *bytes, int *new_dead, int *ack_failed)
{
    ...
    OPAL_OUTPUT_VERBOSE(
        (5, ompi_ftmpi_output_handle,
         "%s ftagree:agreement (ERA) Received UP Message: Agreement ID = (%d.%d).%d, sender: "
         "%d/%s, msg value: %08x.%d.%d/%d\n",
         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), msg_header->agreement_id.ERAID_FIELDS.contextid,
         msg_header->agreement_id.ERAID_FIELDS.epoch,
         msg_header->agreement_id.ERAID_FIELDS.agreementid, msg_header->src_comm_rank,
         OMPI_NAME_PRINT(&msg_header->src_proc_name),
         (0 != ERA_VALUE_BYTES_COUNT(&msg_header->agreement_value_header)) ? *(int *) bytes : 0,
         msg_header->agreement_value_header.ret, msg_header->agreement_value_header.nb_new_dead,
         msg_header->nb_ack));
    ...
    if (opal_hash_table_get_value_uint64(&era_passed_agreements, msg_header->agreement_id.ERAID_KEY,
        ...
        send_msg(NULL, msg_header->src_comm_rank, &msg_header->src_proc_name,
                 msg_header->agreement_id, MSG_DOWN, av, 0, NULL);
        return;
    }
    ...
    OPAL_OUTPUT_VERBOSE((20, ompi_ftmpi_output_handle,
                         ...
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), ci == NULL ? "unknown" : "known"));

    if (NULL == ci) {
        ci = era_create_agreement_info(msg_header->agreement_id,
                                       &msg_header->agreement_value_header);
    ...
    if (ci->status == BROADCASTING) {
         ...
         */
        OPAL_OUTPUT_VERBOSE((20, ompi_ftmpi_output_handle,
                             "%s ftagree:agreement (ERA) Managing UP Message -- Already in "
                             "BROADCASTING state: ignoring message, adding %d in the requesters\n",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), msg_header->src_comm_rank));

        /** We could receive multiple messages from msg_header->src_comm_rank, because the messages
         * are split */
    ...
    OPAL_OUTPUT_VERBOSE((20, ompi_ftmpi_output_handle,
                         "%s ftagree:agreement (ERA) Received UP Message: adding %d in list of "
                         "people that contributed\n",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), rank_item->rank));
...
static void msg_down(era_msg_header_t *msg_header, uint8_t *bytes, int *new_dead)
{
    ...
    OPAL_OUTPUT_VERBOSE(
        (5, ompi_ftmpi_output_handle,
         "%s ftagree:agreement (ERA) Received DOWN Message: Agreement ID = (%d.%d).%d, sender: "
         "%d/%s, msg value: %08x.%d.\n",
         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), msg_header->agreement_id.ERAID_FIELDS.contextid,
         msg_header->agreement_id.ERAID_FIELDS.epoch,
         msg_header->agreement_id.ERAID_FIELDS.agreementid, msg_header->src_comm_rank,
    ...
    if (av->header.nb_new_dead > 0) {
        ...
        memcpy(av->new_dead_array, new_dead, av->header.nb_new_dead * sizeof(int));
    }
    era_decide(av, ci);
                      ...
                      const mca_btl_base_receive_descriptor_t *descriptor)
{
    ...
    if (frag->msg_len == frag->frag_len) {
        ...
        if (opal_hash_table_get_value_uint64(&era_incomplete_messages, src_hash, &value)
            == OMPI_SUCCESS) {
            msg_table = (opal_hash_table_t *) value;
        } else {
            msg_table = OBJ_NEW(opal_hash_table_t);
            opal_hash_table_init(
                msg_table, 3 /* This should be very small: few messages should fly in parallel */);
            opal_hash_table_set_value_uint64(&era_incomplete_messages, src_hash,
                                             (void *) msg_table);
        }
        ...
        memcpy(incomplete_msg->bytes + frag->frag_offset, frag->bytes, frag->frag_len);
    ...
    if ((double) rand() / (double) RAND_MAX < mca_coll_ftagree_rank_fault_proba) {
        OPAL_OUTPUT_VERBOSE(
            (0, ompi_ftmpi_output_handle,
             "%s ftagree:agreement (ERA) INJECT: Killing myself just before receiving message "
             "[(%d.%d).%d, %d, %08x.%d.%d...] from %d/%s\n",
             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), msg_header->agreement_id.ERAID_FIELDS.contextid,
             msg_header->agreement_id.ERAID_FIELDS.epoch,
             msg_header->agreement_id.ERAID_FIELDS.agreementid, msg_header->msg_type,
             (0 != ERA_VALUE_BYTES_COUNT(&msg_header->agreement_value_header))
                 ? *(int *) value_bytes
                 : 0,
             msg_header->agreement_value_header.ret, msg_header->agreement_value_header.nb_new_dead,
             msg_header->src_comm_rank, OMPI_NAME_PRINT(&msg_header->src_proc_name)));
    ...
    if (opal_mutex_trylock(&era_mutex)) {
        ...
        event->value_bytes = (void *) ((intptr_t) &event->msg + (intptr_t) value_bytes
                                       - (intptr_t) msg_header);
        event->new_dead = (void *) ((intptr_t) &event->msg + (intptr_t) new_dead
                                    - (intptr_t) msg_header);
        event->ack_failed = (void *) ((intptr_t) &event->msg + (intptr_t) ack_failed
                                      - (intptr_t) msg_header);
...
static void era_on_comm_rank_failure(ompi_communicator_t *comm, int rank, bool remote)
{
    ...
    OPAL_OUTPUT_VERBOSE((4, ompi_ftmpi_output_handle,
                         ...
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), rank, comm->c_contextid,
                         comm->c_epoch));
    ...
    if (opal_hash_table_get_first_key_uint64(&era_ongoing_agreements, &key64, &value, &node)
        == OPAL_SUCCESS) {
        do {
            cid.ERAID_KEY = key64;
            ...
            rc = opal_hash_table_get_next_key_uint64(&era_ongoing_agreements, &key64, &next_value,
                                                     node, &node);
            ...
            if (cid.ERAID_FIELDS.contextid == comm->c_contextid
                ...
                OPAL_OUTPUT_VERBOSE((6, ompi_ftmpi_output_handle,
                                     "%s ftagree:agreement (ERA) Agreement ID (%d.%d).%d, rank %d "
                                     "died while doing the agreement\n",
                                     OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
                                     ci->agreement_id.ERAID_FIELDS.contextid,
                                     ci->agreement_id.ERAID_FIELDS.epoch,
                                     ci->agreement_id.ERAID_FIELDS.agreementid, rank));
...
int mca_coll_ftagree_era_init(void)
{
    ...
    switch (mca_coll_ftagree_cur_era_topology < 0 ? -mca_coll_ftagree_cur_era_topology
    ...
    opal_free_list_init(&era_iagree_requests, sizeof(era_iagree_request_t), opal_cache_line_size,
                        OBJ_CLASS(era_iagree_request_t), 0, opal_cache_line_size,
                        /* initial number of elements to allocate */ 0,
                        /* maximum number of elements */ INT_MAX,
                        /* increment */ 1, NULL, 0, /* mpool */
                        NULL,                       /* unused */
                        NULL, NULL /* elem_init */);
     ...
     * good performance.
     */
    opal_hash_table_init(&era_passed_agreements,
                         32 /* We have GC, so small storage should be fine */);
    ...
    opal_hash_table_init(&era_incomplete_messages,
                         65536 /* Big Storage. Should be related to the universe size */);

    ompi_stacked_rank_failure_callback_fct = ompi_rank_failure_cbfunc;
    ompi_rank_failure_cbfunc = era_on_comm_rank_failure;

    OPAL_OUTPUT_VERBOSE((10, ompi_ftmpi_output_handle, "%s ftagree:agreement (ERA) Initialized\n",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME)));
...
int mca_coll_ftagree_era_finalize(void)
{
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_ftmpi_output_handle, "%s ftagree:agreement (ERA) Finalizing\n",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME)));
    OPAL_OUTPUT_VERBOSE((7, ompi_ftmpi_output_handle,
                         "%s ftagree:agreement (ERA) GC: %lu passed agreements remain in the "
                         "passed agreements hash table\n",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
    ...
    for (rc = opal_hash_table_get_first_key_uint64(&era_passed_agreements, &key64, &value, &node);
        ...
        assert(0 != pid.ERAID_FIELDS.agreementid);
        OPAL_OUTPUT_VERBOSE((7, ompi_ftmpi_output_handle,
                             "%s ftagree:agreement (ERA) GC: agreement (%d.%d).%d belongs to the "
                             "passed agreements hash table\n",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), pid.ERAID_FIELDS.contextid,
                             pid.ERAID_FIELDS.epoch, pid.ERAID_FIELDS.agreementid));
#endif /* OPAL_ENABLE_DEBUG */
        av = (era_value_t *) value;
        OBJ_RELEASE(av);
    }
    ...
    for (rc = opal_hash_table_get_first_key_uint64(&era_ongoing_agreements, &key64, &value, &node);
         OPAL_SUCCESS == rc;
         rc = opal_hash_table_get_next_key_uint64(&era_ongoing_agreements, &key64, &value, node,
                                                  &node)) {
        un_agreement = (era_agreement_info_t *) value;
        opal_output(0,
                    "%s ftagree:agreement (ERA) ERRONEOUS: Agreement ID (%d.%d).%d was started by "
                    "some processor, but I never completed to it\n",
                    OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
    ...
    for (rc = opal_hash_table_get_first_key_uint64(&era_incomplete_messages, &key64, &value, &node);
         OPAL_SUCCESS == rc;
         rc = opal_hash_table_get_next_key_uint64(&era_incomplete_messages, &key64, &value, node,
                                                  &node)) {
        uint64_t key64_2;
        void *value_2, *node_2;
        int rc2;
        ...
        for (rc2 = opal_hash_table_get_first_key_uint64(msg_table, &key64_2, &value_2, &node_2);
             OPAL_SUCCESS == rc2;
             rc2 = opal_hash_table_get_next_key_uint64(msg_table, &key64_2, &value_2, node_2,
                                                       &node_2)) {
...
static int mca_coll_ftagree_era_prepare_agreement(ompi_communicator_t *comm, ompi_group_t *group,
                                                  ompi_op_t *op, ompi_datatype_t *dt, int dt_count,
                                                  void *contrib, mca_coll_base_module_t *module,
                                                  ...
                                                  era_agreement_info_t **pci)
{
    ...
    if ((double) rand() / (double) RAND_MAX < mca_coll_ftagree_rank_fault_proba) {
        OPAL_OUTPUT_VERBOSE((0, ompi_ftmpi_output_handle,
                             "%s ftagree:agreement (ERA) INJECT: Killing myself just before "
                             "entering the agreement (%d.%d).%d\n",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME),
                             agreement_id.ERAID_FIELDS.contextid, agreement_id.ERAID_FIELDS.epoch,
    ...
    if (opal_hash_table_get_value_uint64(&era_passed_agreements, agreement_id.ERAID_KEY, &value)
        == OMPI_SUCCESS) {
        opal_output(0,
                    "*** WARNING *** %s ftagree:agreement (ERA) removing old agreement (%d.%d).%d "
                    "from history, due to cycling of identifiers\n",
                    OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), agreement_id.ERAID_FIELDS.contextid,
                    agreement_id.ERAID_FIELDS.epoch, agreement_id.ERAID_FIELDS.agreementid);
...
static int mca_coll_ftagree_era_complete_agreement(era_identifier_t agreement_id, void *contrib,
                                                   ompi_group_t **group)
{
    era_value_t *av;
    int ret;
    ...
    ret = opal_hash_table_get_value_uint64(&era_passed_agreements, agreement_id.ERAID_KEY, &value);
    ...
    for (i = 0; i < AGS(comm)->afr_size; i++) {
        ompi_proc_t *proc = ompi_group_get_proc_ptr(comm->c_local_group,
                                                    AGS(comm)->agreed_failed_ranks[i], true);
    ...
    if (NULL != group) {
        OBJ_RELEASE(*group);
        ompi_group_incl(comm->c_local_group, AGS(comm)->afr_size, AGS(comm)->agreed_failed_ranks,
                        group);
        era_debug_print_group(3, *group, comm, "After Agreement");
    }

    OPAL_OUTPUT_VERBOSE((3, ompi_ftmpi_output_handle,
                         "%s ftagree:agreement (ERA) Leaving Agreement ID = (%d.%d).%d with ret = "
                         "%d, 4 first bytes of flag = 0x%08x\n",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), agreement_id.ERAID_FIELDS.contextid,
                         agreement_id.ERAID_FIELDS.epoch, agreement_id.ERAID_FIELDS.agreementid,
                         ret, (0 != ERA_VALUE_BYTES_COUNT(&av->header)) ? *(int *) contrib : 0));
 ...
 */
int mca_coll_ftagree_era_intra(void *contrib, int dt_count, ompi_datatype_t *dt, ompi_op_t *op,
                               ompi_group_t **group, bool grp_update, ompi_communicator_t *comm,
 ...
 * Returns:	- MPI_SUCCESS or an MPI error code
 */
int mca_coll_ftagree_era_inter(void *contrib, int dt_count, ompi_datatype_t *dt, ompi_op_t *op,
                               ompi_group_t **group, bool grp_update, ompi_communicator_t *comm,
                               mca_coll_base_module_t *module)
{
    ...
    if (OPAL_UNLIKELY(op != &ompi_mpi_op_band.op || dt != &ompi_mpi_int.dt || dt_count != 1)) {
    ...
    ompi_comm_set(&shadowcomm, comm, ompi_group_size(uniongrp), NULL, 0, NULL, NULL,
                  comm->error_handler, NULL, uniongrp, NULL);
    ...
    rc = mca_coll_ftagree_era_intra(contriblh, dt_count * 2, dt, op, group, grp_update, shadowcomm,
                                    module);

    comm->agreement_specific = shadowcomm->agreement_specific;
    if (NULL != comm->agreement_specific)
        OBJ_RETAIN(comm->agreement_specific);
...
int mca_coll_ftagree_iera_intra(void *contrib, int dt_count, ompi_datatype_t *dt, ompi_op_t *op,
                                ompi_group_t **group, bool grp_update, ompi_communicator_t *comm,
...
int mca_coll_ftagree_era_free_comm(ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
    ompi_group_t *acked;
    ...
    OPAL_OUTPUT_VERBOSE((4, ompi_ftmpi_output_handle,
                         ...
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), comm->c_contextid, comm->c_epoch));

    opal_mutex_lock(&ompi_group_afp_mutex);
    ompi_group_intersection(comm->c_remote_group, ompi_group_all_failed_procs, &acked);
    opal_mutex_unlock(&ompi_group_afp_mutex);
    do {
        rc = mca_coll_ftagree_era_intra(NULL, 0, &ompi_mpi_int.dt, &ompi_mpi_op_band.op, &acked,
                                        true, comm, comm->c_coll->coll_agree_module);
