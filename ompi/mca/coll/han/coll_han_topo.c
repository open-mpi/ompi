...
static void mca_coll_han_topo_print(int *topo, struct ompi_communicator_t *comm, int num_topo_level)
{
                            ...
                            int num_topo_level)
{
    ...
    /* node leaders translate the node-local ranks to global ranks and check whether they are placed
     * consecutively */
    if (0 == low_rank) {
        ...
        ompi_group_translate_ranks(low_comm->c_local_group, low_size, topo, comm->c_local_group,
                                   my_low_rank_map);
        ...
        up_comm->c_coll->coll_allreduce(MPI_IN_PLACE, &reduce_vals, 3, MPI_INT, MPI_MAX, up_comm,
        ...
        if (ranks_non_consecutive && !is_imbalanced) {
            /* kick off up_comm allgather to collect non-consecutive rank information at node
             * leaders */
            ranks_map = malloc(sizeof(int) * size);
            up_comm->c_coll->coll_iallgather(my_low_rank_map, low_size, MPI_INT, ranks_map,
                                             low_size, MPI_INT, up_comm, &request,
    ...
    low_comm->c_coll->coll_bcast(bcast_vals, 2, MPI_INT, 0, low_comm,
    ...
    low_comm->c_coll->coll_bcast(topo, num_topo_level * size, MPI_INT, 0, low_comm,
                                 low_comm->c_coll->coll_bcast_module);
    free(my_low_rank_map);
