 ...
 */
int mca_coll_han_get_all_coll_modules(struct ompi_communicator_t *comm,
                                      mca_coll_han_module_t *han_module)
{
    ...
    OPAL_LIST_FOREACH (item, comm->c_coll->module_list, mca_coll_base_avail_coll_t) {
    ...
    opal_output_verbose(60, mca_coll_han_component.han_output,
                        ...
                        topo_lvl, mca_coll_han_topo_lvl_to_str(topo_lvl), nb_modules,
                        comm->c_contextid, comm->c_name);
 ...
 */
static const msg_size_rule_t *get_dynamic_rule(COLLTYPE_T collective, size_t msg_size,
                                               ...
                                               mca_coll_han_module_t *han_module)
{
    ...
    for (coll_idx = dynamic_rules->nb_collectives - 1; coll_idx >= 0; coll_idx--) {
    ...
    for (topo_idx = coll_rule->nb_topologic_levels - 1; topo_idx >= 0; topo_idx--) {
    ...
    if (topo_idx < 0 || NULL == topo_rule) {
        ...
        opal_output_verbose(
            60, mca_coll_han_component.han_output,
            "coll:han:get_dynamic_rule HAN searched for topologic level %d (%s) rule "
            "for collective %d (%s) but did not find any rule\n",
            topo_lvl, mca_coll_han_topo_lvl_to_str(topo_lvl), collective,
            mca_coll_base_colltype_to_str(collective));
        return NULL;
    }

    /* Find the configuration rule */
    for (conf_idx = topo_rule->nb_rules - 1; conf_idx >= 0; conf_idx--) {
    ...
    if (conf_idx < 0 || NULL == conf_rule) {
        ...
        opal_output_verbose(60, mca_coll_han_component.han_output,
                            ...
                            collective, mca_coll_base_colltype_to_str(collective), topo_lvl,
                            mca_coll_han_topo_lvl_to_str(topo_lvl), comm_size);
        return NULL;
    }

    /* Find the message size rule */
    for (msg_size_idx = conf_rule->nb_msg_size - 1; msg_size_idx >= 0; msg_size_idx--) {
    ...
    if (msg_size_idx < 0 || NULL == msg_size_rule) {
        ...
        opal_output_verbose(60, mca_coll_han_component.han_output,
                            ...
                            "on topological level %d (%s) for a %d configuration size "
                            "for a %" PRIsize_t
                            " sized message but did not manage to find anything. "
                            "This is the result of an invalid configuration file: "
                            "the first message size of each configuration must be 0\n",
                            collective, mca_coll_base_colltype_to_str(collective), topo_lvl,
                            mca_coll_han_topo_lvl_to_str(topo_lvl), comm_size, msg_size);
    ...
    opal_output_verbose(80, mca_coll_han_component.han_output,
                        ...
                        msg_size_rule->configuration_size, msg_size_rule->msg_size, component,
 ...
 */
static mca_coll_base_module_t *get_module(COLLTYPE_T coll_id, size_t msg_size,
                                          struct ompi_communicator_t *comm,
                                          mca_coll_han_module_t *han_module)
{
    ...
    dynamic_rule = get_dynamic_rule(coll_id, msg_size, comm, han_module);
    ...
    if (mca_rule_component < 0 || mca_rule_component >= COMPONENTS_COUNT) {
        ...
        opal_output_verbose(0, mca_coll_han_component.han_output,
                            ...
                            mca_rule_component, coll_id, mca_coll_base_colltype_to_str(coll_id),
 ...
 */
int mca_coll_han_allgather_intra_dynamic(const void *sbuf, int scount,
                                         ...
                                         mca_coll_base_module_t *module)
{
    ...
    sub_module = get_module(ALLGATHER, dtype_size, comm, han_module);
    ...
    if (NULL == sub_module) {
        ...
        opal_output_verbose(verbosity, mca_coll_han_component.han_output,
                            ...
                            ALLGATHER, mca_coll_base_colltype_to_str(ALLGATHER), topo_lvl,
                            mca_coll_han_topo_lvl_to_str(topo_lvl), comm->c_contextid,
                            comm->c_name);
        ...
        han_module->dynamic_errors++;
        opal_output_verbose(verbosity, mca_coll_han_component.han_output,
                            "coll:han:mca_coll_han_allgather_intra_dynamic HAN found valid module "
                            "for collective %d (%s) "
                            "with topological level %d (%s) on communicator (%d/%s) but this "
                            "module cannot handle this collective. "
                            "Please check dynamic file/mca parameters\n",
                            ALLGATHER, mca_coll_base_colltype_to_str(ALLGATHER), topo_lvl,
                            mca_coll_han_topo_lvl_to_str(topo_lvl), comm->c_contextid,
                            comm->c_name);
 ...
 */
int mca_coll_han_allgatherv_intra_dynamic(const void *sbuf, int scount,
                                          ...
                                          mca_coll_base_module_t *module)
{
    ...
    sub_module = get_module(ALLGATHERV, msg_size, comm, han_module);
    ...
    if (NULL == sub_module) {
        ...
        opal_output_verbose(verbosity, mca_coll_han_component.han_output,
                            ...
                            "with topological level %d (%s) on communicator (%d/%s). "
                            "Please check dynamic file/mca parameters\n",
                            ALLGATHERV, mca_coll_base_colltype_to_str(ALLGATHERV), topo_lvl,
                            mca_coll_han_topo_lvl_to_str(topo_lvl), comm->c_contextid,
                            comm->c_name);
        OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output,
 ...
 * calls the correct module if fallback mechanism is activated
 */
int mca_coll_han_allreduce_intra_dynamic(const void *sbuf, void *rbuf, int count,
                                         ...
                                         mca_coll_base_module_t *module)
{
    ...
    sub_module = get_module(ALLREDUCE, dtype_size, comm, han_module);
    ...
    if (NULL == sub_module) {
        ...
        opal_output_verbose(verbosity, mca_coll_han_component.han_output,
                            ...
                            "with topological level %d (%s) on communicator (%d/%s). "
                            "Please check dynamic file/mca parameters\n",
                            ALLREDUCE, mca_coll_base_colltype_to_str(ALLREDUCE), topo_lvl,
                            mca_coll_han_topo_lvl_to_str(topo_lvl), comm->c_contextid,
                            comm->c_name);
        OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output,
 ...
 * calls the correct module if fallback mechanism is activated
 */
int mca_coll_han_barrier_intra_dynamic(struct ompi_communicator_t *comm,
                                       mca_coll_base_module_t *module)
{
    mca_coll_han_module_t *han_module = (mca_coll_han_module_t *) module;
    TOPO_LVL_T topo_lvl = han_module->topologic_level;
    ...
    /* Compute configuration information for dynamic rules */
    sub_module = get_module(BARRIER, 0, comm, han_module);
    ...
    if (NULL == sub_module) {
        ...
        opal_output_verbose(verbosity, mca_coll_han_component.han_output,
                            ...
                            "with topological level %d (%s) on communicator (%d/%s). "
                            "Please check dynamic file/mca parameters\n",
                            BARRIER, mca_coll_base_colltype_to_str(BARRIER), topo_lvl,
                            mca_coll_han_topo_lvl_to_str(topo_lvl), comm->c_contextid,
                            comm->c_name);
        OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output,
 ...
 * calls the correct module if fallback mechanism is activated
 */
int mca_coll_han_bcast_intra_dynamic(void *buff, int count, struct ompi_datatype_t *dtype, int root,
                                     struct ompi_communicator_t *comm,
                                     mca_coll_base_module_t *module)
{
    mca_coll_han_module_t *han_module = (mca_coll_han_module_t *) module;
    ...
    sub_module = get_module(BCAST, dtype_size, comm, han_module);
    ...
    if (NULL == sub_module) {
        ...
        opal_output_verbose(verbosity, mca_coll_han_component.han_output,
                            ...
                            "with topological level %d (%s) on communicator (%d/%s). "
                            "Please check dynamic file/mca parameters\n",
                            BCAST, mca_coll_base_colltype_to_str(BCAST), topo_lvl,
                            mca_coll_han_topo_lvl_to_str(topo_lvl), comm->c_contextid,
                            comm->c_name);
        OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output,
 ...
 * calls the correct module if fallback mechanism is activated
 */
int mca_coll_han_gather_intra_dynamic(const void *sbuf, int scount, struct ompi_datatype_t *sdtype,
                                      ...
                                      mca_coll_base_module_t *module)
{
    ...
    sub_module = get_module(GATHER, dtype_size, comm, han_module);
    ...
    if (NULL == sub_module) {
        ...
        opal_output_verbose(verbosity, mca_coll_han_component.han_output,
                            ...
                            "with topological level %d (%s) on communicator (%d/%s). "
                            "Please check dynamic file/mca parameters\n",
                            GATHER, mca_coll_base_colltype_to_str(GATHER), topo_lvl,
                            mca_coll_han_topo_lvl_to_str(topo_lvl), comm->c_contextid,
                            comm->c_name);
        OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output,
 ...
 * calls the correct module if fallback mechanism is activated
 */
int mca_coll_han_reduce_intra_dynamic(const void *sbuf, void *rbuf, int count,
                                      ...
                                      mca_coll_base_module_t *module)
{
    ...
    sub_module = get_module(REDUCE, dtype_size, comm, han_module);
    ...
    if (NULL == sub_module) {
        ...
        opal_output_verbose(verbosity, mca_coll_han_component.han_output,
                            ...
                            "with topological level %d (%s) on communicator (%d/%s). "
                            "Please check dynamic file/mca parameters\n",
                            REDUCE, mca_coll_base_colltype_to_str(REDUCE), topo_lvl,
                            mca_coll_han_topo_lvl_to_str(topo_lvl), comm->c_contextid,
                            comm->c_name);
        OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output,
    ...
    return reduce(sbuf, rbuf, count, dtype, op, root, comm, sub_module);
}
 ...
 * calls the correct module if fallback mechanism is activated
 */
int mca_coll_han_scatter_intra_dynamic(const void *sbuf, int scount, struct ompi_datatype_t *sdtype,
                                       void *rbuf, int rcount, struct ompi_datatype_t *rdtype,
                                       int root, struct ompi_communicator_t *comm,
                                       mca_coll_base_module_t *module)
{
    ...
    sub_module = get_module(SCATTER, dtype_size, comm, han_module);
    ...
    if (NULL == sub_module) {
        ...
        opal_output_verbose(verbosity, mca_coll_han_component.han_output,
                            ...
                            "with topological level %d (%s) on communicator (%d/%s). "
                            "Please check dynamic file/mca parameters\n",
                            SCATTER, mca_coll_base_colltype_to_str(SCATTER), topo_lvl,
                            mca_coll_han_topo_lvl_to_str(topo_lvl), comm->c_contextid,
                            comm->c_name);
        OPAL_OUTPUT_VERBOSE((30, mca_coll_han_component.han_output,
     ...
     * Selector's job is done, call the collective
     */
    return scatter(sbuf, scount, sdtype, rbuf, rcount, rdtype, root, comm, sub_module);
}
