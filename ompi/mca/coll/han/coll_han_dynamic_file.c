...
int mca_coll_han_init_dynamic_rules(void)
{
    ...
    if (NULL == (fname = mca_coll_han_component.dynamic_rules_filename)) {
        opal_output_verbose(
            5, mca_coll_han_component.han_output,
            "coll:han:mca_coll_han_init_dynamic_rules coll_han_use_dynamic_file_rules is set but "
            "coll_han_dynamic_rules_filename is not Rules from MCA parameters will be used "
            "instead\n");
    ...
    for (i = 0; i < nb_coll; i++) {
        ...
        if ((coll_id < ALLGATHER) || (coll_id >= COLLCOUNT)) {
            /* maybe the file was in the old format and we read the collective index instead of the
             * name. */
            char *endp;
            coll_id = strtol(coll_name, &endp, 10);
            if (('\0' != *endp) || (coll_id < ALLGATHER)
                || (coll_id >= COLLCOUNT)) { /* there is garbage in the input */
                opal_output_verbose(
                    5, mca_coll_han_component.han_output,
        ...
        for (j = 0; j < nb_topo; j++) {
            ...
            if ((getnext_long(fptr, &topo_lvl) < 0) || (topo_lvl < INTRA_NODE)
                || (topo_lvl >= NB_TOPO_LVL)) {
                opal_output_verbose(5, mca_coll_han_component.han_output,
                                    "coll:han:mca_coll_han_init_dynamic_rules found an error on "
                                    "dynamic rules file %s "
                                    "at line %d: an invalid topo level %ld is given or the reader "
            ...
            for (k = 0; k < nb_rules; k++) {
                ...
                if ((getnext_long(fptr, &conf_size) < 0) || (conf_size < 1)
                    || (0 == k && conf_size > 1)) {
                    opal_output_verbose(
                        5, mca_coll_han_component.han_output,
                        "coll:han:mca_coll_han_init_dynamic_rules invalid configuration size %ld "
                        "at line %d "
                        "or the reader encountered an unexpected EOF the configuration size must "
                        "be at least %d "
                ...
                if ((getnext_long(fptr, &nb_msg_size) < 0) || (nb_msg_size < 0)) {
                    opal_output_verbose(
                        5, mca_coll_han_component.han_output,
                        "coll:han:mca_coll_han_init_dynamic_rules found an error on dynamic rules "
                        "file %s "
                ...
                for (l = 0; l < nb_msg_size; l++) {
                    ...
                    if ((rc < 0) || (0 == l && msg_size > 1)) {
                        opal_output_verbose(
                            5, mca_coll_han_component.han_output,
                            "coll:han:mca_coll_han_init_dynamic_rules found an error on dynamic "
                            "rules file %s "
                    ...
                    if ((component < SELF) || (component >= COMPONENTS_COUNT)) {
                        opal_output_verbose(
                            ...
                            "reader encountered an unexpected EOF. Collective component id must be "
                            "at "
                    ...
                    if (1 == ompi_coll_base_file_peek_next_char_is(fptr, &fileline, '[')) {
                        opal_output_verbose(5, mca_coll_han_component.han_output,
                                            "coll:han:mca_coll_han_init_dynamic_rules found "
                                            "optional pipelining segment lengths\n");
                        long seglength;
                        if (0 != topo_lvl) {
                            opal_output_verbose(
                                5, mca_coll_han_component.han_output,
                                "coll:han:mca_coll_han_init_dynamic_rules "
                                "file %s line %d found segment lengths for topological collective "
                                "at level != 0 "
                        ...
                        while (0 == ompi_coll_base_file_peek_next_char_is(fptr, &fileline, ']')) {
                            if (getnext_long(fptr, &seglength)) {
                                opal_output_verbose(
                                    5, mca_coll_han_component.han_output,
                                    "coll:han:mca_coll_han_init_dynamic_rules "
                                    "file %s line %d found end of file while reading the optional "
                                    "list "
