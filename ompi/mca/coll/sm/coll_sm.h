...
int mca_coll_sm_allreduce_intra(const void *sbuf, void *rbuf, int count,
                                ...
                                struct ompi_communicator_t *comm, mca_coll_base_module_t *module);
int mca_coll_sm_alltoall_intra(const void *sbuf, int scount, struct ompi_datatype_t *sdtype,
                               void *rbuf, int rcount, struct ompi_datatype_t *rdtype,
                               struct ompi_communicator_t *comm, mca_coll_base_module_t *module);
...
int mca_coll_sm_bcast_intra(void *buff, int count, struct ompi_datatype_t *datatype, int root,
                            struct ompi_communicator_t *comm, mca_coll_base_module_t *module);
int mca_coll_sm_bcast_log_intra(void *buff, int count, struct ompi_datatype_t *datatype, int root,
                                struct ompi_communicator_t *comm, mca_coll_base_module_t *module);
int mca_coll_sm_exscan_intra(const void *sbuf, void *rbuf, int count, struct ompi_datatype_t *dtype,
                             struct ompi_op_t *op, struct ompi_communicator_t *comm,
                             mca_coll_base_module_t *module);
int mca_coll_sm_gather_intra(void *sbuf, int scount, struct ompi_datatype_t *sdtype, void *rbuf,
                             int rcount, struct ompi_datatype_t *rdtype, int root,
                             struct ompi_communicator_t *comm, mca_coll_base_module_t *module);
int mca_coll_sm_gatherv_intra(void *sbuf, int scount, struct ompi_datatype_t *sdtype, void *rbuf,
                              int *rcounts, int *disps, struct ompi_datatype_t *rdtype, int root,
                              struct ompi_communicator_t *comm, mca_coll_base_module_t *module);
int mca_coll_sm_reduce_intra(const void *sbuf, void *rbuf, int count, struct ompi_datatype_t *dtype,
...
int mca_coll_sm_reduce_log_intra(const void *sbuf, void *rbuf, int count,
                                 ...
                                 struct ompi_communicator_t *comm, mca_coll_base_module_t *module);
int mca_coll_sm_reduce_scatter_intra(const void *sbuf, void *rbuf, int *rcounts,
                                     struct ompi_datatype_t *dtype, struct ompi_op_t *op,
                                     struct ompi_communicator_t *comm,
                                     mca_coll_base_module_t *module);
int mca_coll_sm_scan_intra(const void *sbuf, void *rbuf, int count, struct ompi_datatype_t *dtype,
                           struct ompi_op_t *op, struct ompi_communicator_t *comm,
                           mca_coll_base_module_t *module);
int mca_coll_sm_scatter_intra(const void *sbuf, int scount, struct ompi_datatype_t *sdtype,
...
#define FLAG_SETUP(flag_num, flag, data)                                        \
    (flag) = (mca_coll_sm_in_use_flag_t *) (((char *) (data)->mcb_in_use_flags) \
...
#define COPY_FRAGMENT_IN(convertor, index, rank, iov, max_data)                              \
    (iov).iov_base = (index)->mcbmi_data + ((rank) *mca_coll_sm_component.sm_fragment_size); \
    (iov).iov_len = (max_data);                                                              \
    opal_convertor_pack(&(convertor), &(iov), &mca_coll_sm_one, &(max_data))
...
#define COPY_FRAGMENT_OUT(convertor, src_rank, index, iov, max_data)              \
    ...
    (iov).iov_len = (max_data);                                                   \
    opal_convertor_unpack(&(convertor), &(iov), &mca_coll_sm_one, &(max_data))
...
#define COPY_FRAGMENT_BETWEEN(src_rank, dest_rank, index, len)                            \
    memcpy(((index)->mcbmi_data + ((dest_rank) *mca_coll_sm_component.sm_fragment_size)), \
           ((index)->mcbmi_data + ((src_rank) *mca_coll_sm_component.sm_fragment_size)), (len))
...
#define CHILD_NOTIFY_PARENT(child_rank, parent_rank, index, value) \
    ((size_t volatile *) (((char *) (index)->mcbmi_control)        \
                          + (mca_coll_sm_component.sm_control_size \
...
#define PARENT_WAIT_FOR_NOTIFY_SPECIFIC(child_rank, parent_rank, index, value, label)         \
    do {                                                                                      \
        size_t volatile *ptr = ((size_t volatile *) (((char *) index->mcbmi_control)          \
                                                     + (mca_coll_sm_component.sm_control_size \
                                                        * (parent_rank))))                    \
                               + child_rank;                                                  \
        SPIN_CONDITION(0 != *ptr, label);                                                     \
