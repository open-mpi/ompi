...
static int sm_verify_mca_variables(void)
{
    ...
    if (0 != (cs->sm_fragment_size % cs->sm_control_size)) {
        cs->sm_fragment_size += cs->sm_control_size - (cs->sm_fragment_size % cs->sm_control_size);
    }
    ...
    cs->sm_segs_per_inuse_flag = cs->sm_comm_num_segments / cs->sm_comm_num_in_use_flags;

    if (cs->sm_tree_degree > cs->sm_control_size) {
        opal_show_help("help-mpi-coll-sm.txt", "tree-degree-larger-than-control", true,
                       cs->sm_tree_degree, cs->sm_control_size);
        cs->sm_tree_degree = cs->sm_control_size;
    }
    if (cs->sm_tree_degree > 255) {
        opal_show_help("help-mpi-coll-sm.txt", "tree-degree-larger-than-255", true,
                       cs->sm_tree_degree);
    ...
    coll_sm_shared_mem_used_data = (int) (4 * cs->sm_control_size
                                          ...
                                          + (cs->sm_comm_num_segments
                                             * (cs->sm_info_comm_size * cs->sm_fragment_size)));
...
static int sm_register(void)
{
    ...
    (void) mca_base_component_var_register(c, "priority", "Priority of the sm coll component",
                                           ...
                                           MCA_BASE_VAR_SCOPE_READONLY, &cs->sm_priority);

    cs->sm_control_size = 4096;
    (void) mca_base_component_var_register(
        c, "control_size",
        "Length of the control data -- should usually be either the length of a cache line on most "
        "SMPs, or the size of a page on machines that support direct memory affinity page "
        "placement (in bytes)",
    ...
    (void) mca_base_component_var_register(
        c, "fragment_size",
        "Fragment size (in bytes) used for passing data through shared memory (will be rounded up "
        "to the nearest control_size size)",
        MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_9, MCA_BASE_VAR_SCOPE_READONLY,
        &cs->sm_fragment_size);
    ...
    (void) mca_base_component_var_register(
        c, "comm_in_use_flags",
        "Number of \"in use\" flags, used to mark a message passing area segment as currently "
        "being used or not (must be >= 2 and <= comm_num_segments)",
        MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_9, MCA_BASE_VAR_SCOPE_READONLY,
    ...
    (void) mca_base_component_var_register(
        c, "comm_num_segments",
        "Number of segments in each communicator's shared memory message passing area (must be >= "
        "2, and must be a multiple of comm_in_use_flags)",
        MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_9, MCA_BASE_VAR_SCOPE_READONLY,
        &cs->sm_comm_num_segments);
    ...
    (void) mca_base_component_var_register(
        c, "shared_mem_used_data",
        "Amount of shared memory used, per communicator, in the shared memory data area for "
        "info_num_procs processes (in bytes)",
        MCA_BASE_VAR_TYPE_INT, NULL, 0, MCA_BASE_VAR_FLAG_DEFAULT_ONLY, OPAL_INFO_LVL_9,
        MCA_BASE_VAR_SCOPE_READONLY, &coll_sm_shared_mem_used_data);
