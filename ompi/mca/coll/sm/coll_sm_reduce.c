 ...
 * Local functions
 */
static int reduce_inorder(const void *sbuf, void *rbuf, int count, struct ompi_datatype_t *dtype,
                          struct ompi_op_t *op, int root, struct ompi_communicator_t *comm,
                          mca_coll_base_module_t *module);
#define WANT_REDUCE_NO_ORDER 0
#if WANT_REDUCE_NO_ORDER
static int reduce_no_order(const void *sbuf, void *rbuf, int count, struct ompi_datatype_t *dtype,
                           struct ompi_op_t *op, int root, struct ompi_communicator_t *comm,
                           mca_coll_base_module_t *module);
#endif
                             ...
                             mca_coll_base_module_t *module)
{
    ...
    if ((int) size > mca_coll_sm_component.sm_control_size) {
        return sm_module->previous_reduce(sbuf, rbuf, count, dtype, op, root, comm,
            ...
            do {
                ...
                if (size - 1 == rank) {
                    ...
                    if (first_operation) {
                        ...
                        if (MPI_IN_PLACE != sbuf) {
                            ompi_datatype_copy_content_same_ddt(dtype, count, reduce_target,
                                                                (char *) sbuf);
                        }
                    }
                }
                ...
                /* Process (size-1) is not the root */
                    ...
                    PARENT_WAIT_FOR_NOTIFY_SPECIFIC(size - 1, rank, index, max_data,
                                                    reduce_root_parent_label1);

                    /* If the datatype is contiguous, just copy it
                       straight to the reduce_target */
                    if (NULL == free_buffer) {
                        memcpy(reduce_target,
                               ((char *) index->mcbmi_data)
                                   + (size - 1) * mca_coll_sm_component.sm_fragment_size,
                               max_data);
                    }
                    /* If the datatype is noncontiguous, use the
                       ...
                       rbuf */
                        ...
                        COPY_FRAGMENT_OUT(rbuf_convertor, size - 1, index, iov, max_data);
                    }
                }
                ...
                for (peer = size - 2; peer >= 0; --peer) {
                    ...
                    /* Handle the case where the source is this
                       ...
                       *only* happen when root==0).  In this case, we
                       ...
                       sbuf. */
                    if (rank == peer) {
                        ompi_op_reduce(op, ((char *) sbuf) + frag_num * extent * segment_ddt_count,
                                       reduce_target, min(count_left, segment_ddt_count), dtype);
                    }
                    ...
                    /* Now handle the case where the source is not
                       ...
                       the segment into shmem. */
                        ...
                        index = &(data->mcb_data_index[segment_num]);
                        PARENT_WAIT_FOR_NOTIFY_SPECIFIC(peer, rank, index, max_data,
                                                        reduce_root_parent_label2);
                        ...
                        if (NULL == free_buffer) {
                            ompi_op_reduce(op,
                                           ...
                                           reduce_target, min(count_left, segment_ddt_count),
                                           dtype);
                        }
                        ...
                        /* Otherwise, unpack the fragment to the temporary
                           buffer and then do the reduction from there */
                            ...
                            /* Unpack the fragment into my temporary
                               buffer */
                            max_data = segment_ddt_bytes;
                            COPY_FRAGMENT_OUT(rtb_convertor, peer, index, iov, max_data);
                            opal_convertor_set_position(&rtb_convertor, &zero);

                            /* Do the reduction on this fragment */
                            ompi_op_reduce(op, reduce_temp_buffer, reduce_target,
                                           min(count_left, segment_ddt_count), dtype);
                        }
