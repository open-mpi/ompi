...
static int setup_scatter_buffers_linear(struct ompi_communicator_t *comm,
                                        ...
                                        mca_coll_portals4_module_t *portals4_module)
{
    ...
    ompi_coll_portals4_create_send_converter(&request->u.scatter.send_converter,
                                             ...
                                             ompi_comm_peer_lookup(comm,
                                                                   request->u.scatter.my_rank),
                                             request->u.scatter.pack_src_count,
                                             request->u.scatter.pack_src_dtype);
    opal_convertor_get_packed_size(&request->u.scatter.send_converter,
    ...
    if (i_am_root) {
        ...
        request->u.scatter.scatter_bytes = request->u.scatter.packed_size
                                           * (ptrdiff_t) request->u.scatter.size;
        ...
        for (int32_t i = 0; i < request->u.scatter.size; i++) {
            ...
            uint64_t offset = request->u.scatter.pack_src_extent * request->u.scatter.pack_src_count
                              * i;

            opal_output_verbose(30, ompi_coll_base_framework.framework_output,
                                "%s:%d:rank(%d): offset(%lu)", __FILE__, __LINE__,
                                request->u.scatter.my_rank, offset);

            ompi_coll_portals4_create_send_converter(&request->u.scatter.send_converter,
                                                     request->u.scatter.pack_src_buf + offset,
                                                     ompi_comm_peer_lookup(comm, request->u.scatter
                                                                                     .my_rank),
                                                     request->u.scatter.pack_src_count,
                                                     request->u.scatter.pack_src_dtype);

            iov.iov_len = request->u.scatter.packed_size;
            iov.iov_base = (IOVBASE_TYPE *) ((char *) request->u.scatter.scatter_buf
                                             + (request->u.scatter.packed_size * i));
        ...
        opal_output_verbose(30, ompi_coll_base_framework.framework_output,
                            "%s:%d:rank(%d): root - scatter_buf(%p) - "
                            "scatter_bytes(%lu)=packed_size(%ld) * size(%d)",
        ...
        opal_output_verbose(
            ...
            __FILE__, __LINE__, request->u.scatter.my_rank, request->u.scatter.scatter_buf,
            request->u.scatter.scatter_bytes, request->u.scatter.packed_size);
    }
...
static int setup_scatter_handles(struct ompi_communicator_t *comm,
                                 ompi_coll_portals4_request_t *request,
                                 mca_coll_portals4_module_t *portals4_module)
{
    int ret, line;
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:setup_scatter_handles enter rank %d",
                         request->u.scatter.my_rank));

    /**********************************/
    /* Setup Scatter Handles           */
    /**********************************/
    COLL_PORTALS4_SET_BITS(request->u.scatter.scatter_match_bits, ompi_comm_get_cid(comm), 0, 0,
                           COLL_PORTALS4_SCATTER, 0, request->u.scatter.coll_count);
    ...
    ret = PtlCTAlloc(mca_coll_portals4_component.ni_h, &request->u.scatter.scatter_cth);
    if (PTL_OK != ret) {
    ...
    ret = PtlMEAppend(mca_coll_portals4_component.ni_h, mca_coll_portals4_component.pt_idx, &me,
                      PTL_PRIORITY_LIST, NULL, &request->u.scatter.scatter_meh);
    if (PTL_OK != ret) {
        ret = OMPI_ERROR;
        line = __LINE__;
        goto err_hdlr;
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:setup_scatter_handles exit rank %d",
                         request->u.scatter.my_rank));
                              ...
                              mca_coll_portals4_module_t *portals4_module)
{
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:setup_sync_handles enter rank %d",
                         request->u.scatter.my_rank));

    /**********************************/
    /* Setup Sync Handles             */
    /**********************************/
    COLL_PORTALS4_SET_BITS(request->u.scatter.sync_match_bits, ompi_comm_get_cid(comm), 0, 1,
                           COLL_PORTALS4_SCATTER, 0, request->u.scatter.coll_count);
    ...
    ret = PtlCTAlloc(mca_coll_portals4_component.ni_h, &request->u.scatter.sync_cth);
    if (PTL_OK != ret) {
        ret = OMPI_ERR_TEMP_OUT_OF_RESOURCE;
        line = __LINE__;
        goto err_hdlr;
    ...
    me.ignore_bits = 0;
    ret = PtlMEAppend(mca_coll_portals4_component.ni_h, mca_coll_portals4_component.pt_idx, &me,
                      PTL_PRIORITY_LIST, NULL, &request->u.scatter.sync_meh);
    if (PTL_OK != ret) {
        ret = OMPI_ERROR;
        line = __LINE__;
        goto err_hdlr;
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:setup_sync_handles exit rank %d",
                         request->u.scatter.my_rank));
...
static int cleanup_scatter_handles(ompi_coll_portals4_request_t *request)
{
    int ret, line;

    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:cleanup_scatter_handles enter rank %d",
                         request->u.scatter.my_rank));
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:cleanup_scatter_handles exit rank %d",
                         request->u.scatter.my_rank));
...
static int cleanup_sync_handles(ompi_coll_portals4_request_t *request)
{
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:cleanup_sync_handles enter rank %d",
                         request->u.scatter.my_rank));
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:cleanup_sync_handles exit rank %d",
                         request->u.scatter.my_rank));
...
err_hdlr:
    opal_output(ompi_coll_base_framework.framework_output,
                "%s:%4d:%4d\tError occurred (ptl_ret=%d) ret=%d, rank %2d", __FILE__, __LINE__,
                line, ptl_ret, ret, request->u.scatter.my_rank);

    return ret;
}

static int ompi_coll_portals4_scatter_intra_linear_top(const void *sbuf, int scount,
                                                       ...
                                                       mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:scatter_intra_linear_top enter rank %d",
                         request->u.scatter.my_rank));
    ...
    ompi_datatype_get_extent(request->u.scatter.pack_src_dtype, &request->u.scatter.pack_src_lb,
    ...
    opal_output_verbose(30, ompi_coll_base_framework.framework_output,
                        "%s:%d:rank(%d): request->u.scatter.unpack_dst_offset(%lu)", __FILE__,
                        __LINE__, request->u.scatter.my_rank, request->u.scatter.unpack_dst_offset);
    ...
    number_of_fragment = (request->u.scatter.packed_size
    ...
    opal_output_verbose(90, ompi_coll_base_framework.framework_output,
                        ...
                        request->u.scatter.my_rank, number_of_fragment);
    ...
    if (i_am_root) {
        ...
        ret = PtlTriggeredCTInc(request->u.scatter.scatter_cth, sync_incr_event,
                                request->u.scatter.sync_cth, expected_rtrs);
        if (PTL_OK != ret) {
            ret = OMPI_ERROR;
            line = __LINE__;
            goto err_hdlr;
        ...
        ret = PtlTriggeredCTInc(request->u.scatter.scatter_cth, sync_incr_event,
                                request->u.scatter.sync_cth, expected_rtrs + expected_acks);
        if (PTL_OK != ret) {
            ret = OMPI_ERROR;
            line = __LINE__;
            goto err_hdlr;
        ...
        for (int32_t i = 0; i < request->u.scatter.size; i++) {
            /* do not put to my scatter_buf.  my data gets unpacked into my out buffer in
             * linear_bottom(). */
            ...
            opal_output_verbose(10, ompi_coll_base_framework.framework_output,
                                "%s:%d:rank(%d): offset(%lu)=rank(%d) * packed_size(%ld)", __FILE__,
                                __LINE__, request->u.scatter.my_rank, offset, i,
                                request->u.scatter.packed_size);

            for (ptl_size_t j = 0; j < number_of_fragment; j++) {

                ptl_size_t frag_size = (size_left
                                        > mca_coll_portals4_component.ni_limits.max_msg_size)
                                           ? mca_coll_portals4_component.ni_limits.max_msg_size
                                           : size_left;
                ...
                OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                                     ...
                                     __FILE__, __LINE__, request->u.scatter.my_rank, j, size_sent,
                                     frag_size));

                ret = PtlTriggeredPut(request->u.scatter.scatter_mdh,
                                      (ptl_size_t) request->u.scatter.scatter_buf + offset
                                          + size_sent,
                                      frag_size, PTL_NO_ACK_REQ,
                                      ompi_coll_portals4_get_peer(comm, i),
                                      mca_coll_portals4_component.pt_idx,
                                      request->u.scatter.scatter_match_bits, size_sent, NULL, 0,
                                      request->u.scatter.scatter_cth, expected_chained_rtrs);
                if (PTL_OK != ret) {
                    ret = OMPI_ERROR;
                    line = __LINE__;
                    goto err_hdlr;
    ...
    /**********************************************/
    if (!i_am_root) {
        ret = PtlTriggeredPut(request->u.scatter.sync_mdh, 0, 0, PTL_NO_ACK_REQ,
                              ompi_coll_portals4_get_peer(comm, request->u.scatter.root_rank),
                              mca_coll_portals4_component.pt_idx,
                              request->u.scatter.sync_match_bits, 0, NULL, 0,
                              request->u.scatter.scatter_cth, expected_ops);
        if (PTL_OK != ret) {
            ret = OMPI_ERROR;
            line = __LINE__;
            goto err_hdlr;
    ...
    if (!request->u.scatter.is_sync) {
        ...
        ret = PtlTriggeredPut(mca_coll_portals4_component.zero_md_h, 0, 0, PTL_NO_ACK_REQ,
                              ...
                              (uintptr_t) request, request->u.scatter.scatter_cth, expected_ops);
        if (PTL_OK != ret) {
            ret = OMPI_ERROR;
            line = __LINE__;
            goto err_hdlr;
    ...
    /**************************************/
    if (!i_am_root) {
        ret = PtlPut(request->u.scatter.sync_mdh, 0, 0, PTL_NO_ACK_REQ,
                     ompi_coll_portals4_get_peer(comm, request->u.scatter.root_rank),
                     mca_coll_portals4_component.pt_idx, request->u.scatter.sync_match_bits, 0,
                     NULL, 0);
        if (PTL_OK != ret) {
            ret = OMPI_ERROR;
            line = __LINE__;
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:scatter_intra_linear_top exit rank %d",
                         request->u.scatter.my_rank));
...
static int ompi_coll_portals4_scatter_intra_linear_bottom(struct ompi_communicator_t *comm,
                                                          ompi_coll_portals4_request_t *request)
{
    int ret, line;

    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:scatter_intra_linear_bottom enter rank %d",
                         request->u.scatter.my_rank));
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:scatter_intra_linear_bottom exit rank %d",
                         request->u.scatter.my_rank));
                                     ...
                                     mca_coll_base_module_t *module)
{
     ...
     */
    ret = ompi_coll_portals4_scatter_intra_linear_top(sbuf, scount, sdtype, rbuf, rcount, rdtype,
...
int ompi_coll_portals4_iscatter_intra_fini(ompi_coll_portals4_request_t *request)
{
    int ret, line;

    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:iscatter_intra_fini enter rank %d",
                         request->u.scatter.my_rank));

    /*
     *  cleanup the scatter
     */
    ret = ompi_coll_portals4_scatter_intra_linear_bottom(request->super.req_mpi_object.comm,
                                                         request);
    if (MPI_SUCCESS != ret) {
        line = __LINE__;
        goto err_hdlr;
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:iscatter_intra_fini exit rank %d",
                         request->u.scatter.my_rank));
