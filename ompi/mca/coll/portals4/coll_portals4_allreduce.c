...
static int allreduce_kary_tree_top(const void *sendbuf, void *recvbuf, int count,
                                   MPI_Datatype dtype, MPI_Op op, struct ompi_communicator_t *comm,
                                   ...
                                   mca_coll_portals4_module_t *module)
{
    ...
    if (request->u.allreduce.is_optim) {
        ...
        get_k_ary_tree(COLL_PORTALS4_ALLREDUCE_MAX_CHILDREN, rank, size, PTL_FIRST_RANK, &parent,
                       child, &child_nb);
        ...
        COLL_PORTALS4_SET_BITS(match_bits, ompi_comm_get_cid(comm), 0, 0, COLL_PORTALS4_ALLREDUCE,
                               0, internal_count);

        if ((ret = PtlCTAlloc(mca_coll_portals4_component.ni_h, &request->u.allreduce.trig_ct_h))
            != 0) {
        ...
        if ((ret = PtlMEAppend(mca_coll_portals4_component.ni_h, mca_coll_portals4_component.pt_idx,
                               &me, PTL_PRIORITY_LIST, NULL, &request->u.allreduce.data_me_h))
            != 0) {
            return opal_stderr("PtlMEAppend failed", __FILE__, __LINE__, ret);
        }

        if (child_nb) {
            if ((ret = PtlCTAlloc(mca_coll_portals4_component.ni_h, &request->u.allreduce.ack_ct_h))
                != 0) {
                return opal_stderr("PtlCTAlloc failed", __FILE__, __LINE__, ret);
            }
            ...
            for (i = 0; i < COLL_PORTALS4_ALLREDUCE_MAX_CHILDREN; i++) {
                if (child[i] != PTL_INVALID_RANK) {
                    ...
                    me.options = PTL_ME_OP_PUT | PTL_ME_EVENT_SUCCESS_DISABLE
                                 ...
                                 | PTL_ME_USE_ONCE | PTL_ME_EVENT_CT_COMM;
                    me.match_id.phys.nid = PTL_NID_ANY;
                    me.match_id.phys.pid = PTL_PID_ANY;
                    ...
                    if ((ret = PtlMEAppend(mca_coll_portals4_component.ni_h,
                                           mca_coll_portals4_component.pt_idx, &me,
                                           PTL_PRIORITY_LIST, NULL, &me_h))
                        != 0) {
                        return opal_stderr("PtlMEAppend failed", __FILE__, __LINE__, ret);
                    }
                    ...
                    if ((ret = PtlTriggeredPut(data_md_h, (uint64_t) recvbuf, length,
                                               PTL_NO_ACK_REQ,
                                               ompi_coll_portals4_get_peer(comm, child[i]),
                                               mca_coll_portals4_component.pt_idx, match_bits, 0,
                                               NULL, 0, request->u.allreduce.trig_ct_h,
                                               (rank != PTL_FIRST_RANK) ? child_nb + 2 : child_nb))
                        != 0) {
        ...
        if (rank != PTL_FIRST_RANK) {
            ...
            if ((ret = PtlTriggeredAtomic(data_md_h, (uint64_t) recvbuf, length, PTL_NO_ACK_REQ,
                                          ...
                                          0, ptl_op, ptl_dtype, request->u.allreduce.trig_ct_h,
                                          child_nb + 1))
                != 0) {
            ...
            if ((ret = PtlTriggeredPut(zero_md_h, 0, 0, PTL_NO_ACK_REQ,
                                       ompi_coll_portals4_get_peer(comm, parent),
                                       mca_coll_portals4_component.pt_idx, match_bits_ack, 0, NULL,
                                       0, request->u.allreduce.trig_ct_h, child_nb + 2))
                != 0) {
                return opal_stderr("PtlTriggeredPut failed", __FILE__, __LINE__, ret);
            }
        ...
        if (child_nb) {
            if (is_sync) {
                if ((ret = PtlCTWait(request->u.allreduce.ack_ct_h, child_nb, &ct)) != 0) {
                ...
                if ((ret = PtlTriggeredPut(zero_md_h, 0, 0, PTL_NO_ACK_REQ,
                                           ...
                                           (uintptr_t) request, request->u.allreduce.ack_ct_h,
                                           child_nb))
                    != 0) {
                    return opal_stderr("PtlTriggeredPut failed", __FILE__, __LINE__, ret);
                }
            ...
            if (is_sync) {
                ...
                if ((ret = PtlTriggeredPut(zero_md_h, 0, 0, PTL_NO_ACK_REQ,
                                           ompi_coll_portals4_get_peer(comm, rank),
                                           mca_coll_portals4_component.finish_pt_idx, 0, 0, NULL,
                                           (uintptr_t) request, request->u.allreduce.trig_ct_h,
                                           (rank != PTL_FIRST_RANK) ? 2 : 0))
                    != 0) {
                    return opal_stderr("PtlTriggeredPut failed", __FILE__, __LINE__, ret);
                }
            }
        ...
        if (request->is_sync) {
            if ((module->previous_allreduce) && (module->previous_allreduce_module)) {
                ret = module->previous_allreduce(sendbuf, recvbuf, count, dtype, op, comm,
                                                 module->previous_allreduce_module);
            } else {
                opal_output_verbose(
            ...
            if ((module->previous_iallreduce) && (module->previous_iallreduce_module)) {
                ret = module->previous_iallreduce(sendbuf, recvbuf, count, dtype, op, comm,
                                                  request->fallback_request,
                                                  module->previous_iallreduce_module);
            } else {
                opal_output_verbose(
                    1, ompi_coll_base_framework.framework_output,
...
static int allreduce_kary_tree_bottom(ompi_coll_portals4_request_t *request)
{
    int ret;
                                       ...
                                       mca_coll_base_module_t *module)
{
    ...
    allreduce_kary_tree_top(sendbuf, recvbuf, count, dtype, op, comm, request, portals4_module);
