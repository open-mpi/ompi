...
static int32_t get_tree_numdescendants_of(struct ompi_communicator_t *comm, int vrank)
{
    int max;
...
ompi_coll_portals4_build_in_order_bmtree(struct ompi_communicator_t *comm, int root)
{
    ...
    opal_output_verbose(30, ompi_coll_base_framework.framework_output,
                        "%d: bmtree result - size(%d)  rank(%d)  vrank(%d)  root(%d)  parent(%d) "
                        "vparent(%d)  numkids(%d)  numdescendants(%d)",
                        __LINE__, size, rank, vrank, bmtree->tree_root, bmtree->tree_prev, vparent,
                        bmtree->tree_nextsize, bmtree->tree_numdescendants);
 ...
 */
static int ompi_coll_portals4_destroy_tree(ompi_coll_portals4_tree_t **tree)
{
...
static int setup_gather_buffers_binomial(struct ompi_communicator_t *comm,
                                         ...
                                         mca_coll_portals4_module_t *portals4_module)
{
    ...
    int vrank = VRANK(request->u.gather.my_rank, request->u.gather.root_rank,
                      request->u.gather.size);

    ompi_coll_portals4_create_send_converter(&request->u.gather.send_converter,
                                             request->u.gather.pack_src_buf
    ...
    opal_convertor_get_packed_size(&request->u.gather.send_converter,
    ...
    if (vrank == 0) {
        request->u.gather.gather_bytes = request->u.gather.packed_size
                                         * (ptrdiff_t) request->u.gather.size;
        ...
        opal_output_verbose(30, ompi_coll_base_framework.framework_output,
                            "%s:%d:vrank(%d): root - gather_buf(%p) - "
                            "gather_bytes(%lu)=packed_size(%ld) * size(%d)",
                            __FILE__, __LINE__, vrank, request->u.gather.gather_buf,
                            request->u.gather.gather_bytes, request->u.gather.packed_size,
                            request->u.gather.size);
         ...
         */
        request->u.gather.gather_bytes = request->u.gather.packed_size
                                         * ((ptrdiff_t) bmtree->tree_numdescendants + 1);
        ...
        opal_output_verbose(
            30, ompi_coll_base_framework.framework_output,
            "%s:%d:vrank(%d): nonleaf - gather_buf(%p) - gather_bytes(%lu)=packed_size(%ld) * "
            "(bmtree->tree_numdescendants(%d) + 1)",
            __FILE__, __LINE__, vrank, request->u.gather.gather_buf, request->u.gather.gather_bytes,
        ...
        opal_output_verbose(
            30, ompi_coll_base_framework.framework_output,
            "%s:%d:vrank(%d): leaf - gather_buf(%p) - gather_bytes(%lu)=packed_size(%ld)", __FILE__,
            __LINE__, vrank, request->u.gather.gather_buf, request->u.gather.gather_bytes,
            request->u.gather.packed_size);
    }
...
err_hdlr:
    opal_output(ompi_coll_base_framework.framework_output,
                "%s:%4d:%4d\tError occurred ret=%d, rank %2d", __FILE__, __LINE__, line, ret,
                request->u.gather.my_rank);

    return ret;
}

static int setup_gather_buffers_linear(struct ompi_communicator_t *comm,
                                       ompi_coll_portals4_request_t *request,
                                       mca_coll_portals4_module_t *portals4_module)
{
    int ret, line;
    ...
    if (i_am_root) {
        ...
        /* pack local data into request->u.gather.gather_buf */
        uint64_t gather_buf_offset = (ptrdiff_t) request->u.gather.my_rank
                                     * request->u.gather.packed_size;
        ...
        opal_output_verbose(
            ...
            __FILE__, __LINE__, request->u.gather.my_rank, request->u.gather.gather_buf,
            request->u.gather.gather_bytes, request->u.gather.packed_size, request->u.gather.size);
    } else {
        ...
        opal_output_verbose(
            30, ompi_coll_base_framework.framework_output,
            "%s:%d:rank(%d): leaf - gather_buf(%p) - gather_bytes(%lu)=packed_size(%ld)", __FILE__,
            __LINE__, request->u.gather.my_rank, request->u.gather.gather_buf,
            request->u.gather.gather_bytes, request->u.gather.packed_size);
    }
...
static int setup_gather_handles(struct ompi_communicator_t *comm,
                                ompi_coll_portals4_request_t *request,
                                mca_coll_portals4_module_t *portals4_module)
{
    int ret, line;
    ...
    /**********************************/
    COLL_PORTALS4_SET_BITS(request->u.gather.gather_match_bits, ompi_comm_get_cid(comm), 0, 0,
                           COLL_PORTALS4_GATHER, 0, request->u.gather.coll_count);

    ret = PtlCTAlloc(mca_coll_portals4_component.ni_h, &request->u.gather.gather_cth);
    if (PTL_OK != ret) {
    ...
    ret = PtlMEAppend(mca_coll_portals4_component.ni_h, mca_coll_portals4_component.pt_idx, &me,
                      PTL_PRIORITY_LIST, NULL, &request->u.gather.gather_meh);
    if (PTL_OK != ret) {
        ret = OMPI_ERROR;
        line = __LINE__;
        goto err_hdlr;
...
static int setup_sync_handles(struct ompi_communicator_t *comm,
                              ompi_coll_portals4_request_t *request,
                              mca_coll_portals4_module_t *portals4_module)
{
    int ret, line;
    ...
    /* Setup Sync Handles             */
    /**********************************/
    COLL_PORTALS4_SET_BITS(request->u.gather.sync_match_bits, ompi_comm_get_cid(comm), 0, 1,
                           COLL_PORTALS4_GATHER, 0, request->u.gather.coll_count);

    ret = PtlCTAlloc(mca_coll_portals4_component.ni_h, &request->u.gather.sync_cth);
    if (PTL_OK != ret) {
        ret = OMPI_ERR_TEMP_OUT_OF_RESOURCE;
        line = __LINE__;
        goto err_hdlr;
    ...
    me.ignore_bits = 0;
    ret = PtlMEAppend(mca_coll_portals4_component.ni_h, mca_coll_portals4_component.pt_idx, &me,
                      PTL_PRIORITY_LIST, NULL, &request->u.gather.sync_meh);
    if (PTL_OK != ret) {
        ret = OMPI_ERROR;
        line = __LINE__;
        goto err_hdlr;
...
static int cleanup_gather_handles(ompi_coll_portals4_request_t *request)
{
    int ret, line;
...
static int cleanup_sync_handles(ompi_coll_portals4_request_t *request)
{
    int ret, line;
...
static int ompi_coll_portals4_gather_intra_binomial_top(const void *sbuf, int scount,
                                                        ...
                                                        mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:gather_intra_binomial_top enter rank %d",
                         request->u.gather.my_rank));
    ...
    ompi_datatype_get_extent(request->u.gather.pack_src_dtype, &request->u.gather.pack_src_lb,
    ...
    ompi_datatype_get_extent(request->u.gather.unpack_dst_dtype, &request->u.gather.unpack_dst_lb,
    ...
    if ((root == request->u.gather.my_rank) && (sbuf == MPI_IN_PLACE)) {
        request->u.gather.pack_src_offset = request->u.gather.pack_src_extent
                                            * request->u.gather.pack_src_count
                                            * request->u.gather.my_rank;
    }

    opal_output_verbose(30, ompi_coll_base_framework.framework_output,
                        "%s:%d:vrank(%d): request->u.gather.pack_src_offset(%lu)", __FILE__,
                        __LINE__, vrank, request->u.gather.pack_src_offset);
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "%s:%d: packed_size=%lu, fragment_size=%lu", __FILE__, __LINE__,
                         request->u.gather.packed_size,
                         mca_coll_portals4_component.ni_limits.max_msg_size));

    for (int i = 0; i < bmtree->tree_nextsize; i++) {
        int child_vrank = VRANK(bmtree->tree_next[i], request->u.gather.root_rank,
                                request->u.gather.size);
        int sub_tree_size = get_tree_numdescendants_of(comm, child_vrank) + 1;
        ptl_size_t local_number_of_fragment = ((sub_tree_size * request->u.gather.packed_size)
                                               + mca_coll_portals4_component.ni_limits.max_msg_size
                                               - 1)
                                              / mca_coll_portals4_component.ni_limits.max_msg_size;
        ...
        OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                             ...
                             __FILE__, __LINE__, bmtree->tree_next[i], vrank,
                             request->u.gather.root_rank, sub_tree_size, local_number_of_fragment));
        number_of_fragment_gathered += local_number_of_fragment;
    }

    number_of_fragment_send = (request->u.gather.gather_bytes
                               + mca_coll_portals4_component.ni_limits.max_msg_size - 1)
                              / mca_coll_portals4_component.ni_limits.max_msg_size;
    ...
    if (vrank != 0) {
        ...
        ret = PtlTriggeredCTInc(request->u.gather.gather_cth, sync_incr_event,
                                request->u.gather.sync_cth, 1);
        if (PTL_OK != ret) {
            ret = OMPI_ERROR;
            line = __LINE__;
            goto err_hdlr;
    ...
    if (vrank == 0) {
        ...
        opal_output_verbose(
            ...
            __FILE__, __LINE__, vrank, remote_offset, vrank, vparent,
            request->u.gather.packed_size);

        expected_ops = number_of_fragment_gathered
                       + 1; /* gather puts from each child + a chained RTR */
        ...
        for (ptl_size_t i = 0; i < number_of_fragment_send; i++) {
            ...
            ret = PtlTriggeredPut(request->u.gather.gather_mdh,
                                  request->u.gather.gather_offset + size_sent, frag_size,
                                  PTL_NO_ACK_REQ, ompi_coll_portals4_get_peer(comm, parent),
                                  mca_coll_portals4_component.pt_idx,
                                  request->u.gather.gather_match_bits, remote_offset + size_sent,
                                  NULL, 0, request->u.gather.gather_cth, expected_ops);
            if (PTL_OK != ret) {
                ret = OMPI_ERROR;
                line = __LINE__;
                goto err_hdlr;
    ...
    for (i = 0; i < bmtree->tree_nextsize; i++) {
        ...
        ret = PtlTriggeredPut(request->u.gather.sync_mdh, 0, 0, PTL_NO_ACK_REQ,
                              ompi_coll_portals4_get_peer(comm, child),
                              mca_coll_portals4_component.pt_idx, request->u.gather.sync_match_bits,
                              0, NULL, 0, request->u.gather.gather_cth, expected_ops);
        if (PTL_OK != ret) {
            ret = OMPI_ERROR;
            line = __LINE__;
            goto err_hdlr;
    ...
    if (!request->u.gather.is_sync) {
        ...
        /******************************************/
        ret = PtlTriggeredPut(mca_coll_portals4_component.zero_md_h, 0, 0, PTL_NO_ACK_REQ,
                              ompi_coll_portals4_get_peer(comm, request->u.gather.my_rank),
                              mca_coll_portals4_component.finish_pt_idx, 0, 0, NULL,
                              (uintptr_t) request, request->u.gather.gather_cth, expected_ops);
        if (PTL_OK != ret) {
            ret = OMPI_ERROR;
            line = __LINE__;
            goto err_hdlr;
    ...
    for (i = 0; i < bmtree->tree_nextsize; i++) {
        ...
        ret = PtlTriggeredPut(request->u.gather.sync_mdh, 0, 0, PTL_NO_ACK_REQ,
                              ompi_coll_portals4_get_peer(comm, child),
                              mca_coll_portals4_component.pt_idx, request->u.gather.sync_match_bits,
                              0, NULL, 0, request->u.gather.sync_cth, 0);
        if (PTL_OK != ret) {
            ret = OMPI_ERROR;
            line = __LINE__;
            goto err_hdlr;
    ...
    for (i = 0; i < bmtree->tree_nextsize; i++) {
        int32_t child = bmtree->tree_next[i];
        ret = PtlPut(request->u.gather.sync_mdh, 0, 0, PTL_NO_ACK_REQ,
                     ompi_coll_portals4_get_peer(comm, child), mca_coll_portals4_component.pt_idx,
                     request->u.gather.sync_match_bits, 0, NULL, 0);
        if (PTL_OK != ret) {
            ret = OMPI_ERROR;
            line = __LINE__;
            goto err_hdlr;
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:gather_intra_binomial_top exit rank %d",
                         request->u.gather.my_rank));
...
static int ompi_coll_portals4_gather_intra_linear_top(const void *sbuf, int scount,
                                                      struct ompi_datatype_t *sdtype, void *rbuf,
                                                      int rcount, struct ompi_datatype_t *rdtype,
                                                      int root, struct ompi_communicator_t *comm,
                                                      ompi_coll_portals4_request_t *request,
                                                      mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:gather_intra_linear_top enter rank %d",
                         request->u.gather.my_rank));
    ...
    opal_output_verbose(30, ompi_coll_base_framework.framework_output,
                        "%s:%d:rank(%d): request->u.gather.pack_src_offset(%lu)", __FILE__,
                        __LINE__, request->u.gather.my_rank, request->u.gather.pack_src_offset);
    ...
    number_of_fragment = (request->u.gather.packed_size
                          > mca_coll_portals4_component.ni_limits.max_msg_size)
                             ? (request->u.gather.packed_size
                                + mca_coll_portals4_component.ni_limits.max_msg_size - 1)
                                   / mca_coll_portals4_component.ni_limits.max_msg_size
    ...
    opal_output_verbose(90, ompi_coll_base_framework.framework_output,
                        "%s:%d:rank %d:number_of_fragment = %lu", __FILE__, __LINE__,
                        request->u.gather.my_rank, number_of_fragment);
    ...
    if (i_am_root) {
        ...
        expected_ops = (request->u.gather.size - 1)
                       * number_of_fragment; /* gather put from all other ranks */
        expected_acks = 0;
        ...
        opal_output_verbose(30, ompi_coll_base_framework.framework_output,
                            ...
                            __FILE__, __LINE__, request->u.gather.my_rank, remote_offset,
                            request->u.gather.my_rank, request->u.gather.packed_size);
        ...
        for (ptl_size_t j = 0; j < number_of_fragment; j++) {
            ...
            opal_output_verbose(10, ompi_coll_base_framework.framework_output,
                                ...
                                __FILE__, __LINE__, request->u.gather.my_rank, j, split_offset,
                                frag_size);

            ret = PtlTriggeredPut(request->u.gather.gather_mdh,
                                  request->u.gather.gather_offset + split_offset, frag_size,
                                  PTL_NO_ACK_REQ,
                                  ompi_coll_portals4_get_peer(comm, request->u.gather.root_rank),
                                  mca_coll_portals4_component.pt_idx,
                                  request->u.gather.gather_match_bits, remote_offset + split_offset,
                                  NULL, 0, request->u.gather.gather_cth, expected_ops);
            if (PTL_OK != ret) {
                ret = OMPI_ERROR;
                line = __LINE__;
    ...
    if (i_am_root) {
        for (i = 0; i < request->u.gather.size; i++) {
            if (i == request->u.gather.root_rank) {
                continue;
            }
            ret = PtlTriggeredPut(request->u.gather.sync_mdh, 0, 0, PTL_NO_ACK_REQ,
                                  ...
                                  mca_coll_portals4_component.pt_idx,
                                  request->u.gather.sync_match_bits, 0, NULL, 0,
                                  request->u.gather.gather_cth, expected_ops);
            if (PTL_OK != ret) {
                ret = OMPI_ERROR;
                line = __LINE__;
                goto err_hdlr;
    ...
    if (i_am_root) {
        for (i = 0; i < request->u.gather.size; i++) {
            ...
            ret = PtlPut(request->u.gather.sync_mdh, 0, 0, PTL_NO_ACK_REQ,
                         ompi_coll_portals4_get_peer(comm, i), mca_coll_portals4_component.pt_idx,
                         request->u.gather.sync_match_bits, 0, NULL, 0);
            if (PTL_OK != ret) {
                ret = OMPI_ERROR;
                line = __LINE__;
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:gather_intra_linear_top exit rank %d",
                         request->u.gather.my_rank));
...
static int ompi_coll_portals4_gather_intra_binomial_bottom(struct ompi_communicator_t *comm,
                                                           ompi_coll_portals4_request_t *request)
{
    int ret, line;
    int i;
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:gather_intra_binomial_bottom enter rank %d",
                         request->u.gather.my_rank));
    ...
    if (request->u.gather.my_rank == request->u.gather.root_rank) {
        ...
        for (i = 0; i < request->u.gather.size; i++) {
            uint64_t offset = request->u.gather.unpack_dst_extent
                              * request->u.gather.unpack_dst_count
                              * ((request->u.gather.my_rank + i) % request->u.gather.size);

            opal_output_verbose(30, ompi_coll_base_framework.framework_output,
                                "%s:%d:rank(%d): offset(%lu)", __FILE__, __LINE__,
                                request->u.gather.my_rank, offset);

            ompi_coll_portals4_create_recv_converter(&request->u.gather.recv_converter,
                                                     request->u.gather.unpack_dst_buf + offset,
                                                     ompi_comm_peer_lookup(comm, request->u.gather
                                                                                     .my_rank),
                                                     request->u.gather.unpack_dst_count,
                                                     request->u.gather.unpack_dst_dtype);

            iov.iov_len = request->u.gather.packed_size;
            iov.iov_base = (IOVBASE_TYPE *) ((char *) request->u.gather.gather_buf
                                             + (request->u.gather.packed_size * i));
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:gather_intra_binomial_bottom exit rank %d",
                         request->u.gather.my_rank));
...
static int ompi_coll_portals4_gather_intra_linear_bottom(struct ompi_communicator_t *comm,
                                                         ompi_coll_portals4_request_t *request)
{
    int ret, line;
    int i;
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:gather_intra_linear_bottom enter rank %d",
                         request->u.gather.my_rank));
    ...
    if (request->u.gather.my_rank == request->u.gather.root_rank) {
        ...
        for (i = 0; i < request->u.gather.size; i++) {
            ompi_coll_portals4_create_recv_converter(&request->u.gather.recv_converter,
                                                     request->u.gather.unpack_dst_buf
                                                         + (request->u.gather.unpack_dst_extent
                                                            * request->u.gather.unpack_dst_count
                                                            * i),
                                                     ompi_comm_peer_lookup(comm, request->u.gather
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:gather_intra_linear_bottom exit rank %d",
                         request->u.gather.my_rank));
                                    ...
                                    mca_coll_base_module_t *module)
{
    ...
    if (1 == mca_coll_portals4_component.use_binomial_gather_algorithm) {
        ret = ompi_coll_portals4_gather_intra_binomial_top(sbuf, scount, sdtype, rbuf, rcount,
                                                           rdtype, root, comm, request, module);
        if (MPI_SUCCESS != ret) {
            line = __LINE__;
            goto err_hdlr;
        }
        ...
        ret = ompi_coll_portals4_gather_intra_linear_top(sbuf, scount, sdtype, rbuf, rcount, rdtype,
                                                         root, comm, request, module);
        if (MPI_SUCCESS != ret) {
            line = __LINE__;
            goto err_hdlr;
        }
...
int ompi_coll_portals4_igather_intra_fini(ompi_coll_portals4_request_t *request)
{
    int ret, line;

    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:igather_intra_fini enter rank %d",
                         request->u.gather.my_rank));
     ...
     */
    if (1 == mca_coll_portals4_component.use_binomial_gather_algorithm) {
        ret = ompi_coll_portals4_gather_intra_binomial_bottom(request->super.req_mpi_object.comm,
                                                              request);
        if (MPI_SUCCESS != ret) {
            line = __LINE__;
            goto err_hdlr;
        ...
        ret = ompi_coll_portals4_gather_intra_linear_bottom(request->super.req_mpi_object.comm,
                                                            request);
        if (MPI_SUCCESS != ret) {
            line = __LINE__;
            goto err_hdlr;
    ...
    OPAL_OUTPUT_VERBOSE((10, ompi_coll_base_framework.framework_output,
                         "coll:portals4:igather_intra_fini exit rank %d",
                         request->u.gather.my_rank));
