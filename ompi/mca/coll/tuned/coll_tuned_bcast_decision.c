...
int ompi_coll_tuned_bcast_intra_check_forced_init(
    coll_tuned_force_algorithm_mca_param_indices_t *mca_param_indices)
{
    ...
    mca_param_indices->algorithm_param_index = mca_base_component_var_register(
        &mca_coll_tuned_component.super.collm_version, "bcast_algorithm",
        "Which bcast algorithm is used. Can be locked down to choice of: 0 ignore, 1 basic linear, "
        "2 chain, 3: pipeline, 4: split binary tree, 5: binary tree, 6: binomial tree, 7: knomial "
        "tree, 8: scatter_allgather, 9: scatter_allgather_ring. "
        ...
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_bcast_forced_algorithm);
    ...
    mca_param_indices->segsize_param_index = mca_base_component_var_register(
        &mca_coll_tuned_component.super.collm_version, "bcast_algorithm_segmentsize",
        ...
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_bcast_segment_size);
    ...
    mca_param_indices->tree_fanout_param_index = mca_base_component_var_register(
        &mca_coll_tuned_component.super.collm_version, "bcast_algorithm_tree_fanout",
        "Fanout for n-tree used for bcast algorithms. Only has meaning if algorithm is forced and "
        "supports n-tree topo based operation.",
        MCA_BASE_VAR_TYPE_INT, NULL, 0, MCA_BASE_VAR_FLAG_SETTABLE, OPAL_INFO_LVL_5,
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_bcast_tree_fanout);
    ...
    mca_param_indices->chain_fanout_param_index = mca_base_component_var_register(
        &mca_coll_tuned_component.super.collm_version, "bcast_algorithm_chain_fanout",
        "Fanout for chains used for bcast algorithms. Only has meaning if algorithm is forced and "
        "supports chain topo based operation.",
        MCA_BASE_VAR_TYPE_INT, NULL, 0, MCA_BASE_VAR_FLAG_SETTABLE, OPAL_INFO_LVL_5,
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_bcast_chain_fanout);
...
int ompi_coll_tuned_bcast_intra_do_this(void *buf, int count, struct ompi_datatype_t *dtype,
                                        ...
                                        int segsize)
{
    ...
    case (2):
        return ompi_coll_base_bcast_intra_chain(buf, count, dtype, root, comm, module, segsize,
                                                faninout);
    case (3):
        return ompi_coll_base_bcast_intra_pipeline(buf, count, dtype, root, comm, module, segsize);
    case (4):
        return ompi_coll_base_bcast_intra_split_bintree(buf, count, dtype, root, comm, module,
                                                        segsize);
    case (5):
    ...
    case (7):
        return ompi_coll_base_bcast_intra_knomial(buf, count, dtype, root, comm, module, segsize,
                                                  coll_tuned_bcast_knomial_radix);
    case (8):
        return ompi_coll_base_bcast_intra_scatter_allgather(buf, count, dtype, root, comm, module,
                                                            segsize);
    case (9):
        return ompi_coll_base_bcast_intra_scatter_allgather_ring(buf, count, dtype, root, comm,
                                                                 module, segsize);
    } /* switch */
    OPAL_OUTPUT(
