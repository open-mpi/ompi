...
static const mca_base_var_enum_value_t reduce_algorithms[] = {{0, "ignore"},
                                                              {1, "linear"},
                                                              {2, "chain"},
                                                              {3, "pipeline"},
                                                              {4, "binary"},
    ...
    coll_tuned_force_algorithm_mca_param_indices_t *mca_param_indices)
{
    ...
    mca_param_indices->algorithm_param_index = mca_base_component_var_register(
        &mca_coll_tuned_component.super.collm_version, "reduce_algorithm",
        "Which reduce algorithm is used. Can be locked down to choice of: 0 ignore, 1 linear, 2 "
        "chain, 3 pipeline, 4 binary, 5 binomial, 6 in-order binary, 7 rabenseifner. "
        ...
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_reduce_forced_algorithm);
    ...
    mca_param_indices->segsize_param_index = mca_base_component_var_register(
        ...
        "Segment size in bytes used by default for reduce algorithms. Only has meaning if "
        ...
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_reduce_segment_size);
    ...
    mca_param_indices->tree_fanout_param_index = mca_base_component_var_register(
        &mca_coll_tuned_component.super.collm_version, "reduce_algorithm_tree_fanout",
        "Fanout for n-tree used for reduce algorithms. Only has meaning if algorithm is forced and "
        "supports n-tree topo based operation.",
        MCA_BASE_VAR_TYPE_INT, NULL, 0, MCA_BASE_VAR_FLAG_SETTABLE, OPAL_INFO_LVL_5,
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_reduce_tree_fanout);
    ...
    mca_param_indices->chain_fanout_param_index = mca_base_component_var_register(
        &mca_coll_tuned_component.super.collm_version, "reduce_algorithm_chain_fanout",
        "Fanout for chains used for reduce algorithms. Only has meaning if algorithm is forced and "
        "supports chain topo based operation.",
        MCA_BASE_VAR_TYPE_INT, NULL, 0, MCA_BASE_VAR_FLAG_SETTABLE, OPAL_INFO_LVL_5,
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_reduce_chain_fanout);

    coll_tuned_reduce_max_requests = 0; /* no limit for reduce by default */
    mca_param_indices->max_requests_param_index = mca_base_component_var_register(
        &mca_coll_tuned_component.super.collm_version, "reduce_algorithm_max_requests",
        "Maximum number of outstanding send requests on leaf nodes. 0 means no limit.",
        MCA_BASE_VAR_TYPE_INT, NULL, 0, MCA_BASE_VAR_FLAG_SETTABLE, OPAL_INFO_LVL_5,
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_reduce_max_requests);
    ...
    if (coll_tuned_reduce_max_requests < 0) {
        if (0 == ompi_comm_rank(MPI_COMM_WORLD)) {
            opal_output(0, "Maximum outstanding requests must be positive number or 0.  "
                           "Initializing to 0 (no limit).\n");
        }
...
int ompi_coll_tuned_reduce_intra_do_this(const void *sbuf, void *rbuf, int count,
                                         ...
                                         int faninout, int segsize, int max_requests)
{
    OPAL_OUTPUT(
    ...
    switch (algorithm) {
    case (0):
        return ompi_coll_tuned_reduce_intra_dec_fixed(sbuf, rbuf, count, dtype, op, root, comm,
                                                      module);
    case (1):
        return ompi_coll_base_reduce_intra_basic_linear(sbuf, rbuf, count, dtype, op, root, comm,
                                                        module);
    case (2):
        return ompi_coll_base_reduce_intra_chain(sbuf, rbuf, count, dtype, op, root, comm, module,
                                                 segsize, faninout, max_requests);
    case (3):
        return ompi_coll_base_reduce_intra_pipeline(sbuf, rbuf, count, dtype, op, root, comm,
                                                    module, segsize, max_requests);
    case (4):
        return ompi_coll_base_reduce_intra_binary(sbuf, rbuf, count, dtype, op, root, comm, module,
    ...
    case (6):
        return ompi_coll_base_reduce_intra_in_order_binary(sbuf, rbuf, count, dtype, op, root, comm,
                                                           module, segsize, max_requests);
    case (7):
        return ompi_coll_base_reduce_intra_redscat_gather(sbuf, rbuf, count, dtype, op, root, comm,
                                                          module);
    } /* switch */
    OPAL_OUTPUT(
        (ompi_coll_tuned_stream,
