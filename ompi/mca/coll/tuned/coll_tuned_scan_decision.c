...
static const mca_base_var_enum_value_t scan_algorithms[] = {{0, "ignore"},
                                                            {1, "linear"},
    ...
    coll_tuned_force_algorithm_mca_param_indices_t *mca_param_indices)
{
    ...
    mca_param_indices->algorithm_param_index = mca_base_component_var_register(
        &mca_coll_tuned_component.super.collm_version, "scan_algorithm",
        "Which scan algorithm is used. Can be locked down to choice of: 0 ignore, 1 linear, 2 "
        "recursive_doubling. "
        ...
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_scan_forced_algorithm);
...
{
    OPAL_OUTPUT(
        (ompi_coll_tuned_stream, "coll:tuned:scan_intra_do_this selected algorithm %d", algorithm));
    ...
    case (2):
        return ompi_coll_base_scan_intra_recursivedoubling(sbuf, rbuf, count, dtype, op, comm,
                                                           module);
    } /* switch */
    OPAL_OUTPUT(
