...
static int tuned_module_enable(mca_coll_base_module_t *module, struct ompi_communicator_t *comm);
/*
 * Initial query function that is invoked during MPI_INIT, allowing
 * this component to disqualify itself if it doesn't support the
 * required level of thread support.
 */
int ompi_coll_tuned_init_query(bool enable_progress_threads, bool enable_mpi_threads)
{
    return OMPI_SUCCESS;
}
 ...
 */
mca_coll_base_module_t *ompi_coll_tuned_comm_query(struct ompi_communicator_t *comm, int *priority)
{
    ...
    *priority = ompi_coll_tuned_priority;
    ...
    tuned_module->super.coll_reduce_scatter_block
...
static int ompi_coll_tuned_forced_getvalues(enum COLLTYPE type,
...
#define COLL_TUNED_EXECUTE_IF_DYNAMIC(TMOD, TYPE, EXECUTE)                                        \
    {                                                                                             \
        ...
        if (NULL != mca_coll_tuned_component.all_base_rules) {                                    \
            (TMOD)->com_rules[(TYPE)] = ompi_coll_tuned_get_com_rule_ptr(mca_coll_tuned_component \
                                                                             .all_base_rules,     \
                                                                         (TYPE), size);           \
        ...
        if (1 == need_dynamic_decision) {                                                         \
            OPAL_OUTPUT(                                                                          \
                (ompi_coll_tuned_stream, "coll:tuned: enable dynamic selection for " #TYPE));     \
            EXECUTE;                                                                              \
        }                                                                                         \
    }
