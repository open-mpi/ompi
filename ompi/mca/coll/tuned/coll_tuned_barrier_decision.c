...
static const mca_base_var_enum_value_t barrier_algorithms[] = {{0, "ignore"},
                                                               {1, "linear"},
                                                               {2, "double_ring"},
    ...
    coll_tuned_force_algorithm_mca_param_indices_t *mca_param_indices)
{
    ...
    mca_param_indices->algorithm_param_index = mca_base_component_var_register(
        &mca_coll_tuned_component.super.collm_version, "barrier_algorithm",
        "Which barrier algorithm is used. Can be locked down to choice of: 0 ignore, 1 linear, 2 "
        "double ring, 3: recursive doubling 4: bruck, 5: two proc only, 6: tree. "
        ...
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_barrier_forced_algorithm);
                                          ...
                                          int faninout, int segsize)
{
    ...
    case (2):
        return ompi_coll_base_barrier_intra_doublering(comm, module);
    case (3):
    ...
    case (6):
        return ompi_coll_base_barrier_intra_tree(comm, module);
    } /* switch */
    OPAL_OUTPUT(
