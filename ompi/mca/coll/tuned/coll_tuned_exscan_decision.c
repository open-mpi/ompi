...
static const mca_base_var_enum_value_t exscan_algorithms[] = {{0, "ignore"},
                                                              {1, "linear"},
...
int ompi_coll_tuned_exscan_intra_check_forced_init(
    coll_tuned_force_algorithm_mca_param_indices_t *mca_param_indices)
{
    ...
    mca_param_indices->algorithm_param_index = mca_base_component_var_register(
        &mca_coll_tuned_component.super.collm_version, "exscan_algorithm",
        "Which exscan algorithm is used. Can be locked down to choice of: 0 ignore, 1 linear, 2 "
        "recursive_doubling. "
        ...
        MCA_BASE_VAR_SCOPE_ALL, &coll_tuned_exscan_forced_algorithm);
                                         ...
                                         mca_coll_base_module_t *module, int algorithm)
{
    ...
    case (2):
        return ompi_coll_base_exscan_intra_recursivedoubling(sbuf, rbuf, count, dtype, op, comm,
                                                             module);
    } /* switch */
    OPAL_OUTPUT(
