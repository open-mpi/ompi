                                                        ...
                                                        struct ompi_communicator_t *comm) 
{
   ...
   if (MPI_IN_PLACE == sbuf) {
      ...
      if (root == rank) {
         ...
         err = comm->c_coll.coll_reduce(tmprbuf, NULL, total_count, dtype, op, 
                                        root, comm);
      }
   } else {
      if (root == rank) {
      ...
      err = comm->c_coll.coll_reduce (sbuf, tmprbuf, total_count, dtype, op, 
                                      root, comm);
   }
   if (MPI_SUCCESS != err) {
                                           ...
                                           struct ompi_communicator_t *comm)
{
   OPAL_OUTPUT((ompi_coll_tuned_stream,"coll:tuned:reduce_scatter_intra_do_forced selected algorithm %d", 
                comm->c_coll_selected_data->user_forced[REDUCESCATTER].algorithm));
   
   switch (comm->c_coll_selected_data->user_forced[REDUCESCATTER].algorithm) {
   case (0): return ompi_coll_tuned_reduce_scatter_intra_dec_fixed (sbuf, rbuf,
                                                                    rcounts, 
                                                                    dtype, 
                                                                    op, 
                                                                    comm);
   case (1): return ompi_coll_tuned_reduce_scatter_intra_nonoverlapping(sbuf,
                                                                        rbuf,
                                                                        rcounts,
                                                                        dtype, 
                                                                        op, 
                                                                        comm);
   case (2): return ompi_coll_tuned_reduce_scatter_intra_basic_recursivehalving(sbuf,
                                                                          rbuf,
                                                                          rcounts,
                                                                          dtype,
                                                                          op, 
                                                                          comm);
   case (3): return ompi_coll_tuned_reduce_scatter_intra_ring (sbuf, rbuf, 
                                                               rcounts, dtype, 
                                                               op, comm);
   default:
      OPAL_OUTPUT((ompi_coll_tuned_stream,"coll:tuned:reduce_scatter_intra_do_forced attempt to select algorithm %d when only 0-%d is valid?",
                   comm->c_coll_selected_data->user_forced[REDUCESCATTER].algorithm, ompi_coll_tuned_forced_max_algorithms[REDUCESCATTER]));
      return (MPI_ERR_ARG);
   } /* switch */
}
                                                 ...
                                                 int algorithm, int faninout, int segsize)
{
   ...
   switch (algorithm) {
   case (0): return ompi_coll_tuned_reduce_scatter_intra_dec_fixed (sbuf, 
                                                                    rbuf,
                                                                    rcounts, 
                                                                    dtype, 
                                                                    op, 
