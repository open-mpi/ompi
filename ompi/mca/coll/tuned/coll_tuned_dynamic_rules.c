...
ompi_coll_msg_rule_t *ompi_coll_tuned_mk_msg_rules(int n_msg_rules, int alg_rule_id,
                                                   int com_rule_id, int mpi_comsize)
{
    int i;
...
int ompi_coll_tuned_dump_msg_rule(ompi_coll_msg_rule_t *msg_p)
{
    ...
    OPAL_OUTPUT((ompi_coll_tuned_stream, "alg_id %3d\tcom_id %3d\tcom_size %3d\tmsg_id %3d\t",
                 msg_p->alg_rule_id, msg_p->com_rule_id, msg_p->mpi_comsize, msg_p->msg_rule_id));

    OPAL_OUTPUT(
        (ompi_coll_tuned_stream,
         "msg_size %10lu -> algorithm %2d\ttopo in/out %2d\tsegsize %5ld\tmax_requests %4d\n",
...
int ompi_coll_tuned_dump_com_rule(ompi_coll_com_rule_t *com_p)
{
    ...
    OPAL_OUTPUT((ompi_coll_tuned_stream, "alg_id %3d\tcom_id %3d\tcom_size %3d\t",
                 com_p->alg_rule_id, com_p->com_rule_id, com_p->mpi_comsize));
    ...
    if (com_p->n_msg_sizes) {
        ...
        if (!msg_p) {
            OPAL_OUTPUT((ompi_coll_tuned_stream,
                         "attempt to free NULL msg_rules when msg count was %d\n",
                         com_p->n_msg_sizes));
            rc = -1; /* some error */
        } else {
...
int ompi_coll_tuned_free_coms_in_alg_rule(ompi_coll_alg_rule_t *alg_p)
{
    ...
    if (alg_p->n_com_sizes) {
        ...
        if (!com_p) {
            OPAL_OUTPUT((ompi_coll_tuned_stream,
                         "attempt to free NULL com_rules when com count was %d\n",
                         alg_p->n_com_sizes));
        } else {
 ...
 *
 */
ompi_coll_com_rule_t *ompi_coll_tuned_get_com_rule_ptr(ompi_coll_alg_rule_t *rules, int alg_id,
                                                       int mpi_comsize)
{
 ...
 * and a (total_)msg_size and it returns (0) and a algorithm to use and a recommended topo faninout
 * and segment size all based on the user supplied rules
 *
 ...
 * else if no rules match we return '0' + '0,0' or used fixed decision table with no topo chand and
 * no segmentation of users data.. shame.
 *
                                             ...
                                             int *result_segsize, int *max_requests)
{
    ...
    while (i < base_com_rule->n_msg_sizes) {
        /*       OPAL_OUTPUT((ompi_coll_tuned_stream,"checking mpi_msgsize %d against com_id %d
         * msg_id %d index %d msg_size %d",  */
