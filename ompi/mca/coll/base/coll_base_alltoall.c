    ...
    for (i = 0; i < size; ++i) {
        for (j = i + 1; j < size; ++j) {
            if (i == rank) {
                ...
                err = MCA_PML_CALL(irecv((char *) rbuf + max_size * j, rcount, rdtype, j,
                ...
                err = MCA_PML_CALL(send((char *) tmp_buffer, rcount, rdtype, j,
                ...
                /* Exchange data with the peer */
                err = MCA_PML_CALL(irecv((char *) rbuf + max_size * i, rcount, rdtype, i,
                                         MCA_COLL_BASE_TAG_ALLTOALL, comm, &req));
                if (MPI_SUCCESS != err) {
                    line = __LINE__;
                    goto error_hndl;
                ...
                err = MCA_PML_CALL(send((char *) tmp_buffer, rcount, rdtype, i,
                                        MCA_COLL_BASE_TAG_ALLTOALL, MCA_PML_BASE_SEND_STANDARD,
                                        comm));
                if (MPI_SUCCESS != err) {
                    line = __LINE__;
                                           ...
                                           mca_coll_base_module_t *module)
{
    ...
    if (MPI_IN_PLACE == sbuf) {
        return mca_coll_base_alltoall_intra_basic_inplace(rbuf, rcount, rdtype, comm, module);
    }
    ...
    for (step = 1; step < size + 1; step++) {
        ...
        /* send and receive */
        err = ompi_coll_base_sendrecv(tmpsend, scount, sdtype, sendto, MCA_COLL_BASE_TAG_ALLTOALL,
                                        ...
                                        mca_coll_base_module_t *module)
{
    ...
    err = ompi_datatype_copy_content_same_ddt(sdtype,
                                              (int32_t)((ptrdiff_t)(size - rank)
                                                        * (ptrdiff_t) scount),
                                              tmpbuf,
                                              ((char *) sbuf)
                                                  + (ptrdiff_t) rank * (ptrdiff_t) scount * sext);
    if (err < 0) {
        line = __LINE__;
        err = -1;
        goto err_hndl;
    ...
    for (distance = 1; distance < size; distance <<= 1) {
        ...
        /* Sendreceive */
        err = ompi_coll_base_sendrecv(tmpbuf, 1, new_ddt, sendto, MCA_COLL_BASE_TAG_ALLTOALL, rbuf,
                                      1, new_ddt, recvfrom, MCA_COLL_BASE_TAG_ALLTOALL, comm,
    ...
    for (i = 0; i < size; i++) {
        ...
        err = ompi_datatype_copy_content_same_ddt(rdtype, (int32_t) rcount,
                                                  ((char *) rbuf)
                                                      + ((ptrdiff_t)((rank - i + size) % size)
                                                         * (ptrdiff_t) rcount * rext),
                                                  tmpbuf
                                                      + (ptrdiff_t) i * (ptrdiff_t) rcount * rext);
        if (err < 0) {
            line = __LINE__;
            err = -1;
            goto err_hndl;
                                              ...
                                              int max_outstanding_reqs)
{
    ...
    total_reqs = (((max_outstanding_reqs > (size - 1)) || (max_outstanding_reqs <= 0))
                      ? (size - 1)
    ...
    for (nreqs = 0, nrreqs = 0, ri = (rank + 1) % size; nreqs < total_reqs;
         ri = (ri + 1) % size, ++nrreqs) {
        error = MCA_PML_CALL(irecv(prcv + (ptrdiff_t) ri * rext, rcount, rdtype, ri,
    ...
    for (nsreqs = 0, si = (rank + size - 1) % size; nreqs < 2 * total_reqs;
        ...
        error = MCA_PML_CALL(isend(psnd + (ptrdiff_t) si * sext, scount, sdtype, si,
                                   MCA_COLL_BASE_TAG_ALLTOALL, MCA_PML_BASE_SEND_STANDARD, comm,
                                   &reqs[nreqs]));
        nreqs++;
        if (MPI_SUCCESS != error) {
            line = __LINE__;
    ...
    if (nreqs == 2 * (size - 1)) {
        ...
        */
        ...
        while (ncreqs < 2 * (size - 1)) {
            int completed;
            error = ompi_request_wait_any(2 * total_reqs, reqs, &completed, MPI_STATUS_IGNORE);
            if (MPI_SUCCESS != error) {
                line = __LINE__;
                goto error_hndl;
            }
            ...
            if (completed < total_reqs) {
                if (nrreqs < (size - 1)) {
                    error = MCA_PML_CALL(irecv(prcv + (ptrdiff_t) ri * rext, rcount, rdtype, ri,
                                               MCA_COLL_BASE_TAG_ALLTOALL, comm, &reqs[completed]));
                    if (MPI_SUCCESS != error) {
                        line = __LINE__;
                        goto error_hndl;
                    }
                ...
                if (nsreqs < (size - 1)) {
                    error = MCA_PML_CALL(isend(psnd + (ptrdiff_t) si * sext, scount, sdtype, si,
                                               ...
                                               MCA_PML_BASE_SEND_STANDARD, comm, &reqs[completed]));
                    if (MPI_SUCCESS != error) {
                        line = __LINE__;
                        goto error_hndl;
                    }
                                            ...
                                            mca_coll_base_module_t *module)
{
    ...
    /* send and receive */
    err = ompi_coll_base_sendrecv(tmpsend, scount, sdtype, remote, MCA_COLL_BASE_TAG_ALLTOALL,
                                  tmprecv, rcount, rdtype, remote, MCA_COLL_BASE_TAG_ALLTOALL, comm,
                                  MPI_STATUS_IGNORE, rank);
    if (err != MPI_SUCCESS) {
        line = __LINE__;
        goto err_hndl;
                                               ...
                                               mca_coll_base_module_t *module)
{
    ...
    for (nreqs = 0, i = (rank + 1) % size; i != rank; i = (i + 1) % size, ++rreq) {
        nreqs++;
        err = MCA_PML_CALL(irecv_init(prcv + (ptrdiff_t) i * rcvinc, rcount, rdtype, i,
     ...
     */
    sreq = rreq;
    for (i = (rank + size - 1) % size; i != rank; i = (i + size - 1) % size, ++sreq) {
        nreqs++;
        err = MCA_PML_CALL(isend_init(psnd + (ptrdiff_t) i * sndinc, scount, sdtype, i,
                                      MCA_COLL_BASE_TAG_ALLTOALL, MCA_PML_BASE_SEND_STANDARD, comm,
                                      sreq));
        if (MPI_SUCCESS != err) {
            line = __LINE__;
