 ...
 */
static inline int ompi_coll_base_sendrecv_zero(int dest, int stag, int source, int rtag,
                                               MPI_Comm comm)

{
    ...
    /* post new irecv */
    rc = MCA_PML_CALL(irecv(NULL, 0, MPI_BYTE, source, rtag, comm, &req));
    if (MPI_SUCCESS != rc) {
        line = __LINE__;
        goto error_handler;
    }
    ...
    rc = MCA_PML_CALL(send(NULL, 0, MPI_BYTE, dest, stag, MCA_PML_BASE_SEND_STANDARD, comm));
    if (MPI_SUCCESS != rc) {
        line = __LINE__;
        goto error_handler;
    }
                                            ...
                                            mca_coll_base_module_t *module)
{
    ...
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                 "ompi_coll_base_barrier_intra_doublering rank %d", rank));

    left = ((rank - 1) % size);
    right = ((rank + 1) % size);

    if (rank > 0) { /* receive message from the left */
        err = MCA_PML_CALL(recv((void *) NULL, 0, MPI_BYTE, left, MCA_COLL_BASE_TAG_BARRIER, comm,
                                MPI_STATUS_IGNORE));
        if (err != MPI_SUCCESS) {
            line = __LINE__;
            goto err_hndl;
                                                   ...
                                                   mca_coll_base_module_t *module)
{
    ...
    rank = ompi_comm_rank(comm);
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                 "ompi_coll_base_barrier_intra_recursivedoubling rank %d", rank));
    ...
    if (adjsize != size) {
        if (rank >= adjsize) {
            ...
            remote = rank - adjsize;
            err = ompi_coll_base_sendrecv_zero(remote, MCA_COLL_BASE_TAG_BARRIER, remote,
    ...
    if (adjsize != size) {
        if (rank < (size - adjsize)) {
            ...
            remote = rank + adjsize;
            err = MCA_PML_CALL(send((void *) NULL, 0, MPI_BYTE, remote, MCA_COLL_BASE_TAG_BARRIER,
                                    MCA_PML_BASE_SEND_SYNCHRONOUS, comm));
                                       ...
                                       mca_coll_base_module_t *module)
{
    ...
    for (distance = 1; distance < size; distance <<= 1) {
        ...
        /* send message to lower ranked node */
        err = ompi_coll_base_sendrecv_zero(to, MCA_COLL_BASE_TAG_BARRIER, from,
                                           MCA_COLL_BASE_TAG_BARRIER, comm);
        if (err != MPI_SUCCESS) {
            line = __LINE__;
            goto err_hndl;
                                              ...
                                              mca_coll_base_module_t *module)
{
    ...
    if (rank > 0) {
        ...
        err = MCA_PML_CALL(
            recv(NULL, 0, MPI_BYTE, 0, MCA_COLL_BASE_TAG_BARRIER, comm, MPI_STATUS_IGNORE));
        if (MPI_SUCCESS != err) {
            line = __LINE__;
            goto err_hndl;
        }
    ...
    /* The root collects and broadcasts the messages. */
        ...
        for (i = 1; i < size; ++i) {
            err = MCA_PML_CALL(irecv(NULL, 0, MPI_BYTE, MPI_ANY_SOURCE, MCA_COLL_BASE_TAG_BARRIER,
                                     comm, &(requests[i])));
            if (MPI_SUCCESS != err) {
                line = __LINE__;
                goto err_hndl;
...
#if OPAL_ENABLE_FT_MPI
        if (MPI_ERR_PROC_FAILED_PENDING
            == err) { /* do not return any-source errors from a collective */
                                      ...
                                      mca_coll_base_module_t *module)
{
    ...
    rank = ompi_comm_rank(comm);
    OPAL_OUTPUT(
        (ompi_coll_base_framework.framework_output, "ompi_coll_base_barrier_intra_tree %d", rank));
    ...
    for (jump = 1; jump < depth; jump <<= 1) {
        ...
        if (!(partner & (jump - 1)) && partner < size) {
            if (partner > rank) {
                err = MCA_PML_CALL(recv(NULL, 0, MPI_BYTE, partner, MCA_COLL_BASE_TAG_BARRIER, comm,
                                        MPI_STATUS_IGNORE));
                if (MPI_SUCCESS != err)
                    return err;
            } else if (partner < rank) {
                err = MCA_PML_CALL(send(NULL, 0, MPI_BYTE, partner, MCA_COLL_BASE_TAG_BARRIER,
                                        MCA_PML_BASE_SEND_STANDARD, comm));
                if (MPI_SUCCESS != err)
                    return err;
            }
