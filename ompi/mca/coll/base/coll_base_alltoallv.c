...
int mca_coll_base_alltoallv_intra_basic_inplace(const void *rbuf, const int *rcounts,
    ...
    for (i = 0; i < size; ++i) {
        for (j = i + 1; j < size; ++j) {
            if (i == rank && 0 != rcounts[j]) {
                ...
                err = ompi_datatype_copy_content_same_ddt(rdtype, rcounts[j], tmp_buffer,
                                                          (char *) rbuf + rdisps[j] * ext);
                if (MPI_SUCCESS != err) {
                    goto error_hndl;
                }

                /* Exchange data with the peer */
                err = ompi_coll_base_sendrecv_actual((void *) tmp_buffer, rcounts[j], rdtype, j,
                                                     MCA_COLL_BASE_TAG_ALLTOALLV,
                                                     (char *) rbuf + rdisps[j] * ext, rcounts[j],
                                                     rdtype, j, MCA_COLL_BASE_TAG_ALLTOALLV, comm,
                                                     MPI_STATUS_IGNORE);
                if (MPI_SUCCESS != err) {
                    goto error_hndl;
                }
                ...
                /* Copy the data into the temporary buffer */
                err = ompi_datatype_copy_content_same_ddt(rdtype, rcounts[i], tmp_buffer,
                                                          (char *) rbuf + rdisps[i] * ext);
                if (MPI_SUCCESS != err) {
                    goto error_hndl;
                }

                /* Exchange data with the peer */
                err = ompi_coll_base_sendrecv_actual((void *) tmp_buffer, rcounts[i], rdtype, i,
                                                     MCA_COLL_BASE_TAG_ALLTOALLV,
                                                     (char *) rbuf + rdisps[i] * ext, rcounts[i],
                                                     rdtype, i, MCA_COLL_BASE_TAG_ALLTOALLV, comm,
                                                     MPI_STATUS_IGNORE);
                if (MPI_SUCCESS != err) {
                    goto error_hndl;
                }
...
int ompi_coll_base_alltoallv_intra_pairwise(const void *sbuf, const int *scounts, const int *sdisps,
                                            ...
                                            mca_coll_base_module_t *module)
{
    ...
    if (MPI_IN_PLACE == sbuf) {
        return mca_coll_base_alltoallv_intra_basic_inplace(rbuf, rcounts, rdisps, rdtype, comm,
                                                           module);
    }
    ...
    for (step = 0; step < size; step++) {
        ...
        err = ompi_coll_base_sendrecv(psnd, scounts[sendto], sdtype, sendto,
                                      MCA_COLL_BASE_TAG_ALLTOALLV, prcv, rcounts[recvfrom], rdtype,
...
err_hndl:
    OPAL_OUTPUT((ompi_coll_base_framework.framework_output,
                 "%s:%4d\tError occurred %d, rank %2d at step %d", __FILE__, line, err, rank,
                 step));
 ...
 */
int ompi_coll_base_alltoallv_intra_basic_linear(const void *sbuf, const int *scounts,
                                                ...
                                                mca_coll_base_module_t *module)
{
    ...
    if (0 != scounts[rank]) {
        err = ompi_datatype_sndrcv(psnd, scounts[rank], sdtype, prcv, rcounts[rank], rdtype);
        if (MPI_SUCCESS != err) {
            return err;
        }
    }
    ...
    for (i = 0; i < size; ++i) {
        ...
        err = MCA_PML_CALL(
            irecv_init(prcv, rcounts[i], rdtype, i, MCA_COLL_BASE_TAG_ALLTOALLV, comm, preq++));
        if (MPI_SUCCESS != err) {
            goto err_hndl;
        }
    }
    ...
    for (i = 0; i < size; ++i) {
        ...
        err = MCA_PML_CALL(isend_init(psnd, scounts[i], sdtype, i, MCA_COLL_BASE_TAG_ALLTOALLV,
