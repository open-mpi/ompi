...
static int mca_coll_basic_neighbor_alltoallv_cart(
    ...
    struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
    ...
    for (dim = 0, nreqs = 0, i = 0; dim < cart->ndims; ++dim, i += 2) {
        ...
        if (MPI_PROC_NULL != drank) {
            nreqs++;
            rc = MCA_PML_CALL(irecv((char *) rbuf + rdisps[i + 1] * rdextent, rcounts[i + 1],
                                    rdtype, drank, MCA_COLL_BASE_TAG_NEIGHBOR_BASE - 2 * dim - 1,
                                    comm, preqs++));
            if (OMPI_SUCCESS != rc)
                break;
        }
    ...
    for (dim = 0, i = 0; dim < cart->ndims; ++dim, i += 2) {
        ...
        if (MPI_PROC_NULL != drank) {
            nreqs++;
            rc = MCA_PML_CALL(isend((char *) sbuf + sdisps[i + 1] * sdextent, scounts[i + 1],
                                    sdtype, drank, MCA_COLL_BASE_TAG_NEIGHBOR_BASE - 2 * dim,
                                    MCA_PML_BASE_SEND_STANDARD, comm, preqs++));
            if (OMPI_SUCCESS != rc)
                break;
        }
...
static int mca_coll_basic_neighbor_alltoallv_graph(
    const void *sbuf, const int scounts[], const int sdisps[], struct ompi_datatype_t *sdtype,
    void *rbuf, const int rcounts[], const int rdisps[], struct ompi_datatype_t *rdtype,
    struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
    ...
    for (neighbor = 0; neighbor < degree; ++neighbor) {
        rc = MCA_PML_CALL(irecv((char *) rbuf + rdisps[neighbor] * rdextent, rcounts[neighbor],
                                rdtype, edges[neighbor], MCA_COLL_BASE_TAG_ALLTOALL, comm,
                                preqs++));
        if (OMPI_SUCCESS != rc)
            break;
    }
    ...
    for (neighbor = 0; neighbor < degree; ++neighbor) {
         ...
         */
        rc = MCA_PML_CALL(isend((char *) sbuf + sdisps[neighbor] * sdextent, scounts[neighbor],
                                sdtype, edges[neighbor], MCA_COLL_BASE_TAG_ALLTOALL,
                                MCA_PML_BASE_SEND_STANDARD, comm, preqs++));
        if (OMPI_SUCCESS != rc)
            break;
    }
...
static int mca_coll_basic_neighbor_alltoallv_dist_graph(
    const void *sbuf, const int scounts[], const int sdisps[], struct ompi_datatype_t *sdtype,
    void *rbuf, const int rcounts[], const int rdisps[], struct ompi_datatype_t *rdtype,
    struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
    ...
    for (neighbor = 0; neighbor < indegree; ++neighbor) {
        rc = MCA_PML_CALL(irecv((char *) rbuf + rdisps[neighbor] * rdextent, rcounts[neighbor],
                                rdtype, inedges[neighbor], MCA_COLL_BASE_TAG_ALLTOALL, comm,
                                preqs++));
        if (OMPI_SUCCESS != rc)
            break;
    }
    ...
    for (neighbor = 0; neighbor < outdegree; ++neighbor) {
         ...
         */
        rc = MCA_PML_CALL(isend((char *) sbuf + sdisps[neighbor] * sdextent, scounts[neighbor],
                                sdtype, outedges[neighbor], MCA_COLL_BASE_TAG_ALLTOALL,
                                MCA_PML_BASE_SEND_STANDARD, comm, preqs++));
        if (OMPI_SUCCESS != rc)
            break;
    }
                                      ...
                                      mca_coll_base_module_t *module)
{
    ...
    if (OMPI_COMM_IS_CART(comm)) {
        return mca_coll_basic_neighbor_alltoallv_cart(sbuf, scounts, sdisps, sdtype, rbuf, rcounts,
                                                      rdisps, rdtype, comm, module);
    } else if (OMPI_COMM_IS_GRAPH(comm)) {
        return mca_coll_basic_neighbor_alltoallv_graph(sbuf, scounts, sdisps, sdtype, rbuf, rcounts,
                                                       rdisps, rdtype, comm, module);
