                                   ...
                                   struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
    ...
    if (rank != root) {
        ...
        err = MCA_PML_CALL(send(sbuf, scount, sdtype, root, MCA_COLL_BASE_TAG_ALLGATHER,
        ...
        err = MCA_PML_CALL(isend(sbuf, scount, sdtype, 0, MCA_COLL_BASE_TAG_ALLGATHER,
                                 MCA_PML_BASE_SEND_STANDARD, comm, &reqs[rsize]));
        if (OMPI_SUCCESS != err) {
            line = __LINE__;
            goto exit;
        }
        ...
        err = MCA_PML_CALL(
            irecv(rbuf, rcount, rdtype, 0, MCA_COLL_BASE_TAG_ALLGATHER, comm, &reqs[0]));
        if (OMPI_SUCCESS != err) {
            line = __LINE__;
            goto exit;
        }
        ...
        for (i = 1; i < rsize; ++i, ptmp += incr) {
            err = MCA_PML_CALL(
                irecv(ptmp, rcount, rdtype, i, MCA_COLL_BASE_TAG_ALLGATHER, comm, &reqs[i]));
            if (MPI_SUCCESS != err) {
                line = __LINE__;
                goto exit;
            }
        ...
        err = MCA_PML_CALL(isend(rbuf, rsize * rcount, rdtype, 0, MCA_COLL_BASE_TAG_ALLGATHER,
        ...
        err = MCA_PML_CALL(recv(tmpbuf, size * scount, sdtype, 0, MCA_COLL_BASE_TAG_ALLGATHER, comm,
                                MPI_STATUS_IGNORE));
        if (OMPI_SUCCESS != err) {
            line = __LINE__;
            goto exit;
    ...
    if (rank != root) {
        /* post the recv */
        err = MCA_PML_CALL(recv(rbuf, rsize * rcount, rdtype, 0, MCA_COLL_BASE_TAG_ALLGATHER, comm,
                                MPI_STATUS_IGNORE));
        if (OMPI_SUCCESS != err) {
            line = __LINE__;
            goto exit;
        ...
        for (i = 1; i < rsize; i++) {
            err = MCA_PML_CALL(isend(tmpbuf, size * scount, sdtype, i, MCA_COLL_BASE_TAG_ALLGATHER,
                                     MCA_PML_BASE_SEND_STANDARD, comm, &reqs[i - 1]));
            if (OMPI_SUCCESS != err) {
                line = __LINE__;
                goto exit;
            }
