...
static int nbc_reduce_scatter_block_init(const void *sendbuf, void *recvbuf, int recvcount,
                                         ...
                                         mca_coll_base_module_t *module, bool persistent)
{
    ...
    if (0 < count) {
        ...
        if ((p == 1) && !inplace) {
            res = NBC_Sched_copy((void *) sendbuf, false, count, datatype, redbuf, false, count,
            ...
            if ((rank % (1 << r)) == 0) {
                ...
                if (peer < p) {
                    ...
                    if (firstred) {
                        ...
                        res = NBC_Sched_op(sendbuf, false, rbuf, true, count, datatype, op,
                                           schedule, true);
                        firstred = 0;
                    } else {
...
int ompi_coll_libnbc_ireduce_scatter_block(const void *sendbuf, void *recvbuf, int recvcount,
                                           MPI_Datatype datatype, MPI_Op op,
                                           struct ompi_communicator_t *comm,
                                           ompi_request_t **request, mca_coll_base_module_t *module)
{
    int res = nbc_reduce_scatter_block_init(sendbuf, recvbuf, recvcount, datatype, op, comm,
...
static int nbc_reduce_scatter_block_inter_init(const void *sendbuf, void *recvbuf, int rcount,
                                               ...
                                               mca_coll_base_module_t *module, bool persistent)
{
    ...
    if (0 == rank) {
        ...
        for (int peer = 1; peer < rsize; ++peer) {
            ...
            res = NBC_Sched_op(lbuf, true, rbuf, true, count, dtype, op, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                OBJ_RELEASE(schedule);
                free(tmpbuf);
                return res;
...
int ompi_coll_libnbc_ireduce_scatter_block_inter(const void *sendbuf, void *recvbuf, int recvcount,
                                                 MPI_Datatype datatype, MPI_Op op,
                                                 struct ompi_communicator_t *comm,
                                                 ompi_request_t **request,
                                                 mca_coll_base_module_t *module)
{
    int res = nbc_reduce_scatter_block_inter_init(sendbuf, recvbuf, recvcount, datatype, op, comm,
                                                  request, module, false);
    if (OPAL_LIKELY(OMPI_SUCCESS != res)) {
        return res;
    }
...
int ompi_coll_libnbc_reduce_scatter_block_init(const void *sendbuf, void *recvbuf, int recvcount,
                                               MPI_Datatype datatype, MPI_Op op,
...
int ompi_coll_libnbc_reduce_scatter_block_inter_init(const void *sendbuf, void *recvbuf,
