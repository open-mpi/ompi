...
static inline int allred_sched_diss(int rank, int p, int count, MPI_Datatype datatype,
                                    ptrdiff_t gap, const void *sendbuf, void *recvbuf, MPI_Op op,
                                    char inplace, NBC_Schedule *schedule, void *tmpbuf);
static inline int allred_sched_recursivedoubling(int rank, int p, const void *sendbuf,
                                                 void *recvbuf, int count, MPI_Datatype datatype,
                                                 ptrdiff_t gap, MPI_Op op, char inplace,
...
static inline int allred_sched_ring(int rank, int p, int count, MPI_Datatype datatype,
                                    ...
                                    int ext, NBC_Schedule *schedule, void *tmpbuf);
static inline int allred_sched_linear(int rank, int p, const void *sendbuf, void *recvbuf,
                                      int count, MPI_Datatype datatype, ptrdiff_t gap, MPI_Op op,
                                      int ext, int size, NBC_Schedule *schedule, void *tmpbuf);
                              ...
                              mca_coll_base_module_t *module, bool persistent)
{
    ...
    search.op = op;
    found = (NBC_Allreduce_args *)
        hb_tree_search((hb_tree *) libnbc_module->NBC_Dict[NBC_ALLREDUCE], &search);
    if (NULL == found) {
#endif
        ...
        if (p == 1) {
            ...
            case NBC_ARED_BINOMIAL:
                res = allred_sched_diss(rank, p, count, datatype, gap, sendbuf, recvbuf, op,
                                        inplace, schedule, tmpbuf);
                break;
            case NBC_ARED_REDSCAT_ALLGATHER:
                res = allred_sched_redscat_allgather(rank, p, count, datatype, gap, sendbuf,
                                                     recvbuf, op, inplace, schedule, tmpbuf, comm);
                break;
            case NBC_ARED_RING:
                res = allred_sched_ring(rank, p, count, datatype, sendbuf, recvbuf, op, size, ext,
                                        schedule, tmpbuf);
                break;
            case NBC_ARED_RDBL:
                res = allred_sched_recursivedoubling(rank, p, sendbuf, recvbuf, count, datatype,
                                                     gap, op, inplace, schedule, tmpbuf);
                break;
            }
        }
...
{
    int res = nbc_allreduce_init(sendbuf, recvbuf, count, datatype, op, comm, request, module,
                                 false);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        return res;
    }
                                    ...
                                    mca_coll_base_module_t *module, bool persistent)
{
    ...
    res = allred_sched_linear(rank, rsize, sendbuf, recvbuf, count, datatype, gap, op, ext, size,
                              schedule, tmpbuf);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        OBJ_RELEASE(schedule);
        free(tmpbuf);
...
{
    int res = nbc_allreduce_inter_init(sendbuf, recvbuf, count, datatype, op, comm, request, module,
                                       false);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        return res;
    }
                                    ...
                                    char inplace, NBC_Schedule *schedule, void *tmpbuf)
{
    ...
    root = 0; /* this makes the code for ireduce and iallreduce nearly identical - could be changed
    ...
    if (0 == (maxr % 2)) {
        ...
        tmprbuf = false;
        if (inplace) {
            res = NBC_Sched_copy(rbuf, false, count, datatype, ((char *) tmpbuf) - gap, false,
                                 count, datatype, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                return res;
            }
        }
                                                 ...
                                                 NBC_Schedule *schedule, void *tmpbuf)
{
    ...
    if (inplace) {
        res = NBC_Sched_copy(recvbuf, false, count, datatype, tmpsend, false, count, datatype,
                             schedule, true);
    } else {
        res = NBC_Sched_copy((void *) sendbuf, false, count, datatype, tmpsend, false, count,
    ...
    if (0 <= vrank) {
        for (int distance = 1; distance < pof2; distance <<= 1) {
            ...
            if (rank < remote) {
                ...
                res = NBC_Sched_op(tmpsend, false, tmprecv, false, count, datatype, op, schedule,
                                   true);
                if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                    return res;
                }
                ...
                /* tmpsend = tmprecv (op) tmpsend */
                res = NBC_Sched_op(tmprecv, false, tmpsend, false, count, datatype, op, schedule,
                                   true);
                if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                    return res;
                }
    ...
    */
    ...
    if (tmpsend != recvbuf) {
        res = NBC_Sched_copy(tmpsend, false, count, datatype, recvbuf, false, count, datatype,
                             schedule, false);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
            return res;
        }
...
static inline int allred_sched_ring(int r, int p, int count, MPI_Datatype datatype,
                                    ...
                                    int ext, NBC_Schedule *schedule, void *tmpbuf)
{
    int segsize, *segsizes, *segoffsets; /* segment sizes and offsets per segment (number of
                                            segments == number of nodes */
     ...
     *   02       12       22+12  2: red(2), [2] -> 0 --> reduce and send element (r+0)%p to node
     * (r+1)%p
     *
     *      *** round 2 ***
     *    0        1        2
     ...
     *   03       13       23       33+23    3: red(3), [3] -> 0 --> reduce and send element (r+0)%p
     * to node (r+1)%p
     *
     *      *** round 2 ***
     *    0        1        2        3
     ...
     *   03+33/23 13       23       33/23    3: red(2), [2] -> 0 --> reduce and send (r-1)%p to node
     * (r+1)%p
     *
     *      *** round 3 ***
     *    0        1        2        3
     ...
     *   03/33/23 all      23       33/23    3: red(1), [1] -> 0 --> reduce and send (r-2)%p to node
     * (r+1)%p
     *
     *      *** round 4 ***
     *    0        1        2        3
     ...
     *   03/33/23 all      all      33/23    3: [0] -> 0 --> receive and send element (r+1)%p to
     * node (r+1)%p
     *
     *      *** round 5 ***
     *    0        1        2        3
     ...
     *   03/33/23 all      all      all      3: [4] -> 4 --> receive and send element (r-0)%p to
     * node (r+1)%p
     *
     *      *** round 6 ***
     *    0        1        2        3
    ...
    for (int round = 0; round < p - 1; ++round) {
        int selement = (r + 1 - round + 2 * p /*2*p avoids negative mod*/)
                       % p; /* the element I am sending */
        int soffset = segoffsets[selement] * ext;
        int relement = (r - round + 2 * p /*2*p avoids negative mod*/)
                       % p; /* the element that I receive from my neighbor */
        int roffset = segoffsets[relement] * ext;

        /* first message come out of sendbuf */
        if (round == 0) {
            res = NBC_Sched_send((char *) sendbuf + soffset, false, segsizes[selement], datatype,
                                 speer, schedule, false);
        } else {
            res = NBC_Sched_send((char *) recvbuf + soffset, false, segsizes[selement], datatype,
                                 speer, schedule, false);
        }
                                                 ...
                                                 struct ompi_communicator_t *comm)
{
    ...
    if (!inplace) {
        res = NBC_Sched_copy((char *) sbuf, false, count, datatype, rbuf, false, count, datatype,
                             schedule, true);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
    ...
    if (rank < 2 * nprocs_rem) {
        ...
        if (rank % 2 != 0) {
             ...
             */
            res = NBC_Sched_send(rbuf, false, count_lhalf, datatype, rank - 1, schedule, false);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            res = NBC_Sched_recv(tmp_buf + (ptrdiff_t) count_lhalf * extent, false, count_rhalf,
                                 datatype, rank - 1, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            res = NBC_Sched_op(tmp_buf + (ptrdiff_t) count_lhalf * extent, false,
                               (char *) rbuf + (ptrdiff_t) count_lhalf * extent, false, count_rhalf,
                               datatype, op, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            /* Send the right half to the left neighbor */
            res = NBC_Sched_send((char *) rbuf + (ptrdiff_t) count_lhalf * extent, false,
                                 count_rhalf, datatype, rank - 1, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            ...
            res = NBC_Sched_recv(tmp_buf, false, count_lhalf, datatype, rank + 1, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            res = NBC_Sched_op(tmp_buf, false, rbuf, false, count_lhalf, datatype, op, schedule,
                               true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            /* Recv the right half from the right neighbor */
            res = NBC_Sched_recv((char *) rbuf + (ptrdiff_t) count_lhalf * extent, false,
                                 count_rhalf, datatype, rank + 1, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
    ...
    if (vrank != -1) {
        ...
        for (int mask = 1; mask < nprocs_pof2; mask <<= 1) {
            ...
            res = NBC_Sched_send((char *) rbuf + (ptrdiff_t) sindex[step] * extent, false,
                                 scount[step], datatype, dest, schedule, false);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            res = NBC_Sched_recv((char *) tmp_buf + (ptrdiff_t) rindex[step] * extent, false,
                                 rcount[step], datatype, dest, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            /* Local reduce: rbuf[] = tmp_buf[] <op> rbuf[] */
            res = NBC_Sched_op((char *) tmp_buf + (ptrdiff_t) rindex[step] * extent, false,
                               (char *) rbuf + (ptrdiff_t) rindex[step] * extent, false,
                               rcount[step], datatype, op, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
        ...
        for (int mask = nprocs_pof2 >> 1; mask > 0; mask >>= 1) {
             ...
             */
            res = NBC_Sched_send((char *) rbuf + (ptrdiff_t) rindex[step] * extent, false,
                                 rcount[step], datatype, dest, schedule, false);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            res = NBC_Sched_recv((char *) rbuf + (ptrdiff_t) sindex[step] * extent, false,
                                 scount[step], datatype, dest, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            step--;
