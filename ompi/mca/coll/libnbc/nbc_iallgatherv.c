 ...
 *   each node begins with it's right node (rank-11)%p and receives from it recvcounts[(rank+1)%p]
 * elements second round: each node sends to node (rank+2)%p sendcount elements each node receives
 * from node (rank-2)%p recvcounts[(rank+2)%p] elements */
static int nbc_allgatherv_init(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                               ...
                               bool persistent)
{
    ...
    if (persistent && !inplace) { /* for nonblocking, data has been copied already */
        ...
        res = NBC_Sched_copy((void *) sendbuf, false, sendcount, sendtype, sbuf, false,
                             recvcounts[rank], recvtype, schedule, true);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
...
int ompi_coll_libnbc_iallgatherv(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                                 ...
                                 ompi_request_t **request, mca_coll_base_module_t *module)
{
    int res = nbc_allgatherv_init(sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs,
                                  recvtype, comm, request, module, false);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        return res;
    }
...
static int nbc_allgatherv_inter_init(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                                     void *recvbuf, const int *recvcounts, const int *displs,
                                     MPI_Datatype recvtype, struct ompi_communicator_t *comm,
                                     ompi_request_t **request, mca_coll_base_module_t *module,
                                     bool persistent)
...
int ompi_coll_libnbc_iallgatherv_inter(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                                       ...
                                       ompi_request_t **request, mca_coll_base_module_t *module)
{
    int res = nbc_allgatherv_inter_init(sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs,
                                        recvtype, comm, request, module, false);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        return res;
    }
...
{
    int res = nbc_allgatherv_init(sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs,
                                  recvtype, comm, request, module, true);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        return res;
    }
