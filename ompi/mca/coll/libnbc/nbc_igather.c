...
int NBC_Gather_args_compare(NBC_Gather_args *a, NBC_Gather_args *b, void *param)
{
    if ((a->sendbuf == b->sendbuf) && (a->sendcount == b->sendcount) && (a->sendtype == b->sendtype)
        ...
        && (a->recvtype == b->recvtype) && (a->root == b->root)) {
        return 0;
    }
            ...
            for (int i = 0; i < p; ++i) {
                ...
                if (i == root) {
                    if (!inplace) {
                        ...
                        res = NBC_Sched_copy((void *) sendbuf, false, sendcount, sendtype, rbuf,
                                             false, recvcount, recvtype, schedule, false);
                        if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                            OBJ_RELEASE(schedule);
                            return res;
                        }
...
static int nbc_gather_inter_init(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                                 void *recvbuf, int recvcount, MPI_Datatype recvtype, int root,
                                 struct ompi_communicator_t *comm, ompi_request_t **request,
                                 mca_coll_base_module_t *module, bool persistent)
{
...
int ompi_coll_libnbc_igather_inter(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                                   ...
                                   mca_coll_base_module_t *module)
{
    int res = nbc_gather_inter_init(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype,
