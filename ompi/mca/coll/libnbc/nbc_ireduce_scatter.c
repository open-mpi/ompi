...
static int nbc_reduce_scatter_init(const void *sendbuf, void *recvbuf, const int *recvcounts,
                                   ...
                                   mca_coll_base_module_t *module, bool persistent)
{
    ...
    if ((1 == p && (!persistent || inplace)) || 0 == count) {
        if (!inplace) {
            ...
            res = NBC_Copy(sendbuf, recvcounts[0], datatype, recvbuf, recvcounts[0], datatype,
                           comm);
    ...
    if (rank == 0) {
        ...
        if (p == 1) {
            /* single node not in_place: copy data to recvbuf */
            res = NBC_Sched_copy((void *) sendbuf, false, recvcounts[0], datatype, recvbuf, false,
                                 recvcounts[0], datatype, schedule, false);
        } else {
            res = NBC_Sched_copy(lbuf, true, recvcounts[0], datatype, recvbuf, false, recvcounts[0],
...
{
    int res = nbc_reduce_scatter_init(sendbuf, recvbuf, recvcounts, datatype, op, comm, request,
                                      module, false);
...
static int nbc_reduce_scatter_inter_init(const void *sendbuf, void *recvbuf, const int *recvcounts,
                                         MPI_Datatype datatype, MPI_Op op,
                                         struct ompi_communicator_t *comm, ompi_request_t **request,
                                         mca_coll_base_module_t *module, bool persistent)
{
    ...
    if (0 == rank) {
        ...
        for (int peer = 1, offset = recvcounts[0] * ext; peer < lsize; ++peer) {
            res = NBC_Sched_local_send(lbuf + offset, true, recvcounts[peer], datatype, peer,
                                       schedule, false);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                OBJ_RELEASE(schedule);
                free(tmpbuf);
...
{
    int res = nbc_reduce_scatter_inter_init(sendbuf, recvbuf, recvcounts, datatype, op, comm,
