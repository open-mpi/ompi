 ...
 */
#define NBC_GET_BYTES(ptr, x)       \
    {                               \
        memcpy(&x, ptr, sizeof(x)); \
        ptr += sizeof(x);           \
    }
#define NBC_PUT_BYTES(ptr, x)       \
    {                               \
        ...
        for (i = 0; i < num; i++) {                                                                \
            ...
            case OP:                                                                               \
                ...
                printf("*buf1: %lu, buf2: %lu, count: %i, type: %lu)\n",                           \
                       (unsigned long) opargs.buf1, (unsigned long) opargs.buf2, opargs.count,     \
                       (unsigned long) opargs.datatype);                                           \
                break;                                                                             \
            case COPY:                                                                             \
                printf("[%i]  COPY   (offset %li) ", myrank, (long) p - (long) schedule);          \
                NBC_GET_BYTES(p, copyargs);                                                        \
                printf("*src: %lu, srccount: %i, srctype: %lu, *tgt: %lu, tgtcount: %i, tgttype: " \
                       "%lu)\n",                                                                   \
                       (unsigned long) copyargs.src, copyargs.srccount,                            \
                       (unsigned long) copyargs.srctype, (unsigned long) copyargs.tgt,             \
                       copyargs.tgtcount, (unsigned long) copyargs.tgttype);                       \
                break;                                                                             \
            case UNPACK:                                                                           \
                ...
                printf("*src: %lu, srccount: %i, srctype: %lu, *tgt: %lu\n",                       \
                       ...
                       (unsigned long) unpackargs.datatype, (unsigned long) unpackargs.outbuf);    \
                break;                                                                             \
            default:                                                                               \
                printf("[%i] NBC_PRINT_ROUND: bad type %i at offset %li\n", myrank, type,          \
                       (long) p - sizeof(type) - (long) schedule);                                 \
...
#define NBC_PRINT_SCHED(schedule)                                                              \
    {                                                                                          \
        ...
        while ((long) ptr - (long) schedule < size) {                                          \
            ...
            printf("[%i] Round at byte %li (size %li) ", myrank, (long) ptr - (long) schedule, \
                   round_size);                                                                \
            NBC_PRINT_ROUND(ptr);                                                              \
            ptr += round_size;                                                                 \
...
static inline int NBC_Type_intrinsic(MPI_Datatype type)
{
    ...
    if ((type == MPI_INT) || (type == MPI_LONG) || (type == MPI_SHORT) || (type == MPI_UNSIGNED)
        || (type == MPI_UNSIGNED_SHORT) || (type == MPI_UNSIGNED_LONG) || (type == MPI_FLOAT)
        || (type == MPI_DOUBLE) || (type == MPI_LONG_DOUBLE) || (type == MPI_BYTE)
        || (type == MPI_FLOAT_INT) || (type == MPI_DOUBLE_INT) || (type == MPI_LONG_INT)
        || (type == MPI_2INT) || (type == MPI_SHORT_INT) || (type == MPI_LONG_DOUBLE_INT))
        return 1;
    else
        return 0;
}
...
static inline int NBC_Unpack(void *src, int srccount, MPI_Datatype srctype, void *tgt,
                             MPI_Comm comm)
{
    MPI_Aint size, pos;
    int res;
