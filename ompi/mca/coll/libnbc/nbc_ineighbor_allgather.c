...
int NBC_Ineighbor_allgather_args_compare(NBC_Ineighbor_allgather_args *a,
                                         NBC_Ineighbor_allgather_args *b, void *param)
{
                                       ...
                                       mca_coll_base_module_t *module, bool persistent)
{
    ...
    search.rtype = rtype;
    found = (NBC_Ineighbor_allgather_args *)
        hb_tree_search((hb_tree *) libnbc_module->NBC_Dict[NBC_NEIGHBOR_ALLGATHER], &search);
    if (NULL == found) {
#endif
        ...
        if (NULL != args) {
            ...
            res = hb_tree_insert((hb_tree *) libnbc_module->NBC_Dict[NBC_NEIGHBOR_ALLGATHER], args,
                                 args, 0);
            if (0 == res) {
                OBJ_RETAIN(schedule);
                ...
                if (++libnbc_module->NBC_Dict_size[NBC_NEIGHBOR_ALLGATHER] > NBC_SCHED_DICT_UPPER) {
                    NBC_SchedCache_dictwipe((hb_tree *)
                                                libnbc_module->NBC_Dict[NBC_NEIGHBOR_ALLGATHER],
    ...
    {
        int res = nbc_neighbor_allgather_init(sbuf, scount, stype, rbuf, rcount, rtype, comm,
