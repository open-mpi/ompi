...
static inline int a2a_sched_linear(int rank, int p, MPI_Aint sndext, MPI_Aint rcvext,
                                   NBC_Schedule *schedule, const void *sendbuf, int sendcount,
                                   MPI_Datatype sendtype, void *recvbuf, int recvcount,
                                   MPI_Datatype recvtype, MPI_Comm comm);
static inline int a2a_sched_pairwise(int rank, int p, MPI_Aint sndext, MPI_Aint rcvext,
                                     NBC_Schedule *schedule, const void *sendbuf, int sendcount,
                                     MPI_Datatype sendtype, void *recvbuf, int recvcount,
                                     MPI_Datatype recvtype, MPI_Comm comm);
static inline int a2a_sched_diss(int rank, int p, MPI_Aint sndext, MPI_Aint rcvext,
                                 NBC_Schedule *schedule, const void *sendbuf, int sendcount,
                                 MPI_Datatype sendtype, void *recvbuf, int recvcount,
...
{
    if ((a->sendbuf == b->sendbuf) && (a->sendcount == b->sendcount) && (a->sendtype == b->sendtype)
        && (a->recvbuf == b->recvbuf) && (a->recvcount == b->recvcount)
...
static int nbc_alltoall_init(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                             ...
                             mca_coll_base_module_t *module, bool persistent)
{
    ...
    if (alg == NBC_A2A_INPLACE) {
        ...
        if (NBC_Type_intrinsic(sendtype)
            && !(opal_cuda_check_bufs((char *) sendbuf, (char *) recvbuf))) {
#else
        if (NBC_Type_intrinsic(sendtype)) {
            ...
            /* non-contiguous - pack */
            res = ompi_datatype_pack_external("external32",
                                              (char *) sendbuf
                                                  + (intptr_t) rank * (intptr_t) sendcount * sndext,
                                              (intptr_t)(p - rank) * (intptr_t) sendcount, sendtype,
                                              tmpbuf, (intptr_t)(p - rank) * datasize, &pos);
            ...
            if (rank != 0) {
                pos = 0;
                res = ompi_datatype_pack_external("external32", sendbuf,
                                                  (intptr_t) rank * (intptr_t) sendcount, sendtype,
                                                  (char *) tmpbuf + datasize * (intptr_t)(p - rank),
                                                  rank * datasize, &pos);
                if (OPAL_UNLIKELY(MPI_SUCCESS != res)) {
                    NBC_Error("MPI Error in ompi_datatype_pack_external() (%i)", res);
                    free(tmpbuf);
    ...
    found = (NBC_Alltoall_args *) hb_tree_search((hb_tree *) libnbc_module->NBC_Dict[NBC_ALLTOALL],
                                                 &search);
    if (NULL == found) {
#endif
        ...
        if (!inplace) {
            ...
            res = NBC_Sched_copy(sbuf, false, sendcount, sendtype, rbuf, false, recvcount, recvtype,
                                 schedule, false);
        ...
        case NBC_A2A_INPLACE:
            res = a2a_sched_inplace(rank, p, schedule, recvbuf, recvcount, recvtype, rcvext, gap,
                                    comm);
            break;
        case NBC_A2A_LINEAR:
            res = a2a_sched_linear(rank, p, sndext, rcvext, schedule, sendbuf, sendcount, sendtype,
                                   recvbuf, recvcount, recvtype, comm);
            break;
        case NBC_A2A_DISS:
            res = a2a_sched_diss(rank, p, sndext, rcvext, schedule, sendbuf, sendcount, sendtype,
                                 recvbuf, recvcount, recvtype, comm, tmpbuf);
            break;
        case NBC_A2A_PAIRWISE:
            res = a2a_sched_pairwise(rank, p, sndext, rcvext, schedule, sendbuf, sendcount,
...
{
    int res = nbc_alltoall_init(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm,
...
static int nbc_alltoall_inter_init(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                                   void *recvbuf, int recvcount, MPI_Datatype recvtype,
                                   struct ompi_communicator_t *comm, ompi_request_t **request,
                                   mca_coll_base_module_t *module, bool persistent)
{
                                 ...
                                 MPI_Datatype recvtype, MPI_Comm comm, void *tmpbuf)
{
    ...
    for (int r = 1; r < p; r <<= 1) {
        ...
        for (int i = 1; i < p; ++i) {
            ...
            if (i & r) {
                 ...
                 * long)(stmpbuf+offset));*/
                res = NBC_Sched_copy((void *) (intptr_t)(i * datasize), true, datasize, MPI_BYTE,
                                     stmpbuf + offset - (intptr_t) tmpbuf, true, datasize, MPI_BYTE,
                                     schedule, false);
                if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                    return res;
                }
        ...
        res = NBC_Sched_recv(rtmpbuf - (intptr_t) tmpbuf, true, offset, MPI_BYTE, rpeer, schedule,
                             false);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
            return res;
        }

        res = NBC_Sched_send(stmpbuf - (intptr_t) tmpbuf, true, offset, MPI_BYTE, speer, schedule,
                             true);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
            return res;
        }
        ...
        for (int i = 1; i < p; ++i) {
            ...
            if (i & r) {
                ...
                res = NBC_Sched_copy(rtmpbuf + offset - (intptr_t) tmpbuf, true, datasize, MPI_BYTE,
                                     (void *) (intptr_t)(i * datasize), true, datasize, MPI_BYTE,
                                     schedule, false);
                if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                    return res;
                }
    ...
    for (int i = 0; i < p; ++i) {
        ...
        res = NBC_Sched_unpack((void *) (intptr_t)(i * datasize), true, recvcount, recvtype, rbuf,
                                    ...
                                    MPI_Datatype type, MPI_Aint ext, ptrdiff_t gap, MPI_Comm comm)
{
    ...
    for (int i = 1; i < (p + 1) / 2; i++) {
        ...
        res = NBC_Sched_copy(rbuf, false, count, type, (void *) (-gap), true, count, type, schedule,
                             true);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
            return res;
        }
