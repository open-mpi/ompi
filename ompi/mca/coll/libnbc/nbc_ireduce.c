...
static inline int red_sched_binomial(int rank, int p, int root, const void *sendbuf, void *redbuf,
                                     char tmpredbuf, int count, MPI_Datatype datatype, MPI_Op op,
...
static inline int red_sched_chain(int rank, int p, int root, const void *sendbuf, void *recvbuf,
                                  int count, MPI_Datatype datatype, MPI_Op op, int ext, size_t size,
                                  NBC_Schedule *schedule, void *tmpbuf, int fragsize);

static inline int red_sched_linear(int rank, int rsize, int root, const void *sendbuf,
                                   void *recvbuf, void *tmpbuf, int count, MPI_Datatype datatype,
                                   MPI_Op op, NBC_Schedule *schedule);
static inline int red_sched_redscat_gather(int rank, int comm_size, int root, const void *sbuf,
                                           void *rbuf, char tmpredbuf, int count,
                                           MPI_Datatype datatype, MPI_Op op, char inplace,
                                           NBC_Schedule *schedule, void *tmp_buf,
...
int NBC_Reduce_args_compare(NBC_Reduce_args *a, NBC_Reduce_args *b, void *param)
{
    if ((a->sendbuf == b->sendbuf) && (a->recvbuf == b->recvbuf) && (a->count == b->count)
        && (a->datatype == b->datatype) && (a->op == b->op) && (a->root == b->root)) {
        return 0;
    }
                           ...
                           bool persistent)
{
    ...
    if (libnbc_ireduce_algorithm == 0) {
        ...
        if (libnbc_ireduce_algorithm == 1) {
            ...
            alg = NBC_RED_BINOMIAL;
        } else if (libnbc_ireduce_algorithm == 3 && ompi_op_is_commute(op) && p > 2
                   && count >= nprocs_pof2) {
            alg = NBC_RED_REDSCAT_GATHER;
        } else {
            alg = NBC_RED_CHAIN;
    ...
    search.op = op;
    search.root = root;
    found = (NBC_Reduce_args *) hb_tree_search((hb_tree *) libnbc_module->NBC_Dict[NBC_REDUCE],
                                               &search);
    if (NULL == found) {
#endif
        ...
        if (p == 1) {
            ...
            case NBC_RED_BINOMIAL:
                res = red_sched_binomial(rank, p, root, sendbuf, redbuf, tmpredbuf, count, datatype,
                                         op, inplace, schedule, tmpbuf);
                break;
            case NBC_RED_CHAIN:
                res = red_sched_chain(rank, p, root, sendbuf, recvbuf, count, datatype, op, ext,
                                      size, schedule, tmpbuf, segsize);
                break;
            case NBC_RED_REDSCAT_GATHER:
                res = red_sched_redscat_gather(rank, p, root, sendbuf, redbuf, tmpredbuf, count,
                                               datatype, op, inplace, schedule, tmpbuf, comm);
                break;
            }
        }
...
int ompi_coll_libnbc_ireduce(const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype,
                             ...
                             ompi_request_t **request, mca_coll_base_module_t *module)
{
    int res = nbc_reduce_init(sendbuf, recvbuf, count, datatype, op, root, comm, request, module,
                              false);
...
static int nbc_reduce_inter_init(const void *sendbuf, void *recvbuf, int count,
                                 ...
                                 mca_coll_base_module_t *module, bool persistent)
{
    ...
    res = red_sched_linear(rank, rsize, root, sendbuf, recvbuf, (void *) (-gap), count, datatype,
                           op, schedule);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        OBJ_RELEASE(schedule);
        free(tmpbuf);
...
{
    int res = nbc_reduce_inter_init(sendbuf, recvbuf, count, datatype, op, root, comm, request,
                                    module, false);
    if (OPAL_LIKELY(OMPI_SUCCESS != res)) {
        return res;
    }
                                           ...
                                           struct ompi_communicator_t *comm)
{
    ...
    if ((rank != root) || !inplace) {
        res = NBC_Sched_copy((char *) sbuf, false, count, datatype, rbuf, tmpredbuf, count,
    ...
    if (rank < 2 * nprocs_rem) {
        ...
        if (rank % 2 != 0) {
             ...
             */
            res = NBC_Sched_send(rbuf, tmpredbuf, count_lhalf, datatype, rank - 1, schedule, false);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }

            res = NBC_Sched_recv((char *) tmp_buf + (ptrdiff_t) count_lhalf * extent, false,
                                 count_rhalf, datatype, rank - 1, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }

            res = NBC_Sched_op((char *) tmp_buf + (ptrdiff_t) count_lhalf * extent, false,
                               (char *) rbuf + (ptrdiff_t) count_lhalf * extent, tmpredbuf,
                               count_rhalf, datatype, op, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }

            /* Send the right half to the left neighbor */
            res = NBC_Sched_send((char *) rbuf + (ptrdiff_t) count_lhalf * extent, tmpredbuf,
                                 count_rhalf, datatype, rank - 1, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            ...
            res = NBC_Sched_recv((char *) tmp_buf, false, count_lhalf, datatype, rank + 1, schedule,
                                 true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }

            res = NBC_Sched_op(tmp_buf, false, rbuf, tmpredbuf, count_lhalf, datatype, op, schedule,
                               true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
            ...
            res = NBC_Sched_recv((char *) rbuf + (ptrdiff_t) count_lhalf * extent, tmpredbuf,
                                 count_rhalf, datatype, rank + 1, schedule, true);
            if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                goto cleanup_and_return;
            }
    ...
    if (vrank != -1) {
        ...
        for (int mask = 1; mask < nprocs_pof2; mask <<= 1) {
            ...
            res = NBC_Sched_send((char *) rbuf + (ptrdiff_t) sindex[step] * extent, tmpredbuf,
            ...
            res = NBC_Sched_op((char *) tmp_buf + (ptrdiff_t) rindex[step] * extent, false,
                               (char *) rbuf + (ptrdiff_t) rindex[step] * extent, tmpredbuf,
    ...
    if (root < 2 * nprocs_rem) {
        if (root % 2 != 0) {
            ...
            if (rank == root) {
                ...
                res = NBC_Sched_recv(rbuf, tmpredbuf, rcount[nsteps - 1], datatype, 0, schedule,
                                     true);
                if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                    goto cleanup_and_return;
                }
                ...
                res = NBC_Sched_send(rbuf, tmpredbuf, rcount[nsteps - 1], datatype, root, schedule,
                                     true);
                if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
                    goto cleanup_and_return;
                }
    ...
    if (vrank != -1) {
        ...
        for (int mask = nprocs_pof2 >> 1; mask > 0; mask >>= 1) {
            ...
            if (vdest_tree == vroot_tree) {
                ...
                res = NBC_Sched_send((char *) rbuf + (ptrdiff_t) rindex[step] * extent, tmpredbuf,
                ...
                /* Recv and continue */
                res = NBC_Sched_recv((char *) rbuf + (ptrdiff_t) sindex[step] * extent, tmpredbuf,
