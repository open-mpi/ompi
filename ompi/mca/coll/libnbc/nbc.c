...
void NBC_Print_times(double div)
{
    printf("*** NBC_TIMES: Isend: %lf, Irecv: %lf, Wait: %lf, Test: %lf\n", Isend_time * 1e6 / div,
           Irecv_time * 1e6 / div, Wait_time * 1e6 / div, Test_time * 1e6 / div);
}
#endif
...
OBJ_CLASS_INSTANCE(NBC_Schedule, opal_object_t, nbc_schedule_constructor, nbc_schedule_destructor);
...
int NBC_Sched_copy(void *src, char tmpsrc, int srccount, MPI_Datatype srctype, void *tgt,
                   char tmptgt, int tgtcount, MPI_Datatype tgttype, NBC_Schedule *schedule,
                   bool barrier)
{
...
int NBC_Sched_unpack(void *inbuf, char tmpinbuf, int count, MPI_Datatype datatype, void *outbuf,
                     char tmpoutbuf, NBC_Schedule *schedule, bool barrier)
{
...
int NBC_Progress(NBC_Handle *handle)
{
    ...
    if ((handle->req_count > 0) && (handle->req_array != NULL)) {
        ...
        while (handle->req_count) {
            ...
            if (REQUEST_COMPLETE(subreq) || OPAL_UNLIKELY(ompi_request_is_failed(subreq))) {
#else
    ...
    if (flag) {
        ...
        if (OPAL_UNLIKELY(OMPI_SUCCESS != handle->super.super.req_status.MPI_ERROR)) {
            ...
            NBC_Error("NBC_Progress: an error %d was found during schedule %p at row-offset %li - "
...
static inline int NBC_Start_round(NBC_Handle *handle)
{
    ...
    for (int i = 0; i < num; ++i) {
        ...
        case SEND:
            ...
            res = MCA_PML_CALL(isend(buf1, sendargs.count, sendargs.datatype, sendargs.dest,
                                     handle->tag, MCA_PML_BASE_SEND_STANDARD,
                                     sendargs.local ? handle->comm->c_local_comm : handle->comm,
                                     handle->req_array + handle->req_count - 1));
            if (OMPI_SUCCESS != res) {
                NBC_Error("Error in MPI_Isend(%lu, %i, %p, %i, %i, %lu) (%i)", (unsigned long) buf1,
                          sendargs.count, sendargs.datatype, sendargs.dest, handle->tag,
        ...
        case RECV:
            ...
            NBC_DEBUG(5, "*buf: %p, count: %i, type: %p, source: %i, tag: %i)\n", recvargs.buf,
                      recvargs.count, recvargs.datatype, recvargs.source, handle->tag);
            ...
            res = MCA_PML_CALL(irecv(buf1, recvargs.count, recvargs.datatype, recvargs.source,
                                     ...
                                     handle->req_array + handle->req_count - 1));
            if (OMPI_SUCCESS != res) {
                NBC_Error("Error in MPI_Irecv(%lu, %i, %p, %i, %i, %lu) (%i)", (unsigned long) buf1,
                          recvargs.count, recvargs.datatype, recvargs.source, handle->tag,
                          (unsigned long) handle->comm, res);
                return res;
            }
        ...
        case COPY:
            ...
            NBC_DEBUG(
                5, "*src: %lu, srccount: %i, srctype: %p, *tgt: %lu, tgtcount: %i, tgttype: %p)\n",
        ...
        case UNPACK:
            ...
            NBC_DEBUG(5, "*src: %lu, srccount: %i, srctype: %p, *tgt: %lu\n",
                      (unsigned long) unpackargs.inbuf, unpackargs.count, unpackargs.datatype,
...
int NBC_Init_comm(MPI_Comm comm, NBC_Comminfo *comminfo)
{
    ...
    comminfo->NBC_Dict[NBC_ALLTOALL] = hb_tree_new((dict_cmp_func) NBC_Alltoall_args_compare,
    ...
    comminfo->NBC_Dict[NBC_ALLGATHER] = hb_tree_new((dict_cmp_func) NBC_Allgather_args_compare,
                                                    NBC_SchedCache_args_delete_key_dummy,
                                                    NBC_SchedCache_args_delete);
    ...
    comminfo->NBC_Dict[NBC_ALLREDUCE] = hb_tree_new((dict_cmp_func) NBC_Allreduce_args_compare,
                                                    NBC_SchedCache_args_delete_key_dummy,
                                                    NBC_SchedCache_args_delete);
    ...
    comminfo->NBC_Dict[NBC_BCAST] = hb_tree_new((dict_cmp_func) NBC_Bcast_args_compare,
                                                NBC_SchedCache_args_delete_key_dummy,
                                                NBC_SchedCache_args_delete);
    ...
    comminfo->NBC_Dict[NBC_GATHER] = hb_tree_new((dict_cmp_func) NBC_Gather_args_compare,
                                                 NBC_SchedCache_args_delete_key_dummy,
                                                 NBC_SchedCache_args_delete);
    ...
    comminfo->NBC_Dict[NBC_REDUCE] = hb_tree_new((dict_cmp_func) NBC_Reduce_args_compare,
                                                 NBC_SchedCache_args_delete_key_dummy,
                                                 NBC_SchedCache_args_delete);
    ...
    comminfo->NBC_Dict[NBC_SCAN] = hb_tree_new((dict_cmp_func) NBC_Scan_args_compare,
                                               NBC_SchedCache_args_delete_key_dummy,
                                               NBC_SchedCache_args_delete);
    ...
    comminfo->NBC_Dict[NBC_SCATTER] = hb_tree_new((dict_cmp_func) NBC_Scatter_args_compare,
                                                  NBC_SchedCache_args_delete_key_dummy,
                                                  NBC_SchedCache_args_delete);
