...
static inline int bcast_sched_binomial(int rank, int p, int root, NBC_Schedule *schedule,
                                       void *buffer, int count, MPI_Datatype datatype);
static inline int bcast_sched_linear(int rank, int p, int root, NBC_Schedule *schedule,
                                     void *buffer, int count, MPI_Datatype datatype);
static inline int bcast_sched_chain(int rank, int p, int root, NBC_Schedule *schedule, void *buffer,
                                    int count, MPI_Datatype datatype, int fragsize, size_t size);
static inline int bcast_sched_knomial(int rank, int comm_size, int root, NBC_Schedule *schedule,
...
{
    if ((a->buffer == b->buffer) && (a->count == b->count) && (a->datatype == b->datatype)
        && (a->root == b->root)) {
        return 0;
    }
                          ...
                          mca_coll_base_module_t *module, bool persistent)
{
    ...
    search.root = root;
    found = (NBC_Bcast_args *) hb_tree_search((hb_tree *) libnbc_module->NBC_Dict[NBC_BCAST],
                                              &search);
    if (NULL == found) {
#endif
        ...
        case NBC_BCAST_CHAIN:
            res = bcast_sched_chain(rank, p, root, schedule, buffer, count, datatype, segsize,
                                    size);
            break;
        case NBC_BCAST_KNOMIAL:
            res = bcast_sched_knomial(rank, p, root, schedule, buffer, count, datatype,
...
{
    int res = nbc_bcast_init(buffer, count, datatype, root, comm, request, module, false);
...
int ompi_coll_libnbc_ibcast_inter(void *buffer, int count, MPI_Datatype datatype, int root,
                                  ...
                                  mca_coll_base_module_t *module)
{
    int res = nbc_bcast_inter_init(buffer, count, datatype, root, comm, request, module, false);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        return res;
    }
...
{
    int res = nbc_bcast_init(buffer, count, datatype, root, comm, request, module, true);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        return res;
    }
...
int ompi_coll_libnbc_bcast_inter_init(void *buffer, int count, MPI_Datatype datatype, int root,
                                      ...
                                      ompi_request_t **request, mca_coll_base_module_t *module)
{
    int res = nbc_bcast_inter_init(buffer, count, datatype, root, comm, request, module, true);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != res)) {
        return res;
    }
