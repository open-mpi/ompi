...
static int recv_nb(dte_data_representation_t data, uint32_t count, void *buffer, rte_ec_handle_t,
                   rte_grp_handle_t, uint32_t tag, rte_request_handle_t *req);

static int send_nb(dte_data_representation_t data, uint32_t count, void *buffer,
                   rte_ec_handle_t ec_h, rte_grp_handle_t grp_h, uint32_t tag,
                   rte_request_handle_t *req);

static int test(rte_request_handle_t *request, int *completed);

static int ec_handle_compare(rte_ec_handle_t handle_1, rte_grp_handle_t group_handle_1,
                             rte_ec_handle_t handle_2, rte_grp_handle_t group_handle_2);

static int get_ec_handles(int num_ec, int *ec_indexes, rte_grp_handle_t,
...
static int get_mpi_type_envelope(void *mpi_type, int *num_integers, int *num_addresses,
                                 int *num_datatypes, hcoll_mpi_type_combiner_t *combiner);
...
static int get_mpi_constants(size_t *mpi_datatype_size, int *mpi_order_c, int *mpi_order_fortran,
                             int *mpi_distribute_block, int *mpi_distribute_cyclic,
...
void hcoll_rte_fns_setup(void)
{
    ...
    opal_free_list_init(&(mca_coll_hcoll_component.requests), sizeof(ompi_coll_base_nbc_request_t),
...
static int recv_nb(struct dte_data_representation_t data, uint32_t count, void *buffer,
                   rte_ec_handle_t ec_h, rte_grp_handle_t grp_h, uint32_t tag,
                   rte_request_handle_t *req)
{
    ...
    if (NULL == ec_h.handle && -1 != ec_h.rank) {
        fprintf(stderr,
                "***Error in hcolrte_rml_recv_nb: wrong null argument: "
    ...
    HCOL_VERBOSE(30, "PML_IRECV: dest = %d: buf = %p: size = %u: comm = %p", ec_h.rank, buffer,
                 (unsigned int) size, (void *) comm);
    if (MCA_PML_CALL(
            irecv(buffer, size, &(ompi_mpi_unsigned_char.dt), ec_h.rank, tag, comm, &ompi_req))) {
        return HCOLL_ERROR;
    }
                   ...
                   rte_request_handle_t *req)
{
    ...
    if (!ec_h.handle) {
        fprintf(stderr,
                "***Error in hcolrte_rml_send_nb: wrong null argument: "
                "ec_h.handle = %p, ec_h.rank = %d\n",
                ec_h.handle, ec_h.rank);
        return HCOLL_ERROR;
    }
    ...
    size = (size_t) data.rep.in_line_rep.data_handle.in_line.packed_size * count / 8;
    HCOL_VERBOSE(30, "PML_ISEND: dest = %d: buf = %p: size = %u: comm = %p", ec_h.rank, buffer,
                 (unsigned int) size, (void *) comm);
    if (MCA_PML_CALL(isend(buffer, size, &(ompi_mpi_unsigned_char.dt), ec_h.rank, tag,
                           MCA_PML_BASE_SEND_STANDARD, comm, &ompi_req))) {
        return HCOLL_ERROR;
    }
    req->data = (void *) ompi_req;
    req->status = HCOLRTE_REQUEST_ACTIVE;
...
static int get_ec_handles(int num_ec, int *ec_indexes, rte_grp_handle_t grp_h,
                          rte_ec_handle_t *ec_handles)
{
    int i;
    ompi_communicator_t *comm = (ompi_communicator_t *) grp_h;
...
{
    ompi_request_t *ompi_req = (ompi_request_t *) handle;
    opal_free_list_return(&mca_coll_hcoll_component.requests, (opal_free_list_item_t *) ompi_req);
}
                                 ...
                                 int *num_datatypes, hcoll_mpi_type_combiner_t *combiner)
{
    ...
    rc = ompi_datatype_get_args((ompi_datatype_t *) mpi_type, 0, num_integers, NULL, num_addresses,
                                NULL, num_datatypes, NULL, &ompi_combiner);
                                 ...
                                 void *array_of_addresses, void *array_of_datatypes)
{
    ...
    rc = ompi_datatype_get_args((ompi_datatype_t *) mpi_type, 1, &max_integers, array_of_integers,
                                &max_addresses, array_of_addresses, &max_datatypes,
                                array_of_datatypes, NULL);
    return rc == OMPI_SUCCESS ? HCOLL_SUCCESS : HCOLL_ERROR;
}
...
{
    int rc;
    mca_coll_hcoll_dtype_t *hcoll_dtype = (mca_coll_hcoll_dtype_t *) opal_free_list_get(
        &mca_coll_hcoll_component.dtypes);
    ompi_datatype_t *dtype = (ompi_datatype_t *) mpi_type;
    hcoll_dtype->type = hcoll_type;
    rc = ompi_attr_set_c(TYPE_ATTR, (void *) dtype, &(dtype->d_keyhash), hcoll_type_attr_keyval,
                         (void *) hcoll_dtype, false);
    if (OMPI_SUCCESS != rc) {
    ...
    return HCOLL_SUCCESS;
Cleanup:
    opal_free_list_return(&mca_coll_hcoll_component.dtypes, &hcoll_dtype->super);
    return rc;
}
