...
int mca_coll_hcoll_get_lib(void)
{
    ...
    memset(&mca_coll_hcoll_component.hcoll_ops, 0, sizeof(mca_coll_hcoll_component.hcoll_ops));
...
enum { REGSTR_EMPTY_OK = 0x01, REGSTR_MAX = 0x88 };
                   ...
                   const char *param_desc, int default_value, int *storage, int flags)
{
    ...
    *storage = default_value;
    index = mca_base_component_var_register(&mca_coll_hcoll_component.super.collm_version,
                                            param_name, param_desc, MCA_BASE_VAR_TYPE_INT, NULL, 0,
...
#define CHECK(expr)              \
    ...
    CHECK(reg_int("priority", NULL, "Priority of the hcol coll component", 90,
                  &mca_coll_hcoll_component.hcoll_priority, 0));

    CHECK(reg_int("verbose", NULL, "Verbose level of the hcol coll component", 0,
                  &mca_coll_hcoll_component.hcoll_verbose, 0));

    CHECK(reg_int("enable", NULL, "[1|0|] Enable/Disable HCOL", 1,
    ...
    CHECK(reg_int("np", NULL,
                  ...
                  2, &mca_coll_hcoll_component.hcoll_np, 0));

    CHECK(reg_int("datatype_fallback", NULL,
                  "[1|0|] Enable/Disable user defined datatypes fallback", 1,
                  &mca_coll_hcoll_component.hcoll_datatype_fallback, 0));
#if HCOLL_API >= HCOLL_VERSION(3, 6)
    CHECK(reg_int("dts", NULL, "[1|0|] Enable/Disable derived types support", 1,
    ...
    mca_base_component_var_register(
        &mca_coll_hcoll_component.super.collm_version, MCA_COMPILETIME_VER,
        "Version of the libhcoll library with which Open MPI was compiled",
        MCA_BASE_VAR_TYPE_VERSION_STRING, NULL, 0, 0, OPAL_INFO_LVL_3, MCA_BASE_VAR_SCOPE_READONLY,
        &mca_coll_hcoll_component.compiletime_version);
    mca_coll_hcoll_component.runtime_version = hcoll_get_version();
    mca_base_component_var_register(
        &mca_coll_hcoll_component.super.collm_version, MCA_RUNTIME_VER,
