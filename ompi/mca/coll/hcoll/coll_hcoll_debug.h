...
#define HCOL_VERBOSE(level, format, ...)                                                     \
    opal_output_verbose(level, mca_coll_hcoll_output, "%s:%d - %s() " format, __HCOL_FILE__, \
                        __LINE__, __FUNCTION__, ##__VA_ARGS__)

#define HCOL_ERROR(format, ...)                                                                 \
    opal_output_verbose(0, mca_coll_hcoll_output, "Error: %s:%d - %s() " format, __HCOL_FILE__, \
                        __LINE__, __FUNCTION__, ##__VA_ARGS__)

#define HCOL_MODULE_VERBOSE(hcoll_module, level, format, ...)                                   \
    HCOL_VERBOSE(level, "[%p:%d] " format, (void *) (hcoll_module)->comm, (hcoll_module)->rank, \
                 ##__VA_ARGS__)
