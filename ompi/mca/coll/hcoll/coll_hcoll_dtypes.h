...
enum { TRY_FIND_DERIVED, NO_DERIVED };

#if HCOLL_API >= HCOLL_VERSION(3, 6)
static inline void hcoll_map_derived_type(ompi_datatype_t *dtype,
                                          dte_data_representation_t *new_dte)
{
    int rc;
    if (NULL == dtype->args) {
    ...
    if (rc != HCOLL_SUCCESS) {
        ...
        ompi_attr_set_c(TYPE_ATTR, (void *) dtype, &(dtype->d_keyhash), hcoll_type_attr_keyval,
                        &zero_dte_mapping, false);
    }
}
...
static dte_data_representation_t find_derived_mapping(ompi_datatype_t *dtype)
{
    ...
    if (mca_coll_hcoll_component.derived_types_support_enabled) {
        int map_found = 0;
        ompi_attr_get_c(dtype->d_keyhash, hcoll_type_attr_keyval, (void **) &hcoll_dtype,
                        &map_found);
        if (!map_found)
...
static inline dte_data_representation_t ompi_predefined_derived_2_hcoll(int ompi_id)
{
    switch (ompi_id) {
...
static dte_data_representation_t ompi_dtype_2_hcoll_dtype(ompi_datatype_t *dtype, const int mode)
{
...
static int hcoll_type_attr_del_fn(MPI_Datatype type, int keyval, void *attr_val, void *extra)
{
    ...
    mca_coll_hcoll_dtype_t *dtype = (mca_coll_hcoll_dtype_t *) attr_val;
    ...
    opal_free_list_return(&mca_coll_hcoll_component.dtypes, &dtype->super);
