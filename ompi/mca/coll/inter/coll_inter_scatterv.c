                                  ...
                                  struct ompi_communicator_t *comm, mca_coll_base_module_t *module)
{
    ...
    if (MPI_PROC_NULL == root) {
        ...
        if (0 == rank) {
            ...
            err = MCA_PML_CALL(recv(counts, size_local, MPI_INT, root, MCA_COLL_BASE_TAG_SCATTERV,
            ...
            err = MCA_PML_CALL(recv(ptmp, total, rdtype, root, MCA_COLL_BASE_TAG_SCATTERV, comm,
                                    MPI_STATUS_IGNORE));
            if (OMPI_SUCCESS != err) {
                return err;
            }
        ...
        err = comm->c_local_comm->c_coll
                  ->coll_scatterv(ptmp, counts, displace, rdtype, rbuf, rcount, rdtype, 0,
                                  comm->c_local_comm,
        ...
        err = MCA_PML_CALL(send(scounts, size, MPI_INT, 0, MCA_COLL_BASE_TAG_SCATTERV,
        ...
        err = MCA_PML_CALL(
            send(sbuf, 1, ndtype, 0, MCA_COLL_BASE_TAG_SCATTERV, MCA_PML_BASE_SEND_STANDARD, comm));
        if (OMPI_SUCCESS != err) {
            return err;
        }
