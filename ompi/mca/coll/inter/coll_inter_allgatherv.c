                                    ...
                                    mca_coll_base_module_t *module)
{
    ...
    if (0 == rank) {
        ...
        err = ompi_coll_base_sendrecv_actual(ptmp, total, sdtype, 0, MCA_COLL_BASE_TAG_ALLGATHERV,
                                             rbuf, 1, ndtype, 0, MCA_COLL_BASE_TAG_ALLGATHERV, comm,
                                             MPI_STATUS_IGNORE);
        if (OMPI_SUCCESS != err) {
            goto exit;
        }
    ...
    err = comm->c_local_comm->c_coll->coll_bcast(rbuf, 1, ndtype, 0, comm->c_local_comm,
