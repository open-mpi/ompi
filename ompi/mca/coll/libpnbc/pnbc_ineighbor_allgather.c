...
int PNBC_Ineighbor_allgather_args_compare(PNBC_Ineighbor_allgather_args *a, PNBC_Ineighbor_allgather_args *b, void *param) {
  if ((a->sbuf == b->sbuf) &&
      (a->scount == b->scount) &&
      (a->stype == b->stype) &&
      (a->rbuf == b->rbuf) &&
...
int ompi_coll_libpnbc_ineighbor_allgather(const void *sbuf, int scount, MPI_Datatype stype, void *rbuf,
...
#ifdef PNBC_CACHE_SCHEDULE
  PNBC_Ineighbor_allgather_args *args, *found, search;
  ...
  search.rtype = rtype;
  found = (PNBC_Ineighbor_allgather_args *) hb_tree_search ((hb_tree *) libpnbc_module->PNBC_Dict[PNBC_NEIGHBOR_ALLGATHER],
                                                           &search);
  if (NULL == found) {
#endif
    ...
    args = (PNBC_Ineighbor_allgather_args *) malloc (sizeof (args));
    if (NULL != args) {
      args->sbuf = sbuf;
      args->scount = scount;
      args->stype = stype;
      ...
      res = hb_tree_insert ((hb_tree *) libpnbc_module->PNBC_Dict[PNBC_NEIGHBOR_ALLGATHER], args, args, 0);
      if (0 == res) {
        OBJ_RETAIN(schedule);

        /* increase number of elements for A2A */
        if (++libpnbc_module->PNBC_Dict_size[PNBC_NEIGHBOR_ALLGATHER] > PNBC_SCHED_DICT_UPPER) {
          PNBC_SchedCache_dictwipe ((hb_tree *) libpnbc_module->PNBC_Dict[PNBC_NEIGHBOR_ALLGATHER],
                                   &libpnbc_module->PNBC_Dict_size[PNBC_NEIGHBOR_ALLGATHER]);
        }
      } else {
