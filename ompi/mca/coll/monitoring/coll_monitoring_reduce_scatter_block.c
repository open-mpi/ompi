...
int mca_coll_monitoring_reduce_scatter_block(const void *sbuf, void *rbuf, int rcount,
                                             ...
                                             mca_coll_base_module_t *module)
{
    ...
    return monitoring_module->real
        .coll_reduce_scatter_block(sbuf, rbuf, rcount, dtype, op, comm,
                                   monitoring_module->real.coll_reduce_scatter_block_module);
}

int mca_coll_monitoring_ireduce_scatter_block(const void *sbuf, void *rbuf, int rcount,
                                              struct ompi_datatype_t *dtype, struct ompi_op_t *op,
                                              struct ompi_communicator_t *comm,
                                              ...
                                              mca_coll_base_module_t *module)
{
    ...
    mca_common_monitoring_coll_a2a(data_size * (comm_size - 1), monitoring_module->data);
    return monitoring_module->real
        .coll_ireduce_scatter_block(sbuf, rbuf, rcount, dtype, op, comm, request,
                                    monitoring_module->real.coll_ireduce_scatter_block_module);
}
