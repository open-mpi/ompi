...
int mca_common_ompio_file_open(ompi_communicator_t *comm, const char *filename, int amode,
                               opal_info_t *info, ompio_file_t *ompio_fh, bool use_sharedfp)
{
    ...
    if (OMPI_SUCCESS != (ret = mca_fs_base_file_select(ompio_fh, NULL))) {
        opal_output(1, "mca_fs_base_file_select() failed\n");
        goto fn_fail;
    }
    if (OMPI_SUCCESS != (ret = mca_fbtl_base_file_select(ompio_fh, NULL))) {
    ...
    if (true == use_sharedfp) {
        if (OMPI_SUCCESS != (ret = mca_sharedfp_base_file_select(ompio_fh, NULL))) {
            ...
            ** pointer operations are not used. However, the first call to any
            *file_read/write_shared
    ...
    ret = ompio_fh->f_fs->fs_file_open(comm, filename, amode, info, ompio_fh);
    ...
    if (true == use_sharedfp) {
        ...
        */
        if (NULL != ompio_fh->f_sharedfp) {
            ret = ompio_fh->f_sharedfp->sharedfp_file_open(comm, filename, amode, info, ompio_fh);
    ...
    mca_common_ompio_set_view(ompio_fh, 0, &ompi_mpi_byte.dt, &ompi_mpi_byte.dt, "native", info);
    ...
    if (ompio_fh->f_amode & MPI_MODE_APPEND) {
        ...
        ompio_fh->f_fs->fs_file_get_size(ompio_fh, &current_size);
...
int mca_common_ompio_file_close(ompio_file_t *ompio_fh)
{
    ...
    ret = ompio_fh->f_comm->c_coll->coll_barrier(ompio_fh->f_comm,
                                                 ompio_fh->f_comm->c_coll->coll_barrier_module);
    if (OMPI_SUCCESS != ret) {
        /* Not sure what to do */
    ...
    if (OMPIO_MCA_GET(ompio_fh, coll_timing_info)) {
        ...
        if (!mca_common_ompio_empty_print_queue(ompio_fh->f_coll_write_time)) {
            ret = mca_common_ompio_print_time_info(ompio_fh->f_coll_write_time, name, ompio_fh);
            if (OMPI_SUCCESS != ret) {
        ...
        if (!mca_common_ompio_empty_print_queue(ompio_fh->f_coll_read_time)) {
            ret = mca_common_ompio_print_time_info(ompio_fh->f_coll_read_time, name, ompio_fh);
            if (OMPI_SUCCESS != ret) {
                printf("Error in print_time_info ");
            }
        }
...
int mca_common_ompio_file_get_size(ompio_file_t *ompio_fh, OMPI_MPI_OFFSET_TYPE *size)
{
    int ret = OMPI_SUCCESS;
...
int mca_common_ompio_file_get_position(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE *offset)
{
    OMPI_MPI_OFFSET_TYPE off;

    if (0 == fh->f_view_extent || 0 == fh->f_view_size || 0 == fh->f_etype_size) {
...
int mca_common_ompio_set_file_defaults(ompio_file_t *fh)
{
    ...
    if (NULL != fh) {
        ...
        ompi_datatype_create_struct(2, blocklen, d, types, &fh->f_iov_type);
...
int mca_common_ompio_file_delete(const char *filename, struct opal_info_t *info)
{
    int ret = OMPI_SUCCESS;
...
int mca_common_ompio_create_incomplete_file_handle(const char *filename, ompio_file_t **fh)
{
    ompio_file_t *file;
...
int mca_common_ompio_decode_datatype(struct ompio_file_t *fh, ompi_datatype_t *datatype, int count,
                                     const void *buf, size_t *max_data, opal_convertor_t *conv,
...
int mca_common_ompio_set_callbacks(
