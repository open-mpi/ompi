...
int mca_common_ompio_set_view(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE disp, ompi_datatype_t *etype,
                              ompi_datatype_t *filetype, const char *datarep, opal_info_t *info)
{
    ...
    if ((MPI_DISPLACEMENT_CURRENT == disp) && (fh->f_amode & MPI_MODE_SEQUENTIAL)) {
        ...
        if (NULL == shared_fp_base_module) {
            opal_output(0,
                        "No shared file pointer component found for this file. Can not execute\n");
            return OMPI_ERROR;
        }
    ...
    if (etype == filetype && ompi_datatype_is_predefined(filetype)
        && ftype_extent == (ptrdiff_t) ftype_size) {
        ompi_datatype_create_contiguous(MCA_IO_DEFAULT_FILE_VIEW_SIZE, &ompi_mpi_byte.dt,
                                        &newfiletype);
    ...
    mca_common_ompio_decode_datatype(fh, newfiletype, 1, NULL, &max_data, fh->f_file_convertor,
                                     &fh->f_decoded_iov, &fh->f_iov_count);
    ...
    contg_groups = (mca_common_ompio_contg *) calloc(1,
                                                     fh->f_size * sizeof(mca_common_ompio_contg));
    ...
    if (-1 != OMPIO_MCA_GET(fh, num_aggregators) || -1 != num_cb_nodes) {
        ...
        if (SIMPLE != OMPIO_MCA_GET(fh, grouping_option)
            ...
            ret = mca_common_ompio_fview_based_grouping(fh, &num_groups, contg_groups);
            if (OMPI_SUCCESS != ret) {
                opal_output(
                    1, "mca_common_ompio_set_view: mca_io_ompio_fview_based_grouping failed\n");
                goto exit;
            }
        } else {
            int done = 0;
            ...
            if (fh->f_comm->c_flags & OMPI_COMM_CART) {
                ...
                if (ndims > 1) {
                    ret = mca_common_ompio_cart_based_grouping(fh, &num_groups, contg_groups);
                    if (OMPI_SUCCESS != ret) {
                        opal_output(
                            1,
                            "mca_common_ompio_set_view: mca_io_ompio_cart_based_grouping failed\n");
                        goto exit;
                    }
                    done = 1;
                }
            ...
            if (!done) {
                ret = mca_common_ompio_simple_grouping(fh, &num_groups, contg_groups);
                if (OMPI_SUCCESS != ret) {
                    opal_output(1,
                                "mca_common_ompio_set_view: mca_io_ompio_simple_grouping failed\n");
                    goto exit;
                }
            }
        }
    ...
    if (fh->f_rank == 0) {
        ...
        for (ii = 0; ii < num_groups; ii++) {
            printf("contg_groups[%d].procs_per_contg_group=%d\n", ii,
                   contg_groups[ii].procs_per_contg_group);
    ...
    ret = mca_common_ompio_finalize_initial_grouping(fh, num_groups, contg_groups);
    if (OMPI_SUCCESS != ret) {
        opal_output(1,
                    "mca_common_ompio_set_view: mca_io_ompio_finalize_initial_grouping failed\n");
        goto exit;
    }
    ...
    if (flag) {
        if (strncmp(stripe_str->string, "false", sizeof("true"))) {
            ...
            OMPIO_MCA_PRINT_INFO(fh, "collective_buffering", stripe_str->string,
                                 "enforcing using individual fcoll component");
        } else {
            OMPIO_MCA_PRINT_INFO(fh, "collective_buffering", stripe_str->string, "");
        }
        OBJ_RELEASE(stripe_str);
...
OMPI_MPI_OFFSET_TYPE get_contiguous_chunk_size(ompio_file_t *fh, int flag)
{
    ...
    */
    ...
    if (flag) {
        ...
        fh->f_comm->c_coll->coll_allreduce(avg, global_avg, 3, OMPI_OFFSET_DATATYPE, MPI_SUM,
