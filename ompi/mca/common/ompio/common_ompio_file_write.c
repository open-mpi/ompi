...
int mca_common_ompio_file_write(ompio_file_t *fh, const void *buf, int count,
                                struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    ...
    if (need_to_copy) {
        ...
        OMPIO_PREPARE_BUF(fh, buf, count, datatype, tbuf, &convertor, max_data, decoded_iov,
                          iov_count);
    ...
    for (index = 0; index < cycles; index++) {
        mca_common_ompio_build_io_array(fh, index, cycles, bytes_per_cycle, max_data, iov_count,
                                        decoded_iov, &i, &j, &total_bytes_written, &spc,
...
int mca_common_ompio_file_write_at(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, const void *buf,
...
int mca_common_ompio_file_iwrite(ompio_file_t *fh, const void *buf, int count,
                                 struct ompi_datatype_t *datatype, ompi_request_t **request)
{
    ...
    if (NULL != fh->f_fbtl->fbtl_ipwritev) {
        ...
        mca_common_ompio_build_io_array(fh,
                                        ...
                                        max_data, iov_count, decoded_iov, &i, &j,
                                        &total_bytes_written, &spc, &fh->f_io_array,
...
int mca_common_ompio_file_iwrite_at(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, const void *buf,
                                    int count, struct ompi_datatype_t *datatype,
                                    ompi_request_t **request)
{
    ...
    mca_common_ompio_set_explicit_offset(fh, offset);
    ret = mca_common_ompio_file_iwrite(fh, buf, count, datatype, request);
...
/******************************************************************/
int mca_common_ompio_file_write_all(ompio_file_t *fh, const void *buf, int count,
                                    struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    ...
    if (!(fh->f_flags & OMPIO_DATAREP_NATIVE)
        ...
        */
        ...
        ret = fh->f_fcoll->fcoll_file_write_all(fh, decoded_iov->iov_base, decoded_iov->iov_len,
                                                MPI_BYTE, status);
        ...
        ret = fh->f_fcoll->fcoll_file_write_all(fh, buf, count, datatype, status);
    }
    return ret;
}

int mca_common_ompio_file_write_at_all(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE offset,
                                       ...
                                       ompi_status_public_t *status)
{
    ...
    mca_common_ompio_set_explicit_offset(fh, offset);
    ret = mca_common_ompio_file_write_all(fh, buf, count, datatype, status);

    mca_common_ompio_set_explicit_offset(fh, prev_offset);
    return ret;
}

int mca_common_ompio_file_iwrite_all(ompio_file_t *fp, const void *buf, int count,
                                     struct ompi_datatype_t *datatype, ompi_request_t **request)
{
    int ret = OMPI_SUCCESS;

    if (NULL != fp->f_fcoll->fcoll_file_iwrite_all) {
        ret = fp->f_fcoll->fcoll_file_iwrite_all(fp, buf, count, datatype, request);
    } else {
...
int mca_common_ompio_file_iwrite_at_all(ompio_file_t *fp, OMPI_MPI_OFFSET_TYPE offset,
...
int mca_common_ompio_build_io_array(ompio_file_t *fh, int index, int cycles, size_t bytes_per_cycle,
                                    size_t max_data, uint32_t iov_count, struct iovec *decoded_iov,
                                    int *ii, int *jj, size_t *tbw, size_t *spc,
                                    mca_common_ompio_io_array_t **io_array, int *num_io_entries)
{
    ptrdiff_t disp;
    int block = 1;
    ...
    f_io_array = (mca_common_ompio_io_array_t *) malloc(OMPIO_IOVEC_INITIAL_SIZE
    ...
    while (bytes_to_write_in_cycle) {
        ...
        if (OMPIO_IOVEC_INITIAL_SIZE * block <= k) {
            block++;
            f_io_array = (mca_common_ompio_io_array_t *)
                realloc(f_io_array,
                        OMPIO_IOVEC_INITIAL_SIZE * block * sizeof(mca_common_ompio_io_array_t));
            if (NULL == f_io_array) {
                opal_output(1, "OUT OF MEMORY\n");
                return OMPI_ERR_OUT_OF_RESOURCE;
            }
        ...
        if (decoded_iov[i].iov_len - (total_bytes_written - sum_previous_counts) <= 0) {
            sum_previous_counts += decoded_iov[i].iov_len;
            i = i + 1;
        }

        disp = (ptrdiff_t) decoded_iov[i].iov_base + (total_bytes_written - sum_previous_counts);
        f_io_array[k].memory_address = (IOVBASE_TYPE *) disp;

        if (decoded_iov[i].iov_len - (total_bytes_written - sum_previous_counts)
        ...
        if (!(fh->f_flags & OMPIO_CONTIGUOUS_FVIEW)) {
            if (fh->f_decoded_iov[j].iov_len - (fh->f_total_bytes - sum_previous_length) <= 0) {
        ...
        if (!(fh->f_flags & OMPIO_CONTIGUOUS_FVIEW)) {
            if (fh->f_decoded_iov[j].iov_len - (fh->f_total_bytes - sum_previous_length)
                < f_io_array[k].length) {
