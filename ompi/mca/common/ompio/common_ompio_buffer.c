...
void mca_common_ompio_check_gpu_buf(ompio_file_t *fh, const void *buf, int *is_gpu, int *is_managed)
{
...
int mca_common_ompio_buffer_alloc_init(void)
{
    ...
    mca_common_ompio_allocator = mca_common_ompio_allocator_component
                                     ->allocator_init(thread_safe,
                                                      mca_common_ompio_buffer_alloc_seg,
                                                      mca_common_ompio_buffer_free_seg, NULL);
...
void *mca_common_ompio_alloc_buf(ompio_file_t *fh, size_t bufsize)
{
    ...
    OPAL_THREAD_LOCK(&mca_common_ompio_buffer_mutex);
    tmp = mca_common_ompio_allocator->alc_alloc(mca_common_ompio_allocator, bufsize, 0);
    OPAL_THREAD_UNLOCK(&mca_common_ompio_buffer_mutex);
    return tmp;
}
...
void mca_common_ompio_release_buf(ompio_file_t *fh, void *buf)
{
    ...
    OPAL_THREAD_LOCK(&mca_common_ompio_buffer_mutex);
    mca_common_ompio_allocator->alc_free(mca_common_ompio_allocator, buf);
    OPAL_THREAD_UNLOCK(&mca_common_ompio_buffer_mutex);
