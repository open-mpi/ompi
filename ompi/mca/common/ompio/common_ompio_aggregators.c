...
int mca_common_ompio_simple_grouping(ompio_file_t *fh, int *num_groups_out,
                                     mca_common_ompio_contg *contg_groups)
{
    int num_groups = 1;

    double time = 0.0, time_prev = 0.0, dtime = 0.0, dtime_abs = 0.0, dtime_diff = 0.0,
           dtime_prev = 0.0;
    ...
    */
    ...
    num_groups = P_a_prev;
#ifdef OMPIO_DEBUG
    printf(" For P=%d d_p=%ld b_c=%d threshold=%f chosen P_a = %d \n", fh->f_size, fh->f_cc_size,
           fh->f_bytes_per_agg, dtime_threshold, P_a_prev);
#endif
...
int mca_common_ompio_forced_grouping(ompio_file_t *fh, int num_groups,
                                     mca_common_ompio_contg *contg_groups)
{
                                          ...
                                          mca_common_ompio_contg *contg_groups)
{
    ...
    start_offsets_lens = (OMPI_MPI_OFFSET_TYPE *) malloc(3 * fh->f_size
    ...
    ret = fh->f_comm->c_coll->coll_allgather(start_offset_len, 3, OMPI_OFFSET_DATATYPE,
                                         ...
                                         mca_common_ompio_contg *contg_groups)
{
    ...
    ret = ompio_fh->f_comm->c_topo->topo.cart.cart_get(ompio_fh->f_comm, cart_topo.ndims,
                                                       cart_topo.dims, cart_topo.periods,
                                                       cart_topo.coords);
    if (OMPI_SUCCESS != ret) {
    ...
    *num_groups = cart_topo.dims[0]; // number of rows
    ...
    for (k = 0; k < cart_topo.dims[0]; k++) {
        ...
        ret = ompio_fh->f_comm->c_topo->topo.cart.cart_rank(ompio_fh->f_comm, coords_tmp,
                                                            &tmp_rank);
        if (OMPI_SUCCESS != ret) {
    ...
    for (z = 0; z < num_groups; z++) {
        for (y = 0; y < contg_groups[z].procs_per_contg_group; y++) {
            if (fh->f_rank == contg_groups[z].procs_in_contg_group[y]) {
                ...
                fh->f_init_procs_in_group = (int *) malloc(fh->f_init_procs_per_group
                                                           * sizeof(int));
                if (NULL == fh->f_init_procs_in_group) {
                    opal_output(1, "OUT OF MEMORY\n");
                    return OMPI_ERR_OUT_OF_RESOURCE;
...
int mca_common_ompio_set_aggregator_props(struct ompio_file_t *fh, int num_aggregators,
...
int mca_common_ompio_create_groups(ompio_file_t *fh, size_t bytes_per_proc)
{
    ...
    ret = mca_common_ompio_prepare_to_group(fh, &start_offsets_lens, &end_offsets,
                                            &aggr_bytes_per_group, &bytes_per_group, &decision_list,
                                            bytes_per_proc, &is_aggregator, &ompio_grouping_flag);
    if (OMPI_SUCCESS != ret) {
        opal_output(1,
                    "mca_common_ompio_create_groups: error in mca_common_ompio_prepare_to_group\n");
        goto exit;
    }
    ...
    case OMPIO_SPLIT:
        ret = mca_common_ompio_split_initial_groups(fh, start_offsets_lens, end_offsets,
                                                    bytes_per_group);
        break;

    case OMPIO_MERGE:
        ret = mca_common_ompio_merge_initial_groups(fh, aggr_bytes_per_group, decision_list,
                                                    is_aggregator);
        break;
    ...
    if (OMPI_SUCCESS != ret) {
        opal_output(
            1,
            "mca_common_ompio_create_groups: error in subroutine called within switch statement\n");
        goto exit;
    }
    ...
    ret = fh->f_comm->c_coll->coll_allreduce(&final_aggr, &final_num_aggrs, 1, MPI_INT, MPI_SUM,
    ...
    ret = fh->f_comm->c_coll->coll_allgather(&final_aggr, 1, MPI_INT, tmp_final_aggrs, 1, MPI_INT,
                                             fh->f_comm, fh->f_comm->c_coll->coll_allgather_module);
    if (OMPI_SUCCESS != ret) {
        opal_output(1, "mca_common_ompio_create_groups: error in allreduce\n");
        goto exit;
                                          ...
                                          int *decision_list, int is_aggregator)
{
    ...
    if (is_aggregator) {
        ...
        while (i < fh->f_init_num_aggrs) {
            if (decision_list[i] >= first_merge_flag) {
                start = i;
                while ((decision_list[i] >= first_merge_flag) && (i < fh->f_init_num_aggrs - 1)) {
                ...
                for (j = 0; j < end - start + 1; j++) {
                    if (fh->f_rank == merge_aggrs[j]) {
                        ret = mca_common_ompio_merge_groups(fh, merge_aggrs, end - start + 1);
                        if (OMPI_SUCCESS != ret) {
                            opal_output(1, "mca_common_ompio_merge_initial_groups: error in "
                                           "mca_common_ompio_merge_groups\n");
                            free(merge_aggrs);
                            return ret;
                        }
                    }
    ...
    if (is_new_aggregator) {
        ...
        for (j = 0; j < fh->f_procs_per_group; j++) {
            ...
            ret = MCA_PML_CALL(isend(&fh->f_procs_per_group, 1, MPI_INT, fh->f_procs_in_group[j],
                                     OMPIO_PROCS_PER_GROUP_TAG, MCA_PML_BASE_SEND_STANDARD,
                                     fh->f_comm, sendreqs + r++));
            if (OMPI_SUCCESS != ret) {
            ...
            ret = MCA_PML_CALL(isend(fh->f_procs_in_group, fh->f_procs_per_group, MPI_INT,
                                     fh->f_procs_in_group[j], OMPIO_PROCS_IN_GROUP_TAG,
                                     MCA_PML_BASE_SEND_STANDARD, fh->f_comm, sendreqs + r++));
            if (OMPI_SUCCESS != ret) {
        ...
        ret = MCA_PML_CALL(recv(&fh->f_procs_per_group, 1, MPI_INT, MPI_ANY_SOURCE,
                                OMPIO_PROCS_PER_GROUP_TAG, fh->f_comm, MPI_STATUS_IGNORE));
        if (OMPI_SUCCESS != ret) {
        ...
        ret = MCA_PML_CALL(recv(fh->f_procs_in_group, fh->f_procs_per_group, MPI_INT,
                                MPI_ANY_SOURCE, OMPIO_PROCS_IN_GROUP_TAG, fh->f_comm,
                                MPI_STATUS_IGNORE));
        if (OMPI_SUCCESS != ret) {
                                          ...
                                          OMPI_MPI_OFFSET_TYPE bytes_per_group)
{
    ...
    // integer round up
    size_new_group = (int) (bytes_per_agg_group / bytes_per_group
                            + (bytes_per_agg_group % bytes_per_group ? 1u : 0u));
    size_old_group = fh->f_init_procs_per_group;

    ret = mca_common_ompio_split_a_group(fh, start_offsets_lens, end_offsets, size_new_group,
                                         &max_cci, &min_cci, &num_groups, &size_smallest_group);
    if (OMPI_SUCCESS != ret) {
        opal_output(
            1, "mca_common_ompio_split_initial_groups: error in mca_common_ompio_split_a_group\n");
        return ret;
    }
    ...
    case CONTIGUITY:
        ...
        while (1) {
            if ((max_cci < OMPIO_CONTG_THRESHOLD) && (size_new_group < size_old_group)) {

                size_new_group = (size_new_group + size_old_group) / 2;
                ret = mca_common_ompio_split_a_group(fh, start_offsets_lens, end_offsets,
                                                     size_new_group, &max_cci, &min_cci,
                                                     &num_groups, &size_smallest_group);
                if (OMPI_SUCCESS != ret) {
                    opal_output(1, "mca_common_ompio_split_initial_groups: error in "
                                   "mca_common_ompio_split_a_group 2\n");
                    return ret;
                }
            } else {
                break;
    ...
    case OPTIMIZE_GROUPING:
        ...
        while (1) {
            if ((max_cci < OMPIO_CONTG_THRESHOLD)
                ...
                if (OMPI_SUCCESS != ret) {
                    opal_output(1, "mca_common_ompio_split_initial_groups: error in "
                                   "mca_common_ompio_split_a_group 3\n");
                    return ret;
                }
            } else {
                break;
...
int mca_common_ompio_merge_groups(ompio_file_t *fh, int *merge_aggrs, int num_merge_aggrs)
{
    int i = 0;
    int *sizes_old_group;
    int ret;
    ...
    ret = ompi_fcoll_base_coll_allgather_array(&fh->f_init_procs_per_group, 1, MPI_INT,
                                               sizes_old_group, 1, MPI_INT, 0, merge_aggrs,
                                               num_merge_aggrs, fh->f_comm);
    ...
    ret = ompi_fcoll_base_coll_allgatherv_array(fh->f_init_procs_in_group,
                                                fh->f_init_procs_per_group, MPI_INT,
                                                fh->f_procs_in_group, sizes_old_group, displs,
                                                MPI_INT, 0, merge_aggrs, num_merge_aggrs,
                                                fh->f_comm);
...
int mca_common_ompio_split_a_group(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE *start_offsets_lens,
                                   OMPI_MPI_OFFSET_TYPE *end_offsets, int size_new_group,
                                   OMPI_MPI_OFFSET_TYPE *max_cci, OMPI_MPI_OFFSET_TYPE *min_cci,
                                   int *num_groups, int *size_smallest_group)
{
    ...
    *size_smallest_group = size_new_group;
    ...
    for (i = 0; i < *num_groups; i++) {
        ...
        for (k = 0; k < size - 1; k++) {
            if (end_offsets[size_new_group * i + k]
                == start_offsets_lens[3 * size_new_group * i + 3 * (k + 1)]) {
...
int mca_common_ompio_finalize_split(ompio_file_t *fh, int size_new_group, int size_last_group)
{
    ...
    for (i = 0; i < fh->f_init_procs_per_group; i++) {
        if (fh->f_rank == fh->f_init_procs_in_group[i]) {
            if (i >= fh->f_init_procs_per_group - size_last_group) {
                ...
                for (j = 0; j < fh->f_procs_per_group; j++) {
                    fh->f_procs_in_group[j] = fh->f_init_procs_in_group[fh->f_init_procs_per_group
                                                                        - size_last_group + j];
                }
            } else {
...
int mca_common_ompio_prepare_to_group(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE **start_offsets_lens,
                                      OMPI_MPI_OFFSET_TYPE **end_offsets, // need it?
                                      OMPI_MPI_OFFSET_TYPE **aggr_bytes_per_group,
                                      OMPI_MPI_OFFSET_TYPE *bytes_per_group, int **decision_list,
                                      size_t bytes_per_proc, int *is_aggregator,
                                      int *ompio_grouping_flag)
{
    ...
    start_offsets_lens_tmp = (OMPI_MPI_OFFSET_TYPE *) malloc(3 * fh->f_init_procs_per_group
                                                             * sizeof(OMPI_MPI_OFFSET_TYPE));
    ...
    ret = ompi_fcoll_base_coll_allgather_array(start_offset_len, 3, OMPI_OFFSET_DATATYPE,
                                               start_offsets_lens_tmp, 3, OMPI_OFFSET_DATATYPE, 0,
                                               fh->f_init_procs_in_group,
                                               fh->f_init_procs_per_group, fh->f_comm);
    if (OMPI_SUCCESS != ret) {
        opal_output(
            1,
            "mca_common_ompio_prepare_to_group: error in ompi_fcoll_base_coll_allgather_array\n");
        free(start_offsets_lens_tmp);
        goto exit;
    }
    end_offsets_tmp = (OMPI_MPI_OFFSET_TYPE *) malloc(fh->f_init_procs_per_group
                                                      * sizeof(OMPI_MPI_OFFSET_TYPE));
    ...
    *end_offsets = &end_offsets_tmp[0];
    ...
    if (*is_aggregator == 1) {
        aggr_bytes_per_group_tmp = (OMPI_MPI_OFFSET_TYPE *) malloc(fh->f_init_num_aggrs
                                                                   * sizeof(OMPI_MPI_OFFSET_TYPE));
        ...
        ret = ompi_fcoll_base_coll_allgather_array(bytes_per_group, 1, OMPI_OFFSET_DATATYPE,
                                                   aggr_bytes_per_group_tmp, 1,
                                                   OMPI_OFFSET_DATATYPE, 0, fh->f_init_aggr_list,
                                                   fh->f_init_num_aggrs, fh->f_comm);
        if (OMPI_SUCCESS != ret) {
            opal_output(1, "mca_common_ompio_prepare_to_group: error in "
                           "ompi_fcoll_base_coll_allgather_array 2\n");
            free(decision_list_tmp);
            goto exit;
        }

        for (i = 0; i < fh->f_init_num_aggrs; i++) {
            if ((size_t)(aggr_bytes_per_group_tmp[i]) > (size_t) OMPIO_MCA_GET(fh, bytes_per_agg)) {
        ...
        *aggr_bytes_per_group = &aggr_bytes_per_group_tmp[0];
        ...
        for (i = 0; i < fh->f_init_num_aggrs; i++) {
            if (decision_list_tmp[i] == OMPIO_MERGE) {
                if ((i == 0) && (decision_list_tmp[i + 1] != OMPIO_MERGE)) { // first group
                    decision_list_tmp[i] = OMPIO_RETAIN;
    ...
    ret = ompi_fcoll_base_coll_bcast_array(ompio_grouping_flag, 1, MPI_INT, 0,
                                           fh->f_init_procs_in_group, fh->f_init_procs_per_group,
                                           fh->f_comm);
