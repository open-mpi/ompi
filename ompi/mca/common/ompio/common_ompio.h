...
#define OMPIO_MCA_PRINT_INFO(_fh, _infostr, _infoval, _msg)                                       \
    {                                                                                             \
        int _verbose = _fh->f_get_mca_parameter_value("verbose_info_parsing",                     \
                                                      strlen("verbose_info_parsing"));            \
        if (1 == _verbose && 0 == _fh->f_rank)                                                    \
            printf("File: %s info: %s value %s %s\n", _fh->f_filename, _infostr, _infoval, _msg); \
        if (2 == _verbose)                                                                        \
            printf("File: %s info: %s value %s %s\n", _fh->f_filename, _infostr, _infoval, _msg); \
    }
...
enum ompio_fs_type { NONE = 0, UFS = 1, PVFS2 = 2, LUSTRE = 3, PLFS = 4, IME = 5, GPFS = 6 };
...
typedef int (*mca_common_ompio_get_mca_parameter_value_fn_t)(char *mca_parameter_name,
                                                             int name_length);
...
OMPI_DECLSPEC int mca_common_ompio_file_write_at(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE offset,
...
OMPI_DECLSPEC int mca_common_ompio_file_write_all(ompio_file_t *fh, const void *buf, int count,
                                                  struct ompi_datatype_t *datatype,
                                                  ompi_status_public_t *status);

OMPI_DECLSPEC int mca_common_ompio_file_write_at_all(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE offset,
                                                     const void *buf, int count,
                                                     struct ompi_datatype_t *datatype,
                                                     ompi_status_public_t *status);

OMPI_DECLSPEC int mca_common_ompio_file_iwrite_all(ompio_file_t *fp, const void *buf, int count,
                                                   struct ompi_datatype_t *datatype,
                                                   ompi_request_t **request);

OMPI_DECLSPEC int mca_common_ompio_file_iwrite_at_all(ompio_file_t *fp, OMPI_MPI_OFFSET_TYPE offset,
                                                      const void *buf, int count,
                                                      struct ompi_datatype_t *datatype,
                                                      ompi_request_t **request);

OMPI_DECLSPEC int mca_common_ompio_build_io_array(ompio_file_t *fh, int index, int cycles,
                                                  size_t bytes_per_cycle, size_t max_data,
                                                  uint32_t iov_count, struct iovec *decoded_iov,
...
OMPI_DECLSPEC int mca_common_ompio_file_read_at(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE offset,
                                                void *buf, int count,
                                                struct ompi_datatype_t *datatype,
                                                ompi_status_public_t *status);
...
OMPI_DECLSPEC int mca_common_ompio_file_read_all(ompio_file_t *fh, void *buf, int count,
                                                 struct ompi_datatype_t *datatype,
                                                 ompi_status_public_t *status);
...
OMPI_DECLSPEC int mca_common_ompio_file_iread_all(ompio_file_t *fp, void *buf, int count,
                                                  struct ompi_datatype_t *datatype,
                                                  ompi_request_t **request);
...
OMPI_DECLSPEC int mca_common_ompio_file_open(ompi_communicator_t *comm, const char *filename,
                                             int amode, opal_info_t *info, ompio_file_t *ompio_fh,
                                             bool use_sharedfp);

OMPI_DECLSPEC int mca_common_ompio_file_delete(const char *filename, struct opal_info_t *info);
...
OMPI_DECLSPEC int mca_common_ompio_file_get_size(ompio_file_t *ompio_fh,
                                                 OMPI_MPI_OFFSET_TYPE *size);
OMPI_DECLSPEC int mca_common_ompio_file_get_position(ompio_file_t *fh,
                                                     OMPI_MPI_OFFSET_TYPE *offset);
OMPI_DECLSPEC int mca_common_ompio_set_explicit_offset(ompio_file_t *fh,
                                                       OMPI_MPI_OFFSET_TYPE offset);
...
OMPI_DECLSPEC int mca_common_ompio_decode_datatype(struct ompio_file_t *fh,
                                                   ...
                                                   const void *buf, size_t *max_data,
                                                   opal_convertor_t *convertor, struct iovec **iov,
                                                   uint32_t *iov_count);

OMPI_DECLSPEC int mca_common_ompio_set_callbacks(
    mca_common_ompio_generate_current_file_view_fn_t generate_current_file_view,
