...
int mca_common_ompio_file_read(ompio_file_t *fh, void *buf, int count,
                               struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    ...
    if (!(fh->f_flags & OMPIO_DATAREP_NATIVE)
        && !(datatype == &ompi_mpi_byte.dt || datatype == &ompi_mpi_char.dt)) {
    ...
    if (need_to_copy) {
        ...
        OMPIO_PREPARE_READ_BUF(fh, buf, count, datatype, tbuf, &convertor, max_data, decoded_iov,
                               iov_count);
    } else {
        mca_common_ompio_decode_datatype(fh, datatype, count, buf, &max_data, fh->f_mem_convertor,
    ...
    for (index = 0; index < cycles; index++) {
        ...
        mca_common_ompio_build_io_array(fh, index, cycles, bytes_per_cycle, max_data, iov_count,
                                        decoded_iov, &i, &j, &total_bytes_read, &spc,
...
int mca_common_ompio_file_read_at(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, void *buf,
...
int mca_common_ompio_file_iread(ompio_file_t *fh, void *buf, int count,
    ...
    if (NULL != fh->f_fbtl->fbtl_ipreadv) {
        ...
        if (need_to_copy) {
            ...
            OMPIO_PREPARE_READ_BUF(fh, buf, count, datatype, tbuf, &ompio_req->req_convertor,
                                   max_data, decoded_iov, iov_count);

            ompio_req->req_tbuf = tbuf;
            ompio_req->req_size = max_data;
        } else {
            mca_common_ompio_decode_datatype(fh, datatype, count, buf, &max_data,
                                             fh->f_mem_convertor, &decoded_iov, &iov_count);
        }
        ...
        mca_common_ompio_build_io_array(fh,
                                        ...
                                        max_data, iov_count, decoded_iov, &i, &j, &total_bytes_read,
                                        &spc, &fh->f_io_array, &fh->f_num_of_io_entries);
...
int mca_common_ompio_file_iread_at(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, void *buf,
                                   int count, struct ompi_datatype_t *datatype,
                                   ompi_request_t **request)
{
    ...
    mca_common_ompio_set_explicit_offset(fh, offset);
    ret = mca_common_ompio_file_iread(fh, buf, count, datatype, request);
...
int mca_common_ompio_file_read_all(ompio_file_t *fh, void *buf, int count,
                                   struct ompi_datatype_t *datatype, ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    ...
    if (!(fh->f_flags & OMPIO_DATAREP_NATIVE)
        ...
        */
        ...
        OMPIO_PREPARE_READ_BUF(fh, buf, count, datatype, tbuf, &convertor, max_data, decoded_iov,
                               iov_count);
        ret = fh->f_fcoll->fcoll_file_read_all(fh, decoded_iov->iov_base, decoded_iov->iov_len,
                                               MPI_BYTE, status);
        opal_convertor_unpack(&convertor, decoded_iov, &iov_count, &pos);
        ...
        ret = fh->f_fcoll->fcoll_file_read_all(fh, buf, count, datatype, status);
    }
    return ret;
}

int mca_common_ompio_file_read_at_all(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE offset, void *buf,
                                      int count, struct ompi_datatype_t *datatype,
                                      ompi_status_public_t *status)
{
    int ret = OMPI_SUCCESS;
    ...
    mca_common_ompio_set_explicit_offset(fh, offset);
    ret = mca_common_ompio_file_read_all(fh, buf, count, datatype, status);

    mca_common_ompio_set_explicit_offset(fh, prev_offset);
    return ret;
}

int mca_common_ompio_file_iread_all(ompio_file_t *fp, void *buf, int count,
                                    struct ompi_datatype_t *datatype, ompi_request_t **request)
{
    int ret = OMPI_SUCCESS;

    if (NULL != fp->f_fcoll->fcoll_file_iread_all) {
        ret = fp->f_fcoll->fcoll_file_iread_all(fp, buf, count, datatype, request);
    } else {
...
int mca_common_ompio_file_iread_at_all(ompio_file_t *fp, OMPI_MPI_OFFSET_TYPE offset, void *buf,
                                       int count, struct ompi_datatype_t *datatype,
                                       ompi_request_t **request)
{
    int ret = OMPI_SUCCESS;
    ...
    ret = mca_common_ompio_file_iread_all(fp, buf, count, datatype, request);

    mca_common_ompio_set_explicit_offset(fp, prev_offset);
    return ret;
}

int mca_common_ompio_set_explicit_offset(ompio_file_t *fh, OMPI_MPI_OFFSET_TYPE offset)
{
    size_t i = 0;
    size_t k = 0;
    ...
    if (fh->f_view_size > 0) {
        ...
        fh->f_offset = (fh->f_view_extent * ((offset * fh->f_etype_size) / fh->f_view_size))
                       + fh->f_disp;
