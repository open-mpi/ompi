...
int MPI_Init(int *argc, char ***argv)
{
    ...
    if (MPIT_result != MPI_SUCCESS) {
        fprintf(stderr, "ERROR : failed to intialize MPI_T interface, preventing to get monitoring "
                        "results: check your OpenMPI installation\n");
        PMPI_Abort(MPI_COMM_WORLD, MPIT_result);
    }

    MPIT_result = MPI_T_pvar_session_create(&session);
    if (MPIT_result != MPI_SUCCESS) {
        fprintf(stderr, "ERROR : failed to create MPI_T session, preventing to get monitoring "
                        "results: check your OpenMPI installation\n");
        PMPI_Abort(MPI_COMM_WORLD, MPIT_result);
    }
...
int MPI_Finalize(void)
{
    ...
    if (0 == comm_world_rank) {
        exchange_count_matrix_1 = (size_t *) calloc(comm_world_size * comm_world_size,
                                                    sizeof(size_t));
        exchange_size_matrix_1 = (size_t *) calloc(comm_world_size * comm_world_size,
                                                   sizeof(size_t));
        exchange_count_matrix_2 = (size_t *) calloc(comm_world_size * comm_world_size,
                                                    sizeof(size_t));
        exchange_size_matrix_2 = (size_t *) calloc(comm_world_size * comm_world_size,
                                                   sizeof(size_t));
        exchange_all_size_matrix = (size_t *) calloc(comm_world_size * comm_world_size,
                                                     sizeof(size_t));
        exchange_all_count_matrix = (size_t *) calloc(comm_world_size * comm_world_size,
                                                      sizeof(size_t));
        exchange_all_avg_matrix = (size_t *) calloc(comm_world_size * comm_world_size,
                                                    sizeof(size_t));
    }

    /* Gather PML and COLL results */
    PMPI_Gather(pml_counts.vector, comm_world_size, MPI_UNSIGNED_LONG, exchange_count_matrix_1,
                comm_world_size, MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
    PMPI_Gather(pml_sizes.vector, comm_world_size, MPI_UNSIGNED_LONG, exchange_size_matrix_1,
                comm_world_size, MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
    PMPI_Gather(coll_counts.vector, comm_world_size, MPI_UNSIGNED_LONG, exchange_count_matrix_2,
                comm_world_size, MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
    PMPI_Gather(coll_sizes.vector, comm_world_size, MPI_UNSIGNED_LONG, exchange_size_matrix_2,
                comm_world_size, MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
        ...
        for (i = 0; i < comm_world_size; ++i) {
            for (j = i + 1; j < comm_world_size; ++j) {
                ...
                exchange_count_matrix_1[i * comm_world_size + j]
                    = exchange_count_matrix_1[j * comm_world_size + i]
                    = (exchange_count_matrix_1[i * comm_world_size + j]
                       + exchange_count_matrix_1[j * comm_world_size + i])
                      / 2;
                ...
                exchange_count_matrix_2[i * comm_world_size + j]
                    = exchange_count_matrix_2[j * comm_world_size + i]
                    = (exchange_count_matrix_2[i * comm_world_size + j]
                       + exchange_count_matrix_2[j * comm_world_size + i])
                      / 2;
    ...
    /* Gather OSC results */
    PMPI_Gather(osc_scounts.vector, comm_world_size, MPI_UNSIGNED_LONG, exchange_count_matrix_1,
                comm_world_size, MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
    PMPI_Gather(osc_ssizes.vector, comm_world_size, MPI_UNSIGNED_LONG, exchange_size_matrix_1,
                comm_world_size, MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
    PMPI_Gather(osc_rcounts.vector, comm_world_size, MPI_UNSIGNED_LONG, exchange_count_matrix_2,
                comm_world_size, MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
    PMPI_Gather(osc_rsizes.vector, comm_world_size, MPI_UNSIGNED_LONG, exchange_size_matrix_2,
                comm_world_size, MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
    ...
    if (MPIT_result != MPI_SUCCESS) {
        fprintf(stderr, "WARNING : failed to free MPI_T session, monitoring results may be "
                        "impacted : check your OpenMPI installation\n");
    }

    MPIT_result = MPI_T_finalize();
    if (MPIT_result != MPI_SUCCESS) {
        fprintf(stderr, "WARNING : failed to finalize MPI_T interface, monitoring results may be "
                        "impacted : check your OpenMPI installation\n");
    }
...
void init_monitoring_result(const char *pvar_name, monitoring_result *res)
{
    ...
    if (MPIT_result != MPI_SUCCESS) {
        fprintf(stderr,
                "ERROR : cannot find monitoring MPI_T \"%s\" pvar, check that you have monitoring "
                "pml\n",
                pvar_name);
        PMPI_Abort(MPI_COMM_WORLD, MPIT_result);
    }

    MPIT_result = MPI_T_pvar_handle_alloc(session, res->pvar_idx, comm_world, &(res->pvar_handle),
                                          &count);
    if (MPIT_result != MPI_SUCCESS) {
        fprintf(stderr,
                "ERROR : failed to allocate handle on \"%s\" pvar, check that you have monitoring "
                "pml\n",
                pvar_name);
        PMPI_Abort(MPI_COMM_WORLD, MPIT_result);
    }
    ...
    if (count != comm_world_size) {
        fprintf(stderr,
                "ERROR : COMM_WORLD has %d ranks \"%s\" pvar contains %d values, check that you "
                "have monitoring pml\n",
...
void start_monitoring_result(monitoring_result *res)
{
    ...
    if (MPIT_result != MPI_SUCCESS) {
        fprintf(stderr,
                "ERROR : failed to start handle on \"%s\" pvar, check that you have enabled the "
                "monitoring pml\n",
                res->pvar_name);
        PMPI_Abort(MPI_COMM_WORLD, MPIT_result);
    }
...
void stop_monitoring_result(monitoring_result *res)
{
    ...
    if (MPIT_result != MPI_SUCCESS) {
        fprintf(stderr,
                "ERROR : failed to stop handle on \"%s\" pvar, check that you have enabled the "
                "monitoring pml\n",
                res->pvar_name);
...
void get_monitoring_result(monitoring_result *res)
{
    ...
    if (MPIT_result != MPI_SUCCESS) {
        fprintf(
            stderr,
            "ERROR : failed to read \"%s\" pvar, check that you have enabled the monitoring pml\n",
            res->pvar_name);
        PMPI_Abort(MPI_COMM_WORLD, MPIT_result);
    }
}
...
void destroy_monitoring_result(monitoring_result *res)
{
    ...
    if (MPIT_result != MPI_SUCCESS) {
        printf("ERROR : failed to free handle on \"%s\" pvar, check that you have enabled the "
               "monitoring pml\n",
               res->pvar_name);
        MPI_Abort(MPI_COMM_WORLD, MPIT_result);
    }
...
int write_mat(char *filename, size_t *mat, unsigned int dim)
{
    ...
    if (!matrix_file) {
        fprintf(stderr,
                "ERROR : failed to open \"%s\" file in write mode, check your permissions\n",
                filename);
        return -1;
    }
