...
static int mca_common_monitoring_get_pml_count(const struct mca_base_pvar_t *pvar, void *value,
                                               void *obj_handle);

/* Retreive the PML recorded amount of data sent */
static int mca_common_monitoring_get_pml_size(const struct mca_base_pvar_t *pvar, void *value,
                                              void *obj_handle);

/* Retreive the OSC recorded count of messages sent */
static int mca_common_monitoring_get_osc_sent_count(const struct mca_base_pvar_t *pvar, void *value,
                                                    void *obj_handle);

/* Retreive the OSC recorded amount of data sent */
static int mca_common_monitoring_get_osc_sent_size(const struct mca_base_pvar_t *pvar, void *value,
                                                   void *obj_handle);

/* Retreive the OSC recorded count of messages received */
static int mca_common_monitoring_get_osc_recv_count(const struct mca_base_pvar_t *pvar, void *value,
                                                    void *obj_handle);

/* Retreive the OSC recorded amount of data received */
static int mca_common_monitoring_get_osc_recv_size(const struct mca_base_pvar_t *pvar, void *value,
                                                   void *obj_handle);

/* Retreive the COLL recorded count of messages sent */
static int mca_common_monitoring_get_coll_count(const struct mca_base_pvar_t *pvar, void *value,
                                                void *obj_handle);

/* Retreive the COLL recorded amount of data sent */
static int mca_common_monitoring_get_coll_size(const struct mca_base_pvar_t *pvar, void *value,
                                               void *obj_handle);

/* Set the filename where to output the monitored data */
static int mca_common_monitoring_set_flush(struct mca_base_pvar_t *pvar, const void *value,
                                           void *obj);

/* Does nothing, as the pml_monitoring_flush pvar has no point to be read */
static int mca_common_monitoring_get_flush(const struct mca_base_pvar_t *pvar, void *value,
                                           void *obj);
...
int mca_common_monitoring_init(void)
{
    ...
    mca_common_monitoring_output_stream_id = opal_output_open(
...
int mca_common_monitoring_register(void)
{
    ...
    (void) mca_base_var_register("ompi", "pml", "monitoring", "enable",
                                 ...
                                 MCA_BASE_VAR_SCOPE_READONLY, &mca_common_monitoring_enabled);
    ...
    /* PML PVARs */
    (void) mca_base_pvar_register("ompi", "pml", "monitoring", "flush",
                                  "Flush the monitoring "
                                  ...
                                  MCA_BASE_VAR_TYPE_STRING, NULL, MPI_T_BIND_NO_OBJECT,
                                  MCA_BASE_PVAR_FLAG_IWG, mca_common_monitoring_get_flush,
                                  mca_common_monitoring_set_flush,
    ...
    (void) mca_base_pvar_register("ompi", "pml", "monitoring", "messages_count",
                                  ...
                                  OPAL_INFO_LVL_4, MPI_T_PVAR_CLASS_SIZE, MCA_MONITORING_VAR_TYPE,
                                  NULL, MPI_T_BIND_MPI_COMM,
    ...
    (void) mca_base_pvar_register("ompi", "pml", "monitoring", "messages_size",
                                  "Size of messages "
    ...
    /* OSC PVARs */
    (void) mca_base_pvar_register("ompi", "osc", "monitoring", "messages_sent_count",
                                  "Number of "
    ...
    (void) mca_base_pvar_register("ompi", "osc", "monitoring", "messages_sent_size",
                                  "Size of "
                                  "messages sent through the OSC framework with each peer.",
                                  OPAL_INFO_LVL_4, MPI_T_PVAR_CLASS_SIZE, MCA_MONITORING_VAR_TYPE,
                                  NULL, MPI_T_BIND_MPI_COMM,
    ...
    (void) mca_base_pvar_register("ompi", "osc", "monitoring", "messages_recv_count",
                                  "Number of "
    ...
    (void) mca_base_pvar_register("ompi", "osc", "monitoring", "messages_recv_size",
                                  "Size of "
                                  "messages received through the OSC framework with each peer.",
                                  OPAL_INFO_LVL_4, MPI_T_PVAR_CLASS_SIZE, MCA_MONITORING_VAR_TYPE,
                                  NULL, MPI_T_BIND_MPI_COMM,
    ...
    /* COLL PVARs */
    (void) mca_base_pvar_register("ompi", "coll", "monitoring", "messages_count",
                                  "Number of "
    ...
    (void) mca_base_pvar_register("ompi", "coll", "monitoring", "messages_size",
                                  "Size of "
                                  "messages exchanged through the COLL framework with each peer.",
                                  OPAL_INFO_LVL_4, MPI_T_PVAR_CLASS_SIZE, MCA_MONITORING_VAR_TYPE,
                                  NULL, MPI_T_BIND_MPI_COMM,
    ...
    (void) mca_base_pvar_register("ompi", "coll", "monitoring", "o2a_count",
                                  "Number of messages "
    ...
    (void) mca_base_pvar_register("ompi", "coll", "monitoring", "o2a_size",
                                  "Size of messages "
                                  "exchanged as one-to-all operations in a communicator.",
    ...
    (void) mca_base_pvar_register("ompi", "coll", "monitoring", "a2o_count",
                                  "Number of messages "
    ...
    (void) mca_base_pvar_register("ompi", "coll", "monitoring", "a2o_size",
                                  "Size of messages "
                                  "exchanged as all-to-one operations in a communicator.",
                                  OPAL_INFO_LVL_4, MPI_T_PVAR_CLASS_AGGREGATE,
                                  MCA_MONITORING_VAR_TYPE, NULL, MPI_T_BIND_MPI_COMM,
    ...
    (void) mca_base_pvar_register("ompi", "coll", "monitoring", "a2a_count",
                                  "Number of messages "
    ...
    (void) mca_base_pvar_register("ompi", "coll", "monitoring", "a2a_size",
                                  "Size of messages "
                                  "exchanged as all-to-all operations in a communicator.",
                                  OPAL_INFO_LVL_4, MPI_T_PVAR_CLASS_AGGREGATE,
                                  MCA_MONITORING_VAR_TYPE, NULL, MPI_T_BIND_MPI_COMM,
 ...
 */
int mca_common_monitoring_add_procs(struct ompi_proc_t **procs, size_t nprocs)
{
    ...
    for (i = 0; i < nprocs; i++) {
        ...
        for (peer_rank = 0; peer_rank < nprocs_world; peer_rank++) {
             ...
             * key */
            if (OPAL_SUCCESS
                != opal_hash_table_set_value_uint64(common_monitoring_translation_ht, key,
                                                    (void *) (uintptr_t) peer_rank)) {
                return OMPI_ERR_OUT_OF_RESOURCE; /* failed to allocate memory or growing the hash
                                                    table */
            }
            break;
        }
    ...
    if (mca_common_monitoring_filter()) {
        for (int i = 0; i < nbprocs; i++) {
            if (filtered_pml_count[i] > 0) {
                fprintf(pf, "I\t%" PRId32 "\t%" PRId32 "\t%zu bytes\t%zu msgs sent%s", my_rank, i,
                        filtered_pml_data[i], filtered_pml_count[i],
                        0 == pml_count[i] ? "\t" : "\n");
                /*
    ...
    for (int i = 0; i < nbprocs; i++) {
        if (osc_count_s[i] > 0) {
            fprintf(pf, "S\t%" PRId32 "\t%" PRId32 "\t%zu bytes\t%zu msgs sent\n", my_rank, i,
                    osc_data_s[i], osc_count_s[i]);
        }
        if (osc_count_r[i] > 0) {
            fprintf(pf, "R\t%" PRId32 "\t%" PRId32 "\t%zu bytes\t%zu msgs sent\n", my_rank, i,
                    osc_data_r[i], osc_count_r[i]);
        }
    }
...
static int mca_common_monitoring_flush(int fd, char *filename)
{
    ...
    if (1 == fd) {
        ...
        if (NULL == pf) { /* Error during open */
            OPAL_MONITORING_PRINT_ERR("Error while flushing to: %s.%" PRId32 ".prof", filename,
                                      rank_world);
            return OMPI_ERROR;
        }
