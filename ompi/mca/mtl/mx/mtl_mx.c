...
int ompi_mtl_mx_module_init(){ 
    ...
    if(mx_return != MX_SUCCESS) { 
        orte_output(ompi_mtl_base_output, "Error in mx_open_endpoint (error %s)\n", mx_strerror(mx_return));
        return OMPI_ERROR;
    }
    ...
    if(mx_return != MX_SUCCESS) { 
        orte_output(ompi_mtl_base_output, "Error in mx_get_endpoint_addr (error %s)\n", mx_strerror(mx_return));
        return OMPI_ERROR;
    }
    ...
    if(mx_return != MX_SUCCESS) { 
        orte_output(ompi_mtl_base_output, "Error in mx_decompose_endpoint_addr (error %s)\n", mx_strerror(mx_return));
        return OMPI_ERROR;
    }
...
ompi_mtl_mx_finalize(struct mca_mtl_base_module_t* mtl) { 
    ...
    if(mx_return != MX_SUCCESS){ 
        orte_output(ompi_mtl_base_output, "Error in mx_close_endpoint (error %s)\n", mx_strerror(mx_return));
        return OMPI_ERROR;
    }
...
int ompi_mtl_mx_progress( void ) { 
    ...
    while(1){
        ...
        if( OPAL_UNLIKELY(mx_return != MX_SUCCESS) ) { 
            orte_output(ompi_mtl_base_output, "Error in mx_ipeek (error %s)\n", mx_strerror(mx_return));
        }
        if(result) { 
            completed++;
            mx_return = mx_test(ompi_mtl_mx.mx_endpoint, 
                                ...
                                &result);
            if( OPAL_UNLIKELY(mx_return != MX_SUCCESS) ) { 
                orte_output(ompi_mtl_base_output, "Error in mx_test (error %s)\n", mx_strerror(mx_return));
                abort();
            }
