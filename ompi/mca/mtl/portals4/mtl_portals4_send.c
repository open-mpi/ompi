                                             ...
                                             bool *complete)
{
    ...
    if (PTL_EVENT_GET == ev->type) {
        ...
        if (ret > 0) {
            ...
            OPAL_OUTPUT_VERBOSE((90, ompi_mtl_base_framework.framework_output,
                                 "PTL_EVENT_GET received now pending_get=%d", ret));
            return retval;
        }
        ...
        /* last get received */
        OPAL_OUTPUT_VERBOSE(
            (90, ompi_mtl_base_framework.framework_output,
             "PTL_EVENT_GET: PtlMEUnlink is called ptl_request->me_h=%d (pending get=%d)",
             ptl_request->me_h, ret));

        if (!PtlHandleIsEqual(ptl_request->me_h, PTL_INVALID_HANDLE)) {
            ret = PtlMEUnlink(ptl_request->me_h);
            if (PTL_OK != ret) {
                opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
                                    "%s:%d: send callback PtlMEUnlink returned %d", __FILE__,
                                    __LINE__, ret);
            }
    ...
    if (OPAL_UNLIKELY(ev->ni_fail_type == PTL_NI_PT_DISABLED)) {
        ompi_mtl_portals4_pending_request_t *pending = ptl_request->pending;

        OPAL_OUTPUT_VERBOSE((10, ompi_mtl_base_framework.framework_output,
                             "send %lu hit flow control (%d)", ptl_request->opcount, ev->type));
        ...
        opal_list_append(&ompi_mtl_portals4.flowctl.pending_sends, &pending->super.super);
    ...
    if (OPAL_UNLIKELY(ev->ni_fail_type != PTL_NI_OK)) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
                            "%s:%d: send callback ni_fail_type: %d", __FILE__, __LINE__,
                            ev->ni_fail_type);
        *complete = true;
        return OMPI_ERROR;
    }
    ...
    OPAL_OUTPUT_VERBOSE((50, ompi_mtl_base_framework.framework_output,
                         "send %lu got event of type %d", ptl_request->opcount, ev->type));

    /* First put achieved successfully (In the Priority List), so it may be necessary to decrement
     * the number of pending get If the protocol is eager, just decrement pending_get Else (the
     * protocol is rndv), decrement pending_get only if length % max_msg_size <= eager_limit (This
     * is the case where the eager part allows to save one get)
     */
    if ((PTL_EVENT_ACK == ev->type) && (PTL_PRIORITY_LIST == ev->ptl_list)
        && (0 < ptl_request->pending_get)) {

        if ((eager == ompi_mtl_portals4.protocol)
            || (ptl_request->length % ompi_mtl_portals4.max_msg_size_mtl
                <= ompi_mtl_portals4.eager_limit)) {
            val = OPAL_THREAD_ADD_FETCH32(&(ptl_request->pending_get), -1);
        }
        if (0 == val) {
            add = 2; /* We haven't to wait for any get, so we have to add an extra count to cause
    ...
    if ((PTL_EVENT_ACK == ev->type) && (PTL_PRIORITY_LIST == ev->ptl_list)
        ...
        if (PTL_OK != ret) {
            opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
                                "%s:%d: send callback PtlMEUnlink returned %d", __FILE__, __LINE__,
                                ret);
        }
        ptl_request->me_h = PTL_INVALID_HANDLE;
        add++;
...
#if OMPI_MTL_PORTALS4_FLOW_CONTROL
        OPAL_THREAD_ADD_FETCH32(&ompi_mtl_portals4.flowctl.send_slots, 1);
        opal_free_list_return(&ompi_mtl_portals4.flowctl.pending_fl, &ptl_request->pending->super);
...
static int ompi_mtl_portals4_send_callback(ptl_event_t *ev,
                                           ompi_mtl_portals4_base_request_t *ptl_base_request)
{
    bool complete = false;
    int ret;
    ompi_mtl_portals4_send_request_t *ptl_request = (ompi_mtl_portals4_send_request_t *)
        ptl_base_request;
...
static int ompi_mtl_portals4_isend_callback(ptl_event_t *ev,
                                            ompi_mtl_portals4_base_request_t *ptl_base_request)
{
    bool complete = false;
    int ret;
...
static inline int ompi_mtl_portals4_short_isend(mca_pml_base_send_mode_t mode, void *start,
                                                int length, int contextid, int tag, int localrank,
                                                ...
                                                ompi_mtl_portals4_isend_request_t *ptl_request)
{
    ...
    MTL_PORTALS4_SET_SEND_BITS(match_bits, contextid, localrank, tag, MTL_PORTALS4_SHORT_MSG);
    ...
    if (MCA_PML_BASE_SEND_SYNCHRONOUS == mode) {
        ...
        me.options = PTL_ME_OP_PUT | PTL_ME_USE_ONCE | PTL_ME_EVENT_LINK_DISABLE
                     | PTL_ME_EVENT_UNLINK_DISABLE;
        me.match_id = ptl_proc;
        me.match_bits = hdr_data;
        me.ignore_bits = 0;

        ret = PtlMEAppend(ompi_mtl_portals4.ni_h, ompi_mtl_portals4.read_idx, &me,
                          PTL_PRIORITY_LIST, ptl_request, &ptl_request->me_h);
    ...
    OPAL_OUTPUT_VERBOSE((50, ompi_mtl_base_framework.framework_output, "Send %lu, start: %p",
                         ptl_request->opcount, start));

    ptl_request->pending_get = 0;
    ret = PtlPut(ompi_mtl_portals4.send_md_h, (ptl_size_t) start, length, PTL_ACK_REQ, ptl_proc,
                 ompi_mtl_portals4.recv_idx, match_bits, 0, ptl_request, hdr_data);
    if (OPAL_UNLIKELY(PTL_OK != ret)) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output, "%s:%d: PtlPut failed: %d",
...
static inline int ompi_mtl_portals4_long_isend(void *start, size_t length, int contextid, int tag,
                                               int localrank, ptl_process_t ptl_proc,
                                               ompi_mtl_portals4_isend_request_t *ptl_request)
{
    int ret;
    ptl_match_bits_t match_bits;
    ...
    me.min_free = 0;
    me.uid = ompi_mtl_portals4.uid;
    me.options = PTL_ME_OP_GET | PTL_ME_EVENT_LINK_DISABLE | PTL_ME_EVENT_UNLINK_DISABLE;
    me.match_id = ptl_proc;
    me.match_bits = hdr_data;
    me.ignore_bits = 0;

    ret = PtlMEAppend(ompi_mtl_portals4.ni_h, ompi_mtl_portals4.read_idx, &me, PTL_PRIORITY_LIST,
    ...
    OPAL_OUTPUT_VERBOSE((50, ompi_mtl_base_framework.framework_output,
                         "Send %lu long send with hdr_data 0x%lx (0x%lx)", ptl_request->opcount,
                         hdr_data, match_bits));

    if (rndv == ompi_mtl_portals4.protocol) {
        ptl_size_t min = (OPAL_LIKELY(ompi_mtl_portals4.eager_limit
    ...
    /* We have to wait for some GET events.
       ...
       else we will re-calculate this number when we received the first ACK event (with remote
       overflow list)
     */

    ptl_request->pending_get = (length - 1) / ompi_mtl_portals4.max_msg_size_mtl + 1;
    OPAL_OUTPUT_VERBOSE(
        (90, ompi_mtl_base_framework.framework_output, "pending_get=%d", ptl_request->pending_get));

    ret = PtlPut(ompi_mtl_portals4.send_md_h, (ptl_size_t) start, put_length, PTL_ACK_REQ, ptl_proc,
                 ompi_mtl_portals4.recv_idx, match_bits, 0, ptl_request, hdr_data);
    if (OPAL_UNLIKELY(PTL_OK != ret)) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output, "%s:%d: PtlPut failed: %d",
                            __FILE__, __LINE__, ret);
...
void ompi_mtl_portals4_pending_list_progress()
{
    ...
    while ((!ompi_mtl_portals4.flowctl.flowctl_active)
        ...
        if (pending->length <= ompi_mtl_portals4.short_limit) {
            ret = ompi_mtl_portals4_short_isend(pending->mode, pending->start, pending->length,
                                                pending->contextid, pending->tag, pending->my_rank,
                                                pending->ptl_proc, pending->ptl_request);
        } else {
            ret = ompi_mtl_portals4_long_isend(pending->start, pending->length, pending->contextid,
                                               pending->tag, pending->my_rank, pending->ptl_proc,
                                               pending->ptl_request);
        }
        if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
            opal_list_prepend(&ompi_mtl_portals4.flowctl.pending_sends, &pending->super.super);
            OPAL_THREAD_ADD_FETCH32(&ompi_mtl_portals4.flowctl.send_slots, 1);
        }
    }
}
...
static inline int ompi_mtl_portals4_send_start(struct mca_mtl_base_module_t *mtl,
                                               ...
                                               ompi_mtl_portals4_isend_request_t *ptl_request)
{
    ...
    OPAL_OUTPUT_VERBOSE((50, ompi_mtl_base_framework.framework_output,
                         ...
                         ptl_proc.phys.nid, ptl_proc.phys.pid, (int64_t) length));
    ...
    if (length <= ompi_mtl_portals4.short_limit) {
        ret = ompi_mtl_portals4_short_isend(mode, start, length, comm->c_contextid, tag,
                                            comm->c_my_rank, ptl_proc, ptl_request);
    } else {
        ret = ompi_mtl_portals4_long_isend(start, length, comm->c_contextid, tag, comm->c_my_rank,
                                           ptl_proc, ptl_request);
    }

    return ret;
}

int ompi_mtl_portals4_send(struct mca_mtl_base_module_t *mtl, struct ompi_communicator_t *comm,
                           ...
                           mca_pml_base_send_mode_t mode)
{
    ...
    ret = ompi_mtl_portals4_send_start(mtl, comm, dest, tag, convertor, mode, &ptl_request.super);
    if (OPAL_UNLIKELY(OMPI_SUCCESS != ret)) {
...
int ompi_mtl_portals4_isend(struct mca_mtl_base_module_t *mtl, struct ompi_communicator_t *comm,
                            int dest, int tag, struct opal_convertor_t *convertor,
                            ...
                            mca_mtl_request_t *mtl_request)
{
    ...
    ret = ompi_mtl_portals4_send_start(mtl, comm, dest, tag, convertor, mode, ptl_request);
