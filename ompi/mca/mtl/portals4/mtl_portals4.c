...
mca_mtl_portals4_module_t ompi_mtl_portals4 = {
    {8191,        /* max cid - 2^13 - 1 */
     ...
     ompi_mtl_portals4_add_procs, ompi_mtl_portals4_del_procs, ompi_mtl_portals4_finalize,

     ompi_mtl_portals4_send, ompi_mtl_portals4_isend, ompi_mtl_portals4_irecv,
     ompi_mtl_portals4_iprobe, ompi_mtl_portals4_imrecv, ompi_mtl_portals4_improbe,

     ompi_mtl_portals4_cancel, ompi_mtl_portals4_add_comm, ompi_mtl_portals4_del_comm}};

static int portals4_init_interface(void)
{
    unsigned int ret;
    ptl_md_t md;
    ptl_me_t me;
    ...
    /* create event queues */
    ret = PtlEQAlloc(ompi_mtl_portals4.ni_h, ompi_mtl_portals4.send_queue_size,
    ...
    ret = PtlEQAlloc(ompi_mtl_portals4.ni_h, ompi_mtl_portals4.recv_queue_size,
    ...
    ret = PtlPTAlloc(ompi_mtl_portals4.ni_h,
                     ...
                     ompi_mtl_portals4.recv_eq_h, REQ_RECV_TABLE_ID, &ompi_mtl_portals4.recv_idx);
    if (PTL_OK != ret) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
    ...
    if (ompi_mtl_portals4.recv_idx != REQ_RECV_TABLE_ID) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
                            ...
                            __LINE__, ompi_mtl_portals4.recv_idx);
        goto error;
    }

    ret = PtlPTAlloc(ompi_mtl_portals4.ni_h, PTL_PT_ONLY_USE_ONCE | PTL_PT_ONLY_TRUNCATE,
                     ompi_mtl_portals4.send_eq_h, REQ_READ_TABLE_ID, &ompi_mtl_portals4.read_idx);
    if (PTL_OK != ret) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
                            "%s:%d: PtlPTAlloc failed: %d\n", __FILE__, __LINE__, ret);
        goto error;
    ...
    if (ompi_mtl_portals4.read_idx != REQ_READ_TABLE_ID) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
                            "%s:%d: PtlPTAlloc did not allocate the requested PT: %d\n", __FILE__,
                            __LINE__, ompi_mtl_portals4.read_idx);
        goto error;
    }
    ...
    ret = PtlMDBind(ompi_mtl_portals4.ni_h, &md, &ompi_mtl_portals4.zero_md_h);
    if (PTL_OK != ret) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
    ...
    ret = PtlMDBind(ompi_mtl_portals4.ni_h, &md, &ompi_mtl_portals4.send_md_h);
    if (PTL_OK != ret) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
                            "%s:%d: PtlMDBind failed: %d\n", __FILE__, __LINE__, ret);
        goto error;
    ...
    ret = PtlMEAppend(ompi_mtl_portals4.ni_h, ompi_mtl_portals4.recv_idx, &me, PTL_OVERFLOW_LIST,
                      NULL, &ompi_mtl_portals4.long_overflow_me_h);
    if (PTL_OK != ret) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
...
static int create_maptable(size_t nprocs, ompi_proc_t **procs)
{
    int ret;
    size_t i;
    ...
    if (NULL == maptable) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output, "%s:%d: malloc failed\n",
                            __FILE__, __LINE__);
    ...
    for (i = 0; i < nprocs; i++) {
        ...
        OPAL_MODEX_RECV(ret, &mca_mtl_portals4_component.mtl_version, &procs[i]->super.proc_name,
                        (uint8_t **) &modex_id, &size);
        if (OMPI_SUCCESS != ret) {
            opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
        ...
        opal_output_verbose(50, ompi_mtl_base_framework.framework_output,
                            ...
                            maptable[i].phys.pid, maptable[i].phys.nid);
    }
    ...
    opal_output_verbose(1, ompi_mtl_base_framework.framework_output, "logical mapping OK\n");
...
static int create_endpoint(ompi_proc_t *proc)
{
    ...
    if (NULL == endpoint) {
        ...
        if (ompi_mtl_portals4.use_logical) {
            ...
            OPAL_MODEX_RECV(ret, &mca_mtl_portals4_component.mtl_version, &proc->super.proc_name,
                            (uint8_t **) &modex_id, &size);
            if (OMPI_SUCCESS != ret) {
                opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
                                    "%s:%d: ompi_modex_recv failed: %d\n", __FILE__, __LINE__, ret);
...
ompi_proc_t *ompi_mtl_portals4_get_proc_group(struct ompi_group_t *group, int rank)
{
    int ret;
...
static int add_endpoints(size_t nprocs, ompi_proc_t **procs)
{
    int ret;
    size_t i;
...
int ompi_mtl_portals4_add_procs(struct mca_mtl_base_module_t *mtl, size_t nprocs,
                                struct ompi_proc_t **procs)
{
     ...
     */
    ret = add_endpoints(nprocs, procs);
    if (OMPI_SUCCESS != ret) {
        opal_output_verbose(1, ompi_mtl_base_framework.framework_output,
...
#if OMPI_MTL_PORTALS4_FLOW_CONTROL
        opal_output_verbose(50, ompi_mtl_base_framework.framework_output, "add_procs() - me=%d\n",
...
int ompi_mtl_portals4_del_procs(struct mca_mtl_base_module_t *mtl, size_t nprocs,
                                struct ompi_proc_t **procs)
{
    size_t i;

    opal_output_verbose(50, ompi_mtl_base_framework.framework_output, "del_procs() - enter\n");
    ...
    opal_output_verbose(50, ompi_mtl_base_framework.framework_output, "del_procs() - exit\n");

    return OMPI_SUCCESS;
}

int ompi_mtl_portals4_finalize(struct mca_mtl_base_module_t *mtl)
{
...
int ompi_mtl_portals4_add_comm(struct mca_mtl_base_module_t *mtl, struct ompi_communicator_t *comm)
{
    return OMPI_SUCCESS;
}

int ompi_mtl_portals4_del_comm(struct mca_mtl_base_module_t *mtl, struct ompi_communicator_t *comm)
{
    return OMPI_SUCCESS;
}
