...
static inline void vprotocol_pessimist_matching_log_prepare(ompi_request_t *req)
{
    ...
    if (MPI_ANY_SOURCE == pmlreq->req_peer) {
        ...
        opal_list_append(&mca_vprotocol_pessimist.pending_events, (opal_list_item_t *) event);
    }
}
...
static inline void vprotocol_pessimist_matching_log_finish(ompi_request_t *req)
{
    ...
    if (ftreq->event) {
        ...
        V_OUTPUT_VERBOSE(70, "pessimist:\tlog\tmatch\t%" PRIpclock "\tsrc %d\tseq %" PRIpclock,
                         ftreq->reqid, req->req_status.MPI_SOURCE,
...
#define __VPROTOCOL_PESSIMIST_SEND_BUFFER()                                                      \
    do {                                                                                         \
        if (OPAL_UNLIKELY(mca_vprotocol_pessimist.event_buffer_length)) {                        \
            ...
            rc = mca_pml_v.host_pml.pml_irecv(&max_clock, 1, MPI_UNSIGNED_LONG_LONG, 0,          \
...
static inline void vprotocol_pessimist_event_flush(void)
{
    if (OPAL_UNLIKELY(!opal_list_is_empty(&mca_vprotocol_pessimist.pending_events))) {
        ...
        for (event = (mca_vprotocol_pessimist_event_t *) opal_list_get_first(
                 &mca_vprotocol_pessimist.pending_events);
             event
             != (mca_vprotocol_pessimist_event_t *) opal_list_get_end(
                 &mca_vprotocol_pessimist.pending_events);
            ...
            if (event->u_event.e_matching.src == -1) {
                ...
                if (event->req->req_ompi.req_status.MPI_SOURCE == -1) {
                    V_OUTPUT_VERBOSE(41,
                                     "pessimist:\tlog\tel\t%" PRIpclock "\tnot matched yet (%d)",
                                     event->u_event.e_matching.reqid,
                                     event->u_event.e_matching.src);
                    continue;
                }
                event->u_event.e_matching.src = event->req->req_ompi.req_status.MPI_SOURCE;
            }
            /* Send this event to EL */
            V_OUTPUT_VERBOSE(40, "pessimist:\tlog\tel\t%" PRIpclock "\tfrom %d\tsent to EL",
                             event->u_event.e_matching.reqid, event->u_event.e_matching.src);
            ...
            assert(mca_vprotocol_pessimist.event_buffer_length
                   < mca_vprotocol_pessimist.event_buffer_max_length);
 ...
 */
#define VPROTOCOL_PESSIMIST_MATCHING_REPLAY(src)                         \
    do {                                                                 \
...
static inline void vprotocol_pessimist_delivery_log(ompi_request_t *req)
{
    ...
    if (req == NULL) {
        ...
        V_OUTPUT_VERBOSE(70, "pessimist:\tlog\tdeliver\t%" PRIpclock "\tnone",
                         mca_vprotocol_pessimist.clock);
        event = (mca_vprotocol_pessimist_event_t *) opal_list_get_last(
            &mca_vprotocol_pessimist.pending_events);
        ...
        V_OUTPUT_VERBOSE(70, "pessimist:\tlog\tdeliver\t%" PRIpclock "\treq %" PRIpclock,
                         mca_vprotocol_pessimist.clock, VPESSIMIST_FTREQ(req)->reqid);
        VPESSIMIST_DELIVERY_EVENT_NEW(event);
        devent = &(event->u_event.e_delivery);
        devent->probeid = mca_vprotocol_pessimist.clock++;
 ...
 */
#define VPROTOCOL_PESSIMIST_DELIVERY_REPLAY(n, reqs, outcount, i, status)      \
    do {                                                                       \
        if (mca_vprotocol_pessimist.replay)                                    \
            vprotocol_pessimist_delivery_replay(n, reqs, outcount, i, status); \
    } while (0)
void vprotocol_pessimist_delivery_replay(size_t, ompi_request_t **, int *, int *,
