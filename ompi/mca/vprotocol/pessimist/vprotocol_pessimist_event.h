...
typedef struct vprotocol_pessimist_delivery_event_t {
    ...
    vprotocol_pessimist_clock_t
        reqid; /* delivered request (recv or send) -TODO: SUPPORT FOR WaitSome/TestSome- */
...
#    define VPESSIMIST_EVENT_RETURN(event) \
        opal_free_list_return(&mca_vprotocol_pessimist.events_pool, (opal_free_list_item_t *) event)
