...
static mca_vprotocol_base_module_t *mca_vprotocol_pessimist_component_init(int *priority, bool,
                                                                           bool);
...
static int mca_vprotocol_pessimist_component_register(void)
{
    ...
    (void) mca_base_component_var_register(&mca_vprotocol_pessimist_component.pmlm_version,
                                           "sender_based_chunk", NULL, MCA_BASE_VAR_TYPE_INT, NULL,
                                           0, 0, OPAL_INFO_LVL_9, MCA_BASE_VAR_SCOPE_READONLY,
                                           &_sender_based_size);
    _event_buffer_size = 1024;
    (void) mca_base_component_var_register(&mca_vprotocol_pessimist_component.pmlm_version,
                                           "event_buffer_size", NULL, MCA_BASE_VAR_TYPE_INT, NULL,
                                           0, 0, OPAL_INFO_LVL_9, MCA_BASE_VAR_SCOPE_READONLY,
                                           &_event_buffer_size);
    _mmap_file_name = "vprotocol_pessimist-senderbased";
    (void) mca_base_component_var_register(&mca_vprotocol_pessimist_component.pmlm_version,
                                           "sender_based_file", NULL, MCA_BASE_VAR_TYPE_STRING,
    ...
    (void) mca_base_component_var_register(
        &mca_vprotocol_pessimist_component.pmlm_version, "allow_thread_multiple",
        "Allow the PML V to work even when the MPI"
...
mca_vprotocol_pessimist_component_init(int *priority, bool enable_progress_threads,
                                       bool enable_mpi_threads)
{
    ...
    *priority = _priority;
    ...
    if (enable_mpi_threads && !ompi_vprotocol_pessimist_allow_thread_multiple) {
         ...
         */
        opal_output(0, "vprotocol_pessimist: component_init: threads are enabled, and not "
                       "supported by vprotocol pessimist fault tolerant layer, will not load");
        return NULL;
    }
    ...
    opal_free_list_init(&mca_vprotocol_pessimist.events_pool,
                        sizeof(mca_vprotocol_pessimist_event_t), opal_cache_line_size,
                        OBJ_CLASS(mca_vprotocol_pessimist_event_t), 0, opal_cache_line_size,
                        _free_list_num, _free_list_max, _free_list_inc, NULL, 0, NULL, NULL, NULL);
    mca_vprotocol_pessimist.event_buffer_max_length = _event_buffer_size
                                                      / sizeof(vprotocol_pessimist_mem_event_t);
    mca_vprotocol_pessimist.event_buffer_length = 0;
    mca_vprotocol_pessimist.event_buffer = (vprotocol_pessimist_mem_event_t *) malloc(
        _event_buffer_size);
