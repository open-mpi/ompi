...
static int sb_mmap_file_open(const char *path)
{
    ...
    if (-1 == sb.sb_fd) {
        V_OUTPUT_ERR("pml_v: vprotocol_pessimist: sender_based_init: open (%s): %s", path,
                     strerror(errno));
...
#    define MAP_NOCACHE 0
    ...
    sb.sb_addr = (uintptr_t) mmap((void *) sb.sb_addr, sb.sb_length, PROT_WRITE | PROT_READ,
                                  MAP_PRIVATE | MAP_NOCACHE, sb.sb_fd, sb.sb_offset);
    if (((uintptr_t) -1) == sb.sb_addr) {
        V_OUTPUT_ERR("pml_v: vprotocol_pessimist: sender_based_alloc: mmap: %s", strerror(errno));
        close(sb.sb_fd);
        ompi_mpi_abort(MPI_COMM_SELF, MPI_ERR_NO_SPACE);
    }
}
...
int vprotocol_pessimist_sender_based_init(const char *mmapfile, size_t size)
{
    ...
    sb.sb_conv_to_pessimist_offset = (uintptr_t) VPROTOCOL_SEND_REQ(NULL)
                                     - ((uintptr_t) &pml_req.req_base.req_convertor
                                        - (uintptr_t) &pml_req);
    V_OUTPUT_VERBOSE(500, "pessimist: conv_to_pessimist_offset: %p",
                     (void *) sb.sb_conv_to_pessimist_offset);
#endif
    sb.sb_offset = 0;
    sb.sb_length = size;
...
void vprotocol_pessimist_sender_based_alloc(size_t len)
{
    ...
    V_OUTPUT_VERBOSE(30, "pessimist:\tsb\tgrow\toffset %llu\tlength %llu\tbase %p\tcursor %p",
                     (unsigned long long) sb.sb_offset, (unsigned long long) sb.sb_length,
                     (void *) sb.sb_addr, (void *) sb.sb_cursor);
}
