...
int vprotocol_pessimist_event_logger_connect(int el_rank, ompi_communicator_t **el_comm)
{
    ...
    if (PMIX_SUCCESS != prc || PMIX_STRING != pdat.value.type || NULL == pdat.value.data.string) {
    ...
    rc = mca_pml_v.host_pml.pml_recv(&connect_info, 2, MPI_UNSIGNED_LONG_LONG, 0,
                                     VPROTOCOL_PESSIMIST_EVENTLOG_NEW_CLIENT_CMD,
...
void vprotocol_pessimist_matching_replay(int *src)
{
    ...
    for (event = (mca_vprotocol_pessimist_event_t *) opal_list_get_first(
             &mca_vprotocol_pessimist.replay_events);
         event
        ...
        if (mevent->reqid == mca_vprotocol_pessimist.clock) {
            ...
            V_OUTPUT_VERBOSE(70, "pessimist: replay\tmatch\t%" PRIpclock "\trecv is forced from %d",
                             mevent->reqid, mevent->src);
            (*src) = mevent->src;
...
void vprotocol_pessimist_delivery_replay(size_t n, ompi_request_t **reqs, int *outcount, int *index,
                                         ompi_status_public_t *status)
{
    ...
    for (event = (mca_vprotocol_pessimist_event_t *) opal_list_get_first(
        ...
        if (devent->probeid < mca_vprotocol_pessimist.clock) {
            ...
            V_OUTPUT_VERBOSE(70, "pessimist:\treplay\tdeliver\t%" PRIpclock "\tnone",
            ...
            *outcount = 0;
            ...
            for (i = 0; i < (int) n; i++) {
                if (VPESSIMIST_FTREQ(reqs[i])->reqid == devent->reqid) {
                    V_OUTPUT_VERBOSE(70, "pessimist:\treplay\tdeliver\t%" PRIpclock "\t%" PRIpclock,
                                     devent->probeid, devent->reqid);
                    opal_list_remove_item(&mca_vprotocol_pessimist.replay_events,
                                          (opal_list_item_t *) event);
                    VPESSIMIST_EVENT_RETURN(event);
                    *index = i;
    ...
    V_OUTPUT_VERBOSE(50, "pessimist:\treplay\tdeliver\t%" PRIpclock "\tnot forced",
                     mca_vprotocol_pessimist.clock);
}
