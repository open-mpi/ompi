...
static void vprotocol_pessimist_request_construct(mca_pml_base_request_t *req)
{
    ...
    V_OUTPUT_VERBOSE(250, "pessimist:\treq\tnew\treq=%p\tPreq=%p (aligned to %p)", (void *) req,
                     (void *) ftreq, (void *) &ftreq->pml_req_free);
    ...
    assert(ftreq->pml_req_free
           == req->req_ompi.req_free); /* detection of aligment issues on different arch */
...
int mca_vprotocol_pessimist_request_free(ompi_request_t **req)
{
    ...
    V_OUTPUT_VERBOSE(50, "pessimist:\treq\tfree\t%" PRIpclock "\tpeer %d\ttag %d\tsize %lu",
                     VPESSIMIST_FTREQ(pmlreq)->reqid, pmlreq->req_peer, pmlreq->req_tag,
                     (unsigned long) pmlreq->req_count);
