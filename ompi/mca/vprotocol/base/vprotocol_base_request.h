 ...
 */
#define VPROTOCOL_RECV_FTREQ(req) (((uintptr_t) req) + mca_pml_v.host_pml_req_recv_size)
...
#define VPROTOCOL_RECV_REQ(ftreq) \
    ((mca_pml_base_recv_request_t *) (((uintptr_t) ftreq) - mca_pml_v.host_pml_req_send_size))

/** Gives the actual address of the protocol specific part of a send request.
 *   @param req (IN) the address of an ompi_request.
 *   @return address of the custom vprotocol data associated with the request.
 */
#define VPROTOCOL_SEND_FTREQ(req) (((uintptr_t) req) + mca_pml_v.host_pml_req_send_size)
...
#define VPROTOCOL_SEND_REQ(ftreq) \
    ((mca_pml_base_send_request_t *) (((uintptr_t) ftreq) - mca_pml_v.host_pml_req_send_size))
...
#define VPROTOCOL_FTREQ(req)                                                          \
    (assert((MCA_PML_REQUEST_SEND == ((mca_pml_base_request_t *) req)->req_type)      \
