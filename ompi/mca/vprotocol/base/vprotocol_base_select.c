 ...
 * caller.
 */
int mca_vprotocol_base_select(bool enable_progress_threads, bool enable_mpi_threads)
{
    ...
    OPAL_LIST_FOREACH (cli, &ompi_vprotocol_base_framework.framework_components,
        ...
        V_OUTPUT_VERBOSE(500, "vprotocol select: initializing %s component %s",
                         component->pmlm_version.mca_type_name,
                         component->pmlm_version.mca_component_name);
        if (strcmp(component->pmlm_version.mca_component_name, mca_vprotocol_base_include_list)) {
            V_OUTPUT_VERBOSE(500, "This component is not in the include list: skipping %s",
                             component->pmlm_version.mca_component_name);
            continue;
        }
        if (NULL == component->pmlm_init) {
            V_OUTPUT_VERBOSE(2, "vprotocol select: no init function; ignoring component %s",
                             component->pmlm_version.mca_component_name);
            continue;
        }
        module = component->pmlm_init(&priority, enable_progress_threads, enable_mpi_threads);
        if (NULL == module) {
            V_OUTPUT_VERBOSE(2, "vprotocol select: init returned failure for component %s",
                             component->pmlm_version.mca_component_name);
            continue;
        }
        V_OUTPUT_VERBOSE(500, "vprotocol select: component %s init returned priority %d",
    ...
    if (NULL == best_component) {
        V_OUTPUT_VERBOSE(2, "vprotocol select: no protocol has returned a positive priority, fault "
                            "tolerance is OFF");
    } else {
        /* Save the winner */
    ...
    for (item = opal_list_remove_first(&opened); NULL != item;
        ...
        if (om->om_component != best_component) {
            /* Finalize */
            V_OUTPUT_VERBOSE(500, "vprotocol select: component %s not selected / finalized",
    ...
    mca_base_components_close(mca_pml_v.output, &ompi_vprotocol_base_framework.framework_components,
