...
static mca_base_var_enum_value_t accumulate_ops_values[] = {
    {
        .value = OMPI_WIN_ACCUMULATE_OPS_SAME_OP_NO_OP,
        .string = "same_op_no_op",
    },
    {
        .value = OMPI_WIN_ACCUMULATE_OPS_SAME_OP,
        .string = "same_op",
    },
    {.value = -1, .string = NULL},
};
...
static mca_base_var_enum_value_flag_t accumulate_order_flags[] = {
    {.flag = OMPI_WIN_ACC_ORDER_NONE,
     .string = "none",
     .conflicting_flag = OMPI_WIN_ACC_ORDER_RAR | OMPI_WIN_ACC_ORDER_WAR | OMPI_WIN_ACC_ORDER_RAW
                         | OMPI_WIN_ACC_ORDER_WAW},
...
OBJ_CLASS_INSTANCE(ompi_win_t, opal_infosubscriber_t, ompi_win_construct, ompi_win_destruct);
...
int ompi_win_init(void)
{
    ...
    if (OPAL_SUCCESS
        != opal_pointer_array_init(&ompi_mpi_windows, 4, OMPI_FORTRAN_HANDLE_MAX, 16)) {
        return OMPI_ERROR;
    }
    ...
    ret = mca_base_var_enum_create("accumulate_ops", accumulate_ops_values,
                                   &ompi_win_accumulate_ops);
    if (OPAL_SUCCESS != ret) {
        return ret;
    }

    ret = mca_base_var_enum_create_flag("accumulate_order", accumulate_order_flags,
                                        &ompi_win_accumulate_order);
    if (OPAL_SUCCESS != ret) {
        return ret;
    }
...
int ompi_win_finalize(void)
{
    ...
    for (size_t i = 1; i < size; ++i) {
        ompi_win_t *win = (ompi_win_t *) opal_pointer_array_get_item(&ompi_mpi_windows, i);
        if (NULL != win) {
...
static int alloc_window(struct ompi_communicator_t *comm, opal_info_t *info, int flavor,
                        ompi_win_t **win_out)
{
    ompi_win_t *win;
    ompi_group_t *group;
    ...
    ret = opal_info_get_value_enum(info, "accumulate_ops", &acc_ops,
                                   OMPI_WIN_ACCUMULATE_OPS_SAME_OP_NO_OP, ompi_win_accumulate_ops,
                                   &flag);
    if (OMPI_SUCCESS != ret) {
        OBJ_RELEASE(win);
        return ret;
...
static int config_window(void *base, size_t size, int disp_unit, int flavor, int model,
                         ompi_win_t *win)
{
    int ret;

    ret = ompi_attr_set_c(WIN_ATTR, win, &win->w_keyhash, MPI_WIN_BASE, base, true);
    if (OMPI_SUCCESS != ret)
        return ret;

    ret = ompi_attr_set_aint(WIN_ATTR, win, &win->w_keyhash, MPI_WIN_SIZE, size, true);
    if (OMPI_SUCCESS != ret)
        return ret;

    ret = ompi_attr_set_int(WIN_ATTR, win, &win->w_keyhash, MPI_WIN_DISP_UNIT, disp_unit, true);
    if (OMPI_SUCCESS != ret)
        return ret;

    ret = ompi_attr_set_int(WIN_ATTR, win, &win->w_keyhash, MPI_WIN_CREATE_FLAVOR, flavor, true);
    if (OMPI_SUCCESS != ret)
        return ret;

    ret = ompi_attr_set_int(WIN_ATTR, win, &win->w_keyhash, MPI_WIN_MODEL, model, true);
    if (OMPI_SUCCESS != ret)
        return ret;
...
int ompi_win_free(ompi_win_t *win)
{
    ...
    if (-1 != win->w_f_to_c_index) {
        opal_pointer_array_set_item(&ompi_mpi_windows, win->w_f_to_c_index, NULL);
    }
...
int ompi_win_set_name(ompi_win_t *win, const char *win_name)
{
