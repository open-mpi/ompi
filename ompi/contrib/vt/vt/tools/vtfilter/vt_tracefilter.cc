...
int main( int argc, char** argv ) {
	...
	for ( int i = 2; i < argc; i++ ) {
		...
		if ( strcmp( "-p", argv[i] ) == 0 ) {
			showprogress = true;
		} else if ( strcmp("-fo", argv[i]) == 0 && ( i+1 < argc ) ) {
			++i;
			outfilt= argv[i];
			...
			if ( '-' != argv[i][0] ) {
				...
				cerr << "Unknown option: \"" << argv[i] << "\". Aborting" << endl;
				exit(1);
			}
		}
	}
	...
	if( intrace.empty() == true ) {
		cerr << "No no input trace has been specified. Aborting" << endl;
		err = true;
	}
	...
	switch(action) {
		case gen:
			if( desired_reducepercentage < 0.0f || desired_reducepercentage > 100.0f ) {
				cerr << "Wrong reduction percentage: " << desired_reducepercentage << ". Aborting" << endl;
				err = true;
			}
			if( outfilt.empty() == true ) {
				cerr << "No output filter has been specified. Aborting" << endl;
				err = true;
			}
			break;
		case filt:
			if( outtrace.empty() == true ) {
				cerr << "No output trace has been specified. Aborting" << endl;
				err = true;
			}
			if( infilt.empty() == true ) {
				cerr << "No input filter has been specified. Aborting" << endl;
				err = true;
			}
			break;
		case none:
		default:
			cerr << "No action has been specified. Aborting" << endl;
			err = true;
			break;
	}
	...
	if( action == gen ) {
		...
		fha.s2f.insert( pair<uint32_t,Filter>(0,Filter()) );
		...
		/* *** Read the trace once without handling records -> get the min and maxbytes.
		       ...
		       Add all streams to the stream2filter mapping *** */
		...
		for( uint32_t i = 0; i < OTF_MasterControl_getCount( pmc ); ++i ) {
			OTF_MapEntry* entry = OTF_MasterControl_getEntryByIndex( pmc, i );
			fha.s2f.insert( pair<uint32_t,Filter>(entry->argument,Filter()) );
		}
		...
		/* gather all information from the streams into a single filter structure */
		map<uint32_t /*streamid*/, Filter>::const_iterator itfuncs;
		for( itfuncs = fha.s2f.begin(); itfuncs != fha.s2f.end(); ++itfuncs ) {
