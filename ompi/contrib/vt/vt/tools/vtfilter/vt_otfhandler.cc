...
int handleDefTimerResolution( void* ud,
	...
	map<uint32_t, Filter>::iterator it = ((HandlerArgument*)ud)->s2f.find( 0 );
...
int handleEnter( void* ud, uint64_t time, uint32_t function,
	...
	map<uint32_t, Filter>::iterator it = ((HandlerArgument*)ud)->s2f.find( process );
...
int handleRecvMsg( void* ud, uint64_t /*time*/,
	...
	map<uint32_t, Filter>::iterator it = ((HandlerArgument*)ud)->s2f.find( receiver );
...
int handleSendMsg( void* ud, uint64_t /*time*/,
	...
	map<uint32_t, Filter>::iterator it = ((HandlerArgument*)ud)->s2f.find( sender );
