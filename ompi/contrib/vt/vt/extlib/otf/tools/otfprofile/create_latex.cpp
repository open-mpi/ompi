                                 ...
                                 struct AllData& alldata)
{
  ...
  if(alldata.messageMapPerGroup.empty()) return;
  ...
  if(grouped){
    it = alldata.messageMapPerGroup.begin(); i = 0;
    while ( itend != it ) {
  ...
  if(byt_avail){
    ...
    if(grouped) tex << ss_bytes_send_e.str() << ss_bytes_recv_e.str();
  ...
  if(dur_avail){
    ...
    if(grouped) tex << ss_duration_send_e.str();
 ...
 */
static void getGroupLabel(struct AllData& alldata, uint64_t id,
                          string& label)
{
  if(grouped){
    set<uint64_t> procs = alldata.grouping.groupsToProcesses.find(id)->second;
                              ...
                              struct AllData& alldata)
{
  ...
  for(it = alldata.collectiveMapPerGroup.begin(); it != itend; it++){
    ...
    if(it->second.bytes_send.cnt){
      double val = (double)it->second.bytes_send.sum/it->second.bytes_send.cnt;
      tex << " " << val;
      if(val < minMax.bytes.min) minMax.bytes.min = val;
      if(val > minMax.bytes.max) minMax.bytes.max = val;
    }else tex << " 0";

    if(it->second.bytes_recv.cnt){
      double val = (double)it->second.bytes_recv.sum/it->second.bytes_recv.cnt;
      tex << " " << val;
      if(val < minMax.bytes.min) minMax.bytes.min = val;
      if(val > minMax.bytes.max) minMax.bytes.max = val;
    }else tex << " 0";
