...
{
  mpi_fid = vt_def_scl_file("MPI");

  vt_mpi_regid[VT__MPI_INIT] =
    vt_def_region("MPI_Init", mpi_fid, VT_NO_LNO, VT_NO_LNO, NULL,
                  VT_MPI_FUNCTION);

  vt_mpi_regid[VT__MPI_INITIALIZED] =
      vt_def_region("MPI_Initialized", mpi_fid, VT_NO_LNO, VT_NO_LNO, NULL,
                    VT_MPI_FUNCTION);

#if defined(HAVE_MPI2_THREAD) && HAVE_MPI2_THREAD

  vt_mpi_regid[VT__MPI_INIT_THREAD] =
    vt_def_region("MPI_Init_thread", mpi_fid, VT_NO_LNO, VT_NO_LNO, NULL,
                  VT_MPI_FUNCTION);
