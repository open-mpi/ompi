...
OBJ_CLASS_INSTANCE(ompi_seq_tracker_range_t, opal_list_item_t, NULL, NULL);

static void ompi_seq_tracker_construct(ompi_seq_tracker_t *seq_tracker)
{
    OBJ_CONSTRUCT(&seq_tracker->seq_ids, opal_list_t);
    seq_tracker->seq_ids_current = (ompi_seq_tracker_range_t *) opal_list_get_end(
        &seq_tracker->seq_ids);
}
...
OBJ_CLASS_INSTANCE(ompi_seq_tracker_t, opal_object_t, ompi_seq_tracker_construct,
...
bool ompi_seq_tracker_check_duplicate(ompi_seq_tracker_t *seq_tracker, uint32_t seq_id)
{
    ompi_seq_tracker_range_t *item;
    const ompi_seq_tracker_range_t *sentinel = (ompi_seq_tracker_range_t *) opal_list_get_end(
        &seq_tracker->seq_ids);
 ...
 */
void ompi_seq_tracker_insert(ompi_seq_tracker_t *seq_tracker, uint32_t seq_id)
{
    ...
    while (true) {
        if (item == sentinel) {
            ...
            if (direction == -1) {
                ...
                opal_list_insert_pos(seq_ids, (opal_list_item_t *) next_item,
            ...
            if (direction == 1) {
                ...
                new_item = OBJ_NEW(ompi_seq_tracker_range_t);
                new_item->seq_id_low = new_item->seq_id_high = seq_id;
                opal_list_insert_pos(seq_ids, (opal_list_item_t *) item,
                                     (opal_list_item_t *) new_item);
...
{
    opal_list_item_t *item;
    for (item = opal_list_get_first(&src->seq_ids); item != opal_list_get_end(&src->seq_ids);
