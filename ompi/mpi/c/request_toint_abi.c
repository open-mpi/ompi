...
int MPI_Request_toint(MPI_Request_ABI_INTERNAL request)
{
    ...
    if ( MPI_PARAM_CHECK ) {
        ...
        if (NULL == request) {
            int intern_err = OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_REQUEST, FUNC_NAME);
...
__opal_attribute_weak__ int MPI_Request_toint(MPI_Request_ABI_INTERNAL request)
{
