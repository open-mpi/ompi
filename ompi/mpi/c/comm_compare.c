...
int MPI_Comm_compare(MPI_Comm comm1, MPI_Comm comm2, int *result)
{
    ...
    MEMCHECKER(memchecker_comm(comm1); memchecker_comm(comm2););
    ...
    if (MPI_PARAM_CHECK) {
        ...
        if (NULL == result) {
            return OMPI_ERRHANDLER_INVOKE(comm1, MPI_ERR_ARG, FUNC_NAME);
        }
    }

    rc = ompi_comm_compare((ompi_communicator_t *) comm1, (ompi_communicator_t *) comm2, result);
