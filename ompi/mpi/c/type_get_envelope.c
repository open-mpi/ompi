...
int MPI_Type_get_envelope(MPI_Datatype type, int *num_integers, int *num_addresses,
                          int *num_datatypes, int *combiner)
{
    ...
    if (MPI_PARAM_CHECK) {
        ...
        } else if (NULL == num_integers || NULL == num_addresses || NULL == num_datatypes
                   || NULL == combiner) {
            return OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_ARG, FUNC_NAME);
        }
    }

    rc = ompi_datatype_get_args(type, 0, num_integers, NULL, num_addresses, NULL, num_datatypes,
                                NULL, combiner);
