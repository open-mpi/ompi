...
int MPI_Intercomm_create(MPI_Comm local_comm, int local_leader, MPI_Comm bridge_comm,
                         int remote_leader, int tag, MPI_Comm *newintercomm)
{
    ...
    MEMCHECKER(memchecker_comm(local_comm); memchecker_comm(bridge_comm););

    if (MPI_PARAM_CHECK) {
        OMPI_ERR_INIT_FINALIZE(FUNC_NAME);

        if (ompi_comm_invalid(local_comm) || (local_comm->c_flags & OMPI_COMM_INTER))
            return OMPI_ERRHANDLER_INVOKE(MPI_COMM_WORLD, MPI_ERR_COMM, FUNC_NAME);

        if (NULL == newintercomm)
            return OMPI_ERRHANDLER_INVOKE(local_comm, MPI_ERR_ARG, FUNC_NAME);
    ...
    if (MPI_PARAM_CHECK) {
        ...
        if (local_rank == local_leader) {
            if (ompi_comm_invalid(bridge_comm) || (bridge_comm->c_flags & OMPI_COMM_INTER)) {
                return OMPI_ERRHANDLER_INVOKE(local_comm, MPI_ERR_COMM, FUNC_NAME);
            }
    ...
    if (local_rank == local_leader) {
        ...
        rc = MCA_PML_CALL(
            send(&local_size, 1, MPI_INT, rleader, tag, MCA_PML_BASE_SEND_STANDARD, bridge_comm));
        if (rc != MPI_SUCCESS) {
#if OPAL_ENABLE_FT_MPI
            if (MPI_ERR_PROC_FAILED == rc) {
                rsize = 0;
    ...
    if (MPI_PARAM_CHECK) {
        if (OMPI_GROUP_IS_DENSE(local_comm->c_local_group)) {
            rc = ompi_comm_overlapping_groups(local_comm->c_local_group->grp_proc_count,
                                              local_comm->c_local_group->grp_proc_pointers, rsize,
                                              rprocs);
        } else {
            ...
            rc = ompi_comm_overlapping_groups(local_comm->c_local_group->grp_proc_count, proc_list,
                                              rsize, rprocs);
        }
        if (OMPI_SUCCESS != rc) {
            goto err_exit;
    ...
    rc = ompi_comm_nextcid(newcomp, local_comm, bridge_comm, &lleader, &rleader, false,
    ...
    rc = ompi_comm_activate(&newcomp, local_comm, bridge_comm, &lleader, &rleader, false,
                            OMPI_COMM_CID_INTRA_BRIDGE);
    if (MPI_SUCCESS != rc) {
        goto err_exit;
    }
...
err_exit:
    ...
    if (OMPI_SUCCESS != rc) {
        *newintercomm = MPI_COMM_NULL;
        return OMPI_ERRHANDLER_INVOKE(local_comm, rc, FUNC_NAME);
    }
