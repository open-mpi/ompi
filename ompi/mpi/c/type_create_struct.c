                           ...
                           const MPI_Datatype array_of_types[], MPI_Datatype *newtype)
{
    ...
    if (count > 0) {
        for (i = 0; i < count; i++) {
            MEMCHECKER(memchecker_datatype(array_of_types[i]););
        }
    }
    ...
    if (MPI_PARAM_CHECK) {
        ...
        for (i = 0; i < count; i++) {
            if (NULL == array_of_types[i] || MPI_DATATYPE_NULL == array_of_types[i]) {
                return OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_TYPE, FUNC_NAME);
...

    {
        ...
        ompi_datatype_set_args(*newtype, count + 1, a_i, count, array_of_displacements, count,
                               array_of_types, MPI_COMBINER_STRUCT);
    }
