...
int MPI_Session_toint(MPI_Session_ABI_INTERNAL session)
{
    ...
    if ( MPI_PARAM_CHECK ) {
        ...
        if (NULL == session) {
            int intern_err = OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_SESSION, FUNC_NAME);
...
__opal_attribute_weak__ int MPI_Session_toint(MPI_Session_ABI_INTERNAL session)
{
