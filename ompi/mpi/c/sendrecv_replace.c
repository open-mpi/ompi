...

{
    ...
    if (source == MPI_PROC_NULL || dest == MPI_PROC_NULL || count == 0) {
        rc = PMPI_Sendrecv(buf, count, datatype, dest, sendtag, buf, count, datatype, source,
                           recvtag, comm, status);
     ...
     * (potentially non-contiguous) and thus the allocated memory will be larger than the size of
     * the datatype. A simpler, but potentially less efficient approach is to work on the data we
     * have control of, aka the sent data, and pack it into a contiguous buffer before posting the
     * receive. Once the send completes, we free it.
     */
    ...
    rc = MCA_PML_CALL(irecv(buf, count, datatype, source, recvtag, comm, &req));
    if (OMPI_SUCCESS != rc) {
        goto cleanup_and_return;
    }

    /* send from the temporary buffer */
    rc = MCA_PML_CALL(send(iov.iov_base, packed_size, MPI_PACKED, dest, sendtag,
