                          ...
                          MPI_Comm *newcomm)
{
    ...
    if (MPI_PARAM_CHECK) {
        ...
        if (ompi_comm_invalid(comm_old)) {
            ...
            return OMPI_ERRHANDLER_INVOKE(comm_old, MPI_ERR_ARG, FUNC_NAME);
                   ...
                   && (NULL == sources || NULL == degrees || NULL == destinations
                       || NULL == weights)) {
            return OMPI_ERRHANDLER_INVOKE(comm_old, MPI_ERR_ARG, FUNC_NAME);
        }
        ...
        for (i = index = 0; i < n; ++i) {
            ...
            for (j = 0; j < degrees[i]; ++j) {
                if (((destinations[index] < 0) && (destinations[index] != MPI_PROC_NULL))
                    || destinations[index] >= comm_size) {
                    return OMPI_ERRHANDLER_INVOKE(comm_old, MPI_ERR_ARG, FUNC_NAME);
    ...
    err = topo->topo.dist_graph.dist_graph_create(topo, comm_old, n, sources, degrees, destinations,
                                                  weights, &(info->super), reorder, newcomm);
    OMPI_ERRHANDLER_RETURN(err, comm_old, err, FUNC_NAME);
}
