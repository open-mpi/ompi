...
#define OMPI_CHECK_DATATYPE_FOR_SEND(RC, DDT, COUNT)           \
    do {                                                       \
        /* (RC) = MPI_SUCCESS; */                              \
        if (NULL == (DDT) || MPI_DATATYPE_NULL == (DDT))       \
            (RC) = MPI_ERR_TYPE;                               \
        else if ((COUNT) < 0)                                  \
            (RC) = MPI_ERR_COUNT;                              \
        else if (!opal_datatype_is_committed(&((DDT)->super))) \
            (RC) = MPI_ERR_TYPE;                               \
        else if (!opal_datatype_is_valid(&((DDT)->super)))     \
            (RC) = MPI_ERR_TYPE;                               \
    } while (0)
...
#define OMPI_CHECK_DATATYPE_FOR_ONE_SIDED(RC, DDT, COUNT)      \
    do {                                                       \
        ...
        if (NULL == (DDT) || MPI_DATATYPE_NULL == (DDT))       \
            ...
            (RC) = MPI_ERR_TYPE;                               \
        else if (opal_datatype_is_overlapped(&((DDT)->super))) \
            (RC) = MPI_ERR_TYPE;                               \
        else if (!opal_datatype_is_valid(&((DDT)->super)))     \
            (RC) = MPI_ERR_TYPE;                               \
    } while (0)
...
#define OMPI_CHECK_DATATYPE_FOR_VIEW(RC, DDT, COUNT)                                           \
    do {                                                                                       \
        ...
        /* XXX Fix flags else if( ompi_datatype_is_overlapped((DDT)) ) (RC) = MPI_ERR_TYPE; */ \
            ...
            (RC) = MPI_ERR_TYPE;                                                               \
        else if (!ompi_datatype_is_monotonic((DDT)))                                           \
            (RC) = MPI_ERR_TYPE;                                                               \
    } while (0)
    ...
    do {                                                                                \
        if (NULL == (BUFFER) && 0 < (COUNT) && MPI_SUCCESS == (RC)) {                   \
            if ((DDT)->super.flags & OPAL_DATATYPE_FLAG_PREDEFINED) {                   \
                ...
                ompi_datatype_get_true_extent((DDT), &ompi_chk_usr_buf_true_lb,         \
                                              &ompi_chk_usr_buf_true_extended);         \
                if (0 < ompi_chk_usr_buf_size && 0 == ompi_chk_usr_buf_true_extended) { \
                    (RC) = MPI_ERR_BUFFER;                                              \
                }                                                                       \
            }                                                                           \
        }                                                                               \
