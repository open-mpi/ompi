...
int MPI_Status_c2f(const MPI_Status *c_status, MPI_Fint *f_status)
{
    const int *c_ints;
    int i;
    MEMCHECKER(if (c_status != MPI_STATUSES_IGNORE) {
        /*
    ...
    if (MPI_PARAM_CHECK) {
        ...
        if (NULL == c_status || MPI_STATUS_IGNORE == c_status || MPI_STATUSES_IGNORE == c_status
            || NULL == f_status) {
            return OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_IN_STATUS, FUNC_NAME);
        }
    }
