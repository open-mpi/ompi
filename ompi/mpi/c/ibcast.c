               ...
               MPI_Request *request)
{
    ...
    MEMCHECKER(
        memchecker_datatype(datatype); memchecker_comm(comm); if (OMPI_COMM_IS_INTRA(comm)) {
    ...
    if ((OMPI_COMM_IS_INTRA(comm) && ompi_comm_size(comm) <= 1) || 0 == count) {
    ...
    err = comm->c_coll->coll_ibcast(buffer, count, datatype, root, comm, request,
