               ...
               int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm)
{
    ...
    if ((0 == sendcount && MPI_ROOT != root
        ...
        || (ompi_comm_rank(comm) == root && MPI_IN_PLACE == sendbuf && 0 == recvcount)
        || (0 == recvcount && (MPI_ROOT == root || MPI_PROC_NULL == root))) {
        return MPI_SUCCESS;
    }

    /* Invoke the coll component to perform the back-end operation */
    err = comm->c_coll->coll_gather(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype,
                                    root, comm, comm->c_coll->coll_gather_module);
    OMPI_ERRHANDLER_RETURN(err, comm, err, FUNC_NAME);
}
