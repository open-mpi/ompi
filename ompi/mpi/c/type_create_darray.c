...

{
    ...
    if (MPI_PARAM_CHECK) {
        ...
        if ((rank < 0) || (size < 0) || (rank >= size)) {
            ...
            return OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_COUNT, FUNC_NAME);
                   ...
                   && ((NULL == gsize_array) || (NULL == distrib_array) || (NULL == darg_array)
                       || (NULL == psize_array))) {
            return OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_ARG, FUNC_NAME);
            ...
            for (i = 0; i < ndims; i++) {
                if ((MPI_DISTRIBUTE_BLOCK != distrib_array[i])
                    ...
                    return OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_ARG, FUNC_NAME);
                           ...
                           || ((darg_array[i] < 0)
                               && (MPI_DISTRIBUTE_DFLT_DARG != darg_array[i]))) {
                    return OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_ARG, FUNC_NAME);
    ...
    rc = ompi_datatype_create_darray(size, rank, ndims, gsize_array, distrib_array, darg_array,
                                     psize_array, order, oldtype, newtype);
    if (OMPI_SUCCESS == rc) {
        const int *a_i[8] = {&size,         &rank,      &ndims,      gsize_array,
                             distrib_array, darg_array, psize_array, &order};
