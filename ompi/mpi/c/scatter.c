                ...
                int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm)
{
    ...
    if ((0 == recvcount && MPI_ROOT != root
        ...
        || (ompi_comm_rank(comm) == root && MPI_IN_PLACE == recvbuf && 0 == sendcount)
        || (0 == sendcount && (MPI_ROOT == root || MPI_PROC_NULL == root))) {
        return MPI_SUCCESS;
    }

    /* Invoke the coll component to perform the back-end operation */
    err = comm->c_coll->coll_scatter(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype,
                                     root, comm, comm->c_coll->coll_scatter_module);
    OMPI_ERRHANDLER_RETURN(err, comm, err, FUNC_NAME);
}
