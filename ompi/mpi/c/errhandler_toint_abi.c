...
int MPI_Errhandler_toint(MPI_Errhandler_ABI_INTERNAL errhandler)
{
    ...
    if (OMPI_ABI_HANDLE_BASE_OFFSET > (intptr_t)errhandler) {
        errhandler_tmp = (intptr_t)errhandler;
        return (int)errhandler_tmp;
    }

    errhandler_ptr = (ompi_errhandler_t *)errhandler;
    o_index = errhandler_ptr->eh_f_to_c_index;
