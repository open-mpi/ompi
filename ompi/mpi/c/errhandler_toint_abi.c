...
int MPI_Errhandler_toint(MPI_Errhandler_ABI_INTERNAL errhandler)
{
    ...
    /* Handle predefined-handle integer values before any validity
       checking: an arbitrary in-range value is not a pointer and must
       not be dereferenced by the handle-validity checks below. */
    ...
    if ( MPI_PARAM_CHECK ) {
        ...
        if (NULL == errhandler) {
            int intern_err = OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_ERRHANDLER, FUNC_NAME);
            return ompi_convert_intern_error_abi_error(intern_err);
        }
    }
...
__opal_attribute_weak__ int MPI_Errhandler_toint(MPI_Errhandler_ABI_INTERNAL errhandler)
{
