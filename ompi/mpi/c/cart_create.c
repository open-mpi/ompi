...
int MPI_Cart_create(MPI_Comm old_comm, int ndims, const int dims[], const int periods[],
                    int reorder, MPI_Comm *comm_cart)
{
    ...
    if (MPI_PARAM_CHECK) {
        ...
        } else if (ndims >= 1 && (NULL == dims || NULL == periods || NULL == comm_cart)) {
            return OMPI_ERRHANDLER_INVOKE(old_comm, MPI_ERR_ARG, FUNC_NAME);
        }
     ...
     */
    if (OMPI_SUCCESS != (err = mca_topo_base_comm_select(old_comm, NULL, &topo, OMPI_COMM_CART))) {
        return err;
    }
    ...
    err = topo->topo.cart.cart_create(topo, old_comm, ndims, dims, periods,
