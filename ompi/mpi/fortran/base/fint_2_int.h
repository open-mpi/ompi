...
#    define OMPI_ARRAY_FINT_2_INT_ALLOC(in, n)  OMPI_ARRAY_NAME_CONVERT(in) = malloc(n * sizeof(int))
...
#    define OMPI_2_DIM_ARRAY_FINT_2_INT(in, n, dim2)                               \
        do {                                                                       \
            int converted_n = (int) (n);                                           \
            OMPI_ARRAY_NAME_CONVERT(in) = (int(*)[dim2]) malloc(                   \
                converted_n * sizeof(*OMPI_ARRAY_NAME_CONVERT(in)));               \
            while (--converted_n >= 0) {                                           \
                for (dim2_index = 0; dim2_index < dim2; ++dim2_index) {            \
                    OMPI_ARRAY_NAME_CONVERT(in)                                    \
                    [converted_n][dim2_index] = (int) in[converted_n][dim2_index]; \
                }                                                                  \
            }                                                                      \
        } while (0)
...
#    define OMPI_SINGLE_FINT_2_INT(in)        OMPI_ARRAY_NAME_CONVERT(in) = (int) *(in)

/* This is for single OUT parameter */
#    define OMPI_SINGLE_INT_2_FINT(in)        *(in) = OMPI_ARRAY_NAME_CONVERT(in)
...
#    define OMPI_2_DIM_ARRAY_FINT_2_INT(in, n, dim2)                         \
        do {                                                                 \
            ...
            while (--converted_n >= 0) {                                     \
                for (dim2_index = 0; dim2_index < dim2; ++dim2_index) {      \
                    OMPI_ARRAY_NAME_CONVERT(in)                              \
                    [converted_n][dim2_index] = in[converted_n][dim2_index]; \
                }                                                            \
            }                                                                \
        } while (0)
...
#    define OMPI_SINGLE_FINT_2_INT(in) OMPI_ARRAY_NAME_CONVERT(in) = *(in)

#    define OMPI_SINGLE_INT_2_FINT(in) *in = (MPI_Fint) OMPI_ARRAY_NAME_CONVERT(in)
...
#    define OMPI_SINGLE_INT_2_LOGICAL( \
        a) /* Single-OUT variable -- Not needed for int==logical, true=1 */
#else
/*
...
#    define OMPI_ARRAY_LOGICAL_2_INT_CLEANUP(in) free(OMPI_LOGICAL_ARRAY_NAME_CONVERT(in))
...
#    define OMPI_ARRAY_LOGICAL_2_INT(in, n)                          \
        do {                                                         \
            ...
            while (--converted_n >= 0) {                             \
                OMPI_LOGICAL_ARRAY_NAME_CONVERT(in)                  \
                [converted_n] = OMPI_LOGICAL_2_INT(in[converted_n]); \
            }                                                        \
        } while (0)
#    define OMPI_ARRAY_INT_2_LOGICAL(in, n)                            \
        do {                                                           \
            int converted_n = (int) (n);                               \
            while (--converted_n >= 0) {                               \
                in[converted_n] = OMPI_INT_2_LOGICAL(                  \
                    OMPI_LOGICAL_ARRAY_NAME_CONVERT(in)[converted_n]); \
            }                                                          \
