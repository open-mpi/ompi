...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_COMM_SPLIT, pmpi_comm_split, pmpi_comm_split_, pmpi_comm_split__,
                           pompi_comm_split_f,
                           (MPI_Fint * comm, MPI_Fint *color, MPI_Fint *key, MPI_Fint *newcomm,
                            MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_COMM_SPLIT, mpi_comm_split, mpi_comm_split_, mpi_comm_split__,
                           ompi_comm_split_f,
                           (MPI_Fint * comm, MPI_Fint *color, MPI_Fint *key, MPI_Fint *newcomm,
                            MPI_Fint *ierr),
                           (comm, color, key, newcomm, ierr))
...
void ompi_comm_split_f(MPI_Fint *comm, MPI_Fint *color, MPI_Fint *key, MPI_Fint *newcomm,
                       MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Comm c_newcomm;
    ...
    c_ierr = PMPI_Comm_split(c_comm, OMPI_FINT_2_INT(*color), OMPI_FINT_2_INT(*key), &c_newcomm);
    if (NULL != ierr)
