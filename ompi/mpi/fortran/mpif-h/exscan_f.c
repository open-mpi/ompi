...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_EXSCAN, pmpi_exscan, pmpi_exscan_, pmpi_exscan__, pompi_exscan_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_EXSCAN, mpi_exscan, mpi_exscan_, mpi_exscan__, ompi_exscan_f,
                           (char *sendbuf, char *recvbuf, MPI_Fint *count, MPI_Fint *datatype,
                            MPI_Fint *op, MPI_Fint *comm, MPI_Fint *ierr),
                           (sendbuf, recvbuf, count, datatype, op, comm, ierr))
#    else
...
void ompi_exscan_f(char *sendbuf, char *recvbuf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *op,
                   MPI_Fint *comm, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Exscan(sendbuf, recvbuf, OMPI_FINT_2_INT(*count), c_type, c_op, c_comm);
    if (NULL != ierr)
