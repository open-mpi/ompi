...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_TESTANY, pmpi_testany, pmpi_testany_, pmpi_testany__,
                           pompi_testany_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_TESTANY, mpi_testany, mpi_testany_, mpi_testany__, ompi_testany_f,
                           (MPI_Fint * count, MPI_Fint *array_of_requests, MPI_Fint *indx,
                            ompi_fortran_logical_t *flag, MPI_Fint *status, MPI_Fint *ierr),
                           (count, array_of_requests, indx, flag, status, ierr))
#    else
...
void ompi_testany_f(MPI_Fint *count, MPI_Fint *array_of_requests, MPI_Fint *indx,
                    ompi_fortran_logical_t *flag, MPI_Fint *status, MPI_Fint *ierr)
{
    MPI_Request *c_req;
    MPI_Status c_status;
    ...
    c_ierr = PMPI_Testany(OMPI_FINT_2_INT(*count), c_req, OMPI_SINGLE_NAME_CONVERT(indx),
    ...
    if (MPI_SUCCESS == c_ierr) {
        ...
        if (*flag && MPI_UNDEFINED != *(OMPI_SINGLE_NAME_CONVERT(indx))) {
