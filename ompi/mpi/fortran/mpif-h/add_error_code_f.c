...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_ADD_ERROR_CODE, pmpi_add_error_code, pmpi_add_error_code_,
                           pmpi_add_error_code__, pompi_add_error_code_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_ADD_ERROR_CODE, mpi_add_error_code, mpi_add_error_code_,
                           mpi_add_error_code__, ompi_add_error_code_f,
                           (MPI_Fint * errorclass, MPI_Fint *errorcode, MPI_Fint *ierr),
                           (errorclass, errorcode, ierr))
#    else
...
void ompi_add_error_code_f(MPI_Fint *errorclass, MPI_Fint *errorcode, MPI_Fint *ierr)
{
    ...
    ierr_c = PMPI_Add_error_code(OMPI_FINT_2_INT(*errorclass), OMPI_SINGLE_NAME_CONVERT(errorcode));
