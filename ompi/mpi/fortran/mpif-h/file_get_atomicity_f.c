...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_GET_ATOMICITY, pmpi_file_get_atomicity,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_GET_ATOMICITY, mpi_file_get_atomicity, mpi_file_get_atomicity_,
                           mpi_file_get_atomicity__, ompi_file_get_atomicity_f,
                           (MPI_Fint * fh, ompi_fortran_logical_t *flag, MPI_Fint *ierr),
                           (fh, flag, ierr))
#    else
...
void ompi_file_get_atomicity_f(MPI_Fint *fh, ompi_fortran_logical_t *flag, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_File_get_atomicity(c_fh, OMPI_LOGICAL_SINGLE_NAME_CONVERT(flag));
    if (NULL != ierr)
