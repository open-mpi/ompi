...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_GRAPH_CREATE, pmpi_graph_create, pmpi_graph_create_,
                           pmpi_graph_create__, pompi_graph_create_f,
                           (MPI_Fint * comm_old, MPI_Fint *nnodes, MPI_Fint *indx, MPI_Fint *edges,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_GRAPH_CREATE, mpi_graph_create, mpi_graph_create_,
                           mpi_graph_create__, ompi_graph_create_f,
                           (MPI_Fint * comm_old, MPI_Fint *nnodes, MPI_Fint *indx, MPI_Fint *edges,
                            ompi_fortran_logical_t *reorder, MPI_Fint *comm_graph, MPI_Fint *ierr),
                           (comm_old, nnodes, indx, edges, reorder, comm_graph, ierr))
#    else
...
void ompi_graph_create_f(MPI_Fint *comm_old, MPI_Fint *nnodes, MPI_Fint *indx, MPI_Fint *edges,
                         ompi_fortran_logical_t *reorder, MPI_Fint *comm_graph, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_Graph_create(c_comm_old, OMPI_FINT_2_INT(*nnodes), OMPI_ARRAY_NAME_CONVERT(indx),
                               OMPI_ARRAY_NAME_CONVERT(edges), OMPI_LOGICAL_2_INT(*reorder),
                               &c_comm_graph);
    if (NULL != ierr)
