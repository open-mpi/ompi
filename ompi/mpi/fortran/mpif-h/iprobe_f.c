...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_IPROBE, pmpi_iprobe, pmpi_iprobe_, pmpi_iprobe__, pompi_iprobe_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_IPROBE, mpi_iprobe, mpi_iprobe_, mpi_iprobe__, ompi_iprobe_f,
                           (MPI_Fint * source, MPI_Fint *tag, MPI_Fint *comm,
                            ompi_fortran_logical_t *flag, MPI_Fint *status, MPI_Fint *ierr),
                           (source, tag, comm, flag, status, ierr))
#    else
...
void ompi_iprobe_f(MPI_Fint *source, MPI_Fint *tag, MPI_Fint *comm, ompi_fortran_logical_t *flag,
                   MPI_Fint *status, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Iprobe(OMPI_FINT_2_INT(*source), OMPI_FINT_2_INT(*tag), c_comm,
