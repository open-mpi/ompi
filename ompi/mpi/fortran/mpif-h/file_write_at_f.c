...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_WRITE_AT, pmpi_file_write_at, pmpi_file_write_at_,
                           pmpi_file_write_at__, pompi_file_write_at_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_WRITE_AT, mpi_file_write_at, mpi_file_write_at_,
                           mpi_file_write_at__, ompi_file_write_at_f,
                           (MPI_Fint * fh, MPI_Offset *offset, char *buf, MPI_Fint *count,
                            MPI_Fint *datatype, MPI_Fint *status, MPI_Fint *ierr),
                           (fh, offset, buf, count, datatype, status, ierr))
#    endif
...
void ompi_file_write_at_f(MPI_Fint *fh, MPI_Offset *offset, char *buf, MPI_Fint *count,
                          MPI_Fint *datatype, MPI_Fint *status, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_File_write_at(c_fh, (MPI_Offset) *offset, OMPI_F2C_BOTTOM(buf),
