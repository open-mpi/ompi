...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_COMM_SPLIT_TYPE, pmpi_comm_split_type, pmpi_comm_split_type_,
                           pmpi_comm_split_type__, pompi_comm_split_type_f,
                           (MPI_Fint * comm, MPI_Fint *split_type, MPI_Fint *key, MPI_Fint *info,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_COMM_SPLIT_TYPE, mpi_comm_split_type, mpi_comm_split_type_,
                           mpi_comm_split_type__, ompi_comm_split_type_f,
                           (MPI_Fint * comm, MPI_Fint *split_type, MPI_Fint *key, MPI_Fint *info,
                            MPI_Fint *newcomm, MPI_Fint *ierr),
                           (comm, split_type, key, info, newcomm, ierr))
#    else
...
void ompi_comm_split_type_f(MPI_Fint *comm, MPI_Fint *split_type, MPI_Fint *key, MPI_Fint *info,
                            MPI_Fint *newcomm, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Comm c_newcomm;
    ...
    c_ierr = PMPI_Comm_split_type(c_comm, OMPI_FINT_2_INT(*split_type), OMPI_FINT_2_INT(*key),
                                  c_info, &c_newcomm);
    if (NULL != ierr)
