...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_WRITE_ALL_END, pmpi_file_write_all_end,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_WRITE_ALL_END, mpi_file_write_all_end, mpi_file_write_all_end_,
                           mpi_file_write_all_end__, ompi_file_write_all_end_f,
                           (MPI_Fint * fh, char *buf, MPI_Fint *status, MPI_Fint *ierr),
                           (fh, buf, status, ierr))
#    else
...
void ompi_file_write_all_end_f(MPI_Fint *fh, char *buf, MPI_Fint *status, MPI_Fint *ierr)
{
    int c_ierr;
