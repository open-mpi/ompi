...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_REQUEST_GET_STATUS, pmpi_request_get_status,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_REQUEST_GET_STATUS, mpi_request_get_status, mpi_request_get_status_,
                           mpi_request_get_status__, ompi_request_get_status_f,
                           (MPI_Fint * request, ompi_fortran_logical_t *flag, MPI_Fint *status,
                            MPI_Fint *ierr),
                           (request, flag, status, ierr))
#    else
...
void ompi_request_get_status_f(MPI_Fint *request, ompi_fortran_logical_t *flag, MPI_Fint *status,
                               MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Status c_status;
