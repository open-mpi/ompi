...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_WIN_START, pmpi_win_start, pmpi_win_start_, pmpi_win_start__,
                           pompi_win_start_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_WIN_START, mpi_win_start, mpi_win_start_, mpi_win_start__,
                           ompi_win_start_f,
                           (MPI_Fint * group, MPI_Fint *assert, MPI_Fint *win, MPI_Fint *ierr),
                           (group, assert, win, ierr))
#    else
...
void ompi_win_start_f(MPI_Fint *group, MPI_Fint *assert, MPI_Fint *win, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Group c_grp = PMPI_Group_f2c(*group);
    MPI_Win c_win = PMPI_Win_f2c(*win);

    c_ierr = PMPI_Win_start(c_grp, OMPI_FINT_2_INT(*assert), c_win);
    if (NULL != ierr)
