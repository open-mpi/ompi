...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_OP_CREATE, pmpi_op_create, pmpi_op_create_, pmpi_op_create__,
                           pompi_op_create_f,
                           (ompi_op_fortran_handler_fn_t * function,
                            ompi_fortran_logical_t *commute, MPI_Fint *op, MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_OP_CREATE, mpi_op_create, mpi_op_create_, mpi_op_create__,
                           ompi_op_create_f,
                           (ompi_op_fortran_handler_fn_t * function,
                            ompi_fortran_logical_t *commute, MPI_Fint *op, MPI_Fint *ierr),
                           (function, commute, op, ierr))
                      ...
                      MPI_Fint *op, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Op_create((MPI_User_function *) function, OMPI_LOGICAL_2_INT(*commute), &c_op);
    if (NULL != ierr)
