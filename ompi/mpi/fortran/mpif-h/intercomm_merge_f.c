...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_INTERCOMM_MERGE, pmpi_intercomm_merge, pmpi_intercomm_merge_,
                           pmpi_intercomm_merge__, pompi_intercomm_merge_f,
                           (MPI_Fint * intercomm, ompi_fortran_logical_t *high,
...
OMPI_GENERATE_F77_BINDINGS(MPI_INTERCOMM_MERGE, mpi_intercomm_merge, mpi_intercomm_merge_,
                           mpi_intercomm_merge__, ompi_intercomm_merge_f,
                           (MPI_Fint * intercomm, ompi_fortran_logical_t *high,
                            MPI_Fint *newintercomm, MPI_Fint *ierr),
                           (intercomm, high, newintercomm, ierr))
#    else
...
void ompi_intercomm_merge_f(MPI_Fint *intercomm, ompi_fortran_logical_t *high,
                            MPI_Fint *newintracomm, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Comm c_newcomm;
    MPI_Comm c_intercomm = PMPI_Comm_f2c(*intercomm);

    c_ierr = PMPI_Intercomm_merge(c_intercomm, OMPI_LOGICAL_2_INT(*high), &c_newcomm);
    if (NULL != ierr)
