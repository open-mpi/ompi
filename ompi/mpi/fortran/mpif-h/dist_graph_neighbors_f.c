...
OMPI_GENERATE_F77_BINDINGS(
    PMPI_DIST_GRAPH_NEIGHBORS, pmpi_dist_graph_neighbors, pmpi_dist_graph_neighbors_,
    pmpi_dist_graph_neighbors__, pompi_dist_graph_neighbors_f,
...
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(
    MPI_DIST_GRAPH_NEIGHBORS, mpi_dist_graph_neighbors, mpi_dist_graph_neighbors_,
    mpi_dist_graph_neighbors__, ompi_dist_graph_neighbors_f,
    (MPI_Fint * comm, MPI_Fint *maxindegree, MPI_Fint *sources, MPI_Fint *sourceweights,
     MPI_Fint *maxoutdegree, MPI_Fint *destinations, MPI_Fint *destweights, MPI_Fint *ierr),
    (comm, maxindegree, sources, sourceweights, maxoutdegree, destinations, destweights, ierr))
#    endif
...
void ompi_dist_graph_neighbors_f(MPI_Fint *comm, MPI_Fint *maxindegree, MPI_Fint *sources,
                                 ...
                                 MPI_Fint *destinations, MPI_Fint *destweights, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Dist_graph_neighbors(c_comm, OMPI_FINT_2_INT(*maxindegree),
                                       ...
                                       OMPI_IS_FORTRAN_UNWEIGHTED(sourceweights)
                                           ? MPI_UNWEIGHTED
                                           : OMPI_ARRAY_NAME_CONVERT(sourceweights),
                                       OMPI_FINT_2_INT(*maxoutdegree),
                                       OMPI_ARRAY_NAME_CONVERT(destinations),
                                       OMPI_IS_FORTRAN_UNWEIGHTED(destweights)
                                           ? MPI_UNWEIGHTED
                                           : OMPI_ARRAY_NAME_CONVERT(destweights));
    if (NULL != ierr)
