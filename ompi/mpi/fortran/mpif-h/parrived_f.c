...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_PARRIVED, pmpi_parrived, pmpi_parrived_, pmpi_parrived__,
                           pompi_parrived_f,
                           (MPI_Fint * request, MPI_Fint *partition, MPI_Fint *flag,
                            MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_PARRIVED, mpi_parrived, mpi_parrived_, mpi_parrived__,
                           ompi_parrived_f,
                           (MPI_Fint * request, MPI_Fint *partition, MPI_Fint *flag,
                            MPI_Fint *ierr),
                           (request, partition, flag, ierr))
...
void ompi_parrived_f(MPI_Fint *request, MPI_Fint *partition, ompi_fortran_logical_t *flag,
                     MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_Parrived(c_req, OMPI_FINT_2_INT(*partition),
