...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_GET_BYTE_OFFSET, pmpi_file_get_byte_offset,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_GET_BYTE_OFFSET, mpi_file_get_byte_offset,
                           mpi_file_get_byte_offset_, mpi_file_get_byte_offset__,
...
void ompi_file_get_byte_offset_f(MPI_Fint *fh, MPI_Offset *offset, MPI_Offset *disp, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_File c_fh;
    ...
    c_ierr = PMPI_File_get_byte_offset(c_fh, (MPI_Offset) *offset, disp);
    if (NULL != ierr)
