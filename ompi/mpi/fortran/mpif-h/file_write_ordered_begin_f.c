...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_WRITE_ORDERED_BEGIN, pmpi_file_write_ordered_begin,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_WRITE_ORDERED_BEGIN, mpi_file_write_ordered_begin,
                           mpi_file_write_ordered_begin_, mpi_file_write_ordered_begin__,
...
void ompi_file_write_ordered_begin_f(MPI_Fint *fh, char *buf, MPI_Fint *count, MPI_Fint *datatype,
                                     MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_File_write_ordered_begin(c_fh, OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count),
                                           c_type);
    if (NULL != ierr)
