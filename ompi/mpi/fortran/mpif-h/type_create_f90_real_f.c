...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_TYPE_CREATE_F90_REAL, pmpi_type_create_f90_real,
                           pmpi_type_create_f90_real_, pmpi_type_create_f90_real__,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_TYPE_CREATE_F90_REAL, mpi_type_create_f90_real,
...
void ompi_type_create_f90_real_f(MPI_Fint *p, MPI_Fint *r, MPI_Fint *newtype, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Datatype c_new;

    c_ierr = PMPI_Type_create_f90_real(OMPI_FINT_2_INT(*p), OMPI_FINT_2_INT(*r), &c_new);
    if (NULL != ierr)
