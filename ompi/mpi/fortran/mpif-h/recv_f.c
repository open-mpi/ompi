...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_RECV, pmpi_recv, pmpi_recv_, pmpi_recv__, pompi_recv_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_RECV, mpi_recv, mpi_recv_, mpi_recv__, ompi_recv_f,
                           (char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *source,
                            MPI_Fint *tag, MPI_Fint *comm, MPI_Fint *status, MPI_Fint *ierr),
                           (buf, count, datatype, source, tag, comm, status, ierr))
#    else
...
void ompi_recv_f(char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *source, MPI_Fint *tag,
                 MPI_Fint *comm, MPI_Fint *status, MPI_Fint *ierr)
{
    ...
    /* Call the C function */
    c_ierr = PMPI_Recv(OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count), c_type,
                       OMPI_FINT_2_INT(*source), OMPI_FINT_2_INT(*tag), c_comm, c_status);
    if (NULL != ierr)
