...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_TYPE_CREATE_F90_COMPLEX, pmpi_type_create_f90_complex,
                           pmpi_type_create_f90_complex_, pmpi_type_create_f90_complex__,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_TYPE_CREATE_F90_COMPLEX, mpi_type_create_f90_complex,
...
void ompi_type_create_f90_complex_f(MPI_Fint *p, MPI_Fint *r, MPI_Fint *newtype, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Datatype c_newtype;

    c_ierr = PMPI_Type_create_f90_complex(OMPI_FINT_2_INT(*p), OMPI_FINT_2_INT(*r), &c_newtype);
    if (NULL != ierr)
