...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_ERROR_STRING, pmpi_error_string, pmpi_error_string_,
                           pmpi_error_string__, pompi_error_string_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_ERROR_STRING, mpi_error_string, mpi_error_string_,
                           mpi_error_string__, ompi_error_string_f,
                           (MPI_Fint * errorcode, char *string, MPI_Fint *resultlen, MPI_Fint *ierr,
                            int string_len),
                           (errorcode, string, resultlen, ierr, string_len))
#    else
...
void ompi_error_string_f(MPI_Fint *errorcode, char *string, MPI_Fint *resultlen, MPI_Fint *ierr,
                         int string_len)
{
    int c_ierr, ret;
    ...
    c_ierr = PMPI_Error_string(OMPI_FINT_2_INT(*errorcode), c_string,
    ...
    if (MPI_SUCCESS == c_ierr) {
        ...
        if (OMPI_SUCCESS != (ret = ompi_fortran_string_c2f(c_string, string, string_len))) {
