...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_COMM_SET_ATTR, pmpi_comm_set_attr, pmpi_comm_set_attr_,
                           pmpi_comm_set_attr__, pompi_comm_set_attr_f,
                           (MPI_Fint * comm, MPI_Fint *comm_keyval, MPI_Aint *attribute_val,
                            MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_COMM_SET_ATTR, mpi_comm_set_attr, mpi_comm_set_attr_,
                           mpi_comm_set_attr__, ompi_comm_set_attr_f,
                           (MPI_Fint * comm, MPI_Fint *comm_keyval, MPI_Aint *attribute_val,
                            MPI_Fint *ierr),
                           (comm, comm_keyval, attribute_val, ierr))
#    else
...
void ompi_comm_set_attr_f(MPI_Fint *comm, MPI_Fint *comm_keyval, MPI_Aint *attribute_val,
                          MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = ompi_attr_set_aint(COMM_ATTR, c_comm, &c_comm->c_keyhash,
                                OMPI_FINT_2_INT(*comm_keyval), *attribute_val, false);
    if (NULL != ierr)
