...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_SEND, pmpi_send, pmpi_send_, pmpi_send__, pompi_send_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_SEND, mpi_send, mpi_send_, mpi_send__, ompi_send_f,
                           (char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *dest,
                            MPI_Fint *tag, MPI_Fint *comm, MPI_Fint *ierr),
                           (buf, count, datatype, dest, tag, comm, ierr))
#    else
...
void ompi_send_f(char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *dest, MPI_Fint *tag,
                 MPI_Fint *comm, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Send(OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count), c_type,
