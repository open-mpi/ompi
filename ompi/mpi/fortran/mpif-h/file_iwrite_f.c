...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_IWRITE, pmpi_file_iwrite, pmpi_file_iwrite_,
                           pmpi_file_iwrite__, pompi_file_iwrite_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_IWRITE, mpi_file_iwrite, mpi_file_iwrite_, mpi_file_iwrite__,
                           ompi_file_iwrite_f,
                           (MPI_Fint * fh, char *buf, MPI_Fint *count, MPI_Fint *datatype,
                            MPI_Fint *request, MPI_Fint *ierr),
                           (fh, buf, count, datatype, request, ierr))
...
void ompi_file_iwrite_f(MPI_Fint *fh, char *buf, MPI_Fint *count, MPI_Fint *datatype,
                        MPI_Fint *request, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_File_iwrite(c_fh, OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count), c_type,
                              &c_request);
    if (NULL != ierr)
