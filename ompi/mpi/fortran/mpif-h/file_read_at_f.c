...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_READ_AT, pmpi_file_read_at, pmpi_file_read_at_,
                           pmpi_file_read_at__, pompi_file_read_at_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_READ_AT, mpi_file_read_at, mpi_file_read_at_,
                           mpi_file_read_at__, ompi_file_read_at_f,
                           (MPI_Fint * fh, MPI_Offset *offset, char *buf, MPI_Fint *count,
                            MPI_Fint *datatype, MPI_Fint *status, MPI_Fint *ierr),
                           (fh, offset, buf, count, datatype, status, ierr))
#    else
                         ...
                         MPI_Fint *datatype, MPI_Fint *status, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_File_read_at(c_fh, (MPI_Offset) *offset, buf, OMPI_FINT_2_INT(*count), c_type,
                               c_status);
    if (NULL != ierr)
