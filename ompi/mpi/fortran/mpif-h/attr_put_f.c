...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_ATTR_PUT, pmpi_attr_put, pmpi_attr_put_, pmpi_attr_put__,
                           pompi_attr_put_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_ATTR_PUT, mpi_attr_put, mpi_attr_put_, mpi_attr_put__,
                           ompi_attr_put_f,
                           (MPI_Fint * comm, MPI_Fint *keyval, MPI_Fint *attribute_val,
                            MPI_Fint *ierr),
                           (comm, keyval, attribute_val, ierr))
...
void ompi_attr_put_f(MPI_Fint *comm, MPI_Fint *keyval, MPI_Fint *attribute_val, MPI_Fint *ierr)
{
    int c_err;
    ...
    c_err = ompi_attr_set_fint(COMM_ATTR, c_comm, &c_comm->c_keyhash, OMPI_FINT_2_INT(*keyval),
                               *attribute_val, false);
    if (NULL != ierr)
