...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_TYPE_MATCH_SIZE, pmpi_type_match_size, pmpi_type_match_size_,
                           pmpi_type_match_size__, pompi_type_match_size_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_TYPE_MATCH_SIZE, mpi_type_match_size, mpi_type_match_size_,
                           mpi_type_match_size__, ompi_type_match_size_f,
                           (MPI_Fint * typeclass, MPI_Fint *size, MPI_Fint *type, MPI_Fint *ierr),
                           (typeclass, size, type, ierr))
#    else
...
void ompi_type_match_size_f(MPI_Fint *typeclass, MPI_Fint *size, MPI_Fint *type, MPI_Fint *ierr)
{
    ...
    case MPI_TYPECLASS_REAL:
        c_type = (MPI_Datatype) ompi_datatype_match_size(c_size, OMPI_DATATYPE_FLAG_DATA_FLOAT,
                                                         OMPI_DATATYPE_FLAG_DATA_FORTRAN);
        break;
    case MPI_TYPECLASS_INTEGER:
        c_type = (MPI_Datatype) ompi_datatype_match_size(c_size, OMPI_DATATYPE_FLAG_DATA_INT,
                                                         OMPI_DATATYPE_FLAG_DATA_FORTRAN);
        break;
    case MPI_TYPECLASS_COMPLEX:
        c_type = (MPI_Datatype) ompi_datatype_match_size(c_size, OMPI_DATATYPE_FLAG_DATA_COMPLEX,
                                                         OMPI_DATATYPE_FLAG_DATA_FORTRAN);
        break;
    default:
