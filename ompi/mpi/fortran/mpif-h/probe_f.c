...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_PROBE, pmpi_probe, pmpi_probe_, pmpi_probe__, pompi_probe_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_PROBE, mpi_probe, mpi_probe_, mpi_probe__, ompi_probe_f,
                           (MPI_Fint * source, MPI_Fint *tag, MPI_Fint *comm, MPI_Fint *status,
                            MPI_Fint *ierr),
                           (source, tag, comm, status, ierr))
#    else
...
void ompi_probe_f(MPI_Fint *source, MPI_Fint *tag, MPI_Fint *comm, MPI_Fint *status, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Probe(OMPI_FINT_2_INT(*source), OMPI_FINT_2_INT(*tag), c_comm, c_status);
    if (NULL != ierr)
