...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_IMPROBE, pmpi_improbe, pmpi_improbe_, pmpi_improbe__,
                           pompi_improbe_f,
                           (MPI_Fint * source, MPI_Fint *tag, MPI_Fint *comm,
                            ompi_fortran_logical_t *flag, MPI_Fint *message, MPI_Fint *status,
                            MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_IMPROBE, mpi_improbe, mpi_improbe_, mpi_improbe__, ompi_improbe_f,
                           (MPI_Fint * source, MPI_Fint *tag, MPI_Fint *comm,
                            ompi_fortran_logical_t *flag, MPI_Fint *message, MPI_Fint *status,
                            MPI_Fint *ierr),
                           (source, tag, comm, flag, message, status, ierr))
...
void ompi_improbe_f(MPI_Fint *source, MPI_Fint *tag, MPI_Fint *comm, ompi_fortran_logical_t *flag,
                    MPI_Fint *message, MPI_Fint *status, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Improbe(OMPI_FINT_2_INT(*source), OMPI_FINT_2_INT(*tag), c_comm,
                          OMPI_LOGICAL_SINGLE_NAME_CONVERT(flag), &c_message, c_status);
    if (NULL != ierr)
