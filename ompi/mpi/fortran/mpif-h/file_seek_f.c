...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_SEEK, pmpi_file_seek, pmpi_file_seek_, pmpi_file_seek__,
                           pompi_file_seek_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_SEEK, mpi_file_seek, mpi_file_seek_, mpi_file_seek__,
                           ompi_file_seek_f,
                           (MPI_Fint * fh, MPI_Offset *offset, MPI_Fint *whence, MPI_Fint *ierr),
                           (fh, offset, whence, ierr))
#    else
...
void ompi_file_seek_f(MPI_Fint *fh, MPI_Offset *offset, MPI_Fint *whence, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_File c_fh = PMPI_File_f2c(*fh);

    c_ierr = PMPI_File_seek(c_fh, (MPI_Offset) *offset, OMPI_FINT_2_INT(*whence));
    if (NULL != ierr)
