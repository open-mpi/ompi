...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_TYPE_SET_NAME, pmpi_type_set_name, pmpi_type_set_name_,
                           pmpi_type_set_name__, pompi_type_set_name_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_TYPE_SET_NAME, mpi_type_set_name, mpi_type_set_name_,
                           mpi_type_set_name__, ompi_type_set_name_f,
                           (MPI_Fint * type, char *type_name, MPI_Fint *ierr, int name_len),
                           (type, type_name, ierr, name_len))
#    else
...
void ompi_type_set_name_f(MPI_Fint *type, char *type_name, MPI_Fint *ierr, int name_len)
{
    int ret, c_ierr;
    char *c_name;
    MPI_Datatype c_type;
    ...
    if (OMPI_SUCCESS != (ret = ompi_fortran_string_f2c(type_name, name_len, &c_name))) {
        c_ierr = OMPI_ERRHANDLER_NOHANDLE_INVOKE(ret, "MPI_TYPE_SET_NAME");
        if (NULL != ierr)
