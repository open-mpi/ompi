...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_TEST, pmpi_test, pmpi_test_, pmpi_test__, pompi_test_f,
                           (MPI_Fint * request, ompi_fortran_logical_t *flag, MPI_Fint *status,
                            MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_TEST, mpi_test, mpi_test_, mpi_test__, ompi_test_f,
                           (MPI_Fint * request, ompi_fortran_logical_t *flag, MPI_Fint *status,
                            MPI_Fint *ierr),
                           (request, flag, status, ierr))
#    else
...
void ompi_test_f(MPI_Fint *request, ompi_fortran_logical_t *flag, MPI_Fint *status, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_Test(&c_req, OMPI_LOGICAL_SINGLE_NAME_CONVERT(flag), &c_status);
    if (NULL != ierr)
