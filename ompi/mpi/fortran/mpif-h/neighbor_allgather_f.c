...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_NEIGHBOR_ALLGATHER, pmpi_neighbor_allgather,
                           pmpi_neighbor_allgather_, pmpi_neighbor_allgather__,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_NEIGHBOR_ALLGATHER, mpi_neighbor_allgather, mpi_neighbor_allgather_,
                           mpi_neighbor_allgather__, ompi_neighbor_allgather_f,
                           (char *sendbuf, MPI_Fint *sendcount, MPI_Fint *sendtype, char *recvbuf,
                            MPI_Fint *recvcount, MPI_Fint *recvtype, MPI_Fint *comm,
                            MPI_Fint *ierr),
                           (sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, ierr))
                               ...
                               MPI_Fint *comm, MPI_Fint *ierr)
{
    ...
    ierr_c = PMPI_Neighbor_allgather(sendbuf, OMPI_FINT_2_INT(*sendcount), c_sendtype, recvbuf,
