...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_GRAPH_GET, pmpi_graph_get, pmpi_graph_get_, pmpi_graph_get__,
                           pompi_graph_get_f,
                           (MPI_Fint * comm, MPI_Fint *maxindex, MPI_Fint *maxedges, MPI_Fint *indx,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_GRAPH_GET, mpi_graph_get, mpi_graph_get_, mpi_graph_get__,
                           ompi_graph_get_f,
                           (MPI_Fint * comm, MPI_Fint *maxindex, MPI_Fint *maxedges, MPI_Fint *indx,
                            MPI_Fint *edges, MPI_Fint *ierr),
                           (comm, maxindex, maxedges, indx, edges, ierr))
...
void ompi_graph_get_f(MPI_Fint *comm, MPI_Fint *maxindex, MPI_Fint *maxedges, MPI_Fint *indx,
                      MPI_Fint *edges, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Comm c_comm;
    ...
    c_ierr = PMPI_Graph_get(c_comm, OMPI_FINT_2_INT(*maxindex), OMPI_FINT_2_INT(*maxedges),
                            OMPI_ARRAY_NAME_CONVERT(indx), OMPI_ARRAY_NAME_CONVERT(edges));
    if (NULL != ierr)
