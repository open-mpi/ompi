...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_COMM_CREATE_GROUP, pmpi_comm_create_group, pmpi_comm_create_group_,
                           pmpi_comm_create_group__, pompi_comm_create_group_f,
                           (MPI_Fint * comm, MPI_Fint *group, MPI_Fint *tag, MPI_Fint *newcomm,
                            MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_COMM_CREATE_GROUP, mpi_comm_create_group, mpi_comm_create_group_,
                           mpi_comm_create_group__, ompi_comm_create_group_f,
                           (MPI_Fint * comm, MPI_Fint *group, MPI_Fint *tag, MPI_Fint *newcomm,
                            MPI_Fint *ierr),
                           (comm, group, tag, newcomm, ierr))
#    else
...
void ompi_comm_create_group_f(MPI_Fint *comm, MPI_Fint *group, MPI_Fint *tag, MPI_Fint *newcomm,
                              MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Comm c_newcomm;
