...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_PREADY_LIST, pmpi_pready_list, pmpi_pready_list_,
                           pmpi_pready_list__, pompi_pready_list_f,
                           (MPI_Fint * length, MPI_Fint *partitions, MPI_Fint *request,
                            MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_PREADY_LIST, mpi_pready_list, mpi_pready_list_, mpi_pready_list__,
                           ompi_pready_list_f,
                           (MPI_Fint * length, MPI_Fint *partitions, MPI_Fint *request,
                            MPI_Fint *ierr),
                           (length, partitions, request, ierr))
