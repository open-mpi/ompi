...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_READ_ORDERED_END, pmpi_file_read_ordered_end,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_READ_ORDERED_END, mpi_file_read_ordered_end,
                           mpi_file_read_ordered_end_, mpi_file_read_ordered_end__,
...
void ompi_file_read_ordered_end_f(MPI_Fint *fh, char *buf, MPI_Fint *status, MPI_Fint *ierr)
{
    int c_ierr;
