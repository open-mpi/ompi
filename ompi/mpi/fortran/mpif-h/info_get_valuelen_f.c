...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_INFO_GET_VALUELEN, pmpi_info_get_valuelen, pmpi_info_get_valuelen_,
                           pmpi_info_get_valuelen__, pompi_info_get_valuelen_f,
                           (MPI_Fint * info, char *key, MPI_Fint *valuelen,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_INFO_GET_VALUELEN, mpi_info_get_valuelen, mpi_info_get_valuelen_,
                           mpi_info_get_valuelen__, ompi_info_get_valuelen_f,
                           (MPI_Fint * info, char *key, MPI_Fint *valuelen,
                            ompi_fortran_logical_t *flag, MPI_Fint *ierr, int key_len),
                           (info, key, valuelen, flag, ierr, key_len))
#    else
...
void ompi_info_get_valuelen_f(MPI_Fint *info, char *key, MPI_Fint *valuelen,
                              ompi_fortran_logical_t *flag, MPI_Fint *ierr, int key_len)
{
    int c_ierr, ret;
    MPI_Info c_info;
    ...
    c_ierr = PMPI_Info_get_valuelen(c_info, c_key, OMPI_SINGLE_NAME_CONVERT(valuelen),
