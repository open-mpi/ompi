...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_INFO_SET, pmpi_info_set, pmpi_info_set_, pmpi_info_set__,
                           pompi_info_set_f,
                           (MPI_Fint * info, char *key, char *value, MPI_Fint *ierr, int key_len,
                            int value_len),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_INFO_SET, mpi_info_set, mpi_info_set_, mpi_info_set__,
                           ompi_info_set_f,
                           (MPI_Fint * info, char *key, char *value, MPI_Fint *ierr, int key_len,
                            int value_len),
                           (info, key, value, ierr, key_len, value_len))
...
void ompi_info_set_f(MPI_Fint *info, char *key, char *value, MPI_Fint *ierr, int key_len,
                     int value_len)
{
    int ret, c_ierr;
    MPI_Info c_info;
    ...
    if (OMPI_SUCCESS != (ret = ompi_fortran_string_f2c(key, key_len, &c_key))
        || OMPI_SUCCESS != (ret = ompi_fortran_string_f2c(value, value_len, &c_value))) {
