...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_IREAD_AT, pmpi_file_iread_at, pmpi_file_iread_at_,
                           pmpi_file_iread_at__, pompi_file_iread_at_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_IREAD_AT, mpi_file_iread_at, mpi_file_iread_at_,
                           mpi_file_iread_at__, ompi_file_iread_at_f,
                           (MPI_Fint * fh, MPI_Offset *offset, char *buf, MPI_Fint *count,
                            MPI_Fint *datatype, MPI_Fint *request, MPI_Fint *ierr),
                           (fh, offset, buf, count, datatype, request, ierr))
#    else
                          ...
                          MPI_Fint *datatype, MPI_Fint *request, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_File_iread_at(c_fh, (MPI_Offset) *offset, OMPI_F2C_BOTTOM(buf),
                                OMPI_FINT_2_INT(*count), c_type, &c_request);
    if (NULL != ierr)
