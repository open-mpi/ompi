...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_WIN_CREATE, pmpi_win_create, pmpi_win_create_, pmpi_win_create__,
                           pompi_win_create_f,
                           (char *base, MPI_Aint *size, MPI_Fint *disp_unit, MPI_Fint *info,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_WIN_CREATE, mpi_win_create, mpi_win_create_, mpi_win_create__,
                           ompi_win_create_f,
                           (char *base, MPI_Aint *size, MPI_Fint *disp_unit, MPI_Fint *info,
                            MPI_Fint *comm, MPI_Fint *win, MPI_Fint *ierr),
                           (base, size, disp_unit, info, comm, win, ierr))
...
void ompi_win_create_f(char *base, MPI_Aint *size, MPI_Fint *disp_unit, MPI_Fint *info,
                       MPI_Fint *comm, MPI_Fint *win, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Win c_win;
    ...
    c_ierr = PMPI_Win_create(base, *size, OMPI_FINT_2_INT(*disp_unit), c_info, c_comm, &c_win);
    if (NULL != ierr)
