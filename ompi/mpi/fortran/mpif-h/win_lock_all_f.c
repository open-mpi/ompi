...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_WIN_LOCK_ALL, pmpi_win_lock_all, pmpi_win_lock_all_,
                           pmpi_win_lock_all__, pompi_win_lock_all_f,
                           (MPI_Fint * assert, MPI_Fint *win, MPI_Fint *ierr), (assert, win, ierr))
#    endif
#endif
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_WIN_LOCK_ALL, mpi_win_lock_all, mpi_win_lock_all_,
                           mpi_win_lock_all__, ompi_win_lock_all_f,
                           (MPI_Fint * assert, MPI_Fint *win, MPI_Fint *ierr), (assert, win, ierr))
#    else
...
void ompi_win_lock_all_f(MPI_Fint *assert, MPI_Fint *win, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Win_lock_all(OMPI_FINT_2_INT(*assert), c_win);
    if (NULL != ierr)
