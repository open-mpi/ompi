...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_PACK, pmpi_pack, pmpi_pack_, pmpi_pack__, pompi_pack_f,
                           (char *inbuf, MPI_Fint *incount, MPI_Fint *datatype, char *outbuf,
                            MPI_Fint *outsize, MPI_Fint *position, MPI_Fint *comm, MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_PACK, mpi_pack, mpi_pack_, mpi_pack__, ompi_pack_f,
                           (char *inbuf, MPI_Fint *incount, MPI_Fint *datatype, char *outbuf,
                            MPI_Fint *outsize, MPI_Fint *position, MPI_Fint *comm, MPI_Fint *ierr),
                           (inbuf, incount, datatype, outbuf, outsize, position, comm, ierr))
#    else
...
void ompi_pack_f(char *inbuf, MPI_Fint *incount, MPI_Fint *datatype, char *outbuf,
                 MPI_Fint *outsize, MPI_Fint *position, MPI_Fint *comm, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Comm c_comm;
    ...
    c_ierr = PMPI_Pack(OMPI_F2C_BOTTOM(inbuf), OMPI_FINT_2_INT(*incount), c_type, outbuf,
                       OMPI_FINT_2_INT(*outsize), OMPI_SINGLE_NAME_CONVERT(position), c_comm);
    if (NULL != ierr)
