...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_GET_VIEW, pmpi_file_get_view, pmpi_file_get_view_,
                           pmpi_file_get_view__, pompi_file_get_view_f,
...
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_GET_VIEW, mpi_file_get_view, mpi_file_get_view_,
                           mpi_file_get_view__, ompi_file_get_view_f,
                           (MPI_Fint * fh, MPI_Offset *disp, MPI_Fint *etype, MPI_Fint *filetype,
                            char *datarep, MPI_Fint *ierr, int datarep_len),
                           (fh, disp, etype, filetype, datarep, ierr, datarep_len))
#    else
...
void ompi_file_get_view_f(MPI_Fint *fh, MPI_Offset *disp, MPI_Fint *etype, MPI_Fint *filetype,
                          char *datarep, MPI_Fint *ierr, int datarep_len)
{
    int c_ierr;
    ...
    c_ierr = PMPI_File_get_view(c_fh, &c_disp, &c_etype, &c_filetype, c_datarep);
    if (NULL != ierr)
