...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_IREAD_ALL, pmpi_file_iread_all, pmpi_file_iread_all_,
                           pmpi_file_iread_all__, pompi_file_iread_all_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_IREAD_ALL, mpi_file_iread_all, mpi_file_iread_all_,
                           mpi_file_iread_all__, ompi_file_iread_all_f,
                           (MPI_Fint * fh, char *buf, MPI_Fint *count, MPI_Fint *datatype,
                            MPI_Fint *request, MPI_Fint *ierr),
                           (fh, buf, count, datatype, request, ierr))
#    else
...
void ompi_file_iread_all_f(MPI_Fint *fh, char *buf, MPI_Fint *count, MPI_Fint *datatype,
                           MPI_Fint *request, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_File_iread_all(c_fh, OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count), c_type,
                                 &c_request);
    if (NULL != ierr)
