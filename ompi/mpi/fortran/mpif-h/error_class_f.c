...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_ERROR_CLASS, pmpi_error_class, pmpi_error_class_,
                           pmpi_error_class__, pompi_error_class_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_ERROR_CLASS, mpi_error_class, mpi_error_class_, mpi_error_class__,
                           ompi_error_class_f,
                           (MPI_Fint * errorcode, MPI_Fint *errorclass, MPI_Fint *ierr),
                           (errorcode, errorclass, ierr))
#    else
...
void ompi_error_class_f(MPI_Fint *errorcode, MPI_Fint *errorclass, MPI_Fint *ierr)
{
    int c_ierr;
    OMPI_SINGLE_NAME_DECL(errorclass);

    c_ierr = PMPI_Error_class(OMPI_FINT_2_INT(*errorcode), OMPI_SINGLE_NAME_CONVERT(errorclass));
    if (NULL != ierr)
