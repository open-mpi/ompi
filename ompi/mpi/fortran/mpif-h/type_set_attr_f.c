...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_TYPE_SET_ATTR, pmpi_type_set_attr, pmpi_type_set_attr_,
                           pmpi_type_set_attr__, pompi_type_set_attr_f,
                           (MPI_Fint * type, MPI_Fint *type_keyval, MPI_Aint *attr_val,
                            MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_TYPE_SET_ATTR, mpi_type_set_attr, mpi_type_set_attr_,
                           mpi_type_set_attr__, ompi_type_set_attr_f,
                           (MPI_Fint * type, MPI_Fint *type_keyval, MPI_Aint *attr_val,
                            MPI_Fint *ierr),
                           (type, type_keyval, attr_val, ierr))
#    else
...
void ompi_type_set_attr_f(MPI_Fint *type, MPI_Fint *type_keyval, MPI_Aint *attribute_val,
                          MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = ompi_attr_set_aint(TYPE_ATTR, c_type, &c_type->d_keyhash,
                                OMPI_FINT_2_INT(*type_keyval), *attribute_val, false);
    if (NULL != ierr)
