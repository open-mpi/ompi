...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_GET_COUNT, pmpi_get_count, pmpi_get_count_, pmpi_get_count__,
                           pompi_get_count_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_GET_COUNT, mpi_get_count, mpi_get_count_, mpi_get_count__,
                           ompi_get_count_f,
                           (MPI_Fint * status, MPI_Fint *datatype, MPI_Fint *count, MPI_Fint *ierr),
                           (status, datatype, count, ierr))
#    else
    ...
    if (OMPI_IS_FORTRAN_STATUS_IGNORE(status)) {
        *count = OMPI_INT_2_FINT(0);
        ...
        if (MPI_SUCCESS == c_ierr) {
            c_ierr = PMPI_Get_count(&c_status, c_type, OMPI_SINGLE_NAME_CONVERT(count));
