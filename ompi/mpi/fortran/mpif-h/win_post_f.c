...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_WIN_POST, pmpi_win_post, pmpi_win_post_, pmpi_win_post__,
                           pompi_win_post_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_WIN_POST, mpi_win_post, mpi_win_post_, mpi_win_post__,
                           ompi_win_post_f,
                           (MPI_Fint * group, MPI_Fint *assert, MPI_Fint *win, MPI_Fint *ierr),
                           (group, assert, win, ierr))
#    else
...
void ompi_win_post_f(MPI_Fint *group, MPI_Fint *assert, MPI_Fint *win, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Win c_win = PMPI_Win_f2c(*win);
    MPI_Group c_grp = PMPI_Group_f2c(*group);

    c_ierr = PMPI_Win_post(c_grp, OMPI_FINT_2_INT(*assert), c_win);
    if (NULL != ierr)
