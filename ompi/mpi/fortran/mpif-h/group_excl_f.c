...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_GROUP_EXCL, pmpi_group_excl, pmpi_group_excl_, pmpi_group_excl__,
                           pompi_group_excl_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_GROUP_EXCL, mpi_group_excl, mpi_group_excl_, mpi_group_excl__,
                           ompi_group_excl_f,
                           (MPI_Fint * group, MPI_Fint *n, MPI_Fint *ranks, MPI_Fint *newgroup,
                            MPI_Fint *ierr),
                           (group, n, ranks, newgroup, ierr))
...
void ompi_group_excl_f(MPI_Fint *group, MPI_Fint *n, MPI_Fint *ranks, MPI_Fint *newgroup,
                       MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_Group_excl(c_group, OMPI_FINT_2_INT(*n), OMPI_ARRAY_NAME_CONVERT(ranks),
                             &c_newgroup);
    if (NULL != ierr)
