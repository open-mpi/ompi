...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_PREADY_RANGE, pmpi_pready_range, pmpi_pready_range_,
                           pmpi_pready_range__, pompi_pready_range_f,
                           (MPI_Fint * partition_low, MPI_Fint *partition_high, MPI_Fint *request,
                            MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_PREADY_RANGE, mpi_pready_range, mpi_pready_range_,
                           mpi_pready_range__, ompi_pready_range_f,
                           (MPI_Fint * partition_low, MPI_Fint *partition_high, MPI_Fint *request,
                            MPI_Fint *ierr),
                           (partition_low, partition_high, request, ierr))
#    else
...
void ompi_pready_range_f(MPI_Fint *partition_low, MPI_Fint *partition_high, MPI_Fint *request,
                         MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Request c_req = PMPI_Request_f2c(*request);

    c_ierr = PMPI_Pready_range(OMPI_FINT_2_INT(*partition_low), OMPI_FINT_2_INT(*partition_high),
                               c_req);
    if (NULL != ierr)
