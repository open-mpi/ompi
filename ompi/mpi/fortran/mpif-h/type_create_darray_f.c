...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_TYPE_CREATE_DARRAY, pmpi_type_create_darray,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_TYPE_CREATE_DARRAY, mpi_type_create_darray, mpi_type_create_darray_,
                           mpi_type_create_darray__, ompi_type_create_darray_f,
                           (MPI_Fint * size, MPI_Fint *rank, MPI_Fint *ndims, MPI_Fint *gsize_array,
                            MPI_Fint *distrib_array, MPI_Fint *darg_array, MPI_Fint *psize_array,
                            MPI_Fint *order, MPI_Fint *oldtype, MPI_Fint *newtype, MPI_Fint *ierr),
                           (size, rank, ndims, gsize_array, distrib_array, darg_array, psize_array,
...
void ompi_type_create_darray_f(MPI_Fint *size, MPI_Fint *rank, MPI_Fint *ndims,
                               MPI_Fint *gsize_array, MPI_Fint *distrib_array, MPI_Fint *darg_array,
                               MPI_Fint *psize_array, MPI_Fint *order, MPI_Fint *oldtype,
                               MPI_Fint *newtype, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Type_create_darray(OMPI_FINT_2_INT(*size), OMPI_FINT_2_INT(*rank),
                                     ...
                                     OMPI_ARRAY_NAME_CONVERT(psize_array), OMPI_FINT_2_INT(*order),
                                     c_old, &c_new);
    if (NULL != ierr)
