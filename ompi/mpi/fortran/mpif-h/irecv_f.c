...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_IRECV, pmpi_irecv, pmpi_irecv_, pmpi_irecv__, pompi_irecv_f,
                           (char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *source,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_IRECV, mpi_irecv, mpi_irecv_, mpi_irecv__, ompi_irecv_f,
                           (char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *source,
                            MPI_Fint *tag, MPI_Fint *comm, MPI_Fint *request, MPI_Fint *ierr),
                           (buf, count, datatype, source, tag, comm, request, ierr))
#    else
...
void ompi_irecv_f(char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *source, MPI_Fint *tag,
                  MPI_Fint *comm, MPI_Fint *request, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Irecv(OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count), c_type,
                        OMPI_FINT_2_INT(*source), OMPI_FINT_2_INT(*tag), c_comm, &c_req);
    if (NULL != ierr)
