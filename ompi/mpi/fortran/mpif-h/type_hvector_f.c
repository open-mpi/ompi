...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_TYPE_HVECTOR, pmpi_type_hvector, pmpi_type_hvector_,
                           pmpi_type_hvector__, pompi_type_hvector_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_TYPE_HVECTOR, mpi_type_hvector, mpi_type_hvector_,
                           mpi_type_hvector__, ompi_type_hvector_f,
                           (MPI_Fint * count, MPI_Fint *blocklength, MPI_Fint *stride,
                            MPI_Fint *oldtype, MPI_Fint *newtype, MPI_Fint *ierr),
                           (count, blocklength, stride, oldtype, newtype, ierr))
#    else
...
void ompi_type_hvector_f(MPI_Fint *count, MPI_Fint *blocklength, MPI_Fint *stride,
                         MPI_Fint *oldtype, MPI_Fint *newtype, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_Type_hvector(OMPI_FINT_2_INT(*count), OMPI_FINT_2_INT(*blocklength),
                               (MPI_Aint) *stride, c_oldtype, &c_newtype);
    if (NULL != ierr)
