...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_ADD_ERROR_STRING, pmpi_add_error_string, pmpi_add_error_string_,
                           pmpi_add_error_string__, pompi_add_error_string_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_ADD_ERROR_STRING, mpi_add_error_string, mpi_add_error_string_,
                           mpi_add_error_string__, ompi_add_error_string_f,
                           (MPI_Fint * errorcode, char *string, MPI_Fint *ierr, int l),
                           (errorcode, string, ierr, l))
#    else
...
void ompi_add_error_string_f(MPI_Fint *errorcode, char *string, MPI_Fint *ierr, int len)
{
    char *c_string;
    int ierr_c;

    if (len > MPI_MAX_ERROR_STRING) {
        ierr_c = OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_ARG, "MPI_ADD_ERROR_STRING");
        if (NULL != ierr)
