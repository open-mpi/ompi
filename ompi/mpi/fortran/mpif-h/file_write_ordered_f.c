...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_WRITE_ORDERED, pmpi_file_write_ordered,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_WRITE_ORDERED, mpi_file_write_ordered, mpi_file_write_ordered_,
                           mpi_file_write_ordered__, ompi_file_write_ordered_f,
                           (MPI_Fint * fh, char *buf, MPI_Fint *count, MPI_Fint *datatype,
                            MPI_Fint *status, MPI_Fint *ierr),
                           (fh, buf, count, datatype, status, ierr))
#    else
...
void ompi_file_write_ordered_f(MPI_Fint *fh, char *buf, MPI_Fint *count, MPI_Fint *datatype,
                               MPI_Fint *status, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_File_write_ordered(c_fh, OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count), c_type,
                                     c_status);
    if (NULL != ierr)
