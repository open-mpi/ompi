...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_INIT_THREAD, pmpi_init_thread, pmpi_init_thread_,
                           pmpi_init_thread__, pompi_init_thread_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_INIT_THREAD, mpi_init_thread, mpi_init_thread_, mpi_init_thread__,
                           ompi_init_thread_f,
                           (MPI_Fint * required, MPI_Fint *provided, MPI_Fint *ierr),
                           (required, provided, ierr))
#    else
...
void ompi_init_thread_f(MPI_Fint *required, MPI_Fint *provided, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Init_thread(&argc, &argv, OMPI_FINT_2_INT(*required),
