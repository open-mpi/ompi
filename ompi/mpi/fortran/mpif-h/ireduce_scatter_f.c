...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_IREDUCE_SCATTER, pmpi_ireduce_scatter, pmpi_ireduce_scatter_,
                           pmpi_ireduce_scatter__, pompi_ireduce_scatter_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_IREDUCE_SCATTER, mpi_ireduce_scatter, mpi_ireduce_scatter_,
                           mpi_ireduce_scatter__, ompi_ireduce_scatter_f,
                           (char *sendbuf, char *recvbuf, MPI_Fint *recvcounts, MPI_Fint *datatype,
                            MPI_Fint *op, MPI_Fint *comm, MPI_Fint *request, MPI_Fint *ierr),
                           (sendbuf, recvbuf, recvcounts, datatype, op, comm, request, ierr))
#    else
...
void ompi_ireduce_scatter_f(char *sendbuf, char *recvbuf, MPI_Fint *recvcounts, MPI_Fint *datatype,
                            MPI_Fint *op, MPI_Fint *comm, MPI_Fint *request, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Comm c_comm;
    ...
    c_ierr = PMPI_Ireduce_scatter(sendbuf, recvbuf, OMPI_ARRAY_NAME_CONVERT(recvcounts), c_type,
                                  c_op, c_comm, &c_request);
    if (NULL != ierr)
