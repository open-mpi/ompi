...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_DELETE, pmpi_file_delete, pmpi_file_delete_,
                           pmpi_file_delete__, pompi_file_delete_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_DELETE, mpi_file_delete, mpi_file_delete_, mpi_file_delete__,
                           ompi_file_delete_f,
                           (char *filename, MPI_Fint *info, MPI_Fint *ierr, int filename_len),
                           (filename, info, ierr, filename_len))
#    else
...
void ompi_file_delete_f(char *filename, MPI_Fint *info, MPI_Fint *ierr, int filename_len)
{
    ...
    if (OMPI_SUCCESS != (ret = ompi_fortran_string_f2c(filename, filename_len, &c_filename))) {
