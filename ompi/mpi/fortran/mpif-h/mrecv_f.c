...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_MRECV, pmpi_mrecv, pmpi_mrecv_, pmpi_mrecv__, pompi_mrecv_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_MRECV, mpi_mrecv, mpi_mrecv_, mpi_mrecv__, ompi_mrecv_f,
                           (char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *message,
                            MPI_Fint *status, MPI_Fint *ierr),
                           (buf, count, datatype, message, status, ierr))
#    else
...
void ompi_mrecv_f(char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *message,
                  MPI_Fint *status, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    /* Call the C function */
    c_ierr = PMPI_Mrecv(OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count), c_type, &c_message,
                        c_status);
    if (NULL != ierr)
