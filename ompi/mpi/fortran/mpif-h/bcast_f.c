...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_BCAST, pmpi_bcast, pmpi_bcast_, pmpi_bcast__, pompi_bcast_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_BCAST, mpi_bcast, mpi_bcast_, mpi_bcast__, ompi_bcast_f,
                           (char *buffer, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *root,
                            MPI_Fint *comm, MPI_Fint *ierr),
                           (buffer, count, datatype, root, comm, ierr))
#    else
...
void ompi_bcast_f(char *buffer, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *root, MPI_Fint *comm,
                  MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Comm c_comm;
    ...
    c_ierr = PMPI_Bcast(OMPI_F2C_BOTTOM(buffer), OMPI_FINT_2_INT(*count), c_type,
