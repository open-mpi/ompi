...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_REDUCE_SCATTER_BLOCK, pmpi_reduce_scatter_block,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_REDUCE_SCATTER_BLOCK, mpi_reduce_scatter_block,
                           mpi_reduce_scatter_block_, mpi_reduce_scatter_block__,
...
void ompi_reduce_scatter_block_f(char *sendbuf, char *recvbuf, MPI_Fint *recvcount,
                                 MPI_Fint *datatype, MPI_Fint *op, MPI_Fint *comm, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Reduce_scatter_block(sendbuf, recvbuf, OMPI_FINT_2_INT(*recvcount), c_type, c_op,
                                       c_comm);
    if (NULL != ierr)
