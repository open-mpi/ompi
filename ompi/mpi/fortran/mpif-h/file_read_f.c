...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_READ, pmpi_file_read, pmpi_file_read_, pmpi_file_read__,
                           pompi_file_read_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_READ, mpi_file_read, mpi_file_read_, mpi_file_read__,
                           ompi_file_read_f,
                           (MPI_Fint * fh, char *buf, MPI_Fint *count, MPI_Fint *datatype,
                            MPI_Fint *status, MPI_Fint *ierr),
                           (fh, buf, count, datatype, status, ierr))
...
void ompi_file_read_f(MPI_Fint *fh, char *buf, MPI_Fint *count, MPI_Fint *datatype,
                      MPI_Fint *status, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_File_read(c_fh, OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count), c_type, c_status);
    if (NULL != ierr)
