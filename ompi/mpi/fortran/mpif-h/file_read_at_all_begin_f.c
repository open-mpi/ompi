...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_READ_AT_ALL_BEGIN, pmpi_file_read_at_all_begin,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_READ_AT_ALL_BEGIN, mpi_file_read_at_all_begin,
                           mpi_file_read_at_all_begin_, mpi_file_read_at_all_begin__,
...
void ompi_file_read_at_all_begin_f(MPI_Fint *fh, MPI_Offset *offset, char *buf, MPI_Fint *count,
                                   MPI_Fint *datatype, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_File_read_at_all_begin(c_fh, (MPI_Offset) *offset, OMPI_F2C_BOTTOM(buf),
                                         OMPI_FINT_2_INT(*count), c_type);
    if (NULL != ierr)
