...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_IMRECV, pmpi_imrecv, pmpi_imrecv_, pmpi_imrecv__, pompi_imrecv_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_IMRECV, mpi_imrecv, mpi_imrecv_, mpi_imrecv__, ompi_imrecv_f,
                           (char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *message,
                            MPI_Fint *request, MPI_Fint *ierr),
                           (buf, count, datatype, message, request, ierr))
#    else
...
void ompi_imrecv_f(char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *message,
                   MPI_Fint *request, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_Imrecv(OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count), c_type, &c_message, &c_req);
    if (NULL != ierr)
