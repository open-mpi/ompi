...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_READ_SHARED, pmpi_file_read_shared, pmpi_file_read_shared_,
                           pmpi_file_read_shared__, pompi_file_read_shared_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_READ_SHARED, mpi_file_read_shared, mpi_file_read_shared_,
                           mpi_file_read_shared__, ompi_file_read_shared_f,
                           (MPI_Fint * fh, char *buf, MPI_Fint *count, MPI_Fint *datatype,
                            MPI_Fint *status, MPI_Fint *ierr),
                           (fh, buf, count, datatype, status, ierr))
#    else
...
void ompi_file_read_shared_f(MPI_Fint *fh, char *buf, MPI_Fint *count, MPI_Fint *datatype,
                             MPI_Fint *status, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_File_read_shared(c_fh, OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count), c_type,
                                   c_status);
    if (NULL != ierr)
