...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_REDUCE_SCATTER, pmpi_reduce_scatter, pmpi_reduce_scatter_,
                           pmpi_reduce_scatter__, pompi_reduce_scatter_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_REDUCE_SCATTER, mpi_reduce_scatter, mpi_reduce_scatter_,
                           mpi_reduce_scatter__, ompi_reduce_scatter_f,
                           (char *sendbuf, char *recvbuf, MPI_Fint *recvcounts, MPI_Fint *datatype,
                            MPI_Fint *op, MPI_Fint *comm, MPI_Fint *ierr),
                           (sendbuf, recvbuf, recvcounts, datatype, op, comm, ierr))
#    else
...
void ompi_reduce_scatter_f(char *sendbuf, char *recvbuf, MPI_Fint *recvcounts, MPI_Fint *datatype,
                           MPI_Fint *op, MPI_Fint *comm, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Comm c_comm;
    ...
    c_ierr = PMPI_Reduce_scatter(sendbuf, recvbuf, OMPI_ARRAY_NAME_CONVERT(recvcounts), c_type,
                                 c_op, c_comm);
    if (NULL != ierr)
