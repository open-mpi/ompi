...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_SEND_INIT, pmpi_send_init, pmpi_send_init_, pmpi_send_init__,
                           pompi_send_init_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_SEND_INIT, mpi_send_init, mpi_send_init_, mpi_send_init__,
                           ompi_send_init_f,
                           (char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *dest,
                            MPI_Fint *tag, MPI_Fint *comm, MPI_Fint *request, MPI_Fint *ierr),
                           (buf, count, datatype, dest, tag, comm, request, ierr))
...
void ompi_send_init_f(char *buf, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *dest, MPI_Fint *tag,
                      MPI_Fint *comm, MPI_Fint *request, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Send_init(OMPI_F2C_BOTTOM(buf), OMPI_FINT_2_INT(*count), c_type,
                            OMPI_FINT_2_INT(*dest), OMPI_FINT_2_INT(*tag), c_comm, &c_req);
    if (NULL != ierr)
