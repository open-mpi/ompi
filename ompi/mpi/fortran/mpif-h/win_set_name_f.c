...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_WIN_SET_NAME, pmpi_win_set_name, pmpi_win_set_name_,
                           pmpi_win_set_name__, pompi_win_set_name_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_WIN_SET_NAME, mpi_win_set_name, mpi_win_set_name_,
                           mpi_win_set_name__, ompi_win_set_name_f,
                           (MPI_Fint * win, char *win_name, MPI_Fint *ierr, int name_len),
                           (win, win_name, ierr, name_len))
#    else
...
void ompi_win_set_name_f(MPI_Fint *win, char *win_name, MPI_Fint *ierr, int name_len)
{
    int ret, c_ierr;
    char *c_name;
    MPI_Win c_win;
    ...
    if (OMPI_SUCCESS != (ret = ompi_fortran_string_f2c(win_name, name_len, &c_name))) {
        c_ierr = OMPI_ERRHANDLER_NOHANDLE_INVOKE(ret, "MPI_WIN_SET_NAME");
        if (NULL != ierr)
