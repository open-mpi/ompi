...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_GROUP_RANGE_INCL, pmpi_group_range_incl, pmpi_group_range_incl_,
                           pmpi_group_range_incl__, pompi_group_range_incl_f,
                           (MPI_Fint * group, MPI_Fint *n, MPI_Fint ranges[][3], MPI_Fint *newgroup,
                            MPI_Fint *ierr),
...
OMPI_GENERATE_F77_BINDINGS(MPI_GROUP_RANGE_INCL, mpi_group_range_incl, mpi_group_range_incl_,
                           mpi_group_range_incl__, ompi_group_range_incl_f,
                           (MPI_Fint * group, MPI_Fint *n, MPI_Fint ranges[][3], MPI_Fint *newgroup,
                            MPI_Fint *ierr),
                           (group, n, ranges, newgroup, ierr))
#    else
...
void ompi_group_range_incl_f(MPI_Fint *group, MPI_Fint *n, MPI_Fint ranges[][3], MPI_Fint *newgroup,
                             MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPI_Group_range_incl(c_group, OMPI_FINT_2_INT(*n), OMPI_ARRAY_NAME_CONVERT(ranges),
                                   &c_newgroup);
    if (NULL != ierr)
