...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_WIN_LOCK, pmpi_win_lock, pmpi_win_lock_, pmpi_win_lock__,
                           pompi_win_lock_f,
                           (MPI_Fint * lock_type, MPI_Fint *rank, MPI_Fint *assert, MPI_Fint *win,
                            MPI_Fint *ierr),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_WIN_LOCK, mpi_win_lock, mpi_win_lock_, mpi_win_lock__,
                           ompi_win_lock_f,
                           (MPI_Fint * lock_type, MPI_Fint *rank, MPI_Fint *assert, MPI_Fint *win,
                            MPI_Fint *ierr),
                           (lock_type, rank, assert, win, ierr))
...
void ompi_win_lock_f(MPI_Fint *lock_type, MPI_Fint *rank, MPI_Fint *assert, MPI_Fint *win,
                     MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Win c_win = PMPI_Win_f2c(*win);

    c_ierr = PMPI_Win_lock(OMPI_FINT_2_INT(*lock_type), OMPI_FINT_2_INT(*rank),
                           OMPI_FINT_2_INT(*assert), c_win);
    if (NULL != ierr)
