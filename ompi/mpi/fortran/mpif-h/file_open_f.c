...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_FILE_OPEN, pmpi_file_open, pmpi_file_open_, pmpi_file_open__,
                           pompi_file_open_f,
                           (MPI_Fint * comm, char *filename, MPI_Fint *amode, MPI_Fint *info,
                            MPI_Fint *fh, MPI_Fint *ierr, int name_len),
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_FILE_OPEN, mpi_file_open, mpi_file_open_, mpi_file_open__,
                           ompi_file_open_f,
                           (MPI_Fint * comm, char *filename, MPI_Fint *amode, MPI_Fint *info,
                            MPI_Fint *fh, MPI_Fint *ierr, int name_len),
                           (comm, filename, amode, info, fh, ierr, name_len))
...
void ompi_file_open_f(MPI_Fint *comm, char *filename, MPI_Fint *amode, MPI_Fint *info, MPI_Fint *fh,
                      MPI_Fint *ierr, int name_len)
{
    ...
    if (OMPI_SUCCESS != (ret = ompi_fortran_string_f2c(filename, name_len, &c_filename))) {
    ...
    c_ierr = PMPI_File_open(c_comm, c_filename, OMPI_FINT_2_INT(*amode), c_info, &c_fh);
    if (NULL != ierr)
        *ierr = OMPI_INT_2_FINT(c_ierr);
