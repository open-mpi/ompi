...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPI_CART_MAP, pmpi_cart_map, pmpi_cart_map_, pmpi_cart_map__,
                           pompi_cart_map_f,
                           (MPI_Fint * comm, MPI_Fint *ndims, MPI_Fint *dims,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPI_CART_MAP, mpi_cart_map, mpi_cart_map_, mpi_cart_map__,
                           ompi_cart_map_f,
                           (MPI_Fint * comm, MPI_Fint *ndims, MPI_Fint *dims,
                            ompi_fortran_logical_t *periods, MPI_Fint *newrank, MPI_Fint *ierr),
                           (comm, ndims, dims, periods, newrank, ierr))
                     ...
                     ompi_fortran_logical_t *periods, MPI_Fint *newrank, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPI_Cart_map(c_comm, size, OMPI_ARRAY_NAME_CONVERT(dims),
