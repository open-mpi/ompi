...
OBJ_CLASS_INSTANCE(ompi_java_buffer_t, opal_free_list_item_t, bufferConstructor, bufferDestructor);
...
static void initFreeList(void)
{
    ...
    int r = opal_free_list_init(&ompi_java_buffers, sizeof(ompi_java_buffer_t),
                                opal_cache_line_size, OBJ_CLASS(ompi_java_buffer_t),
                                0,    /* payload size */
                                0,    /* payload align */
...
{
    jclass c = (*env)->FindClass(env, className), r = (*env)->NewGlobalRef(env, c);
...
static void findClasses(JNIEnv *env)
{
    ...
    ompi_java.DistGraphNeighborsClass = findClass(env, "mpi/DistGraphNeighbors");

    ompi_java.StatusClass = findClass(env, "mpi/Status");
    ompi_java.ExceptionClass = findClass(env, "mpi/MPIException");

    ompi_java.ExceptionInit = (*env)->GetMethodID(env, ompi_java.ExceptionClass, "<init>",
                                                  "(IILjava/lang/String;)V");

    ompi_java.IntegerClass = findClass(env, "java/lang/Integer");
    ompi_java.LongClass = findClass(env, "java/lang/Long");

    ompi_java.IntegerValueOf = (*env)->GetStaticMethodID(env, ompi_java.IntegerClass, "valueOf",
                                                         "(I)Ljava/lang/Integer;");
    ompi_java.LongValueOf = (*env)->GetStaticMethodID(env, ompi_java.LongClass, "valueOf",
                                                      "(J)Ljava/lang/Long;");
}
...
JNIEXPORT jobjectArray JNICALL Java_mpi_MPI_Init_1jni(JNIEnv *env, jclass clazz, jobjectArray argv)
{
    jsize i;
    jclass string;
    jobject value;
    ...
    mca_base_var_register("ompi", "mpi", "java", "eager", "Java buffers eager size",
                          MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_5,
                          MCA_BASE_VAR_SCOPE_READONLY, &ompi_mpi_java_eager);
...
JNIEXPORT jint JNICALL Java_mpi_MPI_InitThread_1jni(JNIEnv *env, jclass clazz, jobjectArray argv,
                                                    jint required)
{
    jsize i;
    int len = (*env)->GetArrayLength(env, argv);
...
JNIEXPORT jboolean JNICALL Java_mpi_MPI_isThreadMain_1jni(JNIEnv *env, jclass clazz)
{
    int flag;
...
JNIEXPORT jobject JNICALL Java_mpi_MPI_getVersionJNI(JNIEnv *env, jclass jthis)
{
    ...
    return (*env)->NewObject(env, ompi_java.VersionClass, ompi_java.VersionInit, version,
                             subversion);
}
...
JNIEXPORT jint JNICALL Java_mpi_MPI_getProcessorName(JNIEnv *env, jclass obj, jbyteArray buf)
{
    int len;
...
JNIEXPORT void JNICALL Java_mpi_MPI_attachBuffer_1jni(JNIEnv *env, jclass jthis, jbyteArray buf)
{
...
JNIEXPORT void JNICALL Java_mpi_MPI_detachBuffer_1jni(JNIEnv *env, jclass jthis, jbyteArray buf)
{
    int size;
    jbyte *bufptr;
...
static void getArrayRegion(JNIEnv *env, jobject buf, int baseType, int offset, int length,
                           void *ptr)
{
    switch (baseType) {
    case 0:
...
static void setArrayRegion(JNIEnv *env, jobject buf, int baseType, int offset, int length,
                           void *ptr)
{
    switch (baseType) {
    case 0:
...
static void *getBuffer(JNIEnv *env, ompi_java_buffer_t **item, int size)
{
    if (size > ompi_mpi_java_eager) {
        *item = NULL;
        ...
        ompi_java_exceptionCheck(env, NULL == freeListItem ? MPI_ERR_NO_MEM : MPI_SUCCESS);
...
static void *getReadPtr(ompi_java_buffer_t **item, JNIEnv *env, jobject buf, int offset, int count,
                        MPI_Datatype type, int baseType)
{
    ...
    if (opal_datatype_is_contiguous_memory_layout(&type->super, count)) {
        ...
        int rc = opal_datatype_copy_content_same_ddt(&type->super, count, ptr, inBuf);

        ompi_java_exceptionCheck(env, rc == OPAL_SUCCESS ? OMPI_SUCCESS : OMPI_ERROR);
...
static void *getReadPtrRank(ompi_java_buffer_t **item, JNIEnv *env, jobject buf, int offset,
                            int count, int size, int rank, MPI_Datatype type, int baseType)
{
    int extent = getTypeExtent(env, type), rLen = extent * count, length = rLen * size,
        rDispl = rLen * rank, rOff = offset + rDispl;
    void *ptr = getBuffer(env, item, length);
    ...
    if (opal_datatype_is_contiguous_memory_layout(&type->super, count)) {
        ...
        int rc = opal_datatype_copy_content_same_ddt(&type->super, count, rPtr, bufPtr);
...
static void *getReadPtrvRank(ompi_java_buffer_t **item, JNIEnv *env, jobject buf, int offset,
                             int *counts, int *displs, int size, int rank, MPI_Datatype type,
                             int baseType)
{
    int extent = getTypeExtent(env, type), length = extent * getCountv(counts, displs, size);
    void *ptr = getBuffer(env, item, length);
    ...
    if (opal_datatype_is_contiguous_memory_layout(&type->super, counts[rank])) {
        ...
        int rc = opal_datatype_copy_content_same_ddt(&type->super, counts[rank], ptr, inBuf);
...
static void *getReadPtrwRank(ompi_java_buffer_t **item, JNIEnv *env, jobject buf, int *offsets,
                             int *counts, int *displs, int size, int rank, MPI_Datatype *types,
                             int *baseTypes)
{
    int extent = getTypeExtent(env, types[rank]), length = getCountv(counts, displs, size);
    void *ptr = getBuffer(env, item, length);
    ...
    if (opal_datatype_is_contiguous_memory_layout(&types[rank]->super, counts[rank])) {
        ...
        int rc = opal_datatype_copy_content_same_ddt(&types[rank]->super, counts[rank], ptr, inBuf);
...
static void *getReadPtrvAll(ompi_java_buffer_t **item, JNIEnv *env, jobject buf, int offset,
                            int *counts, int *displs, int size, MPI_Datatype type, int baseType)
{
    int i, extent = getTypeExtent(env, type), length = extent * getCountv(counts, displs, size);
    void *ptr = getBuffer(env, item, length);

    if (opal_datatype_is_contiguous_memory_layout(&type->super, 2)) {
        for (i = 0; i < size; i++) {
            int iOff = offset + extent * displs[i], iLen = extent * counts[i];
        ...
        for (i = 0; i < size; i++) {
            ...
            char *iBuf = iOff + (char *) bufPtr, *iPtr = iOff + (char *) ptr;

            int rc = opal_datatype_copy_content_same_ddt(&type->super, counts[i], iPtr, iBuf);
...
static void *getReadPtrwAll(ompi_java_buffer_t **item, JNIEnv *env, jobject buf, int *offsets,
                            int *counts, int *displs, int size, MPI_Datatype *types, int *baseTypes)
{
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (opal_datatype_is_contiguous_memory_layout(&types[i]->super, 2)) {
            int iOff = offsets[i] + displs[i], iLen = extent * counts[i];
            ...
            int rc = opal_datatype_copy_content_same_ddt(&types[i]->super, counts[i], iPtr, iBuf);
...
static void *getWritePtr(ompi_java_buffer_t **item, JNIEnv *env, int count, MPI_Datatype type)
{
    int extent = getTypeExtent(env, type), length = count * extent;

    return getBuffer(env, item, length);
}

static void *getWritePtrv(ompi_java_buffer_t **item, JNIEnv *env, int *counts, int *displs,
                          int size, MPI_Datatype type)
{
    int extent = getTypeExtent(env, type), count = getCountv(counts, displs, size),
        length = extent * count;

    return getBuffer(env, item, length);
}

static void *getWritePtrw(ompi_java_buffer_t **item, JNIEnv *env, int *counts, int *displs,
                          int size, MPI_Datatype *types)
{
    int length = getCountv(counts, displs, size);
                              ...
                              MPI_Datatype type, int baseType)
{
    if (buf == NULL || baseType == 0) {
        ...
        *ptr = getReadPtrRank(item, env, buf, offset, count, size, rank, type, baseType);
    }
}

void ompi_java_getReadPtrv(void **ptr, ompi_java_buffer_t **item, JNIEnv *env, jobject buf,
                           jboolean db, int offset, int *counts, int *displs, int size, int rank,
                           MPI_Datatype type, int baseType)
{
    if (buf == NULL) {
        /* Allow NULL buffers to send/recv 0 items as control messages. */
        ...
        *ptr = getReadPtrvAll(item, env, buf, offset, counts, displs, size, type, baseType);
    } else {
        *ptr = getReadPtrvRank(item, env, buf, offset, counts, displs, size, rank, type, baseType);
    }
}

void ompi_java_getReadPtrw(void **ptr, ompi_java_buffer_t **item, JNIEnv *env, jobject buf,
                           jboolean db, int *offsets, int *counts, int *displs, int size, int rank,
                           MPI_Datatype *types, int *baseTypes)
{
    ...
    if (buf == NULL) {
        ...
        *ptr = getReadPtrwAll(item, env, buf, offsets, counts, displs, size, types, baseTypes);
    } else {
        *ptr = getReadPtrwRank(item, env, buf, offsets, counts, displs, size, rank, types,
                               baseTypes);
    }
}
                               ...
                               jboolean db, int offset, int count, MPI_Datatype type, int baseType)
{
    ...
    if (opal_datatype_is_contiguous_memory_layout(&type->super, count)) {
        ...
        int rc = opal_datatype_copy_content_same_ddt(&type->super, count, inBuf, ptr);
...
void ompi_java_releaseWritePtrv(void *ptr, ompi_java_buffer_t *item, JNIEnv *env, jobject buf,
                                jboolean db, int offset, int *counts, int *displs, int size,
                                MPI_Datatype type, int baseType)
{
    if (db || !buf || !ptr)
        return;
    ...
    if (opal_datatype_is_contiguous_memory_layout(&type->super, 2)) {
        ...
        for (i = 0; i < size; i++) {
            ...
            int rc = opal_datatype_copy_content_same_ddt(&type->super, counts[i], iBuf, iPtr);
...
void ompi_java_releaseWritePtrw(void *ptr, ompi_java_buffer_t *item, JNIEnv *env, jobject buf,
                                jboolean db, int *offsets, int *counts, int *displs, int size,
                                MPI_Datatype *types, int *baseTypes)
{
    if (db || !buf || !ptr)
        return;
    ...
    for (i = 0; i < size; i++) {
        ...
        if (opal_datatype_is_contiguous_memory_layout(&types[i]->super, 2)) {
            ...
            int rc = opal_datatype_copy_content_same_ddt(&types[i]->super, counts[i], iBuf, iPtr);
...
{
    return (*env)->CallStaticObjectMethod(env, ompi_java.IntegerClass, ompi_java.IntegerValueOf, i);
}

jobject ompi_java_Long_valueOf(JNIEnv *env, jlong i)
{
    return (*env)->CallStaticObjectMethod(env, ompi_java.LongClass, ompi_java.LongValueOf, i);
}
...
jboolean ompi_java_exceptionCheck(JNIEnv *env, int rc)
{
    ...
    if (MPI_SUCCESS == rc && JNI_FALSE == jni_exception) {
        ...
        jobject mpiex = (*env)->NewObject(env, ompi_java.ExceptionClass, ompi_java.ExceptionInit,
...
void *ompi_java_attrSet(JNIEnv *env, jbyteArray jval)
{
    ...
    *((int *) cval) = length;
    ...
    (*env)->GetByteArrayRegion(env, jval, 0, length, (jbyte *) cval + sizeof(int));
...
jbyteArray ompi_java_attrGet(JNIEnv *env, void *cval)
{
    ...
    (*env)->SetByteArrayRegion(env, jval, 0, length, (jbyte *) cval + sizeof(int));
