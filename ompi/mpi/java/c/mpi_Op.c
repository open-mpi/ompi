...
{
    static MPI_Op Ops[] = {MPI_OP_NULL, MPI_MAX,    MPI_MIN,    MPI_SUM,     MPI_PROD,
...
static void opIntercept(void *invec, void *inoutvec, int *count, MPI_Datatype *datatype,
...
MPI_Op ompi_java_op_getHandle(JNIEnv *env, jobject jOp, jlong hOp, int baseType)
{
    ...
    if (op == NULL) {
        ...
        int commute = (*env)->GetBooleanField(env, jOp, ompi_java.OpCommute);
