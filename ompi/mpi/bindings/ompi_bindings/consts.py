...
JSON -- so anything they must agree on belongs here, not in either one of
them.
"""
...
# Suffix appended to a name to make its MPI Forum ABI counterpart, which
# keeps the ABI names from colliding with Open MPI's own.  Use
# util.abi_internal_name() to apply it rather than concatenating by hand.
ABI_INTERNAL_SUFFIX = '_ABI_INTERNAL'

# MPI functions and constants that Open MPI still provides (for backwards
# compatibility with its own ABI) but that the MPI Forum has deprecated or
# removed, and which are therefore absent from the MPI Forum ABI.  The ABI
# header generator prunes these from the generated header.
#
...
# functions/constants are deprecated.
...
PREDEFINED_DATATYPES = [
    ...
    'MPI_CHARACTER',
    'MPI_DOUBLE_COMPLEX',
    # Fortran MINLOC/MAXLOC reduction pair types.  Like the other
    # Fortran entries above (and unlike the optional list below), the
    # native handles always exist, even in non-Fortran builds.
    'MPI_2REAL',
...
class ConvertFuncs:
    ...
    T_SCOPE = 'ompi_convert_abi_t_scope_intern_t_scope'
