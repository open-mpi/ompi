...
class ABIConverterBuilder:
    ...
    def dump_include_file_code(self):
        ...
        # The emitted converters use malloc/free, assert, and memcpy;
        # include what the header uses so it stays self-contained
        # regardless of what the including translation unit pulled in
        # first.
        self.dump('#include <stddef.h>')
        self.dump('#include <stdint.h>')
        self.dump('#include <stdlib.h>')
        self.dump('#include <string.h>')
        self.dump('#include <assert.h>')
...
""")
        ...
        lines.append('if (count <= 0 || 0 == elsize) {')
        lines.append('/* zero-size conversions are legal (e.g. MPI_Waitall(0, ...)), so')
        lines.append(' * return a freeable non-NULL sentinel: a NULL return here reads as')
        lines.append(' * out-of-memory to the caller.  Negative counts flow through to the')
        lines.append(' * back-end, which reports MPI_ERR_COUNT. */')
        lines.append('return malloc(1);')
        lines.append('}')
...
#       self.generate_datatype_convert_fn_intern_to_abi()
        ...
        self.generate_t_scope_convert_fn()
        self.generate_t_scope_convert_fn_intern_to_abi()
