...
int ompi_mpi_register_params(void)
{
    ...
    (void) mca_base_var_register("ompi", "mpi", "ft", "enable",
                                 ...
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, OPAL_INFO_LVL_4, ftscope,
                                 &ompi_ftmpi_enabled);
    value = 0;
    ...
    (void) mca_base_var_register(
        "ompi", "mpi", NULL, "param_check",
        "Whether you want MPI API parameters checked at run-time or not.  Possible values are 0 "
        "(no checking) and 1 (perform checking at run-time)",
    ...
    if (ompi_mpi_param_check && !MPI_PARAM_CHECK) {
        opal_show_help("help-mpi-runtime.txt", "mpi-param-check-enabled-but-compiled-out", true);
    ...
    (void) mca_base_var_register("ompi", "mpi", NULL, "yield_when_idle",
                                 ...
                                 "processes, will default to 1 when oversubscribing nodes)",
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, OPAL_INFO_LVL_5,
                                 MCA_BASE_VAR_SCOPE_READONLY, &ompi_mpi_yield_when_idle);

    ompi_mpi_event_tick_rate = -1;
    (void) mca_base_var_register(
        "ompi", "mpi", NULL, "event_tick_rate",
    ...
    (void) mca_base_var_register(
        "ompi", "mpi", NULL, "show_handle_leaks",
    ...
    (void)
        mca_base_var_register("ompi", "mpi", NULL, "no_free_handles",
                              "Whether to actually free MPI objects when their handles are freed",
                              MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, OPAL_INFO_LVL_9,
                              MCA_BASE_VAR_SCOPE_READONLY, &ompi_debug_no_free_handles);
    ...
    (void)
        mca_base_var_register("ompi", "mpi", NULL, "show_mpi_alloc_mem_leaks",
                              "If >0, MPI_FINALIZE will show up to this many instances of memory "
                              "allocated by MPI_ALLOC_MEM that was not freed by MPI_FREE_MEM",
                              MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_9,
                              MCA_BASE_VAR_SCOPE_READONLY, &ompi_debug_show_mpi_alloc_mem_leaks);

    /* Whether or not to print all MCA parameters in MPI_INIT */
    ompi_mpi_show_mca_params_string = NULL;
    (void) mca_base_var_register(
        "ompi", "mpi", NULL, "show_mca_params",
        "Whether to show all MCA parameter values during MPI_INIT or not (good for reproducability "
        "of MPI jobs "
    ...
    if (NULL != ompi_mpi_show_mca_params_string) {
        ...
        if (NULL == args) {
            opal_output(0, "WARNING: could not parse mpi_show_mca_params request - defaulting to "
                           "show \"all\"");
    ...
    (void) mca_base_var_register(
        "ompi", "mpi", NULL, "show_mca_params_file",
        "If mpi_show_mca_params is true, setting this string to a valid filename tells Open MPI to "
        "dump all the MCA parameter values into a file suitable for reading via the "
    ...
    (void) mca_base_var_register("ompi", "mpi", NULL, "have_sparse_group_storage",
                                 "Whether this Open MPI installation supports storing of data in "
                                 "MPI groups in \"sparse\" formats (good for extremely large "
                                 "process count MPI jobs that create many communicators/groups)",
    ...
    (void) mca_base_var_register("ompi", "mpi", NULL, "use_sparse_group_storage",
                                 "Whether to use \"sparse\" storage formats for MPI groups (only "
                                 ...
                                 OPAL_INFO_LVL_9,
                                 ompi_mpi_have_sparse_group_storage ? MCA_BASE_VAR_SCOPE_READONLY
                                                                    : MCA_BASE_VAR_SCOPE_CONSTANT,
                                 &ompi_use_sparse_group_storage);
    if (ompi_use_sparse_group_storage && !ompi_mpi_have_sparse_group_storage) {
        opal_show_help("help-mpi-runtime.txt", "sparse groups enabled but compiled out", true);
    ...
    if (opal_cuda_support && !opal_built_with_cuda_support) {
        opal_show_help("help-mpi-runtime.txt", "no cuda support", true);
    ...
    (void) mca_base_var_register("ompi", "mpi", NULL, "add_procs_cutoff",
                                 ...
                                 MCA_BASE_VAR_SCOPE_LOCAL, &ompi_add_procs_cutoff);

    ompi_mpi_dynamics_enabled = true;
    (void) mca_base_var_register(
        "ompi", "mpi", NULL, "dynamics_enabled",
        "Is the MPI dynamic process functionality enabled (e.g., MPI_COMM_SPAWN)?  Default is yes, "
        "but certain transports and/or environments may disable it.",
    ...
    (void) mca_base_var_register("ompi", "async", "mpi", "init",
                                 ...
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, OPAL_INFO_LVL_9,
                                 MCA_BASE_VAR_SCOPE_READONLY, &ompi_async_mpi_init);
    ...
    (void) mca_base_var_register("ompi", "async", "mpi", "finalize",
                                 ...
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, OPAL_INFO_LVL_9,
                                 MCA_BASE_VAR_SCOPE_READONLY, &ompi_async_mpi_finalize);
    ...
    (void) mca_base_var_register(
        "ompi", "mpi", NULL, "compat_mpi3",
        "A boolean value for whether Open MPI operates in MPI-3 compatibility mode; this changes "
        "the following behavior: in operations without a handle, errors are raised on (true) "
        "MPI_COMM_WORLD (MPI-3 behavior) or (false) MPI_COMM_SELF (MPI-4 behavior).",
        MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, OPAL_INFO_LVL_9, MCA_BASE_VAR_SCOPE_READONLY,
    ...
    (void) mca_base_var_register("ompi", "mpi", NULL, "spc_attach",
                                 "A comma-delimeted list of software-based performance counters "
                                 "(SPCs) to enable (\"all\" enables all counters).",
                                 MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0, OPAL_INFO_LVL_4,
                                 MCA_BASE_VAR_SCOPE_READONLY, &ompi_mpi_spc_attach_string);

    ompi_mpi_spc_dump_enabled = false;
    (void) mca_base_var_register("ompi", "mpi", NULL, "spc_dump_enabled",
                                 "A boolean value for whether (true) or not (false) to enable "
                                 "dumping enabled SPC counters in MPI_Finalize.",
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, OPAL_INFO_LVL_4,
                                 MCA_BASE_VAR_SCOPE_READONLY, &ompi_mpi_spc_dump_enabled);
#endif // SPC_ENABLE
...
int ompi_show_all_mca_params(int32_t rank, int requested, char *nodename)
{
    ...
    if (NULL != ompi_mpi_show_mca_params_file && 0 != strlen(ompi_mpi_show_mca_params_file)) {
        if (NULL == (fp = fopen(ompi_mpi_show_mca_params_file, "w"))) {
            opal_output(0, "Unable to open file <%s> to write MCA parameters",
                        ompi_mpi_show_mca_params_file);
