...
int ompi_mpi_register_params(void)
{
    ...
    (void) mca_base_var_register ("ompi", "mpi", NULL, "dpm_group_connect",
                                  "Whether (true) or not (false) MPI_Comm_connect/accept/spawn builds the new "
                                  "intercommunicator with a single PMIx group over both sides, which assigns its "
                                  "context ID in the same operation, rather than with PMIx_Connect followed by an "
                                  "iterative context ID agreement. Used only if both sides allow it and their "
                                  "PMLs support extended context IDs.",
