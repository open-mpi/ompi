...
{
    if (!opal_memchecker_base_runindebugger() || (0 == pConvertor->count)) {
        return OMPI_SUCCESS;
    }
    ...
    if (OPAL_LIKELY(pConvertor->flags & CONVERTOR_NO_OP)) {
        ...
        for (i = 0; i < pConvertor->count; i++) {
            ...
            while (pElem->elem.common.flags & OPAL_DATATYPE_FLAG_DATA) {
                ...
                f((void *) (source_base + pElem->elem.disp),
                  pElem->elem.count * pElem->elem.extent);
 ...
 *
 */
static inline int memchecker_call(int (*f)(void *, size_t), const void *addr, size_t count,
...
static inline int memchecker_comm(MPI_Comm comm)
{
    ...
    /*
     * We should not check underlying objects in this way -- either another
     * opal/include/memchecker.h However, let us assume, that underlying objects are initialized
     * correctly
     */
#    if 0
    /* c_base */
...
#    endif
    ...
    /* c_coll */
    opal_memchecker_base_isdefined(&comm->c_coll->coll_module_init,
                                   sizeof(mca_coll_base_module_init_1_0_0_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_module_finalize,
                                   sizeof(mca_coll_base_module_finalize_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_allgather,
                                   sizeof(mca_coll_base_module_allgather_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_allgatherv,
                                   sizeof(mca_coll_base_module_allgatherv_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_allreduce,
                                   sizeof(mca_coll_base_module_allreduce_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_alltoall,
                                   sizeof(mca_coll_base_module_alltoall_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_alltoallv,
                                   sizeof(mca_coll_base_module_alltoallv_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_alltoallw,
                                   sizeof(mca_coll_base_module_alltoallw_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_barrier,
                                   sizeof(mca_coll_base_module_barrier_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_bcast,
                                   sizeof(mca_coll_base_module_bcast_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_exscan,
                                   sizeof(mca_coll_base_module_exscan_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_gather,
                                   sizeof(mca_coll_base_module_gather_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_gatherv,
                                   sizeof(mca_coll_base_module_gatherv_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_reduce,
                                   sizeof(mca_coll_base_module_reduce_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_reduce_scatter,
                                   sizeof(mca_coll_base_module_reduce_scatter_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_scan,
                                   sizeof(mca_coll_base_module_scan_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_scatter,
                                   sizeof(mca_coll_base_module_scatter_fn_t));
    opal_memchecker_base_isdefined(&comm->c_coll->coll_scatterv,
                                   sizeof(mca_coll_base_module_scatterv_fn_t));

    opal_memchecker_base_isdefined(&comm->c_coll_selected_component,
                                   sizeof(const mca_coll_base_component_2_4_0_t *));
    opal_memchecker_base_isdefined(&comm->c_coll_selected_module,
                                   sizeof(const mca_coll_base_module_1_0_0_t *));
    /* Somehow, this often shows up in petsc with comm_dup'ed communicators*/
    /* opal_memchecker_base_isdefined (&comm->c_coll_selected_data, sizeof(struct
     * mca_coll_base_comm_t *)); */
    opal_memchecker_base_isdefined(&comm->c_coll_basic_module,
                                   sizeof(const mca_coll_base_module_1_0_0_t *));
...
#    endif
    ...
    opal_memchecker_base_isdefined((void *) &(*request)->req_state,
                                   sizeof(volatile ompi_request_state_t));
    ...
    /* req_mpi_object */
    opal_memchecker_base_isdefined(&(*request)->req_mpi_object.comm,
...
static inline int memchecker_datatype(MPI_Datatype type)
{
    ...
    if (NULL != type->super.ptypes)
        opal_memchecker_base_isdefined(&type->super.ptypes,
