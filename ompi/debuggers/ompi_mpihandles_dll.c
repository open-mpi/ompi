                      ...
                      struct mpidbg_comm_info_t **info)
{
    ...
    *info = mqs_malloc(sizeof(struct mpidbg_comm_info_t));
    ...
    if (0 != topo && 0 != ((*info)->comm_bitflags & MPIDBG_COMM_INFO_CARTESIAN)) {
        ...
        (*info)->comm_cart_num_dims = ndims = ompi_fetch_int(process,
                                                             topo
                                                                 + i_info->mca_topo_base_module_t
                                                                       .offset.mtc.cart.ndims,
                                                             p_info);
        ...
        (*info)->comm_graph_num_nodes = nnodes = ompi_fetch_int(process,
                                                                topo
                                                                    + i_info->mca_topo_base_module_t
                                                                          .offset.mtc.graph.nnodes,
                                                                p_info);
    ...
    /* Fortran handle */
    (*info)->comm_fortran_handle = ompi_fetch_int(process,
                                                  c_comm
                                                      + i_info->ompi_communicator_t.offset
                                                            .c_f_to_c_index,
                                                  p_info);
