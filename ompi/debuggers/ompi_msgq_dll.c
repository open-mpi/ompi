                                        ...
                                        mqs_opal_free_list_t_pos *position, mqs_taddr_t free_list)
{
    ...
    position->fl_frag_size = ompi_fetch_size_t(proc,
                                               ...
                                               p_info);
    position->fl_frag_alignment = ompi_fetch_size_t(proc,
                                                    position->free_list
                                                        + i_info->opal_free_list_t.offset
                                                              .fl_frag_alignment,
                                                    p_info);
    position->fl_frag_class = ompi_fetch_pointer(proc,
                                                 position->free_list
                                                     + i_info->opal_free_list_t.offset
                                                           .fl_frag_class,
                                                 p_info);
    position->fl_mpool = ompi_fetch_pointer(proc,
                                            ...
                                            p_info);
    position->fl_num_per_alloc = ompi_fetch_size_t(proc,
                                                   position->free_list
                                                       + i_info->opal_free_list_t.offset
                                                             .fl_num_per_alloc,
                                                   p_info);
    position->fl_num_allocated = ompi_fetch_size_t(proc,
                                                   position->free_list
                                                       + i_info->opal_free_list_t.offset
                                                             .fl_num_allocated,
                                                   p_info);
...
rescan_requests:
    ...
    if (OMPI_REQUEST_PML == req_type) {
        ...
        req_complete = ompi_fetch_bool(proc,
                                       ...
                                       p_info);
        req_pml_complete = ompi_fetch_bool(proc,
                                           current_item
                                               + i_info->mca_pml_base_request_t.offset
                                                     .req_pml_complete,
                                           p_info);
        res->status = (0 == req_complete ? mqs_st_pending : mqs_st_complete);

        res->desired_local_rank = ompi_fetch_int(proc,
                                                 current_item
                                                     + i_info->mca_pml_base_request_t.offset
                                                           .req_peer,
                                                 p_info);
        ...
        if (MCA_PML_REQUEST_SEND == req_type) {
            ...
            req_buffer = ompi_fetch_pointer(proc,
                                            current_item
