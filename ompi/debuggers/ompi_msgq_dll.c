...
static group_t *find_or_create_group(mqs_process *proc, mqs_taddr_t group_base)
{
    ...
    np = ompi_fetch_int(proc, group_base + i_info->ompi_group_t.offset.grp_proc_count, p_info);
    if (np < 0) {
        DEBUG(VERBOSE_COMM, ("Get a size for the communicator = %d\n", np));
        return NULL; /* Makes no sense ! */
    }
    is_dense = ompi_fetch_int(proc, group_base + i_info->ompi_group_t.offset.grp_flags, p_info);
    ...
    for (; comm; comm = comm->next) {
        ...
        if (group && (group->group_base == group_base)) {
            ...
            DEBUG(VERBOSE_GROUP,
                  ("Increase refcount for group 0x%p to %d\n", (void *) group, group->ref_count));
            return group;
        }
    }
    ...
    tablep = ompi_fetch_pointer(proc, group_base + i_info->ompi_group_t.offset.grp_proc_pointers,
                                p_info);

    if ((0 != np)
        && (mqs_ok != mqs_fetch_data(proc, tablep, np * p_info->sizes.pointer_size, trbuffer))) {
    ...
    if (NULL == extra->world_proc_array) {
        ...
        for (i = 0; i < np; i++) {
            mqs_target_to_host(proc, trbuffer + p_info->sizes.pointer_size * i, &value,
                               p_info->sizes.pointer_size);
         ...
         */
        {
            ...
            DEBUG(VERBOSE_GENERAL,
                  ("sizes short = %d int = %d long = %d long long = %d "
                   ...
                   p_info->sizes.short_size, p_info->sizes.int_size, p_info->sizes.long_size,
                   p_info->sizes.long_long_size, p_info->sizes.pointer_size,
                   p_info->sizes.bool_size, p_info->sizes.size_t_size));
        }
...
static int communicators_changed(mqs_process *proc)
{
    ...
    lowest_free = ompi_fetch_int(proc,
                                 extra->commlist_base
                                     + i_info->opal_pointer_array_t.offset.lowest_free,
                                 p_info);
    number_free = ompi_fetch_int(proc,
                                 extra->commlist_base
                                     + i_info->opal_pointer_array_t.offset.number_free,
                                 p_info);
    if ((lowest_free != extra->comm_lowest_free) || (number_free != extra->comm_number_free)) {
        DEBUG(VERBOSE_COMM, ("Recreate the communicator list\n"
                             "    lowest_free [current] %d != [stored] %d\n"
                             "    number_free [current] %d != [stored] %d\n",
                             (int) lowest_free, (int) extra->comm_lowest_free, (int) number_free,
                             (int) extra->comm_number_free));
        return 1;
    }
...
static int rebuild_communicator_list(mqs_process *proc)
{
    ...
    DEBUG(VERBOSE_COMM,
          ("rebuild_communicator_list called "
           "(commlist_base %llx, array offset %ld array size %d)\n",
           (long long) extra->commlist_base, (long) i_info->opal_pointer_array_t.offset.addr,
    ...
    DEBUG(VERBOSE_COMM, ("Number of coms %d lowest_free %d number_free %d\n", (int) comm_size,
                         (int) lowest_free, (int) number_free));
    ...
    comm_addr_base = ompi_fetch_pointer(proc,
                                        extra->commlist_base
                                            + i_info->opal_pointer_array_t.offset.addr,
                                        p_info);
    ...
    for (i = 0; (commcount < (comm_size - number_free)) && (i < comm_size); i++) {
        ...
        DEBUG(VERBOSE_GENERAL,
              ("Retrieve context_id from 0x%llx and local_rank from 0x%llx\n",
        ...
        context_id = ompi_fetch_int(proc, comm_ptr + i_info->ompi_communicator_t.offset.c_contextid,
                                    p_info);
        ...
        if (NULL == old) {
            ...
            old->comm_info.local_rank
                = ompi_fetch_int(proc, comm_ptr + i_info->ompi_communicator_t.offset.c_my_rank,
                                 p_info);
            old->group = NULL;

            DEBUG(VERBOSE_COMM,
                  ("Create new communicator 0x%lx with context_id %d and local_rank %d\n",
                   (long) old, context_id, local_rank));
            /* Now get the information about the group */
            group_base = ompi_fetch_pointer(proc,
                                            comm_ptr
                                                + i_info->ompi_communicator_t.offset.c_local_group,
                                            p_info);
            old->group = find_or_create_group(proc, group_base);
        }
        mqs_fetch_data(proc, comm_ptr + i_info->ompi_communicator_t.offset.c_name, 64,
                       old->comm_info.name);
        ...
        old->present = TRUE;
        DEBUG(VERBOSE_COMM,
              ("Communicator 0x%llx %d local_rank %d name %s group %p\n", (long long) old->comm_ptr,
               (int) old->comm_info.unique_id, (int) old->comm_info.local_rank, old->comm_info.name,
               (void *) old->group));
    }
    ...
    for (; *commp;) {
        ...
        if (comm->present) {
            ...
            *commp = comm->next;     /* Remove from the list, *commp now points to the next */
            DEBUG(VERBOSE_COMM,
                  ("Remove communicator 0x%llx name %s (group %p)\n", (long long) comm->comm_ptr,
                   comm->comm_info.name, (void *) comm->group));
    ...
    if (commcount) {
        ...
        communicator_t **comm_array = (communicator_t **) mqs_malloc(commcount
    ...
    if (extra->current_communicator) {
        *comm = extra->current_communicator->comm_info;
        DEBUG(VERBOSE_COMM, ("mqs_get_communicator %d local_rank %d name %s\n",
                             (int) comm->unique_id, (int) comm->local_rank, comm->name));
        return mqs_ok;
    }
                                   ...
                                   mqs_opal_list_t_pos *position, mqs_taddr_t list)
{
    ...
    position->current_item = ompi_fetch_pointer(
        proc, position->sentinel + i_info->opal_list_item_t.offset.opal_list_next, p_info);
                                 ...
                                 mqs_opal_list_t_pos *position, mqs_taddr_t *active_item)
{
    ...
    *active_item = position->current_item;
    ...
    position->current_item = ompi_fetch_pointer(
        proc, position->current_item + i_info->opal_list_item_t.offset.opal_list_next, p_info);
    if (position->current_item == position->sentinel)
        position->current_item = 0;
    return mqs_ok;
}
                                        ...
                                        mqs_opal_free_list_t_pos *position, mqs_taddr_t free_list)
{
    ...
    position->fl_frag_size = ompi_fetch_size_t(proc,
                                               position->free_list
                                                   + i_info->opal_free_list_t.offset.fl_frag_size,
                                               p_info);
    position->fl_frag_alignment = ompi_fetch_size_t(
        proc, position->free_list + i_info->opal_free_list_t.offset.fl_frag_alignment, p_info);
    position->fl_frag_class = ompi_fetch_pointer(
        proc, position->free_list + i_info->opal_free_list_t.offset.fl_frag_class, p_info);
    position->fl_mpool = ompi_fetch_pointer(proc,
                                            position->free_list
                                                + i_info->opal_free_list_t.offset.fl_mpool,
                                            p_info);
    position->fl_num_per_alloc = ompi_fetch_size_t(
        proc, position->free_list + i_info->opal_free_list_t.offset.fl_num_per_alloc, p_info);
    position->fl_num_allocated = ompi_fetch_size_t(
        proc, position->free_list + i_info->opal_free_list_t.offset.fl_num_allocated, p_info);
    ...
    position->header_space = OPAL_ALIGN(position->header_space, position->fl_frag_alignment,
                                        mqs_taddr_t);
    ...
    if (0 == active_allocation) { /* the end of the list */
        ...
        active_allocation = OPAL_ALIGN(active_allocation, position->fl_frag_alignment, mqs_taddr_t);
        /**
         * Now let's try to compute the upper bound ...
         */
        position->upper_bound = position->fl_num_initial_alloc * position->header_space
                                + active_allocation;
                                      ...
                                      mqs_opal_free_list_t_pos *position, mqs_taddr_t *active_item)
{
    ...
    *active_item = position->current_item;
    ...
    if (position->current_item >= position->upper_bound) {
         ...
         * Now let's try to compute the upper bound ...
         */
        position->upper_bound = position->fl_num_per_alloc * position->header_space
                                + active_allocation;
        position->current_item = active_allocation;
 ...
 */
static int fetch_request(mqs_process *proc, mpi_process_info *p_info, mqs_pending_operation *res,
...
rescan_requests:
    while (1) {
        opal_free_list_t_next_item(proc, p_info, &extra->next_msg, &current_item);
        if (0 == current_item) {
    ...
    res->extra_text[2][0] = 0;
    res->extra_text[3][0] = 0;
    res->extra_text[4][0] = 0;
    ...
    if (OMPI_REQUEST_PML == req_type) {
         ...
         */
        res->desired_tag = ompi_fetch_int(proc,
                                          current_item
                                              + i_info->mca_pml_base_request_t.offset.req_tag,
                                          p_info);
        ...
        req_type = ompi_fetch_int(proc,
                                  current_item + i_info->mca_pml_base_request_t.offset.req_type,
                                  p_info);
        req_complete = ompi_fetch_bool(proc,
                                       current_item + i_info->ompi_request_t.offset.req_complete,
                                       p_info);
        req_pml_complete = ompi_fetch_bool(
            proc, current_item + i_info->mca_pml_base_request_t.offset.req_pml_complete, p_info);
        res->status = (0 == req_complete ? mqs_st_pending : mqs_st_complete);

        res->desired_local_rank
            = ompi_fetch_int(proc, current_item + i_info->mca_pml_base_request_t.offset.req_peer,
                             p_info);
        res->desired_global_rank = translate(extra->current_communicator->group,
                                             res->desired_local_rank);

        res->buffer = ompi_fetch_pointer(proc,
                                         current_item
                                             + i_info->mca_pml_base_request_t.offset.req_addr,
                                         p_info);
        /* Set this to true if it's a buffered request */
        res->system_buffer = FALSE;

        /* The pointer to the request datatype */
        ompi_datatype = ompi_fetch_pointer(proc,
                                           current_item
                                               + i_info->mca_pml_base_request_t.offset.req_datatype,
                                           p_info);
        /* Retrieve the count as specified by the user */
        res->desired_length = ompi_fetch_size_t(proc,
                                                ompi_datatype + i_info->ompi_datatype_t.offset.size,
                                                p_info);
        /* Be user friendly, show the datatype name */
        mqs_fetch_data(proc, ompi_datatype + i_info->ompi_datatype_t.offset.name, 64, data_name);
        if ('\0' != data_name[0]) {
            ...
            data_name[4] = '\0';
            snprintf((char *) res->extra_text[1], 64, "Data: %d", (int) res->desired_length);
            snprintf((char *) res->extra_text[2], 64, "%s", data_name);
        }
        /* And now compute the real length as specified by the user */
        res->desired_length *= ompi_fetch_size_t(
            proc, current_item + i_info->mca_pml_base_request_t.offset.req_count, p_info);

        if (MCA_PML_REQUEST_SEND == req_type) {
            snprintf((char *) res->extra_text[0], 64, "Send: 0x%llx", (long long) current_item);
            req_buffer = ompi_fetch_pointer(
                proc, current_item + i_info->mca_pml_base_send_request_t.offset.req_addr, p_info);
            res->system_buffer = (req_buffer == res->buffer ? FALSE : TRUE);
            res->actual_length = ompi_fetch_size_t(proc,
                                                   current_item
                                                       + i_info->mca_pml_base_send_request_t.offset
                                                             .req_bytes_packed,
                                                   p_info);
             ...
             * is matched.
             */
            res->actual_tag = ompi_fetch_int(proc,
                                             current_item + i_info->ompi_request_t.offset.req_status
                                                 + i_info->ompi_status_public_t.offset.MPI_TAG,
                                             p_info);
            if (MPI_ANY_TAG != (int) res->actual_tag) {
                res->status = mqs_st_matched;
                res->desired_length = ompi_fetch_size_t(proc,
                                                        current_item
                                                            + i_info->mca_pml_base_recv_request_t
                                                                  .offset.req_bytes_packed,
                                                        p_info);
                res->actual_local_rank
                    = ompi_fetch_int(proc,
                                     current_item + i_info->ompi_request_t.offset.req_status
                                         + i_info->ompi_status_public_t.offset.MPI_SOURCE,
                                     p_info);
                res->actual_global_rank = translate(extra->current_communicator->group,
                                                    res->actual_local_rank);
            }
        } else {
            snprintf((char *) res->extra_text[0], 64, "Unknown type of request 0x%llx",
        ...
        /*res->desired_length      =
          ompi_fetch_int( proc, current_item + i_info->mca_pml_base_request_t.offset.req_count,
          p_info );*/

        if ((mqs_st_pending < res->status)
            && (MCA_PML_REQUEST_SEND != req_type)) { /* The real data from the status */
            res->actual_length = ompi_fetch_size_t(proc,
                                                   current_item
                                                       + i_info->ompi_request_t.offset.req_status
                                                       + i_info->ompi_status_public_t.offset
                                                             ._ucount,
                                                   p_info);
            res->actual_tag = ompi_fetch_int(proc,
                                             current_item + i_info->ompi_request_t.offset.req_status
                                                 + i_info->ompi_status_public_t.offset.MPI_TAG,
                                             p_info);
            res->actual_local_rank = ompi_fetch_int(proc,
                                                    current_item
                                                        + i_info->ompi_request_t.offset.req_status
                                                        + i_info->ompi_status_public_t.offset
                                                              .MPI_SOURCE,
...
char *mqs_dll_error_string(int errcode)
{
    ...
    case err_group_corrupt:
        return "Could not read a communicator's group from the process (probably a store "
               "corruption)";
