...
typedef int(peruse_comm_callback_f)(peruse_event_h event_h, MPI_Aint unique_id,
                                    peruse_comm_spec_t *spec, void *param);

/* PERUSE constants */
enum {
    PERUSE_SUCCESS = 0,
    /* Success */            /* XXX Devation from 1.11 */
...
OMPI_DECLSPEC int PERUSE_Query_supported_events(int *num_supported, char ***event_names,
                                                int **events);
...
OMPI_DECLSPEC int PERUSE_Event_comm_register(int event, MPI_Comm comm,
