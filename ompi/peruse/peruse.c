...
{
    if (event < 0 || event > PERUSE_num_events || NULL == PERUSE_events[event].name)
...
int PERUSE_Event_comm_register(int event, MPI_Comm comm, peruse_comm_callback_f *callback_fn,
                               void *param, peruse_event_h *event_h)
{
    ...
    if (NULL == comm->c_peruse_handles) {
        comm->c_peruse_handles = (ompi_peruse_handle_t **) calloc(PERUSE_num_events,
                                                                  sizeof(ompi_peruse_handle_t *));
    }
...
#define PERUSE_MPI_PARAM_CHECK(obj_upper, obj_lower)                                          \
    if (MPI_PARAM_CHECK) {                                                                    \
        OMPI_ERR_PERUSE_INIT_FINALIZE;                                                        \
                                                                                              \
        if (PERUSE_EVENT_HANDLE_NULL == event_h || ((ompi_peruse_handle_t *) event_h)->active \
            || ((ompi_peruse_handle_t *) event_h)->type != PERUSE_TYPE_##obj_upper)           \
            return PERUSE_ERR_EVENT_HANDLE;                                                   \
                                                                                              \
        if (NULL == callback_fn)                                                              \
...
int PERUSE_Event_comm_callback_set(peruse_event_h event_h, peruse_comm_callback_f *callback_fn,
                                   void *param)
{
    ompi_peruse_handle_t *handle = (ompi_peruse_handle_t *) event_h;
...
int PERUSE_Event_comm_callback_get(peruse_event_h event_h, peruse_comm_callback_f **callback_fn,
                                   void **param)
{
    ompi_peruse_handle_t *handle = (ompi_peruse_handle_t *) event_h;
