                                 ...
                                 MPI_Info info, MPI_Request *request)
{
    ...
    MEMCHECKER(
        ...
        err = mca_topo_base_neighbor_count(comm, &indegree, &outdegree); if (MPI_SUCCESS == err) {
            if (MPI_IN_PLACE != sendbuf) {
            ...
            for (i = 0; i < indegree; i++) {
                ...
                memchecker_call(&opal_memchecker_base_isaddressable,
                                (char *) (recvbuf) + rdispls[i] * recv_ext, recvcounts[i],
                                recvtype);
            }
        });
    ...
    if (MPI_PARAM_CHECK) {
        ...
        } else if ((NULL == sendcounts) || (NULL == sdispls) || (NULL == recvcounts)
                   || (NULL == rdispls) || MPI_IN_PLACE == sendbuf || MPI_IN_PLACE == recvbuf) {
    ...
    err = comm->c_coll
              ->coll_neighbor_alltoallv_init(sendbuf, sendcounts, sdispls, sendtype, recvbuf,
