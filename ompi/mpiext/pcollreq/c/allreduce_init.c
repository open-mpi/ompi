                        ...
                        MPI_Op op, MPI_Comm comm, MPI_Info info, MPI_Request *request)
{
    ...
    MEMCHECKER(
        ...
        } else { memchecker_call(&opal_memchecker_base_isdefined, sendbuf, count, datatype); });
    ...
    if (MPI_PARAM_CHECK) {
        ...
        if (ompi_comm_invalid(comm)) {
            ...
            return OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_BUFFER, FUNC_NAME);
        } else if ((sendbuf == recvbuf) && (MPI_BOTTOM != sendbuf) && (count > 1)) {
            return OMPI_ERRHANDLER_NOHANDLE_INVOKE(MPI_ERR_BUFFER, FUNC_NAME);
        } else {
    ...
    err = comm->c_coll->coll_allreduce_init(sendbuf, recvbuf, count, datatype, op, comm, info,
                                            request, comm->c_coll->coll_allreduce_init_module);
