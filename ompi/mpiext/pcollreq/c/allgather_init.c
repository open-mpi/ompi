...
int MPIX_Allgather_init(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf,
                        ...
                        MPI_Request *request)
{
    ...
    MEMCHECKER(
        ...
        if (MPI_IN_PLACE == sendbuf) {
            memchecker_call(&opal_memchecker_base_isdefined, (char *) (recvbuf) + rank * ext,
                            recvcount, recvtype);
        } else {
    ...
    err = comm->c_coll->coll_allgather_init(sendbuf, sendcount, sendtype, recvbuf, recvcount,
