...
int MPIX_Gatherv_init(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf,
                      ...
                      MPI_Comm comm, MPI_Info info, MPI_Request *request)
{
            ...
            if (ompi_comm_rank(comm) == root) {
                ...
                if (MPI_IN_PLACE == sendbuf) {
                    for (i = 0; i < size; i++) {
                        memchecker_call(&opal_memchecker_base_isdefined,
                                        (char *) (recvbuf) + displs[i] * ext, recvcounts[i],
                                        recvtype);
                    }
                } else {
                ...
                for (i = 0; i < size; i++) {
                    memchecker_call(&opal_memchecker_base_isaddressable,
                                    (char *) (recvbuf) + displs[i] * ext, recvcounts[i], recvtype);
                }
            } else {
                memchecker_datatype(sendtype);
    ...
    err = comm->c_coll->coll_gatherv_init(sendbuf, sendcount, sendtype, recvbuf, recvcounts, displs,
