...
int MPIX_Neighbor_alltoallw_init(const void *sendbuf, const int sendcounts[],
                                 ...
                                 MPI_Request *request)
{
    ...
    MEMCHECKER(ptrdiff_t recv_ext; ptrdiff_t send_ext;
               ...
               if (MPI_SUCCESS == err) {
                   ...
                   for (i = 0; i < indegree; i++) {
                       ...
                       memchecker_call(&opal_memchecker_base_isaddressable,
                                       (char *) (recvbuf) + sdispls[i] * recv_ext, recvcounts[i],
                                       recvtypes[i]);
                   }
               });
    ...
    if (MPI_PARAM_CHECK) {
        ...
        if ((NULL == sendcounts) || (NULL == sdispls) || (NULL == sendtypes) || (NULL == recvcounts)
            || (NULL == rdispls) || (NULL == recvtypes) || MPI_IN_PLACE == sendbuf
    ...
    err = comm->c_coll
              ->coll_neighbor_alltoallw_init(sendbuf, sendcounts, sdispls, sendtypes, recvbuf,
