...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPIX_ALLTOALLV_INIT, pmpix_alltoallv_init, pmpix_alltoallv_init_,
                           pmpix_alltoallv_init__, pompix_alltoallv_init_f,
                           (char *sendbuf, MPI_Fint *sendcounts, MPI_Fint *sdispls,
                            MPI_Fint *sendtype, char *recvbuf, MPI_Fint *recvcounts,
                            MPI_Fint *rdispls, MPI_Fint *recvtype, MPI_Fint *comm, MPI_Fint *info,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPIX_ALLTOALLV_INIT, mpix_alltoallv_init, mpix_alltoallv_init_,
                           mpix_alltoallv_init__, ompix_alltoallv_init_f,
                           (char *sendbuf, MPI_Fint *sendcounts, MPI_Fint *sdispls,
                            MPI_Fint *sendtype, char *recvbuf, MPI_Fint *recvcounts,
                            MPI_Fint *rdispls, MPI_Fint *recvtype, MPI_Fint *comm, MPI_Fint *info,
                            MPI_Fint *request, MPI_Fint *ierr),
                            ...
                            MPI_Fint *request, MPI_Fint *ierr)
{
    ...
    c_ierr = PMPIX_Alltoallv_init(sendbuf, OMPI_ARRAY_NAME_CONVERT(sendcounts),
                                  OMPI_ARRAY_NAME_CONVERT(sdispls), c_sendtype, recvbuf,
                                  OMPI_ARRAY_NAME_CONVERT(recvcounts),
                                  OMPI_ARRAY_NAME_CONVERT(rdispls), c_recvtype, c_comm, c_info,
                                  &c_request);
    if (NULL != ierr)
