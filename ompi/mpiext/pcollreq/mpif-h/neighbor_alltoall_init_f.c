...
OMPI_GENERATE_F77_BINDINGS(
    PMPIX_NEIGHBOR_ALLTOALL_INIT, pmpix_neighbor_alltoall_init, pmpix_neighbor_alltoall_init_,
    pmpix_neighbor_alltoall_init__, pompix_neighbor_alltoall_init_f,
...
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(
    MPIX_NEIGHBOR_ALLTOALL_INIT, mpix_neighbor_alltoall_init, mpix_neighbor_alltoall_init_,
    mpix_neighbor_alltoall_init__, ompix_neighbor_alltoall_init_f,
    (char *sendbuf, MPI_Fint *sendcount, MPI_Fint *sendtype, char *recvbuf, MPI_Fint *recvcount,
     MPI_Fint *recvtype, MPI_Fint *comm, MPI_Fint *info, MPI_Fint *request, MPI_Fint *ierr),
    (sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, info, request, ierr))
#    else
...
void ompix_neighbor_alltoall_init_f(char *sendbuf, MPI_Fint *sendcount, MPI_Fint *sendtype,
                                    ...
                                    MPI_Fint *comm, MPI_Fint *info, MPI_Fint *request,
                                    MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Comm c_comm;
    ...
    c_ierr = PMPIX_Neighbor_alltoall_init(sendbuf, OMPI_FINT_2_INT(*sendcount), c_sendtype, recvbuf,
                                          OMPI_FINT_2_INT(*recvcount), c_recvtype, c_comm, c_info,
                                          &c_req);
    if (NULL != ierr)
