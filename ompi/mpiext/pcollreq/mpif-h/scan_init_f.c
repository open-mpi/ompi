...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPIX_SCAN_INIT, pmpix_scan_init, pmpix_scan_init_, pmpix_scan_init__,
                           pompix_scan_init_f,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPIX_SCAN_INIT, mpix_scan_init, mpix_scan_init_, mpix_scan_init__,
                           ompix_scan_init_f,
                           (char *sendbuf, char *recvbuf, MPI_Fint *count, MPI_Fint *datatype,
                            MPI_Fint *op, MPI_Fint *comm, MPI_Fint *info, MPI_Fint *request,
                            MPI_Fint *ierr),
...
void ompix_scan_init_f(char *sendbuf, char *recvbuf, MPI_Fint *count, MPI_Fint *datatype,
                       MPI_Fint *op, MPI_Fint *comm, MPI_Fint *info, MPI_Fint *request,
                       MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPIX_Scan_init(sendbuf, recvbuf, OMPI_FINT_2_INT(*count), c_type, c_op, c_comm,
                             c_info, &c_request);
    if (NULL != ierr)
