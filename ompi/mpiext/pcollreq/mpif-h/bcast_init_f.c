...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPIX_BCAST_INIT, pmpix_bcast_init, pmpix_bcast_init_,
                           pmpix_bcast_init__, pompix_bcast_init_f,
                           (char *buffer, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *root,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPIX_BCAST_INIT, mpix_bcast_init, mpix_bcast_init_, mpix_bcast_init__,
                           ompix_bcast_init_f,
                           (char *buffer, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *root,
                            MPI_Fint *comm, MPI_Fint *info, MPI_Fint *request, MPI_Fint *ierr),
                           (buffer, count, datatype, root, comm, info, request, ierr))
...
void ompix_bcast_init_f(char *buffer, MPI_Fint *count, MPI_Fint *datatype, MPI_Fint *root,
                        MPI_Fint *comm, MPI_Fint *info, MPI_Fint *request, MPI_Fint *ierr)
{
    int c_ierr;
    MPI_Comm c_comm;
    ...
    c_ierr = PMPIX_Bcast_init(OMPI_F2C_BOTTOM(buffer), OMPI_FINT_2_INT(*count), c_type,
                              OMPI_FINT_2_INT(*root), c_comm, c_info, &c_req);
    if (NULL != ierr)
