...
#    else
OMPI_GENERATE_F77_BINDINGS(PMPIX_REDUCE_INIT, pmpix_reduce_init, pmpix_reduce_init_,
                           pmpix_reduce_init__, pompix_reduce_init_f,
                           (char *sendbuf, char *recvbuf, MPI_Fint *count, MPI_Fint *datatype,
                            MPI_Fint *op, MPI_Fint *root, MPI_Fint *comm, MPI_Fint *info,
...
#else
#    if !OMPI_BUILD_MPI_PROFILING
OMPI_GENERATE_F77_BINDINGS(MPIX_REDUCE_INIT, mpix_reduce_init, mpix_reduce_init_,
                           mpix_reduce_init__, ompix_reduce_init_f,
                           (char *sendbuf, char *recvbuf, MPI_Fint *count, MPI_Fint *datatype,
                            MPI_Fint *op, MPI_Fint *root, MPI_Fint *comm, MPI_Fint *info,
                            MPI_Fint *request, MPI_Fint *ierr),
                           (sendbuf, recvbuf, count, datatype, op, root, comm, info, request, ierr))
...
void ompix_reduce_init_f(char *sendbuf, char *recvbuf, MPI_Fint *count, MPI_Fint *datatype,
                         MPI_Fint *op, MPI_Fint *root, MPI_Fint *comm, MPI_Fint *info,
                         MPI_Fint *request, MPI_Fint *ierr)
{
    int c_ierr;
    ...
    c_ierr = PMPIX_Reduce_init(sendbuf, recvbuf, OMPI_FINT_2_INT(*count), c_type, c_op,
