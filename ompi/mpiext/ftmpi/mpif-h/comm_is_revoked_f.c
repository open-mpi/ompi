...
OMPI_GENERATE_F77_BINDINGS(PMPIX_COMM_IS_REVOKED, pmpix_comm_is_revoked, pmpix_comm_is_revoked_,
                           pmpix_comm_is_revoked__, ompix_comm_is_revoked_f,
                           (MPI_Fint * comm, ompi_fortran_logical_t *flag, MPI_Fint *ierr),
                           (comm, flag, ierr))

OMPI_GENERATE_F77_BINDINGS(MPIX_COMM_IS_REVOKED, mpix_comm_is_revoked, mpix_comm_is_revoked_,
                           mpix_comm_is_revoked__, ompix_comm_is_revoked_f,
                           (MPI_Fint * comm, ompi_fortran_logical_t *flag, MPI_Fint *ierr),
                           (comm, flag, ierr))
#endif
...
void ompix_comm_is_revoked_f(MPI_Fint *comm, ompi_fortran_logical_t *flag, MPI_Fint *ierr)
{
    ...
    *ierr = OMPI_INT_2_FINT(PMPIX_Comm_is_revoked(c_comm, OMPI_LOGICAL_SINGLE_NAME_CONVERT(flag)));
}
