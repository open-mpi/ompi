...
OMPI_GENERATE_F77_BINDINGS(MPIX_COMM_IAGREE, mpix_comm_iagree, mpix_comm_iagree_,
                           mpix_comm_iagree__, ompix_comm_iagree_f,
                           (MPI_Fint * comm, ompi_fortran_logical_t *flag, MPI_Fint *request,
                            MPI_Fint *ierr),
                           (comm, flag, request, ierr))

OMPI_GENERATE_F77_BINDINGS(PMPIX_COMM_IAGREE, pmpix_comm_iagree, pmpix_comm_iagree_,
                           pmpix_comm_iagree__, ompix_comm_iagree_f,
                           (MPI_Fint * comm, ompi_fortran_logical_t *flag, MPI_Fint *request,
                            MPI_Fint *ierr),
                           (comm, flag, request, ierr))
#endif

void ompix_comm_iagree_f(MPI_Fint *comm, ompi_fortran_logical_t *flag, MPI_Fint *request,
                         MPI_Fint *ierr)
{
    ...
    *ierr = OMPI_INT_2_FINT(
        PMPIX_Comm_iagree(c_comm, OMPI_LOGICAL_SINGLE_NAME_CONVERT(flag), &c_req));
