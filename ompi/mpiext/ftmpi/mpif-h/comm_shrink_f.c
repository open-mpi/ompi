...
OMPI_GENERATE_F77_BINDINGS(PMPIX_COMM_SHRINK, pmpix_comm_shrink, pmpix_comm_shrink_,
                           pmpix_comm_shrink__, ompix_comm_shrink_f,
                           (MPI_Fint * comm, MPI_Fint *newcomm, MPI_Fint *ierr),
                           (comm, newcomm, ierr))

OMPI_GENERATE_F77_BINDINGS(MPIX_COMM_SHRINK, mpix_comm_shrink, mpix_comm_shrink_,
                           mpix_comm_shrink__, ompix_comm_shrink_f,
                           (MPI_Fint * comm, MPI_Fint *newcomm, MPI_Fint *ierr),
                           (comm, newcomm, ierr))
#endif
...
void ompix_comm_shrink_f(MPI_Fint *comm, MPI_Fint *newcomm, MPI_Fint *ierr)
{
    ...
    *ierr = OMPI_INT_2_FINT(PMPIX_Comm_shrink(c_comm, &c_newcomm));
