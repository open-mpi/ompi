...
OMPI_GENERATE_F77_BINDINGS(PMPIX_COMM_AGREE, pmpix_comm_agree, pmpix_comm_agree_,
                           pmpix_comm_agree__, ompix_comm_agree_f,
                           (MPI_Fint * comm, ompi_fortran_logical_t *flag, MPI_Fint *ierr),
                           (comm, flag, ierr))

OMPI_GENERATE_F77_BINDINGS(MPIX_COMM_AGREE, mpix_comm_agree, mpix_comm_agree_, mpix_comm_agree__,
                           ompix_comm_agree_f,
                           (MPI_Fint * comm, ompi_fortran_logical_t *flag, MPI_Fint *ierr),
                           (comm, flag, ierr))
#endif
...
void ompix_comm_agree_f(MPI_Fint *comm, ompi_fortran_logical_t *flag, MPI_Fint *ierr)
{
    ...
    *ierr = OMPI_INT_2_FINT(PMPIX_Comm_agree(c_comm, OMPI_LOGICAL_SINGLE_NAME_CONVERT(flag)));
}
