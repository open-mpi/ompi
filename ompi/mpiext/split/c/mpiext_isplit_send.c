...
#pragma weak OMPI_Isplit_send = POMPI_Isplit_send
#endif
#define OMPI_Isplit_send POMPI_Isplit_send
#endif

static const char FUNC_NAME[] = "OMPI_Isplit_send";


int OMPI_Isplit_send(const void *buf, int count, MPI_Datatype type, int dest,
                     int tag, MPI_Comm comm, MPI_Request *request)
{
    ...
    rc = MCA_PML_CALL(icsend(&convertor, &size, dest, tag, MCA_PML_BASE_SEND_STANDARD, comm, request));
    if (OMPI_SUCCESS != rc) {
        OMPI_ERRHANDLER_RETURN(rc, comm, rc, FUNC_NAME);
    }
    offset += size;
    opal_convertor_set_position(&convertor, &offset);
    rc = MCA_PML_CALL(icsend(&convertor, &size, dest, tag, MCA_PML_BASE_SEND_STANDARD, comm, request+1));
    OMPI_ERRHANDLER_RETURN(rc, comm, rc, FUNC_NAME);
}
