...
def render_obj_handle_converter():
    ...
    builder.generate_obj_handle_convert_fn()
    return buf.getvalue()
...
class TestObjHandleConverter(unittest.TestCase):
    ...
    -- must appear here.  A defect misroutes a handle to the wrong converter or
    (worse) lets a predefined integer fall through as if it were a real
    internal pointer, which crashes at run time.
    """

    # Maps each obj_handle family list in consts to the converter the
    # dispatcher must use for it.  Mirrors ABIConverterBuilder.
    FAMILIES = {
        'RESERVED_COMMUNICATORS': consts.ConvertFuncs.COMM,
        'PREDEFINED_DATATYPES': consts.ConvertFuncs.DATATYPE,
        'PREDEFINED_OPTIONAL_FORTRAN_DATATYPES': consts.ConvertFuncs.DATATYPE,
        'RESERVED_ERRHANDLERS': consts.ConvertFuncs.ERRHANDLER,
        'RESERVED_FILES': consts.ConvertFuncs.FILE,
        'RESERVED_GROUPS': consts.ConvertFuncs.GROUP,
        'RESERVED_OPS': consts.ConvertFuncs.OP,
        'RESERVED_REQUESTS': consts.ConvertFuncs.REQUEST,
        'RESERVED_WINDOWS': consts.ConvertFuncs.WIN,
        'RESERVED_MESSAGES': consts.ConvertFuncs.MESSAGE,
        'RESERVED_INFOS': consts.ConvertFuncs.INFO,
        'RESERVED_SESSIONS': consts.ConvertFuncs.SESSION,
    }

    @classmethod
    def setUpClass(cls):
        cls.text = render_obj_handle_converter()

    def test_null_is_passed_through_untouched(self):
        # A NULL obj_handle is legal (an unbound cvar/pvar) and must never be
        # fed to a converter.
        self.assertIn('if (NULL == obj_handle) {', self.text)

    def test_every_predefined_handle_is_routed(self):
        # This is the regression guard for the original bug: predefined
        # datatypes above the first few (e.g. MPI_INT8_T, MPI_INTEGER8),
        # MPI_GROUP_EMPTY, and MPI_INFO_ENV were silently dropped.  Require
        # the mangled ABI name of *every* predefined handle to be tested.
        for list_name in self.FAMILIES:
            for name in getattr(consts, list_name):
                mangled = util.abi_internal_name(name)
                with self.subTest(handle=name):
                    self.assertIn(mangled, self.text)

    def test_each_family_calls_its_converter(self):
        for list_name, convert_fn in self.FAMILIES.items():
            if not getattr(consts, list_name):
                continue
            with self.subTest(family=list_name):
                self.assertIn(f'{convert_fn}(obj_handle)', self.text)

    def test_unrecognized_handle_falls_through_unchanged(self):
        # User-created handles are real internal pointers with large values and
        # must be returned as-is; the function's final statement does that.
        self.assertTrue(self.text.rstrip().rstrip('}').rstrip().endswith(
            'return obj_handle;'))

    def test_pointer_round_trip_uses_uintptr_t(self):
        # The pointer<->integer comparison must go through uintptr_t so it is
        # correct (and warning-free) on both 32- and 64-bit targets.
        self.assertIn('uintptr_t obj_value = (uintptr_t) obj_handle;', self.text)
        self.assertNotIn('uint64_t', self.text)
