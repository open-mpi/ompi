...
INSTALLED_C_ABI_PROBES = (
    ...
    },
    {
        ...
        "rank_count": 1,
        # Exercises MPI_Win_create and MPI_File_open handle round-trips.
        "requires_feature": ("rma", "mpi_io"),
...
INSTALLED_C_RUNTIME_API_PROBES = (
    ...
    },
    {
        ...
        # operate on the optional, size-specific / KIND-parameterized Fortran
        # numeric types (MPI-5.0 Section 19.1.9).  Per the standard it is
        # erroneous to request a (typeclass, size) or (p, r) not supported by
        # the compiler, and in the ABI the availability of those optional
        # types is a Fortran property (MPI_ABI_SET_FORTRAN_INFO).  Gate this
        # probe with requires_fortran so it is skipped for MPI installations
        # built without Fortran bindings.
        "name": "runtime_datatype_fortran_kinds",
        "family": "type",
        "rank_count": 1,
        "requires_fortran": True,
        "api_names": (
        ...
        "body_file": "cases/c-runtime/runtime_datatype_fortran_kinds.cbody.in",
    },
    {
...
! The is_set assertion above proved the boolean representation is
! already registered in this environment, so a conforming
! implementation must reject this second registration:
! MPI_Abi_set_fortran_booleans is one-shot per MPI-5.  (If the call
! were accepted, the re-query assertions below would fail anyway,
! because they expect the original -- not the inverted -- values.)
if (ierr .ne. MPI_ERR_ABI) stop 9
logical_true = .false.
logical_false = .true.
is_set = .false.
call MPI_Abi_get_fortran_booleans(logical_size, logical_true, &
