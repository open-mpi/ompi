...
def _discover_mpi_installations(include_open_mpi=True, include_mpich=True):
    ...
    if include_open_mpi:
        if ompi_override_mpicc:
            ...
            if ompi_override_mpirun and ompi_override_mpirun_path is None:
                # The mpirun override names a missing or non-executable
                # path.  Record explicit invalid candidates carrying the
                # bad override so validation reports the actual root
                # cause, instead of silently filtering every PATH
                # candidate below and skipping with a generic
                # tools-unavailable message (typos must be hard
    ...
    if include_mpich:
        if mpich_override_mpicc:
            ...
            if mpich_override_mpirun and mpich_override_mpirun_path is None:
                # Same invalid-override diagnostics as the Open MPI
                # branch above.
                include_invalid_mpich = True
