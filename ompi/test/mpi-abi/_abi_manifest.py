...
def _api_family(name):
    ...
    if _is_collective_op(parts[1]):
        return "collective"
    # "get"/"put" are RMA only as the whole name (MPI_Get / MPI_Put):
    # MPI_Get_count, MPI_Get_version, MPI_Get_processor_name, etc. are
    # unconditional core APIs and must not be tagged with the optional
    # rma feature.
    if parts[1] in ("put", "get") and len(parts) == 2:
        return "rma"
    if lower == "mpi_get_accumulate":
        return "rma"
    if parts[1] in ("accumulate", "rput", "rget"):
        return "rma"
...
def _conditional_enabled(builddir, conditional):
    ...
    The ABI runner is usually invoked from ompi/test/mpi-abi via recursive
