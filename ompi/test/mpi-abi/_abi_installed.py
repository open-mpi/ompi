...
def _parse_c_header_deprecated_functions(srcdir):
    ...
    The list lives in ompi_bindings/consts.py, which is the single source of
    truth shared by the two ABI generators (bindings.py and c_header.py); the
    latter simply re-exports it.  Parse the literal from consts.py rather than
    from c_header.py, where it is a plain reference that ast.literal_eval()
    cannot evaluate.
    """
    consts = (srcdir / "ompi" / "mpi" / "bindings" / "ompi_bindings"
              / "consts.py")
    tree = ast.parse(_read_text(consts), filename=str(consts))
    ...
    def add_signature(signature):
        ...
        if parsed is None:
            raise RuntimeError("could not parse expected C signature: {0}".
                               format(signature))
        name = parsed["name"]
        # Compare the parsed function name, not a raw substring of the
        # whole signature: a deprecated name that is a prefix of a live
        # API (e.g. MPI_Info_get vs MPI_Info_get_nkeys) must not drag
        # the live function into the excluded set.
        base_name = name[1:] if name.startswith("PMPI_") else name
        if base_name in deprecated:
            excluded.add(name)
            return
...
def _installed_wrapper_checks(tools, dirs, progress=None):
    ...
    if progress is not None:
        progress.start("installed_abi_pkgconfig_files")
    _append_check(checks, _abi_pkgconfig_files_check(tools), progress)
...
def _abi_pkgconfig_files_check(tools):
    ...
    for name in expected:
        ...
        if placeholders:
            problems.append("{0}: unsubstituted placeholders {1}".format(
                name, placeholders))
        libs_lines = [line for line in text.splitlines()
                      if line.startswith("Libs:")]
        if not libs_lines or "-lmpi_abi" not in libs_lines[0]:
            problems.append("{0}: Libs does not link -lmpi_abi".format(name))
    if problems:
        return _fail(
            check_name,
            "installed ABI pkg-config files are broken",
...
def _c_probe_source(srcdir, case, body):
    ...
    """
    template = _read_text(srcdir / "ompi" / "test" / "mpi-abi" /
