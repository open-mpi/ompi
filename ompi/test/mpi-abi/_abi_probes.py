...
def _strip_c_casts(value):
    # Only strip a parenthesized token as a cast when it plausibly
    # names a type.  In these headers every type name contains a
    # lowercase letter (int, size_t, MPI_Datatype, MPI_Aint, ...),
    # while ALL-CAPS identifiers are MPI constants.  A fully
    # permissive [A-Za-z_]\w* pattern also matched parenthesized
    # constants used as operands -- e.g. in
    # "#define MPI_FOO (MPI_BAR) - 1" it deleted (MPI_BAR) and
    # silently evaluated the constant as -1, and it defeated the
    # MPI_VERSION guard in _extract_integer_constant by removing
    # "(MPI_VERSION)" before the guard could see it.
    type_name = r"(?:const\s+|volatile\s+)?[A-Za-z_]*[a-z][A-Za-z0-9_]*"
...
def _parse_header_constants(path):
    ...
    for line in path.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        if match is not None:
            ...
            # A parenthesized alias -- "#define MPI_A (MPI_B)" -- is
            # still an alias.
            ...
            elif re.match(r"^MPI\w+$", alias_value):
                aliases[name] = alias_value
            else:
                unparsed[name] = value
            continue
