...
def _abi_converter_checks(srcdir, builddir):
    ...
    handle_convert_header = _first_existing_build_source_path(
        srcdir, builddir, c_relative / "abi_handle_convert.h")
    missing_files = []
    missing_patterns = []
    ...
    required_files = (
        ...
        (handle_convert_header, c_relative / "abi_handle_convert.h"),
    )
    ...
    handle_convert_text = _read_text(handle_convert_header)

    # The shared offset/sentinel translation now lives in
    # abi_handle_convert.h; the per-handle converters below must call these
    # helpers instead of open-coding the OMPI_ABI_HANDLE_BASE_OFFSET math.
    handle_convert_requirements = [
        ("uses ABI base offset", "OMPI_ABI_HANDLE_BASE_OFFSET"),
        ("predefined-handle helper", "ompi_abi_handle_int_is_predefined"),
        ("ABI-handle-to-index helper", "ompi_abi_handle_int_to_index"),
        ("index-to-ABI-handle helper", "ompi_abi_index_to_handle_int"),
    ]
    for label in _require_substrings(handle_convert_text,
                                     handle_convert_requirements):
        missing_patterns.append({
            "file": str(handle_convert_header),
            "pattern": label,
        })
        ...
        if to_file is not None:
            ...
            to_requirements = [
                ...
                ("preserve ABI integer handles",
                 "ompi_abi_handle_int_is_predefined("),
                ("apply ABI base offset",
                 "ompi_abi_index_to_handle_int("),
            ]
