...
from _abi_installed import (
    ...
    _abi_library_preference_key,
    _darwin_otool_install_name,
    _defined_nm_symbols,
    _linux_library_line,
    _linux_resolved_library,
    _parse_c_header_typedef_names,
    _parse_c_signature,
)
from _abi_tables import (
    INSTALLED_FORTRAN_COMPILE_PROBES,
    INSTALLED_FORTRAN_RUNTIME_PROBES,
    LEGACY_ATTRIBUTE_API_NAMES,
)
...
class InstalledParserTests(unittest.TestCase):
    """Canned-fixture checks for the pure parsers in _abi_installed."""

    def test_defined_nm_symbols(self):
        output = "\n".join([
            "0000000000001000 T _MPI_Send",
            "0000000000002000 D MPI_COMM_WORLD_STORAGE",
            "                 U MPI_Recv",
            "0000000000003000 w _weak_undefined",
            "0000000000004000 W MPI_Weak_defined",
            "not-a-symbol-line",
        ])
        symbols = _defined_nm_symbols(output)
        self.assertIn("MPI_Send", symbols)
        self.assertIn("MPI_COMM_WORLD_STORAGE", symbols)
        self.assertIn("MPI_Weak_defined", symbols)
        self.assertNotIn("MPI_Recv", symbols)
        self.assertNotIn("weak_undefined", symbols)

    def test_darwin_otool_install_name(self):
        line = ("\t/opt/open mpi/lib/libmpi_abi.0.dylib "
                "(compatibility version 1.0.0, current version 1.0.0)")
        self.assertEqual(_darwin_otool_install_name(line),
                         "/opt/open mpi/lib/libmpi_abi.0.dylib")
        self.assertEqual(_darwin_otool_install_name("  /plain/path.dylib  "),
                         "/plain/path.dylib")

    def test_linux_library_line_and_resolution(self):
        output = "\n".join([
            "\tlinux-vdso.so.1 (0x0000ffff8dfcd000)",
            "\tlibmpi_abi.so.0 => /opt/ompi/lib/libmpi_abi.so.0 "
            "(0x0000ffff8dc00000)",
            "\tlibmissing.so => not found",
        ])
        self.assertEqual(
            _linux_resolved_library(output, "libmpi_abi.so"),
            "/opt/ompi/lib/libmpi_abi.so.0")
        self.assertIsNone(_linux_resolved_library(output, "libmissing.so"))
        self.assertIsNone(_linux_resolved_library(output, "libabsent.so"))
        self.assertIsNotNone(_linux_library_line(output, "libmissing.so"))

    def test_abi_library_basename_matches(self):
        for basename in ("libmpi_abi.so", "libmpi_abi.so.0",
                         "libmpi_abi.dylib", "libmpi_abi.0.dylib"):
            self.assertTrue(
                _abi_library_basename_matches(basename, "mpi_abi"),
                basename)
        self.assertTrue(
            _abi_library_basename_matches("libmpi_abi.a", "mpi_abi"))
        self.assertFalse(
            _abi_library_basename_matches("libmpi_abi.a", "mpi_abi",
                                          include_static=False))
        for basename in ("libmpi.so", "libmpi_abi.txt", "mpi_abi.so"):
            self.assertFalse(
                _abi_library_basename_matches(basename, "mpi_abi"),
                basename)

    def test_abi_library_preference_key_orders_dynamic_first(self):
        ordered = sorted(
            [Path("libmpi_abi.a"), Path("libmpi_abi.so"),
             Path("libmpi_abi.0.dylib")],
            key=_abi_library_preference_key)
        self.assertEqual([p.name for p in ordered],
                         ["libmpi_abi.0.dylib", "libmpi_abi.so",
                          "libmpi_abi.a"])

    def test_parse_c_signature(self):
        parsed = _parse_c_signature(
            "int  MPI_Send(const void *buf, int count, MPI_Datatype "
            "datatype, int dest, int tag, MPI_Comm comm)")
        self.assertEqual(parsed["name"], "MPI_Send")
        self.assertEqual(parsed["return_type"], "int")
        self.assertTrue(parsed["args"].startswith("const void* buf"))
        parsed = _parse_c_signature("double PMPI_Wtime(void)")
        self.assertEqual(parsed["name"], "PMPI_Wtime")
        self.assertIsNone(_parse_c_signature("not a signature"))

    def test_parse_c_header_typedef_names(self):
        with _header_file("""
...
typedef int not_an_mpi_name;
""") as header:
            names = _parse_c_header_typedef_names(header)
        self.assertIn("MPI_Comm", names)
        self.assertIn("MPI_Comm_copy_attr_function", names)
        self.assertIn("MPI_Status", names)
        self.assertNotIn("not_an_mpi_name", names)


class FortranProbeContractTests(unittest.TestCase):
    """Keep the Fortran probes' coverage declarations honest.

    The C runtime probes get a bidirectional declared-vs-called
    preflight check at run time; the Fortran tables had no equivalent,
    which let declared-vs-called drift land silently.  Fortran calls
    procedures both through "call MPI_..." statements and as function
    references (e.g. MPI_Wtime()), so the strict direction here is
    called-but-undeclared; the reverse direction only requires that a
    declared name appear textually in the probe body.
    """

    CALL_RE = re.compile(r"(?im)^\s*call\s+(MPI_\w+)")

    def _check(self, probes):
        for probe in probes:
            body = probe.get("body", "")
            declared = set(probe.get("api_names", ()))
            declared |= set(probe.get("support_api_names", ()))
            called = {name for name in self.CALL_RE.findall(body)}
            called = {name for name in called
                      if not name.upper().startswith("MPI_T_")}
            undeclared = {name for name in called
                          if name not in declared}
            self.assertFalse(
                undeclared,
                "{0}: called but not declared: {1}".format(
                    probe["name"], sorted(undeclared)))
            lowered = body.lower()
            stale = {name for name in declared
                     if name.lower() not in lowered}
            self.assertFalse(
                stale,
                "{0}: declared but never referenced: {1}".format(
                    probe["name"], sorted(stale)))

    def test_runtime_probe_declarations(self):
        self._check(INSTALLED_FORTRAN_RUNTIME_PROBES)

    def test_compile_probe_declarations(self):
        self._check(INSTALLED_FORTRAN_COMPILE_PROBES)
