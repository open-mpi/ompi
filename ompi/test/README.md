# OMPI layer unit tests (`ompi/test/`)
...
test harness (`support.{c,h}`), so they build and run via `make check`
run solely inside `ompi/`, with no dependency on the top-level `test/`
tree.
...
make -C ompi/test check   # build + run the unit tests
```
...
1. Write `ompi/test/<module>.c` (include `"support.h"`; pick partial or
