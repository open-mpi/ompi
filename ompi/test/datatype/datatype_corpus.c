...
/*
 * A single-level 8-byte-element hvector whose block-to-block stride is a multiple of the 4-byte
 * float alignment but NOT of the 8-byte element size (a small trailing gap of GAP_BYTES that is
 * not a whole element).  This is the only datatype in the suite that drives the pack/unpack
 * movers onto their byte-exact "unaligned" advance path
 * (elem->extent % sizeof(element) != 0); every other shape here advances in whole elements.
 * BLOCKLEN stays <= 8 so the copy goes through the unrolled element mover rather than
 * the medium/large vectorized loops.
 *
 * The element is MPI_Type_contiguous(2, MPI_FLOAT), not MPI_C_FLOAT_COMPLEX.
 * mpi.h only defines the latter under HAVE_FLOAT__COMPLEX; two contiguous
 * floats are the same 8-byte element / 28-byte stride without C complex.
 */
#define COMPLEX_HVECTOR_BLOCKS     2048
#define COMPLEX_HVECTOR_BLOCKLEN   3
#define COMPLEX_HVECTOR_GAP_BYTES  4
#define COMPLEX_HVECTOR_ELEM_BYTES (2 * sizeof(float))

static MPI_Datatype create_complex_hvector_ddt(void)
{
    MPI_Datatype elem, dt;
    MPI_Aint lb, elem_extent, stride;

    MPI_Type_contiguous(2, MPI_FLOAT, &elem);
    MPI_Type_get_extent(elem, &lb, &elem_extent);
    stride = (MPI_Aint) COMPLEX_HVECTOR_BLOCKLEN * elem_extent + COMPLEX_HVECTOR_GAP_BYTES;
    MPI_Type_create_hvector(COMPLEX_HVECTOR_BLOCKS, COMPLEX_HVECTOR_BLOCKLEN, stride, elem, &dt);
    MPI_Type_commit(&dt);
    MPI_Type_free(&elem);
    return dt;
}
...
{
    const size_t elem_size = COMPLEX_HVECTOR_ELEM_BYTES;
...
static const corpus_desc_t corpus_desc[] = {
    ...
    {"struct_constant_gap_resized", build_struct_constant_gap_resized, NULL,
     ...
     DT_TRAIT_CONTIGUOUS | DT_TRAIT_RESIZED | DT_TRAIT_MIXED_TYPES},
    ...
    {"ddtbench_fft2d_scatter", create_ddtbench_fft2d_scatter_ddt, NULL,
     ...
     DT_TRAIT_HAS_GAPS | DT_TRAIT_RESIZED | DT_TRAIT_OVERLAP},
    ...
    {"ddtbench_lammps_full", NULL, create_ddtbench_lammps_full_ddt,
     ...
     DT_TRAIT_HAS_GAPS | DT_TRAIT_PAIR | DT_TRAIT_RESIZED},
    ...
    {"adv_single_iter_gap", create_adv_single_iter_gap, NULL,
     ...
     DT_TRAIT_CONTIGUOUS | DT_TRAIT_SINGLE_ITER | DT_TRAIT_RESIZED},
