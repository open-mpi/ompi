...
/* Hard cap on --trials: print_result() allocates three VLAs of this length
 * on the stack, so the value must stay small enough to avoid a stack overflow
 * regardless of the shell's ulimit. --cycles and --warmups are not used for
 * stack allocations; they share the INT_MAX bound of --min-work-bytes. */
...
static void print_usage(FILE *stream, const char *program_name)
{
    ...
    fprintf(stream, "--validate runs a short pack/unpack self-check against the by-hand reference,\n"
                    "           checks that a packed-endpoint signature type packs to the same stream,\n"
                    "           and prints '# VALIDATION <name> pack=... unpack=... packed_send=... packed_recv=...'\n");
...
static int parse_args(int argc, char *argv[], test_mask_t *run_tests)
{
    ...
    for (int i = 1; i < argc; ++i) {
        if ((0 == strcmp(argv[i], "--help")) || (0 == strcmp(argv[i], "-h"))) {
            ...
            if (0 != parse_integer_option("--cycles", argv[i] + strlen("--cycles="), 1,
                                          INT_MAX, &cycles)) {
                return -1;
            }
            ...
            ++i;
            if (0 != parse_integer_option("--cycles", argv[i], 1, INT_MAX, &cycles)) {
                return -1;
            }
        } else if (0 == strncmp(argv[i], "--trials=", strlen("--trials="))) {
            if (0 != parse_integer_option("--trials", argv[i] + strlen("--trials="), 2,
                                          MAX_TRIALS, &trials)) {
                return -1;
            }
            ...
            ++i;
            if (0 != parse_integer_option("--trials", argv[i], 2, MAX_TRIALS, &trials)) {
                return -1;
            }
            ...
            ++i;
            if (0 != parse_integer_option("--warmups", argv[i], 0, INT_MAX, &warmups)) {
                return -1;
            }
            ...
            ++i;
            if (0 != parse_integer_option("--min-work-bytes", argv[i], 0, INT_MAX,
                                          &min_work_bytes)) {
                return -1;
            }
...
static void print_result(const char *operation, int length, int num_trials, const double *timers)
{
    ...
    if (fallback) {
        printf("# Not enough good timing data after outlier filtering (%d/%d in-fence); "
               "using middle-half (%d/%d retained); rerun with a larger --trials value\n",
               filtered_count, num_trials, good_count, num_trials);
    }
 ...
 * comment line ("# VALIDATION <name> pack=... unpack=... packed_send=... packed_recv=...") so it is
 * ignored by the timing parsers but trivially greppable.  PASS/FAIL/SKIP is reported per check;
 * SKIP means the datatype has no by-hand reference for that pack/unpack direction.
 */
...
/*
 * MPI_Pack of a compact signature type applied to the stream produced by packing `original` must
 * be a byte-for-byte copy of that stream. That fails if the reconstructed typemap has holes or a
 * different size; a same-size permutation of basic types is not visible to homogeneous pack, but
 * it is the check the packed-endpoint path actually needs (the compact type is used as the
 * datatype of an already-packed buffer).
 */
static int packed_signature_matches_original(MPI_Datatype original, MPI_Datatype packed_type,
                                             int count, char *source, int original_pack_capacity)
{
    int orig_type_size = 0, packed_pack_capacity = 0, pos_orig = 0, pos_packed = 0;
    size_t orig_packed_size, packed_span, stream_orig_bytes;
    char *stream_orig = NULL, *stream_packed = NULL;
    int match;

    MPI_Type_size(original, &orig_type_size);
    orig_packed_size = (size_t) orig_type_size * (size_t) count;
    if (0 == orig_packed_size) {
        return 1;
    }

    packed_span = datatype_count_span(packed_type, count);
    MPI_Pack_size(count, packed_type, MPI_COMM_WORLD, &packed_pack_capacity);
    stream_orig_bytes = ((size_t) original_pack_capacity > packed_span)
                            ? (size_t) original_pack_capacity
                            : packed_span;
    stream_orig = (char *) malloc(stream_orig_bytes);
    stream_packed = (char *) malloc((size_t) packed_pack_capacity);
    if ((NULL == stream_orig) || (NULL == stream_packed)) {
        free(stream_orig);
        free(stream_packed);
        MPI_Abort(MPI_COMM_WORLD, MPI_ERR_NO_MEM);
    }

    memset(stream_orig, 0, stream_orig_bytes);
    memset(stream_packed, 0, (size_t) packed_pack_capacity);
    MPI_Pack(source, count, original, stream_orig, original_pack_capacity, &pos_orig,
             MPI_COMM_WORLD);
    MPI_Pack(stream_orig, count, packed_type, stream_packed, packed_pack_capacity, &pos_packed,
             MPI_COMM_WORLD);
    match = (((size_t) pos_orig == orig_packed_size) && ((size_t) pos_packed == orig_packed_size)
             && (0 == memcmp(stream_orig, stream_packed, orig_packed_size)));
    free(stream_orig);
    free(stream_packed);
    return match;
}
                              ...
                              datatype_byhand_fn_t unpack_byhand_fn)
{
    ...
    const char *packed_send_status = "FAIL";
    const char *packed_recv_status = "FAIL";
    int stype_size = 0, capacity = 0, recv_pack_capacity = 0;
    ...
    MPI_Pack_size(count, rddt, MPI_COMM_WORLD, &recv_pack_capacity);
    ...
    packed_send_status = packed_signature_matches_original(sddt, packed_send_type, count, sbuf,
                                                           capacity)
                             ? "PASS"
                             : "FAIL";
    for (size_t i = 0; i < recv_span; ++i) {
        rbuf_mpi[i] = (char) (i % 251);
    }
    packed_recv_status = packed_signature_matches_original(rddt, packed_recv_type, count, rbuf_mpi,
                                                           recv_pack_capacity)
                             ? "PASS"
                             : "FAIL";

    printf("# VALIDATION %s pack=%s unpack=%s packed_send=%s packed_recv=%s\n", datatype_name,
           pack_status, unpack_status, packed_send_status, packed_recv_status);
    fflush(stdout);
    ...
    if ((0 == strcmp(pack_status, "FAIL")) || (0 == strcmp(unpack_status, "FAIL"))
        || (0 == strcmp(packed_send_status, "FAIL"))
        || (0 == strcmp(packed_recv_status, "FAIL"))) {
        fprintf(stderr,
                "VALIDATION FAILED for %s (pack=%s unpack=%s packed_send=%s packed_recv=%s)\n",
                datatype_name, pack_status, unpack_status, packed_send_status, packed_recv_status);
                      ...
                      int rcount, void *rbuf)
{
    ...
    for (t = 0; t < warmups + trials; t++) {
        ...
        if (t >= warmups) {
            record_communication_timer((MPI_Wtime() - start) / num_cycles, &timers[t - warmups]);
        }
    }
    if (ROLE_TARGET != role) {
                           ...
                           datatype_byhand_fn_t unpack_byhand_fn)
{
    ...
    if (0 == stype_size) {
        fprintf(stderr, "Datatype %s has zero size\n", datatype_name);
        MPI_Abort(MPI_COMM_WORLD, MPI_ERR_TYPE);
    }
...
int main(int argc, char *argv[])
{
     ...
     * dump_datatypes is intentionally omitted: datatype optimization is
     * deterministic, so dumping on rank 0 alone is sufficient. */
    if (0 == rank) {
