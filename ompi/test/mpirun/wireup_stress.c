 ...
 * wires neighbors, bcast from the root, reduce into the root, alltoall
 * the complete graph.  Each runs as the MPI collective, which may use
 * an overlay topology, and as a point-to-point emulation posting every
 * edge before waiting.  Ring and rma are point-to-point only.
 *
 * The rma pattern differs in kind: a window wires its peers in its own
 * constructor, and every rank has to pick the same BTL to carry it,
 * because that choice sizes the state each rank exposes.  A rank that
 * could not enumerate a peer would drop a BTL its peers kept, and
 * nothing downstream detects it -- so the window runs first.
 *
 * usage: wireup_stress [pattern] [flavor]
 *   pattern: ring | bcast | reduce | alltoall | rma | all  (default: all)
 *   flavor:  p2p   | coll   | all                          (default: all)
 *
 * Endpoints persist for the life of the job, so a combined run only
 * stresses establishment on the first pattern; launch each pattern in
 * its own mpirun to observe first-use wire-up.
 */
...
/* Neighbor-only: posting both halves before the wait constructs both
   neighbor endpoints at once. */
...
/* One-to-many: wires outward from the root. */
...
/* Many-to-one: wires into the root. */
...
/* Many-to-many: wires the complete graph. */
...
/* The window is created before anything else has wired a peer, so its
   constructor is what resolves them.  Put, accumulate and get on every
   rank, self included, so both the data and the atomic path are used. */
static void do_rma(int rank, int size)
{
    ...
    /* my own slot on every rank, so no two origins write the same word;
       the accumulator at [size] is the one word they all touch */
    for (int dest = 0; dest < size; ++dest) {
...
int main(int argc, char *argv[])
{
    ...
    /* Before any barrier: what the window has to cope with is peers that
       nothing has wired yet. */
