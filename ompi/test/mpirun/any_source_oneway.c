 ...
 * $HEADER$
 *
 * Rank 0 posts MPI_ANY_SOURCE and never sends, so traffic is one-way.
 * With lazy SM endpoints the receiver must still attach the sender on
 * the incoming item to translate the fragment and write completion
 * back; a wild recv must not add_proc every rank of the communicator.
 */
