...
#error "Please provide the name for the memory copy operation"
#endif  /* !defined((MEM_OP_NAME) */
#if !defined(MEM_OP)
#error "Please provide the memory copy operation (memcpy or memmove)"
...
static inline int32_t _copy_content_same_ddt( const ompi_datatype_t* datatype, int32_t count,
                                              char* destination_base, char* source_base )
{
    ...
    if( datatype->flags & DT_FLAG_CONTIGUOUS ) {
        ...
        if( (ptrdiff_t)datatype->size == extent ) {  /* all contiguous == no gaps around */
            ...
            size_t memop_chunk =  ompi_datatype_memcpy_block_size;
            while( total_length > 0 ) {
                if( memop_chunk > total_length ) memop_chunk = total_length;
                OMPI_DDT_SAFEGUARD_POINTER( destination, memop_chunk,
                                            (unsigned char*)destination_base, datatype, count );
                OMPI_DDT_SAFEGUARD_POINTER( source, memop_chunk,
