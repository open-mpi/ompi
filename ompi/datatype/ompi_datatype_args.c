...
static inline int __ompi_datatype_pack_description(ompi_datatype_t *datatype, void **packed_buffer,
                                                   int *next_index);
static ompi_datatype_t *__ompi_datatype_create_from_args(int32_t *i, ptrdiff_t *a,
...
#define ALLOC_ARGS(PDATA, IC, AC, DC)                                                             \
    do {                                                                                          \
        int length = sizeof(ompi_datatype_args_t) + (IC) * sizeof(int) + (AC) * sizeof(ptrdiff_t) \
                     + (DC) * sizeof(MPI_Datatype);                                               \
        ...
        if (pArgs->ci == 0)                                                                       \
            ...
            pArgs->i = (int *) buf;                                                               \
        pArgs->ref_count = 1;                                                                     \
        pArgs->total_pack_size = (4 + (IC) + (DC)) * sizeof(int) + (AC) * sizeof(ptrdiff_t);      \
        (PDATA)->args = (void *) pArgs;                                                           \
        (PDATA)->packed_description = 0;                                                          \
    } while (0)

int32_t ompi_datatype_set_args(ompi_datatype_t *pData, int32_t ci, const int32_t **i, int32_t ca,
                               const ptrdiff_t *a, int32_t cd, ompi_datatype_t *const *d,
                               int32_t type)
{
    int pos;
...
int32_t ompi_datatype_get_args(const ompi_datatype_t *pData, int32_t which, int32_t *ci, int32_t *i,
                               int32_t *ca, ptrdiff_t *a, int32_t *cd, ompi_datatype_t **d,
                               int32_t *type)
{
    ompi_datatype_args_t *pArgs = (ompi_datatype_args_t *) pData->args;
...
int32_t ompi_datatype_copy_args(const ompi_datatype_t *source_data, ompi_datatype_t *dest_data)
{
                                                   ...
                                                   int *next_index)
{
    ...
    if (MPI_COMBINER_DUP == args->create_type) {
        ...
        return __ompi_datatype_pack_description(temp_data, packed_buffer, next_index);
    }
    ...
    for (i = 0; i < args->cd; i++) {
        ...
        if (ompi_datatype_is_predefined(temp_data)) {
            ...
            (*next_index)++;
            __ompi_datatype_pack_description(temp_data, (void **) &next_packed, next_index);
        }
    }
    *packed_buffer = next_packed;
    return OMPI_SUCCESS;
...
int ompi_datatype_get_pack_description(ompi_datatype_t *datatype, const void **packed_buffer)
{
    ompi_datatype_args_t *args = (ompi_datatype_args_t *) datatype->args;
    ...
    if (NULL == packed_description) {
        void *_tmp_ptr = NULL;
        if (opal_atomic_compare_exchange_strong_ptr(&datatype->packed_description,
                                                    (intptr_t *) &_tmp_ptr, 1)) {
            if (ompi_datatype_is_predefined(datatype)) {
            ...
            if (!ompi_datatype_is_predefined(datatype)) {
                 ...
                 */
                assert(args->total_pack_size
                       >= (uintptr_t)((char *) recursive_buffer - (char *) packed_description));
                args->total_pack_size = (uintptr_t)((char *) recursive_buffer
                                                    - (char *) packed_description);
            }
...
__ompi_datatype_create_from_packed_description(void **packed_buffer,
                                               const struct ompi_proc_t *remote_processor)
{
...
#endif
    array_of_datatype = (ompi_datatype_t **) malloc(sizeof(ompi_datatype_t *) * number_of_datatype);
    ...
    for (i = 0; i < number_of_datatype; i++) {
        ...
        array_of_datatype[i] = __ompi_datatype_create_from_packed_description((void *
                                                                                   *) &next_buffer,
                                                                              remote_processor);
        if (NULL == array_of_datatype[i]) {
             ...
             * datatype.
             */
            array_of_datatype[i] = (ompi_datatype_t *)
                ompi_datatype_basicDatatypes[OPAL_DATATYPE_INT1]; /*XXX TODO */
...
#endif
    datatype = __ompi_datatype_create_from_args(array_of_length, array_of_disp, array_of_datatype,
                                                create_type);
                                                         ...
                                                         ompi_datatype_t **d, int32_t type)
{
    ...
    case MPI_COMBINER_CONTIGUOUS:
        ...
        ompi_datatype_set_args(datatype, 1, (const int **) &i, 0, NULL, 1, d,
    ...
    case MPI_COMBINER_INDEXED: /* TO CHECK */
        ompi_datatype_create_indexed(i[0], &(i[1]), &(i[1 + i[0]]), d[0], &datatype);
        {
            ...
            ompi_datatype_set_args(datatype, 2 * i[0] + 1, a_i, 0, NULL, 1, d,
    ...
    case MPI_COMBINER_INDEXED_BLOCK:
        ...
        {
            const int *a_i[3] = {&i[0], &i[1], &i[2]};
            ompi_datatype_set_args(datatype, i[0] + 2, a_i, 0, NULL, 1, d,
    ...
    case MPI_COMBINER_STRUCT:
        ...
        /******************************************************************/
    case MPI_COMBINER_SUBARRAY:
        ompi_datatype_create_subarray(i[0], &i[1 + 0 * i[0]], &i[1 + 1 * i[0]], &i[1 + 2 * i[0]],
                                      i[1 + 3 * i[0]], d[0], &datatype);
        {
            const int *a_i[5] = {&i[0], &i[1 + 0 * i[0]], &i[1 + 1 * i[0]], &i[1 + 2 * i[0]],
                                 &i[1 + 3 * i[0]]};
            ompi_datatype_set_args(datatype, 3 * i[0] + 2, a_i, 0, NULL, 1, d,
    ...
    case MPI_COMBINER_DARRAY:
        ompi_datatype_create_darray(i[0] /* size */, i[1] /* rank */, i[2] /* ndims */,
                                    &i[3 + 0 * i[2]], &i[3 + 1 * i[2]], &i[3 + 2 * i[2]],
                                    &i[3 + 3 * i[2]], i[3 + 4 * i[2]], d[0], &datatype);
        {
            const int *a_i[8] = {&i[0],
                                 &i[1],
                                 &i[2],
                                                              ...
                                                              struct ompi_proc_t *remote_processor)
{
    ...
    datatype = __ompi_datatype_create_from_packed_description(packed_buffer, remote_processor);
    if (NULL == datatype) {
        return NULL;
    }
