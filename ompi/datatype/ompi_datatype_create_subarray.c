...
int32_t ompi_datatype_create_subarray(int ndims,
                                      const ompi_count_array_t size_array,
                                      const ompi_count_array_t subsize_array,
                                      const ompi_count_array_t start_array,
                                      int order,
                                      ...
                                      ompi_datatype_t** newtype)
{
    ...
    if( ndims < 2 ) {
        ...
        ompi_datatype_create_contiguous( ompi_count_array_get(subsize_array, 0), oldtype, &last_type );
        size = ompi_count_array_get(size_array, 0);
        displ = ompi_count_array_get(start_array, 0);
     ...
     */
    ompi_datatype_create_vector( ompi_count_array_get(subsize_array, i+step),
                                 ompi_count_array_get(subsize_array, i),
                                 ompi_count_array_get(size_array, i),
                                 oldtype, newtype );

    last_type = *newtype;
    size = (MPI_Aint)ompi_count_array_get(size_array, i) * (MPI_Aint)ompi_count_array_get(size_array, i+step);
    displ = (MPI_Aint)ompi_count_array_get(start_array, i)
            + (MPI_Aint)ompi_count_array_get(start_array, i+step) * (MPI_Aint)ompi_count_array_get(size_array, i);
    for( i += 2 * step; i != end_loop; i += step ) {
        ompi_datatype_create_hvector( ompi_count_array_get(subsize_array, i), 1, size * extent,
                                      last_type, newtype );
        ompi_datatype_destroy( &last_type );

        displ += size * ompi_count_array_get(start_array, i);
        size *= ompi_count_array_get(size_array, i);
        last_type = *newtype;
    }
