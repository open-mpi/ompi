...
int32_t ompi_datatype_create_subarray(int ndims, int const *size_array, int const *subsize_array,
                                      ...
                                      const ompi_datatype_t *oldtype, ompi_datatype_t **newtype)
{
     ...
     */
    ompi_datatype_create_vector(subsize_array[i + step], subsize_array[i], size_array[i], oldtype,
                                newtype);
    ...
    for (i += 2 * step; i != end_loop; i += step) {
        ompi_datatype_create_hvector(subsize_array[i], 1, size * extent, last_type, newtype);
