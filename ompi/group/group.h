 ...
 */
OMPI_DECLSPEC int ompi_group_translate_ranks(ompi_group_t *group1, int n_ranks, const int *ranks1,
 ...
 */
int ompi_group_translate_ranks_sporadic(ompi_group_t *group1, int n_ranks, const int *ranks1,
                                        ompi_group_t *group2, int *ranks2);
int ompi_group_translate_ranks_sporadic_reverse(ompi_group_t *group1, int n_ranks,
                                                const int *ranks1, ompi_group_t *group2,
                                                int *ranks2);
int ompi_group_translate_ranks_strided(ompi_group_t *group1, int n_ranks, const int *ranks1,
                                       ompi_group_t *group2, int *ranks2);
int ompi_group_translate_ranks_strided_reverse(ompi_group_t *group1, int n_ranks, const int *ranks1,
                                               ompi_group_t *group2, int *ranks2);
int ompi_group_translate_ranks_bmap(ompi_group_t *group1, int n_ranks, const int *ranks1,
                                    ompi_group_t *group2, int *ranks2);
int ompi_group_translate_ranks_bmap_reverse(ompi_group_t *group1, int n_ranks, const int *ranks1,
                                            ompi_group_t *group2, int *ranks2);
 ...
 */
int ompi_group_incl_plist(ompi_group_t *group, int n, const int *ranks, ompi_group_t **new_group);
...
int ompi_group_incl_bmap(ompi_group_t *group, int n, const int *ranks, ompi_group_t **new_group);
 ...
 * to NULL.
 */
static inline struct ompi_proc_t *ompi_group_dense_lookup(ompi_group_t *group, const int peer_id,
                                                          const bool allocate)
{
    ompi_proc_t *proc;
    ...
    if (OPAL_UNLIKELY(ompi_proc_is_sentinel(proc))) {
        ...
        ompi_proc_t *real_proc = (ompi_proc_t *) ompi_proc_for_name(
            ompi_proc_sentinel_to_name((uintptr_t) proc));

        if (opal_atomic_compare_exchange_strong_ptr((opal_atomic_intptr_t
 ...
 */
static inline ompi_proc_t *ompi_group_get_proc_ptr(ompi_group_t *group, int rank,
                                                   const bool allocate)
{
...
static inline int ompi_group_proc_lookup_rank(ompi_group_t *group, ompi_proc_t *proc)
{
    ...
    for (i = 0; i < np; i++) {
        ...
        if (OPAL_LIKELY(!ompi_proc_is_sentinel(p))) {
            ...
            if (OPAL_EQUAL
                == ompi_rte_compare_name_fields(OMPI_RTE_CMP_ALL, &proc->super.proc_name, &name))
                return rank;
        }
    }
    return MPI_PROC_NULL;
