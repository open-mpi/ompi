...
int ompi_group_translate_ranks_sporadic(ompi_group_t *parent_group, int n_ranks, const int *ranks1,
                                        ompi_group_t *child_group, int *ranks2)
{
    int i, count, j;
    for (j = 0; j < n_ranks; j++) {
                ...
                if (child_group->sparse_data.grp_sporadic.grp_sporadic_list[i].rank_first
                    ...
                    && ranks1[j]
                           <= child_group->sparse_data.grp_sporadic.grp_sporadic_list[i].rank_first
                                  + child_group->sparse_data.grp_sporadic.grp_sporadic_list[i]
                                        .length
                                  - 1) {
...
int ompi_group_translate_ranks_sporadic_reverse(ompi_group_t *child_group, int n_ranks,
                                                const int *ranks1, ompi_group_t *parent_group,
                                                int *ranks2)
{
    int i, j, count;
    ...
    for (j = 0; j < n_ranks; j++) {
        if (MPI_PROC_NULL == ranks1[j]) {
            ...
            for (i = 0; i < child_group->sparse_data.grp_sporadic.grp_sporadic_list_len; i++) {
                if (ranks1[j]
                    > (count + child_group->sparse_data.grp_sporadic.grp_sporadic_list[i].length
                       - 1)) {
                    count = count
                            + child_group->sparse_data.grp_sporadic.grp_sporadic_list[i].length;
                } else {
                    ranks2[j] = child_group->sparse_data.grp_sporadic.grp_sporadic_list[i]
                                    .rank_first
                                + (ranks1[j] - count);
                    break;
                }
...
int ompi_group_incl_spor(ompi_group_t *group, int n, const int *ranks, ompi_group_t **new_group)
{
    /* local variables */
    ...
    new_group_pointer->sparse_data.grp_sporadic.grp_sporadic_list[j].rank_first = ranks[0];
    new_group_pointer->sparse_data.grp_sporadic.grp_sporadic_list[j].length = 1;
    ...
    for (i = 1; i < n; i++) {
        if (ranks[i] == ranks[i - 1] + 1) {
            ...
            j++;
            new_group_pointer->sparse_data.grp_sporadic.grp_sporadic_list[j].rank_first = ranks[i];
            new_group_pointer->sparse_data.grp_sporadic.grp_sporadic_list[j].length = 1;
        }
    }
    ...
    for (i = 0; i < new_group_pointer->sparse_data.grp_sporadic.grp_sporadic_list_len; i++) {
        proc_count = proc_count
                     + new_group_pointer->sparse_data.grp_sporadic.grp_sporadic_list[i].length;
    }
    ...
    ompi_group_translate_ranks(group_pointer, 1, &my_group_rank, new_group_pointer,
                               &new_group_pointer->grp_my_rank);
