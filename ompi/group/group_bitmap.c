...
int ompi_group_translate_ranks_bmap(ompi_group_t *parent_group, int n_ranks, const int *ranks1,
                                    ompi_group_t *child_group, int *ranks2)
{
    ...
    for (j = 0; j < n_ranks; j++) {
        if (MPI_PROC_NULL == ranks1[j]) {
            ...
            if (tmp
                == (child_group->sparse_data.grp_bitmap.grp_bitmap_array[(int) (m / BSIZE)]
                    & (1 << (m % BSIZE)))) {
                /*
                ...
                for (i = 0; i <= (int) (m / BSIZE); i++) {
                    for (k = 0; k < BSIZE; k++) {
                        tmp1 = (1 << k);
                        if (tmp1
                            == (child_group->sparse_data.grp_bitmap.grp_bitmap_array[i]
                                & (1 << k))) {
                            count++;
                        }
...
int ompi_group_translate_ranks_bmap_reverse(ompi_group_t *child_group, int n_ranks,
        ...
        if (MPI_PROC_NULL == ranks1[j]) {
            ...
            for (i = 0; i < child_group->sparse_data.grp_bitmap.grp_bitmap_array_len; i++) {
                for (k = 0; k < BSIZE; k++) {
                    tmp = (1 << k);
                    if (tmp
                        == (child_group->sparse_data.grp_bitmap.grp_bitmap_array[i] & (1 << k))) {
                        count++;
                    }
                    if (m == count - 1) {
                        ranks2[j] = i * BSIZE + k;
...
int ompi_group_incl_bmap(ompi_group_t *group, int n, const int *ranks, ompi_group_t **new_group)
{
    ...
    for (i = 0; i < new_group_pointer->sparse_data.grp_bitmap.grp_bitmap_array_len; i++) {
        new_group_pointer->sparse_data.grp_bitmap.grp_bitmap_array[i] = 0;
    }

    /* set the bits */
    for (i = 0; i < n; i++) {
        bit_set = ranks[i] % BSIZE;
        new_group_pointer->sparse_data.grp_bitmap.grp_bitmap_array[(int) (ranks[i] / BSIZE)]
            |= (1 << bit_set);
    }
