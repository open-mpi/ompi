...
int ompi_group_translate_ranks(ompi_group_t *group1, int n_ranks, const int *ranks1,
                               ompi_group_t *group2, int *ranks2)
{
    ...
    if (group1->grp_parent_group_ptr == group2) { /* from child to parent */
        if (OMPI_GROUP_IS_SPORADIC(group1)) {
            return ompi_group_translate_ranks_sporadic_reverse(group1, n_ranks, ranks1, group2,
                                                               ranks2);
        } else if (OMPI_GROUP_IS_STRIDED(group1)) {
            return ompi_group_translate_ranks_strided_reverse(group1, n_ranks, ranks1, group2,
                                                              ranks2);
        } else if (OMPI_GROUP_IS_BITMAP(group1)) {
            return ompi_group_translate_ranks_bmap_reverse(group1, n_ranks, ranks1, group2, ranks2);
        }
    ...
    if (group2->grp_parent_group_ptr == group1) { /* from parent to child*/
        if (OMPI_GROUP_IS_SPORADIC(group2)) {
            return ompi_group_translate_ranks_sporadic(group1, n_ranks, ranks1, group2, ranks2);
        } else if (OMPI_GROUP_IS_STRIDED(group2)) {
            return ompi_group_translate_ranks_strided(group1, n_ranks, ranks1, group2, ranks2);
        } else if (OMPI_GROUP_IS_BITMAP(group2)) {
            return ompi_group_translate_ranks_bmap(group1, n_ranks, ranks1, group2, ranks2);
        }
...
int ompi_group_dump(ompi_group_t *group)
{
    ...
    if (OMPI_GROUP_IS_SPORADIC(group)) {
        ompi_group_translate_ranks(group, 1, &group->grp_my_rank, group->grp_parent_group_ptr,
                                   &new_rank);
...
int ompi_group_compare(ompi_group_t *group1, ompi_group_t *group2, int *result)
{
