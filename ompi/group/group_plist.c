...
static int ompi_group_dense_overlap(ompi_group_t *group1, ompi_group_t *group2,
...
static struct ompi_proc_t *ompi_group_dense_lookup_raw(ompi_group_t *group, const int peer_id)
{
    if (OPAL_UNLIKELY(ompi_proc_is_sentinel(group->grp_proc_pointers[peer_id]))) {
        ompi_proc_t *proc = (ompi_proc_t *) ompi_proc_lookup(
            ompi_proc_sentinel_to_name((uintptr_t) group->grp_proc_pointers[peer_id]));
        if (NULL != proc) {
...
int ompi_group_incl_plist(ompi_group_t *group, int n, const int *ranks, ompi_group_t **new_group)
{
    ...
    for (int proc = 0; proc < n; proc++) {
        new_group_pointer->grp_proc_pointers[proc] = ompi_group_get_proc_ptr_raw(group_pointer,
                                                                                 ranks[proc]);
    } /* end proc loop */
...
int ompi_group_union(ompi_group_t *group1, ompi_group_t *group2, ompi_group_t **new_group)
{
    ...
    for (int proc1 = 0; proc1 < group1->grp_proc_count; ++proc1) {
        new_group_pointer->grp_proc_pointers[proc1] = ompi_group_get_proc_ptr_raw(group1, proc1);
    }
