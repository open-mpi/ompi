 ...
 */
static int ompi_comm_fill_rest(ompi_communicator_t *comm, int num_procs,
                               ompi_proc_t **proc_pointers, int my_rank, ompi_errhandler_t *errh);
/*
...
*/
typedef int ompi_comm_allgatherfct(void *inbuf, int incount, MPI_Datatype intype, void *outbuf,
                                   int outcount, MPI_Datatype outtype, ompi_communicator_t *comm,
 ...
 */
static int ompi_comm_idup_internal(ompi_communicator_t *comm, ompi_group_t *group,
                                   ompi_group_t *remote_group, opal_info_t *info,
...
int ompi_comm_set(ompi_communicator_t **ncomm, ompi_communicator_t *oldcomm, int local_size,
                  int *local_ranks, int remote_size, int *remote_ranks, opal_hash_table_t *attr,
                  ompi_errhandler_t *errh, bool copy_topocomponent, ompi_group_t *local_group,
                  ompi_group_t *remote_group)
{
    ompi_request_t *req;
    int rc;

    rc = ompi_comm_set_nb(ncomm, oldcomm, local_size, local_ranks, remote_size, remote_ranks, attr,
                          errh, copy_topocomponent, local_group, remote_group, &req);
    if (OMPI_SUCCESS != rc) {
        return rc;
    }
...
int ompi_comm_set_nb(ompi_communicator_t **ncomm, ompi_communicator_t *oldcomm, int local_size,
                     int *local_ranks, int remote_size, int *remote_ranks, opal_hash_table_t *attr,
                     ompi_errhandler_t *errh, bool copy_topocomponent, ompi_group_t *local_group,
                     ompi_group_t *remote_group, ompi_request_t **req)
{
    ...
    *req = NULL;
    ...
    if (NULL == local_group) {
        /* determine how the list of local_rank can be stored most
           efficiently */
        ret = ompi_group_incl(oldcomm->c_local_group, local_size, local_ranks,
                              &newcomm->c_local_group);
    ...
    if (NULL != remote_group) {
        ...
        if (&ompi_mpi_group_null.group == remote_group) {
            ret = ompi_group_incl(oldcomm->c_remote_group, remote_size, remote_ranks,
    ...
    if (NULL != oldcomm->c_keyhash) {
        if (NULL != attr) {
            ...
            if (OMPI_SUCCESS
                != (ret = ompi_attr_copy_all(COMM_ATTR, oldcomm, newcomm, attr,
                                             newcomm->c_keyhash))) {
                ompi_comm_free(&newcomm);
                return ret;
            }
...
int ompi_comm_create(ompi_communicator_t *comm, ompi_group_t *group, ompi_communicator_t **newcomm)
{
    ...
    if (OMPI_COMM_IS_INTER(comm)) {
        ...
        rc = comm->c_coll->coll_allgather(&(group->grp_my_rank), 1, MPI_INT, allranks, 1, MPI_INT,
...
int ompi_comm_split_with_info(ompi_communicator_t *comm, int color, int key, opal_info_t *info,
                              ompi_communicator_t **newcomm, bool pass_on_topo)
{
    ...
    if (inter) {
        ...
        rc = comm->c_coll->coll_allgather(myinfo, 2, MPI_INT, rresults, 2, MPI_INT, comm,
                                          comm->c_coll->coll_allgather_module);
        if (OMPI_SUCCESS != rc) {
            goto exit;
        }
    ...
    rc = ompi_comm_set(&newcomp,                  /* new comm */
                       ...
                       NULL,                      /* attrs */
                       comm->error_handler,       /* error handler */
                       pass_on_topo, local_group, /* local group */
                       remote_group);             /* remote group */
    ...
    if (inter) {
        ...
        if (NULL != newcomp->c_local_comm) {
            snprintf(newcomp->c_local_comm->c_name, MPI_MAX_OBJECT_NAME,
                     "MPI COMMUNICATOR %d SPLIT FROM %d", newcomp->c_local_comm->c_contextid,
 ...
 */
static int ompi_comm_split_type_get_part(ompi_group_t *group, const int split_type, int **ranks_out,
                                         int *rank_size)
{
...
static int ompi_comm_split_verify(ompi_communicator_t *comm, int split_type, int key,
                                  bool *need_split)
{
    ...
    *need_split = false;
    ...
    for (int i = 0; i < size; ++i) {
        if (MPI_UNDEFINED == results[i * 2]
            || (i >= 1 && results[i * 2 + 1] < results[i * 2 - 1])) {
            *need_split = true;
            break;
        }
    }
...
int ompi_comm_split_type(ompi_communicator_t *comm, int split_type, int key, opal_info_t *info,
                         ompi_communicator_t **newcomm)
{
    ...
    */
    ...
    /* --------------------------------------------------------- */
    if (inter) {
        rc = ompi_comm_split_type_get_part(comm->c_remote_group, global_split_type, &rranks,
                                           &my_rsize);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != rc)) {
            free(lranks);
            return rc;
    ...
    do {
        rc = ompi_comm_set(&newcomp, comm, my_size, lranks, my_rsize, rranks, NULL,
                           comm->error_handler, false, NULL, NULL);
        if (OPAL_UNLIKELY(OMPI_SUCCESS != rc)) {
            break;
        }
...
int ompi_comm_idup_with_info(ompi_communicator_t *comm, opal_info_t *info,
                             ompi_communicator_t **newcomm, ompi_request_t **req)
{
    return ompi_comm_idup_internal(comm, comm->c_local_group, comm->c_remote_group, info, newcomm,
                                   req);
}
...
static int ompi_comm_idup_getcid(ompi_comm_request_t *request)
{
    ompi_comm_idup_with_info_context_t *context = (ompi_comm_idup_with_info_context_t *)
                                                      request->context;
    ompi_request_t *subreq[1];
    int rc, mode;
    ...
    /* Determine context id. It is identical to f_2_c_handle */
    rc = ompi_comm_nextcid_nb(context->newcomp, context->comm, NULL, NULL, NULL, false, mode,
                              subreq);
    if (OMPI_SUCCESS != rc) {
        ompi_comm_request_return(request);
...
static int ompi_comm_idup_with_info_activate(ompi_comm_request_t *request)
{
    ...
    /* activate communicator and init coll-module */
    rc = ompi_comm_activate_nb(&context->newcomp, context->comm, NULL, NULL, NULL, false, mode,
                               subreq);
    if (OMPI_SUCCESS != rc) {
        OBJ_RELEASE(context->newcomp);
        return rc;
...
int ompi_comm_compare(ompi_communicator_t *comm1, ompi_communicator_t *comm2, int *result)
{
    ...
    ompi_group_compare((ompi_group_t *) comp1->c_local_group, (ompi_group_t *) comp2->c_local_group,
                       &cmp_result);
    ...
    if (rsize1 > 0) {
        ...
        ompi_group_compare((ompi_group_t *) comp1->c_remote_group,
                           (ompi_group_t *) comp2->c_remote_group, &cmp_result);
                                             ...
                                             mca_coll_base_module_t *data)
{
    ...
    rc = MCA_PML_CALL(
        irecv(outbuf, size * outcount, outtype, 0, OMPI_COMM_ALLGATHER_TAG, comm, &sendreq));
    if (OMPI_SUCCESS != rc) {
        goto exit;
    }
    ...
    if (0 == rank) {
        for (i = 0; i < rsize; i++) {
            rc = MCA_PML_CALL(send(tmpbuf, rsize * outcount, outtype, i, OMPI_COMM_ALLGATHER_TAG,
...
int ompi_comm_free(ompi_communicator_t **comm)
{
    ...
    if (is_extra_retain) {
         ...
         */
        ompi_communicator_t *tmpcomm = (ompi_communicator_t *)
            opal_pointer_array_get_item(&ompi_mpi_communicators, cid);
        if (NULL != tmpcomm) {
                         ...
                         ompi_proc_t ***prprocs)
{
    ...
    if (local_rank == local_leader) {
        ...
        if (OMPI_GROUP_IS_DENSE(local_comm->c_local_group)) {
            rc = ompi_proc_pack(local_comm->c_local_group->grp_proc_pointers, local_size, sbuf);
        }
        ...
        rc = MCA_PML_CALL(irecv(&rlen, 1, MPI_INT, remote_leader, tag, bridge_comm, &req));
        if (OMPI_SUCCESS != rc) {
            goto err_exit;
        }
        int_len = (int) size_len;

        rc = MCA_PML_CALL(send(&int_len, 1, MPI_INT, remote_leader, tag, MCA_PML_BASE_SEND_STANDARD,
                               bridge_comm));
        if (OMPI_SUCCESS != rc) {
    ...
    rc = local_comm->c_coll->coll_bcast(&rlen, 1, MPI_INT, local_leader, local_comm,
    ...
    if (local_rank == local_leader) {
        ...
        rc = MCA_PML_CALL(irecv(recvbuf, rlen, MPI_BYTE, remote_leader, tag, bridge_comm, &req));
        if (OMPI_SUCCESS != rc) {
            goto err_exit;
        }
    ...
    rc = local_comm->c_coll->coll_bcast(recvbuf, rlen, MPI_BYTE, local_leader, local_comm,
                                        local_comm->c_coll->coll_bcast_module);
    if (OMPI_SUCCESS != rc) {
        goto err_exit;
    }
    ...
    for (i = 0; i < rsize; i++) {
        ...
        uint16_t *u16ptr, u16;
        u16ptr = &u16;
        OPAL_MODEX_RECV_VALUE_OPTIONAL(rc, PMIX_LOCALITY, &rprocs[i]->super.proc_name, &u16ptr,
                                       PMIX_UINT16);
        if (OPAL_SUCCESS == rc) {
 ...
 * in intercomm_create
 */
int ompi_comm_overlapping_groups(int size, ompi_proc_t **lprocs, int rsize, ompi_proc_t **rprocs)
...
int ompi_comm_determine_first(ompi_communicator_t *intercomm, int high)
{
    ...
    rc = intercomm->c_coll->coll_allgatherv(&high, scount, MPI_INT, &rhigh, rcounts, rdisps,
                                            MPI_INT, intercomm,
...
int ompi_comm_determine_first_auto(ompi_communicator_t *intercomm)
{
    ...
    rc = ompi_rte_compare_name_fields(mask,
                                      (const ompi_process_name_t *) &(ourproc->super.proc_name),
 ...
 */
int ompi_comm_enable(ompi_communicator_t *old_comm, ompi_communicator_t *new_comm, int new_rank,
                     int num_procs, ompi_proc_t **topo_procs)
{
    ...
    /* Determine context id. It is identical to f_2_c_handle */
    ret = ompi_comm_nextcid(new_comm, old_comm, NULL, NULL, NULL, false, OMPI_COMM_CID_INTRA);
    if (OMPI_SUCCESS != ret) {
    ...
    ret = ompi_comm_activate(&new_comm, old_comm, NULL, NULL, NULL, false, OMPI_COMM_CID_INTRA);
    if (OMPI_SUCCESS != ret) {
        /* something wrong happened while setting the communicator */
        goto complete_and_return;
    }
                               ...
                               ompi_proc_t **proc_pointers, int my_rank, ompi_errhandler_t *errh)
{
     ...
     */
    snprintf(comm->c_name, MPI_MAX_OBJECT_NAME, "MPI_COMMUNICATOR %d", comm->c_contextid);
