...
static int ompi_comm_rbcast_null(ompi_communicator_t *comm, ompi_comm_rbcast_message_t *msg,
                                 size_t size)
{
    return OMPI_SUCCESS;
}
static int ompi_comm_rbcast_bmg(ompi_communicator_t *comm, ompi_comm_rbcast_message_t *msg,
                                size_t size);
static int ompi_comm_rbcast_n2(ompi_communicator_t *comm, ompi_comm_rbcast_message_t *msg,
                               size_t size);

int (*ompi_comm_rbcast)(ompi_communicator_t *comm, ompi_comm_rbcast_message_t *msg, size_t size)
    = ompi_comm_rbcast_null;
static int (*ompi_comm_rbcast_fw)(ompi_communicator_t *comm, ompi_comm_rbcast_message_t *msg,
                                  size_t size)
    = ompi_comm_rbcast_null;

static void ompi_rbcast_bml_send_complete_cb(struct mca_btl_base_module_t *module,
                                             struct mca_btl_base_endpoint_t *endpoint,
                                             struct mca_btl_base_descriptor_t *descriptor,
                                             int status);

static void ompi_comm_rbcast_bml_recv_cb(struct mca_btl_base_module_t *btl,
                                ...
                                size_t size)
{
    ...
    OPAL_OUTPUT_VERBOSE((5, ompi_ftmpi_output_handle, "%s %s: rbcast on communicator %3d:%d",
                                         ...
                                         const mca_btl_base_receive_descriptor_t *descriptor)
{
    ...
    if (OPAL_UNLIKELY(msg->cid != comm->c_contextid)) {
        OPAL_OUTPUT_VERBOSE((2, ompi_ftmpi_output_handle,
                             "%s %s: Info: received a late rbcast message with CID %3d:%d during "
                             "an MPI_COMM_DUP that is trying to reuse that CID (thus increasing "
                             "the epoch) - ignoring, nothing to do",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, msg->cid, msg->epoch));
        return;
    }
    ...
    if (OPAL_UNLIKELY(comm->c_epoch != msg->epoch)) {
        OPAL_OUTPUT_VERBOSE((2, ompi_ftmpi_output_handle,
                             "%s %s: Info: Received a late rbcast order for the communicator with "
                             "CID %3d:%d when is is now at epoch %d - ignoring, nothing to do",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, msg->cid, msg->epoch,
                             comm->c_epoch));
        return;
    }
...
int ompi_comm_rbcast_send_msg(ompi_proc_t *proc, ompi_comm_rbcast_message_t *msg, size_t size)
{
    ...
    if (!ompi_proc_is_active(proc)) {
        opal_output_verbose(5, ompi_ftmpi_output_handle,
                            ...
                            OMPI_NAME_PRINT(&proc->super.proc_name), msg->cid, msg->epoch);
    ...
    OPAL_OUTPUT_VERBOSE((5, ompi_ftmpi_output_handle,
                         ...
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__,
                         OMPI_NAME_PRINT(&proc->super.proc_name), msg->cid, msg->epoch));
    mca_bml_base_alloc(bml_btl, &des, MCA_BTL_NO_ORDER, size,
                       MCA_BTL_DES_FLAGS_PRIORITY | MCA_BTL_DES_FLAGS_BTL_OWNERSHIP);
    if (OPAL_UNLIKELY(NULL == des)) {
        opal_output(ompi_ftmpi_output_handle, "%s %s: Error: bml_base_alloc failed.",
                    OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__);
        return OMPI_ERR_OUT_OF_RESOURCE;
    }
    ...
    if (OPAL_LIKELY(ret >= 0)) {
        ...
        OPAL_OUTPUT_VERBOSE((2, ompi_ftmpi_output_handle,
                             ...
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__,
                             OMPI_NAME_PRINT(&proc->super.proc_name), msg->cid, ret));
        return ret;
    }
    return OMPI_SUCCESS;
}
                                             ...
                                             int status)
{
    if (OPAL_UNLIKELY(OMPI_SUCCESS != status)) {
        opal_output_verbose(2, ompi_ftmpi_output_handle, "%s %s: status %d",
                            OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, status);
    } else {
        OPAL_OUTPUT_VERBOSE((91, ompi_ftmpi_output_handle, "%s %s: status %d",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, status));
    }
    return;
}
...
int ompi_comm_rbcast_register_params(void)
{
    (void) mca_base_var_register(
        "ompi", "mpi", "ft", "reliable_bcast",
        "Reliable Broadcast algorithm (1: Binomial Graph Diffusion; 2: N^2 full graph diffusion)",
        MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_9, MCA_BASE_VAR_SCOPE_READONLY, &rbcast);
    return OMPI_SUCCESS;
}
