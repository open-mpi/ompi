...
int ompi_comm_rbcast_send_msg(ompi_proc_t* proc, ompi_comm_rbcast_message_t* msg, size_t size) {
     ...
     * connection info shows up. Both topologies above route around an
     * UNREACH edge but abort the whole broadcast on any other code,
     * leaving a revoke or a failure notification that reached nobody --
     * so a peer merely not connected yet must be reported as UNREACH,
     * while a hard add_proc failure still stops everything. */
