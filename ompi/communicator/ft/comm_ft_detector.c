...
typedef struct {
    ...
    volatile int hb_rdma_flag; /* set to -1 when read locally, set to observing sets it to its rank
                                  through rdma */
...
static comm_detector_t comm_world_detector = {.comm = &ompi_mpi_comm_world.comm,
                                              .fd_event = NULL,
...
int ompi_comm_failure_detector_register_params(void)
{
    (void) mca_base_var_register("ompi", "mpi", "ft", "detector",
                                 "Use the OMPI heartbeat based failure detector, or disable it and "
                                 "use only RTE and in-band detection (slower)",
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, OPAL_INFO_LVL_9,
                                 MCA_BASE_VAR_SCOPE_READONLY, &comm_detector_enable);
    (void) mca_base_var_register("ompi", "mpi", "ft", "detector_thread",
                                 "Delegate failure detector to a separate thread",
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, OPAL_INFO_LVL_9,
                                 MCA_BASE_VAR_SCOPE_READONLY, &comm_detector_use_thread);
    ...
    (void) mca_base_var_register("ompi", "mpi", "ft", "detector_period",
                                 "Period of heartbeat emission (s)", MCA_BASE_VAR_TYPE_DOUBLE, NULL,
                                 0, 0, OPAL_INFO_LVL_9, MCA_BASE_VAR_SCOPE_READONLY,
                                 &comm_heartbeat_period);
    (void) mca_base_var_register(
        "ompi", "mpi", "ft", "detector_timeout",
        "Timeout before we start suspecting a process after the last heartbeat reception (must be "
        "larger than 3*ompi_mpi_ft_detector_period)",
        MCA_BASE_VAR_TYPE_DOUBLE, NULL, 0, 0, OPAL_INFO_LVL_9, MCA_BASE_VAR_SCOPE_READONLY,
        &comm_heartbeat_timeout);
    (void) mca_base_var_register("ompi", "mpi", "ft", "detector_rdma_heartbeat",
                                 ...
                                 MCA_BASE_VAR_TYPE_BOOL, NULL, 0, 0, OPAL_INFO_LVL_9,
                                 MCA_BASE_VAR_SCOPE_READONLY, &comm_detector_use_rdma_hb);
    return OMPI_SUCCESS;
}
...
int ompi_comm_failure_detector_init(void)
{
    ...
    if (!ompi_ftmpi_enabled || !comm_detector_enable)
        return OMPI_SUCCESS;
    ...
    if (comm_detector_use_thread) {
        ...
        if (OPAL_SUCCESS != ret)
            goto cleanup;
        while (0 == fd_thread_active)
            ; /* wait for the fd thread initialization */
...
{
    /* fd not wanted */
    if (-1 == comm_heartbeat_recv_cb_type)
        return OMPI_SUCCESS;
...
int ompi_comm_failure_detector_finalize(void)
{
    ...
    if (opal_sync_event_base != fd_event_base)
        opal_event_base_free(fd_event_base);

    /* Remove rdma registrations, if any */
    if (NULL != detector->hb_rdma_rank_lreg) {
        mca_bml_base_deregister_mem(detector->hb_rdma_bml_btl_observer,
                                    detector->hb_rdma_rank_lreg);
    }
    if (NULL != detector->hb_rdma_flag_lreg) {
        mca_bml_base_deregister_mem(detector->hb_rdma_bml_btl_observing,
                                    detector->hb_rdma_flag_lreg);
    }
...
int ompi_comm_start_detector(ompi_communicator_t *comm)
{
    ...
    detector->hb_rstamp = PMPI_Wtime() + comm_heartbeat_timeout + 1.
                          + log((double) np); /* give some slack for MPI_Init */
    ...
    detector->fd_event = opal_event_new(fd_event_base, -1, OPAL_EV_TIMEOUT | OPAL_EV_PERSIST,
    ...
    OPAL_OUTPUT_VERBOSE((2, ompi_ftmpi_output_handle,
                         ...
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, detector->hb_period / 10.,
                         detector->hb_period, comm_detector_use_thread ? "(in a thread)" : ""));
...
static int fd_heartbeat_request(comm_detector_t *detector)
{
    ...
    for (rank = (np + detector->hb_observing) % np; true; rank = (np + rank - 1) % np) {
        ...
        if (rank == comm->c_my_rank) {
            OPAL_OUTPUT_VERBOSE((2, ompi_ftmpi_output_handle,
                                 ...
                                 OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, comm->c_contextid,
                                 comm->c_epoch));
            detector->hb_observer = detector->hb_observing = MPI_PROC_NULL;
        ...
        OPAL_OUTPUT_VERBOSE((2, ompi_ftmpi_output_handle,
                             ...
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, rank, comm->c_contextid,
                             comm->c_epoch, detector->hb_rstamp - startdate));
        ...
        if (comm_detector_use_rdma_hb) {
            ...
            if (NULL != bml_btl->btl->btl_register_mem) {
                assert(!((size_t) &detector->hb_rdma_flag
                         & ALIGNMENT_MASK(bml_btl->btl->btl_put_alignment)));
    ...
    detector->hb_rstamp = PMPI_Wtime() + detector->hb_timeout; /* we add one timeout slack to
                                                                  account for the send time */
    return ret;
}
...
static int fd_heartbeat_request_cb(ompi_communicator_t *comm, ompi_comm_heartbeat_req_t *msg)
{
    ...
    int np, rr, ro;
    np = ompi_comm_size(comm);
    rr = (np - comm->c_my_rank + msg->from)
         % np; /* translate msg->from in circular space so that myrank==0 */
    ro = (np - comm->c_my_rank + detector->hb_observer) % np; /* same for the observer rank */
    if (rr < ro) {
        opal_output_verbose(1, ompi_ftmpi_output_handle,
                            "%s %s: Received heartbeat request from %d on communicator %3d:%d but "
                            "I am monitored by %d -- this is stall information, ignoring.",
                            OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, msg->from,
                            comm->c_contextid, comm->c_epoch, detector->hb_observer);
        return false; /* never forward on the rbcast */
    }
    OPAL_OUTPUT_VERBOSE((2, ompi_ftmpi_output_handle,
                         "%s %s: Recveived heartbeat request from %d on communicator %3d:%d",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, msg->from, comm->c_contextid,
                         comm->c_epoch));
    ...
    if (comm_detector_use_rdma_hb) {
        ...
        if (NULL != bml_btl->btl->btl_register_mem) {
            assert(!((size_t) &detector->hb_rdma_rank
                     & ALIGNMENT_MASK(bml_btl->btl->btl_put_alignment)));
            mca_bml_base_register_mem(bml_btl, &detector->hb_rdma_rank, sizeof(int), 0,
                                      &detector->hb_rdma_rank_lreg);
            assert(NULL != detector->hb_rdma_rank_lreg);
            /* registration for the remote flag */
            if (NULL != detector->hb_rdma_rreg)
                free(detector->hb_rdma_rreg);
...
static void fd_event_cb(int fd, short flags, void *pdetector)
{
    ...
    OPAL_OUTPUT_VERBOSE((100, ompi_ftmpi_output_handle,
                         "%s %s: evtime triggered at stamp %g; observing %d (recv grace %g); "
                         "observer %d (send grace %g); drift %g",
    ...
    if (comm_detector_use_rdma_hb) {
        ...
        OPAL_OUTPUT_VERBOSE((100, ompi_ftmpi_output_handle, "%s:%s: read flag %d at stamp %g",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, flag,
                             stamp - startdate));
        if (rank == flag) {
            /* this is a quit message from our observed process, stop the
             * detector */
            opal_output_verbose(10, ompi_ftmpi_output_handle,
                                "%s %s: evtimer triggered at stamp %g, RDMA flag is set to my own "
                                "rank, this is a quit message to close the detector.",
                                OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, stamp - startdate);
        ...
        if (0 <= flag) {
            ...
            if (flag != detector->hb_observing) {
                opal_output_verbose(
                    1, ompi_ftmpi_output_handle,
                    "%s %s: evtimer triggered at stamp %g, this is a rdma heartbeat from %d, but I "
                    "am now observing %d, ignoring the heartbeat",
                    OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, stamp - startdate, flag,
    ...
    if (!ompi_proc_is_active(proc) /* found dead inline (btl) or externally (pmix) */
        ...
        if (OPAL_PROC_ON_LOCAL_NODE(proc->super.proc_flags) && ompi_proc_is_active(proc)) {
            /* special case for finalize */
            if (detector->hb_observer == MPI_PROC_NULL) {
                OPAL_OUTPUT_VERBOSE(
                    (10, ompi_ftmpi_output_handle,
                     "%s %s: evtimer triggered at stamp %g, recv grace IGNORED by %.1e, proc %d is "
                     "local and still active but this is FINALIZE.",
                     OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, stamp - startdate,
            ...
            OPAL_OUTPUT_VERBOSE(
                (10, ompi_ftmpi_output_handle,
                 "%s %s: evtimer triggered at stamp %g, recv grace IGNORED by %.1e, proc %d is "
                 "local and still active (I am observed by %d w/ period %g).",
                 OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, stamp - startdate,
                 detector->hb_timeout - (stamp - detector->hb_rstamp), detector->hb_observing,
                 detector->hb_observer, detector->hb_period));
            detector->hb_rstamp = stamp;
            return;
        }
#endif
        if( ompi_proc_is_active(proc) /* not an external notice */
         ...
         && lastpeek <= (detector->hb_rstamp + detector->hb_timeout) /* and granting the jitter as slack, we are still fine */ ) {
            OPAL_OUTPUT_VERBOSE(
                (1, ompi_ftmpi_output_handle,
                 "%s %s: evtimer triggered at stamp %g, recv grace IGNORED by %.1e because of "
                 "drift %.1e, proc %d is still seen as active (I am observed by %d w/ period %g).",
                 OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, stamp - startdate,
                 detector->hb_timeout - (stamp - detector->hb_rstamp), stamp - lastpeek,
                 detector->hb_observing, detector->hb_observer, detector->hb_period));
            return;
        }

        /* this process is now suspected dead. */
        opal_output_verbose(1, ompi_ftmpi_output_handle,
                            "%s %s: evtimer triggered at stamp %g, recv grace MISSED by %.1e, proc "
                            "%d now suspected dead.",
                            OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, stamp - startdate,
...
void *fd_progress(opal_object_t *obj)
{
    ...
    OPAL_THREAD_ADD_FETCH32(&fd_thread_active, 1);
    while (1 == fd_thread_active)
        ; /* wait for init stage 2: start_detector */
    ret = MCA_PML_CALL(
        irecv(NULL, 0, MPI_BYTE, 0, MCA_COLL_BASE_TAG_FT_END, &ompi_mpi_comm_self.comm, &req));
...
{
    OPAL_OUTPUT_VERBOSE((100, ompi_ftmpi_output_handle, "%s %s: rdma_hb_cb status=%d",
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, status));
}
...
static int fd_heartbeat_rdma_put(comm_detector_t *detector)
{
    ...
    if (0 == detector->hb_rdma_raddr)
        return OMPI_SUCCESS; /* not initialized yet */
    OPAL_THREAD_LOCK(&detector->fd_mutex);
    do {
        ret = mca_bml_base_put(detector->hb_rdma_bml_btl_observer, &detector->hb_rdma_rank,
                               detector->hb_rdma_raddr, detector->hb_rdma_rank_lreg,
                               detector->hb_rdma_rreg, sizeof(int), 0, MCA_BTL_NO_ORDER,
                               fd_heartbeat_rdma_cb, NULL);
        OPAL_OUTPUT_VERBOSE((100, ompi_ftmpi_output_handle, "%s %s: bml_put sendseq=%d, rc=%d",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, sendseq++, ret));
...
static int fd_heartbeat_send(comm_detector_t *detector)
{
    ...
    if (OPAL_PROC_ON_LOCAL_NODE(
            ompi_comm_peer_lookup(comm, detector->hb_observer)->super.proc_flags))
        return OMPI_SUCCESS;
#endif
    ...
    if (0. != detector->hb_sstamp && (now - detector->hb_sstamp) >= 2. * detector->hb_period) {
        opal_output_verbose(1, ompi_ftmpi_output_handle,
                            "%s %s: MISSED my SEND %d deadline by %.1e, this could trigger a false "
                            "suspicion for me.",
                            OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, sendseq,
    ...
    OPAL_OUTPUT_VERBOSE((9, ompi_ftmpi_output_handle,
                         ...
                         OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, detector->hb_observer,
                         comm->c_contextid, comm->c_epoch, detector->hb_sstamp - startdate));

    if (comm_detector_use_rdma_hb)
        return fd_heartbeat_rdma_put(detector);
    ...
    msg.from = detector->hb_rdma_rank; /* comm->c_my_rank; except during finalize when it is equal
                                          to detector->hb_observer */
...
static int fd_heartbeat_recv_cb(ompi_communicator_t *comm, ompi_comm_heartbeat_message_t *msg)
{
    ...
    if (comm->c_my_rank == msg->from) {
         ...
         * detector */
        opal_output_verbose(10, ompi_ftmpi_output_handle,
                            "%s %s: Received heartbeat from %d, which is my own rank, this is a "
                            "quit message to close the detector.",
                            OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, msg->from);
        detector->hb_observing = MPI_PROC_NULL;
        detector->hb_rstamp = INFINITY;
        return false;
    ...
    if (msg->from != detector->hb_observing) {
        OPAL_OUTPUT_VERBOSE((2, ompi_ftmpi_output_handle,
                             "%s %s: Received heartbeat from %d on communicator %3d:%d but I am "
                             "now monitoring %d -- ignored.",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, msg->from,
                             comm->c_contextid, comm->c_epoch, detector->hb_observing));
    } else {
        double stamp = PMPI_Wtime();
        double grace = detector->hb_timeout - (stamp - detector->hb_rstamp);
        OPAL_OUTPUT_VERBOSE((9, ompi_ftmpi_output_handle,
                             "%s %s: Received heartbeat from %d on communicator %3d:%d at "
                             "timestamp %g (remained %.1e of %.1e before suspecting)",
                             OMPI_NAME_PRINT(OMPI_PROC_MY_NAME), __func__, msg->from,
                             comm->c_contextid, comm->c_epoch, stamp - startdate, grace,
                             detector->hb_timeout));
        detector->hb_rstamp = stamp;
        if (grace < 0.0) {
            opal_output_verbose(1, ompi_ftmpi_output_handle, "%s %s: MISSED ( %.1e )",
