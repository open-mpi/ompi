                                          ...
                                          int *ranks_in_comm, ompi_communicator_t *comm)
{
    ...
    struct iovec send_iov[2] = {{0, 0}, {0, 0}}, recv_iov[2] = {{0, 0}, {0, 0}};
    ...
    rc = ompi_netpatterns_setup_recursive_doubling_tree_node(n_peers, my_rank_in_group,
                                                             &my_exchange_node);
    if (OMPI_SUCCESS != rc) {
        return rc;
    }
    ...
    if (0 < my_exchange_node.n_extra_sources) {
        ...
        if (EXCHANGE_NODE == my_exchange_node.node_type) {
            ...
            rc = MCA_PML_CALL(recv(dest_buf_current, count, dtype, ranks_in_comm[extra_rank],
                                   -OMPI_COMMON_TAG_ALLREDUCE, comm, MPI_STATUSES_IGNORE));
            if (0 > rc) {
                goto Error;
            }
            ...
            rc = MCA_PML_CALL(send(src_buf_current, count, dtype, ranks_in_comm[extra_rank],
                                   -OMPI_COMMON_TAG_ALLREDUCE, MCA_PML_BASE_SEND_STANDARD, comm));
            if (0 > rc) {
                goto Error;
            }
        }
    ...
    for (exchange = 0; exchange < my_exchange_node.n_exchanges; exchange++) {
        ...
        if (remote_data_start_rank < n_extra_nodes) {
            ...
            recv_iov[1].iov_base = (char *) dest_buf
                                   + (extra_start + my_exchange_node.n_largest_pow_2)
                                         * message_extent;
            recv_iov[1].iov_len = n_extra * count;
            iovec_len = 2;
        }

        rc = MCA_PML_CALL(irecv(recv_iov[0].iov_base, current_data_count, dtype,
                                ranks_in_comm[pair_rank], -OMPI_COMMON_TAG_ALLREDUCE, comm,
                                &(requests[msg_cnt])));
        if (0 > rc) {
            goto Error;
        }
        ...
        if (iovec_len > 1) {
            rc = MCA_PML_CALL(irecv(recv_iov[1].iov_base, recv_iov[1].iov_len, dtype,
                                    ranks_in_comm[pair_rank], -OMPI_COMMON_TAG_ALLREDUCE, comm,
                                    &(requests[msg_cnt])));
            if (0 > rc) {
                goto Error;
        ...
        rc = MCA_PML_CALL(isend(send_iov[0].iov_base, current_data_count, dtype,
                                ranks_in_comm[pair_rank], -OMPI_COMMON_TAG_ALLREDUCE,
        ...
        msg_cnt++;
        if (iovec_len > 1) {
            rc = MCA_PML_CALL(isend(send_iov[1].iov_base, send_iov[1].iov_len, dtype,
                                    ranks_in_comm[pair_rank], -OMPI_COMMON_TAG_ALLREDUCE,
                                    MCA_PML_BASE_SEND_STANDARD, comm, &(requests[msg_cnt])));
            if (0 > rc) {
                goto Error;
    ...
    if (0 < my_exchange_node.n_extra_sources) {
        ...
        if (EXTRA_NODE == my_exchange_node.node_type) {
            ...
            rc = MCA_PML_CALL(recv(dest_buf, count * n_peers, dtype, ranks_in_comm[extra_rank],
                                   -OMPI_COMMON_TAG_ALLREDUCE, comm, MPI_STATUSES_IGNORE));
            if (0 > rc) {
                goto Error;
            }
            ...
            extra_rank = my_exchange_node.rank_extra_source;
            rc = MCA_PML_CALL(send(dest_buf, count * n_peers, dtype, ranks_in_comm[extra_rank],
                                   -OMPI_COMMON_TAG_ALLREDUCE, MCA_PML_BASE_SEND_STANDARD, comm));
            if (0 > rc) {
                goto Error;
            }
