                                          ...
                                          int *ranks_in_comm, ompi_communicator_t *comm)
{
    ...
    if (1 == n_peers) {
        ...
        rc = ompi_datatype_copy_content_same_ddt(dtype, count, (char *) rbuf, (char *) sbuf);
        if (OMPI_SUCCESS != rc) {
            goto Error;
        }
        return OMPI_SUCCESS;
    ...
    for (stripe_number = 0; stripe_number < n_data_segments; stripe_number++) {
        ...
        if (0 < my_exchange_node.n_extra_sources) {
            ...
            if (EXCHANGE_NODE == my_exchange_node.node_type) {
                ...
                */
                ...
                rc = MCA_PML_CALL(recv(scratch_bufers[recv_buffer], count_this_stripe, dtype,
                                       ranks_in_comm[extra_rank], -OMPI_COMMON_TAG_ALLREDUCE, comm,
                                       MPI_STATUSES_IGNORE));
                if (0 > rc) {
                ...
                if (0 < count_this_stripe) {
                    ompi_op_reduce(op, (void *) scratch_bufers[send_buffer],
                                   (void *) scratch_bufers[recv_buffer], count_this_stripe, dtype);
                }
                ...
                */
                extra_rank = my_exchange_node.rank_extra_source;
                rc = MCA_PML_CALL(send(scratch_bufers[send_buffer], count_this_stripe, dtype,
                                       ranks_in_comm[extra_rank], -OMPI_COMMON_TAG_ALLREDUCE,
        ...
        for (exchange = 0; exchange < my_exchange_node.n_exchanges; exchange++) {
            ...
            rc = ompi_coll_base_sendrecv_actual(scratch_bufers[send_buffer], count_this_stripe,
                                                dtype, ranks_in_comm[pair_rank],
                                                -OMPI_COMMON_TAG_ALLREDUCE,
                                                scratch_bufers[recv_buffer], count_this_stripe,
                                                dtype, ranks_in_comm[pair_rank],
        ...
        if (0 < my_exchange_node.n_extra_sources) {
            ...
            if (EXTRA_NODE == my_exchange_node.node_type) {
                ...
                */
                ...
                extra_rank = my_exchange_node.rank_extra_source;
                rc = MCA_PML_CALL(send((char *) scratch_bufers[send_buffer], count_this_stripe,
                                       dtype, ranks_in_comm[extra_rank], -OMPI_COMMON_TAG_ALLREDUCE,
                                       MCA_PML_BASE_SEND_STANDARD, comm));
                if (0 > rc) {
