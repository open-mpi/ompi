                                       ...
                                       netpatterns_tree_node_t *my_node)
{
    ...
    if (my_level_in_tree == (n_lvls_in_tree - 1)) {
        ...
        if (0 < my_node->n_children) {
            my_node->children_ranks = (int *) malloc(sizeof(int) * my_node->n_children);
...
int ompi_netpatterns_setup_narray_knomial_tree(int tree_order, int my_rank, int num_nodes,
                                               netpatterns_narray_knomial_tree_node_t *my_node)
{
    ...
    if (0 == my_rank) {
        ...
        my_node->rank_on_level = my_rank_in_my_level = my_rank - cum_cnt;
        my_node->level_size = cnt;

        rc = ompi_netpatterns_setup_recursive_knomial_tree_node(my_node->level_size,
                                                                my_node->rank_on_level, tree_order,
                                                                &my_node->k_node);
        if (OMPI_SUCCESS != rc) {
            goto Error;
        }
                             ...
                             netpatterns_tree_node_t *nodes_data)
{
    ...
    if (num_children) {
        nodes_data[my_node].children_ranks = (int *) malloc(sizeof(int) * num_children);
                                                   ...
                                                   netpatterns_tree_node_t **tree_nodes)
{
    ...
    *tree_nodes = (netpatterns_tree_node_t *) malloc(sizeof(netpatterns_tree_node_t) * num_nodes);
    if (!(*tree_nodes)) {
    ...
    rc = fill_in_node_data(tree_order, num_descendent_ranks, 0, *tree_nodes);
