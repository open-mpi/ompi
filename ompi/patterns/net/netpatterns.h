...
OMPI_DECLSPEC extern int ompi_netpatterns_base_err(const char *, ...)
    __opal_attribute_format__(__printf__, 1, 2);
#    define NETPATTERNS_VERBOSE(args)                                                             \
        do {                                                                                      \
            if (ompi_netpatterns_base_verbose > 0) {                                              \
                ompi_netpatterns_base_err("[%s]%s[%s:%d:%s] ", ompi_process_info.nodename,        \
...
#define FIND_BASE(base, myid, level, k)                                 \
    do {                                                                \
        ...
        int find_base_knt2;                                             \
                                                                        \
        base = 0;                                                       \
        for (int find_base_i = 0; find_base_i < level; find_base_i++) { \
            find_base_tmp *= k;                                         \
        }                                                               \
        find_base_knt2 = 1;                                             \
        while (myid >= (find_base_knt2 * find_base_tmp)) {              \
            find_base_knt2++;                                           \
        }                                                               \
        base = (find_base_knt2 * find_base_tmp) - find_base_tmp;        \
    } while (0)

/* enum for node type */
enum { ROOT_NODE, LEAF_NODE, INTERIOR_NODE };
...
typedef struct netpatterns_narray_knomial_tree_node_t netpatterns_narray_knomial_tree_node_t;
...
OMPI_DECLSPEC int
ompi_netpatterns_setup_narray_knomial_tree(int tree_order, int my_rank, int num_nodes,
                                           netpatterns_narray_knomial_tree_node_t *my_node);
/* cleanup an n-array tree setup by the above function */
OMPI_DECLSPEC void
ompi_netpatterns_cleanup_narray_knomial_tree(netpatterns_narray_knomial_tree_node_t *my_node);
...
OMPI_DECLSPEC int
ompi_netpatterns_setup_narray_tree_contigous_ranks(int tree_order, int num_nodes,
                                                   netpatterns_tree_node_t **tree_nodes);
