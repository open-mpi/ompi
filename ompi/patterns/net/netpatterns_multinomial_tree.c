        ...
        for (node = 0; node < n_nodes_in_this_level; node++) {
            ...
            if (0 == current_level) {
                ...
                if (current_level == n_lvls_in_tree - 1) {
                    ...
                    parent_rank = node - (node / n_nodes_prev_level) * n_nodes_prev_level;
                    parent_rank = n_cum_nodes - n_nodes_prev_level + parent_rank;
                    tree_nodes[node_index].parent_rank = parent_rank;
                } else {
                    tree_nodes[node_index].parent_rank = (n_cum_nodes - n_nodes_prev_level)
                                                         + node / tree_order;
                }
            }
            ...
            if ((n_lvls_in_tree - 1) == current_level) {
                ...
                if ((n_lvls_in_tree - 2) == current_level) {
                    ...
                    n_nodes_in_last_level = num_nodes - (n_cum_nodes + n_nodes_in_this_level);
                    ...
                    if (0 < n_children) {
                        tree_nodes[node_index].children_ranks = (int *) malloc(sizeof(int)
                                                                               * n_children);
                    ...
                    for (rank = 0; rank < n_children; rank++) {
                        tree_nodes[node_index].children_ranks[rank] = node
                                                                      + rank
                                                                            * n_nodes_in_this_level;
                        tree_nodes[node_index].children_ranks[rank] += (n_cum_nodes
                                                                        + n_nodes_in_this_level);
                    }
                } else {
                    ...
                    for (rank = 0; rank < n_children; rank++) {
                        tree_nodes[node_index].children_ranks[rank] = rank + tree_order * node;
                        tree_nodes[node_index].children_ranks[rank] += (n_cum_nodes
                                                                        + n_nodes_in_this_level);
                    }
                }
