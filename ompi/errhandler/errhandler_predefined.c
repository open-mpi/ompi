 ...
 * Local functions
 */
static void backend_abort(int fatal, char *type, struct ompi_communicator_t *comm, char *name,
                          int *error_code, va_list arglist);
...
void ompi_mpi_errors_abort_comm_handler(struct ompi_communicator_t **comm, int *error_code, ...)
{
    char *name;
    struct ompi_communicator_t *abort_comm;
    va_list arglist;
...
void ompi_mpi_errors_abort_file_handler(struct ompi_file_t **file, int *error_code, ...)
{
    char *name;
    struct ompi_communicator_t *abort_comm;
    va_list arglist;
...
void ompi_mpi_errors_abort_win_handler(struct ompi_win_t **win, int *error_code, ...)
{
    char *name;
    struct ompi_communicator_t *abort_comm = NULL;
    va_list arglist;
...
static void out(char *str, char *arg)
{
    if (ompi_rte_initialized && ompi_mpi_state < OMPI_MPI_STATE_FINALIZE_PAST_COMM_SELF_DESTRUCT) {
        if (NULL != arg) {
 ...
 * errors here.
 */
static void backend_abort_aggregate(int fatal, char *type, struct ompi_communicator_t *comm,
                                    char *name, int *error_code, va_list arglist)
{
    ...
    if (opal_asprintf(&prefix, "[%s:%05d]", ompi_process_info.nodename, (int) ompi_process_info.pid)
        == -1) {
        prefix = NULL;
    ...
    if (NULL != error_code) {
        ...
        if (NULL == err_msg) {
            if (opal_asprintf(&err_msg, unknown_error_code, *error_code) == -1) {
                err_msg = NULL;
    ...
    if (NULL != name) {
        opal_show_help("help-mpi-errors.txt", fatal ? "mpi_errors_are_fatal" : "mpi_errors_abort",
                       false, usable_prefix, (NULL == arg) ? "" : "in", (NULL == arg) ? "" : arg,
                       usable_prefix, OMPI_PROC_MY_NAME->jobid, OMPI_PROC_MY_NAME->vpid,
                       usable_prefix, type, name, usable_prefix, usable_err_msg, usable_prefix,
                       type, usable_prefix);
    } else {
        opal_show_help("help-mpi-errors.txt", "mpi_errors_are_fatal unknown handle", false,
                       usable_prefix, (NULL == arg) ? "" : "in", (NULL == arg) ? "" : arg,
                       usable_prefix, OMPI_PROC_MY_NAME->jobid, OMPI_PROC_MY_NAME->vpid,
                       usable_prefix, type, usable_prefix, usable_err_msg, usable_prefix, type,
                       usable_prefix);
    }
 ...
 */
static void backend_abort_no_aggregate(int fatal, char *type, struct ompi_communicator_t *comm,
                                       char *name, int *error_code, va_list arglist)
{
    char *arg;
