 ...
 */
enum ompi_errhandler_lang_t { OMPI_ERRHANDLER_LANG_C, OMPI_ERRHANDLER_LANG_FORTRAN };
 ...
 */
OMPI_DECLSPEC extern void (*ompi_initial_error_handler)(struct ompi_communicator_t **comm,
                                                        int *error_code, ...);
...
#define OMPI_ERR_INIT_FINALIZE(name)                                                              \
    {                                                                                             \
        ...
        if (OPAL_UNLIKELY(state < OMPI_MPI_STATE_INIT_COMPLETED                                   \
            ...
            ompi_errhandler_invoke(NULL, NULL, -1, ompi_errcode_get_mpi_code(MPI_ERR_ARG), name); \
        }                                                                                         \
    }
...
#define OMPI_ERRHANDLER_INVOKE(mpi_object, err_code, message)         \
    ompi_errhandler_invoke((mpi_object)->error_handler, (mpi_object), \
                           (int) (mpi_object)->errhandler_type,       \
                           ompi_errcode_get_mpi_code(err_code), (message));
...
#define OMPI_ERRHANDLER_NOHANDLE_INVOKE(err_code, message) \
    ompi_errhandler_invoke(NULL, NULL, -1, ompi_errcode_get_mpi_code(err_code), (message));
...
#define OMPI_ERRHANDLER_CHECK(rc, mpi_object, err_code, message)                                  \
    if (OPAL_UNLIKELY(rc != OMPI_SUCCESS)) {                                                      \
        ...
        ompi_errhandler_invoke((mpi_object)->error_handler, (mpi_object),                         \
                               (int) (mpi_object)->errhandler_type, (__mpi_err_code), (message)); \
...
#define OMPI_ERRHANDLER_NOHANDLE_CHECK(rc, err_code, message)                \
    if (OPAL_UNLIKELY(rc != OMPI_SUCCESS)) {                                 \
        int __mpi_err_code = ompi_errcode_get_mpi_code(err_code);            \
        ompi_errhandler_invoke(NULL, NULL, -1, (__mpi_err_code), (message)); \
        return (__mpi_err_code);                                             \
    }
 ...
 */
OMPI_DECLSPEC int ompi_errhandler_invoke(ompi_errhandler_t *errhandler, void *mpi_object, int type,
 ...
 */
int ompi_errhandler_request_invoke(int count, struct ompi_request_t **requests,
                                   const char *message);
...
OMPI_DECLSPEC int ompi_errhandler_proc_failed_internal(struct ompi_proc_t *ompi_proc, int status,
                                                       bool forward);
