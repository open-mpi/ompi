...
#    define OMPI_TIMING_INIT(_size)                                                              \
        ...
        {                                                                                        \
            ...
            if (OMPI_TIMING.enabled) {                                                           \
                ...
                OMPI_TIMING.timing->val = (ompi_timing_val_t *) malloc(sizeof(ompi_timing_val_t) \
                                                                       * _size);                 \
...
#    define OMPI_TIMING_ITEM_EXTEND                                                           \
        do {                                                                                  \
            if (OMPI_TIMING.enabled) {                                                        \
                OMPI_TIMING.cur_timing->next = (struct ompi_timing_list_t *) malloc(          \
                    sizeof(ompi_timing_list_t));                                              \
                ...
                OMPI_TIMING.cur_timing->val = malloc(sizeof(ompi_timing_val_t)                \
                                                     * OMPI_TIMING.size);                     \
            }                                                                                 \
        } while (0)
...
#    define OMPI_TIMING_NEXT(...)                                                                 \
        do {                                                                                      \
            if (!OMPI_TIMING.error && OMPI_TIMING.enabled) {                                      \
                ...
                OMPI_TIMING.cur_timing->val[OMPI_TIMING.cur_timing->use].prefix = strdup(         \
                    __func__);                                                                    \
                OMPI_TIMING.cur_timing->val[OMPI_TIMING.cur_timing->use++].ts = OMPI_TIMING       \
                                                                                    .get_ts()     \
                                                                                - OMPI_TIMING.ts; \
                OMPI_TIMING.cnt++;                                                                \
                OMPI_TIMING.ts = OMPI_TIMING.get_ts();                                            \
...
#    define OMPI_TIMING_IMPORT_OPAL(func) OMPI_TIMING_IMPORT_OPAL_PREFIX("", func);
...
#    define OMPI_TIMING_OUT                                                                       \
        do {                                                                                      \
            if (OMPI_TIMING.enabled) {                                                            \
                ...
                int imported = 0;                                                                 \
                                                                                                  \
                MPI_Reduce(&OMPI_TIMING.error, &error, 1, MPI_INT, MPI_SUM, 0, MPI_COMM_WORLD);   \
                                                                                                  \
                if (error) {                                                                      \
                    if (0 == rank) {                                                              \
                        printf("==OMPI_TIMING== error: something went wrong, timings doesn't "    \
                               "work\n");                                                         \
                    }                                                                             \
                } else {                                                                          \
                    ...
                    if (OMPI_TIMING.cnt > 0) {                                                    \
                        ...
                        do {                                                                      \
                            ...
                            for (use = 0; use < timing->use; use++) {                             \
                                MPI_Reduce(&timing->val[use].ts, avg + i, 1, MPI_DOUBLE, MPI_SUM, \
                                           0, MPI_COMM_WORLD);                                    \
                                MPI_Reduce(&timing->val[use].ts, min + i, 1, MPI_DOUBLE, MPI_MIN, \
                                           0, MPI_COMM_WORLD);                                    \
                                MPI_Reduce(&timing->val[use].ts, max + i, 1, MPI_DOUBLE, MPI_MAX, \
                                           0, MPI_COMM_WORLD);                                    \
                        ...
                        if (0 == rank) {                                                          \
                            if (OMPI_TIMING.timing->next) {                                       \
                                printf("==OMPI_TIMING== warning: added the extra timings "        \
                                       "allocation that might misrepresent the results.\n"        \
                                       "==OMPI_TIMING==          Increase the inited size of "    \
                                       "timings to avoid extra allocation during runtime.\n");    \
                            }                                                                     \
                                                                                                  \
                            printf("------------------ %s ------------------\n",                  \
                                   OMPI_TIMING.prefix);                                           \
                            ...
                            for (i = 0; i < OMPI_TIMING.cnt; i++) {                               \
                                ...
                                if (i == (OMPI_TIMING.cnt - 1)) {                                 \
                                    ...
                                    print_total = imported                                        \
                                                  != OMPI_TIMING.timing->val[i + 1].imported;     \
                                }                                                                 \
                                if (print_total && OMPI_TIMING.timing->val[i].imported) {         \
                                    printf("%s[%s:%s:%s]: %lf / %lf / %lf\n",                     \
                                           imported ? " !! " : "", file[i], prefix[i], "total",   \
