...
def normalized_parameters(proc, param_descs, subs, ref_resolve):
    """Build the normalized (language-neutral) parameter list for a procedure.

    Iterates the language-independent specification (``express.lis``). The
    ``type`` is the MPI semantic kind name (e.g. BUFFER, RANK, COMMUNICATOR),
    which is language-neutral so the same list is valid for every binding.
    Human descriptions come from the RST parameter bullets when present,
    falling back to the language-independent specification's description.
    """
    params = []
    try:
        lis_params = proc.express.lis.parameters
    except Exception:
        lis_params = []
    for p in lis_params:
        name = p.name
        direction = _normalize_direction(getattr(p, 'direction', None))
        kind = getattr(p, 'kind', None)
        kind_name = getattr(kind, 'name', None) if kind is not None else None
        desc = param_descs.get(name)
        if not desc:
            desc = getattr(p, 'description', None) or None
        if desc:
        ...
        params.append({
            ...
            "direction": direction,
            "type": kind_name,
            "intent": direction if direction in ('in', 'out', 'inout') else None,
            "optional": False,
            "description": desc,
        })
    return params
