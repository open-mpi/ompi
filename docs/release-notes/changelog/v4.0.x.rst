...
- Fix finalize of flux components.
...
- Fix a problem with the ORTE ``rmaps_base_oversubscribe`` MCA parameter.
  Thanks to @iassiour for reporting.
...
- Allow for running as root if two environment variables are set.
