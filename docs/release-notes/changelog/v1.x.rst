...
- Add missing MPI_AINT_ADD and MPI_AINT_DIFF function declarations in
  mpif.h
...
- Fixed incorrect memalign threshold in the openib BTL.  Thanks to
...
- Fix MPI_SIZEOF; now available in mpif.h for modern Fortran compilers
  ...
  errors.
- Fixed inadvertent Fortran ABI break between v1.8.1 and v1.8.2 in the
...
- usnic BTL now checks for common connectivity errors at first send to
  a remote server.
...
- Port some upstream hwloc fixes to Open MPI's embedded copy for
  working around buggy NUMA node cpusets and including missing header
...
- Made malloc hooks more friendly to IO interposers.  Thanks to the
...
- Ensure that the OMPI version number is tolerant of handling spaces.
...
- Needed to add volatile keyword to a variable used in debugging
  (MPIR_being_debugged).
...
- Various enhancements to the openib BTL:
   ...
   - Added RDMA CM support, including ``btl_openib_cpc_[in|ex]clude``
     MCA parameters
...
- Fix a number of race conditions with the MPI-2 Onesided
  interface.
- Fix the "tuned" collective component where some cases where
...
- Automatically disable using mallopt() on AIX.
...
- Fix missing MPI_F_STATUS_IGNORE and MPI_F_STATUSES_IGNORE
  instantiations.  Thanks to Anthony Chan for pointing this out.
