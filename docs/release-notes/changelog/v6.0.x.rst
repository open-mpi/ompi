...
- ``MPI_Comm_connect``, ``MPI_Comm_accept`` and ``MPI_Comm_spawn`` now
  detect that the two jobs selected different PMLs, and fail with a
  message naming both instead of failing obscurely later. The two
  roots find this out through the information they already exchange on
  the port, which now also names the PML. A connect against an Open
  MPI version that does not send it is therefore no longer possible;
  the content of that exchange has never been compatible across
  versions.
