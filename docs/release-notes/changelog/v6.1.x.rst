...
- MPI_T event producers now emit the correct MPI object handle
  representation for the tool's MPI ABI.  When a process runs under the
  MPI Standard ABI, the communicator, window, session, error-handler,
  and file handles carried in MPI_T event payloads are now the Standard
  ABI integer handles instead of a placeholder value; under the Open MPI
  ABI they remain the internal object handle as before.
