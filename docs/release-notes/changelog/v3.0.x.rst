...
- Fixed inadvertent use of bitwise operators in the MPI C++ bindings
...
- Fix possible floating point rounding and division issues in OMPIO
  ...
  Thanks to Axel Huebl and René Widera for identifying the issue,
...
- Fix RMA function signatures for ``use-mpi-f08`` bindings to have the
  asynchronous property on all buffers.
...
- Fix a problem reported on the mailing separately by Kevin McGrattan and Stephen
  ...
  also introduces a new mca parameter ``fs_ufs_lock_algorithm`` which allows users to
  control the locking algorithm used by ompio for read/write operations. By
  default, ompio does not perform locking on local UNIX file systems, locks the
