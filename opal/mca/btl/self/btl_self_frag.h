...
#define MCA_BTL_SELF_FRAG_ALLOC_EAGER(frag)                \
    {                                                      \
        frag = (mca_btl_self_frag_t *) opal_free_list_get( \
            &mca_btl_self_component.self_frags_eager);     \
    }

#define MCA_BTL_SELF_FRAG_ALLOC_RDMA(frag)                 \
    {                                                      \
        frag = (mca_btl_self_frag_t *) opal_free_list_get( \
            &mca_btl_self_component.self_frags_rdma);      \
    }

#define MCA_BTL_SELF_FRAG_ALLOC_SEND(frag)                 \
    {                                                      \
        frag = (mca_btl_self_frag_t *) opal_free_list_get( \
            &mca_btl_self_component.self_frags_send);      \
    }
