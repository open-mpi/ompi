...
#ifndef MCA_BTL_IWARP_IP_H
#define MCA_BTL_IWARP_IP_H
 ...
 */
extern uint64_t mca_btl_iwarp_get_ip_subnet_id(struct ibv_device *ib_dev,
                                                   uint8_t port);
 ...
 * @return             IPv4 Address
 */
extern uint32_t mca_btl_iwarp_rdma_get_ipv4addr(struct ibv_context *verbs,
                                                 uint8_t port);

/**
 * Create a list of all available IBV devices and each device's
 * relevant information.  This is necessary for
 * mca_btl_iwarp_rdma_get_ipv4addr to work.
 *
 * @return OPAL_SUCCESS or failure status
 */
extern int mca_btl_iwarp_build_rdma_addr_list(void);

/**
 * Free the list of all available IBV devices created by
 * mca_btl_iwarp_build_rdma_addr_list.
 */
extern void mca_btl_iwarp_free_rdma_addr_list(void);
