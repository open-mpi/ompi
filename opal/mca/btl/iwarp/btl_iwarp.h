...
#define MCA_BTL_IWARP_CQ_POLL_BATCH_DEFAULT (256)
...
struct mca_btl_iwarp_pp_qp_info_t {
    int32_t rd_win;
    int32_t rd_rsv;
}; typedef struct mca_btl_iwarp_pp_qp_info_t mca_btl_iwarp_pp_qp_info_t;

struct mca_btl_iwarp_srq_qp_info_t {
    int32_t sd_max;
    ...
    int32_t srq_limit;
}; typedef struct mca_btl_iwarp_srq_qp_info_t mca_btl_iwarp_srq_qp_info_t;

struct mca_btl_iwarp_qp_info_t {
    mca_btl_iwarp_qp_type_t type;
    size_t size;
    int32_t rd_num;
    int32_t rd_low;
    union {
        mca_btl_iwarp_pp_qp_info_t pp_qp;
        mca_btl_iwarp_srq_qp_info_t srq_qp;
    } u;
}; typedef struct mca_btl_iwarp_qp_info_t mca_btl_iwarp_qp_info_t;

#define BTL_IWARP_QP_TYPE(Q) (mca_btl_iwarp_component.qp_infos[(Q)].type)
#define BTL_IWARP_QP_TYPE_PP(Q) \
...
} btl_iwarp_receive_queues_source_t;

typedef enum {
    BTL_IWARP_DT_IB,
    BTL_IWARP_DT_IWARP,
    BTL_IWARP_DT_ALL
} btl_iwarp_device_type_t;

/* The structer for manage all BTL SRQs */
typedef struct mca_btl_iwarp_srq_manager_t {
    opal_mutex_t lock;
...
} mca_btl_iwarp_srq_manager_t;

struct mca_btl_iwarp_component_t {
    mca_btl_base_component_3_0_0_t          super;  /**< base BTL component */

    int                                ib_max_btls;
    /**< maximum number of devices available to iwarp component */

    int                                ib_num_btls;
    /**< number of devices available to the iwarp component */

    struct mca_btl_iwarp_module_t             **iwarp_btls;
    ...
    uint32_t ib_cq_size[BTL_IWARP_MAX_CQ];  /**< Max outstanding CQE on the CQ */
    ...
    mca_btl_iwarp_srq_manager_t srq_manager;     /**< Hash table for all BTL SRQs */
    /* declare as an int instead of btl_iwarp_device_type_t since there is no
    ...
    /* MCA param btl_iwarp_receive_queues */
    char *receive_queues;
    /* Whether we got a non-default value of btl_iwarp_receive_queues */
    ...
    /** Whether we want a warning if the user specifies a non-existent
        device and/or port via btl_iwarp_if_[in|ex]clude MCA params */
...
OPAL_MODULE_DECLSPEC extern mca_btl_iwarp_component_t mca_btl_iwarp_component;

typedef mca_btl_base_recv_reg_t mca_btl_iwarp_recv_reg_t;
...
} mca_btl_iwarp_device_qp_t;

struct mca_btl_base_endpoint_t;

typedef struct mca_btl_iwarp_device_t {
    opal_object_t super;
    ...
    struct ibv_pd *ib_pd;
    struct ibv_cq *ib_cq[BTL_IWARP_MAX_CQ];
    uint32_t cq_size[BTL_IWARP_MAX_CQ];
...
} mca_btl_iwarp_device_t;
OBJ_CLASS_DECLARATION(mca_btl_iwarp_device_t);

struct mca_btl_iwarp_module_pp_qp_t {
    int32_t dummy;
}; typedef struct mca_btl_iwarp_module_pp_qp_t mca_btl_iwarp_module_pp_qp_t;

struct mca_btl_iwarp_module_srq_qp_t {
    struct ibv_srq *srq;
    int32_t rd_posted;
...
}; typedef struct mca_btl_iwarp_module_srq_qp_t mca_btl_iwarp_module_srq_qp_t;

struct mca_btl_iwarp_module_qp_t {
    union {
        mca_btl_iwarp_module_pp_qp_t pp_qp;
        mca_btl_iwarp_module_srq_qp_t srq_qp;
    } u;
}; typedef struct mca_btl_iwarp_module_qp_t mca_btl_iwarp_module_qp_t;
...
struct mca_btl_iwarp_module_t {
    ...
    mca_btl_iwarp_modex_message_t port_info;
    ...
    mca_btl_iwarp_module_qp_t * qps;

    int local_procs;                   /** number of local procs */

    bool atomic_ops_be;                /** atomic result is big endian */
};
typedef struct mca_btl_iwarp_module_t mca_btl_iwarp_module_t;

extern mca_btl_iwarp_module_t mca_btl_iwarp_module;
...
typedef struct mca_btl_iwarp_reg_t mca_btl_iwarp_reg_t;

#if OPAL_ENABLE_PROGRESS_THREADS == 1
extern void* mca_btl_iwarp_progress_thread(opal_object_t*);
#endif
...
extern int mca_btl_iwarp_finalize(
    struct mca_btl_base_module_t* btl
);
...
extern int mca_btl_iwarp_add_procs(
    struct mca_btl_base_module_t* btl,
    size_t nprocs,
 ...
 *
 */
extern int mca_btl_iwarp_del_procs(
    struct mca_btl_base_module_t* btl,
    size_t nprocs,
    struct opal_proc_t **procs,
    struct mca_btl_base_endpoint_t** peers
 ...
 */
extern int mca_btl_iwarp_sendi( struct mca_btl_base_module_t* btl,
...
struct mca_btl_iwarp_put_frag_t;
 ...
 * @param frag (IN)              Fragment prepared by mca_btl_iwarp_put
 *
 ...
 * @param frag (IN)              Fragment prepared by mca_btl_iwarp_get
 *
 * If the fragment can not be scheduled due to resource limitations then
 ...
 */
extern mca_btl_base_descriptor_t* mca_btl_iwarp_alloc(
        struct mca_btl_base_module_t* btl,
...
extern void mca_btl_iwarp_frag_progress_pending_put_get(
 ...
 */
extern int mca_btl_iwarp_ft_event(int state);
 ...
 *
 * @param iwarp_btl (IN) BTL module
 * @param additional (IN) Additional Bytes to reserve
 * @param prio (IN)       Priority (either BTL_IWARP_HP_QP or BTL_IWARP_LP_QP)
 * @return OPAL_SUCCESS or failure status
 */
...
const char* btl_iwarp_get_transport_name(mca_btl_iwarp_transport_type_t transport_type);
...
{
    if(0 == qp)
        return BTL_IWARP_HP_CQ; /* smallest qp is always HP */
    ...
    return (mca_btl_iwarp_component.qp_infos[qp].size <=
        ...
        BTL_IWARP_HP_CQ : BTL_IWARP_LP_CQ;
}

#define BTL_IWARP_RDMA_QP(QP) \
