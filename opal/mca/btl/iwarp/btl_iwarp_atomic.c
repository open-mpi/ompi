...
static int mca_btl_iwarp_atomic_internal (struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
...
{
    mca_btl_iwarp_get_frag_t* frag = NULL;
    int qp = order;
    int32_t rkey;
    int rc;
    ...
    if (MCA_BTL_NO_ORDER == qp) {
        qp = mca_btl_iwarp_component.rdma_qp;
    }
    ...
    rc = mca_btl_iwarp_get_internal (btl, endpoint, frag);
...
int mca_btl_iwarp_atomic_fop (struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                               ...
                               void *cbcontext, void *cbdata)
{
    ...
    return mca_btl_iwarp_atomic_internal (btl, endpoint, local_address, remote_address, local_handle,
					   remote_handle, IBV_WR_ATOMIC_FETCH_AND_ADD, operand, 0,
					   flags, order, cbfunc, cbcontext, cbdata);
}

int mca_btl_iwarp_atomic_cswap (struct mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                                 void *local_address, uint64_t remote_address,
                                 struct mca_btl_base_registration_handle_t *local_handle,
