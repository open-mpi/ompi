...
static void mca_btl_iwarp_proc_btl_construct(mca_btl_iwarp_proc_btlptr_t* elem);
static void mca_btl_iwarp_proc_btl_destruct(mca_btl_iwarp_proc_btlptr_t* elem);

OBJ_CLASS_INSTANCE(mca_btl_iwarp_proc_btlptr_t,
        opal_list_item_t, mca_btl_iwarp_proc_btl_construct,
        mca_btl_iwarp_proc_btl_destruct);
...
static void mca_btl_iwarp_proc_construct(mca_btl_iwarp_proc_t* proc);
static void mca_btl_iwarp_proc_destruct(mca_btl_iwarp_proc_t* proc);

OBJ_CLASS_INSTANCE(mca_btl_iwarp_proc_t,
        opal_list_item_t, mca_btl_iwarp_proc_construct,
        mca_btl_iwarp_proc_destruct);

void mca_btl_iwarp_proc_construct(mca_btl_iwarp_proc_t* ib_proc)
{
    ib_proc->proc_opal           = 0;
    ...
    OBJ_CONSTRUCT(&ib_proc->iwarp_btls, opal_list_t);
}
...
void mca_btl_iwarp_proc_destruct(mca_btl_iwarp_proc_t* ib_proc)
{
    /* release resources */
    ...
    OPAL_LIST_DESTRUCT(&ib_proc->iwarp_btls);
}
 ...
 */
static mca_btl_iwarp_proc_t* ibproc_lookup_no_lock(opal_proc_t* proc)
{
    mca_btl_iwarp_proc_t* ib_proc;

    OPAL_LIST_FOREACH(ib_proc, &mca_btl_iwarp_component.ib_procs, mca_btl_iwarp_proc_t) {
...
static mca_btl_iwarp_proc_t* ibproc_lookup_and_lock(opal_proc_t* proc)
{
    mca_btl_iwarp_proc_t* ib_proc;

    /* get the process from the list */
    opal_mutex_lock(&mca_btl_iwarp_component.ib_lock);
    ib_proc = ibproc_lookup_no_lock(proc);
    opal_mutex_unlock(&mca_btl_iwarp_component.ib_lock);
    if( NULL != ib_proc ){
 ...
 * between a opal_proc_t and a mca_btl_iwarp_proc_t instance. We
 * cache additional data (specifically the list of
 * mca_btl_iwarp_endpoint_t instances, and published addresses)
...
{
    mca_btl_iwarp_proc_t *ib_proc = NULL, *ib_proc_ret = NULL;
    size_t msg_size;
    uint32_t size;
    int rc, i, j;
    void *message;
    ...
    mca_btl_iwarp_modex_message_t dummy;
    bool is_new = false;
    ...
    ib_proc = OBJ_NEW(mca_btl_iwarp_proc_t);
    if (NULL == ib_proc) {
      return NULL;
    }
    ...
    OPAL_MODEX_RECV(rc, &mca_btl_iwarp_component.super.btl_version,
    ...
    /* Message was packed in btl_iwarp_component.c; the format is
    ...
    if (ib_proc->proc_port_count > 0) {
        ib_proc->proc_ports = (mca_btl_iwarp_proc_modex_t *)
            malloc(sizeof(mca_btl_iwarp_proc_modex_t) *
    ...
    for (i = 0; i < ib_proc->proc_port_count; i++) {
        ...
        MCA_BTL_IWARP_MODEX_MSG_NTOH(ib_proc->proc_ports[i].pm_port_info);
#endif
        offset += size;
        ...
        ib_proc->proc_ports[i].pm_cpc_data = (opal_btl_iwarp_connect_base_module_data_t *)
            calloc(ib_proc->proc_ports[i].pm_cpc_data_count,
                   sizeof(opal_btl_iwarp_connect_base_module_data_t));
        ...
        for (j = 0; j < ib_proc->proc_ports[i].pm_cpc_data_count; ++j) {
            uint8_t u8;
            opal_btl_iwarp_connect_base_module_data_t *cpcd;
            ...
            cpcd->cbm_component =
                opal_btl_iwarp_connect_base_get_cpc_byindex(u8);
    ...
    if (NULL == ib_proc_ret) {
        ...
        opal_mutex_lock(&ib_proc->proc_lock);
        opal_list_append(&mca_btl_iwarp_component.ib_procs, &ib_proc->super);
        ib_proc_ret = ib_proc;
        is_new = true;
    } else {
     ...
     * won't lock mca_btl_iwarp_component.ib_lock */
    if( !is_new ){
...
err_exit:
    ...
    BTL_ERROR(("%d: error exit from mca_btl_iwarp_proc_create", OPAL_PROC_MY_NAME.vpid));
...
{
    size_t i;
    mca_btl_iwarp_proc_t* ib_proc = NULL;

    /* Remove endpoint from the iwarp BTL version of the proc as
       well */
    ib_proc = ibproc_lookup_and_lock(proc);
    if (NULL != ib_proc) {
 ...
 * it an address.
 */
int mca_btl_iwarp_proc_insert(mca_btl_iwarp_proc_t* module_proc,
...
{
    mca_btl_iwarp_proc_btlptr_t* elem;

    OPAL_LIST_FOREACH(elem, &ib_proc->iwarp_btls, mca_btl_iwarp_proc_btlptr_t) {
        if(elem->iwarp_btl == iwarp_btl) {
            /* this is normal return meaning that this BTL has already touched this ib_proc */
            return OPAL_ERR_RESOURCE_BUSY;
        }
    }

    elem = OBJ_NEW(mca_btl_iwarp_proc_btlptr_t);
    if( NULL == elem ){
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    elem->iwarp_btl = iwarp_btl;
    opal_list_append(&ib_proc->iwarp_btls, &elem->super);
    return OPAL_SUCCESS;
}
