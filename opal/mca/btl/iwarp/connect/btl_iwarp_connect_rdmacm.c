...
static int rdmacm_component_query(mca_btl_iwarp_module_t *iwarp_btl,
                                  opal_btl_iwarp_connect_base_module_t **cpc);
static int rdmacm_component_finalize(void);

opal_btl_iwarp_connect_base_component_t opal_btl_iwarp_connect_rdmacm = {
    "rdmacm",
...
static void rdmacm_component_register(void)
{
    ...
    if (rdmacm_port & ~0xfffful) {
        opal_show_help("help-mpi-btl-iwarp-cpc-rdmacm.txt",
 ...
 */
static mca_btl_iwarp_endpoint_t *rdmacm_find_endpoint(rdmacm_contents_t *contents,
                                                       opal_process_name_t rem_name)
{
    mca_btl_iwarp_module_t *btl = contents->iwarp_btl;
    mca_btl_iwarp_endpoint_t *ep = NULL;
    ...
    ep = mca_btl_iwarp_get_ep (&btl->super, opal_proc);
    if (NULL == ep) {
 ...
 */
static uint32_t max_inline_size(int qp, mca_btl_iwarp_device_t *device)
{
    if (mca_btl_iwarp_component.qp_infos[qp].size <= device->max_inline_data) {
        /* If qp message size is smaller than max_inline_data,
         * we should enable inline messages */
        return mca_btl_iwarp_component.qp_infos[qp].size;
    } else if (mca_btl_iwarp_component.rdma_qp == qp || 0 == qp) {
                           ...
                           int qpnum)
{
    ...
    if (qpnum == mca_btl_iwarp_component.credits_qp) {
        int qp;

        for (qp = 0; qp < mca_btl_iwarp_component.num_qps; qp++) {
            if(BTL_IWARP_QP_TYPE_PP(qp)) {
                reserved += mca_btl_iwarp_component.qp_infos[qp].u.pp_qp.rd_rsv;
            }
        }
        credits = mca_btl_iwarp_component.num_qps;
    }

    if (BTL_IWARP_QP_TYPE_PP(qpnum)) {
        max_recv_wr = mca_btl_iwarp_component.qp_infos[qpnum].rd_num + reserved;
        max_send_wr = mca_btl_iwarp_component.qp_infos[qpnum].rd_num + credits;
    } else {
        srq = endpoint->endpoint_btl->qps[qpnum].u.srq_qp.srq;
        max_recv_wr = reserved;
        max_send_wr = mca_btl_iwarp_component.qp_infos[qpnum].u.srq_qp.sd_max + credits;
    }

    memset(&attr, 0, sizeof(attr));
    attr.qp_type = IBV_QPT_RC;
    attr.send_cq = contents->iwarp_btl->device->ib_cq[BTL_IWARP_LP_CQ];
    attr.recv_cq = contents->iwarp_btl->device->ib_cq[qp_cq_prio(qpnum)];
    attr.srq = srq;
    if(BTL_IWARP_QP_TYPE_PP(qpnum)) {
    ...
    attr.cap.max_inline_data = req_inline =
        max_inline_size(qpnum, contents->iwarp_btl->device);
...

    {
        ...
        id->verbs = contents->iwarp_btl->device->ib_pd->context;
        if (0 != rdma_create_qp(id, contents->iwarp_btl->device->ib_pd,
                                &attr)) {
    ...
    if (attr.cap.max_inline_data < req_inline) {
        ...
        opal_show_help("help-mpi-btl-iwarp-cpc-base.txt",
                       ...
                       ibv_get_device_name(contents->iwarp_btl->device->ib_dev),
                       contents->iwarp_btl->port_num,
 ...
 */
static int rdmacm_module_start_connect(opal_btl_iwarp_connect_base_module_t *cpc,
                                       mca_btl_base_endpoint_t *endpoint)
{
    ...
    contents->iwarp_btl = endpoint->endpoint_btl;
    ...
    if (contents->endpoint->endpoint_initiator) {
        ...
        if (OPAL_SUCCESS !=
            (rc = opal_btl_iwarp_connect_base_alloc_cts(contents->endpoint))) {
...
static int handle_connect_request(struct rdma_cm_event *event)
{
    ...
    /* Post a single receive buffer on the smallest QP for the CTS
       protocol */
    if (mca_btl_iwarp_component.credits_qp == qpnum) {
        struct ibv_recv_wr *bad_wr, *wr;

        if (OPAL_SUCCESS !=
            opal_btl_iwarp_connect_base_alloc_cts(endpoint)) {
            BTL_ERROR(("Failed to alloc CTS frag"));
            goto out1;
        }
    ...
    conn_param.responder_resources =
        mymin(contents->iwarp_btl->device->ib_dev_attr.max_qp_rd_atom,
              message->device_max_qp_init_rd_atom);
    conn_param.initiator_depth =
        mymin(contents->iwarp_btl->device->ib_dev_attr.max_qp_init_rd_atom,
              message->device_max_qp_rd_atom);
    conn_param.retry_count = mca_btl_iwarp_component.ib_retry_count;
    conn_param.rnr_retry_count = BTL_IWARP_QP_TYPE_PP(qpnum) ? 0 :
        mca_btl_iwarp_component.ib_rnr_retry;
    conn_param.srq = BTL_IWARP_QP_TYPE_SRQ(qpnum);
...
static void *local_endpoint_cpc_complete(void *context)
{
    ...
    mca_btl_iwarp_endpoint_cpc_complete(endpoint);
                                   ...
                                   struct rdma_cm_event *event)
{
    ...
    /* Only notify the upper layers after the last QP has been
       connected */
    if (++data->rdmacm_counter < mca_btl_iwarp_component.num_qps) {
    ...
    opal_atomic_wmb();
    mca_btl_iwarp_run_in_main (local_endpoint_cpc_complete, endpoint);
...
{
    contents->dummy_cq =
        ibv_create_cq(iwarp_btl->device->ib_dev_context, 1, NULL, NULL, 0);
...
static int finish_connect(id_context_t *context)
{
    ...
    if (contents->endpoint->endpoint_initiator) {
        ...
        if (mca_btl_iwarp_component.credits_qp == context->qpnum) {
            /* Post a single receive buffer on the smallest QP for the CTS
               protocol */
         ...
         * event.
         */
        rc = create_dummy_cq(contents, contents->iwarp_btl);
        if (0 != rc) {
    ...
    conn_param.retry_count = mca_btl_iwarp_component.ib_retry_count;
    conn_param.rnr_retry_count = BTL_IWARP_QP_TYPE_PP(context->qpnum) ? 0 :
        mca_btl_iwarp_component.ib_rnr_retry;
    conn_param.srq = BTL_IWARP_QP_TYPE_SRQ(context->qpnum);
    conn_param.private_data = &msg;
    conn_param.private_data_len = sizeof(private_data_t);
...
static int event_handler(struct rdma_cm_event *event)
{
    ...
    case RDMA_CM_EVENT_CONNECT_ERROR:
        /* Some adapters have broken REJECT behavior; the recipient
           ...
           btl_iwarp_connect_rdmacm_reject_causes_connect_error is
        ...
        if (NULL != context->contents->dummy_cq) {
            ...
            found = false;
            if (OPAL_SUCCESS == opal_btl_iwarp_ini_query(attr->vendor_id,
                                                          attr->vendor_part_id,
                                                          &ini) &&
...
static inline void rdmamcm_event_error(struct rdma_cm_event *event)
{
    ...
    mca_btl_iwarp_run_in_main (mca_btl_iwarp_endpoint_invoke_error,
                                endpoint);
}
 ...
 */
static int rdmacm_init(mca_btl_iwarp_endpoint_t *endpoint)
{
    void *data;
                       ...
                       mca_btl_iwarp_module_t *iwarp_btl)
{
     ...
     * mca_btl_iwarp_get_ip_subnet_id in btl_iwarp_ip.c
     */
    ipaddr =
        mca_btl_iwarp_rdma_get_ipv4addr(iwarp_btl->device->ib_dev_context,
                                         iwarp_btl->port_num);
    if (0 == ipaddr) {
    ...
    BTL_VERBOSE(("Found device %s:%d = IP address %s:%d",
                 ibv_get_device_name(iwarp_btl->device->ib_dev),
                 iwarp_btl->port_num, str, server_tcp_port));
    free(str);
                          ...
                          opal_btl_iwarp_connect_base_module_data_t *data)
{
    ...
    message->device_max_qp_rd_atom =
        iwarp_btl->device->ib_dev_attr.max_qp_rd_atom;
    message->device_max_qp_init_rd_atom =
 ...
 */
static int rdmacm_component_query(mca_btl_iwarp_module_t *iwarp_btl, opal_btl_iwarp_connect_base_module_t **cpc)
{
    int rc;
    ...
    *cpc = (opal_btl_iwarp_connect_base_module_t *) malloc(sizeof(opal_btl_iwarp_connect_base_module_t));
    if (NULL == *cpc) {
        rc = OPAL_ERR_OUT_OF_RESOURCE;
        goto out;
    }

    (*cpc)->data.cbm_component = &opal_btl_iwarp_connect_rdmacm;
    ...
    server->server = true;
    server->iwarp_btl = iwarp_btl;

    context = OBJ_NEW(id_context_t);
    OPAL_OUTPUT((-1, "MAIN Server context: %p", (void*) context));
    if (NULL == context) {
        opal_output_verbose(5, opal_btl_base_framework.framework_output,
                            "iwarp BTL: rdmacm CPC system error (malloc failed)");
        rc = OPAL_ERR_OUT_OF_RESOURCE;
        goto out3;
    }
    ...
    if (0 != rc) {
        opal_output_verbose(5, opal_btl_base_framework.framework_output,
                            "iwarp BTL: rdmacm CPC failed to create ID");
        rc = OPAL_ERR_OUT_OF_RESOURCE;
        goto out4;
    }
    ...
    if (OPAL_SUCCESS != rc) {
        opal_output_verbose(5, opal_btl_base_framework.framework_output,
                            "iwarp BTL: rdmacm IP address not found on port");
        rc = OPAL_ERR_NOT_SUPPORTED;
        goto out5;
    }
    ...
    if (0 != rc) {
        opal_output_verbose(5, opal_btl_base_framework.framework_output,
                            "iwarp BTL: rdmacm CPC unable to listen");
        rc = OPAL_ERR_UNREACH;
        goto out5;
    }

    rc = create_message(server, iwarp_btl, &(*cpc)->data);
    if (0 != rc) {
        opal_output_verbose(5, opal_btl_base_framework.framework_output,
                            "iwarp BTL: rdmacm CPC unable to create message");
        rc = OPAL_ERR_OUT_OF_RESOURCE;
        goto out5;
    }
    ...
    opal_output_verbose(5, opal_btl_base_framework.framework_output,
                        "iwarp BTL: rdmacm CPC available for use on %s:%d",
                        ibv_get_device_name(iwarp_btl->device->ib_dev),
                        iwarp_btl->port_num);
    return OPAL_SUCCESS;
...
out:
    if (OPAL_ERR_NOT_SUPPORTED == rc) {
        opal_output_verbose(5, opal_btl_base_framework.framework_output,
                            "iwarp BTL: rdmacm CPC unavailable for use on %s:%d; skipped",
                            ibv_get_device_name(iwarp_btl->device->ib_dev),
                            iwarp_btl->port_num);
    } else {
        opal_output_verbose(5, opal_btl_base_framework.framework_output,
                            "iwarp BTL: rmacm CPC unavailable for use on %s:%d; fatal error %d (%s)",
                            ibv_get_device_name(iwarp_btl->device->ib_dev),
                            iwarp_btl->port_num, rc,
                            opal_strerror(rc));
    }
    return rc;
}
...
static int rdmacm_component_finalize(void)
{
    ...
    mca_btl_iwarp_free_rdma_addr_list();
...
static int rdmacm_component_init(void)
{
    ...
    if (NULL == event_channel) {
        opal_output_verbose(5, opal_btl_base_framework.framework_output,
                            "iwarp BTL: rdmacm CPC failed to create channel");
    ...
    if (NULL == rdmacm_event_base) {
        opal_output_verbose (5, opal_btl_base_framework.framework_output,
                             "iwarp BTL: could not create rdmacm event thread");
        return OPAL_ERR_UNREACH;
    }
