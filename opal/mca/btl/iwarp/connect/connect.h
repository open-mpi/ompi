 ...
 * RC connections are made from the rest of the iwarp BTL.  There are
 ...
 *   a remote BTL module (in the iwarp BTL, because of BSRQ, an
 ...
 * Component interface:
 *
 * - component_register(): The iwarp BTL's component_open() function
 ...
 * btl_iwarp_cpc_include and btl_iwarp_cpc_exclude MCA parameters
 ...
 *
 * - component_init(): The iwarp BTL's component_init() function
 ...
 * OpenFabrics port.
 *
 * component_finalize(): The iwarp BTL's component_close() function
 ...
 *     call opal_btl_iwarp_post_recvs().
 *   - if false: the CPC will call opal_btl_iwarp_post_recvs() before
 *     calling opal_btl_iwarp_cpc_complete().
 *
 * There are two functions in the main iwarp BTL that the CPC may
 * call:
 *
 * - opal_btl_iwarp_post_recvs(endpoint): once a QP is locally
 ...
 *
 * - opal_btl_iwarp_cpc_complete(endpoint): once that a CPC knows
 * that a QP is connected on *both* sides, this function is invoked to
 * tell the main iwarp BTL "ok, you can use this connection now."
 * (e.g., the main iwarp BTL will either invoke the CTS protocol or
 * start sending out fragments that were queued while the connection
 * was establishing, etc.).
 */
#ifndef BTL_IWARP_CONNECT_H
#define BTL_IWARP_CONNECT_H
 ...
 */
typedef void (*opal_btl_iwarp_connect_base_component_register_fn_t)(void);

/**
 * This function is invoked once by the iwarp BTL component during
 ...
 *   likely causing a fatal error (and/or the iwarp BTL component
 *   being ignored).
 */
typedef int (*opal_btl_iwarp_connect_base_component_init_fn_t)(void);
 ...
 * setup to run on this iwarp module (per description above).
 *
 ...
 */
typedef int (*opal_btl_iwarp_connect_base_func_component_query_t)
    (struct mca_btl_iwarp_module_t *btl,
     struct opal_btl_iwarp_connect_base_module_t **cpc);

/**
 * This function is invoked once by the iwarp BTL component during
 * shutdown.  It is intended to have CPC component-wide shutdown.
 */
typedef int (*opal_btl_iwarp_connect_base_component_finalize_fn_t)(void);

/**
 * CPC component struct
 */
struct opal_btl_iwarp_connect_base_component_t {
    /** Name of this set of connection functions */
    char cbc_name[BCF_MAX_NAME];

    /** Register function.  Can be NULL. */
    opal_btl_iwarp_connect_base_component_register_fn_t cbc_register;

    /** CPC component init function.  Can be NULL. */
    opal_btl_iwarp_connect_base_component_init_fn_t cbc_init;

    /** Query the CPC component to get a CPC module corresponding to
        an iwarp BTL module.  Cannot be NULL. */
    opal_btl_iwarp_connect_base_func_component_query_t cbc_query;

    /** CPC component finalize function.  Can be NULL. */
    opal_btl_iwarp_connect_base_component_finalize_fn_t cbc_finalize;
};
/**
 * Convenience typedef
 */
typedef struct opal_btl_iwarp_connect_base_component_t opal_btl_iwarp_connect_base_component_t;
 ...
 */
typedef int (*opal_btl_iwarp_connect_base_module_endpoint_init_fn_t)
    (struct mca_btl_base_endpoint_t *endpoint);

/**
 * Function to initiate a connection to a remote process.
 */
typedef int (*opal_btl_iwarp_connect_base_module_start_connect_fn_t)
    (struct opal_btl_iwarp_connect_base_module_t *cpc,
     struct mca_btl_base_endpoint_t *endpoint);
 ...
 */
typedef int (*opal_btl_iwarp_connect_base_module_endpoint_finalize_fn_t)
     (struct mca_btl_base_endpoint_t *endpoint);

/**
 * Function to finalize the CPC module.  It is called once when the
 * CPC module's corresponding iwarp BTL module is being finalized.
 */
typedef int (*opal_btl_iwarp_connect_base_module_finalize_fn_t)
    (struct mca_btl_iwarp_module_t *btl,
     struct opal_btl_iwarp_connect_base_module_t *cpc);
 ...
 * iwarp_btl_proc_t struct to hold information received from the
 * modex.
 */
typedef struct opal_btl_iwarp_connect_base_module_data_t {
    /** Pointer back to the component.  Used by the base and iwarp
        btl to calculate this module's index for the modex. */
    opal_btl_iwarp_connect_base_component_t *cbm_component;

    /** Priority of the CPC module (must be >=0 and <=100) */
    uint8_t cbm_priority;

    /** Blob that the CPC wants to include in the iwarp modex message
...
} opal_btl_iwarp_connect_base_module_data_t;

/**
 * Struct for holding CPC module and associated meta data
 */
typedef struct opal_btl_iwarp_connect_base_module_t {
    /** Meta data about the module */
    opal_btl_iwarp_connect_base_module_data_t data;

    /** Endpoint initialization function */
    opal_btl_iwarp_connect_base_module_endpoint_init_fn_t cbm_endpoint_init;

    /** Connect function */
    opal_btl_iwarp_connect_base_module_start_connect_fn_t cbm_start_connect;

    /** Endpoint finalization function */
    opal_btl_iwarp_connect_base_module_endpoint_finalize_fn_t cbm_endpoint_finalize;

    /** Finalize the cpc module */
    opal_btl_iwarp_connect_base_module_finalize_fn_t cbm_finalize;

    /** Whether this module will use the CTS protocol or not.  This
        directly states whether this module will call
        mca_btl_iwarp_endpoint_post_recvs() or not: true = this
        module will *not* call _post_recvs() and instead will post the
        receive buffer provided at endpoint->endpoint_cts_frag on qp
        0. */
    bool cbm_uses_cts;
} opal_btl_iwarp_connect_base_module_t;
