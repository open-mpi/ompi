...
static void mca_btl_iwarp_endpoint_construct(mca_btl_base_endpoint_t* endpoint);
static void mca_btl_iwarp_endpoint_destruct(mca_btl_base_endpoint_t* endpoint);

static inline int acquire_wqe(mca_btl_iwarp_endpoint_t *ep,
...
int mca_btl_iwarp_endpoint_post_send(mca_btl_iwarp_endpoint_t *endpoint,
        mca_btl_iwarp_send_frag_t *frag)
{
    int prio = to_base_frag(frag)->base.des_flags & MCA_BTL_DES_FLAGS_PRIORITY;
    ...
    rc = mca_btl_iwarp_endpoint_credit_acquire (endpoint, qp, prio, size,
                                                 &do_rdma, frag, true);
    ...
    mca_btl_iwarp_endpoint_credit_release (endpoint, qp, do_rdma, frag);
...
OBJ_CLASS_INSTANCE(mca_btl_iwarp_endpoint_t,
                   opal_list_item_t, mca_btl_iwarp_endpoint_construct,
                   mca_btl_iwarp_endpoint_destruct);

/*
 * Initialize state of the endpoint instance.
 *
 */
static mca_btl_iwarp_qp_t *endpoint_alloc_qp(void)
{
    mca_btl_iwarp_qp_t *qp = (mca_btl_iwarp_qp_t *) calloc(1, sizeof(mca_btl_iwarp_qp_t));
    if(!qp) {
...
static void
endpoint_init_qp_pp(mca_btl_iwarp_endpoint_qp_t *ep_qp, const int qp)
{
    mca_btl_iwarp_qp_info_t *qp_info = &mca_btl_iwarp_component.qp_infos[qp];
...
static void
endpoint_init_qp_srq(mca_btl_iwarp_endpoint_qp_t *ep_qp, const int qp)
{
    ep_qp->qp = endpoint_alloc_qp();
    ep_qp->qp->users++;

    /* number of available send WQEs */
    ep_qp->qp->sd_wqe = mca_btl_iwarp_component.qp_infos[qp].u.srq_qp.sd_max;
}

static void endpoint_init_qp(mca_btl_base_endpoint_t *ep, const int qp)
{
    mca_btl_iwarp_endpoint_qp_t *ep_qp = &ep->qps[qp];
    ...
    switch(BTL_IWARP_QP_TYPE(qp)) {
        case MCA_BTL_IWARP_PP_QP:
            endpoint_init_qp_pp(ep_qp, qp);
            break;
        case MCA_BTL_IWARP_SRQ_QP:
...
void mca_btl_iwarp_endpoint_init(mca_btl_iwarp_module_t *btl,
                                  mca_btl_base_endpoint_t *ep,
                                  opal_btl_iwarp_connect_base_module_t *local_cpc,
                                  mca_btl_iwarp_proc_modex_t *remote_proc_info,
                                  opal_btl_iwarp_connect_base_module_data_t *remote_cpc_data)
{
    ...
    ep->use_eager_rdma = btl->device->use_eager_rdma &
        mca_btl_iwarp_component.use_eager_rdma;
    ...
    ep->rem_info.rem_transport_type =
         (mca_btl_iwarp_transport_type_t) (remote_proc_info->pm_port_info).transport_type;
...
{
    /* setup qp structures */
    endpoint->qps = (mca_btl_iwarp_endpoint_qp_t*)
        calloc(mca_btl_iwarp_component.num_qps,
                sizeof(mca_btl_iwarp_endpoint_qp_t));
        endpoint->rem_info.rem_qps = (mca_btl_iwarp_rem_qp_info_t*)
            calloc(mca_btl_iwarp_component.num_qps,
                    sizeof(mca_btl_iwarp_rem_qp_info_t));
    ...
    endpoint->get_tokens = mca_btl_iwarp_component.ib_qp_ous_rd_atom;

    /* initialize RDMA eager related parts */
    endpoint->eager_recv_count = 0;
    memset(&endpoint->eager_rdma_remote, 0,
           sizeof(mca_btl_iwarp_eager_rdma_remote_t));
    memset(&endpoint->eager_rdma_local, 0,
           sizeof(mca_btl_iwarp_eager_rdma_local_t));
...
static void mca_btl_iwarp_endpoint_destruct(mca_btl_base_endpoint_t* endpoint)
{
    ...
    /* Release CTS buffer */
    opal_btl_iwarp_connect_base_free_cts(endpoint);

    /* Release memory resources */
    do {
        void *_tmp_ptr = NULL;
        /* Make sure that mca_btl_iwarp_endpoint_connect_eager_rdma ()
   ...
   for(qp = 0; qp < mca_btl_iwarp_component.num_qps; qp++) {
        MCA_BTL_IWARP_CLEAN_PENDING_FRAGS(&endpoint->qps[qp].no_credits_pending_frags[0]);
        MCA_BTL_IWARP_CLEAN_PENDING_FRAGS(&endpoint->qps[qp].no_credits_pending_frags[1]);
    ...
    /* Clean pending lists */
    MCA_BTL_IWARP_CLEAN_PENDING_FRAGS(&endpoint->pending_lazy_frags);
    OBJ_DESTRUCT(&endpoint->pending_lazy_frags);

    MCA_BTL_IWARP_CLEAN_PENDING_FRAGS(&endpoint->pending_get_frags);
    OBJ_DESTRUCT(&endpoint->pending_get_frags);

    MCA_BTL_IWARP_CLEAN_PENDING_FRAGS(&endpoint->pending_put_frags);
 ...
 */
int mca_btl_iwarp_endpoint_post_recvs(mca_btl_iwarp_endpoint_t *endpoint)
{
    int qp;

    for (qp = 0; qp < mca_btl_iwarp_component.num_qps; ++qp) {
        if (BTL_IWARP_QP_TYPE_PP(qp)) {
            mca_btl_iwarp_endpoint_post_rr_nolock(endpoint, qp);
        } else {
            mca_btl_iwarp_post_srr(endpoint->endpoint_btl, qp);
        }
    }
...
{
    mca_btl_iwarp_send_control_frag_t *sc_frag;
    mca_btl_base_descriptor_t *base_des;
    mca_btl_iwarp_frag_t *iwarp_frag;
    mca_btl_iwarp_com_frag_t *com_frag;
    ...
    iwarp_frag = &(com_frag->super);
    base_des = &(iwarp_frag->base);
    ...
    base_des->order = mca_btl_iwarp_component.credits_qp;
    iwarp_frag->segment.seg_len = sizeof(mca_btl_iwarp_control_header_t);
    ...
    ctl_hdr = (mca_btl_iwarp_control_header_t*)
        iwarp_frag->segment.seg_addr.pval;
    ctl_hdr->type = MCA_BTL_IWARP_CONTROL_CTS;

    /* Send the fragment */
    if (OPAL_SUCCESS != mca_btl_iwarp_endpoint_post_send(endpoint, sc_frag)) {
 ...
 */
void mca_btl_iwarp_endpoint_cpc_complete(mca_btl_iwarp_endpoint_t *endpoint)
{
        ...
        if ( endpoint->endpoint_initiator ||
            ...
            if (endpoint->endpoint_cts_received) {
                OPAL_OUTPUT((-1, "cpc_complete to %s -- already got CTS, so marking endpoint as complete",
                             opal_get_proc_hostname(endpoint->endpoint_proc->proc_opal)));
                mca_btl_iwarp_endpoint_connected(endpoint);
            } else {
 ...
 */
void mca_btl_iwarp_endpoint_connected(mca_btl_iwarp_endpoint_t *endpoint)
{
    ...
    while (NULL != (frag_item = opal_list_remove_first(&(endpoint->pending_lazy_frags)))) {
        ...
        if (OPAL_ERROR == mca_btl_iwarp_endpoint_post_send(endpoint, frag)) {
    ...
    mca_btl_iwarp_frag_progress_pending_put_get(endpoint,
            mca_btl_iwarp_component.rdma_qp);
}
 ...
 */
int mca_btl_iwarp_endpoint_send(mca_btl_base_endpoint_t* ep,
                                 mca_btl_iwarp_send_frag_t* frag)
{
    int rc;
    ...
    if(OPAL_LIKELY(OPAL_SUCCESS == rc)) {
        rc = mca_btl_iwarp_endpoint_post_send(ep, frag);
    }
...
static void mca_btl_iwarp_endpoint_credits(
    ...
    int status)
{
    ...
    mca_btl_iwarp_send_control_frag_t *frag = to_send_control_frag(des);
    ...
    if(check_send_credits(ep, qp) || check_eager_rdma_credits(ep))
        ...
        BTL_IWARP_CREDITS_SEND_UNLOCK(ep, qp);
...
void mca_btl_iwarp_endpoint_send_credits(mca_btl_iwarp_endpoint_t* endpoint,
        const int qp)
{
    mca_btl_iwarp_module_t* iwarp_btl = endpoint->endpoint_btl;
    mca_btl_iwarp_send_control_frag_t* frag;
    mca_btl_iwarp_rdma_credits_header_t *credits_hdr;
    int rc;
    bool do_rdma = false;
    int32_t cm_return;
    ...
    if(OPAL_UNLIKELY(NULL == frag)) {
        frag = alloc_control_frag(iwarp_btl);
        frag->qp_idx = qp;
        endpoint->qps[qp].credit_frag = frag;
        /* set those once and forever */
        to_base_frag(frag)->base.order = mca_btl_iwarp_component.credits_qp;
        to_base_frag(frag)->base.des_cbfunc = mca_btl_iwarp_endpoint_credits;
        ...
        to_base_frag(frag)->segment.seg_len =
            sizeof(mca_btl_iwarp_rdma_credits_header_t);
    }

    assert(frag->qp_idx == qp);
    credits_hdr = (mca_btl_iwarp_rdma_credits_header_t*)
    ...
    if(OPAL_SUCCESS == acquire_eager_rdma_send_credit(endpoint)) {
        ...
        if(OPAL_THREAD_ADD_FETCH32(&endpoint->qps[qp].u.pp_qp.cm_sent, 1) >
                (mca_btl_iwarp_component.qp_infos[qp].u.pp_qp.rd_rsv - 1)) {
            OPAL_THREAD_ADD_FETCH32(&endpoint->qps[qp].u.pp_qp.cm_sent, -1);
            BTL_IWARP_CREDITS_SEND_UNLOCK(endpoint, qp);
            return;
        }
     }

    BTL_IWARP_GET_CREDITS(endpoint->qps[qp].u.pp_qp.rd_credits, frag->hdr->credits);
    ...
    BTL_IWARP_GET_CREDITS(endpoint->eager_rdma_local.credits, credits_hdr->rdma_credits);
    credits_hdr->qpn = qp;
    credits_hdr->control.type = MCA_BTL_IWARP_CONTROL_CREDITS;

    if(endpoint->nbo)
         BTL_IWARP_RDMA_CREDITS_HEADER_HTON(*credits_hdr);
    ...
    if(endpoint->nbo) {
        BTL_IWARP_HEADER_NTOH(*frag->hdr);
        BTL_IWARP_RDMA_CREDITS_HEADER_NTOH(*credits_hdr);
    }
    BTL_IWARP_CREDITS_SEND_UNLOCK(endpoint, qp);
...
static void mca_btl_iwarp_endpoint_eager_rdma_connect_cb(
    mca_btl_base_module_t* btl,
    struct mca_btl_base_endpoint_t* endpoint,
    struct mca_btl_base_descriptor_t* descriptor,
    int status)
{
    mca_btl_iwarp_device_t *device = endpoint->endpoint_btl->device;
...
static int mca_btl_iwarp_endpoint_send_eager_rdma(
    mca_btl_base_endpoint_t* endpoint)
{
    ...
    to_base_frag(frag)->base.des_cbfunc =
        mca_btl_iwarp_endpoint_eager_rdma_connect_cb;
    to_base_frag(frag)->base.des_cbdata = NULL;
    ...
    frag->hdr->tag = MCA_BTL_TAG_IB;
    rdma_hdr = (mca_btl_iwarp_eager_rdma_header_t*)to_base_frag(frag)->segment.seg_addr.pval;
    rdma_hdr->control.type = MCA_BTL_IWARP_CONTROL_RDMA;
    ...
    BTL_VERBOSE(("sending rkey %" PRIu32 ", rdma_start.lval %" PRIx64
                 ...
                 (int) sizeof(mca_btl_iwarp_eager_rdma_header_t)
                 ));

    if(endpoint->nbo) {
        BTL_IWARP_EAGER_RDMA_CONTROL_HEADER_HTON((*rdma_hdr));
    ...
    rc = mca_btl_iwarp_endpoint_send(endpoint, frag);
...
void mca_btl_iwarp_endpoint_connect_eager_rdma(
        mca_btl_iwarp_endpoint_t* endpoint)
{
    mca_btl_iwarp_module_t* iwarp_btl = endpoint->endpoint_btl;
    char *buf, *alloc_base;
    mca_btl_iwarp_recv_frag_t *headers_buf;
    int i, rc;
    ...
    headers_buf = (mca_btl_iwarp_recv_frag_t*)
        malloc(sizeof(mca_btl_iwarp_recv_frag_t) *
    ...
    alloc_base = buf = (char *) iwarp_btl->super.btl_mpool->mpool_alloc(iwarp_btl->super.btl_mpool,
                                                            iwarp_btl->eager_rdma_frag_size *
                                                            mca_btl_iwarp_component.eager_rdma_num,
    ...
    rc = iwarp_btl->device->rcache->rcache_register (iwarp_btl->device->rcache, buf, iwarp_btl->eager_rdma_frag_size *
                                                      mca_btl_iwarp_component.eager_rdma_num, flag, MCA_RCACHE_ACCESS_ANY,
                                                      (mca_rcache_base_registration_t**)&endpoint->eager_rdma_local.reg);
    if (OPAL_SUCCESS != rc) {
        iwarp_btl->super.btl_mpool->mpool_free (iwarp_btl->super.btl_mpool, alloc_base);
        goto free_headers_buf;
    }

    buf = buf + iwarp_btl->eager_rdma_frag_size -
        sizeof(mca_btl_iwarp_footer_t) - iwarp_btl->super.btl_eager_limit -
        sizeof(mca_btl_iwarp_header_t);

    for(i = 0; i < mca_btl_iwarp_component.eager_rdma_num; i++) {
        ...
        item->ptr = buf + i * iwarp_btl->eager_rdma_frag_size;
        OBJ_CONSTRUCT(item, mca_btl_iwarp_recv_frag_t);

        init_data.order = mca_btl_iwarp_component.credits_qp;
        init_data.list = NULL;

        mca_btl_iwarp_frag_init(item, &init_data);
        frag = to_recv_frag(item);
        to_base_frag(frag)->type = MCA_BTL_IWARP_FRAG_EAGER_RDMA;
        to_com_frag(frag)->endpoint = endpoint;
        frag->ftr = (mca_btl_iwarp_footer_t*)
    ...
    endpoint->eager_rdma_local.rd_win =
        mca_btl_iwarp_component.eager_rdma_num >> 2;
    endpoint->eager_rdma_local.rd_win =
    ...
    if(mca_btl_iwarp_endpoint_send_eager_rdma(endpoint) == OPAL_SUCCESS) {
        mca_btl_iwarp_device_t *device = endpoint->endpoint_btl->device;
        mca_btl_iwarp_endpoint_t **p;
        void *_tmp_ptr;
        OBJ_RETAIN(endpoint);
        ...
        OPAL_THREAD_ADD_FETCH32(&iwarp_btl->eager_rdma_channels, 1);
        /* from this point progress function starts to poll new buffer */
        OPAL_THREAD_ADD_FETCH32(&device->eager_rdma_buffers_count, 1);
        return;
    }

    iwarp_btl->device->rcache->rcache_deregister (iwarp_btl->device->rcache,
                                                   (mca_rcache_base_registration_t*)endpoint->eager_rdma_local.reg);
    iwarp_btl->super.btl_mpool->mpool_free(iwarp_btl->super.btl_mpool, buf);
free_headers_buf:
    free(headers_buf);
unlock_rdma_local:
 ...
 */
void *mca_btl_iwarp_endpoint_invoke_error(void *context)
{
    mca_btl_iwarp_endpoint_t *endpoint = (mca_btl_iwarp_endpoint_t*) context;
    mca_btl_iwarp_module_t *btl = NULL;

    if (NULL == endpoint) {
        int i;
        for (i = 0; i < mca_btl_iwarp_component.ib_num_btls; ++i) {
            if (NULL != mca_btl_iwarp_component.iwarp_btls[i] &&
                NULL != mca_btl_iwarp_component.iwarp_btls[i]->error_cb) {
                btl = mca_btl_iwarp_component.iwarp_btls[i];
                break;
            }
        }
    } else {
