...
static int btl_iwarp_cq_size;
static bool btl_iwarp_have_fork_support = OPAL_HAVE_IBV_FORK_INIT;
                      ...
                      int flags)
{
    ...
    *storage = (char *) default_value;
    index = mca_base_component_var_register(&mca_btl_iwarp_component.super.btl_version,
                                            param_name, param_desc, MCA_BASE_VAR_TYPE_STRING,
                                            NULL, 0, 0, OPAL_INFO_LVL_9,
                                            MCA_BASE_VAR_SCOPE_READONLY, storage);
    if (NULL != deprecated_param_name) {
        (void) mca_base_var_register_synonym(index, "ompi", "btl", "iwarp",
...
#define CHECK(expr) do {\
    ...
    /* register iwarp component parameters */
    CHECK(reg_bool("verbose", NULL,
                  "Output some verbose OpenIB BTL information "
                  "(0 = no output, nonzero = output)", false,
                   &mca_btl_iwarp_component.verbose));

    CHECK(reg_bool("warn_no_device_params_found",
                  "warn_no_hca_params_found",
                  "Warn when no device-specific parameters are found in the INI file specified by the btl_iwarp_device_param_files MCA parameter "
                  "(0 = do not warn; any other value = warn)",
                  true, &mca_btl_iwarp_component.warn_no_device_params_found));
    ...
    CHECK(reg_bool("warn_default_gid_prefix", NULL,
                  ...
                  "(0 = do not warn; any other value = warn)",
                  true, &mca_btl_iwarp_component.warn_default_gid_prefix));

    CHECK(reg_bool("warn_nonexistent_if", NULL,
                  "Warn if non-existent devices and/or ports are specified in the btl_iwarp_if_[in|ex]clude MCA parameters "
                  "(0 = do not warn; any other value = warn)",
                  true, &mca_btl_iwarp_component.warn_nonexistent_if));
    ...
    CHECK(reg_bool("abort_not_enough_reg_mem", NULL,
                  ...
                  false, &mca_btl_iwarp_component.abort_not_enough_reg_mem));

    CHECK(reg_uint("poll_cq_batch", NULL,
                   "Retrieve up to poll_cq_batch completions from CQ",
                   MCA_BTL_IWARP_CQ_POLL_BATCH_DEFAULT, &mca_btl_iwarp_component.cq_poll_batch,
                   REGINT_GE_ONE));

    asprintf(&str, "%s/mca-btl-iwarp-device-params.ini",
    ...
    CHECK(reg_string("rcache", NULL,
                     ...
                     "grdma", &mca_btl_iwarp_component.ib_rcache_name,
                     0));
    CHECK(reg_int("reg_mru_len", NULL,
                  "Length of the registration cache most recently used list "
                  "(must be >= 1)",
                  16, (int*) &mca_btl_iwarp_component.reg_mru_len,
                  REGINT_GE_ONE));
    ...
    CHECK(reg_int("cq_size", "ib_cq_size",
                  ...
                  8192, &btl_iwarp_cq_size, REGINT_GE_ONE));
    mca_btl_iwarp_component.ib_cq_size[BTL_IWARP_LP_CQ] =
        mca_btl_iwarp_component.ib_cq_size[BTL_IWARP_HP_CQ] = (uint32_t) btl_iwarp_cq_size;
    ...
    CHECK(reg_int("max_inline_data", "ib_max_inline_data",
                  ...
                  -1, &mca_btl_iwarp_component.ib_max_inline_data,
    ...
    mca_btl_iwarp_component.ib_mtu = 0;
    (void) mca_base_var_enum_create("btl_iwarp_mtus", ib_mtu_values, &new_enum);
    tmp = mca_base_component_var_register(&mca_btl_iwarp_component.super.btl_version,
                                          "mtu", msg, MCA_BASE_VAR_TYPE_INT, new_enum,
                                          0, 0, OPAL_INFO_LVL_9,
                                          MCA_BASE_VAR_SCOPE_READONLY,
                                          &mca_btl_iwarp_component.ib_mtu);
    if (0 <= tmp) {
        (void) mca_base_var_register_synonym(tmp, "ompi", "btl", "iwarp", "ib_mtu",
                                             MCA_BASE_VAR_SYN_FLAG_DEPRECATED);
    } else {
        ret = tmp;
    }
    ...
    CHECK(reg_uint("ib_min_rnr_timer", NULL, "InfiniBand minimum "
                   ...
                   "(must be >= 0 and <= 31)",
                   25, &mca_btl_iwarp_component.ib_min_rnr_timer, 0));

    CHECK(reg_uint("ib_timeout", NULL,
                  "InfiniBand transmit timeout, plugged into formula: 4.096 microseconds * (2^btl_iwarp_ib_timeout) "
                  "(must be >= 0 and <= 31)",
                  20, &mca_btl_iwarp_component.ib_timeout, 0));

    CHECK(reg_uint("ib_retry_count", NULL,
                  "InfiniBand transmit retry count "
                  "(must be >= 0 and <= 7)",
                  7, &mca_btl_iwarp_component.ib_retry_count, 0));
    ...
    CHECK(reg_uint("ib_rnr_retry", NULL,
                   ...
                   7, &mca_btl_iwarp_component.ib_rnr_retry, 0));
    ...
    CHECK(reg_int("use_eager_rdma", NULL, "Use RDMA for eager messages "
                  ...
                  "1 = use eager RDMA)",
                  -1, &mca_btl_iwarp_component.use_eager_rdma, 0));
    ...
    CHECK(reg_int("eager_rdma_threshold", NULL,
                  ...
                  "(must be >= 1)",
                  16, &mca_btl_iwarp_component.eager_rdma_threshold, REGINT_GE_ONE));
    ...
    CHECK(reg_int("max_eager_rdma", NULL, "Maximum number of peers allowed to use "
                  ...
                  "with the \"dr\" pml) "
                  "(must be >= 0)",
                  16, &mca_btl_iwarp_component.max_eager_rdma, REGINT_GE_ZERO));
    ...
    CHECK(reg_uint("hp_cq_poll_per_progress", NULL,
                  ...
                  10, &mca_btl_iwarp_component.cq_poll_progress, REGINT_GE_ONE));
    ...
    CHECK(reg_int("memory_registration_verbose", NULL,
		  ...
		  &mca_btl_iwarp_component.memory_registration_verbose_level, 0));
    ...
    CHECK(reg_int("ignore_locality", NULL,
                  ...
                  &mca_btl_iwarp_component.ignore_locality, REGINT_GE_ZERO));
    ...
    tmp = mca_base_component_var_register(&mca_btl_iwarp_component.super.btl_version,
                                          ...
                                          &btl_iwarp_have_fork_support);

    mca_btl_iwarp_module.super.btl_exclusivity = MCA_BTL_EXCLUSIVITY_DEFAULT;

    mca_btl_iwarp_module.super.btl_eager_limit = 12 * 1024;
    mca_btl_iwarp_module.super.btl_rndv_eager_limit = 12 * 1024;
    mca_btl_iwarp_module.super.btl_max_send_size = 64 * 1024;
    mca_btl_iwarp_module.super.btl_rdma_pipeline_send_length = 1024 * 1024;
    mca_btl_iwarp_module.super.btl_rdma_pipeline_frag_size = 1024 * 1024;
    mca_btl_iwarp_module.super.btl_min_rdma_pipeline_size = 256 * 1024;
    mca_btl_iwarp_module.super.btl_flags = MCA_BTL_FLAGS_RDMA |
	MCA_BTL_FLAGS_NEED_ACK | MCA_BTL_FLAGS_NEED_CSUM | MCA_BTL_FLAGS_HETEROGENEOUS_RDMA |
        MCA_BTL_FLAGS_SEND;
#if HAVE_DECL_IBV_ATOMIC_HCA
    mca_btl_iwarp_module.super.btl_flags |= MCA_BTL_FLAGS_ATOMIC_FOPS;
    mca_btl_iwarp_module.super.btl_atomic_flags = MCA_BTL_ATOMIC_SUPPORTS_ADD | MCA_BTL_ATOMIC_SUPPORTS_CSWAP;
#endif

    /* Default to bandwidth auto-detection */
    mca_btl_iwarp_module.super.btl_bandwidth = 0;
    mca_btl_iwarp_module.super.btl_latency = 4;
    CHECK(mca_btl_base_param_register(
            &mca_btl_iwarp_component.super.btl_version,
            &mca_btl_iwarp_module.super));

    /* setup all the qp stuff */
    /* round mid_qp_size to smallest power of two */
    mid_qp_size = opal_next_poweroftwo (mca_btl_iwarp_module.super.btl_eager_limit / 4) >> 1;
    ...
    asprintf(&default_qps,
            ...
            mid_qp_size,
            (uint32_t)mca_btl_iwarp_module.super.btl_eager_limit,
            (uint32_t)mca_btl_iwarp_module.super.btl_max_send_size);
    if (NULL == default_qps) {
        /* Don't try to recover from this */
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    mca_btl_iwarp_component.default_recv_qps = default_qps;
    CHECK(reg_string("receive_queues", NULL,
                     "Colon-delimited, comma-delimited list of receive queues: P,4096,8,6,4:P,32768,8,6,4",
                     default_qps, &mca_btl_iwarp_component.receive_queues,
                     0
                ));

    CHECK(reg_string("if_include", NULL,
                     "Comma-delimited list of devices/ports to be used (e.g. \"mthca0,mthca1:2\"; empty value means to use all ports found).  Mutually exclusive with btl_iwarp_if_exclude.",
                     NULL, &mca_btl_iwarp_component.if_include,
                     0));

    CHECK(reg_string("if_exclude", NULL,
                     "Comma-delimited list of device/ports to be excluded (empty value means to not exclude any ports).  Mutually exclusive with btl_iwarp_if_include.",
                     NULL, &mca_btl_iwarp_component.if_exclude,
                     0));

    CHECK(reg_string("ipaddr_include", NULL,
                     "Comma-delimited list of IP Addresses to be used (e.g. \"192.168.1.0/24\").  Mutually exclusive with btl_iwarp_ipaddr_exclude.",
                     NULL, &mca_btl_iwarp_component.ipaddr_include,
                     0));

    CHECK(reg_string("ipaddr_exclude", NULL,
                     "Comma-delimited list of IP Addresses to be excluded (e.g. \"192.168.1.0/24\").  Mutually exclusive with btl_iwarp_ipaddr_include.",
                     NULL, &mca_btl_iwarp_component.ipaddr_exclude,
                     0));

    CHECK(reg_int("gid_index", NULL,
                  "GID index to use on verbs device ports",
                  0, &mca_btl_iwarp_component.gid_index,
                  REGINT_GE_ZERO));

    CHECK(reg_bool("allow_different_subnets", NULL,
                   "Allow connecting processes from different IB subnets."
                   "(0 = do not allow; 1 = allow)",
                   false, &mca_btl_iwarp_component.allow_different_subnets));

    /* Register any MCA params for the connect pseudo-components */
    if (OPAL_SUCCESS == ret) {
        ret = opal_btl_iwarp_connect_base_register();
    }

    return btl_iwarp_verify_mca_params();
}

int btl_iwarp_verify_mca_params (void)
{
    if (mca_btl_iwarp_component.cq_poll_batch > MCA_BTL_IWARP_CQ_POLL_BATCH_DEFAULT) {
        mca_btl_iwarp_component.cq_poll_batch = MCA_BTL_IWARP_CQ_POLL_BATCH_DEFAULT;
    }

#if !HAVE_IBV_FORK_INIT
    if (1 == mca_btl_iwarp_component.want_fork_support) {
    ...
    mca_btl_iwarp_component.ib_pkey_val &= MCA_BTL_IB_PKEY_MASK;

    if (mca_btl_iwarp_component.ib_min_rnr_timer > 31) {
        opal_show_help("help-mpi-btl-iwarp.txt", "invalid mca param value",
                       true, "btl_iwarp_ib_min_rnr_timer > 31",
                       "btl_iwarp_ib_min_rnr_timer reset to 31");
        mca_btl_iwarp_component.ib_min_rnr_timer = 31;
    }

    if (mca_btl_iwarp_component.ib_timeout > 31) {
        opal_show_help("help-mpi-btl-iwarp.txt", "invalid mca param value",
                       true, "btl_iwarp_ib_timeout > 31",
                       "btl_iwarp_ib_timeout reset to 31");
        mca_btl_iwarp_component.ib_timeout = 31;
    }

    if (mca_btl_iwarp_component.ib_retry_count > 7) {
        opal_show_help("help-mpi-btl-iwarp.txt", "invalid mca param value",
                       true, "btl_iwarp_ib_retry_count > 7",
                       "btl_iwarp_ib_retry_count reset to 7");
        mca_btl_iwarp_component.ib_retry_count = 7;
    }

    if (mca_btl_iwarp_component.ib_rnr_retry > 7) {
        opal_show_help("help-mpi-btl-iwarp.txt", "invalid mca param value",
    ...
    if(mca_btl_iwarp_component.buffer_alignment <= 1 ||
       (mca_btl_iwarp_component.buffer_alignment & (mca_btl_iwarp_component.buffer_alignment - 1))) {
        opal_show_help("help-mpi-btl-iwarp.txt", "wrong buffer alignment",
                true, mca_btl_iwarp_component.buffer_alignment, opal_process_info.nodename, 64);
        mca_btl_iwarp_component.buffer_alignment = 64;
    }

#if OPAL_CUDA_SUPPORT /* CUDA_ASYNC_RECV */
    if (mca_btl_iwarp_component.cuda_async_send) {
        mca_btl_iwarp_module.super.btl_flags |= MCA_BTL_FLAGS_CUDA_COPY_ASYNC_SEND;
    } else {
        mca_btl_iwarp_module.super.btl_flags &= ~MCA_BTL_FLAGS_CUDA_COPY_ASYNC_SEND;
    }

    if (mca_btl_iwarp_component.cuda_async_recv) {
        mca_btl_iwarp_module.super.btl_flags |= MCA_BTL_FLAGS_CUDA_COPY_ASYNC_RECV;
    } else {
        mca_btl_iwarp_module.super.btl_flags &= ~MCA_BTL_FLAGS_CUDA_COPY_ASYNC_RECV;
    }
    ...
    if (mca_btl_iwarp_component.cuda_want_gdr && mca_btl_iwarp_component.cuda_have_gdr &&
        mca_btl_iwarp_component.driver_have_gdr) {
        if (1 == opal_common_verbs_want_fork_support) {
              opal_show_help("help-mpi-btl-iwarp.txt", "no_fork_with_gdr",
                             true, opal_process_info.nodename);
              return OPAL_ERR_BAD_PARAM;
        }
    }
#endif /* Workaround */
    if (0 != mca_btl_iwarp_module.super.btl_cuda_max_send_size) {
        opal_show_help("help-mpi-btl-iwarp.txt", "do_not_set_iwarp_value",
                       true, opal_process_info.nodename);
        mca_btl_iwarp_module.super.btl_cuda_max_send_size = 0;
    }
#endif
