...
static inline int mca_btl_base_rdma_resp_tag (void) { return MCA_BTL_BASE_TAG_RDMA_RESP; }
                                             ...
                                             size_t segment_count)
{
    ...
    for (size_t i = 1; i < segment_count; ++i) {
        ...
        BTL_VERBOSE (("unpacking %" PRIsize_t " bytes from segment %" PRIsize_t " to %p", seg_len,
                      i, (void *)addr));
...
static void mca_btl_base_am_queue_initiator_descriptor (mca_btl_base_module_t *btl,
                                                        ...
                                                        mca_btl_base_descriptor_t *descriptor)
{
    mca_btl_base_am_rdma_queued_descriptor_t *queued_descriptor =
        OBJ_NEW (mca_btl_base_am_rdma_queued_descriptor_t);
    ...
    OPAL_THREAD_SCOPED_LOCK (
        &default_module.mutex,
        opal_list_append (&default_module.queued_initiator_descriptors, &queued_descriptor->super));
}
                                                ...
                                                bool send_descriptor)
{
    ...
    mca_btl_base_rdma_hdr_t *hdr =
        (mca_btl_base_rdma_hdr_t *)descriptor->des_segments[0].seg_addr.pval;
...
mca_btl_base_rdma_start (mca_btl_base_module_t *btl, struct mca_btl_base_endpoint_t *endpoint,
                         int type, uint64_t operand1, uint64_t operand2,
                         mca_btl_base_atomic_op_t op, int order, int flags, size_t size,
                         void *local_address, mca_btl_base_registration_handle_t *local_handle,
                         int64_t remote_address, mca_btl_base_registration_handle_t *remote_handle,
                                         ...
                                         const mca_btl_base_rdma_hdr_t *hdr)
{
    ...
    *descriptor                                = NULL;
    ...
    if (NULL == send_descriptor) {
        ...
        size_t data_size =
            mca_btl_base_rdma_is_atomic (hdr->type) ? hdr->data.atomic.size : hdr->data.rdma.size;
        ...
        resp_hdr =
            (mca_btl_base_rdma_response_hdr_t *)send_descriptor->des_segments[0].seg_addr.pval;
                                            ...
                                            mca_btl_base_rdma_operation_t **operation)
{
    if (hdr->data.rdma.use_rdma) {
        ...
        mca_btl_base_copy_from_segments (hdr->target_address, sizeof (*hdr), segments,
                                         segment_count);
    }
    ...
    if (!operation->descriptor && !operation->is_completed) {
        ...
        case MCA_BTL_BASE_AM_PUT:
            ret = mca_btl_base_am_rdma_target_put (
                operation->btl, operation->endpoint, &operation->descriptor,
                /*segments=*/NULL,
                /*segment_count=*/0, target_address, &operation->hdr, &operation);
            break;
        case MCA_BTL_BASE_AM_ATOMIC:
...
static int mca_btl_base_am_rdma_progress (void)
{
    ...
    OPAL_THREAD_SCOPED_LOCK (&default_module.mutex, ({
        ...
        {
            mca_btl_base_rdma_context_t *context =
                (mca_btl_base_rdma_context_t *)descriptor->descriptor->des_context;
...
{
    mca_btl_base_rdma_response_hdr_t *resp_hdr =
        (mca_btl_base_rdma_response_hdr_t *)desc->des_segments[0].seg_addr.pval;

    assert (desc->des_segments[0].seg_len >= sizeof (*resp_hdr));

    mca_btl_base_rdma_context_t *context =
        (mca_btl_base_rdma_context_t *)(uintptr_t)resp_hdr->context;
                                          ...
                                          const mca_btl_base_receive_descriptor_t *desc)
{
    ...
    const mca_btl_base_rdma_hdr_t *hdr =
        (mca_btl_base_rdma_hdr_t *)desc->des_segments[0].seg_addr.pval;
    ...
    if (MCA_BTL_BASE_AM_PUT == hdr->type) {
        ...
        ret = mca_btl_base_am_rdma_target_get (btl, desc->endpoint, &descriptor, target_address,
                                               hdr, &operation);
    } else {
                                            ...
                                            const mca_btl_base_receive_descriptor_t *desc)
{
    ...
    BTL_VERBOSE (("got active-message atomic request. hdr->context=0x%" PRIx64
                  ", target_address=%p, "
                  "segment 0 size=%" PRIu64,
                  hdr->context, (void *)(intptr_t)hdr->target_address,
                  adesc->des_segments[0].seg_len));
    ...
    mca_btl_base_descriptor_t *descriptor = NULL;
    int ret =
        mca_btl_base_am_rdma_respond (btl, desc->endpoint, &descriptor, &atomic_response, hdr);
    if (OPAL_SUCCESS != ret) {
