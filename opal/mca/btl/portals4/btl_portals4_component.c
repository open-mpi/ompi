                                                       ...
                                                       bool enable_mpi_threads)
{
    ...
    *num_btls = 0;
    ...
    for (interface=0; interface<mca_btl_portals4_component.num_btls; interface++) {
        ...
        OPAL_OUTPUT_VERBOSE((90, opal_btl_base_framework.framework_output, "PtlMEAppend (overflow list) OK for NI %d\n", interface));
    }
    free(portals4_nis_h);
    portals4_nis_h = NULL;
...
mca_btl_portals4_component_progress(void)
{
    ...
    while (true) {
        ...
        if (PTL_OK == ret) {
            OPAL_OUTPUT_VERBOSE((90, opal_btl_base_framework.framework_output, "PtlEQPoll Event received: %d (%d) on NI %d\n",
