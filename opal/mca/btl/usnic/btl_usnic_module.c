                                    ...
                                    opal_btl_usnic_send_frag_t *frag)
{
    ...
    if (lfrag->lsf_bytes_left == 0) {
        ...
        if (frag->sf_base.uf_remote_seg[0].seg_addr.pval == NULL
            && (frag->sf_base.uf_base.des_flags & MCA_BTL_DES_FLAGS_BTL_OWNERSHIP)) {
...
void opal_btl_usnic_module_progress_sends(opal_btl_usnic_module_t *module)
{
    ...
    while ((get_send_credits(data_channel) > 1)
        ...
        endpoint = (opal_btl_usnic_endpoint_t *) opal_list_get_first(&module->endpoints_with_sends);
        frag = (opal_btl_usnic_send_frag_t *) opal_list_get_first(
        ...
        if (frag->sf_base.uf_type == OPAL_BTL_USNIC_FRAG_SMALL_SEND) {
...
#if MSGDEBUG1
            opal_output(0,
                        "progress send small, frag=%p, ptr=%p, payload=%zd, len=%" PRIu32
                        ", ep=%p, tag=%d\n",
                        (void *) frag, (void *) sseg->ss_ptr, payload_len, sseg->ss_len,
            ...
            if (frag->sf_base.uf_remote_seg[0].seg_addr.pval == NULL) {
                ...
                if ((frag->sf_base.uf_base.des_flags
                     & (MCA_BTL_DES_SEND_ALWAYS_CALLBACK | MCA_BTL_DES_FLAGS_BTL_OWNERSHIP))
                    == (MCA_BTL_DES_SEND_ALWAYS_CALLBACK | MCA_BTL_DES_FLAGS_BTL_OWNERSHIP)) {
        ...
        if (opal_list_is_empty(&endpoint->endpoint_frag_send_queue)
            ...
            opal_list_remove_item(&module->endpoints_with_sends, &endpoint->super);
    ...
    while (get_send_credits(prio_channel) > 1 && endpoint != NULL) {
        ...
        if (endpoint->endpoint_acktime == 0 || endpoint->endpoint_acktime <= get_ticks()) {
                      ...
                      struct mca_btl_base_descriptor_t *descriptor, mca_btl_base_tag_t tag)
{
    ...
    opal_output(0, "usnic_send: frag=%p, endpoint=%p, tag=%d, sf_size=%d\n", (void *) frag,
                (void *) endpoint, tag, (int) frag->sf_size);
#    if MSGDEBUG1
    {
        unsigned i;
...
#    endif
     ...
     */
    if (frag->sf_base.uf_type == OPAL_BTL_USNIC_FRAG_SMALL_SEND
        && frag->sf_ack_bytes_left < module->max_tiny_payload && WINDOW_OPEN(endpoint)
...
static int usnic_dereg_mr(void *reg_data, mca_rcache_base_registration_t *reg)
{
    opal_btl_usnic_reg_t *ur = (opal_btl_usnic_reg_t *) reg;
...
static void module_async_event_callback(int fd, short flags, void *arg)
{
    ...
    if (-FI_EAGAIN == ret) {
        ...
        opal_show_help("help-mpi-btl-usnic.txt", "libfabric API failed", true,
                       opal_process_info.nodename, module->linux_device_name, "fi_eq_read()",
                       __FILE__, __LINE__, ret, "Failed to get domain event");
        fatal = true;
    }
        ...
        case 0: // USD_EVENT_LINK_UP:
            /* This event should never happen, because OMPI will
               ...
               ignore it. */
            opal_output_verbose(10, USNIC_OUT, "btl:usnic: got LINK_UP on %s",
                                module->linux_device_name);
            break;
        ...
        default:
            ...
            opal_show_help("help-mpi-btl-usnic.txt", "async event", true,
                           opal_process_info.nodename, module->linux_device_name, str, entry.data);
            fatal = true;
        }
    }
 ...
 */
static int create_ep(opal_btl_usnic_module_t *module, struct opal_btl_usnic_channel_t *channel)
{
    int rc;
    ...
    if (NULL == hint) {
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name, "fi_dupinfo() failed",
                       __FILE__, __LINE__, -1, "Unknown");
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    if (0 != rc) {
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name, "fi_getinfo() failed",
                       __FILE__, __LINE__, rc, fi_strerror(-rc));
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    if (0 != rc || NULL == channel->ep) {
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name,
                       "fi_endpoint() failed", __FILE__, __LINE__, rc, fi_strerror(-rc));
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    if ((int) channel->info->rx_attr->size < channel->chan_rd_num) {
        ...
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name,
                       "endpoint RX queue length is too short", __FILE__, __LINE__, rc,
                       fi_strerror(rc));
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    if ((int) channel->info->tx_attr->size < channel->chan_sd_num) {
        ...
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name,
                       "endpoint TX queue length is too short", __FILE__, __LINE__, rc,
                       fi_strerror(rc));
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    if (0 != rc) {
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name,
                       "fi_ep_bind() SCQ to EP failed", __FILE__, __LINE__, rc, fi_strerror(-rc));
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    if (0 != rc) {
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name,
                       "fi_ep_bind() RCQ to EP failed", __FILE__, __LINE__, rc, fi_strerror(-rc));
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    if (0 != rc) {
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name,
                       "fi_ep_bind() AV to EP failed", __FILE__, __LINE__, rc, fi_strerror(-rc));
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    if (0 != rc) {
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name, "fi_enable() failed",
                       __FILE__, __LINE__, rc, fi_strerror(-rc));
        return OPAL_ERR_OUT_OF_RESOURCE;
    }
    ...
    if (0 == sin->sin_port) {
        ...
        if (0 != rc) {
            opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                           opal_process_info.nodename, module->linux_device_name,
                           "fi_getname() failed", __FILE__, __LINE__, rc, fi_strerror(-rc));
            return OPAL_ERR_OUT_OF_RESOURCE;
        }
        assert(0 != sin->sin_port);
    }
 ...
 */
static int init_one_channel(opal_btl_usnic_module_t *module, int index, int max_msg_size,
                            int rd_num, int sd_num, int cq_num)
{
    int i;
    int rc;
    uint32_t segsize;
    ...
    if (0 != rc) {
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name, "failed to create CQ",
                       __FILE__, __LINE__, rc, fi_strerror(-rc));
        goto error;
    }
    ...
    if ((int) cq_attr.size < cq_num) {
        ...
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name,
                       "created CQ is too small", __FILE__, __LINE__, rc, fi_strerror(rc));
        goto error;
    }
    ...
    opal_output_verbose(15, USNIC_OUT,
                        "btl:usnic:init_one_channel:%s: channel %s, rx queue size=%" PRIsize_t
                        ", tx queue size=%" PRIsize_t ", cq size=%" PRIsize_t ", send credits=%d",
                        module->linux_device_name,
                        (index == USNIC_PRIORITY_CHANNEL) ? "priority" : "data",
                        channel->info->rx_attr->size, channel->info->tx_attr->size, cq_attr.size,
                        channel->credits);
     ...
     * cache line boundary.
     */
    segsize = (max_msg_size + channel->info->ep_attr->msg_prefix_size + opal_cache_line_size - 1)
              & ~(opal_cache_line_size - 1);
    OBJ_CONSTRUCT(&channel->recv_segs, opal_free_list_t);
    rc = usnic_compat_free_list_init(
        &channel->recv_segs, sizeof(opal_btl_usnic_recv_segment_t) /* frag size */,
        opal_cache_line_size /* frag alignment */, OBJ_CLASS(opal_btl_usnic_recv_segment_t),
        segsize /* payload buffer size */, opal_cache_line_size /* payload alignmt */,
        rd_num /* num erorments to alloc */, rd_num /* max elements to alloc */,
        rd_num /* num elements per alloc */,
        module->super.btl_mpool /* mpool for (1.x, 2.0: reg, 2.1+: allocation) */,
        0 /* mpool reg flags */, module->rcache /* registration cache for 2.1+ */,
        NULL /* item_init */, NULL /* item_init_context */);
    ...
    for (i = 0; i < rd_num; i++) {
        ...
        if (NULL == rseg) {
            opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                           opal_process_info.nodename, module->linux_device_name,
                           "Failed to get receive buffer from freelist", __FILE__, __LINE__);
            goto error;
        }
        ...
        rc = fi_recv(channel->ep, rseg->rs_protocol_header, segsize, NULL, FI_ADDR_UNSPEC, rseg);
        if (0 != rc) {
            opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                           opal_process_info.nodename, module->linux_device_name,
                           "Failed to post receive buffer", __FILE__, __LINE__);
            goto error;
        }
    }
 ...
 */
static opal_btl_usnic_seq_t get_initial_seq_no(void)
{
...
static void init_local_modex_part1(opal_btl_usnic_module_t *module)
{
    ...
    opal_btl_usnic_snprintf_ipv4_addr(module->if_ipv4_addr_str, sizeof(module->if_ipv4_addr_str),
                                      modex->ipv4_addr, modex->netmask);

    opal_output_verbose(5, USNIC_OUT, "btl:usnic: %s IP charactertics: %s, %u Mbps",
                        module->linux_device_name, module->if_ipv4_addr_str,
...
{
    mca_btl_usnic_component.transport_header_len = module->fabric_info->ep_attr->msg_prefix_size;
    mca_btl_usnic_component.transport_protocol = module->fabric_info->ep_attr->protocol;
    ...
    if (1 == FI_MAJOR(libfabric_api) && 0 == FI_MINOR(libfabric_api)) {
        mca_btl_usnic_component.prefix_send_offset = 0;
    } else {
        mca_btl_usnic_component.prefix_send_offset = module->fabric_info->ep_attr->msg_prefix_size;
    }
}
...
static void init_payload_lengths(opal_btl_usnic_module_t *module)
{
    ...
    module->max_frag_payload = module->local_modex.max_msg_size -    /* start with the MTU */
                               sizeof(opal_btl_usnic_btl_header_t) - /* subtract size of
                                                                        the BTL header */
                               mca_btl_usnic_component.prefix_send_offset;

    /* same, but use chunk header */
    module->max_chunk_payload = module->local_modex.max_msg_size
    ...
    module->max_tiny_payload = module->max_tiny_msg_size - sizeof(opal_btl_usnic_btl_header_t);
}
...
static void init_pml_values(opal_btl_usnic_module_t *module)
{
    ...
    if (0 == module->super.btl_eager_limit) {
        ...
        if (1 == mca_btl_usnic_component.num_modules) {
            module->super.btl_eager_limit = USNIC_DFLT_EAGER_LIMIT_1DEVICE;
        } else {
            module->super.btl_eager_limit = USNIC_DFLT_EAGER_LIMIT_NDEVICES;
        }
    }
...
#if BTL_VERSION == 30
    module->super.btl_put_limit = module->super.btl_eager_limit;
#endif
}
...
static void init_connectivity_checker(opal_btl_usnic_module_t *module)
{
    ...
    if (OPAL_SUCCESS != rc) {
        OPAL_ERROR_LOG(rc);
        opal_btl_usnic_util_abort("Failed to notify connectivity agent to listen", __FILE__,
                                  __LINE__);
    }
}
...
static void init_hwloc(opal_btl_usnic_module_t *module)
{
    ...
    if (mca_btl_usnic_component.want_numa_device_assignment) {
        ...
        opal_output_verbose(5, USNIC_OUT,
                            "btl:usnic: not sorting devices by NUMA distance (MCA "
                            "btl_usnic_want_numa_device_assignment)");
    }
}
...
static int init_mpool(opal_btl_usnic_module_t *module)
{
    ...
    module->rcache = mca_rcache_base_module_create(mca_btl_usnic_component.usnic_rcache_name,
                                                   &module->super, &rcache_resources);
    if (NULL == module->rcache) {
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name, "create rcache",
                       __FILE__, __LINE__);
        return OPAL_ERROR;
    }
    module->super.btl_mpool = mca_mpool_base_module_lookup(
        mca_btl_usnic_component.usnic_mpool_hints);
    if (NULL == module->super.btl_mpool) {
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, module->linux_device_name, "create mpool",
                       __FILE__, __LINE__);
        return OPAL_ERROR;
    }
...
static int init_channels(opal_btl_usnic_module_t *module)
{
    ...
    memset(&module->mod_channels[0], 0, sizeof(module->mod_channels[0]));
    memset(&module->mod_channels[1], 0, sizeof(module->mod_channels[1]));
    ...
    rc = fi_open_ops(&module->av->fid, FI_USNIC_AV_OPS_1, 0, (void **) &module->usnic_av_ops, NULL);
    if (rc != OPAL_SUCCESS) {
        goto destroy;
    }
    ...
    rc = init_one_channel(module, USNIC_PRIORITY_CHANNEL, module->max_tiny_msg_size,
                          module->prio_rd_num, module->prio_sd_num, module->prio_cq_num);
    if (rc != OPAL_SUCCESS) {
        goto destroy;
    }
    rc = init_one_channel(module, USNIC_DATA_CHANNEL, module->fabric_info->ep_attr->max_msg_size,
...
static void init_async_event(opal_btl_usnic_module_t *module)
{
    ...
    if (ret != 0) {
        opal_show_help("help-mpi-btl-usnic.txt", "libfabric API failed", true,
                       opal_process_info.nodename, module->linux_device_name,
                       "fi_control(eq, FI_GETWAIT)", __FILE__, __LINE__, ret, fi_strerror(-ret));
        return;
    }
    ...
    opal_event_set(opal_sync_event_base, &(module->device_async_event), fd,
                   OPAL_EV_READ | OPAL_EV_PERSIST, module_async_event_callback, module);
...
static void init_freelists(opal_btl_usnic_module_t *module)
{
    ...
    segsize = (module->local_modex.max_msg_size + mca_btl_usnic_component.prefix_send_offset
               + opal_cache_line_size - 1)
              & ~(opal_cache_line_size - 1);
    ...
    rc = usnic_compat_free_list_init(&module->small_send_frags,
                                     sizeof(opal_btl_usnic_small_send_frag_t), opal_cache_line_size,
                                     OBJ_CLASS(opal_btl_usnic_small_send_frag_t), segsize,
                                     opal_cache_line_size, module->sd_num * 4, -1,
                                     module->sd_num / 2, module->super.btl_mpool,
                                     0 /* mpool reg flags */, module->rcache, NULL /* item_init */,
    ...
    rc = usnic_compat_free_list_init(&module->large_send_frags,
                                     sizeof(opal_btl_usnic_large_send_frag_t), opal_cache_line_size,
                                     OBJ_CLASS(opal_btl_usnic_large_send_frag_t),
                                     0, /* payload size */
                                     0, /* payload align */
                                     module->sd_num / 8, -1, module->sd_num / 8, NULL,
                                     0 /* mpool reg flags */, NULL /* unused0 */,
                                     NULL /* item_init */, NULL /* item_init_context */);
    assert(OPAL_SUCCESS == rc);

    OBJ_CONSTRUCT(&module->put_dest_frags, opal_free_list_t);
    rc = usnic_compat_free_list_init(&module->put_dest_frags,
                                     sizeof(opal_btl_usnic_put_dest_frag_t), opal_cache_line_size,
    ...
    rc = usnic_compat_free_list_init(&module->chunk_segs, sizeof(opal_btl_usnic_chunk_segment_t),
                                     opal_cache_line_size,
                                     OBJ_CLASS(opal_btl_usnic_chunk_segment_t), segsize,
                                     opal_cache_line_size, module->sd_num * 4, -1,
                                     module->sd_num / 2, module->super.btl_mpool,
                                     0 /* mpool reg flags */, module->rcache, NULL /* item_init */,
                                     NULL /* item_init_context */);
    ...
    ack_segment_len = (sizeof(opal_btl_usnic_btl_header_t)
                       + mca_btl_usnic_component.prefix_send_offset + opal_cache_line_size - 1)
                      & ~(opal_cache_line_size - 1);
    OBJ_CONSTRUCT(&module->ack_segs, opal_free_list_t);
    rc = usnic_compat_free_list_init(&module->ack_segs, sizeof(opal_btl_usnic_ack_segment_t),
                                     opal_cache_line_size, OBJ_CLASS(opal_btl_usnic_ack_segment_t),
                                     ack_segment_len, opal_cache_line_size, module->sd_num * 4, -1,
                                     module->sd_num / 2, module->super.btl_mpool,
                                     0 /* mpool reg flags */, module->rcache, NULL /* item_init */,
                                     NULL /* item_init_context */);
    assert(OPAL_SUCCESS == rc);
    ...
    module->module_recv_buffers = calloc(module->last_pool + 1, sizeof(opal_free_list_t));
    ...
    for (int i = module->first_pool; i <= module->last_pool; ++i) {
        ...
        rc = usnic_compat_free_list_init(&module->module_recv_buffers[i], elt_size,
                                         opal_cache_line_size, OBJ_CLASS(opal_btl_usnic_rx_buf_t),
                                         0,   /* payload size */
                                         0,   /* payload align */
                                         128, /* init elts to alloc */
                                         128, /* max elts to alloc */
                                         128, /* num elts per alloc */
                                         NULL /* mpool */, 0 /* mpool reg flags */,
                                         NULL /* unused0 */, NULL /* item_init */,
                                         NULL /* item_init_context */);
        assert(OPAL_SUCCESS == rc);
    }
}
