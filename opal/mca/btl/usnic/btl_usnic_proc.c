...
OBJ_CLASS_INSTANCE(opal_btl_usnic_proc_t, opal_list_item_t, proc_construct, proc_destruct);
...
opal_btl_usnic_proc_t *opal_btl_usnic_proc_lookup_ompi(opal_proc_t *opal_proc)
{
    ...
    for (usnic_proc = (opal_btl_usnic_proc_t *) opal_list_get_first(
             &mca_btl_usnic_component.usnic_procs);
         usnic_proc
         != (opal_btl_usnic_proc_t *) opal_list_get_end(&mca_btl_usnic_component.usnic_procs);
         usnic_proc = (opal_btl_usnic_proc_t *) opal_list_get_next(usnic_proc)) {
 ...
 * name.
 */
opal_btl_usnic_endpoint_t *opal_btl_usnic_proc_lookup_endpoint(opal_btl_usnic_module_t *receiver,
                                                               uint64_t sender_proc_name)
{
    ...
    MSGDEBUG1_OUT("lookup_endpoint: recvmodule=%p sendhash=0x%" PRIx64, (void *) receiver,
                  sender_proc_name);

    opal_mutex_lock(&receiver->all_endpoints_lock);
    for (item = opal_list_get_first(&receiver->all_endpoints);
         item != opal_list_get_end(&receiver->all_endpoints); item = opal_list_get_next(item)) {
        endpoint = container_of(item, opal_btl_usnic_endpoint_t, endpoint_endpoint_li);
        ...
        if (usnic_compat_rte_hash_name(&(proc->proc_opal->proc_name)) == sender_proc_name) {
            MSGDEBUG1_OUT("lookup_endpoint: matched endpoint=%p", (void *) endpoint);
 ...
 */
static int create_proc(opal_proc_t *opal_proc, opal_btl_usnic_proc_t **usnic_proc)
{
    ...
    *usnic_proc = NULL;
    ...
    usnic_compat_modex_recv(&rc, &mca_btl_usnic_component.super.btl_version, opal_proc,
                            &proc->proc_modex, &size);
    ...
    if (OPAL_ERR_NOT_FOUND == rc) {
        ...
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       ...
                       __FILE__, __LINE__, opal_strerror(rc));
        OBJ_RELEASE(proc);
        return OPAL_ERROR;
    }
    ...
    if ((size % sizeof(opal_btl_usnic_modex_t)) != 0) {
        ...
        snprintf(msg, sizeof(msg), "sizeof(modex for peer %s data) == %d, expected multiple of %d",
                 usnic_compat_proc_name_print(&opal_proc->proc_name), (int) size,
                 (int) sizeof(opal_btl_usnic_modex_t));
        opal_show_help("help-mpi-btl-usnic.txt", "internal error during init", true,
                       opal_process_info.nodename, "<none>", 0, "invalid modex data", __FILE__,
                       __LINE__, msg);
    ...
    if (mca_btl_usnic_component.transport_protocol != proc->proc_modex->protocol) {
        uint64_t proto;
        char protostr[32];
        proto = mca_btl_usnic_component.transport_protocol;
        memset(protostr, 0, sizeof(protostr));
        opal_string_copy(protostr, fi_tostr(&proto, FI_TYPE_PROTOCOL), sizeof(protostr));
        proto = proc->proc_modex->protocol;
        opal_show_help("help-mpi-btl-usnic.txt", "transport mismatch", true,
                       opal_process_info.nodename, protostr, "peer",
    ...
    proc->proc_endpoints = (mca_btl_base_endpoint_t **) calloc(proc->proc_modex_count,
...
static uint64_t compute_weight(opal_btl_usnic_module_t *module,
                               opal_btl_usnic_modex_t *proc_modex_addr)
{
    ...
    inet_ntop(AF_INET, &sinp->sin_addr, my_ip_string, sizeof(my_ip_string));
    inet_ntop(AF_INET, &proc_modex_addr->ipv4_addr, peer_ip_string, sizeof(peer_ip_string));
    ...
    opal_output_verbose(5, USNIC_OUT,
                        ...
                        __func__, my_ip_string, usnic_netmask_to_cidrlen(uip->ui.v1.ui_netmask_be),
                        peer_ip_string, usnic_netmask_to_cidrlen(proc_modex_addr->netmask),
                        (mynet == peernet ? "match" : "DO NOT match"));

    min_link_speed_gbps = MIN(module->super.btl_bandwidth, proc_modex_addr->link_speed_mbps) / 1000;
    ...
    if (0 != err || (0 == err && -1 == metric)) {
        ...
        return ((uint64_t)(mynet == peernet) << 48) | ((uint64_t)(min_link_speed_gbps & 0xff) << 40)
               | ((uint64_t) 0x1 << 32) | (/*metric=*/0);
    }
}
 ...
 * "match_modex")
 */
static void edge_pairs_to_match_table(opal_btl_usnic_proc_t *proc, bool proc_is_left, int nme,
                                      int *me)
{
    int i;
    int left, right;
    ...
    for (i = 0; i < num_modules; ++i) {
        if (-1 != proc->proc_ep_match_table[i]) {
            opal_output_verbose(
                5, USNIC_OUT, "btl:usnic:%s: module[%d] (%p) should claim endpoint[%d] on proc %p",
                __func__, i, (void *) mca_btl_usnic_component.usnic_active_modules[i],
                proc->proc_ep_match_table[i], (void *) proc);
        } else {
            opal_output_verbose(
                5, USNIC_OUT, "btl:usnic:%s: module[%d] (%p) will NOT claim an endpoint on proc %p",
                __func__, i, (void *) mca_btl_usnic_component.usnic_active_modules[i],
                (void *) proc);
        }
    }
 ...
 */
static int create_proc_module_graph(opal_btl_usnic_proc_t *proc, bool proc_is_left,
                                    opal_bp_graph_t **g_out)
{
    ...
    *g_out = NULL;
    ...
    for (i = 0; i < num_modules; ++i) {
        int idx = -1;
        err = opal_bp_graph_add_vertex(g, mca_btl_usnic_component.usnic_active_modules[i], &idx);
        if (OPAL_SUCCESS != err) {
            OPAL_ERROR_LOG(err);
            goto out_free_graph;
        }
    ...
    for (i = 0; i < num_modules; ++i) {
        for (j = 0; j < (int) proc->proc_modex_count; ++j) {
            ...
            opal_output_verbose(20, USNIC_OUT,
                                "btl:usnic:%s: weight=0x%016" PRIx64
                                " for edge module[%d] (%p) <--> endpoint[%d] on proc %p",
                                __func__, weight, i,
                                (void *) mca_btl_usnic_component.usnic_active_modules[i], j,
                                (void *) proc);
            ...
            opal_output_verbose(20, USNIC_OUT,
                                "btl:usnic:%s: adding edge (%d,%d) with cost=%" PRIi64
                                " for edge module[%d] <--> endpoint[%d]",
 ...
 */
static int match_modex(opal_btl_usnic_module_t *module, opal_btl_usnic_proc_t *proc, int *index_out)
{
    ...
    *index_out = -1;
    ...
    opal_output_verbose(20, USNIC_OUT, "btl:usnic:%s: module=%p proc=%p with dimensions %d x %d",
                        __func__, (void *) module, (void *) proc, num_modules,
                        (int) proc->proc_modex_count);
    ...
    if (NULL == proc->proc_ep_match_table) {
        proc->proc_ep_match_table = malloc(num_modules * sizeof(*proc->proc_ep_match_table));
        if (NULL == proc->proc_ep_match_table) {
            OPAL_ERROR_LOG(OPAL_ERR_OUT_OF_RESOURCE);
            return OPAL_ERR_OUT_OF_RESOURCE;
        }
...
#else
        proc_is_left = usnic_compat_proc_name_compare(proc->proc_opal->proc_name,
                                                      opal_proc_local_get()->proc_name);
#endif
    ...
    if (!proc->proc_match_exists) {
        opal_output_verbose(5, USNIC_OUT,
                            "btl:usnic:%s: unable to find any valid interface pairs for proc %s",
                            __func__, usnic_compat_proc_name_print(&proc->proc_opal->proc_name));
    ...
    if (*index_out >= 0
        ...
        opal_show_help("help-mpi-btl-usnic.txt", "MTU mismatch", true, opal_process_info.nodename,
                       module->linux_device_name, module->fabric_info->ep_attr->max_msg_size,
                       errhost, proc->proc_modex[*index_out].max_msg_size);
        free(errhost);
        *index_out = -1;
        return OPAL_ERR_UNREACH;
    }
 ...
 */
static int start_av_insert(opal_btl_usnic_module_t *module, opal_btl_usnic_endpoint_t *endpoint,
                           int channel)
{
    int ret;
    ...
    opal_output_verbose(5, USNIC_OUT, "btl:usnic:start_av_insert: to channel %d at %s:%d", channel,
                        str, modex->ports[channel]);
    ...
    ret = fi_av_insert(module->av, &sin, 1, &endpoint->endpoint_remote_addrs[channel], 0, context);
    /* Did an error occur? */
    if (0 != ret) {
        opal_show_help("help-mpi-btl-usnic.txt", "libfabric API failed", true,
                       opal_process_info.nodename, module->linux_device_name, "fi_av_insert()",
                       __FILE__, __LINE__, ret, "Failed to initiate AV insert");
        free(context);
        return OPAL_ERROR;
    }
 ...
 */
int opal_btl_usnic_create_endpoint(opal_btl_usnic_module_t *module, opal_btl_usnic_proc_t *proc,
                                   opal_btl_usnic_endpoint_t **endpoint_o)
{
    ...
    endpoint->endpoint_next_contig_seq_to_recv = endpoint->endpoint_remote_modex.isn;
    endpoint->endpoint_highest_seq_rcvd = endpoint->endpoint_next_contig_seq_to_recv - 1;
    ...
    MSGDEBUG1_OUT("create_endpoint: module=%p claimed endpoint=%p on proc=%p (hash=0x%" PRIx64
                  ")\n",
                  (void *) module, (void *) endpoint, (void *) proc, proc->proc_opal->proc_name);
    ...
    opal_list_append(&(module->all_endpoints), &(endpoint->endpoint_endpoint_li));
 ...
 * info.
 */
int opal_btl_usnic_proc_match(opal_proc_t *opal_proc, opal_btl_usnic_module_t *module,
                              opal_btl_usnic_proc_t **proc)
{
