...
OBJ_CLASS_INSTANCE(mca_base_component_repository_item_t, opal_list_item_t, ri_constructor,
                   ri_destructor);
...
OBJ_CLASS_INSTANCE(mca_base_failed_component_t, opal_list_item_t, clf_constructor, clf_destructor);
...
static int process_repository_item(const char *filename, void *data)
{
    ...
    ret = sscanf(base,
                 "mca_%" STRINGIFY(MCA_BASE_MAX_TYPE_NAME_LEN) "[^_]_%" STRINGIFY(
                     MCA_BASE_MAX_COMPONENT_NAME_LEN) "s",
                 type, name);
    if (0 > ret) {
    ...
    ret = opal_hash_table_get_value_ptr(&mca_base_component_repository, type, strlen(type),
                                        (void **) &component_list);
    if (OPAL_SUCCESS != ret) {
        ...
        ret = opal_hash_table_set_value_ptr(&mca_base_component_repository, type, strlen(type),
                                            (void *) component_list);
        if (OPAL_SUCCESS != ret) {
            free(base);
                                       ...
                                       mca_base_component_repository_item_t *ri)
{
    ...
    if (OPAL_SUCCESS != opal_dl_open(ri->ri_path, true, false, &ri->ri_dlhandle, &err_msg)) {
        ...
        if (0 == strcasecmp("file not found", err_msg)
            && (file_exists(ri->ri_path, "lo") || file_exists(ri->ri_path, "so")
...
void mca_base_component_repository_finalize(void)
{
    ...
    while (OPAL_SUCCESS == ret) {
        ...
        ret = opal_hash_table_get_next_key_ptr(&mca_base_component_repository, &key, &key_size,
