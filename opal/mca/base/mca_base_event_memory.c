 ...
 * allocate, or walk the registration list.  So the hook does the minimum -- a
 * thread-local re-entrancy guard and, if a listener is attached, two atomic
 * adds into a single never-freed sink: one bumping the release COUNT and one
 * adding the release length to a cumulative BYTE total.  The framework later
 * *folds* that {count, bytes} pair (at safe framework operations touching the
 * event, never in the hook) and delivers it as ONE aggregated, data-bearing
 * event whose payload carries the count and byte total.  A registration that
 * has only a dropped handler (no callback) instead observes the aggregated
 * event as a single drop.
 *
...
/* Re-entrancy guard: the hook runs inside free()/munmap() and "can be called
   recursively"; our body allocates nothing, but guard anyway (spec sec. 7.1). */
static opal_thread_local int in_release_hook = 0;

/* Runs inside free()/munmap()/brk() (etc.): no lock, no alloc, no list walk.
   `active` is a relaxed read of an atomic, paired with the release store at
   detach; reading a stale `true` at most over-counts by a hair (best-effort).
