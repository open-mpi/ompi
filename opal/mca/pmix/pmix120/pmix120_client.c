...
#include "pmix120.h"
#include "opal/mca/pmix/pmix120/pmix/include/pmix.h"
#include "opal/mca/pmix/pmix120/pmix/src/buffer_ops/buffer_ops.h"

static pmix_proc_t my_proc;
static char *dbgvalue=NULL;

int pmix120_client_init(void)
{
   ...
   if (NULL != getenv(OPAL_MCA_PREFIX"orte_launch")) {
        ...
        mca_pmix_pmix120_component.native_launch = true;
...
int pmix120_client_finalize(void)
{
    pmix_status_t rc;
                  ...
                  opal_list_t *procs)
{
    ...
    size_t n, cnt=0;
    opal_namelist_t *ptr;
    opal_pmix120_jobid_trkr_t *job, *jptr;
...
int pmix120_store_local(const opal_process_name_t *proc, opal_value_t *val)
{
    pmix_value_t kv;
    pmix_status_t rc;
    pmix_proc_t p;
...
int pmix120_fence(opal_list_t *procs, int collect_data)
{
    pmix_status_t rc;
    pmix_proc_t *parray=NULL;
    size_t n, cnt=0;
...
int pmix120_fencenb(opal_list_t *procs, int collect_data,
...
int pmix120_get(const opal_process_name_t *proc, const char *key,
    ...
    *val = NULL;
    ...
    if (NULL != info) {
        ...
        if (0 < ninfo) {
            ...
            OPAL_LIST_FOREACH(ival, info, opal_value_t) {
                ...
                pmix120_value_load(&pinfo[n].value, ival);
            }
        } else {
            pinfo = NULL;
        }
    ...
    if (PMIX_SUCCESS == rc) {
        if (NULL == kv) {
            ...
            *val = OBJ_NEW(opal_value_t);
            ret = pmix120_value_unload(*val, kv);
            PMIX_VALUE_FREE(kv, 1);
        }
    } else {
        ret = pmix120_convert_rc(rc);
    }
                       ...
                       pmix_value_t *kv, void *cbdata)
{
    ...
    if (PMIX_SUCCESS == status && NULL != kv) {
        rc = pmix120_value_unload(&val, kv);
        v = &val;
    }
...
int pmix120_getnb(const opal_process_name_t *proc, const char *key,
                opal_list_t *info,
    ...
    if (NULL != info) {
        ...
        if (0 < op->sz) {
            ...
            OPAL_LIST_FOREACH(ival, info, opal_value_t) {
                ...
                pmix120_value_load(&op->info[n].value, ival);
            }
        }
    }
...
int pmix120_publish(opal_list_t *info)
{
    pmix_info_t *pinfo;
    pmix_status_t ret;
    opal_value_t *iptr;
    ...
    if (0 < sz) {
        ...
        OPAL_LIST_FOREACH(iptr, info, opal_value_t) {
            ...
            pmix120_value_load(&pinfo[n].value, iptr);
            ++n;
        }
    } else {
        pinfo = NULL;
    ...
    return pmix120_convert_rc(ret);
}

int pmix120_publishnb(opal_list_t *info,
                    opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    pmix_status_t ret;
    opal_value_t *iptr;
    ...
    if (0 < op->sz) {
        ...
        OPAL_LIST_FOREACH(iptr, info, opal_value_t) {
            ...
            pmix120_value_load(&op->info[n].value, iptr);
            ++n;
        }
    }
...
int pmix120_lookup(opal_list_t *data, opal_list_t *info)
{
    pmix_pdata_t *pdata;
    pmix_info_t *pinfo;
    size_t sz, ninfo, n;
    ...
    if (PMIX_SUCCESS == ret) {
        ...
        n=0;
        OPAL_LIST_FOREACH(d, data, opal_pmix_pdata_t) {
            if (mca_pmix_pmix120_component.native_launch) {
                /* if we were launched by the OMPI RTE, then
                 * the jobid is in a special format - so get it */
            ...
            rc = pmix120_value_unload(&d->value, &pdata[n].value);
            if (OPAL_SUCCESS != rc) {
                OPAL_ERROR_LOG(rc);
                      ...
                      void *cbdata)
{
    ...
    if (OPAL_SUCCESS == rc) {
        ...
        for (n=0; n < ndata; n++) {
            ...
            rc = pmix120_value_unload(&d->value, &data[n].value);
            if (OPAL_SUCCESS != rc) {
...
int pmix120_lookupnb(char **keys, opal_list_t *info,
...
int pmix120_unpublish(char **keys, opal_list_t *info)
{
    pmix_status_t ret;
    size_t ninfo, n;
    pmix_info_t *pinfo;
...
int pmix120_unpublishnb(char **keys, opal_list_t *info,
                      opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    pmix_status_t ret;
    pmix120_opcaddy_t *op;
...
int pmix120_spawn(opal_list_t *job_info, opal_list_t *apps, opal_jobid_t *jobid)
{
    pmix_status_t ret;
    ...
    if (NULL != job_info && 0 < (ninfo = opal_list_get_size(job_info))) {
        ...
        OPAL_LIST_FOREACH(info, job_info, opal_value_t) {
            ...
            pmix120_value_load(&pinfo[n].value, info);
            ++n;
        }
    }
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (0 < (papps[n].ninfo = opal_list_get_size(&app->info))) {
            ...
            OPAL_LIST_FOREACH(info, &app->info, opal_value_t) {
                ...
                pmix120_value_load(&papps[n].info[m].value, info);
                ++m;
            }
        }
        ++n;
...
int pmix120_spawnnb(opal_list_t *job_info, opal_list_t *apps,
                    opal_pmix_spawn_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL != job_info && 0 < (op->ninfo = opal_list_get_size(job_info))) {
        ...
        OPAL_LIST_FOREACH(info, job_info, opal_value_t) {
            ...
            pmix120_value_load(&op->info[n].value, info);
            ++n;
        }
    }
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (0 < (op->apps[n].ninfo = opal_list_get_size(&app->info))) {
            ...
            OPAL_LIST_FOREACH(info, &app->info, opal_value_t) {
                ...
                pmix120_value_load(&op->apps[n].info[m].value, info);
                ++m;
            }
        }
        ++n;
...
int pmix120_connect(opal_list_t *procs)
{
    pmix_status_t ret;
    pmix_proc_t *parray=NULL;
    size_t n, cnt=0;
...
int pmix120_connectnb(opal_list_t *procs,
                    ...
                    void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
        /* look thru our list of jobids and find the
         * corresponding nspace */
        OPAL_LIST_FOREACH(job, &mca_pmix_pmix120_component.jobids, opal_pmix120_jobid_trkr_t) {
...
int pmix120_disconnect(opal_list_t *procs)
{
    pmix_status_t ret;
    pmix_proc_t *parray=NULL;
    size_t n, cnt=0;
...
int pmix120_disconnectnb(opal_list_t *procs,
                         opal_pmix_op_cbfunc_t cbfunc,
                         void *cbdata)
{
    pmix_status_t ret;
...
int pmix120_resolve_peers(const char *nodename, opal_jobid_t jobid,
                          opal_list_t *procs)
{
    char *nspace;
...
int pmix120_resolve_nodes(opal_jobid_t jobid, char **nodelist)
{
    pmix_status_t ret;
    char *nspace=NULL;
    opal_pmix120_jobid_trkr_t *job, *jptr;
