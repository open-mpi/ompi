...
pmix_server_module_t pmix120_module = {
...
opal_pmix_server_module_t *pmix120_host_module = NULL;


static void opal_opcbfunc(int status, void *cbdata)
{
    pmix120_opalcaddy_t *opalcaddy = (pmix120_opalcaddy_t*)cbdata;

    if (NULL != opalcaddy->opcbfunc) {
        opalcaddy->opcbfunc(pmix120_convert_opalrc(status), opalcaddy->cbdata);
    }
...
static pmix_status_t server_client_connected_fn(const pmix_proc_t *p, void *server_object)
{
    ...
    if (NULL == pmix120_host_module || NULL == pmix120_host_module->client_connected) {
        return PMIX_SUCCESS;
    }

    /* convert the nspace/rank to an opal_process_name_t */
    if (OPAL_SUCCESS != (rc = opal_convert_string_to_jobid(&proc.jobid, p->nspace))) {
        return pmix120_convert_opalrc(rc);
    }
    proc.vpid = p->rank;

    /* pass it up */
    rc = pmix120_host_module->client_connected(&proc, server_object);
    return pmix120_convert_opalrc(rc);
}
...
{
    int rc;
    pmix120_opalcaddy_t *opalcaddy;
    opal_process_name_t proc;

    if (NULL == pmix120_host_module || NULL == pmix120_host_module->client_finalized) {
        return PMIX_SUCCESS;
    }
    ...
    /* setup the caddy */
    opalcaddy = OBJ_NEW(pmix120_opalcaddy_t);
    opalcaddy->opcbfunc = cbfunc;
    opalcaddy->cbdata = cbdata;

    /* pass it up */
    rc = pmix120_host_module->client_finalized(&proc, server_object, opal_opcbfunc, opalcaddy);
    if (OPAL_SUCCESS != rc) {
        OBJ_RELEASE(opalcaddy);
    }
    return pmix120_convert_opalrc(rc);
                                     ...
                                     pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == pmix120_host_module || NULL == pmix120_host_module->abort) {
    ...
    /* pass it up */
    rc = pmix120_host_module->abort(&proc, server_object, status, msg,
                                       ...
                                       pmix_modex_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == pmix120_host_module || NULL == pmix120_host_module->fence_nb) {
        return PMIX_ERR_NOT_SUPPORTED;
    }
    ...
    for (n=0; n < ninfo; n++) {
        ...
        if (OPAL_SUCCESS != (rc = pmix120_value_unload(iptr, &info[n].value))) {
            OBJ_RELEASE(opalcaddy);
            return pmix120_convert_opalrc(rc);
        }
    }
    ...
    /* pass it up */
    rc = pmix120_host_module->fence_nb(&opalcaddy->procs, &opalcaddy->info,
                                          ...
                                          pmix_modex_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == pmix120_host_module || NULL == pmix120_host_module->direct_modex) {
        return PMIX_ERR_NOT_SUPPORTED;
    }
    ...
    /* pass it up */
    rc = pmix120_host_module->direct_modex(&proc, &opalcaddy->info, opmdx_response, opalcaddy);
                                       ...
                                       pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == pmix120_host_module || NULL == pmix120_host_module->publish) {
        return PMIX_ERR_NOT_SUPPORTED;
    }
   ...
   /* convert the nspace/rank to an opal_process_name_t */
    ...
    for (n=0; n < ninfo; n++) {
        ...
        if (OPAL_SUCCESS != (rc = pmix120_value_unload(oinfo, &info[n].value))) {
            OBJ_RELEASE(opalcaddy);
            return pmix120_convert_opalrc(rc);
        }
    }
    ...
    /* pass it up */
    rc = pmix120_host_module->publish(&proc, &opalcaddy->info, opal_opcbfunc, opalcaddy);
    if (OPAL_SUCCESS != rc) {
        OBJ_RELEASE(opalcaddy);
    }
    ...
    if (NULL != opalcaddy->lkupcbfunc) {
        ...
        if (NULL != data) {
            ...
            OPAL_LIST_FOREACH(p, data, opal_pmix_pdata_t) {
                /* convert the jobid */
                char *str = opal_convert_jobid_to_string(p->proc.jobid);
                (void)strncpy(d[n].proc.nspace, str, PMIX_MAX_NSLEN);
                free(str);
                                      ...
                                      pmix_lookup_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == pmix120_host_module || NULL == pmix120_host_module->lookup) {
        return PMIX_ERR_NOT_SUPPORTED;
    }
    ...
    /* pass it up */
    rc = pmix120_host_module->lookup(&proc, keys, &opalcaddy->info, opal_lkupcbfunc, opalcaddy);
    if (OPAL_SUCCESS != rc) {
        OBJ_RELEASE(opalcaddy);
    }
                                         ...
                                         pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == pmix120_host_module || NULL == pmix120_host_module->unpublish) {
        return PMIX_SUCCESS;
    }
    ...
    /* pass it up */
    rc = pmix120_host_module->unpublish(&proc, keys, &opalcaddy->info, opal_opcbfunc, opalcaddy);
    if (OPAL_SUCCESS != rc) {
        OBJ_RELEASE(opalcaddy);
    }
...
static void opal_spncbfunc(int status, opal_jobid_t jobid, void *cbdata)
{
    ...
    if (NULL != opalcaddy->spwncbfunc) {
        ...
        rc = pmix120_convert_opalrc(status);
        /* convert the jobid */
        str = opal_convert_jobid_to_string(jobid);
        (void)strncpy(nspace, str, PMIX_MAX_NSLEN);
        free(str);
                                     ...
                                     pmix_spawn_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == pmix120_host_module || NULL == pmix120_host_module->spawn) {
        return PMIX_ERR_NOT_SUPPORTED;
    }
    ...
    for (k=0; k < ninfo; k++) {
        ...
        if (OPAL_SUCCESS != (rc = pmix120_value_unload(oinfo, &job_info[k].value))) {
            OBJ_RELEASE(opalcaddy);
            return pmix120_convert_opalrc(rc);
        }
    }
    ...
    for (n=0; n < napps; n++) {
        ...
        for (k=0; k < apps[n].ninfo; k++) {
            ...
            if (OPAL_SUCCESS != (rc = pmix120_value_unload(oinfo, &apps[n].info[k].value))) {
                OBJ_RELEASE(opalcaddy);
                return pmix120_convert_opalrc(rc);
            }
        }
    ...
    /* pass it up */
    rc = pmix120_host_module->spawn(&proc, &opalcaddy->info, &opalcaddy->apps, opal_spncbfunc, opalcaddy);
    if (OPAL_SUCCESS != rc) {
        OPAL_ERROR_LOG(rc);
        OBJ_RELEASE(opalcaddy);
    }
                                       ...
                                       pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == pmix120_host_module || NULL == pmix120_host_module->connect) {
        return PMIX_ERR_NOT_SUPPORTED;
    }
    ...
    /* pass it up */
    rc = pmix120_host_module->connect(&opalcaddy->procs, &opalcaddy->info, opal_opcbfunc, opalcaddy);
    if (OPAL_SUCCESS != rc) {
        OBJ_RELEASE(opalcaddy);
    }
                                          ...
                                          pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == pmix120_host_module || NULL == pmix120_host_module->disconnect) {
        return PMIX_ERR_NOT_SUPPORTED;
    }
    ...
    /* pass it up */
    rc = pmix120_host_module->disconnect(&opalcaddy->procs, &opalcaddy->info, opal_opcbfunc, opalcaddy);
    if (OPAL_SUCCESS != rc) {
        OBJ_RELEASE(opalcaddy);
    }
                                            ...
                                            pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    /* pass it up */
    rc = pmix120_host_module->register_events(&opalcaddy->info, opal_opcbfunc, opalcaddy);
    if (OPAL_SUCCESS != rc) {
        OBJ_RELEASE(opalcaddy);
    }
                                              ...
                                              pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    /* pass it up */
    rc = pmix120_host_module->deregister_events(&opalcaddy->info, opal_opcbfunc, opalcaddy);
    if (OPAL_SUCCESS != rc) {
        OBJ_RELEASE(opalcaddy);
    }
                                        ...
                                        pmix_connection_cbfunc_t cbfunc)
{
    ...
    if (NULL == pmix120_host_module || NULL == pmix120_host_module->listener) {
        return PMIX_ERR_NOT_SUPPORTED;
    }

    rc = pmix120_host_module->listener(listening_sd, cbfunc);
    return pmix120_convert_opalrc(rc);
}
