...
int pmix3x_client_init(void)
{
    ...
    PMIx_Register_event_handler(NULL, 0, NULL, 0, pmix3x_event_hdlr, errreg_cbfunc, NULL);
    return OPAL_SUCCESS;
    ...
    *val = NULL;
    ...
    if (PMIX_SUCCESS == rc) {
        if (NULL == kv) {
            ...
            *val = OBJ_NEW(opal_value_t);
            ret = pmix3x_value_unload(*val, kv);
    ...
    if (NULL != info) {
        ...
        if (0 < op->sz) {
            ...
            OPAL_LIST_FOREACH(ival, info, opal_value_t) {
                ...
                pmix3x_value_load(&op->info[n].value, ival);
                ++n;
            }
        }
    }
...
int pmix3x_spawn(opal_list_t *job_info, opal_list_t *apps, opal_jobid_t *jobid)
{
    ...
    if (NULL != job_info && 0 < (ninfo = opal_list_get_size(job_info))) {
        ...
        OPAL_LIST_FOREACH(info, job_info, opal_value_t) {
            ...
            pmix3x_value_load(&pinfo[n].value, info);
            ++n;
        }
    }
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (0 < (papps[n].ninfo = opal_list_get_size(&app->info))) {
            ...
            OPAL_LIST_FOREACH(info, &app->info, opal_value_t) {
                ...
                pmix3x_value_load(&papps[n].info[m].value, info);
                ++m;
            }
        }
        ++n;
...
int pmix3x_resolve_peers(const char *nodename, opal_jobid_t jobid,
                         opal_list_t *procs)
{
    char *nspace;
