...
int ext2x_client_init(void)
{
    ...
    PMIx_Register_event_handler(NULL, 0, NULL, 0, ext2x_event_hdlr, errreg_cbfunc, NULL);
    return OPAL_SUCCESS;
    ...
    *val = NULL;
    ...
    if (PMIX_SUCCESS == rc) {
        if (NULL == kv) {
            ...
            *val = OBJ_NEW(opal_value_t);
            ret = ext2x_value_unload(*val, key, kv);
                       ...
                       pmix_value_t *kv, void *cbdata)
{
    ...
    if (PMIX_SUCCESS == status && NULL != kv) {
        rc = ext2x_value_unload(&val, op->key, kv);
        v = &val;
    }
    ...
    if (NULL != info) {
        ...
        if (0 < op->sz) {
            ...
            OPAL_LIST_FOREACH(ival, info, opal_value_t) {
                ...
                ext2x_value_load(&op->info[n].value, ival);
                ++n;
            }
        }
    }
...
int ext2x_lookup(opal_list_t *data, opal_list_t *info)
{
    ...
    if (PMIX_SUCCESS == ret) {
        ...
        OPAL_LIST_FOREACH(d, data, opal_pmix_pdata_t) {
            ...
            rc = ext2x_value_unload(&d->value, pdata[n].key, &pdata[n].value);
            if (OPAL_SUCCESS != rc) {
                OPAL_ERROR_LOG(rc);
                      ...
                      void *cbdata)
{
    ...
    if (OPAL_SUCCESS == rc) {
        ...
        for (n=0; n < ndata; n++) {
            ...
            rc = ext2x_value_unload(&d->value, data[n].key, &data[n].value);
            if (OPAL_SUCCESS != rc) {
...
int ext2x_spawn(opal_list_t *job_info, opal_list_t *apps, opal_jobid_t *jobid)
{
    ...
    if (NULL != job_info && 0 < (ninfo = opal_list_get_size(job_info))) {
        ...
        OPAL_LIST_FOREACH(info, job_info, opal_value_t) {
            ...
            ext2x_value_load(&pinfo[n].value, info);
            ++n;
        }
    }
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (0 < (papps[n].ninfo = opal_list_get_size(&app->info))) {
            ...
            OPAL_LIST_FOREACH(info, &app->info, opal_value_t) {
                ...
                ext2x_value_load(&papps[n].info[m].value, info);
                ++m;
            }
        }
        ++n;
...
int ext2x_resolve_peers(const char *nodename, opal_jobid_t jobid,
                        opal_list_t *procs)
{
    char *nspace;
