...
static void errreg_cbfunc (pmix_status_t status,
                           ...
                           void *cbdata)
{
    opal_ext4x_event_t *event = (opal_ext4x_event_t*)cbdata;
...
int ext4x_client_init(opal_list_t *ilist)
{
    ...
    if (NULL != ilist && 0 < (ninfo = opal_list_get_size(ilist))) {
        ...
        OPAL_LIST_FOREACH(ival, ilist, opal_value_t) {
            ...
            ext4x_value_load(&pinfo[n].value, ival);
            ++n;
        }
    } else {
        pinfo = NULL;
    ...
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
    rc = PMIx_Init(&mca_pmix_ext4x_component.myproc, pinfo, ninfo);
    if (NULL != pinfo) {
        PMIX_INFO_FREE(pinfo, ninfo);
    }
    if (PMIX_SUCCESS != rc) {
        dbg = ext4x_convert_rc(rc);
        OPAL_ERROR_LOG(dbg);
        return dbg;
    }
    OPAL_PMIX_ACQUIRE_THREAD(&opal_pmix_base.lock);
    ...
    if (NULL != getenv(OPAL_MCA_PREFIX"orte_launch")) {
        ...
        mca_pmix_ext4x_component.native_launch = true;
        opal_convert_string_to_jobid(&pname.jobid, mca_pmix_ext4x_component.myproc.nspace);
    } else {
        /* we were launched by someone else, so make the
         * jobid just be the hash of the nspace */
        OPAL_HASH_JOBID(mca_pmix_ext4x_component.myproc.nspace, pname.jobid);
    }
    /* insert this into our list of jobids - it will be the
     * first, and so we'll check it first */
    job = OBJ_NEW(opal_ext4x_jobid_trkr_t);
    (void)strncpy(job->nspace, mca_pmix_ext4x_component.myproc.nspace, PMIX_MAX_NSLEN);
    job->jobid = pname.jobid;
    opal_list_append(&mca_pmix_ext4x_component.jobids, &job->super);

    pname.vpid = ext4x_convert_rank(mca_pmix_ext4x_component.myproc.rank);
    ...
    PMIx_Register_event_handler(NULL, 0, NULL, 0, ext4x_event_hdlr, errreg_cbfunc, event);
...
int ext4x_client_finalize(void)
{
    pmix_status_t rc;
...
int ext4x_tool_init(opal_list_t *info)
{
    pmix_info_t *pinfo;
    size_t ninfo, n;
    opal_ext4x_jobid_trkr_t *job;
    ...
    if (NULL != info && 0 < (ninfo = opal_list_get_size(info))) {
        ...
        OPAL_LIST_FOREACH(val, info, opal_value_t) {
            ...
            ext4x_value_load(&pinfo[n].value, val);
            ++n;
            ...
            if (0 == strcmp(val->key, OPAL_PMIX_TOOL_NSPACE)) {
                ...
                (void)strncpy(mca_pmix_ext4x_component.myproc.nspace, val->data.string, PMIX_MAX_NSLEN);
            } else if (0 == strcmp(val->key, OPAL_PMIX_TOOL_RANK)) {
                pname.vpid = val->data.name.vpid;
                mca_pmix_ext4x_component.myproc.rank = pname.vpid;
            }
        }
    } else {
        pinfo = NULL;
    ...
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
    rc = PMIx_tool_init(&mca_pmix_ext4x_component.myproc, pinfo, ninfo);
    if (NULL != pinfo) {
        PMIX_INFO_FREE(pinfo, ninfo);
    }
    if (PMIX_SUCCESS != rc) {
...
int ext4x_abort(int flag, const char *msg,
                 opal_list_t *procs)
{
    pmix_status_t rc;
    ...
    if (NULL != procs && 0 < (cnt = opal_list_get_size(procs))) {
        ...
        OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
            if (NULL == (nsptr = ext4x_convert_jobid(ptr->name.jobid))) {
                PMIX_PROC_FREE(parray, cnt);
                return OPAL_ERR_NOT_FOUND;
            }
            (void)strncpy(parray[n].nspace, nsptr, PMIX_MAX_NSLEN);
            parray[n].rank = ext4x_convert_opalrank(ptr->name.vpid);
            ++n;
        }
    }
...
int ext4x_store_local(const opal_process_name_t *proc, opal_value_t *val)
{
    pmix_value_t kv;
    pmix_status_t rc;
    pmix_proc_t p;
    ...
    if (NULL != proc) {
        if (NULL == (nsptr = ext4x_convert_jobid(proc->jobid))) {
            job = OBJ_NEW(opal_ext4x_jobid_trkr_t);
        ...
        /* use our name */
        (void)strncpy(p.nspace, mca_pmix_ext4x_component.myproc.nspace, PMIX_MAX_NSLEN);
        p.rank = ext4x_convert_opalrank(OPAL_PROC_MY_NAME.vpid);
    }

    PMIX_VALUE_CONSTRUCT(&kv);
    ext4x_value_load(&kv, val);
...
int ext4x_fence(opal_list_t *procs, int collect_data)
{
    pmix_status_t rc;
    opal_namelist_t *ptr;
    char *nsptr;
...
int ext4x_fencenb(opal_list_t *procs, int collect_data,
...
int ext4x_put(opal_pmix_scope_t opal_scope,
               opal_value_t *val)
{
    pmix_value_t kv;
    pmix_scope_t pmix_scope = ext4x_convert_opalscope(opal_scope);
    pmix_status_t rc;
...
int ext4x_get(const opal_process_name_t *proc, const char *key,
               opal_list_t *info, opal_value_t **val)
{
    ...
    opal_output_verbose(1, opal_pmix_base_framework.framework_output,
                        "%s ext4x:client get on proc %s key %s",
    ...
    if (NULL == proc) {
        ...
        if (0 == strcmp(key, OPAL_PMIX_RANK)) {
            ...
            (*val)->key = strdup(key);
            (*val)->type = OPAL_INT;
            (*val)->data.integer = ext4x_convert_rank(mca_pmix_ext4x_component.myproc.rank);
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_SUCCESS;
        }
    }
    *val = NULL;
    ...
    if (NULL == proc) {
        (void)strncpy(p.nspace, mca_pmix_ext4x_component.myproc.nspace, PMIX_MAX_NSLEN);
        p.rank = ext4x_convert_rank(PMIX_RANK_WILDCARD);
    } else {
        if (NULL == (nsptr = ext4x_convert_jobid(proc->jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
    ...
    if (PMIX_SUCCESS == rc) {
        ...
        if (OPAL_SUCCESS != (ret = ext4x_value_unload(ival, pval))) {
            rc = ext4x_convert_opalrc(ret);
        } else {
            *val = ival;
        }
...
int ext4x_getnb(const opal_process_name_t *proc, const char *key,
                 opal_list_t *info,
                 opal_pmix_value_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL == proc) {
        (void)strncpy(op->p.nspace, mca_pmix_ext4x_component.myproc.nspace, PMIX_MAX_NSLEN);
        op->p.rank = ext4x_convert_rank(PMIX_RANK_WILDCARD);
    } else {
        if (NULL == (nsptr = ext4x_convert_jobid(proc->jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
    ...
    if (NULL != info && 0 < (op->sz = opal_list_get_size(info))) {
        ...
        OPAL_LIST_FOREACH(val, info, opal_value_t) {
            ...
            ext4x_value_load(&op->info[n].value, val);
            ++n;
        }
    }
...
int ext4x_publish(opal_list_t *info)
{
    pmix_info_t *pinfo;
    pmix_status_t ret;
    opal_value_t *iptr;
    ...
    if (0 < sz) {
        ...
        OPAL_LIST_FOREACH(iptr, info, opal_value_t) {
            ...
            ext4x_value_load(&pinfo[n].value, iptr);
            ++n;
        }
    } else {
        pinfo = NULL;
    ...
    return ext4x_convert_rc(ret);
}

int ext4x_publishnb(opal_list_t *info,
                     opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    pmix_status_t ret;
    opal_value_t *iptr;
    ...
    if (0 < op->sz) {
        ...
        OPAL_LIST_FOREACH(iptr, info, opal_value_t) {
            ...
            ext4x_value_load(&op->info[n].value, iptr);
            ++n;
        }
    }
...
int ext4x_lookup(opal_list_t *data, opal_list_t *info)
{
    opal_pmix_pdata_t *d;
    pmix_pdata_t *pdata;
    pmix_info_t *pinfo = NULL;
    ...
    size_t cnt, n, sz = 0;
    opal_value_t *iptr;
    opal_ext4x_jobid_trkr_t *jptr, *job;
    int ret;
    ...
    if (PMIX_SUCCESS == rc) {
        ...
        OPAL_PMIX_ACQUIRE_THREAD(&opal_pmix_base.lock);
        OPAL_LIST_FOREACH(d, data, opal_pmix_pdata_t) {
            if (mca_pmix_ext4x_component.native_launch) {
                /* if we were launched by the OMPI RTE, then
                 * the jobid is in a special format - so get it */
            ...
            d->proc.vpid = ext4x_convert_rank(pdata[n].proc.rank);
            if (OPAL_SUCCESS != (ret = ext4x_value_unload(&d->value, &pdata[n].value))) {
                OPAL_ERROR_LOG(ret);
            }
        }
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
                      ...
                      void *cbdata)
{
    ...
    int rc;
    size_t n;
    opal_ext4x_jobid_trkr_t *job, *jptr;
    ...
    if (OPAL_SUCCESS == rc) {
        ...
        for (n=0; n < ndata; n++) {
            ...
            d->proc.vpid = ext4x_convert_rank(data[n].proc.rank);
            d->value.key = strdup(data[n].key);
            rc = ext4x_value_unload(&d->value, &data[n].value);
            if (OPAL_SUCCESS != rc) {
...
int ext4x_lookupnb(char **keys, opal_list_t *info,
...
int ext4x_unpublish(char **keys, opal_list_t *info)
{
    pmix_status_t ret;
    size_t ninfo, n;
    pmix_info_t *pinfo;
...
int ext4x_unpublishnb(char **keys, opal_list_t *info,
                       opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    pmix_status_t ret;
    ext4x_opcaddy_t *op;
...
int ext4x_spawn(opal_list_t *job_info, opal_list_t *apps, opal_jobid_t *jobid)
{
    pmix_status_t rc;
    ...
    *jobid = OPAL_JOBID_INVALID;
    ...
    if (NULL != job_info && 0 < (ninfo = opal_list_get_size(job_info))) {
        ...
        OPAL_LIST_FOREACH(ival, job_info, opal_value_t) {
            ...
            ext4x_value_load(&info[n].value, ival);
            ++n;
        }
    }
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (0 < (papps[n].ninfo = opal_list_get_size(&app->info))) {
            ...
            OPAL_LIST_FOREACH(ival, &app->info, opal_value_t) {
                ...
                ext4x_value_load(&papps[n].info[m].value, ival);
                ++m;
            }
        }
        ++n;
...
int ext4x_spawnnb(opal_list_t *job_info, opal_list_t *apps,
                   opal_pmix_spawn_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL != job_info && 0 < (op->ninfo = opal_list_get_size(job_info))) {
        ...
        OPAL_LIST_FOREACH(info, job_info, opal_value_t) {
            ...
            ext4x_value_load(&op->info[n].value, info);
            ++n;
        }
    }
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (0 < (op->apps[n].ninfo = opal_list_get_size(&app->info))) {
            ...
            OPAL_LIST_FOREACH(info, &app->info, opal_value_t) {
                ...
                ext4x_value_load(&op->apps[n].info[m].value, info);
                ++m;
            }
        }
        ++n;
...
int ext4x_connect(opal_list_t *procs)
{
    pmix_proc_t *p;
    size_t nprocs;
    opal_namelist_t *ptr;
    ...
    OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
        ...
        p[n].rank = ext4x_convert_opalrank(ptr->name.vpid);
        ++n;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
...
int ext4x_connectnb(opal_list_t *procs,
                     ...
                     void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
        ...
        op->procs[n].rank = ext4x_convert_opalrank(ptr->name.vpid);
        ++n;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
...
int ext4x_disconnect(opal_list_t *procs)
{
    size_t nprocs, n;
    opal_namelist_t *ptr;
...
int ext4x_disconnectnb(opal_list_t *procs,
                        opal_pmix_op_cbfunc_t cbfunc,
                        void *cbdata)
{
    ext4x_opcaddy_t *op;
...
int ext4x_resolve_peers(const char *nodename,
                         opal_jobid_t jobid,
                         opal_list_t *procs)
{
    pmix_status_t ret;
    ...
    if (OPAL_JOBID_WILDCARD != jobid) {
        if (NULL == (nspace = ext4x_convert_jobid(jobid))) {
            OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
            return OPAL_ERR_NOT_FOUND;
        }
    } else {
    ...
    if (NULL != array && 0 < nprocs) {
        ...
        for (n=0; n < nprocs; n++) {
            ...
            /* if we don't already have it, add this to our jobid tracker */
            if (NULL == ext4x_convert_jobid(nm->name.jobid)) {
                job = OBJ_NEW(opal_ext4x_jobid_trkr_t);
                (void)strncpy(job->nspace, array[n].nspace, PMIX_MAX_NSLEN);
                job->jobid = nm->name.jobid;
                opal_list_append(&mca_pmix_ext4x_component.jobids, &job->super);
            }
            nm->name.vpid = ext4x_convert_rank(array[n].rank);
        }
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
    }
...
int ext4x_resolve_nodes(opal_jobid_t jobid, char **nodelist)
{
    pmix_status_t ret;
    char *nsptr;
    ...
    if (NULL == (nsptr = ext4x_convert_jobid(jobid))) {
        OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
        return OPAL_ERR_NOT_FOUND;
    }
    OPAL_PMIX_RELEASE_THREAD(&opal_pmix_base.lock);
...
int ext4x_allocate(opal_pmix_alloc_directive_t directive,
                    opal_list_t *info,
...
int ext4x_job_control(opal_list_t *targets,
