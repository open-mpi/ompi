...
int ext20_client_init(void)
{
    ...
    if (NULL != getenv(OPAL_MCA_PREFIX"orte_launch")) {
        ...
        mca_pmix_ext20_component.native_launch = true;
    ...
    pname.vpid = ext20_convert_rank(my_proc.rank);
    opal_proc_set_name(&pname);

    /* register the default event handler */
    PMIx_Register_event_handler(NULL, 0, NULL, 0, ext20_event_hdlr, errreg_cbfunc, NULL);
    return OPAL_SUCCESS;

}

int ext20_client_finalize(void)
{
    pmix_status_t rc;
...
int ext20_abort(int flag, const char *msg,
                opal_list_t *procs)
{
    pmix_status_t rc;
    ...
    size_t n, cnt=0;
    opal_namelist_t *ptr;
    opal_ext20_jobid_trkr_t *job, *jptr;
    ...
    if (NULL != procs && 0 < (cnt = opal_list_get_size(procs))) {
        ...
        OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
            ...
            parray[n].rank = ext20_convert_opalrank(ptr->name.vpid);
            ++n;
        }
    }
...
int ext20_store_local(const opal_process_name_t *proc, opal_value_t *val)
{
    pmix_value_t kv;
    pmix_status_t rc;
    pmix_proc_t p;
    ...
    if (NULL != proc) {
        ...
        p.rank = ext20_convert_opalrank(OPAL_PROC_MY_NAME.vpid);
    }

    PMIX_VALUE_CONSTRUCT(&kv);
    ext20_value_load(&kv, val);
...
int ext20_fence(opal_list_t *procs, int collect_data)
{
    pmix_status_t rc;
    pmix_proc_t *parray=NULL;
    size_t n, cnt=0;
...
int ext20_fencenb(opal_list_t *procs, int collect_data,
...
int ext20_put(opal_pmix_scope_t opal_scope,
              opal_value_t *val)
{
    pmix_value_t kv;
    pmix_scope_t pmix_scope = ext20_convert_opalscope(opal_scope);
    pmix_status_t rc;
...
int ext20_get(const opal_process_name_t *proc, const char *key,
    ...
    *val = NULL;
    if (NULL != proc) {
        ...
        if (0 == strcmp(key, OPAL_PMIX_JOBID)) {
            ...
            (*val) = OBJ_NEW(opal_value_t);
            (*val)->type = OPAL_INT;
            (*val)->data.integer = ext20_convert_rank(my_proc.rank);
            return OPAL_SUCCESS;
        }
        pptr = NULL;
    }
    ...
    if (NULL != info) {
        ...
        if (0 < ninfo) {
            ...
            OPAL_LIST_FOREACH(ival, info, opal_value_t) {
                ...
                ext20_value_load(&pinfo[n].value, ival);
                ++n;
            }
        } else {
            pinfo = NULL;
    ...
    if (PMIX_SUCCESS == rc) {
        if (NULL == kv) {
            ...
            *val = OBJ_NEW(opal_value_t);
            ret = ext20_value_unload(*val, kv);
            PMIX_VALUE_FREE(kv, 1);
        }
    } else {
        ret = ext20_convert_rc(rc);
    }
...
int ext20_getnb(const opal_process_name_t *proc, const char *key,
                opal_list_t *info,
                opal_pmix_value_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL != proc) {
        ...
        op->p.rank = ext20_convert_rank(PMIX_RANK_WILDCARD);
    }
    ...
    if (NULL != info) {
        ...
        if (0 < op->sz) {
            ...
            OPAL_LIST_FOREACH(ival, info, opal_value_t) {
                ...
                ext20_value_load(&op->info[n].value, ival);
                ++n;
            }
        }
    }
...
int ext20_publish(opal_list_t *info)
{
    pmix_info_t *pinfo;
    pmix_status_t ret;
    opal_value_t *iptr;
    ...
    if (0 < sz) {
        ...
        OPAL_LIST_FOREACH(iptr, info, opal_value_t) {
            ...
            ext20_value_load(&pinfo[n].value, iptr);
            ++n;
        }
    } else {
        pinfo = NULL;
    ...
    return ext20_convert_rc(ret);
}

int ext20_publishnb(opal_list_t *info,
                    opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    pmix_status_t ret;
    opal_value_t *iptr;
    ...
    if (0 < op->sz) {
        ...
        OPAL_LIST_FOREACH(iptr, info, opal_value_t) {
            ...
            ext20_value_load(&op->info[n].value, iptr);
            ++n;
        }
    }
...
int ext20_lookup(opal_list_t *data, opal_list_t *info)
{
    pmix_pdata_t *pdata;
    pmix_info_t *pinfo;
    size_t sz, ninfo, n;
    ...
    if (PMIX_SUCCESS == ret) {
        ...
        OPAL_LIST_FOREACH(d, data, opal_pmix_pdata_t) {
            ...
            d->proc.vpid = ext20_convert_rank(pdata[n].proc.rank);
            rc = ext20_value_unload(&d->value, &pdata[n].value);
            if (OPAL_SUCCESS != rc) {
                OPAL_ERROR_LOG(rc);
                      ...
                      void *cbdata)
{
    ...
    if (OPAL_SUCCESS == rc) {
        ...
        for (n=0; n < ndata; n++) {
            ...
            d->proc.vpid = ext20_convert_rank(data[n].proc.rank);
            d->value.key = strdup(data[n].key);
            rc = ext20_value_unload(&d->value, &data[n].value);
            if (OPAL_SUCCESS != rc) {
...
int ext20_lookupnb(char **keys, opal_list_t *info,
...
int ext20_unpublish(char **keys, opal_list_t *info)
{
    pmix_status_t ret;
    size_t ninfo, n;
    pmix_info_t *pinfo;
...
int ext20_unpublishnb(char **keys, opal_list_t *info,
                      opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    pmix_status_t ret;
    ext20_opcaddy_t *op;
...
int ext20_spawn(opal_list_t *job_info, opal_list_t *apps, opal_jobid_t *jobid)
{
    pmix_status_t ret;
    ...
    if (NULL != job_info && 0 < (ninfo = opal_list_get_size(job_info))) {
        ...
        OPAL_LIST_FOREACH(info, job_info, opal_value_t) {
            ...
            ext20_value_load(&pinfo[n].value, info);
            ++n;
        }
    }
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (0 < (papps[n].ninfo = opal_list_get_size(&app->info))) {
            ...
            OPAL_LIST_FOREACH(info, &app->info, opal_value_t) {
                ...
                ext20_value_load(&papps[n].info[m].value, info);
                ++m;
            }
        }
        ++n;
...
int ext20_spawnnb(opal_list_t *job_info, opal_list_t *apps,
                  opal_pmix_spawn_cbfunc_t cbfunc, void *cbdata)
{
    ...
    if (NULL != job_info && 0 < (op->ninfo = opal_list_get_size(job_info))) {
        ...
        OPAL_LIST_FOREACH(info, job_info, opal_value_t) {
            ...
            ext20_value_load(&op->info[n].value, info);
            ++n;
        }
    }
    ...
    OPAL_LIST_FOREACH(app, apps, opal_pmix_app_t) {
        ...
        if (0 < (op->apps[n].ninfo = opal_list_get_size(&app->info))) {
            ...
            OPAL_LIST_FOREACH(info, &app->info, opal_value_t) {
                ...
                ext20_value_load(&op->apps[n].info[m].value, info);
                ++m;
            }
        }
        ++n;
...
int ext20_connect(opal_list_t *procs)
{
    pmix_status_t ret;
    pmix_proc_t *parray=NULL;
    size_t n, cnt=0;
...
int ext20_connectnb(opal_list_t *procs,
                    ...
                    void *cbdata)
{
    ...
    OPAL_LIST_FOREACH(ptr, procs, opal_namelist_t) {
        /* look thru our list of jobids and find the
         * corresponding nspace */
        OPAL_LIST_FOREACH(job, &mca_pmix_ext20_component.jobids, opal_ext20_jobid_trkr_t) {
            if (job->jobid == ptr->name.jobid) {
                (void)strncpy(op->procs[n].nspace, job->nspace, PMIX_MAX_NSLEN);
                break;
            }
        }
        op->procs[n].rank = ext20_convert_opalrank(ptr->name.vpid);
        ++n;
    }
...
int ext20_disconnect(opal_list_t *procs)
{
    pmix_status_t ret;
    pmix_proc_t *parray=NULL;
    size_t n, cnt=0;
...
int ext20_disconnectnb(opal_list_t *procs,
                       opal_pmix_op_cbfunc_t cbfunc,
                       void *cbdata)
{
    pmix_status_t ret;
...
int ext20_resolve_peers(const char *nodename, opal_jobid_t jobid,
                        opal_list_t *procs)
{
    char *nspace;
    ...
    if (NULL != array && 0 < nprocs) {
        for (n=0; n < nprocs; n++) {
            ...
            nm->name.vpid = ext20_convert_rank(array[n].rank);
        }
    }
