                                 ...
                                 void *cbdata)
{
    ...
    job->jobid = jobid;
    opal_list_append(&mca_pmix_pmix111_component.jobids, &job->super);
...
void pmix1_server_deregister_nspace(opal_jobid_t jobid)
{
    ...
    OPAL_LIST_FOREACH(jptr, &mca_pmix_pmix111_component.jobids, opal_pmix1_jobid_trkr_t) {
        if (jptr->jobid == jobid) {
            /* found it - tell the server to deregister */
            PMIx_server_deregister_nspace(jptr->nspace);
            /* now get rid of it from our list */
            opal_list_remove_item(&mca_pmix_pmix111_component.jobids, &jptr->super);
            OBJ_RELEASE(jptr);
            return;
        }
    }
