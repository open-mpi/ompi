...
int opal_util_init_sys_limits(char **errmsg)
{
    ...
    for (i = 0; NULL != lims[i]; i++) {
        ...
        if (0 == strcmp(lim[0], "1")) {
            ...
            if (OPAL_SUCCESS != opal_setlimit(RLIMIT_NOFILE, "max", &value)) {
                *errmsg = opal_show_help_string("help-opal-util.txt", "sys-limit-failed", true,
                                                "openfiles", "max");
                goto out;
            }
            ...
            if (OPAL_SUCCESS != opal_setlimit(RLIMIT_FSIZE, "max", &value)) {
                *errmsg = opal_show_help_string("help-opal-util.txt", "sys-limit-failed", true,
                                                "filesize", "max");
                goto out;
            }
        ...
        if (0 == strcmp(lim[0], "core")) {
            ...
            *errmsg = opal_show_help_string("help-opal-util.txt", "sys-limit-unrecognized", true,
                                            lim[0], setlim);
            goto out;
        }
        opal_argv_free(lim);
