...
static void set_stacktrace_filename(void)
{
    ...
    if (NULL == my_proc) {
        snprintf(opal_stacktrace_output_filename, opal_stacktrace_output_filename_max_len, "%s.%lu",
        ...
        snprintf(opal_stacktrace_output_filename, opal_stacktrace_output_filename_max_len,
                 "%s.%lu.%lu", opal_stacktrace_output_filename_base,
                 (unsigned long) my_proc->proc_name.vpid, (unsigned long) getpid());
    }
...
#    endif
        ...
        if (0 != info->si_errno) {
            ret = snprintf(tmp, size, HOSTFORMAT "Associated errno: %s (%d)\n", stacktrace_hostname,
                           getpid(), strerror(info->si_errno), info->si_errno);
            size -= ret;
            tmp += ret;
        }
        ...
        case SIGCHLD: {
            ret = snprintf(tmp, size, HOSTFORMAT "Sending PID: %d, Sending UID: %d, Status: %d\n",
                           stacktrace_hostname, getpid(), info->si_pid, info->si_uid,
                           info->si_status);
            size -= ret;
            tmp += ret;
            break;
...
int opal_util_register_stackhandlers(void)
{
    ...
    if (NULL == opal_stacktrace_output_filename
        ...
        opal_stacktrace_output_filename = (char *) malloc(
        ...
        free(opal_stacktrace_output_filename);
        // Magic number: 8 = space for .PID and .RANK (allow 7 digits each)
        opal_stacktrace_output_filename_max_len = strlen(opal_stacktrace_output_filename_base) + 8
                                                  + 8;
        opal_stacktrace_output_filename = (char *) malloc(
            sizeof(char) * opal_stacktrace_output_filename_max_len);
        set_stacktrace_filename();
...
#    endif
    ...
    for (tmp = next = opal_signal_string; next != NULL && *next != '\0'; tmp = next + 1) {
        int sig;
        int ret;
        ...
        if (((0 == sig) && (tmp == next)) || (0 > sig) || (_NSIG <= sig)) {
            opal_show_help("help-opal-util.txt", "stacktrace bad signal", true, opal_signal_string,
                           tmp);
        ...
        if (SIG_IGN != old.sa_handler && SIG_DFL != old.sa_handler) {
            if (!showed_help && complain) {
                ...
                opal_show_help("help-opal-util.txt", "stacktrace signal override", true, sig, sig,
                               sig, opal_signal_string);
                showed_help = true;
            }
        }
