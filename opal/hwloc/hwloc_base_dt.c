                      ...
                      opal_data_type_t type)
{
    ...
    for (i=0, j=0; i < *num_vals; i++) {
        ...
        if (OPAL_SUCCESS != (rc = opal_hwloc_filter_cpus(t))) {
            goto cleanup;
        }
...
{
    /* we choose not to support the copy function as it isn't
     * actually used anywhere in the code base, and including
     * it would mandate hwloc v1.8 or above - which would
     * exclude some supported OS versions */
