 ...
 * well as to invoke opal_hwloc_set_process_membind_policy()
 ...
 */
OPAL_DECLSPEC extern bool opal_hwloc_inited;
OPAL_DECLSPEC extern bool opal_hwloc_topology_inited;
OPAL_DECLSPEC extern int opal_hwloc_verbose;
OPAL_DECLSPEC extern int opal_hwloc_output;
...
OPAL_DECLSPEC char* opal_hwloc_print_locality(opal_hwloc_locality_t locality);

OPAL_DECLSPEC extern char *opal_hwloc_cpu_list;
OPAL_DECLSPEC extern hwloc_cpuset_t opal_hwloc_given_cpus;
OPAL_DECLSPEC extern char *opal_hwloc_topo_file;
...
#define OPAL_HWLOC_SHOW_BINDING(n, v, t)                                \
    do {                                                                \
        ...
        if (hwloc_get_cpubind(t, bind,                                  \
            ...
            opal_hwloc_cset2mapstr(tmp1, sizeof(tmp1), t, bind);        \
            opal_output_verbose(n, v,                                   \
                                "BINDINGS AT %s:%d: %s",                \
...
OPAL_DECLSPEC opal_hwloc_locality_t opal_hwloc_get_relative_locality(hwloc_topology_t topo,
                                                                          char *cpuset1, char *cpuset2);

OPAL_DECLSPEC int opal_hwloc_set_binding_policy(opal_binding_policy_t *policy, char *spec);
 ...
 * cpuset.
 */
OPAL_DECLSPEC void opal_hwloc_get_local_cpuset(void);
...
typedef enum {
    OPAL_HWLOC_MAP_NONE,
    OPAL_HWLOC_MAP_LOCAL_ONLY
} opal_hwloc_map_t;

/**
 * Global reflecting the MAP (set by MCA param).
 */
OPAL_DECLSPEC extern opal_hwloc_map_t opal_hwloc_map;
 ...
 */
typedef enum {
    OPAL_HWLOC_MBFA_SILENT,
    OPAL_HWLOC_MBFA_WARN,
    OPAL_HWLOC_MBFA_ERROR
} opal_hwloc_mbfa_t;

/**
 * Global reflecting the BFA (set by MCA param).
 */
OPAL_DECLSPEC extern opal_hwloc_mbfa_t opal_hwloc_mbfa;

/* some critical helper functions */
OPAL_DECLSPEC int opal_hwloc_filter_cpus(hwloc_topology_t topo);

/**
 * Discover / load the hwloc topology (i.e., call hwloc_topology_init() and
 * hwloc_topology_load()).
 */
OPAL_DECLSPEC int opal_hwloc_get_topology(void);

/**
 * Set the hwloc topology to that from the given topo file
 */
OPAL_DECLSPEC int opal_hwloc_set_topology(char *topofile);

/**
 * Free the hwloc topology.
 */
OPAL_DECLSPEC void opal_hwloc_free_topology(hwloc_topology_t topo);
OPAL_DECLSPEC hwloc_cpuset_t opal_hwloc_get_available_cpus(hwloc_topology_t topo,
                                                                hwloc_obj_t obj);
OPAL_DECLSPEC unsigned int opal_hwloc_get_nbobjs_by_type(hwloc_topology_t topo,
                                                         hwloc_obj_type_t target,
                                                         unsigned cache_level,
                                                         opal_hwloc_resource_type_t rtype);
OPAL_DECLSPEC hwloc_obj_t opal_hwloc_find_min_bound_target_under_obj(hwloc_topology_t topo,
                                                                     hwloc_obj_t obj,
                                                                     hwloc_obj_type_t target,
                                                                     unsigned cache_leve);
OPAL_DECLSPEC void opal_hwloc_clear_usage(hwloc_topology_t topo);

OPAL_DECLSPEC hwloc_obj_t opal_hwloc_get_obj_by_type(hwloc_topology_t topo,
                                                     hwloc_obj_type_t target,
                                                     unsigned cache_level,
                                                     unsigned int instance,
                                                     opal_hwloc_resource_type_t rtype);
OPAL_DECLSPEC unsigned int opal_hwloc_get_obj_idx(hwloc_topology_t topo,
                                                  hwloc_obj_t obj,
                                                  opal_hwloc_resource_type_t rtype);
 ...
 */
OPAL_DECLSPEC unsigned int opal_hwloc_get_npus(hwloc_topology_t topo,
                                                    hwloc_obj_t target);
OPAL_DECLSPEC char* opal_hwloc_print_binding(opal_binding_policy_t binding);

/**
 * Determine if there is a single cpu in a bitmap.
 */
OPAL_DECLSPEC bool opal_hwloc_single_cpu(hwloc_cpuset_t cpuset);
 ...
 */
OPAL_DECLSPEC int opal_hwloc_cpu_list_parse(const char *slot_str,
                                            hwloc_topology_t topo,
                                            opal_hwloc_resource_type_t rtype,
                                            hwloc_cpuset_t cpumask);

OPAL_DECLSPEC char* opal_hwloc_find_coprocessors(hwloc_topology_t topo);
OPAL_DECLSPEC char* opal_hwloc_check_on_coprocessor(void);


/**
 * Report a bind failure using the normal mechanisms if a component
 * fails to bind memory -- according to the value of the
 * hwloc_bind_failure_action MCA parameter.
 */
OPAL_DECLSPEC int opal_hwloc_report_bind_failure(const char *file,
                                                 int line,
                                                 const char *msg,
                                                 int rc);
 ...
 * according to opal_hwloc_map and opal_hwloc_mbfa.  It needs
 * to be a separate, standalone function (as opposed to being done
 * during opal_hwloc_open()) because opal_hwloc_topology is not
 * loaded by opal_hwloc_open().  Hence, an upper layer needs to
 * invoke this function after opal_hwloc_topology has been loaded.
 */
OPAL_DECLSPEC int opal_hwloc_set_process_membind_policy(void);

OPAL_DECLSPEC int opal_hwloc_membind(opal_hwloc_memory_segment_t *segs,
                                     size_t count, int node_id);

OPAL_DECLSPEC int opal_hwloc_node_name_to_id(char *node_name, int *id);

OPAL_DECLSPEC int opal_hwloc_memory_set(opal_hwloc_memory_segment_t *segments,
                                        size_t num_segments);
 ...
 * 2[core 3]").
 */
OPAL_DECLSPEC int opal_hwloc_cset2str(char *str, int len,
                                      hwloc_topology_t topo,
 ...
 */
OPAL_DECLSPEC int opal_hwloc_cset2mapstr(char *str, int len,
                                              hwloc_topology_t topo,
                                              hwloc_cpuset_t cpuset);

/* get the hwloc object that corresponds to the given processor id  and type */
OPAL_DECLSPEC hwloc_obj_t opal_hwloc_get_pu(hwloc_topology_t topo,
                                            int lid,
                                            opal_hwloc_resource_type_t rtype);

/* get the topology "signature" so we can check for differences - caller
 * if responsible for freeing the returned string */
OPAL_DECLSPEC char* opal_hwloc_get_topo_signature(hwloc_topology_t topo);


/* get a string describing the locality of a given process */
OPAL_DECLSPEC char* opal_hwloc_get_locality_string(hwloc_topology_t topo, char *bitmap);

/* extract a location from the locality string */
OPAL_DECLSPEC char* opal_hwloc_get_location(char *locality,
