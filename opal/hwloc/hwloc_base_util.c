 ...
 */
hwloc_obj_t opal_hwloc_get_pu(hwloc_topology_t topo,
                                   int lid,
                                   opal_hwloc_resource_type_t rtype)
{
    ...
    if (OPAL_HWLOC_PHYSICAL == rtype) {
        ...
        OPAL_OUTPUT_VERBOSE((5, opal_hwloc_output,
                             ...
                             lid, (NULL == obj) ? "not" : "is",
                             (NULL == opal_hwloc_cpu_list) ? "None" : opal_hwloc_cpu_list));
 ...
 */
int opal_hwloc_filter_cpus(hwloc_topology_t topo)
{
    hwloc_obj_t root, pu;
    ...
    if (NULL == opal_hwloc_cpu_list) {
        ...
        ranges = opal_argv_split(opal_hwloc_cpu_list, ',');
        avail = hwloc_bitmap_alloc();
        ...
        for (idx=0; idx < opal_argv_count(ranges); idx++) {
            ...
            case 1:
                ...
                if (NULL != (pu = opal_hwloc_get_pu(topo, cpu, OPAL_HWLOC_LOGICAL))) {
...
static void fill_cache_line_size(void)
{
    ...
    while (cache_level > 0 && !found) {
        i=0;
        while (1) {
            obj = opal_hwloc_get_obj_by_type(opal_hwloc_topology,
...
int opal_hwloc_get_topology(void)
{
    int rc;
    opal_process_name_t wildcard_rank;
    char *val = NULL;

    OPAL_OUTPUT_VERBOSE((2, opal_hwloc_output,
    ...
    if (NULL != opal_pmix.get) {
        ...
        opal_output_verbose(1, opal_hwloc_output,
    ...
    if (OPAL_SUCCESS == rc && NULL != val) {
        ...
        if (OPAL_SUCCESS != (rc = opal_hwloc_filter_cpus(opal_hwloc_topology))) {
            hwloc_topology_destroy(opal_hwloc_topology);
            return rc;
        }
    } else if (NULL == opal_hwloc_topo_file) {
        if (0 != hwloc_topology_init(&opal_hwloc_topology) ||
        ...
        if (OPAL_SUCCESS != (rc = opal_hwloc_set_topology(opal_hwloc_topo_file))) {
            return rc;
        }
    }
     ...
     */
    opal_hwloc_get_local_cpuset();

    return OPAL_SUCCESS;
}

int opal_hwloc_set_topology(char *topofile)
{
   ...
   if (NULL != opal_hwloc_topology) {
    ...
    /* filter the cpus thru any default cpu set */
    rc = opal_hwloc_filter_cpus(opal_hwloc_topology);
    if (OPAL_SUCCESS != rc) {
        return rc;
    }
...
void opal_hwloc_free_topology(hwloc_topology_t topo)
{
    hwloc_obj_t obj;
...
void opal_hwloc_get_local_cpuset(void)
{
    hwloc_obj_t root;
    ...
    if (NULL != opal_hwloc_topology) {
        ...
        if (hwloc_get_cpubind(opal_hwloc_topology,
            ...
            base_cpus = opal_hwloc_get_available_cpus(opal_hwloc_topology, root);
            hwloc_bitmap_copy(opal_hwloc_my_cpuset, base_cpus);
        }
    }
}

int opal_hwloc_report_bind_failure(const char *file,
                                        int line,
...
hwloc_cpuset_t opal_hwloc_get_available_cpus(hwloc_topology_t topo,
                                                  hwloc_obj_t obj)
{
    hwloc_obj_t root;
    ...
    OPAL_OUTPUT_VERBOSE((10, opal_hwloc_output,
...
bool opal_hwloc_single_cpu(hwloc_cpuset_t cpuset)
{
    int i;
    bool one=false;
...
unsigned int opal_hwloc_get_npus(hwloc_topology_t topo,
                                      hwloc_obj_t obj)
{
    opal_hwloc_obj_data_t *data;
    unsigned int cnt = 0;
    ...
    if (NULL == data || !data->npus_calculated) {
        if (!opal_hwloc_use_hwthreads_as_cpus) {
             ...
             */
            if (NULL == (cpuset = opal_hwloc_get_available_cpus(topo, obj))) {
                return 0;
            }
            /* count the number of bits that are set - there is
             * one bit for each available pu. We could just
...
unsigned int opal_hwloc_get_obj_idx(hwloc_topology_t topo,
                                         hwloc_obj_t obj,
                                         opal_hwloc_resource_type_t rtype)
{
    ...
    nobjs = opal_hwloc_get_nbobjs_by_type(topo, obj->type, cache_level, rtype);
    ...
    /* find this object */
    for (i=0; i < nobjs; i++) {
        ptr = opal_hwloc_get_obj_by_type(topo, obj->type, cache_level, i, rtype);
        if (ptr == obj) {
            data->idx = i;
            return i;
        }
                             ...
                             unsigned int *num_objs)
{
    ...
    if (target == start->type) {
        ...
        if (OPAL_HWLOC_AVAILABLE == rtype) {
            ...
            if (NULL == data->available) {
                data->available = opal_hwloc_get_available_cpus(topo, start);
            }
...
unsigned int opal_hwloc_get_nbobjs_by_type(hwloc_topology_t topo,
                                                hwloc_obj_type_t target,
                                                unsigned cache_level,
                                                opal_hwloc_resource_type_t rtype)
{
                                       ...
                                       unsigned int *min_bound)
{
    ...
    if (target == start->type) {
        ...
        if (0 == (k = opal_hwloc_get_npus(topo, start))) {
            goto notfound;
        }
        if (HWLOC_OBJ_CACHE == start->type && cache_level != start->attr->cache.depth) {
            goto notfound;
...
hwloc_obj_t opal_hwloc_find_min_bound_target_under_obj(hwloc_topology_t topo,
                                                            hwloc_obj_t obj,
                                                            hwloc_obj_type_t target,
                                                            unsigned cache_level)
{
 ...
 */
hwloc_obj_t opal_hwloc_get_obj_by_type(hwloc_topology_t topo,
                                            hwloc_obj_type_t target,
                                            unsigned cache_level,
...
void opal_hwloc_clear_usage(hwloc_topology_t topo)
{
    hwloc_obj_t root;
    unsigned k;
                             ...
                             hwloc_bitmap_t cpumask)
{
    ...
    if ('*' == cpus[0]) {
         ...
         */
        res = opal_hwloc_get_available_cpus(topo, obj);
    ...
    case 1:  /* no range was present, so just one socket given */
        ...
        obj = opal_hwloc_get_obj_by_type(topo, HWLOC_OBJ_SOCKET, 0, socket_id, rtype);
                                  ...
                                  hwloc_bitmap_t cpumask)
{
    ...
    if (NULL == (socket = opal_hwloc_get_obj_by_type(topo, HWLOC_OBJ_SOCKET, 0,
                                                          socket_id, rtype))) {
    ...
    for (i=1; NULL != socket_core[i]; i++) {
        ...
        if ('*' == corestr[0]) {
            ...
            res = opal_hwloc_get_available_cpus(topo, socket);
            hwloc_bitmap_or(cpumask, cpumask, res);
            ...
            case 1:  /* only one core, or a list of cores, specified */
                ...
                for (j=0; NULL != list[j]; j++) {
                    ...
                    /* get the cpus */
                    res = opal_hwloc_get_available_cpus(topo, core);
                    hwloc_bitmap_or(cpumask, cpumask, res);
                }
                opal_argv_free(list);
                break;
...
int opal_hwloc_cpu_list_parse(const char *slot_str,
                                    hwloc_topology_t topo,
                                    opal_hwloc_resource_type_t rtype,
            ...
            for (k=0; NULL != rngs[k]; k++) {
                ...
                case 1:  /* only one core, or a list of cores, specified */
                    ...
                    for (j=0; NULL != list[j]; j++) {
                        ...
                        if (NULL == (pu = opal_hwloc_get_pu(topo, core_id, rtype))) {
                            opal_argv_free(range);
                            opal_argv_free(item);
                            opal_argv_free(rngs);
                            opal_argv_free(list);
                        ...
                        pucpus = opal_hwloc_get_available_cpus(topo, pu);
...
opal_hwloc_locality_t opal_hwloc_get_relative_locality(hwloc_topology_t topo,
                                                            char *cpuset1, char *cpuset2)
{
    ...
    for (d=1; d < depth; d++) {
        ...
        for (w=0; w < width; w++) {
            ...
            avail = opal_hwloc_get_available_cpus(topo, obj);
 ...
 */
char* opal_hwloc_find_coprocessors(hwloc_topology_t topo)
{
    hwloc_obj_t osdev;
    unsigned i;
    char **cps = NULL;
 ...
 */
char* opal_hwloc_check_on_coprocessor(void)
{
...
char* opal_hwloc_print_binding(opal_binding_policy_t binding)
{
    char *ret, *bind;
 ...
 */
int opal_hwloc_cset2str(char *str, int len,
                             hwloc_topology_t topo,
                             hwloc_cpuset_t cpuset)
{
    bool first;
 ...
 */
int opal_hwloc_cset2mapstr(char *str, int len,
                                hwloc_topology_t topo,
                                hwloc_cpuset_t cpuset)
{
    char tmp[BUFSIZ];
...
int opal_hwloc_get_sorted_numa_list(hwloc_topology_t topo, char* device_name, opal_list_t *sorted_list)
{
    ...
    /* we call opal_hwloc_get_nbobjs_by_type() before it to fill summary object so it should exist*/
    data = (opal_hwloc_topo_data_t*)obj->userdata;
    if (NULL != data) {
        for (item = opal_list_get_first(&data->summaries);
                item != opal_list_get_end(&data->summaries);
...
char* opal_hwloc_get_topo_signature(hwloc_topology_t topo)
{
    ...
    nnuma = opal_hwloc_get_nbobjs_by_type(topo, HWLOC_OBJ_NODE, 0, OPAL_HWLOC_AVAILABLE);
    nsocket = opal_hwloc_get_nbobjs_by_type(topo, HWLOC_OBJ_SOCKET, 0, OPAL_HWLOC_AVAILABLE);
    nl3 = opal_hwloc_get_nbobjs_by_type(topo, HWLOC_OBJ_CACHE, 3, OPAL_HWLOC_AVAILABLE);
    nl2 = opal_hwloc_get_nbobjs_by_type(topo, HWLOC_OBJ_CACHE, 2, OPAL_HWLOC_AVAILABLE);
    nl1 = opal_hwloc_get_nbobjs_by_type(topo, HWLOC_OBJ_CACHE, 1, OPAL_HWLOC_AVAILABLE);
    ncore = opal_hwloc_get_nbobjs_by_type(topo, HWLOC_OBJ_CORE, 0, OPAL_HWLOC_AVAILABLE);
    nhwt = opal_hwloc_get_nbobjs_by_type(topo, HWLOC_OBJ_PU, 0, OPAL_HWLOC_AVAILABLE);
...
char* opal_hwloc_get_locality_string(hwloc_topology_t topo,
                                          char *bitmap)
{
    hwloc_obj_t obj;
...
char* opal_hwloc_get_location(char *locality,
                                   hwloc_obj_type_t type,
                                   unsigned index)
{
    char **loc;
