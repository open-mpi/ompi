 ...
 * displayed).
 */
int opal_hwloc_set_process_membind_policy(void)
{
    int rc = 0, flags;
    ...
    /* Set the default memory allocation policy according to MCA
       param */
    switch (opal_hwloc_map) {
    case OPAL_HWLOC_MAP_LOCAL_ONLY:
        policy = HWLOC_MEMBIND_BIND;
        flags = HWLOC_MEMBIND_STRICT;
        break;

    case OPAL_HWLOC_MAP_NONE:
    default:
    ...
    if (NULL == cpuset) {
        ...
        /* See if hwloc was able to do it.  If hwloc failed due to
           ...
           error. */
        if (0 != rc && ENOSYS == e &&
            OPAL_HWLOC_MAP_NONE == opal_hwloc_map) {
            rc = 0;
        }
    }
...
int opal_hwloc_memory_set(opal_hwloc_memory_segment_t *segments,
                          size_t num_segments)
{
    int rc = OPAL_SUCCESS;
    char *msg = NULL;
    ...
    if (OPAL_SUCCESS != opal_hwloc_get_topology()) {
        ...
        return opal_hwloc_report_bind_failure(__FILE__, __LINE__, msg, rc);
    }
...
int opal_hwloc_node_name_to_id(char *node_name, int *id)
{
    /* GLB: fix me */
    *id = atoi(node_name + 3);
...
int opal_hwloc_membind(opal_hwloc_memory_segment_t *segs,
