 ...
 * Globals
 */
bool opal_hwloc_inited = false;
hwloc_topology_t opal_hwloc_topology=NULL;
hwloc_cpuset_t opal_hwloc_my_cpuset=NULL;
hwloc_cpuset_t opal_hwloc_given_cpus=NULL;
opal_hwloc_map_t opal_hwloc_map = OPAL_HWLOC_MAP_NONE;
opal_hwloc_mbfa_t opal_hwloc_mbfa = OPAL_HWLOC_MBFA_WARN;
opal_binding_policy_t opal_hwloc_binding_policy=0;
char *opal_hwloc_cpu_list=NULL;
...
char *opal_hwloc_topo_file = NULL;
int opal_hwloc_output = -1;
int opal_hwloc_verbose = -1;

static mca_base_var_enum_value_t hwloc_map[] = {
    {OPAL_HWLOC_MAP_NONE, "none"},
    {OPAL_HWLOC_MAP_LOCAL_ONLY, "local_only"},
    {0, NULL}
};

static mca_base_var_enum_value_t hwloc_failure_action[] = {
    {OPAL_HWLOC_MBFA_SILENT, "silent"},
    {OPAL_HWLOC_MBFA_WARN, "warn"},
    {OPAL_HWLOC_MBFA_ERROR, "error"},
    {0, NULL}
};
...
int opal_hwloc_register(void)
{
    ...
    /* debug support */
    opal_hwloc_verbose = -1;
    (void) mca_base_var_register("opal", "hwloc", "base", NULL, "verbose",
                                 MCA_BASE_VAR_TYPE_INT, NULL, 0, 0, OPAL_INFO_LVL_9,
                                 MCA_BASE_VAR_SCOPE_READONLY, &opal_hwloc_verbose);
    ...
    opal_hwloc_map = OPAL_HWLOC_MAP_NONE;
    mca_base_var_enum_create("hwloc memory allocation policy", hwloc_map, &new_enum);
    ret = mca_base_var_register("opal", "hwloc", "base", "mem_alloc_policy",
                                ...
                                MCA_BASE_VAR_SCOPE_READONLY, &opal_hwloc_map);
    OBJ_RELEASE(new_enum);
    if (0 > ret) {
        return ret;
    }

    /* hwloc_bind_failure_action */
    opal_hwloc_mbfa = OPAL_HWLOC_MBFA_WARN;
    ...
    ret = mca_base_var_register("opal", "hwloc", "base", "mem_bind_failure_action",
                                "What Open MPI will do if it explicitly tries to bind memory to a specific NUMA location, and fails.  Note that this is a different case than the general allocation policy described by hwloc_alloc_policy.  A value of \"silent\" means that Open MPI will proceed without comment. A value of \"warn\" means that Open MPI will warn the first time this happens, but allow the job to continue (possibly with degraded performance).  A value of \"error\" means that Open MPI will abort the job if this happens.",
                                MCA_BASE_VAR_TYPE_INT, new_enum, 0, 0, OPAL_INFO_LVL_9,
                                MCA_BASE_VAR_SCOPE_READONLY, &opal_hwloc_mbfa);
    OBJ_RELEASE(new_enum);
    if (0 > ret) {
        return ret;
    }
    ...
    varid = mca_base_var_register("opal", "hwloc", "base", "cpu_list",
                                  ...
                                  MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0, OPAL_INFO_LVL_9,
                                  MCA_BASE_VAR_SCOPE_READONLY, &opal_hwloc_cpu_list);
    ...
    opal_hwloc_topo_file = NULL;
    (void) mca_base_var_register("opal", "hwloc", "base", "topo_file",
                                 "Read local topology from file instead of directly sensing it",
                                 MCA_BASE_VAR_TYPE_STRING, NULL, 0, 0, OPAL_INFO_LVL_9,
                                 MCA_BASE_VAR_SCOPE_READONLY, &opal_hwloc_topo_file);

    /* register parameters */
    return OPAL_SUCCESS;
}

int opal_hwloc_open(void)
{
    int rc;
    opal_data_type_t tmp;

    if (opal_hwloc_inited) {
        return OPAL_SUCCESS;
    }
    opal_hwloc_inited = true;

    if (0 < opal_hwloc_verbose) {
        /* set default output */
        opal_hwloc_output = opal_output_open(NULL);
        opal_output_set_verbosity(opal_hwloc_output, opal_hwloc_verbose);

    }

    if (OPAL_SUCCESS != (rc = opal_hwloc_set_binding_policy(&opal_hwloc_binding_policy,
    ...
    if (opal_hwloc_base_bind_to_core) {
        opal_show_help("help-opal-hwloc-base.txt", "deprecated", true,
                       ...
                       "hwloc_bind_to_core", "hwloc_binding_policy=core");
        ...
        if (OPAL_BINDING_POLICY_IS_SET(opal_hwloc_binding_policy) &&
            ...
            opal_show_help("help-opal-hwloc-base.txt", "redefining-policy", true,
                           "core", opal_hwloc_print_binding(opal_hwloc_binding_policy));
    ...
    if (opal_hwloc_base_bind_to_socket) {
        opal_show_help("help-opal-hwloc-base.txt", "deprecated", true,
                       ...
                       "hwloc_bind_to_socket", "hwloc_binding_policy=socket");
        ...
        if (OPAL_BINDING_POLICY_IS_SET(opal_hwloc_binding_policy) &&
            ...
            /* error - cannot redefine the default ranking policy */
            opal_show_help("help-opal-hwloc-base.txt", "redefining-policy", true,
                           "socket", opal_hwloc_print_binding(opal_hwloc_binding_policy));
...
int opal_hwloc_close(void)
{
    if (!opal_hwloc_inited) {
        return OPAL_SUCCESS;
    }

    if (0 <= opal_hwloc_output) {
        opal_output_close(opal_hwloc_output);
    }
    ...
    if (NULL != opal_hwloc_topology) {
        opal_hwloc_free_topology(opal_hwloc_topology);
        opal_hwloc_topology = NULL;
    }


    /* All done */
    opal_hwloc_inited = false;
    return OPAL_SUCCESS;
}
...
char* opal_hwloc_print_locality(opal_hwloc_locality_t locality)
{
    opal_hwloc_print_buffers_t *ptr;
    int idx;
...
int opal_hwloc_set_binding_policy(opal_binding_policy_t *policy, char *spec)
{
    int i;
