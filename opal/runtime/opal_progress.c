...
/* track whether the async progress thread was spawned */
/* default=disabled. Enabled with runtime env var [OMPI_ASYNC_PROGRESS|OPAL_ASYNC_PROGRESS]=1
 * Preference order : OMPI_ASYNC_PROGRESS is preferred to OPAL_ASYNC_PROGRESS if both set
 * */
...
#if OPAL_ENABLE_PROGRESS_THREADS == 1
    // check if the env var OMPI_ASYNC_PROGRESS or OPAL_ASYNC_PROGRESS is set
    // at runtime.
    char *evar_ompi = getenv("OMPI_ASYNC_PROGRESS");
    char *evar_opal = getenv("OPAL_ASYNC_PROGRESS");
    // prefer OMPI_ASYNC_PROGRESS to OPAL_ASYNC_PROGRESS
    char *evar = evar_ompi ? evar_ompi : evar_opal;

    const bool opal_async_progress_enabled = evar ? (atoi(evar) != 0) : false;

    if (opal_async_progress_enabled && !opal_async_progress_thread_spawned) {
        /* prepare the thread */
