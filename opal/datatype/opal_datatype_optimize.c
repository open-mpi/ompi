...
opal_datatype_opt_find_copy_boundaries(const dt_type_desc_t *type_desc, size_t nb_loops,
                                       opal_datatype_opt_region_t *first,
                                       opal_datatype_opt_region_t *last)
{
    const dt_elem_desc_t *desc = type_desc->desc;
    dt_stack_t *stack;
    /* The stack only grows on LOOP entries, so it is bounded by the loop-nesting
     * depth (at most nb_loops), never by the total descriptor entry count. */
    size_t stack_length = nb_loops + 2;
    int32_t stack_pos = -1;
    int32_t pos = 0;
                                                    ...
                                                    const dt_type_desc_t *type_desc)
{
    ...
    if (!opal_datatype_opt_find_copy_boundaries(type_desc, pData->loops, &first, &last)) {
        return;
    }
                                            ...
                                            bool *reevaluate)
{
    ...
    /* The stack only grows on LOOP entries, so it is bounded by the loop-nesting
     * depth, never by the total descriptor entry count.  pData->loops counts
     * every loop marker in the datatype (including the wrapper added for the
     * contiguous-consolidation path), so it is always >= the nesting depth of
     * any descriptor traversed here; +2 covers the base SAVE_STACK slot plus a
     * slot of headroom, matching the other stack allocations in this file. */
    size_t stack_length = pData->loops + 2;
 ...
 * kept only when it strictly reduces that count (the `next_expanded && !next_reevaluate &&
 * next_ranges >= candidate_ranges` guard discards any that does not). This count is a non-negative
 * integer, so it can strictly decrease only finitely often -- its floor of 1 is precisely the
 * optimum (a single memcpy per element). (2) The only transform that temporarily *grows* the
 * descriptor (loop-boundary unrolling of the head/tail iteration) is bounded by growth_limit
 * (opal_datatype_config.optimize.max_desc_growth * input->used); once a pass would exceed it the loop stops and
 * keeps the best descriptor so far. A retained pass therefore either lowers the range count or
 * enlarges the descriptor toward a hard ceiling, and neither can happen unboundedly, so the
 * iteration converges. It is a hard invariant of this file that no transform added here may violate
 * that monotonicity (e.g. by re-expanding a region it just fused); doing so would be the only way to
 * reintroduce a non-terminating restart.
 *
                                                    ...
                                                    bool top_loop_boundary_only)
{
    ...
    const size_t growth_limit = input_desc->used * opal_datatype_config.optimize.max_desc_growth;
    ...
    record_report_copy:
        {
            ...
            for (uint32_t i = 0; i < block_count; ++i) {
                ...
                if (compare_previous && previous.valid && (previous.end == disp)) {
                    ...
                    /*
                     * This is an opt-in tuning diagnostic (opal_datatype_check_missed_optimizations)
                     * that must remain usable in optimized builds, so it is not gated by
                     * OPAL_ENABLE_DEBUG. Route it through the private datatype output stream at
                     * verbosity 10; when that stream is not open (default) opal_output_verbose()
                     * short-circuits, so there is deliberately no fallback to the stderr stream.
                     */
                    opal_output_verbose(10, opal_datatype_dfd,
