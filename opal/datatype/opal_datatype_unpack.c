                                      ...
                                      unsigned char **memory, size_t *SPACE)
{
    ...
    from = (char *) _packed;
    to = (char *) _memory;
    /* Leftover is contiguous in the current block (do_now < blocklen), same as the homogeneous
     * unpack_partial_blocklen path. Guard the destination span before the conversion writes it. */
    OPAL_DATATYPE_SAFEGUARD_POINTER(_memory, do_now * local_elem_size, (CONVERTOR)->pBaseBuf,
...
#if OPAL_ENABLE_DEBUG
    {
        /* Same per-block / epilog destination checks as unpack_predefined_data(). The conversion
         * callback does not consult pBaseBuf/pDesc, so without this there is no debug detector for
         * an out-of-bounds write into user memory. Release builds skip the walk: the macro is empty
         * and a loop over empty macros would still cost per block. */
        size_t local_elem_size = opal_datatype_basicDatatypes[_elem->common.type]->size;
        unsigned char *guard_mem = _memory;
        size_t remaining = cando_count;
        size_t block_bytes = (size_t) _elem->blocklen * local_elem_size;

        while (_elem->blocklen <= remaining) {
            OPAL_DATATYPE_SAFEGUARD_POINTER(guard_mem, block_bytes, (CONVERTOR)->pBaseBuf,
                                            (CONVERTOR)->pDesc, (CONVERTOR)->count);
            guard_mem += _elem->extent;
            remaining -= _elem->blocklen;
        }
        if (0 != remaining) {
            OPAL_DATATYPE_SAFEGUARD_POINTER(guard_mem, remaining * local_elem_size,
                                            (CONVERTOR)->pBaseBuf, (CONVERTOR)->pDesc,
                                            (CONVERTOR)->count);
        }
    }
#endif /* OPAL_ENABLE_DEBUG */
