...
struct opal_datatype_t {
    ...
    uint32_t stack_depth; /**< convertor stack depth. After commit this is the max LOOP
                               nesting of desc and opt_desc (the convertor may walk either).
                               Before commit it is the add-time LOOP/END_LOOP marker total,
                               a safe overestimate unused by pack/unpack. */
