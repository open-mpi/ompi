...
int opal_convertor_generic_simple_position(opal_convertor_t *pConvertor, size_t *position)
{
     ...
     *
     * *position and bConverted are packed-stream byte offsets, so whole datatype instances
     * are skipped using the packed per-instance size. That size is summed from
     * convertor->sizes -- the same table position_predefined_data() uses -- so the
     * instance skip and the element walk stay in one unit system (remote when PREPARE
     * selected remote_sizes for RECV or SEND_CONVERSION, local otherwise). Dividing
     * get_packed_size() by count used to disagree for RECV|SEND_CONVERSION. The
     * in-memory stack displacements still advance by the local extent.
     */
    iov_len_local = *position - pConvertor->bConverted;
    const size_t stream_size = opal_datatype_compute_remote_size(pConvertor->pDesc,
                                                                 pConvertor->sizes);
    if ((0 != stream_size) && (iov_len_local > stream_size)) {
