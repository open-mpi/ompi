 ...
 */
static inline int opal_convertor_merge_iov(struct iovec *iov, uint32_t *iov_count,
                                           IOVBASE_TYPE *base, size_t len, uint32_t *idx)
{
 ...
 */
int32_t opal_convertor_raw(opal_convertor_t *pConvertor, struct iovec *iov, uint32_t *iov_count,
                           size_t *length)
{
    ...
    DO_DEBUG(opal_output(0, "opal_convertor_raw( %p, {%p, %" PRIu32 "}, %" PRIsize_t " )\n",
                         (void *) pConvertor, (void *) iov, *iov_count, *length););
    ...
    DO_DEBUG(opal_output(0,
                         "raw start pos_desc %d count_desc %" PRIsize_t " disp %ld\n"
    ...
    if (pElem->elem.common.flags & OPAL_DATATYPE_FLAG_DATA) {
        ...
        if (count_desc
            ...
            if ((do_now = count_desc % current->blocklen)) {
                ...
                opal_convertor_merge_iov(iov, iov_count, (IOVBASE_TYPE *) source_base, blength,
                                         &index);
    ...
    while (1) {
        ...
        if (OPAL_DATATYPE_LOOP == pElem->elem.common.type) {
            ...
            if (pElem->loop.common.flags & OPAL_DATATYPE_FLAG_CONTIGUOUS) {
                ...
                source_base += offset;
                for (; count_desc > 0;) {
                    OPAL_DATATYPE_SAFEGUARD_POINTER(source_base, end_loop->size,
                                                    pConvertor->pBaseBuf, pConvertor->pDesc,
                                                    pConvertor->count);
                    if (opal_convertor_merge_iov(iov, iov_count, (IOVBASE_TYPE *) source_base,
                                                 end_loop->size, &index)) {
                        source_base -= offset;
                        goto complete_loop;
                    }
...
complete_loop:
    ...
    *iov_count = index;
    ...
    DO_DEBUG(
        opal_output(0,
                    "raw save stack stack_pos %d pos_desc %d count_desc %" PRIsize_t " disp %ld\n",
