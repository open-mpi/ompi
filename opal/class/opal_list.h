...
{
    return (list->opal_list_sentinel.opal_list_next == &(list->opal_list_sentinel) ? true : false);
}
 ...
 * it's usually a cheap operation.
 */
static inline opal_list_item_t *opal_list_remove_item(opal_list_t *list, opal_list_item_t *item)
{
#if OPAL_ENABLE_DEBUG
    ...
    for (item_ptr = opal_list_get_first(list); item_ptr != opal_list_get_end(list);
...
#if OPAL_ENABLE_DEBUG
#    define opal_list_append(l, i) _opal_list_append(l, i, __FILE__, __LINE__)
#else
 ...
 * it's usually a cheap operation.
 */
static inline void opal_list_prepend(opal_list_t *list, opal_list_item_t *item)
{
    opal_list_item_t *sentinel = &(list->opal_list_sentinel);
#if OPAL_ENABLE_DEBUG
    /* Spot check: ensure that this item is previously on no lists */
