...
SHMEM_GENERATE_FORTRAN_BINDINGS_SUB(void, SHMEM_IGET128, shmem_iget128_, shmem_iget128__,
                                    shmem_iget128_f,
...
void shmem_iget128_f(FORTRAN_POINTER_T target, FORTRAN_POINTER_T source, MPI_Fint *tst,
                     MPI_Fint *sst, MPI_Fint *len, MPI_Fint *pe)
{
    ...
    for (i = 0; i < length; i++) {
        MCA_SPML_CALL(get(oshmem_ctx_default, (uint8_t *) FPTR_2_VOID_PTR(source) + i * sst_c * 16,
                          16, (uint8_t *) FPTR_2_VOID_PTR(target) + i * tst_c * 16,
                          OMPI_FINT_2_INT(*pe)));
    }
}
