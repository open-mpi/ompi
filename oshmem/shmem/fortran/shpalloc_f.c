...
SHMEM_GENERATE_FORTRAN_BINDINGS_SUB(void, SHPALLOC, shpalloc_, shpalloc__, shpalloc_f,
                                    (FORTRAN_POINTER_T * addr, MPI_Fint *length, MPI_Fint *errcode,
                                     MPI_Fint *abort),
...
{
    *errcode = 0;
    /*current shmem spec implies that the length parameter to SHPALLOC function is the number of
     * 32-bit words to allocate*/
    uint32_t *address;
    if (*length <= 0) {
        *errcode = -1;
    }
