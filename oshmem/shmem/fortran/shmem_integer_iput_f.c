...
SHMEM_GENERATE_FORTRAN_BINDINGS_SUB(void, SHMEM_INTEGER_IPUT, shmem_integer_iput_,
                                    shmem_integer_iput__, shmem_integer_iput_f,
...
void shmem_integer_iput_f(FORTRAN_POINTER_T target, FORTRAN_POINTER_T source, MPI_Fint *tst,
                          MPI_Fint *sst, MPI_Fint *len, MPI_Fint *pe)
{
    ...
    for (i = 0; i < length; i++) {
        MCA_SPML_CALL(put(oshmem_ctx_default,
