...
SHMEM_GENERATE_FORTRAN_BINDINGS_SUB(void, SHMEM_IGET4, shmem_iget4_, shmem_iget4__, shmem_iget4_f,
...
void shmem_iget4_f(FORTRAN_POINTER_T target, FORTRAN_POINTER_T source, MPI_Fint *tst, MPI_Fint *sst,
                   MPI_Fint *len, MPI_Fint *pe)
{
    ...
    for (i = 0; i < length; i++) {
        MCA_SPML_CALL(get(oshmem_ctx_default, (uint8_t *) FPTR_2_VOID_PTR(source) + i * sst_c * 4,
                          4, (uint8_t *) FPTR_2_VOID_PTR(target) + i * tst_c * 4,
                          OMPI_FINT_2_INT(*pe)));
    }
}
