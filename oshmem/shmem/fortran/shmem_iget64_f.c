...
SHMEM_GENERATE_FORTRAN_BINDINGS_SUB(void, SHMEM_IGET64, shmem_iget64_, shmem_iget64__,
                                    shmem_iget64_f,
...
void shmem_iget64_f(FORTRAN_POINTER_T target, FORTRAN_POINTER_T source, MPI_Fint *tst,
                    MPI_Fint *sst, MPI_Fint *len, MPI_Fint *pe)
{
    ...
    for (i = 0; i < length; i++) {
        MCA_SPML_CALL(get(oshmem_ctx_default, (uint8_t *) FPTR_2_VOID_PTR(source) + i * sst_c * 8,
                          8, (uint8_t *) FPTR_2_VOID_PTR(target) + i * tst_c * 8,
                          OMPI_FINT_2_INT(*pe)));
    }
}
