 ...
 */
#define DO_SHMEM_TYPE_GET(ctx, type, target, source, nelems, pe)                  \
    do {                                                                          \
        ...
        rc = MCA_SPML_CALL(get(ctx, (void *) source, size, (void *) target, pe)); \
        RUNTIME_CHECK_RC(rc);                                                     \
    } while (0)

#define SHMEM_CTX_TYPE_GET(type_name, type)                                            \
    void shmem_ctx##type_name##_get(shmem_ctx_t ctx, type *target, const type *source, \
...
#define SHMEM_TYPE_GET(type_name, type)                                                  \
    ...
    {                                                                                    \
        DO_SHMEM_TYPE_GET(oshmem_ctx_default, type, target, source, nelems, pe);         \
        return;                                                                          \
    }
...
#define DO_SHMEM_GETMEM(ctx, target, source, element_size, nelems, pe)            \
    do {                                                                          \
        int rc = OSHMEM_SUCCESS;                                                  \
        size_t size = 0;                                                          \
                                                                                  \
...
#define SHMEM_CTX_TYPE_GETMEM(name, element_size, prefix)                                     \
    void prefix##_ctx##name(shmem_ctx_t ctx, void *target, const void *source, size_t nelems, \
                            int pe)                                                           \
    {                                                                                         \
...
#define SHMEM_TYPE_GETMEM(name, element_size, prefix)                                  \
    ...
    {                                                                                  \
        DO_SHMEM_GETMEM(oshmem_ctx_default, target, source, element_size, nelems, pe); \
        return;                                                                        \
    }
