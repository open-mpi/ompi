 ...
 */
#define DO_SHMEM_TYPE_PUT(ctx, type, target, source, len, pe)                     \
    do {                                                                          \
        ...
        rc = MCA_SPML_CALL(put(ctx, (void *) target, size, (void *) source, pe)); \
        RUNTIME_CHECK_RC(rc);                                                     \
    } while (0)

#define SHMEM_CTX_TYPE_PUT(type_name, type)                                                        \
    void shmem_ctx##type_name##_put(shmem_ctx_t ctx, type *target, const type *source, size_t len, \
                                    int pe)                                                        \
    {                                                                                              \
...
#define SHMEM_TYPE_PUT(type_name, type)                                               \
    ...
    {                                                                                 \
        DO_SHMEM_TYPE_PUT(oshmem_ctx_default, type, target, source, len, pe);         \
        return;                                                                       \
    }
...
#define DO_SHMEM_PUTMEM(ctx, target, source, element_size, nelems, pe)            \
    do {                                                                          \
        int rc = OSHMEM_SUCCESS;                                                  \
        size_t size = 0;                                                          \
                                                                                  \
...
#define SHMEM_CTX_TYPE_PUTMEM(name, element_size, prefix)                                     \
    void prefix##_ctx##name(shmem_ctx_t ctx, void *target, const void *source, size_t nelems, \
                            int pe)                                                           \
    {                                                                                         \
        DO_SHMEM_PUTMEM(ctx, target, source, element_size, nelems, pe);                       \
        return;                                                                               \
    }
...
#define SHMEM_TYPE_PUTMEM(name, element_size, prefix)                                  \
    ...
    {                                                                                  \
        DO_SHMEM_PUTMEM(oshmem_ctx_default, target, source, element_size, nelems, pe); \
        return;                                                                        \
    }
