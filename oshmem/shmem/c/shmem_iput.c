...
/*
 * These routines copy strided data from the local PE to a strided data object on the destination
 * PE. The shmem_iput() routines read the elements of a local array (source) and write them to a
 * remote array (target) on the PE indicated by pe. These routines return when the data has been
 * copied out of the source array on the local PE but not necessarily before the data has been
 * delivered to the remote data object.
 */
#define DO_SHMEM_TYPE_IPUT(ctx, type, target, source, tst, sst, nelemes, pe)       \
    do {                                                                           \
        ...
        for (i = 0; i < nelems; i++) {                                             \
            rc = MCA_SPML_CALL(put(ctx, (void *) (target + i * tst), element_size, \
                                   (void *) (source + i * sst), pe));              \
        }                                                                          \
        RUNTIME_CHECK_RC(rc);                                                      \
    } while (0)

#define SHMEM_CTX_TYPE_IPUT(type_name, type)                                              \
    void shmem_ctx##type_name##_iput(shmem_ctx_t ctx, type *target, const type *source,   \
                                     ptrdiff_t tst, ptrdiff_t sst, size_t nelems, int pe) \
    {                                                                                     \
        DO_SHMEM_TYPE_IPUT(ctx, type, target, source, tst, sst, nelems, pe);              \
        return;                                                                           \
    }

#define SHMEM_TYPE_IPUT(type_name, type)                                                         \
    void shmem##type_name##_iput(type *target, const type *source, ptrdiff_t tst, ptrdiff_t sst, \
                                 size_t nelems, int pe)                                          \
    {                                                                                            \
        DO_SHMEM_TYPE_IPUT(oshmem_ctx_default, type, target, source, tst, sst, nelems, pe);      \
        return;                                                                                  \
    }
...
#define DO_SHMEM_IPUTMEM(ctx, target, source, tst, sst, element_size, nelems, pe)             \
    do {                                                                                      \
        int rc = OSHMEM_SUCCESS;                                                              \
        size_t i = 0;                                                                         \
                                                                                              \
        ...
        RUNTIME_CHECK_ADDR(target);                                                           \
                                                                                              \
        for (i = 0; i < nelems; i++) {                                                        \
            rc = MCA_SPML_CALL(put(ctx, (void *) ((char *) target + i * tst * element_size),  \
                                   element_size,                                              \
                                   (void *) ((char *) source + i * sst * element_size), pe)); \
        }                                                                                     \
        RUNTIME_CHECK_RC(rc);                                                                 \
    } while (0)

#define SHMEM_CTX_TYPE_IPUTMEM(name, element_size, prefix)                                    \
    void prefix##_ctx##name(shmem_ctx_t ctx, void *target, const void *source, ptrdiff_t tst, \
                            ptrdiff_t sst, size_t nelems, int pe)                             \
    {                                                                                         \
        DO_SHMEM_IPUTMEM(ctx, target, source, tst, sst, element_size, nelems, pe);            \
        return;                                                                               \
    }

#define SHMEM_TYPE_IPUTMEM(name, element_size, prefix)                                            \
    void prefix##name(void *target, const void *source, ptrdiff_t tst, ptrdiff_t sst,             \
                      size_t nelems, int pe)                                                      \
    {                                                                                             \
        DO_SHMEM_IPUTMEM(oshmem_ctx_default, target, source, tst, sst, element_size, nelems, pe); \
        return;                                                                                   \
    }
