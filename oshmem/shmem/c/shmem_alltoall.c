...
static void _shmem_alltoall(void *target, const void *source, ptrdiff_t dst, ptrdiff_t sst,
                            size_t nelems, size_t element_size, int PE_start, int logPE_stride,
...
#define SHMEM_TYPE_ALLTOALL(name, element_size)                                             \
    ...
    {                                                                                       \
        ...
        RUNTIME_CHECK_ADDR_SIZE(source, nelems);                                            \
                                                                                            \
        _shmem_alltoall(target, source, 1, 1, nelems, element_size, PE_start, logPE_stride, \
                        PE_size, pSync);                                                    \
    }

#define SHMEM_TYPE_ALLTOALLS(name, element_size)                                                \
    void shmem##name(void *target, const void *source, ptrdiff_t dst, ptrdiff_t sst,            \
    ...
    {                                                                                           \
        ...
        RUNTIME_CHECK_ADDR_SIZE(source, nelems);                                                \
                                                                                                \
        _shmem_alltoall(target, source, dst, sst, nelems, element_size, PE_start, logPE_stride, \
                        PE_size, pSync);                                                        \
    }
                            ...
                            int PE_size, long *pSync)
{
    ...
    rc = group->g_scoll.scoll_alltoall(group, target, source, dst, sst, nelems, element_size, pSync,
                                       SCOLL_DEFAULT_ALG);
