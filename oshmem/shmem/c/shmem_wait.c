...
#define SHMEM_TYPE_WAIT(type_name, type, code, prefix)                                \
    ...
    {                                                                                 \
        ...
        RUNTIME_CHECK_INIT();                                                         \
                                                                                      \
        rc = MCA_SPML_CALL(wait((void *) addr, SHMEM_CMP_NE, (void *) &value, code)); \
        RUNTIME_CHECK_RC(rc);                                                         \
                                                                                      \
        return;                                                                       \
    }
...
#define SHMEM_TYPE_WAIT_UNTIL(type_name, type, code, prefix)                 \
    ...
    {                                                                        \
        ...
        RUNTIME_CHECK_INIT();                                                \
                                                                             \
        rc = MCA_SPML_CALL(wait((void *) addr, cmp, (void *) &value, code)); \
        RUNTIME_CHECK_RC(rc);                                                \
                                                                             \
        return;                                                              \
    }
...
#define SHMEM_TYPE_TEST(type_name, type, code, prefix)                                   \
    ...
    {                                                                                    \
        ...
        RUNTIME_CHECK_INIT();                                                            \
                                                                                         \
        rc = MCA_SPML_CALL(test((void *) addr, cmp, (void *) &value, code, &out_value)); \
        RUNTIME_CHECK_RC(rc);                                                            \
                                                                                         \
        return out_value;                                                                \
    }
