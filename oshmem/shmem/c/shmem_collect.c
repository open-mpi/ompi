...
static void _shmem_collect(void *target, const void *source, size_t nbytes, int PE_start,
                           int logPE_stride, int PE_size, long *pSync, bool nlong_type);

#define SHMEM_TYPE_COLLECT(name, element_size, nelems_type)                                   \
    void shmem##name(void *target, const void *source, size_t nelems, int PE_start,           \
    ...
    {                                                                                         \
        ...
        RUNTIME_CHECK_ADDR_SIZE(source, nelems);                                              \
                                                                                              \
        _shmem_collect(target, source, nelems *element_size, PE_start, logPE_stride, PE_size, \
                       pSync, nelems_type);                                                   \
    }

static void _shmem_collect(void *target, const void *source, size_t nbytes, int PE_start,
                           int logPE_stride, int PE_size, long *pSync, bool array_type)
{
    int rc;
    ...
    rc = group->g_scoll.scoll_collect(group, target, source, nbytes, pSync, array_type,
                                      SCOLL_DEFAULT_ALG);
