 ...
 */
#define DO_SHMEM_TYPE_IGET(ctx, type, target, source, tst, sst, nelems, pe)        \
    do {                                                                           \
        ...
        for (i = 0; i < nelems; i++) {                                             \
            rc = MCA_SPML_CALL(get(ctx, (void *) (source + i * sst), element_size, \
                                   (void *) (target + i * tst), pe));              \
        }                                                                          \
        RUNTIME_CHECK_RC(rc);                                                      \
    } while (0)

#define SHMEM_CTX_TYPE_IGET(type_name, type)                                              \
    void shmem_ctx##type_name##_iget(shmem_ctx_t ctx, type *target, const type *source,   \
...
#define SHMEM_TYPE_IGET(type_name, type)                                                         \
    void shmem##type_name##_iget(type *target, const type *source, ptrdiff_t tst, ptrdiff_t sst, \
                                 size_t nelems, int pe)                                          \
    {                                                                                            \
        DO_SHMEM_TYPE_IGET(oshmem_ctx_default, type, target, source, tst, sst, nelems, pe);      \
        return;                                                                                  \
    }
...
#define DO_SHMEM_IGETMEM(ctx, target, source, tst, sst, element_size, nelems, pe)             \
    do {                                                                                      \
        int rc = OSHMEM_SUCCESS;                                                              \
        size_t i = 0;                                                                         \
                                                                                              \
        ...
        RUNTIME_CHECK_ADDR(source);                                                           \
                                                                                              \
        for (i = 0; i < nelems; i++) {                                                        \
            rc = MCA_SPML_CALL(get(ctx, (void *) ((char *) source + i * sst * element_size),  \
                                   element_size,                                              \
                                   (void *) ((char *) target + i * tst * element_size), pe)); \
        }                                                                                     \
        RUNTIME_CHECK_RC(rc);                                                                 \
    } while (0)
...
#define SHMEM_CTX_TYPE_IGETMEM(name, element_size, prefix)                                    \
    ...
    {                                                                                         \
        DO_SHMEM_IGETMEM(ctx, target, source, tst, sst, element_size, nelems, pe);            \
        return;                                                                               \
    }
...
#define SHMEM_TYPE_IGETMEM(name, element_size, prefix)                                            \
    void prefix##name(void *target, const void *source, ptrdiff_t tst, ptrdiff_t sst,             \
                      size_t nelems, int pe)                                                      \
    {                                                                                             \
        DO_SHMEM_IGETMEM(oshmem_ctx_default, target, source, tst, sst, element_size, nelems, pe); \
        return;                                                                                   \
    }
