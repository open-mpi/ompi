 ...
 */
#define DO_SHMEM_TYPE_ATOMIC_FETCH_ADD(ctx, type_name, type, target, value, pe, out_value)      \
    do {                                                                                        \
        ...
        rc = MCA_ATOMIC_CALL(fadd(ctx, (void *) target, (void *) &out_value, value, size, pe)); \
        RUNTIME_CHECK_RC(rc);                                                                   \
    } while (0)

#define SHMEM_CTX_TYPE_ATOMIC_FETCH_ADD(type_name, type, prefix)                               \
    type prefix##_ctx##type_name##_atomic_fetch_add(shmem_ctx_t ctx, type *target, type value, \
                                                    int pe)                                    \
    {                                                                                          \
        type out_value;                                                                        \
        DO_SHMEM_TYPE_ATOMIC_FETCH_ADD(ctx, type_name, type, target, value, pe, out_value);    \
        return out_value;                                                                      \
    }
...
#define SHMEM_TYPE_ATOMIC_FETCH_ADD(type_name, type, prefix)                                   \
    ...
    {                                                                                          \
        type out_value;                                                                        \
        DO_SHMEM_TYPE_ATOMIC_FETCH_ADD(oshmem_ctx_default, type_name, type, target, value, pe, \
                                       out_value);                                             \
        return out_value;                                                                      \
    }
