...
static void _shmem_broadcast(void *target, const void *source, size_t nbytes, int PE_root,
                             int PE_start, int logPE_stride, int PE_size, long *pSync);

#define SHMEM_TYPE_BROADCAST(name, element_size)                                                 \
    void shmem##name(void *target, const void *source, size_t nelems, int PE_root, int PE_start, \
    ...
    {                                                                                            \
        ...
        RUNTIME_CHECK_ADDR_SIZE(source, nelems);                                                 \
                                                                                                 \
        _shmem_broadcast(target, source, nelems *element_size, PE_root, PE_start, logPE_stride,  \
                         PE_size, pSync);                                                        \
    }
                             ...
                             int PE_start, int logPE_stride, int PE_size, long *pSync)
{
    ...
    if ((0 <= PE_root) && (PE_root < PE_size)) {
        ...
        rc = group->g_scoll.scoll_broadcast(group, PE_root, target, source, nbytes, pSync, true,
                                            SCOLL_DEFAULT_ALG);
    out:
