...
#define DO_SHMEM_TYPE_ATOMIC_ADD(ctx, type_name, type, target, value, pe) \
    {                                                                     \
        ...
        RUNTIME_CHECK_ADDR(target);                                       \
                                                                          \
        size = sizeof(value);                                             \
        rc = MCA_ATOMIC_CALL(add(ctx, (void *) target, value, size, pe)); \
        RUNTIME_CHECK_RC(rc);                                             \
    }                                                                     \
    while (0)
...
#define SHMEM_CTX_TYPE_ATOMIC_ADD(type_name, type, prefix)                                       \
    ...
    {                                                                                            \
        DO_SHMEM_TYPE_ATOMIC_ADD(ctx, type_name, type, target, value, pe);                       \
        return;                                                                                  \
    }
...
#define SHMEM_TYPE_ATOMIC_ADD(type_name, type, prefix)                                    \
    ...
    {                                                                                     \
        DO_SHMEM_TYPE_ATOMIC_ADD(oshmem_ctx_default, type_name, type, target, value, pe); \
                                                                                          \
        return;                                                                           \
    }
