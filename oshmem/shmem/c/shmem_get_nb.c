 ...
 */
#define DO_SHMEM_TYPE_GET_NB(ctx, type, target, source, nelems, pe)                        \
    do {                                                                                   \
        ...
        rc = MCA_SPML_CALL(get_nb(ctx, (void *) source, size, (void *) target, pe, NULL)); \
        RUNTIME_CHECK_RC(rc);                                                              \
    } while (0)

#define SHMEM_CTX_TYPE_GET_NB(type_name, type)                                             \
    void shmem_ctx##type_name##_get_nbi(shmem_ctx_t ctx, type *target, const type *source, \
...
#define SHMEM_TYPE_GET_NB(type_name, type)                                                   \
    ...
    {                                                                                        \
        DO_SHMEM_TYPE_GET_NB(oshmem_ctx_default, type, target, source, nelems, pe);          \
        return;                                                                              \
    }
...
#define DO_SHMEM_GETMEM_NB(ctx, target, source, element_size, nelems, pe)                  \
    do {                                                                                   \
        int rc = OSHMEM_SUCCESS;                                                           \
        size_t size = 0;                                                                   \
                                                                                           \
...
#define SHMEM_TYPE_GETMEM_NB(name, element_size, prefix)                                  \
    ...
    {                                                                                     \
        DO_SHMEM_GETMEM_NB(oshmem_ctx_default, target, source, element_size, nelems, pe); \
        return;                                                                           \
    }
