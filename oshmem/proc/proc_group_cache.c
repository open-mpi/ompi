...
oshmem_group_t* find_group_in_cache(int PE_start, int logPE_stride, int PE_size)
{
    ...
    {
        if (!memcmp(((oshmem_group_cache_t *)item)->cache_id,cache_look_up_id,3*sizeof(int)))
        {
...
int cache_group(oshmem_group_t *group, int PE_start, int logPE_stride, int PE_size)
{
    ...
    if (opal_list_get_size(&oshmem_group_cache_list) < oshmem_group_cache_size)
    {
    ...
    else
    {
        ...
        opal_output(0,"error: group cache overflow on rank %i: cache_size = %u: try encreasing oshmem_group_cache_size mca parameter",
                group->my_pe,oshmem_group_cache_size);
...
int oshmem_group_cache_list_init(void)
{
    ...
    mca_base_param_reg_int_name(NULL,"oshmem_group_cache_size",
            "The depth of the oshmem_group cache list used to speed up collective operations",
            false,false,
            cache_size_default,
            &mca_value);
    if (mca_value < 0){
        opal_output(0,"error: oshmem_group_cache_size mca parameter was set to %i while it has to be positive value. Default value %i will be used.",mca_value,cache_size_default);
