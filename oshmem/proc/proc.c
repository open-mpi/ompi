...
int oshmem_proc_finalize (void)
{
     ...
     */
    while (opal_list_get_end(&oshmem_proc_list) != (item = opal_list_get_first(&oshmem_proc_list))) {
        OBJ_RELEASE(item);
    }
...
{
    oshmem_proc_t **procs =
        (oshmem_proc_t**) malloc(opal_list_get_size(&oshmem_proc_list) * sizeof(oshmem_proc_t*));
    oshmem_proc_t *proc;
    size_t count = 0;
    ...
    OPAL_THREAD_LOCK(&oshmem_proc_lock);
    for(proc =  (oshmem_proc_t*)opal_list_get_first(&oshmem_proc_list);
        ((proc != (oshmem_proc_t*)opal_list_get_end(&oshmem_proc_list)) && (proc != NULL));
        proc =  (oshmem_proc_t*)opal_list_get_next(proc)) {
...
int oshmem_proc_refresh(void) {
    ...
    for( item  = opal_list_get_first(&oshmem_proc_list), i = 0;
        ...
        comm_bcast_pml(&proc->proc_arch, i, sizeof(uint32_t),
                MPI_BYTE, ORTE_PROC_MY_NAME->vpid, orte_process_info.num_procs,
                ranks_in_comm, (ompi_communicator_t *)&ompi_mpi_comm_world);

        int hostname_length = strlen(orte_process_info.nodename);
        comm_bcast_pml(&hostname_length, i, sizeof(int),
                MPI_BYTE, ORTE_PROC_MY_NAME->vpid, orte_process_info.num_procs,
                ranks_in_comm, (ompi_communicator_t *)&ompi_mpi_comm_world);
        ...
        proc->proc_hostname = (i == ORTE_PROC_MY_NAME->vpid ?
                strdup(orte_process_info.nodename) : (char *) malloc (hostname_length));

        comm_bcast_pml(proc->proc_hostname, i, hostname_length,
                MPI_BYTE, ORTE_PROC_MY_NAME->vpid, orte_process_info.num_procs,
                ranks_in_comm, (ompi_communicator_t *)&ompi_mpi_comm_world);
...
int
oshmem_proc_pack(oshmem_proc_t **proclist, int proclistsize, opal_buffer_t* buf)
{
    int i, rc;
...
int
oshmem_proc_unpack(opal_buffer_t* buf, 
                 ...
                 int *newproclistsize, oshmem_proc_t ***newproclist)
{
    ...
    for ( i=0; i<proclistsize; i++ ){
        ...
        if (isnew) {
            ...
            if (0 == strcmp(oshmem_proc_local_proc->proc_hostname,new_hostname)) {
...
OSHMEM_DECLSPEC int oshmem_proc_group_init(void)
{
    ...
    if( OPAL_SUCCESS != opal_pointer_array_init(&oshmem_group_array, 0,
                                                ORTE_GLOBAL_ARRAY_MAX_SIZE, 1) ) {
        return OSHMEM_ERROR;
    }

    /* Setup SHMEM_GROUP_ALL */
    if (NULL == (oshmem_group_all = oshmem_proc_group_create(0, 1, opal_list_get_size(&oshmem_proc_list)))) {
        oshmem_proc_group_destroy(oshmem_group_all);
        return OSHMEM_ERROR;                
    }

    /* Setup SHMEM_GROUP_SELF */
    if (NULL == (oshmem_group_self = oshmem_proc_group_create(oshmem_proc_local()->proc_name.vpid, 0, 1))) {
...
OSHMEM_DECLSPEC oshmem_group_t* oshmem_proc_group_create(int pe_start, int pe_stride, size_t pe_size)
{
    ...
    if (group)
    {
        ...
        for (proc =  (oshmem_proc_t*)opal_list_get_first(&oshmem_proc_list);
            ...
            else if ((cur_pe >= pe_start) && ((pe_stride == 0) || (((cur_pe - pe_start) % pe_stride) == 0))) 
            {
        ...
        if (OSHMEM_SUCCESS != mca_scoll_base_select(group)){
            opal_output(0,"Error: No collective modules are available: group is not created, returning NULL");
            oshmem_proc_group_destroy(group);
            OPAL_THREAD_UNLOCK(&oshmem_proc_lock);
            return NULL;
        }
