...
static int _algorithm_central_counter(struct oshmem_group_t *group, int PE_root, void *target,
                                      const void *source, size_t nlong, long *pSync);
static int _algorithm_binomial_tree(struct oshmem_group_t *group, int PE_root, void *target,
                                    const void *source, size_t nlong, long *pSync);
    ...
    if ((rc == OSHMEM_SUCCESS) && oshmem_proc_group_is_member(group)) {
        ...
        if (pSync) {
            ...
            case SCOLL_ALG_BROADCAST_CENTRAL_COUNTER: {
                rc = _algorithm_central_counter(group, PE_root, target, source, nlong, pSync);
                break;
            }
            case SCOLL_ALG_BROADCAST_BINOMIAL: {
                rc = _algorithm_binomial_tree(group, PE_root, target, source, nlong, pSync);
                break;
            }
            default: {
                rc = _algorithm_binomial_tree(group, PE_root, target, source, nlong, pSync);
                                    ...
                                    const void *source, size_t nlong, long *pSync)
{
    ...
    for (i = hibit + 1, mask = 1 << i; i <= dim; ++i, mask <<= 1) {
        ...
        if (peer_id < group->proc_count) {
            ...
            SCOLL_VERBOSE(14, "[#%d] check remote pe is ready to receive #%d", group->my_pe,
                          peer_pe);
            do {
                rc = MCA_SPML_CALL(
                    get(oshmem_ctx_default, (void *) pSync, sizeof(long), (void *) pSync, peer_pe));
            } while ((OSHMEM_SUCCESS == rc) && (pSync[0] != SHMEM_SYNC_READY));

            SCOLL_VERBOSE(14, "[#%d] send data to #%d", group->my_pe, peer_pe);
            rc = MCA_SPML_CALL(put(oshmem_ctx_default, target, nlong,
                                   (my_id == root_id ? (void *) source : target), peer_pe));
