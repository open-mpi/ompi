...
static int __algorithm_central_counter(struct oshmem_group_t *group, int PE_root, void *target, const void *source, size_t nlong, long *pSync);
static int __algorithm_binomial_tree(struct oshmem_group_t *group, int PE_root, void *target, const void *source, size_t nlong, long *pSync);


int mca_scoll_basic_broadcast(struct oshmem_group_t *group, int PE_root, void *target, const void *source, size_t nlong, long *pSync, int alg)
{
    ...
    if ((rc == OSHMEM_SUCCESS) && oshmem_proc_group_is_member(group))
    {
        ...
        if (pSync)
        {
            alg = ( alg == SCOLL_DEFAULT_ALG ? mca_scoll_basic_param_broadcast_algorithm : alg);
            switch(alg)
            {
                case SCOLL_ALG_BROADCAST_CENTRAL_COUNTER:
                {
                    rc = __algorithm_central_counter(group, PE_root, target, source, nlong, pSync);
                    break;
                }
                case SCOLL_ALG_BROADCAST_BINOMIAL:
                {
                    rc = __algorithm_binomial_tree(group, PE_root, target, source, nlong, pSync);
                    break;
                }
                default:
                {
...
static int __algorithm_central_counter(struct oshmem_group_t *group, int PE_root, void *target, const void *source, size_t nlong, long *pSync)
{
    ...
    SCOLL_VERBOSE(12, "[#%d] Broadcast algorithm: Central Counter", group->my_pe);
    SCOLL_VERBOSE(15, "[#%d] pSync[0] = %ld root = #%d", group->my_pe, pSync[0], PE_root);
    ...
    if (PE_root == group->my_pe)
    {
        ...
        for (i = 0; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) 
        {
            ...
            if (pe_cur != PE_root)
            {
                SCOLL_VERBOSE(15, "[#%d] send data to #%d", group->my_pe, pe_cur);
    ...
    if (rc == OSHMEM_SUCCESS)
    {
        ...
        rc = group->g_scoll.scoll_barrier(group, (pSync + 1), SCOLL_DEFAULT_ALG);
    }
...
static int __algorithm_binomial_tree(struct oshmem_group_t *group, int PE_root, void *target, const void *source, size_t nlong, long *pSync)
{
    ...
    SCOLL_VERBOSE(15, "[#%d] dim = %d vrank = %d hibit = %d", group->my_pe, dim, vrank, hibit);
    ...
    if (vrank > 0) 
    {
        ...
        {
            SCOLL_VERBOSE(14, "[#%d] Broadcast size is a negative value (%li)\n", group->my_pe, pSync[0]);
    ...
    for (i = hibit + 1, mask = 1 << i; i <= dim; ++i, mask <<= 1) 
    {
        ...
        if (peer_id < group->proc_count) 
        {
            ...
            SCOLL_VERBOSE(14, "[#%d] check remote pe is ready to receive #%d", group->my_pe, peer_pe);
            do {
