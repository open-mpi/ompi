...
static int __algorithm_central_counter(struct oshmem_group_t *group, struct oshmem_op_t *op, void *target, const void *source, size_t nlong, long *pSync, void *pWrk);
static int __algorithm_tournament(struct oshmem_group_t *group, struct oshmem_op_t *op, void *target, const void *source, size_t nlong, long *pSync, void *pWrk);
static int __algorithm_recursive_doubling(struct oshmem_group_t *group, struct oshmem_op_t *op, void *target, const void *source, size_t nlong, long *pSync, void *pWrk);
static int __algorithm_linear(struct oshmem_group_t *group, struct oshmem_op_t *op, void *target, const void *source, size_t nlong, long *pSync, void *pWrk);
static int __algorithm_log(struct oshmem_group_t *group, struct oshmem_op_t *op, void *target, const void *source, size_t nlong, long *pSync, void *pWrk);


int mca_scoll_basic_reduce(struct oshmem_group_t *group, struct oshmem_op_t *op, void *target, const void *source, size_t nlong, long *pSync, void *pWrk, int alg)
{
    ...
    if ((rc == OSHMEM_SUCCESS) && oshmem_proc_group_is_member(group))
    {
        ...
        if (pSync)
        {
            ...
            switch(alg)
            {
                ...
                {
                    rc = __algorithm_central_counter(group, op, target, source, nlong, pSync, pWrk);
                    break;
                }
                case SCOLL_ALG_REDUCE_TOURNAMENT:
                {
                    rc = __algorithm_tournament(group, op, target, source, nlong, pSync, pWrk);
                    break;
                }
                case SCOLL_ALG_REDUCE_RECURSIVE_DOUBLING:
                {
                    rc = __algorithm_recursive_doubling(group, op, target, source, nlong, pSync, pWrk);
                    break;
                }
                case SCOLL_ALG_REDUCE_LEGACY_LINEAR:
                {
                    rc = __algorithm_linear(group, op, target, source, nlong, pSync, pWrk);
                    break;
                }
                case SCOLL_ALG_REDUCE_LEGACY_LOG:
                {
                    rc = __algorithm_log(group, op, target, source, nlong, pSync, pWrk);
                    break;
                }
                default:
                {
...
static int __algorithm_central_counter(struct oshmem_group_t *group, struct oshmem_op_t *op, void *target, const void *source, size_t nlong, long *pSync, void *pWrk)
{
    ...
    if (rc == OSHMEM_SUCCESS)
    {
        ...
        rc = group->g_scoll.scoll_broadcast(group, PE_root, target, target, nlong, (pSync + 1), SCOLL_DEFAULT_ALG);
    }
...
static int __algorithm_linear(struct oshmem_group_t *group, struct oshmem_op_t *op, void *target, const void *source, size_t nlong, long *pSync, void *pWrk)
{
    ...
    if (rc == OSHMEM_SUCCESS)
    {
        ...
        rc = group->g_scoll.scoll_broadcast(group, root_pe, target, target, nlong, (pSync + 1), SCOLL_DEFAULT_ALG);
    }
...
static int __algorithm_log(struct oshmem_group_t *group, struct oshmem_op_t *op, void *target, const void *source, size_t nlong, long *pSync, void *pWrk)
{
    ...
    SCOLL_VERBOSE(12, "[#%d] Reduce algorithm: Log", group->my_pe);
