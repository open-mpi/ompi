...
static int _algorithm_central_collector(struct oshmem_group_t *group, void *target,
                                        const void *source, size_t nlong, long *pSync);
static int _algorithm_f_central_counter(struct oshmem_group_t *group, void *target,
                                        const void *source, size_t nlong, long *pSync);
static int _algorithm_f_tournament(struct oshmem_group_t *group, void *target, const void *source,
                                   size_t nlong, long *pSync);
static int _algorithm_f_recursive_doubling(struct oshmem_group_t *group, void *target,
                                           const void *source, size_t nlong, long *pSync);
static int _algorithm_f_ring(struct oshmem_group_t *group, void *target, const void *source,
                             size_t nlong, long *pSync);
                            ...
                            size_t nlong, long *pSync, bool nlong_type, int alg)
{
    ...
    if ((rc == OSHMEM_SUCCESS) && oshmem_proc_group_is_member(group)) {
        ...
        if (nlong_type) {
            ...
            alg = (alg == SCOLL_DEFAULT_ALG ? mca_scoll_basic_param_collect_algorithm : alg);
            switch (alg) {
            case SCOLL_ALG_COLLECT_CENTRAL_COUNTER: {
                rc = _algorithm_f_central_counter(group, target, source, nlong, pSync);
                break;
            }
            case SCOLL_ALG_COLLECT_TOURNAMENT: {
                rc = _algorithm_f_tournament(group, target, source, nlong, pSync);
                break;
            }
            case SCOLL_ALG_COLLECT_RECURSIVE_DOUBLING: {
                rc = _algorithm_f_recursive_doubling(group, target, source, nlong, pSync);
                break;
            }
            case SCOLL_ALG_COLLECT_RING: {
                rc = _algorithm_f_ring(group, target, source, nlong, pSync);
                break;
            }
            default: {
                rc = _algorithm_f_central_counter(group, target, source, nlong, pSync);
            ...
            rc = _algorithm_central_collector(group, target, source, nlong, pSync);
        }
                                        ...
                                        const void *source, size_t nlong, long *pSync)
{
    ...
    SCOLL_VERBOSE(12, "[#%d] Collect algorithm: Central Counter (identical size)", group->my_pe);
    SCOLL_VERBOSE(15, "[#%d] pSync[0] = %ld", group->my_pe, pSync[0]);

    if (PE_root == group->my_pe) {
        int pe_cur = 0;

        memcpy((void *) ((unsigned char *) target + 0 * nlong), (void *) source, nlong);
        ...
        for (i = 0; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) {
            ...
            SCOLL_VERBOSE(14, "[#%d] Gather data (%d bytes) from #%d", group->my_pe, (int) nlong,
                          pe_cur);

            /* Get data from the current peer */
            rc = MCA_SPML_CALL(get(oshmem_ctx_default, (void *) source, nlong,
                                   (void *) ((unsigned char *) target + i * nlong), pe_cur));
        }
    }
    ...
    if (rc == OSHMEM_SUCCESS) {
        ...
        rc = BCAST_FUNC(group, PE_root, target, target, group->proc_count * nlong, (pSync + 1),
                        true, SCOLL_DEFAULT_ALG);
    }
                                   ...
                                   size_t nlong, long *pSync)
{
    ...
    SCOLL_VERBOSE(12, "[#%d] Collect algorithm: Tournament (identical size)", group->my_pe);
    SCOLL_VERBOSE(15, "[#%d] pSync[0] = %ld", group->my_pe, pSync[0]);
    ...
    /* Copy data to itself */
    memcpy((void *) ((unsigned char *) target + my_id * nlong), (void *) source, nlong);
    ...
    while (exit_flag && (rc == OSHMEM_SUCCESS)) {
       ...
       */
            ...
            rc = MCA_SPML_CALL(put(oshmem_ctx_default,
                                   (void *) ((unsigned char *) target + my_id * nlong),
                                   (1 << (round - 1)) * nlong,
                                   (void *) ((unsigned char *) target + my_id * nlong), peer_pe));
                             ...
                             size_t nlong, long *pSync)
{
    ...
    SCOLL_VERBOSE(12, "[#%d] Collect algorithm: Ring (identical size)", group->my_pe);
    SCOLL_VERBOSE(15, "[#%d] pSync[0] = %ld", group->my_pe, pSync[0]);
    ...
    for (i = 0; (i < (group->proc_count - 1)) && (rc == OSHMEM_SUCCESS); i++) {
        SCOLL_VERBOSE(14, "[#%d] round = %d send data to #%d by index = %d", group->my_pe, i,
                      peer_pe, data_index);
        rc = MCA_SPML_CALL(put(oshmem_ctx_default,
                               (void *) ((unsigned char *) target + data_index * nlong), nlong,
                               (void *) ((unsigned char *) target + data_index * nlong), peer_pe));

        MCA_SPML_CALL(fence(oshmem_ctx_default));

        SCOLL_VERBOSE(14, "[#%d] round = %d signals to #%d", group->my_pe, i, peer_pe);
        value = i;
        rc = MCA_SPML_CALL(
            put(oshmem_ctx_default, (void *) pSync, sizeof(value), (void *) &value, peer_pe));
        ...
        SCOLL_VERBOSE(14, "[#%d] round = %d wait for data by index = %d", group->my_pe, i,
                      data_index);
        if (i == 0) {
                                           ...
                                           const void *source, size_t nlong, long *pSync)
{
    ...
    SCOLL_VERBOSE(12, "[#%d] Collect algorithm: Recursive Doubling (identical size)", group->my_pe);
    ...
    if (my_id >= floor2_proc) {
        ...
        for (i = 0; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) {
            ...
            SCOLL_VERBOSE(14, "[#%d] is extra send data to #%d", group->my_pe, pe_cur);
            rc = MCA_SPML_CALL(put(oshmem_ctx_default,
                                   (void *) ((unsigned char *) target + data_index * nlong), nlong,
                                   (void *) source, pe_cur));
        }
       ...
       */
            ...
            SCOLL_VERBOSE(14, "[#%d] round = %d send data to #%d by index = %d", group->my_pe,
                          round, peer_pe, data_index);
            rc = MCA_SPML_CALL(
                put(oshmem_ctx_default, (void *) ((unsigned char *) target + data_index * nlong),
                    (1 << (round - 1)) * nlong,
                    (void *) ((unsigned char *) target + data_index * nlong), peer_pe));
        ...
        if ((group->proc_count - floor2_proc) > my_id) {
            ...
            rc = MCA_SPML_CALL(
                put(oshmem_ctx_default, target, group->proc_count * nlong, target, peer_pe));
                                        ...
                                        const void *source, size_t nlong, long *pSync)
{
    ...
    SCOLL_VERBOSE(12, "[#%d] Collect algorithm: Central Counter (vary size)", group->my_pe);
    ...
    if (PE_root == group->my_pe) {
        ...
        if (wait_pe_array) {
            memset((void *) wait_pe_array, 0, sizeof(*wait_pe_array) * wait_pe_count);
            wait_pe_array[0] = nlong;
            wait_pe_count--;

            while (wait_pe_count) {
                SCOLL_VERBOSE(14, "[#%d] Gather data size info from all PEs in the group",
                              group->my_pe);
                for (i = 1; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) {
                    if (wait_pe_array[i] == 0) {
                        pe_cur = oshmem_proc_pe(group->proc_array[i]);
                        value = 0;
                        rc = MCA_SPML_CALL(get(oshmem_ctx_default, (void *) pSync, sizeof(value),
                                               (void *) &value, pe_cur));
                        if ((rc == OSHMEM_SUCCESS) && (value != _SHMEM_SYNC_VALUE)) {
                            wait_pe_array[i] = value;
                            wait_pe_count--;
                            SCOLL_VERBOSE(
                                14, "Got source data size as %d from #%d (wait list counter: %d)",
            ...
            for (i = 1; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) {
                ...
                /* Get data from the current peer */
                rc = MCA_SPML_CALL(get(oshmem_ctx_default, (void *) source,
                                       (size_t) wait_pe_array[i],
                                       (void *) ((unsigned char *) target + offset), pe_cur));

                SCOLL_VERBOSE(14, "Got %d bytes of data from #%d (offset: %d)",
    ...
    if (rc == OSHMEM_SUCCESS) {
        ...
        rc = BCAST_FUNC(group, PE_root, target, target, offset, (pSync + 1), false,
                        SCOLL_DEFAULT_ALG);
    }
