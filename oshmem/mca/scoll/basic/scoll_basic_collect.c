...
static int __algorithm_central_collector(struct oshmem_group_t *group, void *target, const void *source, size_t nlong, long *pSync);
static int __algorithm_f_central_counter(struct oshmem_group_t *group, void *target, const void *source, size_t nlong, long *pSync);
static int __algorithm_f_tournament(struct oshmem_group_t *group, void *target, const void *source, size_t nlong, long *pSync);
static int __algorithm_f_recursive_doubling(struct oshmem_group_t *group, void *target, const void *source, size_t nlong, long *pSync);
static int __algorithm_f_ring(struct oshmem_group_t *group, void *target, const void *source, size_t nlong, long *pSync);


int mca_scoll_basic_collect(struct oshmem_group_t *group, void *target, const void *source, size_t nlong, long *pSync, bool nlong_type, int alg)
{
    ...
    if ((rc == OSHMEM_SUCCESS) && oshmem_proc_group_is_member(group))
    {
        ...
        if (nlong_type)
        {
            ...
            switch(alg)
            {
                ...
                {
                    rc = __algorithm_f_central_counter(group, target, source, nlong, pSync);
                    break;
                }
                case SCOLL_ALG_COLLECT_TOURNAMENT:
                {
                    rc = __algorithm_f_tournament(group, target, source, nlong, pSync);
                    break;
                }
                case SCOLL_ALG_COLLECT_RECURSIVE_DOUBLING:
                {
                    rc = __algorithm_f_recursive_doubling(group, target, source, nlong, pSync);
                    break;
                }
                case SCOLL_ALG_COLLECT_RING:
                {
                    rc = __algorithm_f_ring(group, target, source, nlong, pSync);
                    break;
                }
                default:
                {
        ...
        {
            rc = __algorithm_central_collector(group, target, source, nlong, pSync);
        }
...
static int __algorithm_f_central_counter(struct oshmem_group_t *group, void *target, const void *source, size_t nlong, long *pSync)
{
    ...
    if (PE_root == group->my_pe)
    {
        ...
        for (i = 0; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) 
        {
            ...
            if (pe_cur == group->my_pe) continue;

            SCOLL_VERBOSE(14, "[#%d] Gather data (%d bytes) from #%d", group->my_pe, (int)nlong, pe_cur);
    ...
    if (rc == OSHMEM_SUCCESS)
    {
        ...
        rc = group->g_scoll.scoll_broadcast(group, PE_root, target, target, group->proc_count * nlong, (pSync + 1), SCOLL_DEFAULT_ALG);
    }
...
static int __algorithm_f_ring(struct oshmem_group_t *group, void *target, const void *source, size_t nlong, long *pSync)
{
    ...
    {
        SCOLL_VERBOSE(14, "[#%d] round = %d send data to #%d by index = %d", group->my_pe, i, peer_pe, data_index);
        ...
        SCOLL_VERBOSE(14, "[#%d] round = %d wait for data by index = %d", group->my_pe, i, data_index);
        if ( i == 0 )
        {
...
static int __algorithm_f_recursive_doubling(struct oshmem_group_t *group, void *target, const void *source, size_t nlong, long *pSync)
{
    ...
    else
    {
       ...
       */
            ...
            SCOLL_VERBOSE(14, "[#%d] round = %d send data to #%d by index = %d", group->my_pe, round, peer_pe, data_index);
...
static int __algorithm_central_collector(struct oshmem_group_t *group, void *target, const void *source, size_t nlong, long *pSync)
{
    ...
    if (PE_root == group->my_pe)
    {
        ...
        if (wait_pe_array)
        {
            ...
            while (wait_pe_count)
            {
                SCOLL_VERBOSE(14, "[#%d] Gather data size info from all PEs in the group", group->my_pe);
                for (i = 1; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) 
                {
                    if (wait_pe_array[i] == 0)
                    {
                        ...
                        if ( (rc == OSHMEM_SUCCESS) && (value != _SHMEM_SYNC_VALUE) && (value > 0) )
                        {
                            wait_pe_array[i] = (size_t)value;
                            wait_pe_count--;
                            SCOLL_VERBOSE(14, "Got source data size as %d from #%d (wait list counter: %d)", (int)value, pe_cur, (int)wait_pe_count);
                        }
                    }
                }
            }
            ...
            for (i = 1; (i < group->proc_count) && (rc == OSHMEM_SUCCESS); i++) 
            {
                ...
                SCOLL_VERBOSE(14, "Got %d bytes of data from #%d (offset: %d)", (int)wait_pe_array[i], pe_cur, (int)offset);
    ...
    {
        SCOLL_VERBOSE(14, "[#%d] Broadcast from the root #%d", group->my_pe, PE_root);
        rc = group->g_scoll.scoll_broadcast(group, PE_root, target, target, offset, (pSync + 1), SCOLL_DEFAULT_ALG);
    }
