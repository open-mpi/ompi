 ...
 * Global variables
 */
bool mca_scoll_base_components_available_valid = false;
opal_list_t mca_scoll_base_components_available;
                                  ...
                                  bool enable_threads)
{
    ...
    OBJ_CONSTRUCT(&mca_scoll_base_components_available, opal_list_t);
    mca_scoll_base_components_available_valid = true;

    /* The list of components that we should check has already been
       established in mca_coll_base_open. */

    for (found = false,
         p = opal_list_remove_first(&mca_scoll_base_components_opened);
         p != NULL;
         p = opal_list_remove_first(&mca_scoll_base_components_opened)) {
        ...
        if (OSHMEM_SUCCESS == init_query(component, entry,
                                       ...
                                       enable_threads)) {
            opal_list_append(&mca_scoll_base_components_available,
    ...
    if (!found) {
        ...
        SCOLL_VERBOSE(10,"scoll:find_available: no scoll components available!");
        return OSHMEM_ERROR;
    }
                      ...
                      bool enable_progress_threads, bool enable_threads)
{
    ...
    SCOLL_VERBOSE(10,"coll:find_available: querying coll component %s",
    ...
    if (1 == component->mca_type_major_version &&
        ...
        0 == component->mca_type_release_version) {
            ...
            ret = scoll->scoll_init(enable_progress_threads,
                                    enable_threads);
    } else {
        /* Unrecognized coll API version */

        SCOLL_VERBOSE(10,"scoll:find_available: unrecognized scoll API version (%d.%d.%d, ignored)",
