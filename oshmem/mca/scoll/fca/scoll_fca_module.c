...
static int __internal_barrier(mca_scoll_fca_module_t *fca_module)
{
    ...
    return fca_module->previous_barrier(fca_module->comm,&pSync, SCOLL_ALG_BARRIER_BASIC);
#endif
}
...
static int have_remote_peers(struct oshmem_group_t *group, size_t size, int *local_peers)
{
...
static int __fca_comm_new(mca_scoll_fca_module_t *fca_module)
{
    ...
    MCA_SPML_CALL(put((void *)&mca_scoll_fca_component.rcounts[my_id],
                (size_t)sizeof(info_size), (void *)&info_size, root_pe));
    ...
    if (root_pe == comm->my_pe)
    {
        ...
        for (i=0; i<comm->proc_count; i++)
        {
            MCA_SPML_CALL(wait((void *)&mca_scoll_fca_component.rcounts[i],
                               SHMEM_CMP_NE, &value, SHMEM_INT));
        }
    }
    ...
    if (my_info)
    {
        memcpy(mca_scoll_fca_component.my_info_exchangeable,my_info,info_size);
    }
    __internal_barrier(fca_module);
    if (root_pe == comm->my_pe)
    {
        for (i=0; i<comm->proc_count; i++)
        {
            ...
            {
                MCA_SPML_CALL(get((void *)mca_scoll_fca_component.my_info_exchangeable,
                                  mca_scoll_fca_component.rcounts[i],
                                  (void*)(((char*)all_info)+disps[i]),comm->proc_array[i]->proc_name.vpid));
            }
        }
    }
    ...
    if (root_pe == comm->my_pe) {
        ...
        *mca_scoll_fca_component.ret = fca_comm_new(mca_scoll_fca_component.fca_context,
    ...
    if (root_pe != comm->my_pe) 
    {
        MCA_SPML_CALL(get((void *)mca_scoll_fca_component.ret,sizeof(int),
                          (void *)mca_scoll_fca_component.ret, root_pe));
    }
    ...
    {
        FCA_ERROR("rank %i: COMM_NEW failed: %s", fca_module->rank,
                  fca_strerror(*mca_scoll_fca_component.ret));
        return OSHMEM_ERROR;
    }
...

    {
        ...
        if (root_pe != comm->my_pe)
        {
            MCA_SPML_CALL(get((void *)mca_scoll_fca_component.fca_comm_desc_exchangeable,
                              sizeof(fca_module->fca_comm_desc),
                              (void *)&fca_module->fca_comm_desc,
                              root_pe));
        }
...
static int __create_fca_comm(mca_scoll_fca_module_t *fca_module)
{
    ...
    ret = mca_scoll_fca_comm_init(mca_scoll_fca_component.fca_context,
            oshmem_proc_group_find_id(fca_module->comm,fca_module->rank), comm_size,
            fca_module->local_proc_idx, fca_module->num_local_procs,
...
mca_scoll_fca_comm_query(struct oshmem_group_t *comm, int *priority)
{
    ...
    *priority = 0;
    ...
    if (NULL == mca_scoll_fca_component.ret){
        ...
        MCA_MEMHEAP_CALL(private_alloc(oshmem_group_all->proc_count*sizeof(*mca_scoll_fca_component.rcounts),
                                       (void **)&mca_scoll_fca_component.rcounts ));
    ...
    if (size < mca_scoll_fca_component.fca_np) {
        FCA_VERBOSE(20,"size(%d) < fca_np(%d)", size, mca_scoll_fca_component.fca_np);
        goto exit;
    }
    ...
    if (!have_remote_peers(comm, size, &local_peers) /* || OMPI_COMM_IS_INTER(comm)*/) {
        FCA_VERBOSE(1,"all peers in group are on the same node, fca disabled\n");
        goto exit;
    }
    ...
    fca_module->super.scoll_collect  = mca_scoll_fca_component.fca_enable_allgather?  mca_scoll_fca_collect  : NULL;
    fca_module->super.scoll_reduce  = mca_scoll_fca_component.fca_enable_allreduce?  mca_scoll_fca_reduce  : NULL;
    fca_module->super.scoll_barrier    = mca_scoll_fca_component.fca_enable_barrier?    mca_scoll_fca_barrier    : NULL;
    fca_module->super.scoll_broadcast      = mca_scoll_fca_component.fca_enable_bcast?      mca_scoll_fca_broadcast      : NULL;
...
exit:
    FCA_VERBOSE(4, "Query FCA module for comm %p size %d rank %d local_peers=%d: priority=%d %s",
            (void *)comm, size, comm->my_pe, local_peers,
