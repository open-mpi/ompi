...
int mca_sshmem_segment_create(map_segment_t *ds_buf, const char *file_name, size_t size, long hint)
{
...
void *mca_sshmem_segment_attach(map_segment_t *ds_buf, sshmem_mkey_t *mkey)
{
    if (!mca_sshmem_base_selected) {
        return NULL;
    }
...
int mca_sshmem_segment_detach(map_segment_t *ds_buf, sshmem_mkey_t *mkey)
{
    if (!mca_sshmem_base_selected) {
        return OSHMEM_ERROR;
    }
...
int mca_sshmem_unlink(map_segment_t *ds_buf)
{
    if (!mca_sshmem_base_selected) {
        return OSHMEM_ERROR;
    }
...
char *oshmem_get_unique_file_name(uint64_t pe)
{
    ...
    snprintf(file_name, OPAL_PATH_MAX, "%s/shmem_job_%u_pe_%llu", mca_sshmem_base_backing_file_dir,
             OMPI_PROC_MY_NAME->jobid, (unsigned long long) pe);
