...
static int segment_create(map_segment_t *ds_buf, const char *file_name, size_t size, long hint);

static void *segment_attach(map_segment_t *ds_buf, sshmem_mkey_t *mkey);

static int segment_detach(map_segment_t *ds_buf, sshmem_mkey_t *mkey);

static int segment_unlink(map_segment_t *ds_buf);
...
mca_sshmem_mmap_module_t mca_sshmem_mmap_module = {
    /* super */
    {module_init, segment_create, segment_attach, segment_detach, segment_unlink, module_finalize}};
...
static int segment_create(map_segment_t *ds_buf, const char *file_name, size_t size, long hint)
{
    ...
    addr = mmap((void *) mca_sshmem_base_start_address, size, PROT_READ | PROT_WRITE,
                MAP_PRIVATE |
    ...
    if (MAP_FAILED == addr) {
        opal_show_help("help-oshmem-sshmem.txt", "create segment failure", true, "mmap",
                       ompi_process_info.nodename, (unsigned long long) size, strerror(errno),
                       errno);
        opal_show_help("help-oshmem-sshmem-mmap.txt", "mmap:create segment failure", true);
    ...
    OPAL_OUTPUT_VERBOSE((70, oshmem_sshmem_base_framework.framework_output,
                         "%s: %s: create %s "
                         "(id: %d, addr: %p size: %lu)\n",
                         ...
                         (rc ? "failure" : "successful"), ds_buf->seg_id, ds_buf->super.va_base,
...
static void *segment_attach(map_segment_t *ds_buf, sshmem_mkey_t *mkey)
{
    ...
    if (mca_sshmem_mmap_component.is_anonymous) {
         ...
         */
        addr = mmap((void *) mca_sshmem_base_start_address, ds_buf->seg_size,
        ...
        if (NULL == (file_name = oshmem_get_unique_file_name(mkey->u.key))) {
            OPAL_OUTPUT((oshmem_sshmem_base_framework.framework_output, "Can't get file name"));
            return NULL;
        }

        int fd;
        if (-1 == (fd = open(file_name, O_CREAT | O_RDWR, 0600))) {
            OPAL_OUTPUT((oshmem_sshmem_base_framework.framework_output, "file open failed: %s",
                         strerror(errno)));
            free(file_name);
            return NULL;
        }
        ...
        addr = mmap((void *) NULL, ds_buf->seg_size, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);

        if (0 != close(fd)) {
            OPAL_OUTPUT((oshmem_sshmem_base_framework.framework_output, "file close failed: %s",
                         strerror(errno)));
        }
    }
