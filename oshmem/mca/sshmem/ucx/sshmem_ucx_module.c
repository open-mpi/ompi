...
static int sshmem_ucx_memheap_realloc(map_segment_t *s, size_t size, void *old_ptr, void **new_ptr);
...
static segment_allocator_t sshmem_ucx_allocator = {.sa_realloc = sshmem_ucx_memheap_realloc,
                                                   .sa_free = sshmem_ucx_memheap_free};

static int segment_create_internal(map_segment_t *ds_buf, void *address, size_t size,
...
#if HAVE_UCX_DEVICE_MEM
static uct_ib_device_mem_h alloc_device_mem(mca_spml_ucx_t *spml, size_t size, void **address_p)
{
...
static int segment_create(map_segment_t *ds_buf, const char *file_name, size_t size, long hint)
{
    ...
    if (hint & SHMEM_HINT_DEVICE_NIC_MEM) {
        ...
        uct_ib_device_mem_h dev_mem = alloc_device_mem(spml, size, &dev_mem_address);
        if (dev_mem != NULL) {
            int ret;
            ret = segment_create_internal(ds_buf, dev_mem_address, size, 0, hint, dev_mem);
    ...
    if (hint) {
        ...
        return segment_create_internal(ds_buf, mca_sshmem_base_start_address, size,
                                       flags | UCP_MEM_MAP_FIXED, hint, NULL);
    }
}
...
static void *segment_attach(map_segment_t *ds_buf, sshmem_mkey_t *mkey)
{
    ...
    OPAL_OUTPUT((oshmem_sshmem_base_framework.framework_output, "can not attach to ucx segment"));
...
static int sshmem_ucx_memheap_realloc(map_segment_t *s, size_t size, void *old_ptr, void **new_ptr)
{
    ...
    if (!old_ptr) {
        ...
        res = sshmem_ucx_shadow_realloc(ctx->shadow_allocator, alloc_count, old_index, &index,
                                        &inplace);
    }
...
static int sshmem_ucx_memheap_free(map_segment_t *s, void *ptr)
{
    ...
    return sshmem_ucx_shadow_free(ctx->shadow_allocator, sshmem_ucx_memheap_ptr2index(s, ptr));
}
