...
static int segment_create(map_segment_t *ds_buf, const char *file_name, size_t size, long hint)
{
    ...
    size = ((size + sshmem_sysv_gethugepagesize() - 1) / sshmem_sysv_gethugepagesize())
...
retry_alloc:
    ...
    if (shmid == MAP_SEGMENT_SHM_INVALID) {
        ...
        opal_show_help("help-oshmem-sshmem.txt", "create segment failure", true, "sysv",
                       ompi_process_info.nodename, (unsigned long long) size, strerror(errno),
                       errno);
        opal_show_help("help-oshmem-sshmem-sysv.txt", "sysv:create segment failure", true);
        return OSHMEM_ERROR;
    }
...
static int segment_detach(map_segment_t *ds_buf, sshmem_mkey_t *mkey)
{
    ...
    if (mca_sshmem_sysv_component.use_hp != 0) {
        /**
         *  Workaround kernel panic when detaching huge pages from user space simultanously from
         * several processes dont detach here instead let kernel do it during process cleanup
         */
