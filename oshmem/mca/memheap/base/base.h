...
#define MEMHEAP_BASE_PRIVATE_SIZE \
    (1ULL << MEMHEAP_BASE_PAGE_ORDER) /* should be at least the same as a huge page size */
#define MEMHEAP_BASE_MIN_SIZE \
    (1ULL << MEMHEAP_BASE_PAGE_ORDER) /* must fit into at least one huge page */
...
OSHMEM_DECLSPEC sshmem_mkey_t *mca_memheap_base_get_mkey(void *va, int tr_id);
OSHMEM_DECLSPEC sshmem_mkey_t *mca_memheap_base_get_cached_mkey_slow(shmem_ctx_t ctx,
                                                                     map_segment_t *s, int pe,
                                                                     void *va, int btl_id,
                                                                     void **rva);
...
#define MEMHEAP_ERROR(...)                                                              \
    oshmem_output(oshmem_memheap_base_framework.framework_output, "Error %s:%d - %s()", \
                  __SPML_FILE__, __LINE__, __func__, __VA_ARGS__)
...
static inline map_base_segment_t *map_segment_find_va(map_base_segment_t *segs, size_t elem_size,
                                                      void *va)
{
...
static inline map_segment_t *memheap_find_va(void *va)
{
    ...
    if (s) {
        MEMHEAP_VERBOSE(5, "match seg#%02ld: 0x%llX - 0x%llX %llu bytes va=%p",
                        s - memheap_map->mem_segs, (long long) s->super.va_base,
                        (long long) s->super.va_end,
                        (long long) (s->super.va_end - s->super.va_base), (void *) va);
    }
#endif
    return s;
}

static inline sshmem_mkey_t *mca_memheap_base_get_cached_mkey(shmem_ctx_t ctx, int pe, void *va,
                                                              int btl_id, void **rva)
{
    map_segment_t *s;
    sshmem_mkey_t *mkey;
    ...
    if (OPAL_LIKELY(s->mkeys_cache[pe])) {
        ...
        *rva = memheap_va2rva(va, s->super.va_base, mkey->va_base);
        MEMHEAP_VERBOSE_FASTPATH(10, "rkey: pe=%d va=%p -> (cached) %lx %p", pe, (void *) va,
                                 mkey->u.key, (void *) *rva);
        return mkey;
    }
