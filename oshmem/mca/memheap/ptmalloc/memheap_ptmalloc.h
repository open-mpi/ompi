...
/*
 * At the moment we use only dlmalloc part of the ptmalloc3. Thread safety is implemented by using
 * locks on alloc operations. Since all shmem alloc ops are collectives, malloc performance is not a
 * problem. So it makes sense to use simpler algorithm.
 *
 * Heap is allocate in one chunk, and we implement our on sbrk like function that serves portions of
 * the memory to malloc.
 *
