...
mca_memheap_buddy_module_t memheap_buddy = {
    {&mca_memheap_buddy_component, mca_memheap_buddy_finalize, mca_memheap_buddy_alloc,
     mca_memheap_buddy_align, mca_memheap_buddy_realloc, mca_memheap_buddy_free,

     mca_memheap_buddy_private_alloc, mca_memheap_buddy_private_free,
...
int mca_memheap_buddy_module_init(memheap_context_t *context)
{
    ...
    if (context->user_size != (1ULL << memheap_buddy.heap.max_order)) {
        MEMHEAP_VERBOSE(1,
                        "Memheap rounded to the nearest power of two: requested %llu bytes, "
                        "allocated %llu bytes",
                        (unsigned long long) context->user_size,
...
static int buddy_init(mca_memheap_buddy_module_t *buddy)
{
    ...
    buddy->private_heap.bits = (unsigned long **) calloc((buddy->private_heap.max_order + 1),
                                                         sizeof(unsigned long *));
    buddy->heap.num_free = (unsigned int *) calloc((buddy->heap.max_order + 1),
                                                   sizeof(unsigned int));
    buddy->private_heap.num_free = (unsigned int *) calloc((buddy->private_heap.max_order + 1),
                                                           sizeof(unsigned int));
    ...
    for (i = buddy->heap.min_order; i <= buddy->heap.max_order; ++i) {
        ...
        MEMHEAP_VERBOSE(20, "%d: (order=%d) allocating %llu longs (sizeof long = %d)", i,
                        buddy->heap.max_order, s, (int) sizeof(unsigned long));
    ...
    for (i = buddy->private_heap.min_order; i <= buddy->private_heap.max_order; ++i) {
        ...
        MEMHEAP_VERBOSE(20, "%d: (order=%d) allocating %llu longs (sizeof long = %d)", i,
                        buddy->private_heap.max_order, s, (int) sizeof(unsigned long));
        total_size += s * sizeof(unsigned long);
    ...
    for (o = order; o <= heap->max_order; ++o) {
        if (heap->num_free[o]) {
            ...
            *seg = find_first_bit(heap->bits[o], m);
            MEMHEAP_VERBOSE(20, "found free bit: order=%d, bits=0x%lx m=%d, *seg=%d", o,
                            heap->bits[o][0], m, *seg);
            if (*seg < m)
                goto found;
        }
...
static int _buddy_free(mca_memheap_buddy_module_t *buddy, uint32_t seg, unsigned order,
...
static int _do_alloc(uint32_t order, void **p_buff, mca_memheap_buddy_heap_t *heap)
{
    ...
    *p_buff = 0;
    ...
    rc = opal_hash_table_set_value_uint64(heap->symmetric_heap_hashtable, addr,
...
int mca_memheap_buddy_private_free(void *ptr)
{
    ...
    rc = opal_hash_table_get_value_uint64(memheap_buddy.private_heap.symmetric_heap_hashtable, addr,
                                          &order);
    if (OPAL_SUCCESS != rc) {
        return OSHMEM_ERROR;
    }
...
int mca_memheap_buddy_realloc(size_t new_size, void *p_buff, void **p_new_buff)
{
    ...
    rc = opal_hash_table_get_value_uint64(memheap_buddy.heap.symmetric_heap_hashtable, addr,
                                          &order);
    if (OPAL_SUCCESS != rc) {
        *p_new_buff = NULL;
        return OSHMEM_ERROR;
