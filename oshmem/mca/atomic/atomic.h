...
#define DO_SHMEM_TYPE_OP(ctx, type_name, type, op, target, value, pe)    \
    do {                                                                 \
        ...
        RUNTIME_CHECK_ADDR(target);                                      \
                                                                         \
        size = sizeof(value);                                            \
        rc = MCA_ATOMIC_CALL(op(ctx, (void *) target, value, size, pe)); \
        RUNTIME_CHECK_RC(rc);                                            \
                                                                         \
        return;                                                          \
    } while (0)
...
#define OSHMEM_TYPE_OP(type_name, type, prefix, op)                                   \
    ...
    {                                                                                 \
        DO_SHMEM_TYPE_OP(oshmem_ctx_default, type_name, type, op, target, value, pe); \
    }
...
#define OSHMEM_CTX_TYPE_OP(type_name, type, prefix, op)                                            \
    ...
    {                                                                                              \
        DO_SHMEM_TYPE_OP(ctx, type_name, type, op, target, value, pe);                             \
    }

#define DO_OSHMEM_TYPE_FOP(ctx, type_name, type, op, target, value, pe)                          \
    do {                                                                                         \
        int rc = OSHMEM_SUCCESS;                                                                 \
        size_t size = 0;                                                                         \
        type out_value;                                                                          \
        ...
        rc = MCA_ATOMIC_CALL(f##op(ctx, (void *) target, (void *) &out_value, value, size, pe)); \
        RUNTIME_CHECK_RC(rc);                                                                    \
                                                                                                 \
        return out_value;                                                                        \
    } while (0)
...
#define OSHMEM_TYPE_FOP(type_name, type, prefix, op)                                    \
    ...
    {                                                                                   \
        DO_OSHMEM_TYPE_FOP(oshmem_ctx_default, type_name, type, op, target, value, pe); \
    }

#define OSHMEM_CTX_TYPE_FOP(type_name, type, prefix, op)                                         \
    type prefix##_ctx_##type_name##_atomic_fetch_##op(shmem_ctx_t ctx, type *target, type value, \
                                                      int pe)                                    \
    {                                                                                            \
        DO_OSHMEM_TYPE_FOP(ctx, type_name, type, op, target, value, pe);                         \
    }
...
struct mca_atomic_base_module_1_0_0_t {
    ...
    int (*atomic_add)(shmem_ctx_t ctx, void *target, uint64_t value, size_t size, int pe);
    int (*atomic_and)(shmem_ctx_t ctx, void *target, uint64_t value, size_t size, int pe);
    int (*atomic_or)(shmem_ctx_t ctx, void *target, uint64_t value, size_t size, int pe);
    int (*atomic_xor)(shmem_ctx_t ctx, void *target, uint64_t value, size_t size, int pe);
    int (*atomic_fadd)(shmem_ctx_t ctx, void *target, void *prev, uint64_t value, size_t size,
                       int pe);
    int (*atomic_fand)(shmem_ctx_t ctx, void *target, void *prev, uint64_t value, size_t size,
                       int pe);
    int (*atomic_for)(shmem_ctx_t ctx, void *target, void *prev, uint64_t value, size_t size,
                      int pe);
    int (*atomic_fxor)(shmem_ctx_t ctx, void *target, void *prev, uint64_t value, size_t size,
                       int pe);
    int (*atomic_swap)(shmem_ctx_t ctx, void *target, void *prev, uint64_t value, size_t size,
                       int pe);
    int (*atomic_cswap)(shmem_ctx_t ctx, void *target,
                        uint64_t *prev, /* prev is used internally by wrapper, we may
                                           always use 64-bit value */
                        uint64_t cond, uint64_t value, size_t size, int pe);
};
 ...
 */
#define MCA_ATOMIC_BASE_VERSION_2_0_0 OSHMEM_MCA_BASE_VERSION_2_1_0("atomic", 1, 0, 0)
