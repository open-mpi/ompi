...
mca_atomic_base_component_t mca_atomic_ucx_component = {
    ...
    {MCA_ATOMIC_BASE_VERSION_2_0_0,
     ...
     "ucx", OSHMEM_MAJOR_VERSION, OSHMEM_MINOR_VERSION, OSHMEM_RELEASE_VERSION,
...
static int ucx_register(void)
{
    ...
    mca_base_component_var_register(&mca_atomic_ucx_component.atomic_version, "priority",
                                    "Priority of the atomic:ucx "
...
static int ucx_open(void)
{
    ...
    if (strcmp(mca_spml_base_selected_component.spmlm_version.mca_component_name, "ucx")) {
        ATOMIC_VERBOSE(5, "Can not use atomic/ucx because spml ucx component disabled");
...
OBJ_CLASS_INSTANCE(mca_atomic_ucx_module_t, mca_atomic_base_module_t, NULL, NULL);
