...
enum { ATOMIC_LOCK_IDLE = 0, ATOMIC_LOCK_WAITING = 1, ATOMIC_LOCK_ACTIVE = 2 };
            ...
            *atomic_lock_turn = 0;
            if (rc == OSHMEM_SUCCESS) {
                ...
                if (!local_lock_sync || !local_lock_turn) {
                    ...
                    memcpy((void *) local_lock_sync, (void *) atomic_lock_sync,
                           sizeof(char) * num_pe);
...
static inline int mca_atomic_basic_fop(shmem_ctx_t ctx, void *target, void *prev, uint64_t value,
                                       size_t size, int pe, struct oshmem_op_t *op)
{
    int rc = OSHMEM_SUCCESS;
    ...
    op->o_func.c_fn((void *) value, (void *) &temp_value, size / op->dt_size);
...
static inline int mca_atomic_basic_op(shmem_ctx_t ctx, void *target, uint64_t value, size_t size,
                                      int pe, struct oshmem_op_t *op)
{
    long long prev;
...
static int mca_atomic_basic_add(shmem_ctx_t ctx, void *target, uint64_t value, size_t size, int pe)
{
    return mca_atomic_basic_op(ctx, target, value, size, pe,
                               MCA_BASIC_OP(size, oshmem_op_sum_int32, oshmem_op_sum_int64));
}

static int mca_atomic_basic_and(shmem_ctx_t ctx, void *target, uint64_t value, size_t size, int pe)
{
    return mca_atomic_basic_op(ctx, target, value, size, pe,
                               MCA_BASIC_OP(size, oshmem_op_sum_int32, oshmem_op_and_int64));
}

static int mca_atomic_basic_or(shmem_ctx_t ctx, void *target, uint64_t value, size_t size, int pe)
{
    return mca_atomic_basic_op(ctx, target, value, size, pe,
                               MCA_BASIC_OP(size, oshmem_op_sum_int32, oshmem_op_or_int64));
}

static int mca_atomic_basic_xor(shmem_ctx_t ctx, void *target, uint64_t value, size_t size, int pe)
{
    return mca_atomic_basic_op(ctx, target, value, size, pe,
...
static int mca_atomic_basic_fand(shmem_ctx_t ctx, void *target, void *prev, uint64_t value,
                                 size_t size, int pe)
{
    return mca_atomic_basic_fop(ctx, target, prev, value, size, pe,
                                MCA_BASIC_OP(size, oshmem_op_sum_int32, oshmem_op_and_int64));
...
void atomic_basic_lock(shmem_ctx_t ctx, int pe)
{
    ...
    do {
        ...
        do {
            MCA_SPML_CALL(put(ctx, (void *) (atomic_lock_sync + me), sizeof(lock_required),
                              (void *) &lock_required, root_pe));
            MCA_SPML_CALL(get(ctx, (void *) atomic_lock_sync, num_pe * sizeof(*atomic_lock_sync),
                              (void *) local_lock_sync, root_pe));
        ...
        while (index != me) {
            if (local_lock_sync[index] != ATOMIC_LOCK_IDLE) {
                MCA_SPML_CALL(
                    get(ctx, (void *) atomic_lock_turn, sizeof(index), (void *) &index, root_pe));
                MCA_SPML_CALL(get(ctx, (void *) atomic_lock_sync,
                                  num_pe * sizeof(*atomic_lock_sync), (void *) local_lock_sync,
                                  root_pe));
            } else {
        ...
        do {
            MCA_SPML_CALL(put(ctx, (void *) (atomic_lock_sync + me), sizeof(lock_active),
                              (void *) &lock_active, root_pe));
            MCA_SPML_CALL(get(ctx, (void *) atomic_lock_sync, num_pe * sizeof(*atomic_lock_sync),
                              (void *) local_lock_sync, root_pe));
        ...
        MCA_SPML_CALL(get(ctx, (void *) atomic_lock_turn, sizeof(*atomic_lock_turn),
                          (void *) local_lock_turn, root_pe));
    } while (!(
        (index >= num_pe)
...
void atomic_basic_unlock(shmem_ctx_t ctx, int pe)
{
    ...
    do {
        MCA_SPML_CALL(put(ctx, (void *) (atomic_lock_sync + me), sizeof(lock_idle),
                          (void *) &lock_idle, root_pe));
        MCA_SPML_CALL(get(ctx, (void *) atomic_lock_sync, num_pe * sizeof(*atomic_lock_sync),
                          (void *) local_lock_sync, root_pe));
