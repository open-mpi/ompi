...
{
    mca_spml_yoda_get_request_t* getreq = *(mca_spml_yoda_get_request_t**)request;
...
static int mca_spml_yoda_get_request_cancel(struct oshmem_request_t* request, int complete)
{
...
static void mca_spml_yoda_get_request_construct(mca_spml_yoda_get_request_t* req)
{
    ...
    req->req_get.req_base.req_oshmem.req_cancel = mca_spml_yoda_get_request_cancel;
}
...
void mca_spml_yoda_get_completion( mca_btl_base_module_t* btl,
         ...
         int status )
 {
     mca_spml_yoda_rdma_frag_t* frag = (mca_spml_yoda_rdma_frag_t*)des->des_cbdata;
     mca_spml_yoda_get_request_t* getreq = (mca_spml_yoda_get_request_t*)frag->rdma_req;
     ...
     if (getreq->p_dst) {
         memcpy(getreq->p_dst, des->des_dst->seg_addr.pval, des->des_dst->seg_len);
     }

     /* restore descriptor */
     if (frag->btl_seg) {
         des->des_dst = frag->btl_seg;
     }
     ...
     /* swap back source and dest segments */
