 ...
 */
OSHMEM_DECLSPEC int mca_spml_base_select(bool enable_progress_threads, bool enable_threads);
 ...
 */
OSHMEM_DECLSPEC int mca_spml_base_spml_check_selected(const char *my_spml, ompi_proc_t **procs,
                                                      size_t nprocs);
...
OSHMEM_DECLSPEC int mca_spml_base_test(void *addr, int cmp, void *value, int datatype,
                                       int *out_value);
OSHMEM_DECLSPEC int mca_spml_base_oob_get_mkeys(shmem_ctx_t ctx, int pe, uint32_t seg,
                                                sshmem_mkey_t *mkeys);

OSHMEM_DECLSPEC void mca_spml_base_rmkey_unpack(shmem_ctx_t ctx, sshmem_mkey_t *mkey, uint32_t seg,
                                                int pe, int tr_id);
...
OSHMEM_DECLSPEC int mca_spml_base_put_nb(void *dst_addr, size_t size, void *src_addr, int dst,
                                         void **handle);
OSHMEM_DECLSPEC int mca_spml_base_get_nb(void *dst_addr, size_t size, void *src_addr, int src,
                                         void **handle);

OSHMEM_DECLSPEC void mca_spml_base_memuse_hook(void *addr, size_t length);

OSHMEM_DECLSPEC int mca_spml_base_put_all_nb(void *target, const void *source, size_t size,
                                             long *counter);
...
#    define SPML_VERBOSE(level, ...)                                                              \
        oshmem_output_verbose(level, oshmem_spml_base_framework.framework_output, "%s:%d - %s()", \
...
#define SPML_ERROR(...)                                                              \
    oshmem_output(oshmem_spml_base_framework.framework_output, "Error %s:%d - %s()", \
                  __SPML_FILE__, __LINE__, __func__, __VA_ARGS__)

#define SPML_WARNING(...)                                                                         \
    oshmem_output_verbose(0, oshmem_spml_base_framework.framework_output, "Warning %s:%d - %s()", \
                          __SPML_FILE__, __LINE__, __func__, __VA_ARGS__)
