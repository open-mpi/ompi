...
int mca_spml_base_select(bool enable_progress_threads, bool enable_mpi_threads)
{
    ...
    OPAL_LIST_FOREACH (cli, &oshmem_spml_base_framework.framework_components,
        ...
        for (i = 0; i < opal_pointer_array_get_size(&mca_spml_base_spml); i++) {
            ...
            if (0
                == strncmp(component->spmlm_version.mca_component_name, tmp_val,
                           strlen(component->spmlm_version.mca_component_name))) {
                found_spml = true;
                break;
            }
        }
        ...
        module = component->spmlm_init(&priority, enable_progress_threads, enable_mpi_threads);
        if (NULL == module) {
    ...
    if (NULL == best_component) {
        opal_show_help("help-oshmem-memheap.txt", "find-available:none-found", true, "spml");
        for (i = 0; i < opal_pointer_array_get_size(&mca_spml_base_spml); i++) {
            char *tmp_val = NULL;
            tmp_val = (char *) opal_pointer_array_get_item(&mca_spml_base_spml, i);
            if (NULL == tmp_val) {
        ...
        if (0 == i) {
            ompi_rte_abort(2, "No spml component available.  This shouldn't happen.");
        }
    }
