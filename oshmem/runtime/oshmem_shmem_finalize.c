...
int oshmem_shmem_finalize(void)
{
        ...
        !oshmem_shmem_aborted) 
    {
        ...
        if ((OSHMEM_SUCCESS == ret) && ompi_mpi_initialized && !ompi_mpi_finalized)
        {
...
static int __shmem_finalize(void)
{
    ...
    if (OSHMEM_SUCCESS != (ret = mca_atomic_base_close())) {
        return ret;
    }

    if (OSHMEM_SUCCESS != (ret = mca_scoll_base_close())) {
        return ret;
    }

    /* Close Symmetric Heap */
    if(OSHMEM_SUCCESS != (ret = mca_memheap_base_close()))
    {
        return ret;
    }

    if(OSHMEM_SUCCESS != (ret = MCA_SPML_CALL(del_procs(oshmem_group_all->proc_array, oshmem_group_all->proc_count))))
    {
        return ret;
    }
    ...
    if (OSHMEM_SUCCESS != (ret = mca_spml_base_close())) {
        return ret;
    }
