...
int
oshmem_shmem_abort(int errcode)
{
    ...
    orte_show_help("help-shmem-api.txt", "shmem-abort", true,
            ORTE_PROC_MY_NAME->vpid, pid, host, errcode);
    ...
    if (!orte_initialized || !oshmem_shmem_initialized) {
        if (orte_show_help_is_available()) {
            ...
            fprintf(stderr, "[%s:%d] Local abort completed successfully; not able to aggregate error messages, and not able to guarantee that all other processes were killed!\n",
                    host, (int) pid);
        }
