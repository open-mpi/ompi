...
int pmix_server_finalized_fn(opal_process_name_t *proc, void* server_object,
                             opal_pmix_op_cbfunc_t cbfunc, void *cbdata)
{
    ...
    for (i=0; i < jdata->procs->size; i++) {
        if (NULL == (p = (orte_proc_t*)opal_pointer_array_get_item(jdata->procs, i))) {
            continue;
        }
        if (proc->jobid != p->name.jobid) {
            continue;
        }
        if (proc->vpid == p->name.vpid) {
            /* mark the process as having finalized */
