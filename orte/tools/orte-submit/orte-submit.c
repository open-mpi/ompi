                      ...
                      bool *made_app, char ***app_env)
{
    ...
    *made_app = false;
    ...
    /* did they ask us to report bindings? */
    if (myglobals.report_bindings) {
        /* set the MCA param in the app's environment */
        opal_setenv("OMPI_MCA_hwloc_base_report_bindings", "1", true, &app->env);
    }

    /* Capture any preload flags */
    if (myglobals.preload_binaries) {
        app->preload_binary = myglobals.preload_binaries;
    }
    ...
    if (NULL != myglobals.preload_files) {
        app->preload_files = strdup(myglobals.preload_files);
    }
