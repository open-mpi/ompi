 ...
 */
orte_sds_base_component_t mca_sds_slurm_component = {
    {
        ...
        orte_sds_slurm_component_open,
        orte_sds_slurm_component_close
    },
    ...
    orte_sds_slurm_component_init
};


int
orte_sds_slurm_component_open(void)
{
    return ORTE_SUCCESS;
}
...
orte_sds_slurm_component_init(int *priority)
{
    int id;
    char *slurm_nodeid, *mode;

    /* okay, not seed/singleton - see if we're running under slurm */
    slurm_nodeid = getenv("SLURM_NODEID");
    if (NULL == slurm_nodeid) {
        return NULL;
    }
...
int
orte_sds_slurm_component_close(void)
{
    return ORTE_SUCCESS;
}
