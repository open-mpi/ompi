...
static int rte_init(void)
{
    ...
    OPAL_MODEX_RECV_VALUE(ret, OPAL_PMIX_JOBID, NULL, &val, OPAL_STRING);
    if (OPAL_SUCCESS != ret) {
        error = "getting jobid";
        goto error;
    }
    ORTE_PROC_MY_NAME->jobid = strtoul(val, NULL, 10);
    free(val);

    /* get our global rank from PMI */
    OPAL_MODEX_RECV_VALUE(ret, OPAL_PMIX_RANK, NULL, &u32ptr, OPAL_INT);
    if (OPAL_SUCCESS != ret) {
        error = "getting rank";
        goto error;
    }
    ORTE_PROC_MY_NAME->vpid = u32;
