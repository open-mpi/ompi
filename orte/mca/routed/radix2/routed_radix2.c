...
static int radix2_ft_event(int state);
#endif

orte_routed_module_t orte_routed_radix2_module = {
    init,
    finalize,
    delete_route,
    update_route,
...
static int delete_route(orte_process_name_t *proc)
{
    ...
    OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                         "%s routed_radix2_delete_route for %s",
...
{ 
    ...
    if (ORTE_JOB_FAMILY(target->jobid) != ORTE_JOB_FAMILY(ORTE_PROC_MY_NAME->jobid)) {
        ...
        OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                             "%s routed_radix2_update: diff job family routing job %s --> %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
        ...
        for (i=0; i < orte_routed_jobfams.size; i++) {
            ...
            if (jfam->job_family == jfamily) {
                OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                                     "%s routed_radix2: updating route to %s via %s",
                                     ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
        ...
        OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                             "%s routed_radix2: adding route to %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                             ORTE_JOB_FAMILY_PRINT(target->jobid)));
...
static int init_routes(orte_jobid_t job, opal_buffer_t *ndat)
{
    /* the radix2 module routes all proc communications through
    ...
    if (ORTE_PROC_IS_DAEMON) {
        ...
        OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                             "%s routed_radix2: init routes for daemon job %s\n\thnp_uri %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
        ...
        OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                             "%s routed_radix2: completed init routes",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
    ...
    if (ORTE_PROC_IS_HNP) {
        ...
        OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                             "%s routed_radix2: init routes for HNP job %s",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                             ORTE_JOBID_PRINT(job)));
    ...
    {  /* MUST BE A PROC */
        ...
        if (NULL != ndat) {
            ...
            OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                                 "%s routed_radix2: init routes w/non-NULL data",
                                 ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
            ...
            if (ORTE_JOB_FAMILY(ORTE_PROC_MY_NAME->jobid) != ORTE_JOB_FAMILY(job)) {
                 ...
                 */
                OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                                     "%s routed_radix2_init_routes: diff job family - sending update to %s",
                                     ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                ...
                OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                                     "%s routed_radix2_init_routes: ack recvd",
                                     ORTE_NAME_PRINT(ORTE_PROC_MY_NAME)));
        ...
        OPAL_OUTPUT_VERBOSE((1, orte_routed_base_output,
                             "%s routed_radix2: init routes for proc job %s\n\thnp_uri %s\n\tdaemon uri %s",
        ...
        if (NULL == orte_process_info.my_daemon_uri) {
            ...
            opal_output(0, "%s ERROR: Failed to identify the local daemon's URI",
                        ORTE_NAME_PRINT(ORTE_PROC_MY_NAME));
            opal_output(0, "%s ERROR: This is a fatal condition when the radix2 router",
                        ORTE_NAME_PRINT(ORTE_PROC_MY_NAME));
...
static int route_lost(const orte_process_name_t *route)
{
    ...
    if ((ORTE_JOB_FAMILY(route->jobid) != ORTE_JOB_FAMILY(ORTE_PROC_MY_NAME->jobid)) &&
        ...
        for (i=0; i < orte_routed_jobfams.size; i++) {
            ...
            if (jfam->job_family == jfamily) {
                OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                                     "%s routed_radix2: route to %s lost",
                                     ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
    ...
    if (!orte_finalizing &&
        ...
        OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                             "%s routed:radix2: Connection to lifeline %s lost",
                             ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
    ...
    if (ORTE_JOB_FAMILY(target->jobid) != ORTE_JOB_FAMILY(ORTE_PROC_MY_NAME->jobid)) {
        if (ORTE_PROC_IS_HNP) {
            ...
            for (i=0; i < orte_routed_jobfams.size; i++) {
                ...
                if (jfam->job_family == jfamily) {
                    OPAL_OUTPUT_VERBOSE((2, orte_routed_base_output,
                                         "%s routed_radix2: route to %s is defined",
                                         ORTE_NAME_PRINT(ORTE_PROC_MY_NAME),
                                         ORTE_JOB_FAMILY_PRINT(target->jobid)));
                    return true;
                }
...
static void radix2_tree(int rank, int *num_children,
                       opal_list_t *children, opal_bitmap_t *relatives)
{
    ...
    while ( Sum < (rank+1) ) {
        NInLevel *= mca_routed_radix2_component.radix;
        Sum += NInLevel;
    }
    
    /* our children start at our rank + num_in_level */
    peer = rank + NInLevel;
    for (i = 0; i < mca_routed_radix2_component.radix; i++) {
        if (peer < (int)orte_process_info.num_procs) {
            ...
            radix2_tree(peer, NULL, NULL, relations);
        }
        peer += NInLevel;
    }
}
...
static void update_routing_plan(void)
{
    ...
    NInPrevLevel = NInLevel/mca_routed_radix2_component.radix;
     ...
     */
    radix2_tree(Ii, &num_children, &my_children, NULL);
