...
static int init(void)
{
    ...
    if (VM_HOLE_NONE != mca_rtc_hwloc_component.kind &&
        ...
        /* create a fake shmem file in our session dir */
        asprintf(&hwlocfile, "%s/hwloc.sm", orte_process_info.jobfam_session_dir);
        asprintf(&cmd, "dd if=/dev/zero of=%s bs=%luM count=1 >& /dev/null",
                 hwlocfile,
                 (unsigned long)mca_rtc_hwloc_component.size/(1024*1024));
        system(cmd);
        free(cmd);
    }
...
static void assign(orte_job_t *jdata)
{
    ...
    opal_output_verbose(2, orte_rtc_base_framework.framework_output,
                        ...
                        (unsigned long)mca_rtc_hwloc_component.addr,
                        (unsigned long)mca_rtc_hwloc_component.size);

    kv = OBJ_NEW(opal_value_t);
    kv->key = strdup(OPAL_PMIX_HWLOC_SHMEM_FILE);
    kv->type = OPAL_STRING;
    kv->data.string = strdup(hwlocfile);
    opal_list_append(cache, &kv->super);

    kv = OBJ_NEW(opal_value_t);
    kv->key = strdup(OPAL_PMIX_HWLOC_SHMEM_ADDR);
    kv->type = OPAL_SIZE;
    kv->data.size = mca_rtc_hwloc_component.addr;
    opal_list_append(cache, &kv->super);

    kv = OBJ_NEW(opal_value_t);
    kv->key = strdup(OPAL_PMIX_HWLOC_SHMEM_SIZE);
    kv->type = OPAL_SIZE;
    kv->data.size = mca_rtc_hwloc_component.size;
    opal_list_append(cache, &kv->super);
}
