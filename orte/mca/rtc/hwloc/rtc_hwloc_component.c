...
static int rtc_hwloc_register(void)
{
    ...
    (void) mca_base_component_var_register(c, "vmhole",
                                           "Kind of VM hole to identify - none, begin, biggest, libs, heap, stack, <address>",
    ...
    if (0 == strcasecmp(vmhole, "none")) {
        ...
        mca_rtc_hwloc_component.addr = strtoull(vmhole, NULL, 0);
    }

    (void) mca_base_component_var_register(c, "vmhole_size",
                                           "Size of hole to find (in MBytes)",
                                            MCA_BASE_VAR_TYPE_INT, NULL, 0, 0,
                                           OPAL_INFO_LVL_9,
                                           MCA_BASE_VAR_SCOPE_READONLY,
                                           &mca_rtc_hwloc_component.size);
    mca_rtc_hwloc_component.size *= 1024*1024;
    return ORTE_SUCCESS;
}
