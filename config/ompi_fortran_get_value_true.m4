...
AC_DEFUN([OMPI_FORTRAN_GET_VALUE_TRUE],[
    ...
    # A config.cache produced by an older Open MPI may contain a cached
    # .TRUE. value but no .FALSE. value (the .FALSE. computation was added
    # later).  In that case the whole cache block above would be skipped
    # and OMPI_FORTRAN_VALUE_FALSE would end up empty, so recompute both.
    if test -n "$ompi_cv_fortran_true_value" && \
       test -z "$ompi_cv_fortran_false_value" ; then
        unset ompi_cv_fortran_true_value
    fi
