...
AC_DEFUN([_OPAL_CONFIG_LIBEVENT_INTERNAL], [
    OPAL_VAR_SCOPE_PUSH(subconfig_happy subconfig_prefix internal_libevent_location libevent_config_extra_args)

    AS_IF([test ! -z $prefix], [subconfig_prefix="--prefix=$prefix"])

    libevent_config_extra_args=

    # On macOS, never let the packaged Libevent use pipe2().
    #
    # Apple's Command Line Tools routinely install an SDK *newer* than the
    # running OS (e.g., the macOS 27 SDK on a macOS 26 machine), and clang
    # defaults the deployment target to the running OS.  pipe2() is new in
    # the macOS 27 SDK, so in that situation the SDK declares pipe2() but
    # the OS does not provide it.  Libevent detects it with an ordinary
    # Autoconf link test, which *succeeds*: the symbol resolves against the
    # SDK stub and, the deployment target being older than the version that
    # introduced it, is recorded as a weak import rather than an error.
    # Libevent then compiles a pipe2() call that dyld binds to NULL, and
    # every MPI process segfaults inside MPI_Init().
    #
    # Autoconf grew a guard for exactly this in 2.73, but the Libevent
    # tarball Open MPI ships -- libevent-2.1.12-stable-ompi, per the
    # $libevent_version constant in autogen.pl -- carries a configure
    # generated by 2.72, so it does not benefit.  Upgrading does not help
    # either: 2.1.13-stable still ships a 2.72-generated configure and
    # still probes pipe2() with an unguarded AC_CHECK_FUNCS.  We must not
    # edit the tarball, so pre-seed its configure cache instead.
    #
    # This is unconditional on macOS rather than conditional on detecting
    # the SDK mismatch, because the thing being given up is worth nothing
    # measurable: pipe2() only ever replaces pipe() plus four fcntl() calls
    # when an internal pipe is *created* (about 0.7us, a handful of times
    # per process, at init).  The resulting fds are identical, so nothing on
    # any message path differs.  A conditional check would cost more in
    # build complexity and rot than the microseconds it could recover.
    #
    # The full analysis -- including why this surfaces as a crash rather
    # than a link error, and why it reaches MPI processes at all -- is in
    # https://github.com/open-mpi/ompi/issues/14462
    AS_CASE([$host_os],
        [darwin*], [libevent_config_extra_args="ac_cv_func_pipe2=no"])
    ...
    PAC_CONFIG_SUBDIR_ARGS([3rd-party/libevent_directory],
       [--disable-dns --disable-http --disable-rpc --disable-openssl --enable-thread-support --disable-evport --disable-gcc-warnings --disable-libevent-regress ${libevent_config_extra_args}],
