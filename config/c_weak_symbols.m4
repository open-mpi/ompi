...
# _OMPI_C_MACRO_WEAK_SYMBOLS(action_if_found, [action_if_not_found])
# ------------------------------------------------------------
AC_DEFUN([_OMPI_C_MACRO_WEAK_SYMBOLS],[
    # need two files because icc will incorrectly not create the
    # symbols if they are not used in the object file in which they
    # are defined.  Blah!
    # To get to compile with icc, have them in a separate header.
...
OPAL_LOG_COMMAND(
    ...
    [OPAL_LOG_COMMAND(
        [$CC $CFLAGS  conftest.c conftest_weak.o -o conftest $LDFLAGS $LIBS],
        [ompi_c_macro_weak_symbols_happy=1],
	[ompi_c_macro_weak_symbols_happy=0])],
    [ompi_c_macro_weak_symbols_happy=0])

    AS_IF([test "$ompi_c_macro_weak_symbols_happy" = "1"], [$1], [$2])

    unset ompi_c_macro_weak_symbols_happy
    rm -f conftest_weak.h conftest_weak.c conftest.c conftest
])


# OMPI_C_MACRO_WEAK_SYMBOLS()
# ---------------------
# sets OMPI_C_MACRO_WEAK_SYMBOLS=1 if C compiler has support for weak symbols
AC_DEFUN([OMPI_C_MACRO_WEAK_SYMBOLS],[
    AC_CACHE_CHECK([for weak symbol support],
                   [ompi_cv_c_macro_weak_symbols],
                   [_OMPI_C_MACRO_WEAK_SYMBOLS([ompi_cv_c_macro_weak_symbols="yes"],
                                               [ompi_cv_c_macro_weak_symbols="no"])])

    AS_IF([test "$ompi_cv_c_macro_weak_symbols" = "yes"],
