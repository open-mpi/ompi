...
- [x] Chunk 12A: Add MPICH cross-test infrastructure and environment
      ...
      with guidance pointing to `ompi/test/mpi-abi/README.md`.
...
- [x] Add `ompi/test/mpi-abi/` with Open MPI-style source headers.
- [x] Add `ompi/test/mpi-abi/Makefile.am`.
- [x] Add `ompi/test/mpi-abi/requirements.txt`.
- [x] Add `ompi/test/mpi-abi/README.md` documenting the test tiers and
      runner variables.
- [x] Add `ompi/test/mpi-abi/Makefile` to the Autoconf-generated file list.
- [x] Add `ompi/test/mpi-abi` to the appropriate `test/Makefile.am`
      `SUBDIRS` / `DIST_SUBDIRS` entries.
- [x] Gate `ompi/test/mpi-abi` build participation on OMPI project support
      where appropriate.
...
- [x] Add a top-level convenience `check-abi-mpich` target that
      dispatches to `ompi/test/mpi-abi`.
...
- [x] Add the Python runner entry point under `ompi/test/mpi-abi/`.
...
- [x] Verify `make check` passes with ABI support enabled.
      Verified both directly in `ompi/test/mpi-abi` and as part of an
...
- [ ] Verify a distribution tarball can run `make check-abi-mpich` when
      ...
      unavailable.  Not yet performed from a distribution tarball; see
      the Chunk 13A summary above (explicit distribution-tarball ABI
