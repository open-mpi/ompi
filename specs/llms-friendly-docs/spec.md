...
`openmpi-runtime-introspection.md` is a concise curated guide telling a consumer how to query an *installed* Open MPI with the `ompi_info` command. It covers:

- detecting the Open MPI version and MPI standard level, and the build configuration (`ompi_info --version`, `ompi_info`, `ompi_info --parsable`);
- listing the MCA components present in this build;
- enumerating the run-time MCA parameters comprehensively with `ompi_info --all --parsable`, and reading the self-describing `mca:<framework>:<component>:param:<name>:<field>:<value>` output;
- setting MCA parameters, with concrete examples for the command line (`mpirun/mpiexec --mca`), the environment (`OMPI_MCA_*`), and parameter files, in precedence order.

It is the corpus's answer to a discoverability gap surfaced in review: a consumer reading the API artifacts has no signal that the MCA surface (and the live build configuration) is queryable at run time. The guide closes that gap by pointing at the authoritative command rather than duplicating its output.

### Why no MCA-parameter snapshot

The same review suggested *maybe* committing an `ompi_info --all --parsable` snapshot as an artifact. This is intentionally **not** done:

- The output is **installation-specific**: it reflects exactly which components a given build compiled in, and is full of machine-specific values (install `prefix`, config command line, compiler paths, host name). A committed snapshot would bake in one machine's reality and be misleading on every other installation.
- It would **immediately drift** from any installation that differs in version, configure options, or available hardware/components.
- It **cannot be produced at docs-build time**: Read the Docs (and a docs-only checkout) builds the documentation without a full Open MPI installation, so there is no `ompi_info` to run. Embedding a snapshot would also violate the "derived from the documentation sources, never independently authored" principle.

The discoverability fix is therefore guidance (run the self-describing command against the live installation), not a catalog or snapshot.
